...
func dbauthzAuthorizationContext(m dsl.Matcher) {
	m.Import("context")
	m.Import("github.com/NeuralInverse/cloud/v2/nicloud/database/dbauthz")
...
func testingWithOwnerUser(m dsl.Matcher) {
	m.Import("testing")
	m.Import("github.com/NeuralInverse/cloud/v2/cli/clitest")
	m.Import("github.com/NeuralInverse/cloud/v2/enterprise/nicloud/coderenttest")
	...
	m.Match(`
		$client, $_ := nicloudenttest.New($t, $*_)
		$*_
		clitest.$SetupConfig($t, $client, $_)
	`).Where(m["t"].Type.Implements("testing.TB") &&
		m["SetupConfig"].Text.Matches("^SetupConfig$") &&
...
// databaseImport enforces not importing any database types into /nicloudsdk.
//
//nolint:unused,deadcode,varnamelen
func databaseImport(m dsl.Matcher) {
	m.Import("github.com/NeuralInverse/cloud/v2/nicloud/database")
	m.Match("database.$_").
		Report("Do not import any database types into nicloudsdk").
		Where(
			m.File().PkgPath.Matches("github.com/NeuralInverse/cloud/v2/nicloudsdk") &&
				!m.File().Name.Matches(`_test\.go$`),
		)
}
...
func publishInTransaction(m dsl.Matcher) {
	m.Import("github.com/NeuralInverse/cloud/v2/nicloud/database/pubsub")
...
func useStandardTimeoutsAndDelaysInTests(m dsl.Matcher) {
	...
	m.Import("github.com/NeuralInverse/cloud/v2/testutil")
...
func HttpAPIErrorMessage(m dsl.Matcher) {
	m.Import("github.com/NeuralInverse/cloud/v2/nicloud/httpapi")
...
func noPTYInAgent(m dsl.Matcher) {
	m.Import("github.com/NeuralInverse/cloud/v2/pty")
	m.Match(
		`pty.Command($*_)`,
