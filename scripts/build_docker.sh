...
# $NEURALINVERSE_IMAGE_BUILD_BASE_TAG for easily forcing a fresh build in CI.
#
# The default base image can be controlled via $NEURALINVERSE_BASE_IMAGE_TAG.
#
...
DEFAULT_BASE="${NEURALINVERSE_BASE_IMAGE_TAG:-ghcr.io/coder/coder-base:latest}"

arch=""
image_tag=""
build_base="${NEURALINVERSE_IMAGE_BUILD_BASE_TAG:-}"
version=""
push=0
...
docker build \
	...
	--build-arg "NEURALINVERSE_VERSION=$version" \
	--no-cache \
	--tag "$image_tag" \
	-f Dockerfile \
	. 1>&2
