...
#
# If $NEURALINVERSE_RELEASE is set to "true", the returned version will equal the
# current git tag. If the current commit is not tagged, this will fail.
#
# If $NEURALINVERSE_RELEASE is not set, the returned version will always be a dev
# version.
...
if [[ -n "${NEURALINVERSE_FORCE_VERSION:-}" ]]; then
	echo "${NEURALINVERSE_FORCE_VERSION}"
	exit 0
fi
...
# SHA.
if [[ "${NEURALINVERSE_RELEASE:-}" == *t* ]]; then
