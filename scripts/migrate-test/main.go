...
func main() {
	...
	expectedSchemaAfter, err := gitShow("nicloud/database/dump.sql", migrateToVersion)
	if err != nil {
		panic(err)
	}
...
func makeMigrateFS(version string) (fs.FS, error) {
	...
	out, err := exec.Command("git", "archive", "--format=zip", version, "nicloud/database/migrations").CombinedOutput()
	if err != nil {
	...
	subbed, err := fs.Sub(zr, "nicloud/database/migrations")
	if err != nil {
