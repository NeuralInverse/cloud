...
if [[ "${NEURALINVERSE_LIBSH_NO_CHECK_DEPENDENCIES:-}" != *t* ]]; then
	...
	if command -v yq4 >/dev/null; then
		export NEURALINVERSE_LIBSH_YQ=yq4
	elif command -v yq >/dev/null; then
		if [[ $(yq --version) == *" v4."* ]]; then
			export NEURALINVERSE_LIBSH_YQ=yq
		fi
	fi
	...
	export NEURALINVERSE_LIBSH_NO_CHECK_DEPENDENCIES=true
fi

# Alias yq to the version we want by shadowing with a function.
if [[ -n ${NEURALINVERSE_LIBSH_YQ:-} ]]; then
	yq() {
		command $NEURALINVERSE_LIBSH_YQ "$@"
	}
fi
