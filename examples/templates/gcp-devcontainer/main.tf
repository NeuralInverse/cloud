...
locals {
  ...
  startup_script = <<-META
    ...
    docker run \
     --rm \
     --net=host \
     -h ${lower(data.ni_workspace.me.name)} \
...
resource "google_compute_instance" "vm" {
  ...
  machine_type = data.ni_parameter.instance_type.value
  # data.ni_workspace_owner.me.name == "default"  is a workaround to suppress error in the terraform plan phase while creating a new workspace.
  desired_status = (data.ni_workspace_owner.me.name == "default" || data.ni_workspace.me.start_count == 1) ? "RUNNING" : "TERMINATED"
