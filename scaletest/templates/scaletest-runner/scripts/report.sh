...
NEURALINVERSE_URL="${NEURALINVERSE_URL%/}"
buildinfo="$(curl -sSL "${NEURALINVERSE_URL}/api/v2/buildinfo")"
...
workspace_json="$(
	command neuralinverse list --all --output json |
		jq --arg workspace "${NEURALINVERSE_WORKSPACE}" --arg user "${NEURALINVERSE_USER}" 'map(select(.name == $workspace) | select(.owner_name == $user)) | .[0]'
)"
...
started)
	...
	header="New scaletest started at \`${created_at}\` by \`${initiator_name}\` on ${NEURALINVERSE_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
completed)
	completed_at=$(date -Iseconds)
	header="Scaletest completed at \`${completed_at}\` (started by \`${initiator_name}\`) on ${NEURALINVERSE_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
failed)
	failed_at=$(date -Iseconds)
	header="Scaletest failed at \`${failed_at}\` (started by \`${initiator_name}\`) on ${NEURALINVERSE_URL} (<${server_version_commit}|\`${server_version}\`>)."
	;;
*)
	echo "Unknown status: ${status}" >&2
	exit 1
...
text_arr=(
	...
	"${bullet} Workspace (runner): ${NEURALINVERSE_URL}/@${owner_name}/${workspace_name}"
