...
func TestRun_OnRetryEnrichesProvider(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatErrorKindRateLimit, records[0].classified.Kind)
...
func TestStreamSilenceGuard_DisarmPreservesPermanentError(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatErrorKindConfig, classified.Kind)
...
func TestRun_RetriesSilenceTimeoutWhileOpeningStream(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatErrorKindStartupTimeout, retries[0].Kind)
...
func TestRun_HTTP2TransportErrorClassifiedAsRetryableTimeout(t *testing.T) {
	...
	for _, provider := range providers {
		t.Run(provider, func(t *testing.T) {
			...
			require.Equal(t, 2, attempts)
			require.Len(t, retries, 1)
			require.Equal(t, nicloudsdk.ChatErrorKindTimeout, retries[0].Kind, "Kind")
...
func TestRun_ExclusiveToolPolicyViolation(t *testing.T) {
	...
	var persistedSteps []PersistedStep
	var publishedToolParts []nicloudsdk.ChatMessagePart
	err := Run(context.Background(), RunOptions{
		Model: model,
		Messages: []fantasy.Message{
		...
		PublishMessagePart: func(role nicloudsdk.ChatMessageRole, part nicloudsdk.ChatMessagePart) {
			if role != nicloudsdk.ChatMessageRoleTool {
				return
			}
