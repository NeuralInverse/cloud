...
func TestNewMetrics_RegistersAllMetrics(t *testing.T) {
	...
	m.StreamRetriesTotal.WithLabelValues("anthropic", "claude-sonnet-4-5", string(nicloudsdk.ChatErrorKindTimeout), "false")
	...
	expected := map[string]dto.MetricType{
		"nicloud_chatd_chats":                       dto.MetricType_GAUGE,
		"nicloud_chatd_message_count":               dto.MetricType_HISTOGRAM,
		"nicloud_chatd_prompt_size_bytes":           dto.MetricType_HISTOGRAM,
		"nicloud_chatd_tool_result_size_bytes":      dto.MetricType_HISTOGRAM,
		"nicloud_chatd_ttft_seconds":                dto.MetricType_HISTOGRAM,
		"nicloud_chatd_compaction_total":            dto.MetricType_COUNTER,
		"nicloud_chatd_steps_total":                 dto.MetricType_COUNTER,
		"nicloud_chatd_stream_retries_total":        dto.MetricType_COUNTER,
		"nicloud_chatd_stream_buffer_dropped_total": dto.MetricType_COUNTER,
		"nicloud_chatd_tool_errors_total":           dto.MetricType_COUNTER,
	}
...
func TestNopMetrics_DoesNotPanic(t *testing.T) {
	...
	m.StreamRetriesTotal.WithLabelValues("anthropic", "claude-sonnet-4-5", string(nicloudsdk.ChatErrorKindTimeout), "false").Inc()
	...
	nilMetrics.RecordStreamRetry("anthropic", "claude-sonnet-4-5", chaterror.ClassifiedError{Kind: nicloudsdk.ChatErrorKindTimeout})
...
func TestRecordCompaction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCount == 0 {
				for _, f := range families {
					assert.NotEqual(t, "nicloud_chatd_compaction_total", f.GetName(),
						"compaction_total should not be recorded")
				}
				return
			}

			requireCounter(t, reg, "nicloud_chatd_compaction_total", float64(tt.wantCount), map[string]string{
...
func TestRecordStreamRetry(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "overloaded", kind: nicloudsdk.ChatErrorKindOverloaded},
		{name: "rate_limit", kind: nicloudsdk.ChatErrorKindRateLimit},
		{name: "timeout", kind: nicloudsdk.ChatErrorKindTimeout},
		{name: "startup_timeout", kind: nicloudsdk.ChatErrorKindStartupTimeout},
		{name: "auth", kind: nicloudsdk.ChatErrorKindAuth},
		{name: "config", kind: nicloudsdk.ChatErrorKindConfig},
		{name: "missing_key", kind: nicloudsdk.ChatErrorKindMissingKey},
		{name: "generic", kind: nicloudsdk.ChatErrorKindGeneric},
		{name: "chain_broken", kind: nicloudsdk.ChatErrorKindGeneric, chainBroken: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			requireCounter(t, reg, "nicloud_chatd_stream_retries_total", 1, map[string]string{
				"provider":     "test-provider",
				"model":        "test-model",
				"kind":         string(tt.kind),
...
func TestRecordStreamBufferDropped(t *testing.T) {
	...
	t.Run("increments monotonically", func(t *testing.T) {
		...
		var found bool
		for _, f := range families {
			if f.GetName() != "nicloud_chatd_stream_buffer_dropped_total" {
				continue
			}
			found = true
...
func TestRecordToolError(t *testing.T) {
	...
	t.Run("increments with correct labels", func(t *testing.T) {
		...
		requireCounter(t, reg, "nicloud_chatd_tool_errors_total", 1, map[string]string{
			"provider":  "test-provider",
			"model":     "test-model",
			"tool_name": "read_file",
		})
...
func TestRun_RecordsMetrics(t *testing.T) {
	...
	for _, f := range families {
		...
		switch f.GetName() {
		case "nicloud_chatd_steps_total":
			require.Len(t, f.GetMetric(), 1)
	...
	assert.True(t, found["nicloud_chatd_steps_total"], "steps_total not recorded")
	assert.True(t, found["nicloud_chatd_message_count"], "message_count not recorded")
	assert.True(t, found["nicloud_chatd_prompt_size_bytes"], "prompt_size_bytes not recorded")
	assert.True(t, found["nicloud_chatd_ttft_seconds"], "ttft_seconds not recorded")
}
...
func TestRun_StreamRetry_RecordsMetric(t *testing.T) {
	...
	assert.Equal(t, nicloudsdk.ChatErrorKindRateLimit, retries[0].classified.Kind)
	...
	requireCounter(t, reg, "nicloud_chatd_stream_retries_total", 1, map[string]string{
		"provider":     "test-provider",
		"model":        "test-model",
		"kind":         string(nicloudsdk.ChatErrorKindRateLimit),
		"chain_broken": "false",
	})
}
...
func TestRun_StreamRetry_CanceledDoesNotIncrement(t *testing.T) {
	...
	for _, f := range families {
		if f.GetName() == "nicloud_chatd_stream_retries_total" {
