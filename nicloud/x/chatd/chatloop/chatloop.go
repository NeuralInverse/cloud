...
func processStepStream(
	ctx context.Context,
	stream fantasy.StreamResponse,
	publishMessagePart func(nicloudsdk.ChatMessageRole, nicloudsdk.ChatMessagePart),
) (stepResult, error) {
	var result stepResult
	...
	for part := range stream {
		...
		case fantasy.StreamPartTypeTextDelta:
			...
			publishMessagePart(nicloudsdk.ChatMessageRoleAssistant, nicloudsdk.ChatMessageText(part.Delta))
		...
		case fantasy.StreamPartTypeReasoningDelta:
			...
			publishMessagePart(nicloudsdk.ChatMessageRoleAssistant, nicloudsdk.ChatMessageReasoning(part.Delta))
		...
		case fantasy.StreamPartTypeToolInputDelta:
			...
			publishMessagePart(nicloudsdk.ChatMessageRoleAssistant, nicloudsdk.ChatMessagePart{
		...
		case fantasy.StreamPartTypeToolCall:
			...
			publishMessagePart(
				nicloudsdk.ChatMessageRoleAssistant,
				ssePart,
			)
		...
		case fantasy.StreamPartTypeToolResult:
			...
			if part.ProviderExecuted {
				...
				ssePart.CreatedAt = &now
				publishMessagePart(
					nicloudsdk.ChatMessageRoleTool,
					ssePart,
				)
			}
...
func executeToolsForStep(
	...
	if exclusiveViolation {
		...
		for _, tr := range policyResults {
			...
			ssePart.CreatedAt = &now
			publishMessagePart(nicloudsdk.ChatMessageRoleTool, ssePart)
		}
		for _, tr := range policyResults {
			result.content = append(result.content, tr)
		}
...
func publishToolAttachments(
	...
	for _, attachment := range attachments {
		filePart := nicloudsdk.ChatMessageFile(
			attachment.FileID,
			attachment.MediaType,
			attachment.Name,
		)
		filePart.CreatedAt = &createdAt
		publishMessagePart(nicloudsdk.ChatMessageRoleAssistant, filePart)
	}
}
