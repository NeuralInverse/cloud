...
func tryCompact(
	...
	if config.PublishMessagePart != nil && config.ToolCallID != "" {
		config.PublishMessagePart(
			...
			nicloudsdk.ChatMessageToolCall(config.ToolCallID, config.ToolName, nil),
		)
	}
	...
	if config.PublishMessagePart != nil && config.ToolCallID != "" {
		...
		config.PublishMessagePart(
			...
			nicloudsdk.ChatMessageToolResult(config.ToolCallID, config.ToolName, resultJSON, false, false),
		)
	}
...
func publishCompactionError(config CompactionOptions, msg string) {
	...
	config.PublishMessagePart(
		nicloudsdk.ChatMessageRoleTool,
		nicloudsdk.ChatMessageToolResult(config.ToolCallID, config.ToolName, errJSON, true, false),
	)
}
