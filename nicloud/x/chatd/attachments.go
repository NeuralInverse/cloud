...
func buildAssistantPartsForPersist(
	...
	parts := make([]nicloudsdk.ChatMessagePart, 0, len(assistantBlocks)+len(toolResults))
	...
	for _, block := range assistantBlocks {
		...
		if part.Type == nicloudsdk.ChatMessagePartTypeToolCall && part.ToolCallID != "" && step.ToolCallCreatedAt != nil {
			if ts, ok := step.ToolCallCreatedAt[part.ToolCallID]; ok {
				part.CreatedAt = &ts
			}
		}
		if part.Type == nicloudsdk.ChatMessagePartTypeToolResult && part.ToolCallID != "" && step.ToolResultCreatedAt != nil {
			if ts, ok := step.ToolResultCreatedAt[part.ToolCallID]; ok {
				part.CreatedAt = &ts
			}
		}
		if part.Type == nicloudsdk.ChatMessagePartTypeReasoning {
	...
	for _, tr := range toolResults {
		...
		for _, attachment := range attachments {
			parts = append(parts, nicloudsdk.ChatMessageFile(
				attachment.FileID,
