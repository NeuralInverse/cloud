...
// dynamicTool wraps a nicloudsdk.DynamicTool as a fantasy.AgentTool.
...
// dynamicToolsFromSDK converts nicloudsdk.DynamicTool definitions
// into fantasy.AgentTool implementations for inclusion in the LLM
// tool list.
func dynamicToolsFromSDK(logger slog.Logger, tools []nicloudsdk.DynamicTool) []fantasy.AgentTool {
	if len(tools) == 0 {
		return nil
	}
