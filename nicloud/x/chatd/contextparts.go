...
func FilterContextParts(
	...
	var filtered []nicloudsdk.ChatMessagePart
...
func CollectContextPartsFromMessages(
	...
	var collected []nicloudsdk.ChatMessagePart
...
func latestContextAgentIDFromParts(parts []nicloudsdk.ChatMessagePart) (uuid.UUID, bool) {
...
// slice unchanged.
func FilterContextPartsToLatestAgent(parts []nicloudsdk.ChatMessagePart) []nicloudsdk.ChatMessagePart {
	latestAgentID, ok := latestContextAgentIDFromParts(parts)
	if !ok {
		return parts
	}

	filtered := make([]nicloudsdk.ChatMessagePart, 0, len(parts))
	for _, part := range parts {
		switch part.Type {
		case nicloudsdk.ChatMessagePartTypeContextFile,
			nicloudsdk.ChatMessagePartTypeSkill:
...
func BuildLastInjectedContext(
	...
	stripped := make([]nicloudsdk.ChatMessagePart, 0, len(filtered))
