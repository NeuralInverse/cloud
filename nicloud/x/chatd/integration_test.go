...
func defaultIntegrationAIProviderBaseURL(providerType nicloudsdk.AIProviderType) string {
	switch providerType {
	case nicloudsdk.AIProviderTypeAnthropic:
		return "https://api.anthropic.com"
	case nicloudsdk.AIProviderTypeOpenAI:
...
func aiProviderDisplayName(providerType nicloudsdk.AIProviderType) string {
	switch providerType {
	case nicloudsdk.AIProviderTypeAnthropic:
		return "Anthropic"
	case nicloudsdk.AIProviderTypeOpenAI:
...
func TestAnthropicWebSearchRoundTrip(t *testing.T) {
	...
	provider := createIntegrationAIProvider(
		ctx, t, expClient, nicloudsdk.AIProviderTypeAnthropic, apiKey, baseURL,
	)
	...
	require.Equal(t, nicloudsdk.ChatStatusWaiting, chatData.Status,
	...
	require.Contains(t, partTypes, nicloudsdk.ChatMessagePartTypeToolCall,
		"assistant message should contain a PE tool-call part")
	require.Contains(t, partTypes, nicloudsdk.ChatMessagePartTypeSource,
		"assistant message should contain source parts for UI citations")
	require.Contains(t, partTypes, nicloudsdk.ChatMessagePartTypeToolResult,
		"assistant message should contain a PE tool-result part")
	require.Contains(t, partTypes, nicloudsdk.ChatMessagePartTypeText,
	...
	require.Equal(t, nicloudsdk.ChatStatusWaiting, chatData2.Status,
...
func findAssistantWithText(t *testing.T, msgs []nicloudsdk.ChatMessage) *nicloudsdk.ChatMessage {
	t.Helper()
	for i := range msgs {
		...
		for _, part := range msgs[i].Content {
			if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text != "" {
				return &msgs[i]
			}
		}
	}
...
// contains a non-empty text part.
func findLastAssistantWithText(t *testing.T, msgs []nicloudsdk.ChatMessage) *nicloudsdk.ChatMessage {
	t.Helper()
...
func logMessages(t *testing.T, msgs []nicloudsdk.ChatMessage) {
	t.Helper()
...
func TestOpenAIReasoningRoundTrip(t *testing.T) {
	...
	provider := createIntegrationAIProvider(
		ctx, t, expClient, nicloudsdk.AIProviderTypeOpenAI, apiKey, baseURL,
	)
	...
	partTypes := partTypeSet(assistantMsg.Content)
	require.Contains(t, partTypes, nicloudsdk.ChatMessagePartTypeReasoning,
...
func partTypeSet(parts []nicloudsdk.ChatMessagePart) map[nicloudsdk.ChatMessagePartType]struct{} {
	set := make(map[nicloudsdk.ChatMessagePartType]struct{}, len(parts))
	for _, p := range parts {
		set[p.Type] = struct{}{}
	}
	return set
