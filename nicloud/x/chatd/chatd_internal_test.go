...
import (
	...
	openaicomputeruse "github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatopenai/computeruse"
...
func TestRegenerateChatTitle_PersistsAndBroadcasts(t *testing.T) {
	...
	cancelSub, err := pubsub.SubscribeWithErr(
		nicloudpubsub.ChatWatchEventChannel(ownerID),
	...
	db.EXPECT().GetChatMessagesByChatIDAscPaginated(
		...
		mustChatMessage(
			...
			database.ChatMessageVisibilityBoth,
			nicloudsdk.ChatMessageText("checking the diff now"),
		),
	}, nil)
	...
	case event := <-messageEvents:
		...
		require.Equal(t, nicloudsdk.ChatWatchEventKindTitleChange, event.payload.Kind)
...
func TestPersistInstructionFilesIncludesAgentMetadata(t *testing.T) {
	...
	db.EXPECT().UpdateChatLastInjectedContext(gomock.Any(),
		gomock.Cond(func(x any) bool {
			...
			for _, p := range parts {
				if p.Type == nicloudsdk.ChatMessagePartTypeContextFile && p.ContextFilePath != "" {
...
func TestPersistInstructionFilesSentinelWithSkills(t *testing.T) {
	...
	db.EXPECT().UpdateChatLastInjectedContext(gomock.Any(),
		gomock.Cond(func(x any) bool {
			...
			for _, p := range parts {
				if p.Type == nicloudsdk.ChatMessagePartTypeSkill &&
...
func TestSubscribeUsesDurableCacheWhenLocalMessageWasNotDelivered(t *testing.T) {
	...
	cachedMessage := nicloudsdk.ChatMessage{
		ID:     2,
		ChatID: chatID,
	...
	server := newSubscribeTestServer(t, db)
	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
	...
	server.publishChatStreamNotify(chatID, nicloudpubsub.ChatStreamNotifyMessage{
		AfterMessageID: 1,
	})
...
func TestSubscribeReplaysCurrentRetryPhaseInSnapshot(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatStreamEventTypeRetry, snapshot[1].Type)
...
func TestSubscribeDoesNotReplayRetryAfterStreamResumes(t *testing.T) {
	...
	server.publishMessagePart(chatID, nicloudsdk.ChatMessageRoleAssistant, nicloudsdk.ChatMessageText("retry recovered"))
...
func TestSubscribeDoesNotReplayFailedAttemptPartsAfterRetry(t *testing.T) {
	...
	server.publishMessagePart(chatID, nicloudsdk.ChatMessageRoleAssistant, nicloudsdk.ChatMessageText("failed partial"))
...
func TestSubscribeFallsBackToLegacyErrorStringViaPubsub(t *testing.T) {
	...
	event := requireStreamErrorEvent(t, events)
	require.Equal(t, &nicloudsdk.ChatError{Message: "legacy error only"}, event.Error)
	requireNoStreamEvent(t, events, 200*time.Millisecond)
}

func newTestRetryPayload() *nicloudsdk.ChatStreamRetry {
...
func TestSubscribeAuthorizedFallsBackToStaleRowWhenRefreshFails(t *testing.T) {
	...
	state.buffer = []bufferedStreamPart{{
		event: nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
				Part: nicloudsdk.ChatMessageText("thinking"),
			},
		},
	}}
	state.mu.Unlock()
	...
	require.Equal(t, nicloudsdk.ChatStreamEventTypeStatus, initialSnapshot[0].Type)
	require.NotNil(t, initialSnapshot[0].Status)
	require.Equal(t, nicloudsdk.ChatStatusPending, initialSnapshot[0].Status.Status)
	require.Equal(t, nicloudsdk.ChatStreamEventTypeMessagePart, initialSnapshot[1].Type)
...
func TestSubscribeSurfacesTransientLookupFailureAsInitialError(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatStreamEventTypeError, snapshot[0].Type)
...
func requireStreamMessageEvent(t *testing.T, events <-chan nicloudsdk.ChatStreamEvent) nicloudsdk.ChatStreamEvent {
	t.Helper()

	select {
	case event, ok := <-events:
		require.True(t, ok, "chat stream closed before delivering an event")
		require.Equal(t, nicloudsdk.ChatStreamEventTypeMessage, event.Type)
...
func requireStreamRetryEvent(t *testing.T, events <-chan nicloudsdk.ChatStreamEvent) nicloudsdk.ChatStreamEvent {
	t.Helper()

	select {
	case event, ok := <-events:
		require.True(t, ok, "chat stream closed before delivering an event")
		require.Equal(t, nicloudsdk.ChatStreamEventTypeRetry, event.Type)
...
func requireSnapshotRetryEvent(t *testing.T, snapshot []nicloudsdk.ChatStreamEvent) nicloudsdk.ChatStreamEvent {
	t.Helper()

	var retryEvents []nicloudsdk.ChatStreamEvent
	for _, event := range snapshot {
		if event.Type == nicloudsdk.ChatStreamEventTypeRetry {
...
func requireNoSnapshotRetryEvent(t *testing.T, snapshot []nicloudsdk.ChatStreamEvent) {
	t.Helper()

	for _, event := range snapshot {
		require.NotEqual(t, nicloudsdk.ChatStreamEventTypeRetry, event.Type,
			"unexpected retry event in snapshot: %+v", event)
	}
}

func requireSnapshotMessagePartEvent(t *testing.T, snapshot []nicloudsdk.ChatStreamEvent) nicloudsdk.ChatStreamEvent {
	t.Helper()
...
func requireStreamErrorEvent(t *testing.T, events <-chan nicloudsdk.ChatStreamEvent) nicloudsdk.ChatStreamEvent {
	t.Helper()

	select {
	case event, ok := <-events:
		require.True(t, ok, "chat stream closed before delivering an event")
		require.Equal(t, nicloudsdk.ChatStreamEventTypeError, event.Type)
...
func requireNoStreamEvent(t *testing.T, events <-chan nicloudsdk.ChatStreamEvent, wait time.Duration) {
	t.Helper()
...
func TestPublishToStream_DropWarnRateLimiting(t *testing.T) {
	...
	chatID := uuid.New()
	subCh := make(chan nicloudsdk.ChatStreamEvent, 1)
	subCh <- nicloudsdk.ChatStreamEvent{} // pre-fill so sends always drop
	...
	partEvent := nicloudsdk.ChatStreamEvent{
		Type:        nicloudsdk.ChatStreamEventTypeMessagePart,
		MessagePart: &nicloudsdk.ChatStreamMessagePart{},
	}
	for i := 0; i < 50; i++ {
...
func TestResolveUserCompactionThreshold(t *testing.T) {
	...
	userID := uuid.New()
	modelConfigID := uuid.New()
	expectedKey := nicloudsdk.CompactionThresholdKey(modelConfigID)
...
func TestSkillsFromParts(t *testing.T) {
	...
	t.Run("NoSkillParts", func(t *testing.T) {
		...
		msgs := []database.ChatMessage{
			chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{
	...
	t.Run("MixedPartTypes", func(t *testing.T) {
		...
		msgs := []database.ChatMessage{
			...
			chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "user turn"},
			}),
		}
		got := skillsFromParts(msgs)
		require.Len(t, got, 1)
...
func TestProcessChat_IgnoresStaleControlNotification(t *testing.T) {
	...
	staleNotify, err := json.Marshal(nicloudpubsub.ChatStreamNotifyMessage{
		Status: string(database.ChatStatusPending),
	})
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chatID), staleNotify)
	require.NoError(t, err)
...
func TestSweepIdleStreams_DoesNotReapWithSubscribers(t *testing.T) {
	...
	state := &chatStreamState{
		...
		bufferRetainedAt: mClock.Now(),
		subscribers: map[uuid.UUID]chan nicloudsdk.ChatStreamEvent{
			uuid.New(): make(chan nicloudsdk.ChatStreamEvent, 1),
		},
		buffer: []bufferedStreamPart{{
			event: nicloudsdk.ChatStreamEvent{
				Type:        nicloudsdk.ChatStreamEventTypeMessagePart,
...
func TestPublishToStream_DropZeroesBackingSlot(t *testing.T) {
	...
	sentinel := &nicloudsdk.ChatStreamMessagePart{
		Role: nicloudsdk.ChatMessageRoleAssistant,
	}
	...
	newPart := &nicloudsdk.ChatStreamMessagePart{
		Role: nicloudsdk.ChatMessageRoleAssistant,
	}
	server.publishToStream(chatID, nicloudsdk.ChatStreamEvent{
		Type:        nicloudsdk.ChatStreamEventTypeMessagePart,
		MessagePart: newPart,
	})
...
func TestAutoPromote_InsertFailureSkipsStatusUpdate(t *testing.T) {
	...
	interruptMsg, err := json.Marshal(nicloudpubsub.ChatStreamNotifyMessage{
		Status: string(database.ChatStatusWaiting),
	})
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chatID), interruptMsg)
	require.NoError(t, err)
	close(modelRelease)
...
func makeInProgressPart(text string) bufferedStreamPart {
	return bufferedStreamPart{
		event: nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: nicloudsdk.ChatMessageRoleAssistant,
				Part: nicloudsdk.ChatMessageText(text),
			},
		},
	}
}
...
func partText(event nicloudsdk.ChatStreamEvent) string {
