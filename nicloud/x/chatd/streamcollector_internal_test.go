...
func assertGauges(t *testing.T, reg *prometheus.Registry, want gaugeExpectations) {
	...
	assert.Equal(t, want.active, got["nicloud_chatd_streams_active"], "streams_active")
	assert.Equal(t, want.bufferMax, got["nicloud_chatd_stream_buffer_size_max"], "buffer_size_max")
	assert.Equal(t, want.bufferTotal, got["nicloud_chatd_stream_buffer_events"], "buffer_events")
	assert.Equal(t, want.subscribers, got["nicloud_chatd_stream_subscribers"], "subscribers")
}

func newSubscribers(t *testing.T, n int) map[uuid.UUID]chan nicloudsdk.ChatStreamEvent {
	t.Helper()
	subs := make(map[uuid.UUID]chan nicloudsdk.ChatStreamEvent, n)
	for range n {
		subs[uuid.New()] = make(chan nicloudsdk.ChatStreamEvent, 1)
	}
	return subs
}
...
func TestStreamStateCollector_BufferDroppedIncrementsOnCapacity(t *testing.T) {
	...
	assert.Equal(t, float64(1), counterValue(t, reg, "nicloud_chatd_stream_buffer_dropped_total"))

	server.publishToStream(chatID, partEvent)
	assert.Equal(t, float64(2), counterValue(t, reg, "nicloud_chatd_stream_buffer_dropped_total"))
}
