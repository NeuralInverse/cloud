...
func TestShouldActivateChainMode(t *testing.T) {
	...
	unresolvedLocalInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		chainModeSystemMessage(),
		chainModeUserMessage("prior user message"),
		chainModeAssistantMessage(modelConfigID, []nicloudsdk.ChatMessagePart{localCall}),
		chainModeUserMessage("latest user message"),
	})
	localResult := nicloudsdk.ChatMessageToolResult(
		"call-local",
		"read_file",
	...
	missingToolResultsInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		...
		chainModeUserMessage("prior user message"),
		chainModeAssistantMessage(modelConfigID, []nicloudsdk.ChatMessagePart{localCall}),
		chainModeToolMessage([]nicloudsdk.ChatMessagePart{localResult}),
		chainModeUserMessage("latest user message"),
	})
...
func TestResolveChainModeIgnoresSkillOnlySentinelMessages(t *testing.T) {
	...
	user := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{{
...
func TestResolveChainMode_BlocksOnUnresolvedLocalToolCall(t *testing.T) {
	...
	modelConfigID := uuid.New()
	toolCall := nicloudsdk.ChatMessageToolCall(
		"call-local",
		"read_file",
		json.RawMessage(`{"path":"main.go"}`),
	)
	...
	chainInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		chainModeSystemMessage(),
		chainModeUserMessage("prior user message"),
		chainModeAssistantMessage(modelConfigID, []nicloudsdk.ChatMessagePart{toolCall}),
		chainModeUserMessage("latest user message"),
	})
...
func TestResolveChainMode_AllowsProviderExecutedOnly(t *testing.T) {
	...
	toolCall := nicloudsdk.ChatMessageToolCall(
		"call-web-search",
		"web_search",
		json.RawMessage(`{"query":"neuralinverse docs"}`),
	)
...
func TestResolveChainMode_BlocksOnMixedProviderExecutedAndUnresolvedLocalCall(t *testing.T) {
	...
	chainInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		...
		chainModeAssistantMessage(
			modelConfigID,
			[]nicloudsdk.ChatMessagePart{providerCall, localCall},
		),
		chainModeUserMessage("latest user message"),
	})
...
func TestResolveChainMode_AllowsResolvedLocalCall(t *testing.T) {
	...
	toolResult := nicloudsdk.ChatMessageToolResult(
		"call-local",
		"read_file",
		json.RawMessage(`{"ok":true}`),
		false,
	...
	chainInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		...
		chainModeUserMessage("prior user message"),
		chainModeAssistantMessage(modelConfigID, []nicloudsdk.ChatMessagePart{toolCall}),
		chainModeToolMessage([]nicloudsdk.ChatMessagePart{toolResult}),
		followUp,
		chainModeUserMessage("latest user message"),
	})
...
func TestResolveChainMode_BlocksOnMixedResolvedAndUnresolved(t *testing.T) {
	...
	modelConfigID := uuid.New()
	firstCall := nicloudsdk.ChatMessageToolCall(
		"call-first",
		"read_file",
		json.RawMessage(`{"path":"main.go"}`),
	)
	secondCall := nicloudsdk.ChatMessageToolCall(
		"call-second",
		"read_file",
	...
	chainInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		...
		chainModeAssistantMessage(
			modelConfigID,
			[]nicloudsdk.ChatMessagePart{firstCall, secondCall},
		),
		chainModeToolMessage([]nicloudsdk.ChatMessagePart{toolResult}),
		chainModeUserMessage("latest user message"),
	})
...
func TestResolveChainMode_BlocksProviderMissingWithMultipleToolCalls(t *testing.T) {
	...
	modelConfigID := uuid.New()
	call1 := nicloudsdk.ChatMessageToolCall(
		"call-1",
		"propose_plan",
		json.RawMessage(`{"path":"plan.md"}`),
	)
	call2 := nicloudsdk.ChatMessageToolCall(
		"call-2",
		"write_file",
		json.RawMessage(`{"path":"foo.go"}`),
	)
	result1 := nicloudsdk.ChatMessageToolResult(
		"call-1",
		"propose_plan",
		json.RawMessage(`{"ok":true}`),
		false,
	...
	result2 := nicloudsdk.ChatMessageToolResult(
		"call-2",
		"write_file",
		json.RawMessage(`{"ok":true}`),
		false,
	...
	chainInfo := chatopenai.ResolveChainMode([]database.ChatMessage{
		...
		chainModeAssistantMessage(modelConfigID, []nicloudsdk.ChatMessagePart{call1, call2}),
		chainModeToolMessage([]nicloudsdk.ChatMessagePart{result1, result2}),
...
func TestFilterPromptForChainModeKeepsContributingUsersAcrossSkippedSentinelTurns(t *testing.T) {
	...
	modelConfigID := uuid.New()
	priorUser := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{{
		Type: nicloudsdk.ChatMessagePartTypeText,
	...
	firstTrailingUser := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{{
		Type: nicloudsdk.ChatMessagePartTypeText,
		Text: "first trailing user",
	}})
	firstTrailingUser.Role = database.ChatMessageRoleUser
	skillOnly := chainModeSkillOnlyUserMessage()
	lastTrailingUser := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{{
		Type: nicloudsdk.ChatMessagePartTypeText,
...
func TestFilterPromptForChainModeUsesContributingTrailingUsers(t *testing.T) {
	...
	skillOnly := chainModeSkillOnlyUserMessage()
	latestUser := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{{
		Type: nicloudsdk.ChatMessagePartTypeText,
		Text: "latest user message",
	}})
...
func chainModeUserMessage(text string) database.ChatMessage {
	msg := chattest.ChatMessageWithParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText(text),
	})
...
func chainModeToolMessage(parts []nicloudsdk.ChatMessagePart) database.ChatMessage {
	msg := chattest.ChatMessageWithParts(parts)
