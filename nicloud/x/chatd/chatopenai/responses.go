...
func userMessageContributesToChainMode(msg database.ChatMessage) bool {
	...
	for _, part := range parts {
		...
		case nicloudsdk.ChatMessagePartTypeText,
			nicloudsdk.ChatMessagePartTypeReasoning:
			if strings.TrimSpace(part.Text) != "" {
				return true
			}
		case nicloudsdk.ChatMessagePartTypeFile,
			nicloudsdk.ChatMessagePartTypeFileReference:
			return true
...
func providerHasMissingToolResults(
	...
	if !slices.ContainsFunc(parts, func(p nicloudsdk.ChatMessagePart) bool {
		return p.Type == nicloudsdk.ChatMessagePartTypeToolCall && !p.ProviderExecuted
	}) {
		return false
	}
