...
func ComputeUsagePeriodBounds(now time.Time, period nicloudsdk.ChatUsageLimitPeriod) (start, end time.Time) {
	utcNow := now.UTC()

	switch period {
	case nicloudsdk.ChatUsageLimitPeriodDay:
		start = time.Date(utcNow.Year(), utcNow.Month(), utcNow.Day(), 0, 0, 0, 0, time.UTC)
		end = start.AddDate(0, 0, 1)
	case nicloudsdk.ChatUsageLimitPeriodWeek:
	...
	case nicloudsdk.ChatUsageLimitPeriodMonth:
...
func ResolveUsageLimitStatus(ctx context.Context, db database.Store, userID uuid.UUID, organizationID uuid.NullUUID, now time.Time) (*nicloudsdk.ChatUsageLimitStatus, error) {
	...
	return &nicloudsdk.ChatUsageLimitStatus{
		IsLimited:        true,
		Period:           period,
...
func mapDBPeriodToSDK(dbPeriod string) (nicloudsdk.ChatUsageLimitPeriod, bool) {
	switch dbPeriod {
	case string(nicloudsdk.ChatUsageLimitPeriodDay):
		return nicloudsdk.ChatUsageLimitPeriodDay, true
	case string(nicloudsdk.ChatUsageLimitPeriodWeek):
		return nicloudsdk.ChatUsageLimitPeriodWeek, true
	case string(nicloudsdk.ChatUsageLimitPeriodMonth):
		return nicloudsdk.ChatUsageLimitPeriodMonth, true
	default:
		return "", false
	}
}
