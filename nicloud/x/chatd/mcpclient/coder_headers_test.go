...
// TestConnectAll_ForwardNIHeaders_DefaultOff is a regression guard
// that the Coder identity headers are NOT sent when the option is
// left at its default (false).
func TestConnectAll_ForwardNIHeaders_DefaultOff(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	...
	assert.False(t, cfg.ForwardNIHeaders, "default must be false")

	coderHeaders := map[string]string{
		chatprovider.HeaderNIOwnerID:     uuid.NewString(),
		chatprovider.HeaderNIChatID:      uuid.NewString(),
		chatprovider.HeaderNIWorkspaceID: uuid.NewString(),
	}
	...
	for _, h := range *recorded {
		assert.Empty(t, h.Get(chatprovider.HeaderNIOwnerID))
		assert.Empty(t, h.Get(chatprovider.HeaderNIChatID))
		assert.Empty(t, h.Get(chatprovider.HeaderNISubchatID))
		assert.Empty(t, h.Get(chatprovider.HeaderNIWorkspaceID))
	}
}

// TestConnectAll_ForwardNIHeaders_Enabled verifies that when the
...
func TestConnectAll_ForwardNIHeaders_Enabled(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true})
	...
	assert.Equal(t, ownerID.String(), last.Get(chatprovider.HeaderNIOwnerID))
	assert.Equal(t, chatID.String(), last.Get(chatprovider.HeaderNIChatID))
	assert.Equal(t, subchatID.String(), last.Get(chatprovider.HeaderNISubchatID))
	assert.Equal(t, workspaceID.String(), last.Get(chatprovider.HeaderNIWorkspaceID))
}

// TestConnectAll_ForwardNIHeaders_RootChat verifies that for a root
// chat (no parent), the chat's own ID is forwarded as
// X-Coder-Chat-Id and the X-Coder-Subchat-Id header is absent.
func TestConnectAll_ForwardNIHeaders_RootChat(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true})
	...
	assert.Equal(t, ownerID.String(), last.Get(chatprovider.HeaderNIOwnerID))
	assert.Equal(t, chatID.String(), last.Get(chatprovider.HeaderNIChatID))
	assert.Empty(t, last.Get(chatprovider.HeaderNISubchatID))
	assert.Empty(t, last.Get(chatprovider.HeaderNIWorkspaceID))
}

// TestConnectAll_ForwardNIHeaders_WithAPIKeyAuth verifies that the
// api_key auth header is preserved when Coder identity headers are
// forwarded alongside.
func TestConnectAll_ForwardNIHeaders_WithAPIKeyAuth(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true})
...
// TestConnectAll_ForwardNIHeaders_WithOAuth2 verifies that the
...
func TestConnectAll_ForwardNIHeaders_WithOAuth2(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true})
	...
	coderHeaders := map[string]string{
		...
		chatprovider.HeaderNIOwnerID: ownerID,
	}
	...
	assert.Equal(t, ownerID, last.Get(chatprovider.HeaderNIOwnerID))
}

// TestConnectAll_ForwardNIHeaders_WithCustomHeaders verifies that
...
func TestConnectAll_ForwardNIHeaders_WithCustomHeaders(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true})
	...
	assert.Equal(t, "admin-controlled", last.Get(chatprovider.HeaderNIOwnerID))
	assert.Equal(t, chatID.String(), last.Get(chatprovider.HeaderNIChatID))
}
