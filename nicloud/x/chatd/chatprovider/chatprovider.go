...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatutil"
...
// ambient credentials from the Neural Inverse Cloud server instead of an explicit
// API key.
...
func InlineImageCapBytes(provider string) (int, bool) {
	...
	case fantasyanthropic.Name, fantasybedrock.Name:
		return nicloudsdk.AnthropicInlineImageCapBytes, true
	default:
		return 0, false
	}
}
...
type ProviderAvailability struct {
	Available         bool
	UnavailableReason nicloudsdk.ChatModelProviderUnavailableReason
}
	...
	for _, provider := range providers {
		...
		case centralKey != "":
			if !provider.AllowUserAPIKey || provider.AllowCentralAPIKeyFallback {
				...
				resolved.UnavailableReason = nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired
			}
		case normalizedProvider == fantasybedrock.Name && provider.CentralAPIKeyEnabled:
			// Bedrock can use ambient AWS credentials from the Neural Inverse Cloud server
		...
		case provider.AllowUserAPIKey:
			resolved.UnavailableReason = nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired
		default:
			resolved.UnavailableReason = nicloudsdk.ChatModelProviderUnavailableMissingAPIKey
		}
...
func (*ModelCatalog) ListConfiguredModels(
...
) (nicloudsdk.ChatModelsResponse, bool) {
	if len(configuredModels) == 0 {
		return nicloudsdk.ChatModelsResponse{}, false
	}

	modelsByProvider := make(map[string][]nicloudsdk.ChatModel)
	...
	response := nicloudsdk.ChatModelsResponse{
		Providers: make([]nicloudsdk.ChatModelProvider, 0, len(providers)),
	}
	for _, provider := range providers {
		...
		result := nicloudsdk.ChatModelProvider{
			Provider: provider,
			Models:   models,
		}
		if avail, ok := availabilityByProvider[provider]; ok {
			...
			result.UnavailableReason = nicloudsdk.ChatModelProviderUnavailableMissingAPIKey
		}
...
func (*ModelCatalog) ListConfiguredProviderAvailability(
	...
	response := nicloudsdk.ChatModelsResponse{
		Providers: make([]nicloudsdk.ChatModelProvider, 0, len(supportedProviderNames)),
	}
...
func newChatModel(provider, modelID, displayName string) nicloudsdk.ChatModel {
	name := strings.TrimSpace(displayName)
	if name == "" {
		name = modelID
	}

	return nicloudsdk.ChatModel{
...
func sortChatModels(models []nicloudsdk.ChatModel) {
...
func MergeMissingModelCostConfig(
	dst **nicloudsdk.ModelCostConfig,
	defaults *nicloudsdk.ModelCostConfig,
) {
	if defaults == nil {
		return
	}
...
func MergeMissingProviderOptions(
	dst **nicloudsdk.ChatModelProviderOptions,
...
// Neural Inverse Cloud entities.
const (
	// HeaderNIOwnerID identifies the Neural Inverse Cloud user who owns the chat.
	HeaderNIOwnerID = "X-Coder-Owner-Id"
	// HeaderNeural Inverse CloudChatID identifies the top-level (parent) chat.
	// For root chats this is the chat's own ID; for subchats it
	// is the parent chat's ID.
	HeaderNIChatID = "X-Coder-Chat-Id"
	// HeaderNeural Inverse CloudSubchatID identifies the current subchat. Only
	// present when the request originates from a child chat.
	HeaderNISubchatID = "X-Coder-Subchat-Id"
	// HeaderNeural Inverse CloudWorkspaceID identifies the workspace associated
	// with the chat, if any.
	HeaderNIWorkspaceID = "X-Coder-Workspace-Id"
)

// NIHeaders builds the set of Neural Inverse Cloud identity headers to attach
// to outgoing LLM API requests for the given chat.
func NIHeaders(chat database.Chat) map[string]string {
	chatID := chat.ID
	if chat.ParentChatID.Valid {
		chatID = chat.ParentChatID.UUID
	}
	h := map[string]string{
		HeaderNIOwnerID: chat.OwnerID.String(),
		HeaderNIChatID:  chatID.String(),
	}
	if chat.ParentChatID.Valid {
		h[HeaderNISubchatID] = chat.ID.String()
	}
	if chat.WorkspaceID.Valid {
		h[HeaderNIWorkspaceID] = chat.WorkspaceID.UUID.String()
	}
	return h
}

// NIHeadersFromIDs is a convenience form of Neural Inverse CloudHeaders for call
// sites that do not have a full database.Chat in scope.
func NIHeadersFromIDs(
	ownerID uuid.UUID,
	chatID uuid.UUID,
	parentChatID uuid.NullUUID,
	workspaceID uuid.NullUUID,
) map[string]string {
	return NIHeaders(database.Chat{
		ID:           chatID,
		OwnerID:      ownerID,
...
func anthropicProviderOptionsFromChatConfig(
	options *nicloudsdk.ChatModelAnthropicProviderOptions,
...
func googleProviderOptionsFromChatConfig(
	options *nicloudsdk.ChatModelGoogleProviderOptions,
...
func openAICompatProviderOptionsFromChatConfig(
	options *nicloudsdk.ChatModelOpenAICompatProviderOptions,
...
func openRouterProviderOptionsFromChatConfig(
	options *nicloudsdk.ChatModelOpenRouterProviderOptions,
...
func vercelProviderOptionsFromChatConfig(
	options *nicloudsdk.ChatModelVercelProviderOptions,
