...
func TestResolveUserProviderKeys(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantAvailability: map[string]chatprovider.ProviderAvailability{
				fantasyopenai.Name: {Available: false, UnavailableReason: nicloudsdk.ChatModelProviderUnavailableMissingAPIKey},
			},
			wantKeys: map[string]string{
				fantasyopenai.Name: "",
			},
		...
		},
		{
			...
			wantAvailability: map[string]chatprovider.ProviderAvailability{
				fantasybedrock.Name: {Available: false, UnavailableReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired},
			},
			wantKeys: map[string]string{
				fantasybedrock.Name: "",
			},
		...
		},
		{
			...
			wantAvailability: map[string]chatprovider.ProviderAvailability{
				fantasybedrock.Name: {Available: false, UnavailableReason: nicloudsdk.ChatModelProviderUnavailableMissingAPIKey},
			},
			wantKeys: map[string]string{
				fantasybedrock.Name: "",
			},
		...
		},
		{
			...
			providers: []chatprovider.ConfiguredProvider{configuredProvider(openAIProviderID, fantasyopenai.Name, false, "sk-central", true, false)},
			wantAvailability: map[string]chatprovider.ProviderAvailability{
				fantasyopenai.Name: {Available: false, UnavailableReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired},
			},
			wantKeys: map[string]string{
				fantasyopenai.Name: "",
			},
		...
		},
		{
			...
			wantAvailability: map[string]chatprovider.ProviderAvailability{
				fantasyopenai.Name:    {Available: true},
				fantasyanthropic.Name: {Available: false, UnavailableReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired},
			},
			wantKeys: map[string]string{
				fantasyopenai.Name:    "sk-central",
...
func TestResolveUserProviderKeys_UnavailableReason(t *testing.T) {
	...
	tests := []struct {
		...
		wantReason nicloudsdk.ChatModelProviderUnavailableReason
	}{
		{
			...
			wantReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired,
		},
		{
...
func TestListConfiguredModels_PolicyAwareAvailability(t *testing.T) {
	...
	tests := []struct {
		...
		want                   nicloudsdk.ChatModelsResponse
	}{
		{
			...
			availabilityByProvider: map[string]chatprovider.ProviderAvailability{
				fantasyopenai.Name: {
					Available:         false,
					UnavailableReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired,
				},
			},
			enabledProviders: enabledProviders(fantasyopenai.Name),
			want: nicloudsdk.ChatModelsResponse{Providers: []nicloudsdk.ChatModelProvider{{
				Provider:          fantasyopenai.Name,
				Available:         false,
				UnavailableReason: nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired,
				Models: []nicloudsdk.ChatModel{{
		...
		},
		{
			...
			want: nicloudsdk.ChatModelsResponse{Providers: []nicloudsdk.ChatModelProvider{{
				Provider:          fantasyopenai.Name,
				Available:         false,
				UnavailableReason: nicloudsdk.ChatModelProviderUnavailableMissingAPIKey,
				Models: []nicloudsdk.ChatModel{{
...
func TestNIHeaders(t *testing.T) {
	t.Parallel()
	...
	t.Run("RootChatNoWorkspace", func(t *testing.T) {
		...
		h := chatprovider.NIHeaders(chat)
		require.Equal(t, ownerID.String(), h[chatprovider.HeaderNIOwnerID])
		require.Equal(t, chatID.String(), h[chatprovider.HeaderNIChatID])
		require.NotContains(t, h, chatprovider.HeaderNISubchatID)
		require.NotContains(t, h, chatprovider.HeaderNIWorkspaceID)
	})
	...
	t.Run("RootChatWithWorkspace", func(t *testing.T) {
		...
		require.Equal(t, chatID.String(), h[chatprovider.HeaderNIChatID])
		require.NotContains(t, h, chatprovider.HeaderNISubchatID)
		require.Equal(t, workspaceID.String(), h[chatprovider.HeaderNIWorkspaceID])
	})
	...
	t.Run("SubchatWithWorkspace", func(t *testing.T) {
		...
		h := chatprovider.NIHeaders(chat)
		require.Equal(t, ownerID.String(), h[chatprovider.HeaderNIOwnerID])
		require.Equal(t, parentID.String(), h[chatprovider.HeaderNIChatID])
		require.Equal(t, subchatID.String(), h[chatprovider.HeaderNISubchatID])
		require.Equal(t, workspaceID.String(), h[chatprovider.HeaderNIWorkspaceID])
	})
...
// chattest test servers. NIHeaders construction is tested
// separately in TestNIHeaders.
func TestModelFromConfig_ExtraHeaders(t *testing.T) {
	...
	headers := chatprovider.NIHeaders(chat)

	assertNIHeaders := func(t *testing.T, got http.Header) {
		t.Helper()
		assert.Equal(t, ownerID.String(), got.Get(chatprovider.HeaderNIOwnerID))
		assert.Equal(t, parentID.String(), got.Get(chatprovider.HeaderNIChatID))
		assert.Equal(t, subchatID.String(), got.Get(chatprovider.HeaderNISubchatID))
		assert.Equal(t, workspaceID.String(), got.Get(chatprovider.HeaderNIWorkspaceID))
	}
...
func TestModelFromConfig_NilExtraHeaders(t *testing.T) {
	...
	serverURL := chattest.NewOpenAI(t, func(req *chattest.OpenAIRequest) chattest.OpenAIResponse {
		...
		assert.Empty(t, req.Header.Get(chatprovider.HeaderNIOwnerID))
		assert.Empty(t, req.Header.Get(chatprovider.HeaderNIChatID))
		assert.Empty(t, req.Header.Get(chatprovider.HeaderNISubchatID))
		assert.Empty(t, req.Header.Get(chatprovider.HeaderNIWorkspaceID))
		close(called)
		return chattest.OpenAINonStreamingResponse("hello")
	})
...
func TestMergeMissingProviderOptions_OpenRouterNested(t *testing.T) {
	...
	options := &nicloudsdk.ChatModelProviderOptions{
		OpenRouter: &nicloudsdk.ChatModelOpenRouterProviderOptions{
			Reasoning: &nicloudsdk.ChatModelReasoningOptions{
				Enabled: ptr.Ref(true),
			},
			Provider: &nicloudsdk.ChatModelOpenRouterProvider{
				Order: []string{"openai"},
			},
		},
	}
	defaults := &nicloudsdk.ChatModelProviderOptions{
		OpenRouter: &nicloudsdk.ChatModelOpenRouterProviderOptions{
			Reasoning: &nicloudsdk.ChatModelReasoningOptions{
