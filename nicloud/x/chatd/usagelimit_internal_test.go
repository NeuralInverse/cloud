...
func TestComputeUsagePeriodBounds(t *testing.T) {
	...
	tests := []struct {
		name      string
		now       time.Time
		period    nicloudsdk.ChatUsageLimitPeriod
		wantStart time.Time
		wantEnd   time.Time
	}{
		{
			...
			period:    nicloudsdk.ChatUsageLimitPeriodDay,
		...
		},
		{
			...
			period:    nicloudsdk.ChatUsageLimitPeriodWeek,
		...
		},
		{
			...
			now:       time.Date(2025, time.June, 15, 0, 0, 0, 0, time.UTC),
			period:    nicloudsdk.ChatUsageLimitPeriodMonth,
