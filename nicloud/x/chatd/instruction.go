...
func formatSystemInstructions(
	...
	for _, part := range parts {
		if part.Type == nicloudsdk.ChatMessagePartTypeContextFile && part.ContextFileContent != "" {
			hasContent = true
			break
		}
	}
...
func instructionFromContextFiles(
	...
	var contextParts []nicloudsdk.ChatMessagePart
	var os, dir string
	for _, msg := range messages {
		if !msg.Content.Valid ||
			!bytes.Contains(msg.Content.RawMessage, []byte(`"context-file"`)) {
...
func skillsFromParts(
	...
	for _, msg := range messages {
		...
		for _, part := range parts {
			if part.Type != nicloudsdk.ChatMessagePartTypeSkill {
				continue
			}
			if filterByAgent && part.ContextFileAgentID.Valid &&
				part.ContextFileAgentID.UUID != filterAgentID {
...
func filterSkillParts(parts []nicloudsdk.ChatMessagePart) []nicloudsdk.ChatMessagePart {
