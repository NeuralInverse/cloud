	...
	tests := []struct {
	...
	}{
		{
			...
			want: chaterror.ClassifiedError{
				...
				Kind:       nicloudsdk.ChatErrorKindOverloaded,
				Provider:   "",
				Retryable:  true,
				StatusCode: 529,
			},
		...
		},
		{
			...
			want: chaterror.ClassifiedError{
				...
				Kind:       nicloudsdk.ChatErrorKindAuth,
				Provider:   "",
				Retryable:  false,
				StatusCode: 0,
			},
		...
		},
		{
			...
			want: chaterror.ClassifiedError{
				...
				Kind:       nicloudsdk.ChatErrorKindRateLimit,
				Provider:   "",
				Retryable:  true,
				StatusCode: 429,
			},
		...
		// the trailing newline written by http.Error).
		{
			...
			err: &fantasy.ProviderError{
				...
				Message:    fmt.Sprintf("%s: AI provider %q is disabled\n", nicloudsdk.ChatErrorKindProviderDisabled, "openai"),
				StatusCode: http.StatusServiceUnavailable,
			},
			want: chaterror.ClassifiedError{
				Message:    "The OpenAI provider has been disabled. Contact your Coder administrator.",
				Detail:     fmt.Sprintf("%s: AI provider %q is disabled", nicloudsdk.ChatErrorKindProviderDisabled, "openai"),
				Kind:       nicloudsdk.ChatErrorKindProviderDisabled,
				Provider:   "openai",
				Retryable:  false,
				StatusCode: 503,
			},
		},
		{
			...
			err: &fantasy.ProviderError{
				Title:      fantasy.ErrorTitleForStatusCode(http.StatusServiceUnavailable),
				Message:    fmt.Sprintf("%s: AI provider %q is disabled\n", nicloudsdk.ChatErrorKindProviderDisabled, "mycustomprovider"),
				StatusCode: http.StatusServiceUnavailable,
			},
			want: chaterror.ClassifiedError{
				Message:    "The AI provider has been disabled. Contact your Coder administrator.",
				Detail:     fmt.Sprintf("%s: AI provider %q is disabled", nicloudsdk.ChatErrorKindProviderDisabled, "mycustomprovider"),
				Kind:       nicloudsdk.ChatErrorKindProviderDisabled,
				Provider:   "",
				Retryable:  false,
				StatusCode: 503,
		...
		},
		{
			...
			err:  xerrors.New(fmt.Sprintf("%s: AI provider %q is disabled", nicloudsdk.ChatErrorKindProviderDisabled, "anthropic")),
			want: chaterror.ClassifiedError{
		...
		},
		{
			...
			err: &fantasy.ProviderError{
				Title:      fantasy.ErrorTitleForStatusCode(http.StatusServiceUnavailable),
				Message:    fmt.Sprintf("%s: AI provider %q is disabled\n", nicloudsdk.ChatErrorKindProviderDisabled, "google"),
				StatusCode: http.StatusServiceUnavailable,
			},
			want: chaterror.ClassifiedError{
				Message:    "The Google provider has been disabled. Contact your Coder administrator.",
				Detail:     fmt.Sprintf("%s: AI provider %q is disabled", nicloudsdk.ChatErrorKindProviderDisabled, "google"),
				Kind:       nicloudsdk.ChatErrorKindProviderDisabled,
				Provider:   "google",
				Retryable:  false,
				StatusCode: 503,
...
func TestClassify_OpenAIResponsesAPIDiagnostics(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"/BareString", func(t *testing.T) {
			...
			require.Equal(t, nicloudsdk.ChatErrorKindGeneric, classified.Kind)
...
func TestClassify_PatternCoverage(t *testing.T) {
	...
	tests := []struct {
		name      string
		err       string
		wantKind  nicloudsdk.ChatErrorKind
		wantRetry bool
	}{
		{name: "OverloadedLiteral", err: "overloaded", wantKind: nicloudsdk.ChatErrorKindOverloaded, wantRetry: true},
		{name: "RateLimitLiteral", err: "rate limit", wantKind: nicloudsdk.ChatErrorKindRateLimit, wantRetry: true},
		{name: "RateLimitUnderscoreLiteral", err: "rate_limit", wantKind: nicloudsdk.ChatErrorKindRateLimit, wantRetry: true},
		{name: "RateLimitedLiteral", err: "rate limited", wantKind: nicloudsdk.ChatErrorKindRateLimit, wantRetry: true},
		{name: "RateLimitedHyphenLiteral", err: "rate-limited", wantKind: nicloudsdk.ChatErrorKindRateLimit, wantRetry: true},
		{name: "TooManyRequestsLiteral", err: "too many requests", wantKind: nicloudsdk.ChatErrorKindRateLimit, wantRetry: true},
		{name: "TimeoutLiteral", err: "timeout", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "TimedOutLiteral", err: "timed out", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "ServiceUnavailableLiteral", err: "service unavailable", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "UnavailableLiteral", err: "unavailable", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "ConnectionResetLiteral", err: "connection reset", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "ConnectionRefusedLiteral", err: "connection refused", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "EOFLiteral", err: "eof", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "BrokenPipeLiteral", err: "broken pipe", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "BadGatewayLiteral", err: "bad gateway", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "GatewayTimeoutLiteral", err: "gateway timeout", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "ClientConnLiteral", err: "client conn", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "GOAWAYLiteral", err: "goaway", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2StreamClosedLiteral", err: "http2: stream closed", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "UseOfClosedNetworkConnectionLiteral", err: "use of closed network connection", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2InternalErrorReceivedFromPeerLiteral", err: "internal_error; received from peer", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2RefusedStreamReceivedFromPeerLiteral", err: "refused_stream; received from peer", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2CancelReceivedFromPeerLiteral", err: "cancel; received from peer", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2EnhanceYourCalmReceivedFromPeerLiteral", err: "enhance_your_calm; received from peer", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "HTTP2NoErrorReceivedFromPeerLiteral", err: "no_error; received from peer", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "AuthenticationLiteral", err: "authentication", wantKind: nicloudsdk.ChatErrorKindAuth, wantRetry: false},
		{name: "UnauthorizedLiteral", err: "unauthorized", wantKind: nicloudsdk.ChatErrorKindAuth, wantRetry: false},
		{name: "InvalidAPIKeyLiteral", err: "invalid api key", wantKind: nicloudsdk.ChatErrorKindAuth, wantRetry: false},
		{name: "InvalidAPIKeyUnderscoreLiteral", err: "invalid_api_key", wantKind: nicloudsdk.ChatErrorKindAuth, wantRetry: false},
		{name: "QuotaLiteral", err: "quota", wantKind: nicloudsdk.ChatErrorKindUsageLimit, wantRetry: false},
		{name: "BillingLiteral", err: "billing", wantKind: nicloudsdk.ChatErrorKindUsageLimit, wantRetry: false},
		{name: "InsufficientQuotaLiteral", err: "insufficient_quota", wantKind: nicloudsdk.ChatErrorKindUsageLimit, wantRetry: false},
		{name: "PaymentRequiredLiteral", err: "payment required", wantKind: nicloudsdk.ChatErrorKindUsageLimit, wantRetry: false},
		{name: "ForbiddenLiteral", err: "forbidden", wantKind: nicloudsdk.ChatErrorKindAuth, wantRetry: false},
		{name: "InvalidModelLiteral", err: "invalid model", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "ModelNotFoundLiteral", err: "model not found", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "ModelNotFoundUnderscoreLiteral", err: "model_not_found", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "UnsupportedModelLiteral", err: "unsupported model", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "ContextLengthExceededLiteral", err: "context length exceeded", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "ContextExceededLiteral", err: "context_exceeded", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "MaximumContextLengthLiteral", err: "maximum context length", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "MalformedConfigLiteral", err: "malformed config", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "MalformedConfigurationLiteral", err: "malformed configuration", wantKind: nicloudsdk.ChatErrorKindConfig, wantRetry: false},
		{name: "ServerErrorLiteral", err: "server error", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: true},
		{name: "InternalServerErrorLiteral", err: "internal server error", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: true},
		{name: "ChatInterruptedLiteral", err: "chat interrupted", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: false},
		{name: "RequestInterruptedLiteral", err: "request interrupted", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: false},
		{name: "OperationInterruptedLiteral", err: "operation interrupted", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: false},
		{name: "Status408", err: "status 408", wantKind: nicloudsdk.ChatErrorKindTimeout, wantRetry: true},
		{name: "Status500", err: "status 500", wantKind: nicloudsdk.ChatErrorKindGeneric, wantRetry: true},
		{name: "ProviderDisabledLiteral", err: "provider_disabled", wantKind: nicloudsdk.ChatErrorKindProviderDisabled, wantRetry: false},
	}
...
func TestClassify_TransportFailuresUseBroaderRetryMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			classified := chaterror.Classify(xerrors.New(tt.err))
			require.Equal(t, nicloudsdk.ChatErrorKindTimeout, classified.Kind)
...
func TestClassify_HTTP2TransportErrors(t *testing.T) {
	...
	for _, tt := range transportOnly {
		t.Run("TransportOnly/"+tt.name, func(t *testing.T) {
			...
			classified := chaterror.Classify(xerrors.New(tt.err))
			require.Equal(t, nicloudsdk.ChatErrorKindTimeout, classified.Kind, "Kind")
...
func TestClassify_UsageLimitBeatsAuth(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantKind:  nicloudsdk.ChatErrorKindUsageLimit,
			wantRetry: false,
		},
		{
		...
		},
		{
			...
			err:       "unauthorized",
			wantKind:  nicloudsdk.ChatErrorKindAuth,
			wantRetry: false,
		},
		{
...
func TestClassify_StatusCodeBeatsHTTP2Transport(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantKind:      nicloudsdk.ChatErrorKindRateLimit,
			wantRetryable: true,
			wantStatus:    429,
		},
		{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, nicloudsdk.ChatErrorKindGeneric, classified.Kind,
				"chain-broken keeps the user-visible kind unchanged"+
					" so we don't add a new nicloudsdk surface")
		})
	}
}
...
func TestClassify_MissingKeyPreClassified(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatErrorKindMissingKey, classified.Kind)
	require.False(t, classified.Retryable)
