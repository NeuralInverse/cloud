...
type ClassifiedError struct {
	...
	// nicloudsdk.ChatErrorKind so the user-visible kind set stays
	// stable.
	ChainBroken bool
}
...
func normalizeClassification(classified ClassifiedError) ClassifiedError {
	...
	classified.Kind = nicloudsdk.ChatErrorKind(strings.TrimSpace(string(classified.Kind)))
	...
	if classified.Kind == "" && classified.Message == "" {
		...
		classified.Kind = nicloudsdk.ChatErrorKindGeneric
	}
	if classified.Kind == "" {
		classified.Kind = nicloudsdk.ChatErrorKindGeneric
	}
