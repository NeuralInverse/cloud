...
func TestOpenAIResponsesNoStaleWebSearchReplay(t *testing.T) {
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		...
		InitialUserContent: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("search for the latest Coder docs"),
		},
	})
	...
	_, err = server.SendMessage(ctx, chatd.SendMessageOptions{
		...
		Content: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("summarize the result without searching again"),
		},
	})
	require.NoError(t, err)
	waitForChatProcessed(ctx, t, db, chat.ID, server)
...
func TestOpenAIResponsesChainModeSkipsWhenLocalCallPending(t *testing.T) {
	...
	insertOpenAIResponsesMessages(ctx, t, db, chat.ID, user.ID, model.ID,
		persistedResponsesMessage{
			...
			parts: []nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText("please inspect the README"),
			},
		},
		persistedResponsesMessage{
			role:               database.ChatMessageRoleAssistant,
			parts:              []nicloudsdk.ChatMessagePart{localCall},
	...
	_, err := server.SendMessage(ctx, chatd.SendMessageOptions{
		...
		ModelConfigID: model.ID,
		Content: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("continue after that tool call"),
		},
	})
	require.NoError(t, err)
	waitForChatProcessed(ctx, t, db, chat.ID, server)
...
func TestOpenAIResponsesChainModeStillFiresForProviderExecutedOnly(t *testing.T) {
	...
	webSearchCall := nicloudsdk.ChatMessageToolCall(
		webSearchID,
		"web_search",
		json.RawMessage(`{"query":"neuralinverse docs"}`),
	)
	webSearchCall.ProviderExecuted = true
	webSearchResult := nicloudsdk.ChatMessageToolResult(
		webSearchID,
		"web_search",
	...
	insertOpenAIResponsesMessages(ctx, t, db, chat.ID, user.ID, model.ID,
		persistedResponsesMessage{
			role: database.ChatMessageRoleUser,
			parts: []nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText("look up the docs"),
			},
		},
		persistedResponsesMessage{
			role: database.ChatMessageRoleAssistant,
	...
	_, err := server.SendMessage(ctx, chatd.SendMessageOptions{
		...
		ModelConfigID: model.ID,
		Content: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("what did it find"),
		},
	})
	require.NoError(t, err)
	waitForChatProcessed(ctx, t, db, chat.ID, server)
