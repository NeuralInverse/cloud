...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/util/shellparse"
...
func ConvertMessagesWithFiles(
	...
	for i, msg := range messages {
		...
		parsed[i] = parsedMessage{role: nicloudsdk.ChatMessageRole(msg.Role), parts: parts}

		// Collect file IDs from user messages for resolution.
		if resolver != nil && msg.Role == database.ChatMessageRoleUser {
			for _, part := range parts {
				if part.Type == nicloudsdk.ChatMessagePartTypeFile && part.FileID.Valid {
	...
	for _, pm := range parsed {
		...
		switch pm.role {
		case nicloudsdk.ChatMessageRoleSystem:
		...
		case nicloudsdk.ChatMessageRoleAssistant:
...
const (
	...
	// ContentVersionV1 stores content as []nicloudsdk.ChatMessagePart
	// JSON for all roles.
...
func ParseContent(msg database.ChatMessage) ([]nicloudsdk.ChatMessagePart, error) {
	if !msg.Content.Valid || len(msg.Content.RawMessage) == 0 {
		return nil, nil
	}

	role := nicloudsdk.ChatMessageRole(msg.Role)
...
func parseLegacyContent(role nicloudsdk.ChatMessageRole, raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	switch role {
	case nicloudsdk.ChatMessageRoleSystem:
...
func parseContentV1(role nicloudsdk.ChatMessageRole, raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
...
// single text part.
func parseSystemRole(raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	var text string
	...
	return []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText(text)}, nil
}
...
func parseAssistantRole(raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	if isFantasyEnvelopeFormat(raw.RawMessage) {
		return parseLegacyFantasyBlocks(string(nicloudsdk.ChatMessageRoleAssistant), raw)
	}
...
func parseToolRole(raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	// Try SDK parts.
	var parts []nicloudsdk.ChatMessagePart
	...
	parts = make([]nicloudsdk.ChatMessagePart, 0, len(rows))
	for _, row := range rows {
		part := nicloudsdk.ChatMessageToolResult(row.ToolCallID, row.ToolName, row.Result, row.IsError, row.IsMedia)
...
func parseUserRole(raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
...
func parseLegacyUserBlocks(raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	var rawBlocks []json.RawMessage
	if err := json.Unmarshal(raw.RawMessage, &rawBlocks); err != nil {
		return nil, xerrors.Errorf("parse user content: %w", err)
	}

	parts := make([]nicloudsdk.ChatMessagePart, 0, len(rawBlocks))
...
func parseLegacyFantasyBlocks(role string, raw pqtype.NullRawMessage) ([]nicloudsdk.ChatMessagePart, error) {
	var rawBlocks []json.RawMessage
	if err := json.Unmarshal(raw.RawMessage, &rawBlocks); err != nil {
		return nil, xerrors.Errorf("parse %s content: %w", role, err)
	}
...
func hasNonEmptyType(parts []nicloudsdk.ChatMessagePart) bool {
	for _, p := range parts {
		if p.Type != "" {
			return true
		}
...
func hasToolResultType(parts []nicloudsdk.ChatMessagePart) bool {
	for _, p := range parts {
		if p.Type == nicloudsdk.ChatMessagePartTypeToolResult {
			return true
		}
	}
	return false
...
func PartFromContent(block fantasy.Content) nicloudsdk.ChatMessagePart {
...
func sdkPartFromContent(
	...
	case fantasy.SourceContent:
		return nicloudsdk.ChatMessagePart{
			Type:             nicloudsdk.ChatMessagePartTypeSource,
			SourceID:         value.ID,
			URL:              value.URL,
			Title:            value.Title,
			ProviderMetadata: marshalProviderMetadata(value.ProviderMetadata),
...
// database.
func ToolResultToPart(toolCallID, toolName string, result json.RawMessage, isError bool, isMedia bool) nicloudsdk.ChatMessagePart {
	return nicloudsdk.ChatMessageToolResult(toolCallID, toolName, result, isError, isMedia)
}
...
// Keep in sync with nicloud/x/chatd/subagent.go.
...
func MarshalParts(parts []nicloudsdk.ChatMessagePart) (pqtype.NullRawMessage, error) {
	if len(parts) == 0 {
		return pqtype.NullRawMessage{}, nil
	}
...
// representation.
func fileReferencePartToText(part nicloudsdk.ChatMessagePart) string {
...
func toolResultPartToMessagePart(logger slog.Logger, part nicloudsdk.ChatMessagePart) fantasy.ToolResultPart {
...
func partsToMessageParts(
	...
	for _, part := range parts {
		...
		case nicloudsdk.ChatMessagePartTypeReasoning:
			opts := providerMetadataToOptions(logger, part.ProviderMetadata)
		...
		case nicloudsdk.ChatMessagePartTypeToolCall:
		...
		case nicloudsdk.ChatMessagePartTypeToolResult:
			result = append(result, toolResultPartToMessagePart(logger, part))
		case nicloudsdk.ChatMessagePartTypeFile:
			data := part.Data
		...
		case nicloudsdk.ChatMessagePartTypeSource:
...
func encodeNulInParts(parts []nicloudsdk.ChatMessagePart) []nicloudsdk.ChatMessagePart {
	encoded := make([]nicloudsdk.ChatMessagePart, len(parts))
	copy(encoded, parts)
	for i := range encoded {
		p := &encoded[i]
...
func decodeNulInParts(parts []nicloudsdk.ChatMessagePart) {
	for i := range parts {
		p := &parts[i]
