...
func testMsg(role nicloudsdk.ChatMessageRole, raw pqtype.NullRawMessage) database.ChatMessage {
...
func TestConvertMessagesWithFilesPreservesEmptyRedactedReasoning(t *testing.T) {
	...
	content, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		...
		nicloudsdk.ChatMessageText("done"),
	})
	require.NoError(t, err)
...
func TestConvertMessagesWithFilesRoundTripsAnthropicInterleavedWebSearch(t *testing.T) {
			...
			ProviderMetadata: fantasy.ProviderMetadata{
				fantasyanthropic.Name: &fantasyanthropic.WebSearchResultMetadata{
					Results: []fantasyanthropic.WebSearchResultItem{
						{
							URL:              "https://cloud.neuralinverse.com",
							Title:            "Coder",
	...
	storedParts := make([]nicloudsdk.ChatMessagePart, 0, len(content))
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeReasoning, parsedParts[0].Type)
	require.Equal(t, nicloudsdk.ChatMessagePartTypeToolCall, parsedParts[1].Type)
	require.Equal(t, nicloudsdk.ChatMessagePartTypeToolResult, parsedParts[2].Type)
	require.Equal(t, nicloudsdk.ChatMessagePartTypeReasoning, parsedParts[3].Type)
	require.Equal(t, nicloudsdk.ChatMessagePartTypeText, parsedParts[4].Type)
...
func testMsgV1(role nicloudsdk.ChatMessageRole, raw pqtype.NullRawMessage) database.ChatMessage {
	return database.ChatMessage{
		Role:           database.ChatMessageRole(role),
		Content:        raw,
		ContentVersion: chatprompt.CurrentContentVersion,
...
func TestInjectMissingToolResults_SkipsProviderExecutedAndInjectsLocal(t *testing.T) {
	...
	providerCall := nicloudsdk.ChatMessageToolCall(
		"srvtoolu_web_search",
		"web_search",
		json.RawMessage(`{"query":"coder"}`),
	)
	providerCall.ProviderExecuted = true
	localCall := nicloudsdk.ChatMessageToolCall(
		"toolu_read",
		"read_file",
...
func TestInjectMissingToolResults_AdjacentAssistantsInjectLocalResults(t *testing.T) {
	...
	assistantAContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("assistant a"),
		nicloudsdk.ChatMessageToolCall(
			"toolu_a",
			"read_file",
	...
	require.NoError(t, err)
	assistantBContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("assistant b"),
		nicloudsdk.ChatMessageToolCall(
			"toolu_b",
			"read_file",
	...
	require.NoError(t, err)
	userContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("next user message"),
	})
	require.NoError(t, err)
...
func TestSDKPartsNeverProduceFantasyEnvelopeShape(t *testing.T) {
	...
	parts := []nicloudsdk.ChatMessagePart{
		...
		{Type: nicloudsdk.ChatMessagePartTypeFile, FileID: uuid.NullUUID{UUID: uuid.New(), Valid: true}, MediaType: "image/png"},
		{Type: nicloudsdk.ChatMessagePartTypeFile, MediaType: "image/png", Data: []byte("fake-image-data")},
		{Type: nicloudsdk.ChatMessagePartTypeFileReference, FileName: "main.go", StartLine: 1, EndLine: 10, Content: "func main() {}"},
		{Type: nicloudsdk.ChatMessagePartTypeReasoning, Text: "thinking..."},
		{Type: nicloudsdk.ChatMessagePartTypeToolCall, ToolCallID: "abc", ToolName: "read_file", Args: json.RawMessage(`{"path":"main.go"}`)},
		{Type: nicloudsdk.ChatMessagePartTypeToolResult, ToolCallID: "abc", ToolName: "read_file", Result: json.RawMessage(`{"output":"code"}`)},
		{Type: nicloudsdk.ChatMessagePartTypeSource, SourceID: "s1", URL: "https://example.com", Title: "Example"},
	}
...
func TestParseContent_BackwardCompat(t *testing.T) {
	...
	newAssistantWithMeta, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{{
		Type:             nicloudsdk.ChatMessagePartTypeText,
		Text:             "here is my answer",
		ProviderMetadata: sdkMetadata,
	}})
	require.NoError(t, err)

	newAssistantToolCall, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{{
	...
	newToolResult, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{{
	...
	tests := []struct {
		...
		check func(t *testing.T, parts []nicloudsdk.ChatMessagePart)
	}{
		{
			name: "system/plain_string",
			role: nicloudsdk.ChatMessageRoleSystem,
			raw:  nullRaw(mustJSON(t, "You are helpful.")),
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeText, parts[0].Type)
				assert.Equal(t, "You are helpful.", parts[0].Text)
			},
		},
		{
			name: "user/fantasy_text",
			role: nicloudsdk.ChatMessageRoleUser,
		...
		},
		{
			...
			role: nicloudsdk.ChatMessageRoleAssistant,
			raw: nullRaw(mustJSON(t, []json.RawMessage{
				mustJSON(t, map[string]any{
					"type": "text",
		...
		},
		{
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeFile, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeReasoning, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeSource, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeToolCall, parts[0].Type)
		...
		},
		{
			...
			role: nicloudsdk.ChatMessageRoleTool,
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeToolResult, parts[0].Type)
				assert.Equal(t, "call_123", parts[0].ToolCallID)
				assert.Equal(t, "read_file", parts[0].ToolName)
		...
		},
		{
			name: "user/sdk_text",
			role: nicloudsdk.ChatMessageRoleUser,
			raw: nullRaw(mustJSON(t, []nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "hello sdk"},
			})),
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeText, parts[0].Type)
		...
		},
		{
			...
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeFileReference, parts[0].Type)
		...
		},
		{
			...
			role: nicloudsdk.ChatMessageRoleUser,
			raw: nullRaw(mustJSON(t, []nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeFile, FileID: uuid.NullUUID{UUID: fileID, Valid: true}, MediaType: "image/png"},
			})),
			check: func(t *testing.T, parts []nicloudsdk.ChatMessagePart) {
				require.Len(t, parts, 1)
				assert.Equal(t, nicloudsdk.ChatMessagePartTypeFile, parts[0].Type)
...
func TestParseContent_V1(t *testing.T) {
	...
	t.Run("system", func(t *testing.T) {
		t.Parallel()
		raw, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("You are helpful."),
		})
		require.NoError(t, err)

		parts, err := chatprompt.ParseContent(testMsgV1(nicloudsdk.ChatMessageRoleSystem, raw))
		require.NoError(t, err)
		require.Len(t, parts, 1)
		assert.Equal(t, nicloudsdk.ChatMessagePartTypeText, parts[0].Type)
		assert.Equal(t, "You are helpful.", parts[0].Text)
	...
	t.Run("system_bare_string_errors", func(t *testing.T) {
		...
		_, err := chatprompt.ParseContent(testMsgV1(
			nicloudsdk.ChatMessageRoleSystem,
	...
	t.Run("unknown_version_errors", func(t *testing.T) {
		t.Parallel()
		msg := testMsgV1(nicloudsdk.ChatMessageRoleUser, nullRaw(json.RawMessage(`[{"type":"text","text":"hi"}]`)))
...
func TestProviderMetadataRoundTrip(t *testing.T) {
	...
	parts, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleAssistant, legacyContent))
	require.NoError(t, err)
	require.Len(t, parts, 1)
...
func TestFileReferencePreservation(t *testing.T) {
	...
	raw, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{{
		Type:      nicloudsdk.ChatMessagePartTypeFileReference,
		FileName:  "main.go",
		StartLine: 10,
		EndLine:   20,
		Content:   "func main() {}",
	...
	parts, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleUser, raw))
	require.NoError(t, err)
	require.Len(t, parts, 1)
	assert.Equal(t, nicloudsdk.ChatMessagePartTypeFileReference, parts[0].Type)
	assert.Equal(t, "main.go", parts[0].FileName)
...
func TestAssistantWriteRoundTrip(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeText, sdkPart.Type)
	require.NotNil(t, sdkPart.ProviderMetadata)

	raw, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{sdkPart})
	require.NoError(t, err)

	// Read back via ParseContent (takes the new SDK path, not
	// the legacy fallback, because the stored format is flat).
	parts, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleAssistant, raw))
	require.NoError(t, err)
	require.Len(t, parts, 1)
...
func TestMixedFormatConversation(t *testing.T) {
	...
	newUserRaw, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		{Type: nicloudsdk.ChatMessagePartTypeText, Text: "Check this diff."},
		{Type: nicloudsdk.ChatMessagePartTypeFileReference, FileName: "main.go", StartLine: 5, EndLine: 15, Content: "func main() {}"},
	})
	require.NoError(t, err)
	...
	newAssistantRaw, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		{Type: nicloudsdk.ChatMessagePartTypeText, Text: "Here is my analysis.", ProviderMetadata: newAssistantMeta},
	})
	require.NoError(t, err)
...
func TestQueuedMessageRoundTrip(t *testing.T) {
	...
	parts := []nicloudsdk.ChatMessagePart{
		{Type: nicloudsdk.ChatMessagePartTypeText, Text: "Review this change."},
		{Type: nicloudsdk.ChatMessagePartTypeFileReference, FileName: "api.go", StartLine: 42, EndLine: 58, Content: "func handleRequest() {}"},
	}
	...
	assert.Equal(t, nicloudsdk.ChatMessagePartTypeText, queuedMsg.Content[0].Type)
	assert.Equal(t, "Review this change.", queuedMsg.Content[0].Text)
	assert.Equal(t, nicloudsdk.ChatMessagePartTypeFileReference, queuedMsg.Content[1].Type)
	...
	promoted, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleUser, pqtype.NullRawMessage{
		RawMessage: raw.RawMessage,
		Valid:      true,
	}))
	require.NoError(t, err)
	require.Len(t, promoted, 2)
	assert.Equal(t, nicloudsdk.ChatMessagePartTypeText, promoted[0].Type)
	assert.Equal(t, "Review this change.", promoted[0].Text)
	assert.Equal(t, nicloudsdk.ChatMessagePartTypeFileReference, promoted[1].Type)
...
func TestParseContent_ErrorPaths(t *testing.T) {
	...
	t.Run("null_content_returns_nil", func(t *testing.T) {
		t.Parallel()
		parts, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleUser, pqtype.NullRawMessage{}))
		require.NoError(t, err)
		assert.Nil(t, parts)
	})
	...
	t.Run("empty_content_returns_nil", func(t *testing.T) {
		t.Parallel()
		parts, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleAssistant, pqtype.NullRawMessage{
			RawMessage: []byte{},
			Valid:      true,
		}))
		require.NoError(t, err)
	...
	t.Run("unknown_role", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRole("banana"), nullRaw(json.RawMessage(`"hello"`))))
		require.Error(t, err)
	...
	t.Run("system/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleSystem, nullRaw(json.RawMessage(`not json`))))
		require.Error(t, err)
	...
	t.Run("user/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleUser, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})

	t.Run("assistant/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleAssistant, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})

	t.Run("tool/malformed_json", func(t *testing.T) {
		t.Parallel()
		_, err := chatprompt.ParseContent(testMsg(nicloudsdk.ChatMessageRoleTool, nullRaw(json.RawMessage(`{not json`))))
		require.Error(t, err)
	})
}
...
func TestNulEscapeRoundTrip(t *testing.T) {
	...
	for _, tc := range textTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			parts := []nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText(tc.input),
			}
			...
			msg := testMsgV1(nicloudsdk.ChatMessageRoleAssistant, encoded)
	...
	t.Run("ToolResultWithNul", func(t *testing.T) {
		...
		parts := []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult("call-1", "my_tool", resultJSON, false, false),
		}
		...
		msg := testMsgV1(nicloudsdk.ChatMessageRoleTool, encoded)
		decoded, err := chatprompt.ParseContent(msg)
		require.NoError(t, err)
		require.Len(t, decoded, 1)
	...
	t.Run("MultiPartMixed", func(t *testing.T) {
		...
		parts := []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("clean text"),
			nicloudsdk.ChatMessageText("has\x00nul"),
		}
...
func TestConvertMessagesWithFiles_FiltersEmptyTextAndReasoningParts(t *testing.T) {
	...
	makeMsg := func(t *testing.T, role database.ChatMessageRole, parts []nicloudsdk.ChatMessagePart) database.ChatMessage {
		t.Helper()
		encoded, err := chatprompt.MarshalParts(parts)
		require.NoError(t, err)
		return database.ChatMessage{
	...
	t.Run("UserRole", func(t *testing.T) {
		...
		parts := []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText(""),                     // empty, filtered
			nicloudsdk.ChatMessageText("   \t\n "),             // whitespace, filtered
			nicloudsdk.ChatMessageReasoning(""),                // empty, filtered
			nicloudsdk.ChatMessageReasoning("  \n"),            // whitespace, filtered
			nicloudsdk.ChatMessageText("hello"),                // kept
			nicloudsdk.ChatMessageText("  hello  "),            // kept with original whitespace
			nicloudsdk.ChatMessageReasoning("thinking deeply"), // kept
			nicloudsdk.ChatMessageToolCall("call-1", "my_tool", json.RawMessage(`{"x":1}`)),
			nicloudsdk.ChatMessageToolResult("call-1", "my_tool", json.RawMessage(`{"ok":true}`), false, false),
		}
	...
	t.Run("AssistantRole", func(t *testing.T) {
		...
		parts := []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText(""),          // empty, filtered
			nicloudsdk.ChatMessageText(" "),         // whitespace, filtered
			nicloudsdk.ChatMessageReasoning(""),     // empty, filtered
			nicloudsdk.ChatMessageText("  reply  "), // kept with whitespace
			nicloudsdk.ChatMessageToolCall("tc-1", "read_file", json.RawMessage(`{"path":"x"}`)),
		}
...
func TestConvertMessagesWithFiles_AssistantAttachmentIsNotReplayed(t *testing.T) {
	...
	userContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageFile(userFileID, "image/png", "user.png"),
	})
	require.NoError(t, err)

	assistantContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("I attached logs above."),
		nicloudsdk.ChatMessageFile(assistantFileID, "text/plain", "agent.log"),
	})
	require.NoError(t, err)
...
func TestMediaToolResultRoundTrip(t *testing.T) {
	...
	insertPair := func(
		...
		resultParts []nicloudsdk.ChatMessagePart,
	) database.Chat {
		t.Helper()
		...
		callPart := nicloudsdk.ChatMessageToolCall(callID, toolName, json.RawMessage(`{}`))
		assistantEncoded, encErr := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{callPart})
	...
	t.Run("MediaResultRoundTripsAsMedia", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{sdkPart})
	...
	t.Run("TextResultStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, textResult, false, false),
		})
	...
	t.Run("MissingMimeTypeStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, noMimeJSON, false, false),
		})
	...
	t.Run("MissingDataStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, noDataJSON, false, false),
		})
	...
	t.Run("NonMediaResultTypeStaysText", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, textJSON, false, false),
		})
	...
	t.Run("IsMediaTrueButMissingMimeType", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, noMimeJSON, false, true),
		})
	...
	t.Run("IsMediaTrueButMissingData", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, noDataJSON, false, true),
		})
	...
	t.Run("IsMediaTrueButGarbageJSON", func(t *testing.T) {
		...
		chat := insertPair(t, callID, toolName, []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageToolResult(callID, toolName, garbageJSON, false, true),
		})
...
func TestPartFromContent_ExecuteToolParsedCommands(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, nicloudsdk.ChatMessagePartTypeToolCall, part.Type)
