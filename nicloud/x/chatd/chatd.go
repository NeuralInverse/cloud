...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatloop"
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatretry"
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatsanitize"
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/mcpclient"
	skillspkg "github.com/NeuralInverse/cloud/v2/nicloud/x/skills"
...
func (p *Server) loadAdvisorConfig(ctx context.Context, logger slog.Logger) nicloudsdk.AdvisorConfig {
	cfg, err := p.configCache.AdvisorConfig(ctx)
	if err != nil {
		logger.Warn(ctx, "failed to load advisor config", slog.Error(err))
		return nicloudsdk.AdvisorConfig{}
	}
	return cfg
}
...
func (p *Server) resolveAdvisorModelOverride(
	ctx context.Context,
	chat database.Chat,
	advisorCfg nicloudsdk.AdvisorConfig,
	fallbackModel fantasy.LanguageModel,
	fallbackCallConfig nicloudsdk.ChatModelCallConfig,
	providerKeys chatprovider.ProviderAPIKeys,
	modelOpts modelBuildOptions,
	logger slog.Logger,
) (fantasy.LanguageModel, nicloudsdk.ChatModelCallConfig, error) {
	...
	overrideCallConfig := nicloudsdk.ChatModelCallConfig{}
	...
	if err != nil {
		if p.shouldUseAIGatewayRouting() && overrideConfig.AIProviderID.Valid {
			return nil, nicloudsdk.ChatModelCallConfig{}, xerrors.Errorf("resolve advisor override route: %w", err)
		}
		logger.Warn(
			ctx,
	...
	overrideModel, err := p.newModel(ctx, modelClientRequest{
		...
		ExtraHeaders: chatprovider.NIHeaders(chat),
	}, route, modelOpts)
	if err != nil {
		if p.shouldUseAIGatewayRouting() && overrideConfig.AIProviderID.Valid {
			return nil, nicloudsdk.ChatModelCallConfig{}, xerrors.Errorf("create advisor override model: %w", err)
		}
		logger.Warn(
			ctx,
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if c.conn == nil {
			...
			agentConn.SetExtraHeaders(http.Header{
				workspacesdk.NIChatIDHeader:          {chatSnapshot.ID.String()},
				workspacesdk.NIAncestorChatIDsHeader: {string(ancestorJSON)},
			})
...
) <-chan nicloudsdk.ChatStreamEvent
...
type bufferedStreamPart struct {
	event nicloudsdk.ChatStreamEvent
...
type chatStreamState struct {
	...
	buffering            bool
	durableMessages      []nicloudsdk.ChatStreamEvent
	durableEvictedBefore int64 // highest message ID evicted from durable cache
	subscribers          map[uuid.UUID]chan nicloudsdk.ChatStreamEvent
	...
	// to retry.
	currentRetry *nicloudsdk.ChatStreamRetry
...
func (p *Server) CreateChat(ctx context.Context, opts CreateOptions) (database.Chat, error) {
	...
	txErr := p.db.InTx(func(tx database.Store) error {
		...
		workspaceAwarenessContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText(workspaceAwareness),
		})
		if err != nil {
		...
		if deploymentPrompt != "" {
			deploymentContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText(deploymentPrompt),
			})
			if err != nil {
		...
		if userPrompt != "" {
			userPromptContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText(userPrompt),
			})
			if err != nil {
	...
	p.publishChatPubsubEvent(chat, nicloudsdk.ChatWatchEventKindCreated, nil)
	p.signalWake()
	return chat, nil
}
...
func (p *Server) SendMessage(
	...
	var (
		...
		queuedMessagesSDK []nicloudsdk.ChatQueuedMessage
	)
	...
	if result.Queued {
		p.publishEvent(opts.ChatID, nicloudsdk.ChatStreamEvent{
			Type:           nicloudsdk.ChatStreamEventTypeQueueUpdate,
			ChatID:         opts.ChatID,
			QueuedMessages: queuedMessagesSDK,
		})
		p.publishChatStreamNotify(opts.ChatID, nicloudpubsub.ChatStreamNotifyMessage{
			QueueUpdate: true,
		})
	...
	p.publishChatPubsubEvent(result.Chat, nicloudsdk.ChatWatchEventKindStatusChange, nil)
	p.signalWake()
	return result, nil
}
...
func (p *Server) EditMessage(
	...
	p.publishEvent(opts.ChatID, nicloudsdk.ChatStreamEvent{
		Type:           nicloudsdk.ChatStreamEventTypeQueueUpdate,
		QueuedMessages: []nicloudsdk.ChatQueuedMessage{},
	})
	p.publishChatStreamNotify(opts.ChatID, nicloudpubsub.ChatStreamNotifyMessage{
		QueueUpdate: true,
	})
...
func (p *Server) ArchiveChat(ctx context.Context, chat database.Chat) error {
	...
	for _, interruptedChat := range interruptedChats {
		...
		p.publishChatPubsubEvent(interruptedChat, nicloudsdk.ChatWatchEventKindStatusChange, nil)
	}
	...
	p.publishChatPubsubEvents(archivedChats, nicloudsdk.ChatWatchEventKindDeleted)
	return nil
}
...
func (p *Server) UnarchiveChat(ctx context.Context, chat database.Chat) error {
	...
	if !chat.ParentChatID.Valid {
		return p.applyChatLifecycleTransition(
			...
			chat.ID,
			"unarchive",
			nicloudsdk.ChatWatchEventKindCreated,
	...
	p.publishChatPubsubEvents(updated, nicloudsdk.ChatWatchEventKindCreated)
	return nil
}
...
func (p *Server) applyChatLifecycleTransition(
	...
	chatID uuid.UUID,
	action string,
	kind nicloudsdk.ChatWatchEventKind,
...
func (p *Server) DeleteQueued(
	...
	if queueLoadedOK {
		p.publishEvent(chatID, nicloudsdk.ChatStreamEvent{
			Type:           nicloudsdk.ChatStreamEventTypeQueueUpdate,
	...
	p.publishChatStreamNotify(chatID, nicloudpubsub.ChatStreamNotifyMessage{
		QueueUpdate: true,
	})
	return nil
}
...
func (p *Server) PromoteQueued(
	...
	if deferred {
		...
		p.publishChatPubsubEvent(updatedChat, nicloudsdk.ChatWatchEventKindStatusChange, nil)
		return result, nil
	}
	...
	txErr := p.db.InTx(func(tx database.Store) error {
		...
		for _, msg := range afterMsgs {
			...
			for _, mp := range msgParts {
				if mp.Type == nicloudsdk.ChatMessagePartTypeToolResult {
		...
		for _, part := range parts {
			if part.Type == nicloudsdk.ChatMessagePartTypeToolCall &&
		...
		for _, result := range opts.Results {
			...
			marshaled, marshalErr := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{part})
			if marshalErr != nil {
...
func (p *Server) PublishTitleChange(chat database.Chat) {
	p.publishChatPubsubEvent(chat, nicloudsdk.ChatWatchEventKindTitleChange, nil)
}
...
func (p *Server) regenerateChatTitleWithStore(
	...
	p.publishChatPubsubEvent(updatedChat, nicloudsdk.ChatWatchEventKindTitleChange, nil)
	return updatedChat, nil
}
...
func fantasyUsageToChatMessageUsage(usage fantasy.Usage) nicloudsdk.ChatMessageUsage {
	var chatUsage nicloudsdk.ChatMessageUsage
...
func New(cfg Config) *Server {
	...
	if p.pubsub != nil {
		cancelConfigSub, err := p.pubsub.SubscribeWithErr(
			nicloudpubsub.ChatConfigEventChannel,
			nicloudpubsub.HandleChatConfigEvent(func(ctx context.Context, ev nicloudpubsub.ChatConfigEvent, err error) {
				if err != nil {
					p.logger.Warn(ctx, "chat config event error", slog.Error(err))
					return
				}
				switch ev.Kind {
				case nicloudpubsub.ChatConfigEventProviders:
					p.configCache.InvalidateProviders()
				case nicloudpubsub.ChatConfigEventModelConfig:
					p.configCache.InvalidateModelConfig(ev.EntityID)
				case nicloudpubsub.ChatConfigEventUserPrompt:
					p.configCache.InvalidateUserPrompt(ev.EntityID)
				case nicloudpubsub.ChatConfigEventAdvisorConfig:
...
func shouldClearRetryPhaseForStatus(status nicloudsdk.ChatStatus) bool {
	switch status {
	case nicloudsdk.ChatStatusWaiting,
		nicloudsdk.ChatStatusPending,
		nicloudsdk.ChatStatusPaused,
		nicloudsdk.ChatStatusCompleted,
		nicloudsdk.ChatStatusError,
		nicloudsdk.ChatStatusRequiresAction:
		return true
	default:
		return false
	}
...
func (p *Server) publishToStream(chatID uuid.UUID, event nicloudsdk.ChatStreamEvent) {
	...
	subscribers := make([]chan nicloudsdk.ChatStreamEvent, 0, len(state.subscribers))
...
func (p *Server) cacheDurableMessage(chatID uuid.UUID, event nicloudsdk.ChatStreamEvent) {
	state := p.getOrCreateStreamState(chatID)
	state.mu.Lock()
	...
	if len(state.durableMessages) >= maxDurableMessageCacheSize {
		...
		state.durableMessages[0] = nicloudsdk.ChatStreamEvent{}
...
func (p *Server) getCachedDurableMessages(
	...
	var result []nicloudsdk.ChatStreamEvent
...
func snapshotBufferLocked(buffer []bufferedStreamPart) []nicloudsdk.ChatStreamEvent {
	if len(buffer) == 0 {
		return nil
	}
	snapshot := make([]nicloudsdk.ChatStreamEvent, 0, len(buffer))
...
func (p *Server) subscribeToStream(chatID uuid.UUID) (
	...
	*nicloudsdk.ChatStreamRetry,
	...
	var currentRetry *nicloudsdk.ChatStreamRetry
	if state.currentRetry != nil {
		retryCopy := *state.currentRetry
		currentRetry = &retryCopy
	}
	id := uuid.New()
	ch := make(chan nicloudsdk.ChatStreamEvent, 128)
...
func (p *Server) getOrCreateStreamState(chatID uuid.UUID) *chatStreamState {
	...
	val, _ := p.chatStreams.LoadOrStore(chatID, &chatStreamState{
		subscribers: make(map[uuid.UUID]chan nicloudsdk.ChatStreamEvent),
	})
	state, _ := val.(*chatStreamState)
	return state
}
...
func subscribeWithInitialError(chatID uuid.UUID, message string) (
	...
	events := make(chan nicloudsdk.ChatStreamEvent)
	close(events)
	return []nicloudsdk.ChatStreamEvent{{
		Type:   nicloudsdk.ChatStreamEventTypeError,
		ChatID: chatID,
		Error:  &nicloudsdk.ChatError{Message: message},
	}}, events, func() {}, true
}
...
func (p *Server) SubscribeAuthorized(
	...
	mergedEvents := make(chan nicloudsdk.ChatStreamEvent, 128)
	...
	var notifications <-chan nicloudpubsub.ChatStreamNotifyMessage
	var errCh <-chan error
	if p.pubsub != nil {
		notifyCh := make(chan nicloudpubsub.ChatStreamNotifyMessage, 10)
		...
		listener := func(_ context.Context, message []byte, listenErr error) {
			...
			var notify nicloudpubsub.ChatStreamNotifyMessage
		...
		if pubsubCancel, pubsubErr := p.pubsub.SubscribeWithErr(
			nicloudpubsub.ChatStreamNotifyChannel(chatID),
			listener,
		); pubsubErr == nil {
	...
	// window.
	initialSnapshot := make([]nicloudsdk.ChatStreamEvent, 0)
	...
	var retryEvent *nicloudsdk.ChatStreamEvent
	if localRetry != nil {
		retryEvent = &nicloudsdk.ChatStreamEvent{
			Type:   nicloudsdk.ChatStreamEventTypeRetry,
			ChatID: chatID,
			Retry:  localRetry,
		}
	}
	...
	if err != nil {
		...
		initialSnapshot = append(initialSnapshot, nicloudsdk.ChatStreamEvent{
			Type:   nicloudsdk.ChatStreamEventTypeError,
			ChatID: chatID,
			Error:  &nicloudsdk.ChatError{Message: "failed to load initial snapshot"},
		})
	} else {
		for _, msg := range messages {
	...
	// query.
	statusEvent := nicloudsdk.ChatStreamEvent{
		Type:   nicloudsdk.ChatStreamEventTypeStatus,
		ChatID: chatID,
		Status: &nicloudsdk.ChatStreamStatus{
			Status: nicloudsdk.ChatStatus(snapshotChat.Status),
		},
	}
	// Prepend so the frontend sees the current stream phases
	// before any message_part events.
	prefix := []nicloudsdk.ChatStreamEvent{statusEvent}
	if retryEvent != nil {
	...
	var relayEvents <-chan nicloudsdk.ChatStreamEvent
	...
	go func() {
		...
		for {
			...
			case psErr := <-errCh:
				...
				select {
				case mergedEvents <- nicloudsdk.ChatStreamEvent{
					Type:   nicloudsdk.ChatStreamEventTypeError,
					ChatID: chatID,
					Error: &nicloudsdk.ChatError{
						Message: psErr.Error(),
					},
				}:
				case <-mergedCtx.Done():
			...
			case notify := <-notifications:
				...
				if notify.Status != "" {
					...
					case mergedEvents <- nicloudsdk.ChatStreamEvent{
						Type:   nicloudsdk.ChatStreamEventTypeStatus,
						ChatID: chatID,
						Status: &nicloudsdk.ChatStreamStatus{Status: nicloudsdk.ChatStatus(status)},
					}:
					}
			...
			case event, ok := <-localParts:
				...
				if hasPubsub {
					...
					if event.Type == nicloudsdk.ChatStreamEventTypeMessagePart ||
						event.Type == nicloudsdk.ChatStreamEventTypeStatus ||
						event.Type == nicloudsdk.ChatStreamEventTypeActionRequired {
						select {
						case <-mergedCtx.Done():
							return
						case mergedEvents <- event:
...
func (p *Server) publishEvent(chatID uuid.UUID, event nicloudsdk.ChatStreamEvent) {
...
func (p *Server) publishStatus(chatID uuid.UUID, status database.ChatStatus, workerID uuid.NullUUID) {
	...
	notify := nicloudpubsub.ChatStreamNotifyMessage{
...
func (p *Server) publishChatStreamNotify(chatID uuid.UUID, notify nicloudpubsub.ChatStreamNotifyMessage) {
	if p.pubsub == nil {
		return
	}
	...
	if err := p.pubsub.Publish(nicloudpubsub.ChatStreamNotifyChannel(chatID), payload); err != nil {
...
func (p *Server) publishChatPubsubEvents(chats []database.Chat, kind nicloudsdk.ChatWatchEventKind) {
	for _, chat := range chats {
...
func (p *Server) publishChatPubsubEvent(chat database.Chat, kind nicloudsdk.ChatWatchEventKind, diffStatus *nicloudsdk.ChatDiffStatus) {
	if p.pubsub == nil {
		return
	}
	...
	if err := p.pubsub.Publish(nicloudpubsub.ChatWatchEventChannel(chat.OwnerID), payload); err != nil {
...
func pendingToStreamToolCalls(pending []chatloop.PendingToolCall) []nicloudsdk.ChatStreamToolCall {
	calls := make([]nicloudsdk.ChatStreamToolCall, len(pending))
	for i, tc := range pending {
		calls[i] = nicloudsdk.ChatStreamToolCall{
...
func (p *Server) publishChatActionRequired(chat database.Chat, pending []chatloop.PendingToolCall) {
	...
	event := nicloudsdk.ChatWatchEvent{
		Kind:      nicloudsdk.ChatWatchEventKindActionRequired,
		Chat:      sdkChat,
		ToolCalls: toolCalls,
	}
	payload, err := json.Marshal(event)
...
func (p *Server) PublishDiffStatusChange(ctx context.Context, chatID uuid.UUID) error {
	...
	p.publishChatPubsubEvent(chat, nicloudsdk.ChatWatchEventKindDiffStatusChange, &sdkStatus)
	return nil
}

func (p *Server) publishRetry(chatID uuid.UUID, payload *nicloudsdk.ChatStreamRetry) {
	if payload == nil {
		return
	}
	p.publishEvent(chatID, nicloudsdk.ChatStreamEvent{
...
func encodeChatLastErrorPayload(payload *nicloudsdk.ChatError) (pqtype.NullRawMessage, error) {
	if payload == nil {
...
func (p *Server) publishEditedMessage(chatID uuid.UUID, message database.ChatMessage) {
	...
	state := p.getOrCreateStreamState(chatID)
	state.mu.Lock()
	state.durableMessages = []nicloudsdk.ChatStreamEvent{event}
...
func (p *Server) publishMessagePart(chatID uuid.UUID, role nicloudsdk.ChatMessageRole, part nicloudsdk.ChatMessagePart) {
	if part.Type == "" {
		return
	}
	...
	p.publishEvent(chatID, nicloudsdk.ChatStreamEvent{
		...
		MessagePart: &nicloudsdk.ChatStreamMessagePart{
			Role: role,
			Part: part,
		},
	})
		...
		for _, f := range files {
			if hasImageCap &&
				...
				return nil, chaterror.WithClassification(err, chaterror.ClassifiedError{
					Kind:     nicloudsdk.ChatErrorKindConfig,
...
func (p *Server) processChat(ctx context.Context, chat database.Chat) {
	...
	var lastErrorPayload *nicloudsdk.ChatError
	...
	defer func() {
		...
		if shouldPublishQueueUpdate {
			p.publishEvent(chat.ID, nicloudsdk.ChatStreamEvent{
				Type:           nicloudsdk.ChatStreamEventTypeQueueUpdate,
				QueuedMessages: db2sdk.ChatQueuedMessages(remainingQueuedMessages),
			})
			p.publishChatStreamNotify(chat.ID, nicloudpubsub.ChatStreamNotifyMessage{
				QueueUpdate: true,
			})
		}
		if p.shouldPublishFinishedChatState(cleanupCtx, logger, finishResult.updatedChat) {
			...
			p.publishChatPubsubEvent(finishResult.updatedChat, nicloudsdk.ChatWatchEventKindStatusChange, nil)
		}
		...
		if status == database.ChatStatusRequiresAction && len(runResult.PendingDynamicToolCalls) > 0 {
			...
			p.publishEvent(chat.ID, nicloudsdk.ChatStreamEvent{
				Type: nicloudsdk.ChatStreamEventTypeActionRequired,
				ActionRequired: &nicloudsdk.ChatStreamActionRequired{
					ToolCalls: toolCalls,
				},
			})
...
func (p *Server) fetchPersonalSkillMetadata(
	...
	// See package nicloud/x/skills (doc.go) for why metadata fetch failures
...
func appendDynamicTools(
	...
	var dynamicToolDefs []nicloudsdk.DynamicTool
	if raw.Valid {
	...
	var filteredDefs []nicloudsdk.DynamicTool
...
func (p *Server) runChat(
	...
	persistStep := func(persistCtx context.Context, step chatloop.PersistedStep) error {
		...
		for i, tr := range toolResults {
			...
			var marshalErr error
			toolResultContents[i], marshalErr = chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{trPart})
			if marshalErr != nil {
	...
	compactionOptions = &chatloop.CompactionOptions{
		...
		PublishMessagePart: func(role nicloudsdk.ChatMessageRole, part nicloudsdk.ChatMessagePart) {
	...
	if advisorRuntime != nil {
		tools = append(tools, chatadvisor.Tool(chatadvisor.ToolOptions{
			...
			PublishAdviceDelta: func(toolCallID string, delta string) {
				...
				p.publishMessagePart(chat.ID, nicloudsdk.ChatMessageRoleTool, nicloudsdk.ChatMessagePart{
					Type:        nicloudsdk.ChatMessagePartTypeToolResult,
	...
	loopErr = chatloop.Run(ctx, chatloop.RunOptions{
		...
		PublishMessagePart: func(
			role nicloudsdk.ChatMessageRole,
			part nicloudsdk.ChatMessagePart,
		) {
			if part.ToolName != "" {
...
func buildProviderTools(options *nicloudsdk.ChatModelProviderOptions) []chatloop.ProviderTool {
...
func (p *Server) persistChatContextSummary(
	...
	systemContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText(result.SystemSummary),
	})
	if err != nil {
	...
	assistantContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageToolCall(toolCallID, "chat_summarized", args),
	})
	if err != nil {
	...
	toolResult, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageToolResult(toolCallID, "chat_summarized", summaryResult, false, false),
	})
	if err != nil {
...
func contextFileAgentID(messages []database.ChatMessage) (uuid.UUID, bool) {
	...
	for _, msg := range messages {
		...
		for _, p := range parts {
			if p.Type != nicloudsdk.ChatMessagePartTypeContextFile ||
...
func (p *Server) fetchWorkspaceContext(
...
) (agent *database.WorkspaceAgent, agentParts []nicloudsdk.ChatMessagePart, discoveredSkills []chattool.SkillMeta, workspaceConnOK bool) {
	...
	for i := range agentParts {
		...
		case nicloudsdk.ChatMessagePartTypeContextFile:
		...
		case nicloudsdk.ChatMessagePartTypeSkill:
...
func (p *Server) persistInstructionFiles(
	...
	for _, part := range agentParts {
		if part.Type == nicloudsdk.ChatMessagePartTypeContextFile {
	...
	if !hasContent {
		...
		if !hasContextFilePart {
			agentParts = append([]nicloudsdk.ChatMessagePart{{
	...
	stripped := make([]nicloudsdk.ChatMessagePart, len(agentParts))
...
// the turn.
func (p *Server) updateLastInjectedContext(ctx context.Context, chatID uuid.UUID, parts []nicloudsdk.ChatMessagePart) {
...
func insertSyntheticToolResultsTx(
	...
	for _, part := range parts {
		if part.Type != nicloudsdk.ChatMessagePartTypeToolCall || !dynamicToolNames[part.ToolName] {
			continue
		}
		if handledCallIDs[part.ToolCallID] {
			continue
		}
		resultPart := nicloudsdk.ChatMessagePart{
			Type:       nicloudsdk.ChatMessagePartTypeToolResult,
		...
		marshaled, marshalErr := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{resultPart})
		if marshalErr != nil {
...
func parseDynamicToolNames(raw pqtype.NullRawMessage) (map[string]bool, error) {
	...
	var tools []nicloudsdk.DynamicTool
...
func (p *Server) updateLastTurnSummary(
	...
	p.publishChatPubsubEvent(updatedChat, nicloudsdk.ChatWatchEventKindSummaryChange, nil)
...
func (p *Server) dispatchPush(
	...
	pushMsg := nicloudsdk.WebpushMessage{
		Title: chat.Title,
		Body:  body,
		Icon:  "/favicon.ico",
