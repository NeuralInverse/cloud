...
type cachedAdvisorConfig struct {
	config    nicloudsdk.AdvisorConfig
	expiresAt time.Time
}
...
type chatConfigCache struct {
	...
	advisorConfigFetches    singleflight.Group[string, nicloudsdk.AdvisorConfig]
}
...
func (c *chatConfigCache) AdvisorConfig(ctx context.Context) (nicloudsdk.AdvisorConfig, error) {
	...
	config, err := singleflightDoChan(
		...
		func() (nicloudsdk.AdvisorConfig, error) {
			...
			if err != nil {
				return nicloudsdk.AdvisorConfig{}, err
			}
			var cfg nicloudsdk.AdvisorConfig
...
func (c *chatConfigCache) cachedAdvisorConfig() (nicloudsdk.AdvisorConfig, bool) {
	c.mu.RLock()
	entry := c.advisorConfig
	c.mu.RUnlock()
	if entry == nil {
		return nicloudsdk.AdvisorConfig{}, false
	}
	if c.clock.Now().Before(entry.expiresAt) {
...
func (c *chatConfigCache) storeAdvisorConfig(generation uint64, config nicloudsdk.AdvisorConfig) {
	c.mu.Lock()
	defer c.mu.Unlock()
