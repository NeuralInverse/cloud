...
func TestSendSubagentMessagePropagatesActiveTurnAPIKeyID(t *testing.T) {
	...
	child, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("do work"),
		},
	})
	require.NoError(t, err)
...
func TestCreateChildSubagentChatCopiesPlanMode(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		PlanMode:       planMode,
		InitialUserContent: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("plan this change"),
		},
	})
	require.NoError(t, err)
...
func TestSpawnAgent_GeneralHonorsPersonalModelOverrides(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			enablePersonalOverride: true,
			personalRaw: func(database.ChatModelConfig) string {
				return string(nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault)
			},
			wantModelID: func(_, deploymentModel, _ database.ChatModelConfig) uuid.UUID {
				return deploymentModel.ID
			},
		},
		{
			...
			enablePersonalOverride: true,
			personalRaw: func(database.ChatModelConfig) string {
				return string(nicloudsdk.ChatPersonalModelOverrideModeChatDefault)
			},
		...
		},
		{
			...
			personalRaw: func(personalModel database.ChatModelConfig) string {
				return string(nicloudsdk.ChatPersonalModelOverrideModeModel) + ":" +
...
func TestSpawnAgent_GeneralOverrideLogsAndFallsBackWhenCredentialsUnavailable(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:  model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("delegate work"),
		},
	})
	require.NoError(t, err)
	parentChat, err := db.GetChatByID(ctx, parent.ID)
...
func TestCreateChat_ExploreRootStartsWithoutMCPSnapshot(t *testing.T) {
	...
	root, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("inspect the codebase")},
	})
	require.NoError(t, err)
...
func TestSpawnAgent_PlanModeDescriptionOmitsComputerUse(t *testing.T) {
	...
	parent, err := server.CreateChat(ctx, CreateOptions{
		...
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("plan this change")},
	})
	require.NoError(t, err)
	parentChat, err := db.GetChatByID(ctx, parent.ID)
	require.NoError(t, err)
...
func TestSubagentLifecycleToolErrorsIncludePersistedSubagentType(t *testing.T) {
	...
	unrelated, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("other")},
	})
	require.NoError(t, err)
...
func TestIsSubagentDescendant(t *testing.T) {
	...
	root, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "root",
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("root")},
	})
	require.NoError(t, err)
	...
	child, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "child",
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("child")},
	})
	require.NoError(t, err)
	...
	grandchild, err := server.CreateChat(ctx, CreateOptions{
		...
		Title:              "grandchild",
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("grandchild")},
	})
	require.NoError(t, err)
	...
	unrelated, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("unrelated")},
	})
	require.NoError(t, err)
	...
	unrelatedChild, err := server.CreateChat(ctx, CreateOptions{
		...
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("unrelated-child")},
	})
	require.NoError(t, err)
...
func createParentChildChats(
	...
	child, err = server.CreateChat(ctx, CreateOptions{
		...
		Title:              "child-" + t.Name(),
		ModelConfigID:      model.ID,
		InitialUserContent: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("do work")},
	})
	require.NoError(t, err)
...
func setChatStatus(
	...
	if lastError != "" {
		encodedLastError, err := json.Marshal(nicloudsdk.ChatError{
			Message: lastError,
...
func insertAssistantMessage(
	...
	parts := []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText(text)}
...
func TestAwaitSubagentCompletion(t *testing.T) {
	...
	t.Run("CompletesViaPubsub", func(t *testing.T) {
		...
		cancelProbe, err := ps.SubscribeWithErr(
			nicloudpubsub.ChatStreamNotifyChannel(child.ID),
