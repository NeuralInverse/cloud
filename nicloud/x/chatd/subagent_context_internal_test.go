...
func TestCollectContextPartsFromMessagesSkipsSentinelContextFiles(t *testing.T) {
	...
	content, err := json.Marshal([]nicloudsdk.ChatMessagePart{
		...
		nicloudsdk.ChatMessageText("ignored"),
	})
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeSkill, parts[0].Type)
	require.Equal(t, "my-skill", parts[0].SkillName)
	require.Equal(t, nicloudsdk.ChatMessagePartTypeContextFile, parts[1].Type)
...
func createParentChatWithInheritedContext(
	...
	inheritedParts := []nicloudsdk.ChatMessagePart{
		{
			Type:                 nicloudsdk.ChatMessagePartTypeContextFile,
			ContextFilePath:      "/home/coder/project/AGENTS.md",
			ContextFileContent:   "# Project instructions",
...
func assertChildInheritedContext(
	...
	var cached []nicloudsdk.ChatMessagePart
	...
	for i, msg := range childMessages {
		...
		if len(parts) == 1 && parts[0].Type == nicloudsdk.ChatMessagePartTypeText && parts[0].Text == prompt {
...
func createParentChatWithRotatedInheritedContext(
	...
	oldAgentID := uuid.New()
	newAgentID := uuid.New()
	oldContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
		{
			Type:                 nicloudsdk.ChatMessagePartTypeContextFile,
	...
	require.NoError(t, err)
	newContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
		{
			Type:                 nicloudsdk.ChatMessagePartTypeContextFile,
...
func TestCreateChildSubagentChatCopiesOnlyLatestAgentContext(t *testing.T) {
	...
	var inherited [][]nicloudsdk.ChatMessagePart
	for _, msg := range childMessages {
		...
		var parts []nicloudsdk.ChatMessagePart
		require.NoError(t, json.Unmarshal(msg.Content.RawMessage, &parts))
		if len(parts) == 0 || parts[0].Type == nicloudsdk.ChatMessagePartTypeText {
			continue
		}
