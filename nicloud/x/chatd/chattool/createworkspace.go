	...
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, args createWorkspaceArgs, _ fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if err != nil {
				...
				d, parseErr := nicloudsdk.ParseChatWorkspaceTTL(raw)
				if parseErr != nil {
			...
			} else if err := nicloudsdk.NameValid(name); err != nil {
			...
			for k, v := range args.Parameters {
				createReq.RichParameterValues = append(
					...
					nicloudsdk.WorkspaceBuildParameter{Name: k, Value: v},
				)
			}
...
func waitForBuild(
	...
	for {
		...
		case database.ProvisionerJobStatusFailed:
			...
			var code nicloudsdk.JobErrorCode
			if job.ErrorCode.Valid {
				code = nicloudsdk.JobErrorCode(job.ErrorCode.String)
			}
...
func generatedWorkspaceName(seed string) string {
	base := nicloudsdk.UsernameFrom(strings.TrimSpace(strings.ToLower(seed)))
	...
	if err := nicloudsdk.NameValid(name); err == nil {
		return name
	}
