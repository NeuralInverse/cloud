...
func TestStopWorkspace(t *testing.T) {
	...
	t.Run("NoWorkspace", func(t *testing.T) {
		...
		tool := chattool.StopWorkspace(db, chat.ID, chattool.StopWorkspaceOptions{
			StopFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
	...
	t.Run("RunningWorkspaceStops", func(t *testing.T) {
		...
		tool := chattool.StopWorkspace(db, chat.ID, chattool.StopWorkspaceOptions{
			OwnerID: user.ID,
			StopFn: func(_ context.Context, _ uuid.UUID, wsID uuid.UUID, req nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
				stopCalled.Store(true)
				require.Equal(t, ws.ID, wsID)
				require.Equal(t, nicloudsdk.WorkspaceTransitionStop, req.Transition)
	...
	t.Run("FailedLatestStopBuildStillStops", func(t *testing.T) {
		...
		tool := chattool.StopWorkspace(db, chat.ID, chattool.StopWorkspaceOptions{
			OwnerID: user.ID,
			StopFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, req nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
				stopCalled.Store(true)
				require.Equal(t, nicloudsdk.WorkspaceTransitionStop, req.Transition)
				buildResp := dbfake.WorkspaceBuild(t, db, ws).Seed(database.WorkspaceBuild{
					Transition:  database.WorkspaceTransitionStop,
