...
func TestStartWorkspace(t *testing.T) {
	...
	t.Run("NoWorkspace", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
				t.Fatal("StartFn should not be called")
				return nicloudsdk.WorkspaceBuild{}, nil
			},
	...
	t.Run("AlreadyRunningPrefersChatSuffixAgent", func(t *testing.T) {
		...
		for _, agent := range wsResp.Agents {
			if agent.Name == "dev-nicloud-chat" {
	...
	t.Run("AlreadyRunningPreservesAgentSelectionError", func(t *testing.T) {
		...
		wsResp := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
			...
			agents[0].Name = "alpha-nicloud-chat"
			return append(agents, &sdkproto.Agent{
				Id:   uuid.NewString(),
				Name: "beta-nicloud-chat",
				Auth: &sdkproto.Agent_Token{Token: uuid.NewString()},
				Env:  map[string]string{},
			})
		}).Seed(database.WorkspaceBuild{
	...
	t.Run("StoppedWorkspace", func(t *testing.T) {
		...
		startFn := func(_ context.Context, _ uuid.UUID, wsID uuid.UUID, req nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
			startCalled = true
			require.Equal(t, nicloudsdk.WorkspaceTransitionStart, req.Transition)
			...
			return nicloudsdk.WorkspaceBuild{ID: buildResp.Build.ID}, nil
		}
	...
	t.Run("PassesParameters", func(t *testing.T) {
		...
		expectedParams := []nicloudsdk.WorkspaceBuildParameter{
	...
	t.Run("ManualUpdateRequired", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			OwnerID: user.ID,
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
				return nicloudsdk.WorkspaceBuild{}, httperror.NewResponseError(400, nicloudsdk.Response{
	...
	t.Run("ResponderErrorWithoutValidationsOmitsTemplateID", func(t *testing.T) {
		...
		tool := chattool.StartWorkspace(db, chat.ID, chattool.StartWorkspaceOptions{
			OwnerID: user.ID,
			StartFn: func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ nicloudsdk.CreateWorkspaceBuildRequest) (nicloudsdk.WorkspaceBuild, error) {
				return nicloudsdk.WorkspaceBuild{}, httperror.NewResponseError(502, nicloudsdk.Response{
