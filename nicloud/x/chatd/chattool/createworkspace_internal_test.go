...
func TestCreateWorkspace_PrefersChatSuffixAgent(t *testing.T) {
	...
	db.EXPECT().
		...
		Return([]database.WorkspaceAgent{
			...
			{ID: chatAgentID, Name: "dev-nicloud-chat", DisplayOrder: 1},
		}, nil)
	db.EXPECT().
...
func TestCreateWorkspace_ReturnsSelectionErrorImmediately(t *testing.T) {
	...
	db.EXPECT().
		GetWorkspaceAgentsInLatestBuildByWorkspaceID(gomock.Any(), workspaceID).
		Return([]database.WorkspaceAgent{
			{ID: uuid.New(), Name: "alpha-nicloud-chat", DisplayOrder: 0},
			{ID: uuid.New(), Name: "beta-nicloud-chat", DisplayOrder: 1},
		}, nil)
...
func TestCreateWorkspace_PostCreationQuotaFailure(t *testing.T) {
	...
	db.EXPECT().
		...
		Return(database.ProvisionerJob{
			...
			ErrorCode: sql.NullString{
				String: string(nicloudsdk.InsufficientQuota),
				Valid:  true,
			},
		}, nil)
	...
	var result map[string]any
	require.NoError(t, json.Unmarshal([]byte(resp.Content), &result))
	require.Equal(t, string(nicloudsdk.InsufficientQuota), result["error_code"])
...
func TestCreateWorkspace_ExistingBuildQuotaFailure(t *testing.T) {
	...
	tool := CreateWorkspace(db, orgID, chatID, CreateWorkspaceOptions{
		OwnerID: ownerID,
		CreateFn: func(context.Context, uuid.UUID, nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
			t.Fatal("CreateFn should not be called when an existing build is in progress")
			return nicloudsdk.Workspace{}, nil
		},
		WorkspaceMu: &sync.Mutex{},
		Logger:      slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}),
	})
...
func TestCreateWorkspace_ResponderErrorPreservesStructuredFields(t *testing.T) {
	...
	tool := CreateWorkspace(db, orgID, chatID, CreateWorkspaceOptions{
		OwnerID: ownerID,
		CreateFn: func(context.Context, uuid.UUID, nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
			return nicloudsdk.Workspace{}, httperror.NewResponseError(400, nicloudsdk.Response{
	...
	var result struct {
		...
		Validations []nicloudsdk.ValidationError `json:"validations"`
	}
	require.NoError(t, json.Unmarshal([]byte(resp.Content), &result))
	...
	require.Equal(t, []nicloudsdk.ValidationError{{
		Field:  "region",
		Detail: "region must be set before the workspace can start",
	}}, result.Validations)
}
...
func TestCreateWorkspaceWithNameRetry(t *testing.T) {
	...
	t.Run("NameConflictRetriesWithGeneratedName", func(t *testing.T) {
		...
		workspace, err := createWorkspaceWithNameRetry(
			context.Background(),
			uuid.New(),
			nicloudsdk.CreateWorkspaceRequest{Name: "fun-dashboard"},
			func(_ context.Context, _ uuid.UUID, req nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
				names = append(names, req.Name)
				if len(names) == 1 {
					return nicloudsdk.Workspace{}, workspaceNameConflictError(req.Name)
				}

				require.Regexp(t, `^fun-dashboard-[0-9a-f]{4}$`, req.Name)
				return nicloudsdk.Workspace{Name: req.Name}, nil
			},
		)
	...
	t.Run("OtherConflictDoesNotRetry", func(t *testing.T) {
		...
		calls := 0
		wantErr := httperror.NewResponseError(http.StatusConflict, nicloudsdk.Response{
		...
		_, err := createWorkspaceWithNameRetry(
			...
			uuid.New(),
			nicloudsdk.CreateWorkspaceRequest{Name: "fun-dashboard"},
			func(context.Context, uuid.UUID, nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
				calls++
				return nicloudsdk.Workspace{}, wantErr
			},
		)
...
func TestCreateWorkspace_GlobalTTL(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var capturedReq nicloudsdk.CreateWorkspaceRequest
			createFn := func(_ context.Context, _ uuid.UUID, req nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
				capturedReq = req
				return nicloudsdk.Workspace{
					ID:        workspaceID,
...
func TestCreateWorkspace_InvalidPresetID(t *testing.T) {
	...
	tool := CreateWorkspace(s.DB, s.OrgID, s.ChatID, CreateWorkspaceOptions{
		OwnerID: s.OwnerID,
		CreateFn: func(_ context.Context, _ uuid.UUID, _ nicloudsdk.CreateWorkspaceRequest) (nicloudsdk.Workspace, error) {
