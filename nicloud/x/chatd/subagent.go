...
func personalModelOverrideContextForSubagent(
	overrideContext nicloudsdk.ChatModelOverrideContext,
) (nicloudsdk.ChatPersonalModelOverrideContext, error) {
	switch overrideContext {
	case nicloudsdk.ChatModelOverrideContextGeneral:
		return nicloudsdk.ChatPersonalModelOverrideContextGeneral, nil
	case nicloudsdk.ChatModelOverrideContextExplore:
		return nicloudsdk.ChatPersonalModelOverrideContextExplore, nil
	default:
		return "", xerrors.Errorf(
...
func (p *Server) resolvePersonalSubagentModelConfigID(
	...
	parsed := ParseChatPersonalModelOverride(
		raw,
		nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault,
	)
	if parsed.Malformed {
		p.logger.Debug(ctx,
...
func (p *Server) createChildSubagentChatWithOptions(
	...
	txErr := p.db.InTx(func(tx database.Store) error {
		...
		userContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText(prompt)})
		if err != nil {
		...
		if childSystemPrompt != "" {
			childSystemPromptContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText(childSystemPrompt),
			})
			if err != nil {
	...
	p.publishChatPubsubEvent(child, nicloudsdk.ChatWatchEventKindCreated, nil)
	p.signalWake()
	return child, nil
}
...
func copyParentContextMessages(
	...
	parent database.Chat,
	child database.Chat,
) ([]nicloudsdk.ChatMessagePart, error) {
	...
	var (
		copiedParts      []nicloudsdk.ChatMessagePart
...
func (p *Server) sendSubagentMessage(
	...
	sendResult, err := p.SendMessage(ctx, SendMessageOptions{
		...
		Content:      []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText(message)},
		APIKeyID:     apiKeyID,
...
func (p *Server) awaitSubagentCompletion(
	...
	if p.pubsub != nil {
		...
		cancel, subErr := p.pubsub.SubscribeWithErr(
			nicloudpubsub.ChatStreamNotifyChannel(targetChatID),
