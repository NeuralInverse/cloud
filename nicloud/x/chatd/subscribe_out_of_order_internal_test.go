...
func TestSubscribeDeliversOutOfOrderDurableMessage(t *testing.T) {
	...
	toolResult := nicloudsdk.ChatMessage{ID: 5, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleTool}
	resumed := nicloudsdk.ChatMessage{ID: 7, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleAssistant}
	promoted := nicloudsdk.ChatMessage{ID: 6, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleUser}

	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
		Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID,
		Message: &nicloudsdk.ChatMessage{ID: 4, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleAssistant},
	})
	...
	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
		Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID, Message: &toolResult,
	})
	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
		Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID, Message: &resumed,
	})
	server.publishChatStreamNotify(chatID, nicloudpubsub.ChatStreamNotifyMessage{AfterMessageID: 4})

	first := testutil.RequireReceive(ctx, t, events)
	require.Equal(t, nicloudsdk.ChatStreamEventTypeMessage, first.Type)
	require.NotNil(t, first.Message)
	require.Equal(t, int64(5), first.Message.ID)
	second := testutil.RequireReceive(ctx, t, events)
	require.Equal(t, nicloudsdk.ChatStreamEventTypeMessage, second.Type)
	...
	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
		Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID, Message: &promoted,
	})
	server.publishChatStreamNotify(chatID, nicloudpubsub.ChatStreamNotifyMessage{AfterMessageID: 5})

	third := testutil.RequireReceive(ctx, t, events)
	require.Equal(t, nicloudsdk.ChatStreamEventTypeMessage, third.Type)
...
func TestSubscribeRespectsAfterMessageIDOnLateNotify(t *testing.T) {
	...
	for _, id := range []int64{96, 97, 98, 99, 100, 101} {
		msg := &nicloudsdk.ChatMessage{ID: id, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleAssistant}
		server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
			Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID, Message: msg,
		})
	}
	...
	server.publishChatStreamNotify(chatID, nicloudpubsub.ChatStreamNotifyMessage{AfterMessageID: 95})

	ev := testutil.RequireReceive(ctx, t, events)
	require.Equal(t, nicloudsdk.ChatStreamEventTypeMessage, ev.Type)
...
func TestSubscribeRunsDBFallbackWhenCacheDeliversUnrelatedMessage(t *testing.T) {
	...
	localOnly := nicloudsdk.ChatMessage{ID: 8, ChatID: chatID, Role: nicloudsdk.ChatMessageRoleAssistant}
	server.cacheDurableMessage(chatID, nicloudsdk.ChatStreamEvent{
		Type: nicloudsdk.ChatStreamEventTypeMessage, ChatID: chatID, Message: &localOnly,
	})
