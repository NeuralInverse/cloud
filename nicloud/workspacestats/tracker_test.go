...
func TestTracker_MultipleInstances(t *testing.T) {
	...
	// Given we have two nicloud instances connected to the same database
	var (
		...
		clientB = nicloudtest.New(t, &nicloudtest.Options{
			WorkspaceUsageTrackerTick:  wuTickB,
			WorkspaceUsageTrackerFlush: wuFlushB,
			Database:                   db,
			Pubsub:                     ps,
		})
		owner = nicloudtest.CreateFirstUser(t, clientA)
		now   = dbtime.Now()
	)
	...
	updated := make([]nicloudsdk.Workspace, numWorkspaces)
	for i := 0; i < numWorkspaces; i++ {
