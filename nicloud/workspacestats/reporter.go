...
//
//  1. The workspace agent periodically POSTs stats to nicloud.  On receiving
//     this POST, if there is an active SSH or web terminal session, bump both
//     the workspace's last_used_at and the deadline.
//  2. The nicloud app proxy and wsproxy will periodically report app status
//     (nicloud calls directly, wsproxy POSTs).  This only bumps the workspace's
