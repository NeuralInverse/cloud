...
func TestWorkspaceAgent(t *testing.T) {
	...
	t.Run("Connect", func(t *testing.T) {
		...
		tmpDir := t.TempDir()
		anotherClient, anotherUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		testutil.Eventually(ctx, t, func(ctx context.Context) (done bool) {
			...
			return workspace.LatestBuild.Resources[0].Agents[0].Status == nicloudsdk.WorkspaceAgentTimeout
	...
	t.Run("DisplayApps", func(t *testing.T) {
		...
		require.NoError(t, err)
		expectedApps := []nicloudsdk.DisplayApp{
...
func TestWorkspaceAgentLogs(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var logChunk []nicloudsdk.WorkspaceAgentLog
		select {
		case <-ctx.Done():
		case logChunk = <-logs:
		}
	...
	t.Run("Close logs on outdated build", func(t *testing.T) {
		...
		_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	...
	t.Run("PublishesOnOverflow", func(t *testing.T) {
		...
		for {
			var update nicloudsdk.Workspace
			select {
			case <-ctx.Done():
				t.FailNow()
			case update = <-updates:
...
func TestWorkspaceAgentAppStatus_ActivityBump(t *testing.T) {
	...
	tests := []struct {
		name       string
		prevState  *nicloudsdk.WorkspaceAppStatusState // nil means no previous state
		newState   nicloudsdk.WorkspaceAppStatusState
		shouldBump bool
	}{
		{
			name:       "FirstStatusBumps",
			prevState:  nil,
			newState:   nicloudsdk.WorkspaceAppStatusStateWorking,
			shouldBump: true,
		},
		{
			name:       "WorkingToIdleBumps",
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateWorking),
			newState:   nicloudsdk.WorkspaceAppStatusStateIdle,
			shouldBump: true,
		},
		{
			name:       "WorkingToCompleteBumps",
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateWorking),
			newState:   nicloudsdk.WorkspaceAppStatusStateComplete,
			shouldBump: true,
		},
		{
			name:       "CompleteToIdleNoBump",
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateComplete),
			newState:   nicloudsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
		...
		},
		{
			...
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateFailure),
			newState:   nicloudsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
			...
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateFailure),
			newState:   nicloudsdk.WorkspaceAppStatusStateFailure,
			shouldBump: false,
		},
		{
		...
		},
		{
			...
			prevState:  ptr.Ref(nicloudsdk.WorkspaceAppStatusStateIdle),
			newState:   nicloudsdk.WorkspaceAppStatusStateIdle,
			shouldBump: false,
		},
		{
...
func TestWorkspaceAgentConnectRPC(t *testing.T) {
	...
	t.Run("Connect", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				resources := nicloudtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).AgentNames([]string{}).Wait()
	...
	t.Run("FailNonLatestBuild", func(t *testing.T) {
		...
		version = nicloudtest.UpdateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:         echo.ParseComplete,
			ProvisionPlan: echo.PlanComplete,
		...
		stopBuild, err := client.CreateWorkspaceBuild(ctx, workspace.ID, nicloudsdk.CreateWorkspaceBuildRequest{
...
func TestWorkspaceAgentClientCoordinate_BadVersion(t *testing.T) {
	...
	resp, err := client.Request(ctx, http.MethodGet,
		...
		nil,
		nicloudsdk.WithQueryParam("version", "99.99"))
	require.NoError(t, err)
...
// returned.
func connectToCoordinatorAndFetchResumeToken(ctx context.Context, logger slog.Logger, sdkClient *nicloudsdk.Client, agentID uuid.UUID, resumeToken string) (string, error) {
	...
	wsConn, resp, err := websocket.Dial(ctx, u.String(), &websocket.DialOptions{
		HTTPHeader: http.Header{
			"NI-Session-Token": []string{sdkClient.SessionToken()},
		},
	})
	if err != nil {
...
func (g *fakeListeningPortsGetter) GetListeningPorts() ([]nicloudsdk.WorkspaceAgentListeningPort, error) {
	g.Lock()
	defer g.Unlock()
	return slices.Clone(g.ports), nil
}

func (g *fakeListeningPortsGetter) setPorts(ports ...nicloudsdk.WorkspaceAgentListeningPort) {
	g.Lock()
	defer g.Unlock()
...
func TestWorkspaceAgentListeningPorts(t *testing.T) {
	...
	testPort := nicloudsdk.WorkspaceAgentListeningPort{
		Network:     "tcp",
		ProcessName: "test-app",
		Port:        44762,
	}
	filteredPort := nicloudsdk.WorkspaceAgentListeningPort{
		Network:     "tcp",
		ProcessName: "postgres",
		Port:        5432,
	}

	setup := func(t *testing.T, apps []*proto.App, dv *nicloudsdk.DeploymentValues) (*nicloudsdk.Client, uuid.UUID, *fakeListeningPortsGetter) {
		t.Helper()
		...
		resources := nicloudtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).Wait()
	...
	for _, tc := range []struct {
		name  string
		setDV func(t *testing.T, dv *nicloudsdk.DeploymentValues)
	}{
		{
			name:  "Mainline",
			setDV: func(*testing.T, *nicloudsdk.DeploymentValues) {},
		},
		{
			name: "BlockDirect",
			setDV: func(t *testing.T, dv *nicloudsdk.DeploymentValues) {
		...
		t.Run("OK_"+tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, []nicloudsdk.WorkspaceAgentListeningPort{testPort}, res.Ports)
...
func TestWorkspaceAgentContainers(t *testing.T) {
	...
	// NEURALINVERSE_TEST_USE_DOCKER=1 go test -run TestWorkspaceAgentContainers/Docker ./nicloud
	...
	t.Run("Mock", func(t *testing.T) {
		...
		testResponse := nicloudsdk.WorkspaceAgentListContainersResponse{
		...
		for _, tc := range []struct {
			name      string
			setupMock func(*acmock.MockContainerCLI) (nicloudsdk.WorkspaceAgentListContainersResponse, error)
		}{
			{
				name: "test response",
				setupMock: func(mcl *acmock.MockContainerCLI) (nicloudsdk.WorkspaceAgentListContainersResponse, error) {
			...
			},
			{
				name: "error response",
				setupMock: func(mcl *acmock.MockContainerCLI) (nicloudsdk.WorkspaceAgentListContainersResponse, error) {
					mcl.EXPECT().List(gomock.Any()).Return(nicloudsdk.WorkspaceAgentListContainersResponse{}, assert.AnError).AnyTimes()
					return nicloudsdk.WorkspaceAgentListContainersResponse{}, assert.AnError
				},
			},
		} {
			t.Run(tc.name, func(t *testing.T) {
...
func TestWatchWorkspaceAgentDevcontainers(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		var (
			...
			client, db = nicloudtest.NewWithDatabase(t, &nicloudtest.Options{Logger: &logger})
			user       = nicloudtest.CreateFirstUser(t, client)
			r          = dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
				OrganizationID: user.OrganizationID,
				OwnerID:        user.UserID,
			...
			fakeContainer1 = nicloudsdk.WorkspaceAgentContainer{
				ID:           "container1",
				CreatedAt:    dbtime.Now(),
			...
			fakeContainer2 = nicloudsdk.WorkspaceAgentContainer{
				ID:           "container1",
				CreatedAt:    dbtime.Now(),
		...
		mCCLI.EXPECT().List(gomock.Any()).Return(nicloudsdk.WorkspaceAgentListContainersResponse{Containers: stages[0].containers}, nil)
		...
		for i, stage := range stages[1:] {
			mCCLI.EXPECT().List(gomock.Any()).Return(nicloudsdk.WorkspaceAgentListContainersResponse{Containers: stage.containers}, nil)
...
func TestWorkspaceAgentRecreateDevcontainer(t *testing.T) {
	...
	t.Run("Mock", func(t *testing.T) {
		...
		var (
			...
			devContainer = nicloudsdk.WorkspaceAgentContainer{
				ID:           uuid.NewString(),
				CreatedAt:    dbtime.Now(),
				FriendlyName: testutil.GetRandomName(t),
				Image:        "busybox:latest",
			...
			devcontainer = nicloudsdk.WorkspaceAgentDevcontainer{
				ID:              devcontainerID,
		...
		for _, tc := range []struct {
		...
		}{
			{
				...
				devcontainers:  []nicloudsdk.WorkspaceAgentDevcontainer{devcontainer},
				containers:     []nicloudsdk.WorkspaceAgentContainer{devContainer},
				expectRecreate: true,
			},
			{
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.expectErrorCode > 0 {
					...
					require.True(t, ok, "expected error to be a neuralinverse error")
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var (
				...
				_, workspaceOwner      = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
				templateAdminClient, _ = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID, tc.role(admin.OrganizationID))
	...
	setupDevcontainerMocks := func(t *testing.T) (
		...
		*acmock.MockDevcontainerCLI,
		...
		devcontainerAPIOptions := []agentcontainers.Option{
			...
			agentcontainers.WithDevcontainerCLI(mDCCLI),
			agentcontainers.WithWatcher(watcher.NewNoop()),
			agentcontainers.WithDevcontainers([]nicloudsdk.WorkspaceAgentDevcontainer{devcontainer}, nil),
		}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.useAnotherUser {
				testClient, _ = nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
			}
...
func TestWorkspaceAgentAppHealth(t *testing.T) {
	...
	manifest := requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, nicloudsdk.WorkspaceAppHealthDisabled, manifest.Apps[0].Health)
	require.EqualValues(t, nicloudsdk.WorkspaceAppHealthInitializing, manifest.Apps[1].Health)
	// empty
	...
	require.NoError(t, err)
	manifest = requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, nicloudsdk.WorkspaceAppHealthHealthy, manifest.Apps[1].Health)
	// update to unhealthy
	_, err = aAPI.BatchUpdateAppHealths(ctx, &agentproto.BatchUpdateAppHealthRequest{
		Updates: []*agentproto.BatchUpdateAppHealthRequest_HealthUpdate{
			{
	...
	require.NoError(t, err)
	manifest = requireGetManifest(ctx, t, aAPI)
	require.EqualValues(t, nicloudsdk.WorkspaceAppHealthUnhealthy, manifest.Apps[1].Health)
}
	...
	t.Run("Set", func(t *testing.T) {
		...
		for _, res := range workspace.LatestBuild.Resources {
			for _, a := range res.Agents {
				require.Equal(t, nicloudsdk.WorkspaceAgentLifecycleCreated, a.LifecycleState)
			}
		}
		...
		tests := []struct {
			state   nicloudsdk.WorkspaceAgentLifecycle
			wantErr bool
		}{
			{nicloudsdk.WorkspaceAgentLifecycleCreated, false},
			{nicloudsdk.WorkspaceAgentLifecycleStarting, false},
			{nicloudsdk.WorkspaceAgentLifecycleStartTimeout, false},
			{nicloudsdk.WorkspaceAgentLifecycleStartError, false},
			{nicloudsdk.WorkspaceAgentLifecycleReady, false},
			{nicloudsdk.WorkspaceAgentLifecycleShuttingDown, false},
			{nicloudsdk.WorkspaceAgentLifecycleShutdownTimeout, false},
			{nicloudsdk.WorkspaceAgentLifecycleShutdownError, false},
			{nicloudsdk.WorkspaceAgentLifecycleOff, false},
			{nicloudsdk.WorkspaceAgentLifecycle("nonexistent_state"), true},
			{nicloudsdk.WorkspaceAgentLifecycle(""), true},
		}
...
func TestWorkspaceAgent_Metadata(t *testing.T) {
	...
	post := func(ctx context.Context, key string, mr nicloudsdk.WorkspaceAgentMetadataResult) {
	...
	var update []nicloudsdk.WorkspaceAgentMetadata

	wantMetadata1 := nicloudsdk.WorkspaceAgentMetadataResult{
	...
	recvUpdate := func() []nicloudsdk.WorkspaceAgentMetadata {
		select {
		case <-ctx.Done():
	...
	check := func(want nicloudsdk.WorkspaceAgentMetadataResult, got nicloudsdk.WorkspaceAgentMetadata, retry bool) {
...
func TestWorkspaceAgent_Metadata_CatchMemoryLeak(t *testing.T) {
	...
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Named("nicloud").Leveled(slog.LevelDebug)
	client := nicloudtest.New(t, &nicloudtest.Options{
		Database:                 db,
		Pubsub:                   psub,
		IncludeProvisionerDaemon: true,
	...
	post := func(ctx context.Context, key, value string) error {
		_, err := aAPI.BatchUpdateMetadata(ctx, &agentproto.BatchUpdateMetadataRequest{
			Metadata: []*agentproto.Metadata{
				{
					Key: key,
					Result: agentsdk.ProtoFromMetadataResult(nicloudsdk.WorkspaceAgentMetadataResult{
						CollectedAt: time.Now(),
						Value:       value,
					}),
				},
...
func TestWorkspaceAgent_Startup(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		var (
			...
			expectedSubsystems = []nicloudsdk.AgentSubsystem{
				nicloudsdk.AgentSubsystemEnvbox,
				nicloudsdk.AgentSubsystemExectrace,
			}
		)
...
// TestWorkspaceAgent_UpdatedDERP runs a real nicloud server, with a real agent
		...
		ownerClient, db := nicloudtest.NewWithDatabase(t, &nicloudtest.Options{
			...
			ExternalAuthConfigs: []*externalauth.Config{
				fake.ExternalAuthConfig(t, providerID, nil, func(cfg *externalauth.Config) {
					cfg.Type = nicloudsdk.EnhancedExternalAuthProviderGitLab.String()
				}),
			},
		})
		first := nicloudtest.CreateFirstUser(t, ownerClient)
		tmpDir := t.TempDir()
		client, user := nicloudtest.CreateAnotherUser(t, ownerClient, first.OrganizationID)
...
func TestOwnedWorkspacesCoordinate(t *testing.T) {
	...
	ctx := testutil.Context(t, testutil.WaitLong)
	logger := testutil.Logger(t)
	firstClient, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
		Coordinator: tailnet.NewCoordinator(logger),
	})
	firstUser := nicloudtest.CreateFirstUser(t, firstClient)
	member, memberUser := nicloudtest.CreateAnotherUser(t, firstClient, firstUser.OrganizationID, rbac.RoleTemplateAdmin())
...
func TestUserTailnetTelemetry(t *testing.T) {
	...
	testCases := []struct {
		...
		// only used for DeviceID, DeviceOS, NIDesktopVersion
		...
		},
		{
			name: "full header",
			headers: map[string]string{
				nicloudsdk.CoderDesktopTelemetryHeader: string(fullHeader),
			},
			expected: telemetry.UserTailnetConnection{
				DeviceOS:            ptr.Ref("Windows"),
				DeviceID:            ptr.Ref("device001"),
				NIDesktopVersion: ptr.Ref("0.22.1"),
			},
		},
		{
			name: "empty header",
			headers: map[string]string{
				nicloudsdk.CoderDesktopTelemetryHeader: "",
			},
			expected: telemetry.UserTailnetConnection{},
		},
		{
			name: "invalid header",
			headers: map[string]string{
				nicloudsdk.CoderDesktopTelemetryHeader: "{\"device_os",
			},
			expected: telemetry.UserTailnetConnection{},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			firstClient := nicloudtest.New(t, &nicloudtest.Options{
				Logger:            &logger,
			...
			requireEqualOrBothNil(t, telemetryConnection.NIDesktopVersion, tc.expected.NIDesktopVersion)
...
func buildWorkspaceWithAgent(
	...
	_ = agenttest.New(t, client.URL, r.AgentToken)
	nicloudtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID).Wait()
	return r.Workspace
}
...
func TestAgentConnectionInfo(t *testing.T) {
	...
	client, db := nicloudtest.NewWithDatabase(t, &nicloudtest.Options{DeploymentValues: dv})
	user := nicloudtest.CreateFirstUser(t, client)
	r := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
		OrganizationID: user.OrganizationID,
		OwnerID:        user.UserID,
