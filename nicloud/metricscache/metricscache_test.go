...
func requireBuildTimeStatsEmpty(t *testing.T, stats nicloudsdk.TemplateBuildTimeStats) {
	require.Empty(t, stats[nicloudsdk.WorkspaceTransitionStart])
	require.Empty(t, stats[nicloudsdk.WorkspaceTransitionStop])
	require.Empty(t, stats[nicloudsdk.WorkspaceTransitionDelete])
}
...
func TestCache_BuildTime(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want.loads {
				wantTransition := nicloudsdk.WorkspaceTransition(tt.args.transition)
				gotStats := cache.TemplateBuildTimeStats(template.ID)
