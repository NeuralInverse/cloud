...
// one.
func New(ctx context.Context, logger slog.Logger, db database.Store, vals *nicloudsdk.DeploymentValues, reg prometheus.Registerer, auditor *atomic.Pointer[audit.Auditor], opts ...Option) io.Closer {
...
func (i *instance) purgeTick(ctx context.Context, db database.Store, start time.Time) error {
	...
	chatAutoArchiveDays, chatAutoArchiveErr := db.GetChatAutoArchiveDays(ctx, nicloudsdk.DefaultChatAutoArchiveDays)
	...
	chatDebugRetentionDays, chatDebugRetentionErr := db.GetChatDebugRetentionDays(ctx, nicloudsdk.DefaultChatDebugRetentionDays)
...
// golden fixtures in nicloud/notifications/testdata. Truncation keeps
