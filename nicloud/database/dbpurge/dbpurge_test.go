...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/notifications/notificationsmock"
...
func TestPurge(t *testing.T) {
	...
	mDB.EXPECT().GetChatAutoArchiveDays(gomock.Any(), nicloudsdk.DefaultChatAutoArchiveDays).Return(int32(0), nil).AnyTimes()
	mDB.EXPECT().GetChatDebugRetentionDays(gomock.Any(), nicloudsdk.DefaultChatDebugRetentionDays).Return(int32(0), nil).AnyTimes()
	mDB.EXPECT().InTx(gomock.Any(), database.DefaultTXOptions().WithID("db_purge")).Return(nil).Times(2)
	purger := dbpurge.New(context.Background(), testutil.Logger(t), mDB, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), nopAuditorPtr(t), dbpurge.WithClock(clk))
...
func TestMetrics(t *testing.T) {
	t.Run("SuccessfulIteration", func(t *testing.T) {
		...
		closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{
			Retention: nicloudsdk.RetentionConfig{
		...
		hist := promhelp.HistogramValue(t, reg, "nicloud_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "true",
		})
		require.NotNil(t, hist)
		require.Greater(t, hist.GetSampleCount(), uint64(0), "should have at least one sample")

		expiredAPIKeys := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
		...
		workspaceAgentLogs := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "workspace_agent_logs",
		})
		require.GreaterOrEqual(t, workspaceAgentLogs, 0)

		aibridgeRecords := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "aibridge_records",
		})
		require.GreaterOrEqual(t, aibridgeRecords, 0)

		connectionLogs := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "connection_logs",
		})
		require.GreaterOrEqual(t, connectionLogs, 0)

		auditLogs := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "audit_logs",
		})
		require.GreaterOrEqual(t, auditLogs, 0)

		chats := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "chats",
		})
		require.GreaterOrEqual(t, chats, 0)

		chatDebugRuns := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
			"record_type": "chat_debug_runs",
		})
		require.GreaterOrEqual(t, chatDebugRuns, 0)

		chatFiles := promhelp.CounterValue(t, reg, "nicloud_dbpurge_records_purged_total", prometheus.Labels{
	...
	t.Run("LockNotAcquiredSkipsIterationMetric", func(t *testing.T) {
		...
		mDB.EXPECT().GetChatRetentionDays(gomock.Any()).Return(int32(0), nil).AnyTimes()
		mDB.EXPECT().GetChatAutoArchiveDays(gomock.Any(), nicloudsdk.DefaultChatAutoArchiveDays).
			Return(int32(0), nil).AnyTimes()
		mDB.EXPECT().GetChatDebugRetentionDays(gomock.Any(), nicloudsdk.DefaultChatDebugRetentionDays).
			Return(int32(0), nil).AnyTimes()
		...
		closer := dbpurge.New(ctx, logger, mDB, &nicloudsdk.DeploymentValues{}, reg, nopAuditorPtr(t), dbpurge.WithClock(clk))
		defer closer.Close()
		testutil.TryReceive(ctx, t, done)

		successHist := promhelp.MetricValue(t, reg, "nicloud_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "true",
		})
		require.Nil(t, successHist, "lock contention should not record a successful purge iteration")

		failedHist := promhelp.MetricValue(t, reg, "nicloud_dbpurge_iteration_duration_seconds", prometheus.Labels{
			"success": "false",
		})
...
func TestDeleteOldWorkspaceAgentStats(t *testing.T) {
	...
	// when
	closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), nopAuditorPtr(t), dbpurge.WithClock(clk))
	defer closer.Close()
...
func TestDeleteOldWorkspaceAgentLogsRetention(t *testing.T) {
	...
	testCases := []struct {
		name            string
		retentionConfig nicloudsdk.RetentionConfig
...
func TestDeleteOldAIBridgeRecords(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{
				AI: nicloudsdk.AIConfig{
					BridgeConfig: nicloudsdk.AIBridgeConfig{
	...
	createDebugRunWithStep := func(ctx context.Context, t *testing.T, db database.Store, chatID uuid.UUID, updatedAt time.Time, finished bool) database.ChatDebugRun {
		...
		if finished {
			run, err = db.UpdateChatDebugRun(ctx, database.UpdateChatDebugRunParams{
				Status:     sql.NullString{String: string(nicloudsdk.ChatDebugStatusCompleted), Valid: true},
				FinishedAt: sql.NullTime{Time: updatedAt, Valid: true},
				Now:        updatedAt,
				ID:         run.ID,
				ChatID:     run.ChatID,
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, reg, nopAuditorPtr(t), dbpurge.WithClock(clk))
				defer closer.Close()
				testutil.TryReceive(ctx, t, done)
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				require.Zero(t, nicloudsdk.DefaultChatAutoArchiveDays)
				require.NoError(t, db.UpsertChatAutoArchiveDays(ctx, nicloudsdk.DefaultChatAutoArchiveDays))
				...
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), auditorPtr, dbpurge.WithNotificationsEnqueuer(enqueuer), dbpurge.WithClock(clk))
				defer closer.Close()
				testutil.TryReceive(ctx, t, done)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), auditorPtr, dbpurge.WithClock(clk))
				defer closer.Close()
				defer driver.close()
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				enqueuer := notificationstest.NewFakeEnqueuer()
				driver := newTickDriver(t, clk)
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), auditorPtr, dbpurge.WithNotificationsEnqueuer(enqueuer), dbpurge.WithClock(clk), dbpurge.WithChatAutoArchiveBatchSize(20))
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), nopAuditorPtr(t), dbpurge.WithNotificationsEnqueuer(mockEnq), dbpurge.WithClock(clk))
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				closer := dbpurge.New(ctx, logger, db, &nicloudsdk.DeploymentValues{}, prometheus.NewRegistry(), auditorPtr, dbpurge.WithNotificationsEnqueuer(mockEnq), dbpurge.WithClock(clk))
				defer closer.Close()
				testutil.TryReceive(ctx, t, done)
