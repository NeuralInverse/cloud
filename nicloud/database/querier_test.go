...
func TestGetDeploymentWorkspaceAgentUsageStats(t *testing.T) {
	...
	t.Run("Aggregates", func(t *testing.T) {
		...
		db = dbauthz.New(db, authz, slogtest.Make(t, &slogtest.Options{}), nicloudtest.AccessControlStorePointer())
		ctx := context.Background()
		agentID := uuid.New()
...
func TestGetAuthorizedWorkspacesAndAgentsByOwnerID(t *testing.T) {
	...
	t.Run("dbauthz", func(t *testing.T) {
		...
		authzdb := dbauthz.New(db, authorizer, slogtest.Make(t, &slogtest.Options{}), nicloudtest.AccessControlStorePointer())
...
func TestProxyByHostname(t *testing.T) {
	...
	proxies := []struct {
		...
		{
			name:             "one",
			accessURL:        "https://one.cloud.neuralinverse.com",
			wildcardHostname: "*.wildcard.one.cloud.neuralinverse.com",
		},
		{
			name:             "two",
			accessURL:        "https://two.cloud.neuralinverse.com",
			wildcardHostname: "*--suffix.two.cloud.neuralinverse.com",
		},
	}
	...
	cases := []struct {
		...
		{
			name:              "MatchAccessURL",
			testHostname:      "one.cloud.neuralinverse.com",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "one",
		},
		{
			name:              "MatchWildcard",
			testHostname:      "something.wildcard.one.cloud.neuralinverse.com",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "one",
		},
		{
			name:              "MatchSuffix",
			testHostname:      "something--suffix.two.cloud.neuralinverse.com",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "two",
		},
		{
			...
			testHostname:      ".*ne.cloud.neuralinverse.com",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "",
		},
		{
			...
			testHostname:      "https://one.cloud.neuralinverse.com",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "",
		},
		{
			...
			testHostname:      "one.cloud.neuralinverse.com:8080/hello",
			allowAccessURL:    true,
			allowWildcardHost: true,
			matchProxyName:    "",
		},
		...
		},
		{
			...
			testHostname:      "hi.wildcard.one.cloud.neuralinverse.com",
			allowAccessURL:    true,
...
func TestGetUsers_IncludeSystem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Given: a system user
			// postgres: introduced by migration nicloud/database/migrations/00030*_system_user.up.sql
			db, _ := dbtestutil.NewDB(t)
...
func TestUpdateSystemUser(t *testing.T) {
	...
	// Given: a system user introduced by migration nicloud/database/migrations/00030*_system_user.up.sql
	db, _ := dbtestutil.NewDB(t)
	users, err := db.GetUsers(ctx, database.GetUsersParams{
		IncludeSystem: true,
	})
...
func TestInsertUserServiceAccountConstraints(t *testing.T) {
	...
	t.Run("ServiceAccountWithNonEmptyEmail", func(t *testing.T) {
		...
		_, err := db.InsertUser(ctx, database.InsertUserParams{
			Email:            "sa@cloud.neuralinverse.com",
			LoginType:        database.LoginTypeNone,
			ID:               uuid.New(),
...
func TestGetAuthorizedConnectionLogsOffset(t *testing.T) {
	...
	db, _ := dbtestutil.NewDB(t)
	authz := rbac.NewAuthorizer(prometheus.NewRegistry())
	authDb := dbauthz.New(db, authz, slogtest.Make(t, &slogtest.Options{}), nicloudtest.AccessControlStorePointer())
...
func TestConnectionLogsOffsetFilters(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			name: "StatusOngoing",
			params: database.GetConnectionLogsOffsetParams{
				Status: string(nicloudsdk.ConnectionLogStatusOngoing),
			},
			expectedLogIDs: []uuid.UUID{log4.ID},
		},
		{
			...
			params: database.GetConnectionLogsOffsetParams{
				Status: string(nicloudsdk.ConnectionLogStatusCompleted),
			},
...
func TestUserSecretsAuthorization(t *testing.T) {
	...
	db, _ := dbtestutil.NewDB(t)
	authorizer := rbac.NewStrictCachingAuthorizer(prometheus.NewRegistry())
	authDB := dbauthz.New(db, authorizer, slogtest.Make(t, &slogtest.Options{}), nicloudtest.AccessControlStorePointer())
