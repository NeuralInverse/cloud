...
// This is also in nicloudsdk/workspaceagents.go and should be kept in sync.
const (
...
func (s APIKeyScope) ToRBAC() rbac.ScopeName {
	switch s {
	case ApiKeyScopeNIAll:
		return rbac.ScopeAll
	case ApiKeyScopeNIApplicationConnect:
...
func (s APIKeyScopes) Name() rbac.RoleIdentifier {
	if len(s) == 0 {
		...
		return rbac.RoleIdentifier{Name: string(ApiKeyScopeNIAll)}
	}
