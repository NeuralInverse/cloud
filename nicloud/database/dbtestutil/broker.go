...
func (b *Broker) Create(t TBSubset, opts ...OpenOption) (ConnectionParams, error) {
	...
	if err = createDatabaseFromTemplate(t, defaultConnectionParams, b.niTestingDB, dbName, templateDBName); err != nil {
...
func (b *Broker) clean(t TBSubset, dbName string) func() {
	return func() {
		_, err := b.niTestingDB.Exec("DROP DATABASE " + dbName + ";")
		if err != nil {
			t.Logf("failed to clean up database %q: %s\n", dbName, err.Error())
			return
		}
		_, err = b.niTestingDB.Exec("UPDATE test_databases SET dropped_at = CURRENT_TIMESTAMP WHERE name = $1", dbName)
		if err != nil {
...
func getTestPackageName(t TBSubset) string {
	...
	for {
		...
		if strings.HasPrefix(frame.Function, "github.com/NeuralInverse/cloud/v2/") {
			packageName = strings.SplitN(strings.TrimPrefix(frame.Function, "github.com/NeuralInverse/cloud/v2/"), ".", 2)[0]
		}
