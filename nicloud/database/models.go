...
const (
	ApiKeyScopeNIAll                            APIKeyScope = "coder:all"
	ApiKeyScopeNIApplicationConnect             APIKeyScope = "coder:application_connect"
...
// Hashed bearer secrets used by AI Gateway standalone replicas to authenticate into nicloud.
...
// Runtime configuration for AI providers. Authoritative source for the provider set served by aibridged. Replaces deployment-time NEURALINVERSE_AIBRIDGE_* environment variables.
...
type BoundarySession struct {
	...
	// Time when the first log for this session was received by nicloud.
	StartedAt time.Time `db:"started_at" json:"started_at"`
...
type CustomRole struct {
	...
	ID uuid.UUID `db:"id" json:"id"`
	// System roles are managed by Neural Inverse Cloud and cannot be modified or deleted by users.
...
type MCPServerConfig struct {
	...
	ForwardNIHeaders     bool           `db:"forward_coder_headers" json:"forward_coder_headers"`
}
...
// A table used to configure apps that can use Neural Inverse Cloud as an OAuth2 provider, the reverse of what we are calling external authentication.
...
type UsageEvent struct {
	...
	CreatedAt time.Time       `db:"created_at" json:"created_at"`
	// Set to a timestamp while the event is being published by a Neural Inverse Cloud replica to the usage collector service. Used to avoid duplicate publishes by multiple replicas. Timestamps older than 1 hour are considered expired.
...
type User struct {
	...
	// Name of the Neural Inverse Cloud user
	Name string `db:"name" json:"name"`
	// The GitHub.com numerical user ID. It is used to check if the user has starred the Neural Inverse Cloud repository. It is also used for filtering users in the users list CLI command, and may become more widely used in the future.
...
type WorkspaceAgent struct {
	...
	// The resolved path of a user-specified directory. e.g. ~/coder -> /home/neuralinverse/coder
