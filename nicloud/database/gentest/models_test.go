...
// TestAuditDBEnumsCovered ensures that all enums in the database are covered by the nicloudsdk enums
...
func TestAuditDBEnumsCovered(t *testing.T) {
	...
	for _, ty := range dbTypes {
		str := nicloudsdk.ResourceType(ty).FriendlyString()
		require.NotEqualf(t, "unknown", str, "ResourceType %q not covered by nicloudsdk.ResourceType", ty)
	}
}
