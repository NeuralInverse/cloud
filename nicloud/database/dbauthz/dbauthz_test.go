...
func TestAsNoActor(t *testing.T) {
	...
	t.Run("AsActor", func(t *testing.T) {
		t.Parallel()
		ctx := dbauthz.As(context.Background(), nicloudtest.RandomRBACSubject())
...
func TestPing(t *testing.T) {
	...
	q := dbauthz.New(db, &nicloudtest.RecordingAuthorizer{}, slog.Make(), nicloudtest.AccessControlStorePointer())
...
func TestInTX(t *testing.T) {
	...
	q := dbauthz.New(db, &nicloudtest.RecordingAuthorizer{
		Wrapped: (&nicloudtest.FakeAuthorizer{}).AlwaysReturn(xerrors.New("custom error")),
	}, slog.Make(), nicloudtest.AccessControlStorePointer())
...
func TestNew(t *testing.T) {
	...
	// should be made.
	az := dbauthz.New(db, rec, slog.Make(), nicloudtest.AccessControlStorePointer())
	az = dbauthz.New(az, rec, slog.Make(), nicloudtest.AccessControlStorePointer())
...
func TestChatFilesAllowLinkedChatReads(t *testing.T) {
	...
	t.Run("GetChatFileByID", func(t *testing.T) {
		...
		q := dbauthz.New(db, authorizer, slogtest.Make(t, nil), nicloudtest.AccessControlStorePointer())
...
func TestUpdateChatACLByIDGuards(t *testing.T) {
	...
	t.Run("Disabled", func(t *testing.T) { //nolint:paralleltest // It toggles the global chat ACL flag.
		...
		q := dbauthz.New(db, &nicloudtest.FakeAuthorizer{}, slogtest.Make(t, nil), nicloudtest.AccessControlStorePointer())
...
func (s *MethodTestSuite) TestAPIKey() {
	...
	s.Run("InsertAPIKey", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		...
		arg := database.InsertAPIKeyParams{UserID: u.ID, LoginType: database.LoginTypePassword, Scopes: database.APIKeyScopes{database.ApiKeyScopeNIAll}, IPAddress: defaultIPAddress()}
	...
	s.Run("DeleteApplicationConnectAPIKeysByUserID", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		a := testutil.Fake(s.T(), faker, database.APIKey{Scopes: database.APIKeyScopes{database.ApiKeyScopeNIApplicationConnect}})
...
func (s *MethodTestSuite) TestOrganization() {
	...
	s.Run("InsertOrganizationMember", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		o := testutil.Fake(s.T(), faker, database.Organization{})
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.InsertOrganizationMemberParams{OrganizationID: o.ID, UserID: u.ID, Roles: []string{nicloudsdk.RoleOrganizationAdmin}}
	...
	s.Run("UpdateMemberRoles", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		o := testutil.Fake(s.T(), faker, database.Organization{})
		u := testutil.Fake(s.T(), faker, database.User{})
		mem := testutil.Fake(s.T(), faker, database.OrganizationMember{OrganizationID: o.ID, UserID: u.ID, Roles: []string{nicloudsdk.RoleOrganizationAdmin}})
		out := mem
		out.Roles = []string{}
...
func (s *MethodTestSuite) TestUser() {
	...
	s.Run("ListUserChatCompactionThresholds", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		uc := database.UserConfig{UserID: u.ID, Key: nicloudsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001", Value: "75"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("GetUserChatCompactionThreshold", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.GetUserChatCompactionThresholdParams{UserID: u.ID, Key: nicloudsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("DeleteUserChatCompactionThreshold", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		arg := database.DeleteUserChatCompactionThresholdParams{UserID: u.ID, Key: nicloudsdk.ChatCompactionThresholdKeyPrefix + "00000000-0000-0000-0000-000000000001"}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("UpdateUserRoles", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{RBACRoles: []string{nicloudsdk.RoleTemplateAdmin}})
		o := u
		o.RBACRoles = []string{nicloudsdk.RoleUserAdmin}
		arg := database.UpdateUserRolesParams{GrantedRoles: []string{nicloudsdk.RoleUserAdmin}, ID: u.ID}
		dbm.EXPECT().GetUserByID(gomock.Any(), u.ID).Return(u, nil).AnyTimes()
	...
	s.Run("SitePermissions/UpdateCustomRole", s.Mocked(func(_ *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		arg := database.UpdateCustomRoleParams{
			...
			SitePermissions: slice.List(nicloudsdk.CreatePermissions(map[nicloudsdk.RBACResource][]nicloudsdk.RBACAction{
				nicloudsdk.ResourceTemplate: {nicloudsdk.ActionCreate, nicloudsdk.ActionRead, nicloudsdk.ActionUpdate, nicloudsdk.ActionDelete, nicloudsdk.ActionViewInsights},
			}), convertSDKPerm),
			OrgPermissions: nil,
			UserPermissions: slice.List(nicloudsdk.CreatePermissions(map[nicloudsdk.RBACResource][]nicloudsdk.RBACAction{
	...
	s.Run("OrgPermissions/UpdateCustomRole", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		...
		arg := database.UpdateCustomRoleParams{
			...
			OrgPermissions: slice.List(nicloudsdk.CreatePermissions(map[nicloudsdk.RBACResource][]nicloudsdk.RBACAction{
				nicloudsdk.ResourceTemplate: {nicloudsdk.ActionCreate, nicloudsdk.ActionRead},
			}), convertSDKPerm),
		}
		dbm.EXPECT().UpdateCustomRole(gomock.Any(), arg).Return(database.CustomRole{}, nil).AnyTimes()
		check.Args(arg).Asserts(
...
func (s *MethodTestSuite) TestNotifications() {
	...
	s.Run("InsertInboxNotification", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		u := testutil.Fake(s.T(), faker, database.User{})
		notifID := uuid.New()
		arg := database.InsertInboxNotificationParams{ID: notifID, UserID: u.ID, TemplateID: notifications.TemplateWorkspaceAutoUpdated, Targets: []uuid.UUID{u.ID, notifications.TemplateWorkspaceAutoUpdated}, Title: "test title", Content: "test content notification", Icon: "https://cloud.neuralinverse.com/favicon.ico", Actions: json.RawMessage("{}")}
...
func TestGetLatestWorkspaceBuildByWorkspaceID_FastPath(t *testing.T) {
	...
	authorizer := &nicloudtest.RecordingAuthorizer{
		Wrapped: (&nicloudtest.FakeAuthorizer{}).AlwaysReturn(nil),
	}
	...
	t.Run("WithWorkspaceRBAC", func(t *testing.T) {
		...
		q := dbauthz.New(dbm, authorizer, slogtest.Make(t, nil), nicloudtest.AccessControlStorePointer())
...
func TestGetWorkspaceAgentByID_FastPath(t *testing.T) {
	...
	t.Run("WithWorkspaceRBAC", func(t *testing.T) {
		...
		q := dbauthz.New(mockDB, authorizer, slogtest.Make(t, nil), nicloudtest.AccessControlStorePointer())
