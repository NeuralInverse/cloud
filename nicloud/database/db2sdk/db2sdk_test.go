...
func TestProvisionerJobStatus(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			status: nicloudsdk.ProvisionerJobCanceling,
		},
		{
			name: "canceled",
			job: database.ProvisionerJob{
			...
			status: nicloudsdk.ProvisionerJobCanceled,
		},
		{
			...
			status: nicloudsdk.ProvisionerJobFailed,
		},
		{
			name:   "pending",
			job:    database.ProvisionerJob{},
		...
		},
		{
			...
			status: nicloudsdk.ProvisionerJobSucceeded,
		},
		{
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			actual := nicloudsdk.ProvisionerJobStatus(inserted.JobStatus)
...
func TestChatDebugRunSummary(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatDebugRunKindChatTurn, sdk.Kind)
	require.Equal(t, nicloudsdk.ChatDebugStatusCompleted, sdk.Status)
...
func TestChatDebugStep(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatDebugStepOperationStream, sdk.Operation)
	require.Equal(t, nicloudsdk.ChatDebugStatusCompleted, sdk.Status)
	require.Equal(t, startedAt, sdk.StartedAt)
	require.Equal(t, finishedAt, sdk.UpdatedAt)
	require.Equal(t, &finishedAt, sdk.FinishedAt)
...
func TestChatDebugRunDetail(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatDebugStepOperationStream, sdk.Steps[0].Operation)
	require.Equal(t, steps[1].ID, sdk.Steps[1].ID)
	require.Equal(t, nicloudsdk.ChatDebugStepOperationGenerate, sdk.Steps[1].Operation)
}
...
func TestAIBridgeInterception(t *testing.T) {
	...
	cases := []struct {
		...
		expected     nicloudsdk.AIBridgeInterception
	}{
		{
			...
			expected: nicloudsdk.AIBridgeInterception{
				ID: interceptionID,
				Initiator: nicloudsdk.MinimalUser{
					ID:        initiatorID,
					Username:  "testuser",
					Name:      "Test User",
					AvatarURL: "https://example.com/avatar.png",
...
func TestChatMessage_PreservesProviderExecutedOnToolResults(t *testing.T) {
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeToolCall, result.Content[0].Type)
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeToolResult, result.Content[1].Type)
...
func TestChatQueuedMessage_ParsesUserContentParts(t *testing.T) {
	...
	rawContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("queued text"),
	})
	require.NoError(t, err)
	...
	require.Equal(t, nicloudsdk.ChatMessagePartTypeText, queued.Content[0].Type)
...
func TestChat_AllFieldsPopulated(t *testing.T) {
	...
	// field to nicloudsdk.Chat, this test will fail until the
	// converter is updated.
	now := dbtime.Now()
	lastErrorPayload := nicloudsdk.ChatError{
		Message:    "boom",
	...
	for i := range typ.NumField() {
		...
		require.False(t, v.Field(i).IsZero(),
			"nicloudsdk.Chat field %q is zero-valued — db2sdk.Chat may not be populating it",
			field.Name,
		)
	}
}
...
func TestChat_LastErrorFallback(t *testing.T) {
	...
	tests := []struct {
		name          string
		raw           json.RawMessage
		expectPayload *nicloudsdk.ChatError
	}{
		{
			name: "MalformedJSON",
			raw:  json.RawMessage(`{`),
