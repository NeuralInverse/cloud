...
func TestProvisionerJobs(t *testing.T) {
	...
	t.Run("ProvisionerJobs", func(t *testing.T) {
		...
		t.Run("Single", func(t *testing.T) {
			...
			t.Run("Workspace", func(t *testing.T) {
				...
				t.Run("OK", func(t *testing.T) {
					...
					assert.Equal(t, job2.Metadata, nicloudsdk.ProvisionerJobMetadata{
						...
						WorkspaceBuildTransition: nicloudsdk.WorkspaceTransitionStart,
					})
				})
			})
		...
		t.Run("IDs", func(t *testing.T) {
			t.Parallel()
			ctx := testutil.Context(t, testutil.WaitMedium)
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &nicloudsdk.OrganizationProvisionerJobsOptions{
		...
		t.Run("Status", func(t *testing.T) {
			...
			ctx := testutil.Context(t, testutil.WaitMedium)
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &nicloudsdk.OrganizationProvisionerJobsOptions{
				Status: []nicloudsdk.ProvisionerJobStatus{nicloudsdk.ProvisionerJobRunning},
			})
			require.NoError(t, err)
		...
		t.Run("InitiatorWithOtherFilters", func(t *testing.T) {
			...
			jobs, err := templateAdminClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &nicloudsdk.OrganizationProvisionerJobsOptions{
				...
				Status:    []nicloudsdk.ProvisionerJobStatus{nicloudsdk.ProvisionerJobSucceeded},
			})
			require.NoError(t, err)
			...
			for _, job := range jobs {
				...
				require.Equal(t, nicloudsdk.ProvisionerJobSucceeded, job.Status)
			}
		})
		...
		t.Run("MemberDeniedWithInitiator", func(t *testing.T) {
			...
			jobs, err := memberClient.OrganizationProvisionerJobs(ctx, owner.OrganizationID, &nicloudsdk.OrganizationProvisionerJobsOptions{
				Initiator: member.ID.String(),
			})
			require.Error(t, err)
			require.Len(t, jobs, 0)
	...
	t.Run("AssignedProvisionerJob", func(t *testing.T) {
		...
		provisionerDaemon := nicloudtest.NewTaggedProvisionerDaemon(t, nicloudAPI, provisionerDaemonName, map[string]string{"owner": "", "scope": "organization"})
		owner := nicloudtest.CreateFirstUser(t, client)
		templateAdminClient, _ := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.ScopedRoleOrgTemplateAdmin(owner.OrganizationID))
		...
		t.Run("Get_IncludesWorkerIDAndName", func(t *testing.T) {
			...
			for _, job := range jobs {
				if job.Type == nicloudsdk.ProvisionerJobTypeWorkspaceBuild {
