...
type Config struct {
	...
	// NoRefresh stops Neural Inverse Cloud from using the refresh token
	...
	// injected into Neural Inverse Cloud AI Bridge upstream requests.
...
func (c *Config) Git(client *http.Client) (gitprovider.Provider, error) {
	...
	if !nicloudsdk.EnhancedExternalAuthProvider(norm).Git() {
...
validate:
	...
	if !valid {
		...
		if c.Type == string(nicloudsdk.EnhancedExternalAuthProviderGitHub) && r.Wait(retryCtx) {
			goto validate
		}
...
// or 429).
func (c *Config) ValidateToken(ctx context.Context, link *oauth2.Token) (bool, *nicloudsdk.ExternalAuthUser, error) {
	if link == nil {
	...
	var user *nicloudsdk.ExternalAuthUser
	if c.Type == string(nicloudsdk.EnhancedExternalAuthProviderGitHub) {
		var ghUser github.User
		err = json.NewDecoder(res.Body).Decode(&ghUser)
		if err == nil {
			user = &nicloudsdk.ExternalAuthUser{
				ID:         ghUser.GetID(),
...
func (c *Config) AppInstallations(ctx context.Context, token string) ([]nicloudsdk.ExternalAuthAppInstallation, bool, error) {
	...
	installs := []nicloudsdk.ExternalAuthAppInstallation{}
	if c.Type == string(nicloudsdk.EnhancedExternalAuthProviderGitHub) {
		...
		for _, installation := range ghInstalls.Installations {
			...
			installs = append(installs, nicloudsdk.ExternalAuthAppInstallation{
				ID:           int(installation.GetID()),
				ConfigureURL: installation.GetHTMLURL(),
				Account: nicloudsdk.ExternalAuthUser{
					ID:         account.GetID(),
...
func (c *Config) TokenRevocationRequest(ctx context.Context, link database.ExternalAuthLink) (*http.Request, error) {
	if c.Type == nicloudsdk.EnhancedExternalAuthProviderGitHub.String() {
...
func (c *DeviceAuth) AuthorizeDevice(ctx context.Context) (*nicloudsdk.ExternalAuthDevice, error) {
	if c.CodeURL == "" {
...
func (c *DeviceAuth) ExchangeDeviceCode(ctx context.Context, deviceCode string) (*oauth2.Token, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, nicloudsdk.ReadBodyAsError(resp)
	}
...
// to the parsed and ready-to-consume in nicloud provider type.
func ConvertConfig(instrument *promoauth.Factory, entries []nicloudsdk.ExternalAuthConfig, accessURL *url.URL) ([]*Config, error) {
	...
	for _, entry := range entries {
		...
		valid := nicloudsdk.NameValid(entry.ID)
		if valid != nil {
		...
		if entry.Type == string(nicloudsdk.EnhancedExternalAuthProviderAzureDevops) {
			oauthConfig = &jwtConfig{oc}
		}
		if entry.Type == string(nicloudsdk.EnhancedExternalAuthProviderAzureDevopsEntra) {
			oauthConfig = &entraV1Oauth{oc}
		}
		if entry.Type == string(nicloudsdk.EnhancedExternalAuthProviderJFrog) {
			oauthConfig = &exchangeWithClientSecret{oc}
		}

		instrumented := instrument.New(entry.ID, oauthConfig)
		if strings.EqualFold(entry.Type, string(nicloudsdk.EnhancedExternalAuthProviderGitHub)) {
...
func applyDefaultsToConfig(config *nicloudsdk.ExternalAuthConfig) {
	configType := nicloudsdk.EnhancedExternalAuthProvider(strings.ToLower(config.Type))
	if configType == "bitbucket" {
		// For backwards compatibility, we need to support the "bitbucket" string.
		configType = nicloudsdk.EnhancedExternalAuthProviderBitBucketCloud
		defer func() {
			...
			config.Type = string(nicloudsdk.EnhancedExternalAuthProviderBitBucketCloud)
		}()
	}
	...
	// Dynamic defaults
	switch configType {
	case nicloudsdk.EnhancedExternalAuthProviderGitHub:
		copyDefaultSettings(config, gitHubDefaults(config))
		return
	case nicloudsdk.EnhancedExternalAuthProviderGitLab:
		copyDefaultSettings(config, gitlabDefaults(config))
		return
	case nicloudsdk.EnhancedExternalAuthProviderBitBucketServer:
		copyDefaultSettings(config, bitbucketServerDefaults(config))
		return
	case nicloudsdk.EnhancedExternalAuthProviderJFrog:
		copyDefaultSettings(config, jfrogArtifactoryDefaults(config))
		return
	case nicloudsdk.EnhancedExternalAuthProviderGitea:
		copyDefaultSettings(config, giteaDefaults(config))
		return
	case nicloudsdk.EnhancedExternalAuthProviderAzureDevopsEntra:
	...
	default:
		...
		copyDefaultSettings(config, nicloudsdk.ExternalAuthConfig{})
		return
	}
}

func copyDefaultSettings(config *nicloudsdk.ExternalAuthConfig, defaults nicloudsdk.ExternalAuthConfig) {
	if config.AuthURL == "" {
	...
	if config.APIBaseURL == "" {
		...
		switch nicloudsdk.EnhancedExternalAuthProvider(normType) {
		case nicloudsdk.EnhancedExternalAuthProviderGitHub:
...
func gitHubDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	defaults := nicloudsdk.ExternalAuthConfig{
...
func bitbucketServerDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	defaults := nicloudsdk.ExternalAuthConfig{
	...
	// Bitbucket servers will have some base url, e.g. https://bitbucket.cloud.neuralinverse.com.
...
func gitlabDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	cloud := nicloudsdk.ExternalAuthConfig{
	...
	return nicloudsdk.ExternalAuthConfig{
...
func jfrogArtifactoryDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	defaults := nicloudsdk.ExternalAuthConfig{
	...
	// Artifactory servers will have some base url, e.g. https://jfrog.cloud.neuralinverse.com.
...
func giteaDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	defaults := nicloudsdk.ExternalAuthConfig{
		DisplayName:                   "Gitea",
	...
	// Gitea's servers will have some base url, e.g: https://gitea.cloud.neuralinverse.com.
...
func azureDevopsEntraDefaults(config *nicloudsdk.ExternalAuthConfig) nicloudsdk.ExternalAuthConfig {
	defaults := nicloudsdk.ExternalAuthConfig{
...
var staticDefaults = map[nicloudsdk.EnhancedExternalAuthProvider]nicloudsdk.ExternalAuthConfig{
	nicloudsdk.EnhancedExternalAuthProviderAzureDevops: {
	...
	nicloudsdk.EnhancedExternalAuthProviderBitBucketCloud: {
	...
	nicloudsdk.EnhancedExternalAuthProviderSlack: {
