...
func TestRevokeToken(t *testing.T) {
	...
	t.Run("RevokeTokenGitHub_OK", func(t *testing.T) {
		...
		config.Type = nicloudsdk.EnhancedExternalAuthProviderGitHub.String()
...
func TestExchangeWithClientSecret(t *testing.T) {
	...
	configs, err := externalauth.ConvertConfig(instrument, []nicloudsdk.ExternalAuthConfig{{
		// JFrog just happens to require this custom type.

		Type:         nicloudsdk.EnhancedExternalAuthProviderJFrog.String(),
		ClientID:     "id",
		ClientSecret: "secret",
	}}, &url.URL{})
	require.NoError(t, err)
...
func TestTokenRevocationResponseOk(t *testing.T) {
	...
	ghType := nicloudsdk.EnhancedExternalAuthProviderGitHub.String()
	rfcType := nicloudsdk.EnhancedExternalAuthProviderAzureDevops.String()
	tests := []struct {
		name string
...
func TestConvertYAML(t *testing.T) {
	...
	for _, tc := range []struct {
		Name   string
		Input  []nicloudsdk.ExternalAuthConfig
		Output []*externalauth.Config
		Error  string
	}{{
		Name: "InvalidID",
		Input: []nicloudsdk.ExternalAuthConfig{{
			Type: string(nicloudsdk.EnhancedExternalAuthProviderGitHub),
			ID:   "$hi$",
		}},
		...
		Name: "NoDeviceURL",
		Input: []nicloudsdk.ExternalAuthConfig{{
			Type:         string(nicloudsdk.EnhancedExternalAuthProviderGitLab),
			ClientID:     "example",
			ClientSecret: "example",
			DeviceFlow:   true,
		}},
	...
	t.Run("CustomScopesAndEndpoint", func(t *testing.T) {
		t.Parallel()
		config, err := externalauth.ConvertConfig(instrument, []nicloudsdk.ExternalAuthConfig{{
			Type:         string(nicloudsdk.EnhancedExternalAuthProviderGitLab),
			ClientID:     "id",
			ClientSecret: "secret",
...
func TestConstantQueryParams(t *testing.T) {
	...
	fake, config, _ := setupOauth2Test(t, testConfig{
		...
		CoderOIDCConfigOpts: []func(cfg *nicloud.OIDCConfig){
			func(cfg *nicloud.OIDCConfig) {
	...
	// We are not using a real server, so it fakes https://cloud.neuralinverse.com
