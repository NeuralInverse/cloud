...
// @Produce json
// @Tags AI Providers
// @Success 200 {array} nicloudsdk.AIProvider
...
func (api *API) aiProvidersList(rw http.ResponseWriter, r *http.Request) {
	...
	out := make([]nicloudsdk.AIProvider, 0, len(rows))
...
// @Success 200 {object} nicloudsdk.AIProvider
...
// @Produce json
// @Tags AI Providers
// @Param request body nicloudsdk.CreateAIProviderRequest true "Create AI provider request"
// @Success 201 {object} nicloudsdk.AIProvider
...
func (api *API) aiProvidersCreate(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateAIProviderRequest
...
// @Tags AI Providers
// @Param idOrName path string true "Provider ID or name"
// @Param request body nicloudsdk.UpdateAIProviderRequest true "Update AI provider request"
// @Success 200 {object} nicloudsdk.AIProvider
...
func (api *API) aiProvidersUpdate(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateAIProviderRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
func lookupAIProvider(ctx context.Context, store database.Store, idOrName string) (database.AIProvider, error) {
	...
	if !nicloudsdk.AIProviderNameRegex.MatchString(idOrName) {
...
func writeAIProviderError(ctx context.Context, logger slog.Logger, rw http.ResponseWriter, err error, logMsg, userMsg string) {
	if errors.Is(err, errAIProviderInvalidName) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
			Message: fmt.Sprintf("Invalid provider id or name: must be a UUID or match %s.", nicloudsdk.AIProviderNameRegex),
		})
		return
	}
...
func applyAIProviderKeyOps(ctx context.Context, tx database.Store, providerID uuid.UUID, muts []nicloudsdk.AIProviderKeyMutation) ([]database.AIProviderKey, aiProviderKeyOpsAudit, aiProviderKeyChanges, error) {
	var (
...
func encodeAIProviderSettings(s nicloudsdk.AIProviderSettings) (sql.NullString, error) {
	if s.IsZero() {
...
// in a single PATCH.
func mergeAIProviderSettings(existing, patch nicloudsdk.AIProviderSettings) nicloudsdk.AIProviderSettings {
	if patch.Bedrock == nil {
		// Patch carries no type-specific data; treat as a clear.
		return nicloudsdk.AIProviderSettings{}
	}
	...
	return nicloudsdk.AIProviderSettings{Bedrock: &merged}
}
