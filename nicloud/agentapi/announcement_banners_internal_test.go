...
func TestGetAnnouncementBanners(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		cfg := []nicloudsdk.BannerConfig{{
			Enabled:         true,
			Message:         "The beep-bop will be boop-beeped on Saturday at 12AM PST.",
			BackgroundColor: "#00FF00",
		}}

		var ff appearance.Fetcher = fakeFetcher{cfg: nicloudsdk.AppearanceConfig{AnnouncementBanners: cfg}}
...
func (f fakeFetcher) Fetch(context.Context) (nicloudsdk.AppearanceConfig, error) {
	return f.cfg, f.err
}
