...
func SeedAIProvidersFromEnv(
	...
	err = db.InTx(func(tx database.Store) error {
		...
		for _, dp := range desired {
			settings, err := encodeAIProviderSettings(nicloudsdk.AIProviderSettings{Bedrock: dp.Bedrock})
			if err != nil {
			...
			case found:
				...
				return xerrors.Errorf("AI provider %q already exists in the database and differs from the current environment configuration; update the provider through the API or remove the NEURALINVERSE_AIBRIDGE_* env vars to stop seeding it", dp.Name)
			}
...
func computeKeysHash(bearerKeys []string, bedrock *nicloudsdk.AIProviderBedrockSettings) string {
...
// (legacy single-provider env vars and indexed NEURALINVERSE_AIBRIDGE_PROVIDER_<N>_*
...
func providersFromEnv(ctx context.Context, cfg nicloudsdk.AIBridgeConfig, logger slog.Logger) ([]desiredAIProvider, error) {
	...
	// Bedrock provider.
	bedrock := nicloudsdk.NewAIProviderBedrockSettings(
	...
	hasLegacyBedrock := nicloudsdk.IsBedrockConfigured(cfg.LegacyBedrock.BaseURL.String(), bedrock)
	if hasAnthropicKey || hasLegacyBedrock {
		...
		if hasLegacyBedrock {
			...
			// Bedrock-only deployments use NEURALINVERSE_AIBRIDGE_BEDROCK_BASE_URL
	...
	for _, p := range cfg.Providers {
		...
		if !nicloudsdk.AIProviderNameRegex.MatchString(name) {
			return nil, xerrors.Errorf("invalid AI provider name %q: must match %s", name, nicloudsdk.AIProviderNameRegex)
		}
		...
		if dp.Type == database.AiProviderTypeAnthropic || dp.Type == database.AiProviderTypeBedrock {
			...
			isBedrock = nicloudsdk.IsBedrockConfigured(p.BedrockBaseURL, bedrock)
			if isBedrock {
				dp.Bedrock = &bedrock
