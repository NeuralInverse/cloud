...
//
// appHostIsPrimary is true if the app host is also the primary neuralinverse API
...
func Run(t *testing.T, appHostIsPrimary bool, factory DeploymentFactory) {
	...
	t.Run("ReconnectingPTY", func(t *testing.T) {
		...
		t.Run("SignedTokenQueryParameter", func(t *testing.T) {
			...
			issueRes, err := appDetails.SDKClient.IssueReconnectingPTYSignedToken(ctx, nicloudsdk.IssueReconnectingPTYSignedTokenRequest{
				URL:     u.String(),
			...
			unauthedAppClient := nicloudsdk.New(appDetails.AppClient(t).URL)
	...
	t.Run("WorkspaceAppsProxyPath", func(t *testing.T) {
		...
		t.Run("NoAccessShould404", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, nil)
			userClient, _ := nicloudtest.CreateAnotherUser(t, appDetails.SDKClient, appDetails.FirstUser.OrganizationID, rbac.RoleMember())
		...
		t.Run("Proxies", func(t *testing.T) {
			...
			appTokenCookie := mustFindCookie(t, resp.Cookies(), nicloudsdk.SignedAppTokenCookie)
		...
		t.Run("BadJWT", func(t *testing.T) {
			...
			newTokenCookie := mustFindCookie(t, resp.Cookies(), nicloudsdk.SignedAppTokenCookie)
	...
	t.Run("WorkspaceApplicationCORS", func(t *testing.T) {
		...
		unauthenticatedClient := func(t *testing.T, appDetails *Details) *nicloudsdk.Client {
			c := appDetails.AppClient(t)
			c.SetSessionToken("")
			return c
		}

		authenticatedClient := func(t *testing.T, appDetails *Details) *nicloudsdk.Client {
			uc, _ := nicloudtest.CreateAnotherUser(t, appDetails.SDKClient, appDetails.FirstUser.OrganizationID, rbac.RoleMember())
			c := appDetails.AppClient(t)
		...
		tests := []struct {
			...
			client               func(t *testing.T, appDetails *Details) *nicloudsdk.Client
			behavior             nicloudsdk.CORSBehavior
			httpMethod           string
			...
			{ // fails
				...
				behavior:           nicloudsdk.CORSBehaviorSimple,
			...
			{ // fails
				...
				behavior:           nicloudsdk.CORSBehaviorPassthru,
				client:             unauthenticatedClient,
				origin:             ownSubdomain,
				httpMethod:         http.MethodOptions,
				expectedStatusCode: http.StatusOK,
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				appDetails := setupProxyTest(t, &DeploymentOptions{
					handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						_, err := r.Cookie(nicloudsdk.SessionTokenCookie)
						...
						switch tc.behavior {
						case nicloudsdk.CORSBehaviorPassthru:
						...
						case nicloudsdk.CORSBehaviorSimple:
				...
				b := tc.behavior
				template, err := appDetails.SDKClient.UpdateTemplateMeta(ctx, appDetails.Workspace.TemplateID, nicloudsdk.UpdateTemplateMeta{
					CORSBehavior: &b,
				})
				require.NoError(t, err)
		...
		t.Run("End-to-End", func(t *testing.T) {
			...
			cases := []struct {
			...
			}{
				{
					...
					sessionTokenCookieName: nicloudsdk.SubdomainAppSessionTokenCookie,
				},
				{
					name:                   "Path",
					appURL:                 appDetails.PathAppURL(appDetails.Apps.Owner),
					sessionTokenCookieName: nicloudsdk.PathAppSessionTokenCookie,
				},
			}
			...
			for _, c := range cases {
				...
				t.Run(c.name, func(t *testing.T) {
					...
					user, err := appDetails.SDKClient.User(ctx, nicloudsdk.Me)
					require.NoError(t, err)
					...
					resp, err = requestWithRetries(ctx, t, appDetails.SDKClient, http.MethodGet, "/api/v2/applications/auth-redirect", nil, nicloudsdk.WithQueryParam(
					...
					require.Equal(t, nicloudsdk.LoginTypePassword, apiKeyInfo.LoginType)
					...
					appTokenAPIClient := nicloudsdk.New(appDetails.SDKClient.URL)
					...
					authRes, err := appTokenAPIClient.AuthCheck(ctx, nicloudsdk.AuthorizationRequest{
						Checks: map[string]nicloudsdk.AuthorizationCheck{
							canApplicationConnect: {
								...
								Action: nicloudsdk.ActionApplicationConnect,
							},
							canReadUserMe: {
								Object: nicloudsdk.AuthorizationObject{
									ResourceType: "user",
					...
					req, err = http.NewRequestWithContext(ctx, "GET", gotLocation.String(), nil)
					require.NoError(t, err)
					req.Header.Set(nicloudsdk.SessionTokenHeader, apiKey)
					resp, err = doWithRetries(t, appClient, req)
					require.NoError(t, err)
					resp.Body.Close()
					require.Equal(t, http.StatusOK, resp.StatusCode)
	...
	t.Run("WorkspaceAppsProxySubdomainHostnamePrefix/Different", func(t *testing.T) {
		...
		for _, c := range resp.Cookies() {
			if c.Name == nicloudsdk.SignedAppTokenCookie {
				appTokenCookie = c
				break
			}
		}
	...
	t.Run("WorkspaceAppsProxySubdomainPassthrough", func(t *testing.T) {
		...
		u.Host = "app--agent--workspace--username.test.cloud.neuralinverse.com"
		u.Path = "/api/v2/users/me"
		resp, err := requestWithRetries(ctx, t, appDetails.AppClient(t), http.MethodGet, u.String(), nil)
		require.NoError(t, err)
		defer resp.Body.Close()

		// Should look like a nicloudsdk.User response.
		require.Equal(t, http.StatusOK, resp.StatusCode)
	...
	t.Run("WorkspaceAppsProxySubdomain", func(t *testing.T) {
		...
		t.Run("ProxyPortMinimumError", func(t *testing.T) {
			...
			require.Equal(t, http.StatusBadRequest, resp.StatusCode)
			var resBody nicloudsdk.Response
		...
		t.Run("SuffixWildcardOK", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, &DeploymentOptions{
				AppHost: "*-suffix.test.cloud.neuralinverse.com",
			})
		...
		t.Run("WildcardPortOK", func(t *testing.T) {
			...
			appDetails := setupProxyTest(t, &DeploymentOptions{
				...
				AppHost:        "*.test.cloud.neuralinverse.com:4444",
				PrimaryAppHost: "*.test.cloud.neuralinverse.com:4444",
			})
			...
			require.NoError(t, err)
			require.Equal(t, "*.test.cloud.neuralinverse.com:4444", apphost.Host, "apphost has port")
	...
	t.Run("PortSharing", func(t *testing.T) {
		...
		t.Run("AuthenticatedOK", func(t *testing.T) {
			...
			_, err = appDetails.SDKClient.UpsertWorkspaceAgentPortShare(ctx, appDetails.Workspace.ID, nicloudsdk.UpsertWorkspaceAgentPortShareRequest{
	...
	t.Run("CORS", func(t *testing.T) {
		...
		ownerClient := func(t *testing.T, appDetails *Details) *nicloudsdk.Client {
		...
		tests := []struct {
			name                string
			shareLevel          nicloudsdk.WorkspaceAgentPortShareLevel
			behavior            nicloudsdk.CORSBehavior
			client              func(t *testing.T, appDetails *Details) *nicloudsdk.Client
			expectedStatusCode  int
			...
			{
				name:                "Default/Owner",
				shareLevel:          nicloudsdk.WorkspaceAgentPortShareLevelAuthenticated, // Owner is not a valid share level for ports.
				behavior:            nicloudsdk.CORSBehaviorSimple,
				expectedCORSHeaders: false,
				client:              ownerClient,
				expectedStatusCode:  http.StatusOK,
	...
	t.Run("AppSharing", func(t *testing.T) {
		...
		setup := func(t *testing.T, allowPathAppSharing, allowSiteOwnerAccess bool) (appDetails *Details, workspace nicloudsdk.Workspace, agnt nicloudsdk.WorkspaceAgent, user nicloudsdk.User, ownerClient *nicloudsdk.Client, client *nicloudsdk.Client, clientInOtherOrg *nicloudsdk.Client, clientWithNoAuth *nicloudsdk.Client) {
			//nolint:gosec
			...
			user, err := ownerClient.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			...
			_, err = ownerClient.UpdateUserRoles(ctx, user.ID.String(), nicloudsdk.UpdateRoles{
				Roles: []string{"template-admin", "member"},
			})
			require.NoError(t, err)

			client = nicloudsdk.New(ownerClient.URL)
			...
			found := map[string]nicloudsdk.WorkspaceAppSharingLevel{}
			expected := map[string]nicloudsdk.WorkspaceAppSharingLevel{
				proxyTestAppNameFake:                      nicloudsdk.WorkspaceAppSharingLevelOwner,
				proxyTestAppNameOwner:                     nicloudsdk.WorkspaceAppSharingLevelOwner,
				proxyTestAppNameAuthenticated:             nicloudsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublic:                    nicloudsdk.WorkspaceAppSharingLevelPublic,
				proxyTestAppNameAuthenticatedCORSPassthru: nicloudsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublicCORSPassthru:        nicloudsdk.WorkspaceAppSharingLevelPublic,
				proxyTestAppNameAuthenticatedCORSDefault:  nicloudsdk.WorkspaceAppSharingLevelAuthenticated,
				proxyTestAppNamePublicCORSDefault:         nicloudsdk.WorkspaceAppSharingLevelPublic,
			}
			...
			otherOrg, err := ownerClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
				Name: "a-different-org",
			})
			require.NoError(t, err)
			userInOtherOrg, err := ownerClient.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
				Email:           "no-template-access@cloud.neuralinverse.com",
			...
			clientInOtherOrg = nicloudsdk.New(client.URL)
			loginRes, err = clientInOtherOrg.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
			...
			// Create an unauthenticated nicloudsdk client.
			clientWithNoAuth = nicloudsdk.New(client.URL)
		...
		verifyAccess := func(t *testing.T, appDetails *Details, isPathApp bool, username, workspaceName, agentName, appName string, client *nicloudsdk.Client, shouldHaveAccess, shouldRedirectToLogin bool) {
			t.Helper()
			...
			if client.SessionToken() != "" {
				token, err := client.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{
					Scope: nicloudsdk.APIKeyScopeApplicationConnect,
				})
				require.NoError(t, err)
	...
	t.Run("WorkspaceAppsNonCanonicalHeaders", func(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				req.Header.Set(nicloudsdk.SessionTokenHeader, appDetails.SDKClient.SessionToken())
	...
	t.Run("WorkspaceOffline", func(t *testing.T) {
		...
		_ = nicloudtest.MustTransitionWorkspace(t, appDetails.SDKClient, appDetails.Workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
...
func testReconnectingPTY(ctx context.Context, t *testing.T, client *nicloudsdk.Client, agentID uuid.UUID, signedToken string) {
