...
const (
	...
	proxyTestSubdomainRaw = "*.test.cloud.neuralinverse.com"
	proxyTestSubdomain    = "test.cloud.neuralinverse.com"
)
...
type Deployment struct {
	...
	FirstUser      nicloudsdk.CreateFirstUserResponse
...
type App struct {
	...
	CORSBehavior nicloudsdk.CORSBehavior
}
...
// AppClient returns a *nicloudsdk.Client that will route all requests to the
...
func (d *Details) AppClient(t *testing.T) *nicloudsdk.Client {
	client := nicloudsdk.New(d.PathAppBaseURL, nicloudsdk.WithSessionToken(d.SDKClient.SessionToken()))
...
func setupProxyTestWithFactory(t *testing.T, factory DeploymentFactory, opts *DeploymentOptions) *Details {
	...
	// requests regardless of hostname to the nicloud test server.
	...
	me, err := deployment.SDKClient.User(ctx, nicloudsdk.Me)
	...
	details.Apps.PublicCORSPassthru = App{
		...
		CORSBehavior:  nicloudsdk.CORSBehaviorPassthru,
		Query:         proxyTestAppQuery,
	}
...
//nolint:revive
func createWorkspaceWithApps(t *testing.T, client *nicloudsdk.Client, orgID uuid.UUID, me nicloudsdk.User, port uint16, serveHTTPS bool, workspaceMutators ...func(*nicloudsdk.CreateWorkspaceRequest)) (nicloudsdk.Workspace, nicloudsdk.WorkspaceAgent) {
	...
	workspaceMutators = append([]func(*nicloudsdk.CreateWorkspaceRequest){
		func(req *nicloudsdk.CreateWorkspaceRequest) {
	...
	workspace := nicloudtest.CreateWorkspace(t, client, template.ID, workspaceMutators...)
	...
	agents := make([]nicloudsdk.WorkspaceAgent, 0, 1)
...
func doWithRetries(t require.TestingT, client *nicloudsdk.Client, req *http.Request) (*http.Response, error) {
...
func requestWithRetries(ctx context.Context, t testing.TB, client *nicloudsdk.Client, method, urlOrPath string, body interface{}, opts ...nicloudsdk.RequestOption) (*http.Response, error) {
	t.Helper()
	var resp *http.Response
	var err error
	require.Eventually(t, func() bool {
...
func forceURLTransport(t *testing.T, client *nicloudsdk.Client) {
