...
func TestAppCookies(t *testing.T) {
	...
	const (
		...
		expectedSubdomainCookie = nicloudsdk.SubdomainAppSessionTokenCookie + "_" + hash
	)

	cookies := workspaceapps.NewAppCookies(domain)
	require.Equal(t, nicloudsdk.PathAppSessionTokenCookie, cookies.PathAppSessionToken)
	require.Equal(t, expectedSubdomainCookie, cookies.SubdomainAppSessionToken)
	require.Equal(t, nicloudsdk.SignedAppTokenCookie, cookies.SignedAppToken)
