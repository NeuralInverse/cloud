...
func NewAppCookies(hostname string) AppCookies {
	return AppCookies{
		PathAppSessionToken:      nicloudsdk.PathAppSessionTokenCookie,
		SubdomainAppSessionToken: SubdomainAppSessionTokenCookie(hostname),
		SignedAppToken:           nicloudsdk.SignedAppTokenCookie,
	}
}
...
//
// E.g. `*.dev.cloud.neuralinverse.com` and `*.sydney.dev.cloud.neuralinverse.com`
//
// If you have an expired cookie on the primary proxy (valid for
// `*.dev.cloud.neuralinverse.com`), your browser will send it on all requests to the Sydney
...
func SubdomainAppSessionTokenCookie(hostname string) string {
	...
	return nicloudsdk.SubdomainAppSessionTokenCookie + "_" + str
}
...
// Coder token extraction (query parameters, Neural Inverse Cloud-Session-Token header, etc.).
func (c AppCookies) TokenFromRequest(r *http.Request, accessMethod AccessMethod) string {
	...
	// that upstream header as a Neural Inverse Cloud session token and ignore the app session
	...
	// Fall back to standard Neural Inverse Cloud token extraction (session cookie, query param,
	// Neural Inverse Cloud-Session-Token header, and then Authorization: Bearer).
