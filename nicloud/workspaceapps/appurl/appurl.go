...
//
// Arguments:
// apphost: Expected to contain a wildcard, example: "*.cloud.neuralinverse.com"
...
func CompileHostnamePattern(pattern string) (*regexp.Regexp, error) {
	...
	for i, label := range strings.Split(pattern, ".") {
		if i == 0 {
			...
			// test below. This makes `*.cloud.neuralinverse.com` become `a.cloud.neuralinverse.com` and
			// `*--prod.cloud.neuralinverse.com` become `a--prod.cloud.neuralinverse.com`.
			label = "a" + label
		}
...
// For example *--apps.cloud.neuralinverse.com must become *.cloud.neuralinverse.com.  If there is no
