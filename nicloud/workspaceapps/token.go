...
type SignedToken struct {
	...
	CORSBehavior nicloudsdk.CORSBehavior `json:"cors_behavior"`
}
...
func FromRequest(r *http.Request, mgr cryptokeys.SigningKeycache) (*SignedToken, bool) {
	...
	if q := r.URL.Query().Get(nicloudsdk.SignedAppTokenQueryParameter); q != "" {
		hasQueryParam = true
		tokens = append(tokens, q)
	}
	for _, cookie := range r.Cookies() {
		if cookie.Name == nicloudsdk.SignedAppTokenCookie {
