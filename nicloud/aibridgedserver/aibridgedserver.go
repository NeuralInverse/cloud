...
func NewServer(lifecycleCtx context.Context, store store, logger slog.Logger, accessURL string,
	bridgeCfg nicloudsdk.AIBridgeConfig, externalAuthConfigs []*externalauth.Config, experiments nicloudsdk.Experiments,
	aiSeatTracker aiseats.SeatTracker,
) (*Server, error) {
	...
	if bridgeCfg.InjectCoderMCPTools {
		...
		if err != nil {
			logger.Warn(lifecycleCtx, "failed to retrieve neuralinverse MCP server config, Coder MCP will not be available", slog.Error(err))
		}
...
// IsAuthorized validates a given Neural Inverse Cloud API key and returns the user ID to which it belongs (if valid).
//
...
func getCoderMCPServerConfig(experiments nicloudsdk.Experiments, accessURL string) (*proto.MCPServerConfig, error) {
	// Both the MCP & OAuth2 experiments are currently required in order to use our
	// internal MCP server.
	if !experiments.Enabled(nicloudsdk.ExperimentMCPServerHTTP) {
		return nil, xerrors.Errorf("%q experiment not enabled", nicloudsdk.ExperimentMCPServerHTTP)
	}
	if !experiments.Enabled(nicloudsdk.ExperimentOAuth2) {
		return nil, xerrors.Errorf("%q experiment not enabled", nicloudsdk.ExperimentOAuth2)
	}
