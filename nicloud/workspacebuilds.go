...
// @Success 200 {object} nicloudsdk.WorkspaceBuild
...
// @Success 200 {array} nicloudsdk.WorkspaceBuild
...
// @Tags Builds
// @Param workspace path string true "Workspace ID" format(uuid)
// @Param request body nicloudsdk.CreateWorkspaceBuildRequest true "Create workspace build request"
// @Success 200 {object} nicloudsdk.WorkspaceBuild
...
func (api *API) postWorkspaceBuilds(rw http.ResponseWriter, r *http.Request) {
	...
	workspace := httpmw.WorkspaceParam(r)
	var createBuild nicloudsdk.CreateWorkspaceBuildRequest
...
func (api *API) postWorkspaceBuildsInternal(
	...
	createBuild nicloudsdk.CreateWorkspaceBuildRequest,
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		if createBuild.Orphan {
			if createBuild.Transition != nicloudsdk.WorkspaceTransitionDelete {
	...
	if provisionerJob != nil {
		...
		if createBuild.Orphan && createBuild.Transition == nicloudsdk.WorkspaceTransitionDelete && provisionerJob.CompletedAt.Valid {
	...
	if err != nil {
		return nicloudsdk.WorkspaceBuild{}, httperror.NewResponseError(
...
func (api *API) patchCancelWorkspaceBuild(rw http.ResponseWriter, r *http.Request) {
	...
	if api.Experiments.Enabled(nicloudsdk.ExperimentWorkspaceBuildUpdates) {
		err = wspubsub.PublishWorkspaceBuildUpdate(ctx, api.Pubsub, nicloudsdk.WorkspaceBuildUpdate{
			WorkspaceID:   workspace.ID,
			WorkspaceName: workspace.Name,
...
// @Tags Builds
// @Param workspacebuild path string true "Workspace build ID"
// @Success 200 {array} nicloudsdk.WorkspaceBuildParameter
...
// @Success 200 {array} nicloudsdk.ProvisionerJobLog
...
// @Param request body nicloudsdk.UpdateWorkspaceBuildStateRequest true "Request body"
// @Success 204
...
func (api *API) workspaceBuildUpdateState(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateWorkspaceBuildStateRequest
...
func (api *API) convertWorkspaceBuilds(
	...
	apiBuilds := []nicloudsdk.WorkspaceBuild{}
	for _, build := range workspaceBuilds {
...
func (api *API) convertWorkspaceBuild(
	...
	apiResources := make([]nicloudsdk.WorkspaceResource, 0)
	...
	for _, resource := range resources {
		...
		apiAgents := make([]nicloudsdk.WorkspaceAgent, 0)
		...
		for _, agent := range agents {
			...
			if err != nil {
				return nicloudsdk.WorkspaceBuild{}, xerrors.Errorf("converting workspace agent: %w", err)
			}
	...
	transition := nicloudsdk.WorkspaceTransition(build.Transition)
	return nicloudsdk.WorkspaceBuild{
		ID:                      build.ID,
		...
		Job:                     apiJob,
		Deadline:                nicloudsdk.NewNullTime(build.Deadline, !build.Deadline.IsZero()),
		MaxDeadline:             nicloudsdk.NewNullTime(build.MaxDeadline, !build.MaxDeadline.IsZero()),
		Reason:                  nicloudsdk.BuildReason(build.Reason),
		Resources:               apiResources,
		Status:                  nicloudsdk.ConvertWorkspaceStatus(apiJob.Status, transition),
...
func convertWorkspaceResource(resource database.WorkspaceResource, agents []nicloudsdk.WorkspaceAgent, metadata []database.WorkspaceResourceMetadatum) nicloudsdk.WorkspaceResource {
	var convertedMetadata []nicloudsdk.WorkspaceResourceMetadata
	for _, field := range metadata {
		convertedMetadata = append(convertedMetadata, nicloudsdk.WorkspaceResourceMetadata{
			Key:       field.Key,
	...
	return nicloudsdk.WorkspaceResource{
		ID:         resource.ID,
		CreatedAt:  resource.CreatedAt,
		JobID:      resource.JobID,
		Transition: nicloudsdk.WorkspaceTransition(resource.Transition),
		Type:       resource.Type,
		Name:       resource.Name,
		Hide:       resource.Hide,
		Icon:       resource.Icon,
...
func (api *API) buildTimings(ctx context.Context, build database.WorkspaceBuild) (nicloudsdk.WorkspaceBuildTimings, error) {
	provisionerTimings, err := api.Database.GetProvisionerJobTimingsByJobID(ctx, build.JobID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nicloudsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching provisioner job timings: %w", err)
	}
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nicloudsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace agent script timings: %w", err)
	}

	resources, err := api.Database.GetWorkspaceResourcesByJobID(ctx, build.JobID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nicloudsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace resources: %w", err)
	}
	...
	agents, err := api.Database.GetWorkspaceAgentsByResourceIDs(dbauthz.AsSystemRestricted(ctx), resourceIDs)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nicloudsdk.WorkspaceBuildTimings{}, xerrors.Errorf("fetching workspace agents: %w", err)
	}

	res := nicloudsdk.WorkspaceBuildTimings{
		ProvisionerTimings:     make([]nicloudsdk.ProvisionerTiming, 0, len(provisionerTimings)),
		AgentScriptTimings:     make([]nicloudsdk.AgentScriptTiming, 0, len(agentScriptTimings)),
		AgentConnectionTimings: make([]nicloudsdk.AgentConnectionTiming, 0, len(agents)),
	}
	...
	for _, t := range provisionerTimings {
		...
		res.ProvisionerTimings = append(res.ProvisionerTimings, nicloudsdk.ProvisionerTiming{
			JobID:     t.JobID,
			Stage:     nicloudsdk.TimingStage(t.Stage),
			Source:    t.Source,
			Action:    t.Action,
			Resource:  t.Resource,
	...
	for _, t := range agentScriptTimings {
		...
		res.AgentScriptTimings = append(res.AgentScriptTimings, nicloudsdk.AgentScriptTiming{
			StartedAt:          t.StartedAt,
			EndedAt:            t.EndedAt,
			ExitCode:           t.ExitCode,
			Stage:              nicloudsdk.TimingStage(t.Stage),
	...
	for _, agent := range agents {
		...
		res.AgentConnectionTimings = append(res.AgentConnectionTimings, nicloudsdk.AgentConnectionTiming{
			WorkspaceAgentID:   agent.ID.String(),
			WorkspaceAgentName: agent.Name,
			StartedAt:          agent.CreatedAt,
			Stage:              nicloudsdk.TimingStageConnect,
