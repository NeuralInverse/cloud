...
// @Param request body nicloudsdk.CreateUserSecretRequest true "Create secret request"
// @Success 201 {object} nicloudsdk.UserSecret
...
func (api *API) postUserSecret(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateUserSecretRequest
...
// @Tags Secrets
// @Param user path string true "User ID, username, or me"
// @Success 200 {array} nicloudsdk.UserSecret
...
// @Success 200 {object} nicloudsdk.UserSecret
...
// @Param user path string true "User ID, username, or me"
// @Param name path string true "Secret name"
// @Param request body nicloudsdk.UpdateUserSecretRequest true "Update secret request"
// @Success 200 {object} nicloudsdk.UserSecret
...
func (api *API) patchUserSecret(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateUserSecretRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
func writeUserSecretValidationErrors(ctx context.Context, rw http.ResponseWriter, status int, validations []nicloudsdk.ValidationError) {
...
func createUserSecretValidationErrors(req nicloudsdk.CreateUserSecretRequest) []nicloudsdk.ValidationError {
	var validations []nicloudsdk.ValidationError
	validations = appendUserSecretValidationError(validations, userSecretNameField, nicloudsdk.UserSecretNameValid(req.Name))
	if req.Value == "" {
		...
		validations = appendUserSecretValidationError(validations, userSecretValueField, nicloudsdk.UserSecretValueValid(req.Value))
	}
	validations = appendUserSecretValidationError(validations, userSecretEnvNameField, nicloudsdk.UserSecretEnvNameValid(req.EnvName))
	validations = appendUserSecretValidationError(validations, userSecretFilePathField, nicloudsdk.UserSecretFilePathValid(req.FilePath))
	return validations
}

func updateUserSecretValidationErrors(req nicloudsdk.UpdateUserSecretRequest) []nicloudsdk.ValidationError {
	var validations []nicloudsdk.ValidationError
	if req.Value != nil {
		validations = appendUserSecretValidationError(validations, userSecretValueField, nicloudsdk.UserSecretValueValid(*req.Value))
	}
...
func appendUserSecretValidationError(validations []nicloudsdk.ValidationError, field string, err error) []nicloudsdk.ValidationError {
	if err == nil {
		return validations
	}
	return append(validations, nicloudsdk.ValidationError{
		Field:  field,
		Detail: err.Error(),
	})
}
...
// nicloudsdk.MaxUserSecretsPerUserCount for the rationale behind the caps.
func userSecretLimitResponse(err error) (nicloudsdk.Response, bool) {
	switch {
	case database.IsCheckViolation(err, userSecretsCountLimitConstraint):
		return nicloudsdk.Response{
			...
			Detail: fmt.Sprintf(
				...
				nicloudsdk.MaxUserSecretsPerUserCount,
			),
		}, true
	case database.IsCheckViolation(err, userSecretsTotalBytesLimitConstraint):
		return nicloudsdk.Response{
			...
			Detail: fmt.Sprintf(
				...
				nicloudsdk.MaxUserSecretsTotalValueBytes,
			),
		}, true
	case database.IsCheckViolation(err, userSecretsEnvBytesLimitConstraint):
		return nicloudsdk.Response{
			...
			Detail: fmt.Sprintf(
				...
				nicloudsdk.MaxUserSecretValueBytes,
			),
		}, true
	}
	return nicloudsdk.Response{}, false
}

func userSecretConflictValidationErrors(err error) []nicloudsdk.ValidationError {
	switch {
