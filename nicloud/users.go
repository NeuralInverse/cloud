...
// @Produce json
// @Tags Users
// @Success 200 {object} nicloudsdk.OIDCClaimsResponse
...
func (api *API) userOIDCClaims(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.OIDCClaimsResponse{
		Claims: claims,
	})
}
...
// Creates the initial user for a Neural Inverse Cloud deployment.
//
...
// @Produce json
// @Tags Users
// @Param request body nicloudsdk.CreateFirstUserRequest true "First user request"
// @Success 201 {object} nicloudsdk.CreateFirstUserResponse
...
func (api *API) postFirstUser(rw http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	var createUser nicloudsdk.CreateFirstUserRequest
	...
	if createUser.Trial && api.TrialGenerator != nil {
		err = api.TrialGenerator(ctx, nicloudsdk.LicensorTrialRequest{
	...
	user, err := api.CreateUser(dbauthz.AsSystemRestricted(ctx), api.Database, CreateUserRequest{
		CreateUserRequestWithOrgs: nicloudsdk.CreateUserRequestWithOrgs{
			Email:    createUser.Email,
		...
		accountCreatorName: "neuralinverse",
	})
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
	...
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.CreateFirstUserResponse{
		UserID:         user.ID,
...
// @Success 200 {object} nicloudsdk.GetUsersResponse
...
func (api *API) users(rw http.ResponseWriter, r *http.Request) {
	...
	if api.Entitlements.Enabled(nicloudsdk.FeatureAIGovernanceUserLimit) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.GetUsersResponse{
...
// @Produce json
// @Tags Users
// @Param request body nicloudsdk.CreateUserRequestWithOrgs true "Create user request"
// @Success 201 {object} nicloudsdk.User
...
func (api *API) postUser(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateUserRequestWithOrgs
	...
	if req.ServiceAccount {
		...
		// them know.
		if req.UserLoginType != "" && req.UserLoginType != nicloudsdk.LoginTypeNone {
			httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
		...
		if !api.Entitlements.Enabled(nicloudsdk.FeatureServiceAccounts) {
			httpapi.Write(ctx, rw, http.StatusForbidden, nicloudsdk.Response{
				Message: fmt.Sprintf("%s is a Premium feature. Contact sales!", nicloudsdk.FeatureServiceAccounts.Humanize()),
			})
			return
		}
	} else if req.UserLoginType == "" {
		// Default to password auth
		req.UserLoginType = nicloudsdk.LoginTypePassword
	}

	if req.UserLoginType != nicloudsdk.LoginTypePassword && req.Password != "" {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	if api.DeploymentValues.DisablePasswordAuth && req.UserLoginType == nicloudsdk.LoginTypePassword {
		httpapi.Write(ctx, rw, http.StatusForbidden, nicloudsdk.Response{
...
func (api *API) deleteUser(rw http.ResponseWriter, r *http.Request) {
	...
	if len(workspaces) > 0 {
		httpapi.Write(ctx, rw, http.StatusExpectationFailed, nicloudsdk.Response{
...
// @Success 200 {object} nicloudsdk.User
...
// @Success 200 {array} nicloudsdk.UserParameter
...
func (api *API) userAutofillParameters(rw http.ResponseWriter, r *http.Request) {
	...
	sdkParams := []nicloudsdk.UserParameter{}
	for _, param := range params {
		sdkParams = append(sdkParams, nicloudsdk.UserParameter{
			Name:  param.Name,
			Value: param.Value,
		})
	}
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} nicloudsdk.UserLoginType
...
func (*API) userLoginType(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.UserLoginType{
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Param request body nicloudsdk.UpdateUserProfileRequest true "Updated profile"
// @Success 200 {object} nicloudsdk.User
...
func (api *API) putUserProfile(rw http.ResponseWriter, r *http.Request) {
	...
	var params nicloudsdk.UpdateUserProfileRequest
	...
	if err == nil && isDifferentUser {
		responseErrors := []nicloudsdk.ValidationError{{
			Field:  "username",
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} nicloudsdk.UserAppearanceSettings
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Param request body nicloudsdk.UpdateUserAppearanceSettingsRequest true "New appearance settings"
// @Success 200 {object} nicloudsdk.UserAppearanceSettings
...
func (api *API) putUserAppearanceSettings(rw http.ResponseWriter, r *http.Request) {
	...
	var params nicloudsdk.UpdateUserAppearanceSettingsRequest
	if !httpapi.Read(ctx, rw, r, &params) {
		return
	}
	...
	themeModeProvided := params.ThemeMode != nicloudsdk.ThemeModeUnset
	updateThemeMode := themeModeProvided
	isSyncMode := params.ThemeMode == nicloudsdk.ThemeModeSync
	isSingleMode := params.ThemeMode == nicloudsdk.ThemeModeSingle
	...
	if !updateThemeMode && isLegacyAutoThemePreference(params.ThemePreference) {
		...
		themeMode = nicloudsdk.ThemeModeUnset
	}
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} nicloudsdk.UserPreferenceSettings
...
func (api *API) userPreferenceSettings(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.UserPreferenceSettings{
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Param request body nicloudsdk.UpdateUserPreferenceSettingsRequest true "New preference settings"
// @Success 200 {object} nicloudsdk.UserPreferenceSettings
...
func (api *API) putUserPreferenceSettings(rw http.ResponseWriter, r *http.Request) {
	...
	var params nicloudsdk.UpdateUserPreferenceSettingsRequest
	if !httpapi.Read(ctx, rw, r, &params) {
		return
	}

	if params.ThinkingDisplayMode != "" &&
		!slices.Contains(nicloudsdk.ValidThinkingDisplayModes, params.ThinkingDisplayMode) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	if params.ShellToolDisplayMode != "" &&
		!slices.Contains(nicloudsdk.ValidAgentDisplayModes, params.ShellToolDisplayMode) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	if params.CodeDiffDisplayMode != "" &&
		!slices.Contains(nicloudsdk.ValidAgentDisplayModes, params.CodeDiffDisplayMode) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	if params.AgentChatSendShortcut != "" &&
		!slices.Contains(nicloudsdk.ValidAgentChatSendShortcuts, params.AgentChatSendShortcut) {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	var settings nicloudsdk.UserPreferenceSettings
	err := api.Database.InTx(func(tx database.Store) error {
		var err error
...
func sanitizeThinkingDisplayMode(raw string) nicloudsdk.ThinkingDisplayMode {
	mode := nicloudsdk.ThinkingDisplayMode(raw)
	if slices.Contains(nicloudsdk.ValidThinkingDisplayModes, mode) {
		return mode
	}
	return nicloudsdk.ThinkingDisplayModeAuto
}

func sanitizeShellToolDisplayMode(raw string) nicloudsdk.AgentDisplayMode {
	mode := sanitizeAgentDisplayMode(raw)
	if mode == "" {
		return nicloudsdk.AgentDisplayModeAlwaysCollapsed
	}
	return mode
}

func sanitizeAgentDisplayMode(raw string) nicloudsdk.AgentDisplayMode {
	mode := nicloudsdk.AgentDisplayMode(raw)
	if slices.Contains(nicloudsdk.ValidAgentDisplayModes, mode) {
		return mode
	}
	return ""
}

func sanitizeAgentChatSendShortcut(raw string) nicloudsdk.AgentChatSendShortcut {
	shortcut := nicloudsdk.AgentChatSendShortcut(raw)
	if slices.Contains(nicloudsdk.ValidAgentChatSendShortcuts, shortcut) {
		return shortcut
	}
	return nicloudsdk.AgentChatSendShortcutEnter
}

func isValidFontName(font nicloudsdk.TerminalFontName) bool {
	return slices.Contains(nicloudsdk.TerminalFontNames, font)
}
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Param request body nicloudsdk.UpdateUserPasswordRequest true "Update password request"
// @Success 204
...
func (api *API) putUserPassword(rw http.ResponseWriter, r *http.Request) {
	var (
		ctx               = r.Context()
		user              = httpmw.UserParam(r)
		params            nicloudsdk.UpdateUserPasswordRequest
		apiKey            = httpmw.APIKey(r)
		auditor           = *api.Auditor.Load()
		aReq, commitAudit = audit.InitRequest[database.User](rw, &audit.RequestParams{
			Audit:   auditor,
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Param request body nicloudsdk.UpdateRoles true "Update roles request"
// @Success 200 {object} nicloudsdk.User
...
func (api *API) putUserRoles(rw http.ResponseWriter, r *http.Request) {
	...
	var params nicloudsdk.UpdateRoles
	if !httpapi.Read(ctx, rw, r, &params) {
		return
	}
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {array} nicloudsdk.Organization
...
func (api *API) CreateUser(ctx context.Context, store database.Store, req CreateUserRequest) (database.User, error) {
	...
	if usernameValid := nicloudsdk.NameValid(req.Username); usernameValid != nil {
	...
	err := store.InTx(func(tx database.Store) error {
		...
		params := database.InsertUserParams{
			...
			Email:            req.Email,
			Username:         req.Username,
			Name:             nicloudsdk.NormalizeRealUsername(req.Name),
...
func findUserAdmins(ctx context.Context, store database.Store) ([]database.GetUsersRow, error) {
	userAdmins, err := store.GetUsers(ctx, database.GetUsersParams{
		RbacRole: []string{nicloudsdk.RoleOwner, nicloudsdk.RoleUserAdmin},
	})
	if err != nil {
...
func (api *API) enrichUserAISeat(ctx context.Context, user *nicloudsdk.User) {
	if !api.Entitlements.Enabled(nicloudsdk.FeatureAIGovernanceUserLimit) {
		return
	}
...
func convertUsers(users []database.User, organizationIDsByUserID map[uuid.UUID][]uuid.UUID, aiSeatSet map[uuid.UUID]struct{}) []nicloudsdk.User {
	converted := make([]nicloudsdk.User, 0, len(users))
	for _, u := range users {
...
func convertAPIKey(k database.APIKey) nicloudsdk.APIKey {
	...
	var legacyScope nicloudsdk.APIKeyScope
	if k.Scopes.Has(database.ApiKeyScopeNIApplicationConnect) {
		legacyScope = nicloudsdk.APIKeyScopeApplicationConnect
	} else if k.Scopes.Has(database.ApiKeyScopeNIAll) {
		legacyScope = nicloudsdk.APIKeyScopeAll
	}

	scopes := make([]nicloudsdk.APIKeyScope, 0, len(k.Scopes))
	for _, s := range k.Scopes {
		scopes = append(scopes, nicloudsdk.APIKeyScope(s))
	}

	return nicloudsdk.APIKey{
		ID:              k.ID,
		UserID:          k.UserID,
		LastUsed:        k.LastUsed,
		...
		LoginType:       nicloudsdk.LoginType(k.LoginType),
		Scope:           legacyScope,
		Scopes:          scopes,
