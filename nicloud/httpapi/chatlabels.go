...
func ValidateChatLabels(labels map[string]string) []nicloudsdk.ValidationError {
	var errs []nicloudsdk.ValidationError

	if len(labels) > maxLabelsPerChat {
		errs = append(errs, nicloudsdk.ValidationError{
			Field:  "labels",
