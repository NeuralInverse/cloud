...
import (
	...
	"cdr.dev/slog/v3"
	"github.com/NeuralInverse/cloud/v2/nicloud/httpapi/httpapiconstraints"
...
func init() {
	...
	displayNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := nicloudsdk.DisplayNameValid(str)
		return valid == nil
	}
	...
	templateVersionNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := nicloudsdk.TemplateVersionNameValid(str)
		return valid == nil
	}
	...
	userRealNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := nicloudsdk.UserRealNameValid(str)
		return valid == nil
	}
	...
	groupNameValidator := func(fl validator.FieldLevel) bool {
		...
		valid := nicloudsdk.GroupNameValid(str)
		return valid == nil
	}
...
var ResourceNotFoundResponse = nicloudsdk.Response{Message: "Resource not found or you do not have access to this resource"}
...
var ResourceForbiddenResponse = nicloudsdk.Response{
	Message: "Forbidden.",
...
func InternalServerError(rw http.ResponseWriter, err error) {
	...
	Write(context.Background(), rw, http.StatusInternalServerError, nicloudsdk.Response{
...
func RouteNotFound(rw http.ResponseWriter) {
	Write(context.Background(), rw, http.StatusNotFound, nicloudsdk.Response{
...
func Read(ctx context.Context, rw http.ResponseWriter, r *http.Request, value interface{}) bool {
	...
	if err != nil {
		Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	if errors.As(err, &validationErrors) {
		apiErrors := make([]nicloudsdk.ValidationError, 0, len(validationErrors))
		for _, validationError := range validationErrors {
			apiErrors = append(apiErrors, nicloudsdk.ValidationError{
	...
	if err != nil {
		Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
...
type EventSender func(rw http.ResponseWriter, r *http.Request) (
	sendEvent func(sse nicloudsdk.ServerSentEvent) error,
	done <-chan struct{},
	err error,
)
...
func ServerSentEventSender(rw http.ResponseWriter, r *http.Request) (
	func(sse nicloudsdk.ServerSentEvent) error,
	<-chan struct{},
	error,
) {
	h := rw.Header()
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				event = sseEvent{
					payload: []byte(fmt.Sprintf("event: %s\n\n", nicloudsdk.ServerSentEventTypePing)),
				}
			}
	...
	sendEvent := func(newEvent nicloudsdk.ServerSentEvent) error {
		buf := &bytes.Buffer{}
...
func OneWayWebSocketEventSender(log slog.Logger, watcher *WSWatcher) func(rw http.ResponseWriter, r *http.Request) (
	func(event nicloudsdk.ServerSentEvent) error,
	<-chan struct{},
	error,
) {
	return func(rw http.ResponseWriter, r *http.Request) (
		...
		eventC := make(chan nicloudsdk.ServerSentEvent, 64)
		...
		sendEvent := func(event nicloudsdk.ServerSentEvent) error {
...
func WriteOAuth2Error(ctx context.Context, rw http.ResponseWriter, status int, errorCode nicloudsdk.OAuth2ErrorCode, description string) {
	// RFC 6749 §5.2: invalid_client SHOULD use 401 and MUST include a
	// WWW-Authenticate response header.
	if status == http.StatusUnauthorized && errorCode == nicloudsdk.OAuth2ErrorCodeInvalidClient {
		rw.Header().Set("WWW-Authenticate", `Basic realm="neuralinverse"`)
	}

	Write(ctx, rw, status, nicloudsdk.OAuth2Error{
		Error:            errorCode,
