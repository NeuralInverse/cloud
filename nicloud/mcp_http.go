...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/mcp"
...
func (api *API) mcpHTTPHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			httpapi.Write(r.Context(), w, http.StatusInternalServerError, nicloudsdk.Response{
				Message: "MCP server initialization failed",
			})
			return
		}
		// Extract the original session token from the request
		authenticatedClient := nicloudsdk.New(api.AccessURL,
			nicloudsdk.WithSessionToken(httpmw.APITokenFromRequest(r)))
		...
		default:
			httpapi.Write(r.Context(), w, http.StatusBadRequest, nicloudsdk.Response{
