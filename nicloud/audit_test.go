...
func TestAuditLogs(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		err := client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			ResourceID:     user.UserID,
			OrganizationID: user.OrganizationID,
		})
		require.NoError(t, err)

		alogs, err := client.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
	...
	t.Run("IncludeUser", func(t *testing.T) {
		...
		client := nicloudtest.New(t, nil)
		user := nicloudtest.CreateFirstUser(t, client)
		client2, user2 := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleOwner())

		err := client2.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			ResourceID:     user2.ID,
			OrganizationID: user.OrganizationID,
		})
		require.NoError(t, err)
	...
	t.Run("WorkspaceBuildAuditLink", func(t *testing.T) {
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			Action:           nicloudsdk.AuditActionStop,
			ResourceType:     nicloudsdk.ResourceTypeWorkspaceBuild,
		...
		auditLogs, err := client.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
			Pagination: nicloudsdk.Pagination{
				Limit: 1,
			},
		})
	...
	t.Run("Organization", func(t *testing.T) {
		...
		alogs, err := orgAdmin.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
			Pagination: nicloudsdk.Pagination{
				Limit: 5,
			},
		})
	...
	t.Run("Organization404", func(t *testing.T) {
		...
		_, err := orgAdmin.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
...
func TestAuditLogsFilter(t *testing.T) {
	...
	t.Run("Filter", func(t *testing.T) {
		...
		var (
			...
			client   = nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true})
			user     = nicloudtest.CreateFirstUser(t, client)
			version  = nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, completeWithAgentAndApp())
			template = nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		)

		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		workspace := nicloudtest.CreateWorkspace(t, client, template.ID)
		workspace.LatestBuild = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)

		// Create two logs with "Create"
		err := client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionCreate,
			ResourceType:   nicloudsdk.ResourceTypeTemplate,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionCreate,
			ResourceType:   nicloudsdk.ResourceTypeUser,
			ResourceID:     user.UserID,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionDelete,
			ResourceType:   nicloudsdk.ResourceTypeUser,
			ResourceID:     user.UserID,
			Time:           time.Date(2022, 8, 15, 14, 30, 45, 100, time.UTC), // 2022-8-15 14:30:45
		})
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionStart,
			ResourceType:   nicloudsdk.ResourceTypeWorkspaceBuild,
			ResourceID:     workspace.LatestBuild.ID,
			Time:           time.Date(2022, 8, 15, 14, 30, 45, 100, time.UTC), // 2022-8-15 14:30:45
		})
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionConnect,
			RequestID:      connectRequestID,
			ResourceType:   nicloudsdk.ResourceTypeWorkspaceAgent,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionDisconnect,
			RequestID:      connectRequestID,
			ResourceType:   nicloudsdk.ResourceTypeWorkspaceAgent,
			ResourceID:     workspace.LatestBuild.Resources[0].Agents[0].ID,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionOpen,
			RequestID:      openRequestID,
			ResourceType:   nicloudsdk.ResourceTypeWorkspaceApp,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			OrganizationID: user.OrganizationID,
			Action:         nicloudsdk.AuditActionClose,
			RequestID:      openRequestID,
			ResourceType:   nicloudsdk.ResourceTypeWorkspaceApp,
			ResourceID:     workspace.LatestBuild.Resources[0].Agents[0].Apps[0].ID,
			Time:           time.Date(2022, 8, 15, 14, 35, 0o0, 100, time.UTC), // 2022-8-15 14:35:00
		...
		testCases := []struct {
			...
			{
				Name:           "FilterByEmail",
				SearchQuery:    "email:" + nicloudtest.FirstUserParams.Email,
				ExpectedResult: 9,
			},
			{
				Name:           "FilterByUsername",
				SearchQuery:    "username:" + nicloudtest.FirstUserParams.Username,
				ExpectedResult: 9,
			},
			{
...
func TestDeprecatedConnEvents(t *testing.T) {
	...
	connLog, err := client.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
	...
	dcLog, err := client.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
	...
	openLog, err := client.AuditLogs(ctx, nicloudsdk.AuditLogsRequest{
