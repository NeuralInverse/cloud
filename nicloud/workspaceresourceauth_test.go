...
func TestPostWorkspaceAuthAzureInstanceIdentity(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		certificates, metadataClient := nicloudtest.NewAzureInstanceIdentity(t, instanceID)
		client, _ := setupInstanceIDWorkspace(t, &nicloudtest.Options{
	...
	t.Run("Ambiguous/AzureWithSelector", func(t *testing.T) {
		...
		instanceID := newTestInstanceID(t)
		certificates, metadataClient := nicloudtest.NewAzureInstanceIdentity(t, instanceID)
		client, store := setupInstanceIDWorkspace(t, &nicloudtest.Options{
			AzureCertificates: certificates,
...
func TestPostWorkspaceAuthAWSInstanceIdentity(t *testing.T) {
	...
	t.Run("Ambiguous/SingleAgent", func(t *testing.T) {
		...
		instanceID := newTestInstanceID(t)
		certificates, metadataClient := nicloudtest.NewAWSInstanceIdentity(t, instanceID)
		client, _ := setupInstanceIDWorkspace(t, &nicloudtest.Options{
	...
	t.Run("RecycledInstanceID", func(t *testing.T) {
		...
		instanceID := newTestInstanceID(t)
		certificates, metadataClient := nicloudtest.NewAWSInstanceIdentity(t, instanceID)
		setup := setupInstanceIDWorkspaceWithResources(t, &nicloudtest.Options{
			AWSCertificates: certificates,
		}, workspaceAgentsForInstanceID(instanceID, "dev"))

		successorVersion := nicloudtest.CreateTemplateVersion(t, setup.client, setup.user.OrganizationID, &echo.Responses{
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, setup.client, successorVersion.ID)
		build := nicloudtest.CreateWorkspaceBuild(t, setup.client, setup.workspace, database.WorkspaceTransitionStart, func(req *nicloudsdk.CreateWorkspaceBuildRequest) {
			req.TemplateVersionID = successorVersion.ID
		})
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, setup.client, build.ID)
	...
	t.Run("Ambiguous/MultipleAgentsNoSelector", func(t *testing.T) {
		...
		require.Contains(t, apiErr.Message, "NEURALINVERSE_AGENT_NAME")
...
func TestPostWorkspaceAuthGoogleInstanceIdentity(t *testing.T) {
	...
	t.Run("Expired", func(t *testing.T) {
		...
		instanceID := newTestInstanceID(t)
		validator, metadata := nicloudtest.NewGoogleInstanceIdentity(t, instanceID, true)
	...
	t.Run("InstanceNotFound", func(t *testing.T) {
		...
		instanceID := newTestInstanceID(t)
		validator, metadata := nicloudtest.NewGoogleInstanceIdentity(t, instanceID, false)
		client := nicloudtest.New(t, &nicloudtest.Options{
			GoogleTokenValidator: validator,
		})
...
func setupInstanceIDWorkspace(t *testing.T, opts *nicloudtest.Options, agents []*proto.Agent) (*nicloudsdk.Client, database.Store) {
	t.Helper()
...
func setupInstanceIDWorkspaceWithResources(
	...
	actualOpts := &nicloudtest.Options{}
	if opts != nil {
		*actualOpts = *opts
	}
	actualOpts.IncludeProvisionerDaemon = true

	client, store := nicloudtest.NewWithDatabase(t, actualOpts)
