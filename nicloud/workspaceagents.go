...
// @Success 200 {object} nicloudsdk.WorkspaceAgent
...
func (api *API) patchWorkspaceAgentLogs(rw http.ResponseWriter, r *http.Request) {
	...
	for _, logEntry := range req.Logs {
		...
		if logEntry.Level == "" {
			...
			logEntry.Level = nicloudsdk.LogLevelInfo
		}
...
// @Success 200 {array} nicloudsdk.WorkspaceAgentLog
...
func (api *API) workspaceAgentLogs(rw http.ResponseWriter, r *http.Request) {
	...
	encoder := wsjson.NewEncoder[[]nicloudsdk.WorkspaceAgentLog](conn, websocket.MessageText)
...
// @Tags Agents
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Success 200 {object} nicloudsdk.WorkspaceAgentListeningPortsResponse
...
func (api *API) workspaceAgentListeningPorts(rw http.ResponseWriter, r *http.Request) {
	...
	if apiAgent.Status != nicloudsdk.WorkspaceAgentConnected {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
			Message: fmt.Sprintf("Agent state is %q, it must be in the %q state.", apiAgent.Status, nicloudsdk.WorkspaceAgentConnected),
		})
		return
	}
	...
	filteredPorts := make([]nicloudsdk.WorkspaceAgentListeningPort, 0, len(portsResponse.Ports))
...
// @Tags Agents
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Success 200 {object} nicloudsdk.WorkspaceAgentListContainersResponse
...
func (api *API) workspaceAgentListContainers(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			httpapi.Write(ctx, rw, http.StatusRequestTimeout, nicloudsdk.Response{
				Message: "Failed to fetch containers from agent.",
				Detail:  "Request timed out.",
			})
			return
		}
		// If the agent returns a nicloudsdk.Error, we can return that directly.
	...
	cts.Containers = slices.DeleteFunc(cts.Containers, func(ct nicloudsdk.WorkspaceAgentContainer) bool {
...
// @Param workspaceagent path string true "Workspace agent ID" format(uuid)
// @Param devcontainer path string true "Devcontainer ID"
// @Success 202 {object} nicloudsdk.Response
...
func (api *API) workspaceAgentRecreateDevcontainer(rw http.ResponseWriter, r *http.Request) {
	...
	agentConn, release, err := api.agentProvider.AgentConn(dialCtx, waws.WorkspaceAgent.ID)
	if err != nil {
		httpapi.Write(dialCtx, rw, http.StatusInternalServerError, nicloudsdk.Response{
			Message: "Internal error dialing workspace agent.",
			Detail:  err.Error(),
		})
		return
...
func (api *API) workspaceAgentClientCoordinate(rw http.ResponseWriter, r *http.Request) {
	...
	// Namely, disabling the route using `NEURALINVERSE_BROWSER_ONLY`.
...
func (api *API) handleResumeToken(ctx context.Context, rw http.ResponseWriter, r *http.Request) (peerID uuid.UUID, err error) {
	...
	if resumeToken != "" {
		...
		case xerrors.Is(err, jwtutils.ErrMissingKeyID):
			...
			err = nil
		case err != nil:
			httpapi.Write(ctx, rw, http.StatusUnauthorized, nicloudsdk.Response{
...
// @Success 200 {object} nicloudsdk.WorkspaceAgentLogSource
...
func (api *API) workspaceAgentPostLogSource(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if database.IsUniqueViolation(err, "workspace_agent_log_sources_pkey") {
			httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.WorkspaceAgentLogSource{
				WorkspaceAgentID: workspaceAgent.ID,
				CreatedAt:        dbtime.Now(),
				ID:               req.ID,
...
// @Failure 409 {object} nicloudsdk.Response
...
func convertProvisionedApps(dbApps []database.WorkspaceApp) []nicloudsdk.WorkspaceApp {
	return db2sdk.Apps(dbApps, []database.WorkspaceAppStatus{}, database.WorkspaceAgent{}, "", database.WorkspaceTable{})
}

func convertLogSources(dbLogSources []database.WorkspaceAgentLogSource) []nicloudsdk.WorkspaceAgentLogSource {
	logSources := make([]nicloudsdk.WorkspaceAgentLogSource, 0)
	for _, dbLogSource := range dbLogSources {
		logSources = append(logSources, nicloudsdk.WorkspaceAgentLogSource{
			ID:               dbLogSource.ID,
...
func convertScripts(dbScripts []database.GetWorkspaceAgentScriptsByAgentIDsRow) []nicloudsdk.WorkspaceAgentScript {
	scripts := make([]nicloudsdk.WorkspaceAgentScript, 0)
...
func convertWorkspaceAgentMetadata(db []database.WorkspaceAgentMetadatum) []nicloudsdk.WorkspaceAgentMetadata {
	...
	result := make([]nicloudsdk.WorkspaceAgentMetadata, len(db))
	for i, datum := range db {
		result[i] = nicloudsdk.WorkspaceAgentMetadata{
			...
			Description: nicloudsdk.WorkspaceAgentMetadataDescription{
...
func (api *API) workspaceAgentsExternalAuth(rw http.ResponseWriter, r *http.Request) {
	...
	// MarkStale will trigger a refresh by nicloud/gitsync. This allows us to
	...
	if err != nil {
		handleRetrying(http.StatusInternalServerError, nicloudsdk.Response{
...
func (api *API) workspaceAgentsExternalAuthListen(ctx context.Context, rw http.ResponseWriter, previous *database.ExternalAuthLink, externalAuthConfig *externalauth.Config, workspace database.Workspace, gitRef chatGitRef) {
	...
	for {
		...
		// MarkStale will trigger a refresh by nicloud/gitsync.
		//nolint:gocritic // Chat processor context required for cross-user chat lookup
		api.gitSyncWorker.MarkStale(dbauthz.AsChatd(ctx), gitsync.MarkStaleParams{
			WorkspaceID: workspace.ID,
			Branch:      gitRef.Branch,
...
// fillCoderDesktopTelemetry fills out the provided event based on a Neural Inverse Cloud Desktop telemetry header on the request, if
// present.
func fillCoderDesktopTelemetry(r *http.Request, event *telemetry.UserTailnetConnection, logger slog.Logger) {
	...
	desktopTelemetryHeader := r.Header.Get(nicloudsdk.CoderDesktopTelemetryHeader)
	if desktopTelemetryHeader != "" {
		var telemetryData nicloudsdk.CoderDesktopTelemetry
		if err := telemetryData.FromHeader(desktopTelemetryHeader); err == nil {
			...
			if telemetryData.NIDesktopVersion != "" {
				event.NIDesktopVersion = &telemetryData.NIDesktopVersion
			}
			logger.Debug(r.Context(), "received desktop telemetry",
				...
				slog.F("desktop_version", telemetryData.NIDesktopVersion))
		} else {
...
func createExternalAuthResponse(typ, token string, extra pqtype.NullRawMessage) (agentsdk.ExternalAuthResponse, error) {
	...
	switch typ {
	case string(nicloudsdk.EnhancedExternalAuthProviderGitLab):
		// https://stackoverflow.com/questions/25409700/using-gitlab-token-to-clone-without-authentication
		resp = agentsdk.ExternalAuthResponse{
			Username: "oauth2",
			Password: token,
		}
	case string(nicloudsdk.EnhancedExternalAuthProviderBitBucketCloud), string(nicloudsdk.EnhancedExternalAuthProviderBitBucketServer):
...
func convertWorkspaceAgentLogs(logs []database.WorkspaceAgentLog) []nicloudsdk.WorkspaceAgentLog {
	sdk := make([]nicloudsdk.WorkspaceAgentLog, 0, len(logs))
...
func (api *API) workspaceAgentAddChatContext(rw http.ResponseWriter, r *http.Request) {
	...
	for i := range req.Parts {
		...
		if req.Parts[i].Type != nicloudsdk.ChatMessagePartTypeContextFile {
			continue
		}
...
func clearAgentChatContext(
	...
	return db.InTx(func(tx database.Store) error {
		...
		for _, msg := range messages {
			...
			hasContextFile := messageHasPartTypes(msg.Content.RawMessage, nicloudsdk.ChatMessagePartTypeContextFile)
			hasSkill := messageHasPartTypes(msg.Content.RawMessage, nicloudsdk.ChatMessagePartTypeSkill)
...
func prependAgentChatContextSentinelIfNeeded(
	...
	return append([]nicloudsdk.ChatMessagePart{{
...
func messageHasPartTypes(raw []byte, types ...nicloudsdk.ChatMessagePartType) bool {
