...
func AuditLogs(ctx context.Context, db database.Store, query string) (database.GetAuditLogsOffsetParams,
	database.CountAuditLogsParams, []nicloudsdk.ValidationError,
) {
...
func ConnectionLogs(ctx context.Context, db database.Store, query string, apiKey database.APIKey) (database.GetConnectionLogsOffsetParams, database.CountConnectionLogsParams, []nicloudsdk.ValidationError) {
	// Always lowercase for all searches.
	query = strings.ToLower(query)
	values, errors := searchTerms(query, func(term string, values url.Values) error {
	...
	filter := database.GetConnectionLogsOffsetParams{
		...
		Status:              string(httpapi.ParseCustom(parser, values, "", "status", httpapi.ParseEnum[nicloudsdk.ConnectionLogStatus])),
	}
...
func Users(query string) (database.GetUsersParams, []nicloudsdk.ValidationError) {
	// Always lowercase for all searches.
	query = strings.ToLower(query)
	values, errors := searchTerms(query, func(term string, values url.Values) error {
		values.Add("search", term)
...
func Members(query string, organizationID uuid.UUID) (database.OrganizationMembersParams, []nicloudsdk.ValidationError) {
...
func Workspaces(ctx context.Context, db database.Store, query string, page nicloudsdk.Pagination, agentInactiveDisconnectTimeout time.Duration) (database.GetWorkspacesParams, []nicloudsdk.ValidationError) {
...
func Templates(ctx context.Context, db database.Store, actorID uuid.UUID, query string) (database.GetTemplatesWithFilterParams, []nicloudsdk.ValidationError) {
	// Always lowercase for all searches.
	query = strings.ToLower(query)
	values, errors := searchTerms(query, func(term string, values url.Values) error {
	...
	if filter.AuthorUsername == nicloudsdk.Me {
...
func AIBridgeInterceptions(ctx context.Context, db database.Store, query string, page nicloudsdk.Pagination, actorID uuid.UUID) (database.ListAIBridgeInterceptionsParams, []nicloudsdk.ValidationError) {
	...
	if !filter.StartedBefore.IsZero() && !filter.StartedAfter.IsZero() && !filter.StartedBefore.After(filter.StartedAfter) {
		parser.Errors = append(parser.Errors, nicloudsdk.ValidationError{
...
func AIBridgeSessions(ctx context.Context, db database.Store, query string, page nicloudsdk.Pagination, actorID uuid.UUID, afterSessionID string) (database.ListAIBridgeSessionsParams, []nicloudsdk.ValidationError) {
	// nolint:exhaustruct // Empty values just means "don't filter by that field".
...
func AIBridgeModels(query string, page nicloudsdk.Pagination) (database.ListAIBridgeModelsParams, []nicloudsdk.ValidationError) {
	// nolint:exhaustruct // Empty values just means "don't filter by that field".
...
func AIBridgeClients(query string, page nicloudsdk.Pagination) (database.ListAIBridgeClientsParams, []nicloudsdk.ValidationError) {
	// nolint:exhaustruct // Empty values just means "don't filter by that field".
...
func Tasks(ctx context.Context, db database.Store, query string, actorID uuid.UUID) (database.ListTasksParams, []nicloudsdk.ValidationError) {
...
func Chats(query string) (database.GetChatsParams, []nicloudsdk.ValidationError) {
...
func searchTerms(query string, defaultKey func(term string, values url.Values) error) (url.Values, []nicloudsdk.ValidationError) {
	...
	for _, element := range elements {
		if strings.HasPrefix(element, ":") || strings.HasSuffix(element, ":") {
			return nil, []nicloudsdk.ValidationError{
				{
					Field:  "q",
...
func parseUser(ctx context.Context, db database.Store, parser *httpapi.QueryParamParser, vals url.Values, queryParam string, actorID uuid.UUID) uuid.UUID {
	return httpapi.ParseCustom(parser, vals, uuid.Nil, queryParam, func(v string) (uuid.UUID, error) {
		...
		if v == nicloudsdk.Me && actorID != uuid.Nil {
			return actorID, nil
		}
...
func parseGroup(ctx context.Context, db database.Store, parser *httpapi.QueryParamParser, vals url.Values, queryParam string) uuid.UUID {
	return httpapi.ParseCustom(parser, vals, uuid.Nil, queryParam, func(v string) (uuid.UUID, error) {
		...
		case 2:
			orgName := parts[0]
			if err := nicloudsdk.NameValid(orgName); err != nil {
		...
		if err := nicloudsdk.GroupNameValid(groupName); err != nil {
