...
func TestSearchWorkspace(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			values, errs := searchquery.Workspaces(context.Background(), db, c.Query, nicloudsdk.Pagination{}, 0)
	...
	t.Run("AgentInactiveDisconnectTimeout", func(t *testing.T) {
		...
		db, _ := dbtestutil.NewDB(t)
		values, errs := searchquery.Workspaces(context.Background(), db, query, nicloudsdk.Pagination{}, timeout)
	...
	testCases := []struct {
		...
		},
		{
			...
			Expected: database.GetUsersParams{
				Search:    "user-name",
				Status:    []database.UserStatus{database.UserStatusActive},
				RbacRole:  []string{nicloudsdk.RoleOwner},
				LoginType: []database.LoginType{},
			},
		},
		{
			...
			Expected: database.GetUsersParams{
				...
				RbacRole:  []string{nicloudsdk.RoleMember},
				LoginType: []database.LoginType{},
			},
		},
		{
