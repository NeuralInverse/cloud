...
func TestTelemetry(t *testing.T) {
	...
	t.Run("HashedEmail", func(t *testing.T) {
		...
		_ = dbgen.User(t, db, database.User{
			Email: "kyle@cloud.neuralinverse.com",
		})
		_, snapshot := collectSnapshot(ctx, t, db, nil)
		require.Len(t, snapshot.Users, 1)
		require.Equal(t, snapshot.Users[0].EmailHashed, "bb44bf07cf9a2db0554bba63a03d822c927deae77df101874496df5a6a3e896d@cloud.neuralinverse.com")
	})
	t.Run("HashedModule", func(t *testing.T) {
		...
		_ = dbgen.WorkspaceModule(t, db, database.WorkspaceModule{
			JobID:   pj.ID,
			Source:  "registry.cloud.neuralinverse.com/terraform/aws",
			Version: "1.0.0",
		})
		_ = dbgen.WorkspaceModule(t, db, database.WorkspaceModule{
			JobID:   pj.ID,
		...
		require.Equal(t, modules[1].Source, "registry.cloud.neuralinverse.com/terraform/aws")
	...
	t.Run("ModuleSourceType", func(t *testing.T) {
		...
		cases := []struct {
			...
			// Coder registry
			{source: "registry.cloud.neuralinverse.com/terraform/aws", want: telemetry.ModuleSourceTypeCoderRegistry},
			{source: "registry.cloud.neuralinverse.com/modules/base", want: telemetry.ModuleSourceTypeCoderRegistry},
	...
	t.Run("IDPOrgSync", func(t *testing.T) {
		...
		deployment, _ = collectSnapshot(ctx, t, db, func(opts telemetry.Options) telemetry.Options {
			opts.DeploymentConfig = &nicloudsdk.DeploymentValues{
				OIDC: nicloudsdk.OIDCConfig{
...
func TestTelemetryInstallSource(t *testing.T) {
	t.Setenv("NEURALINVERSE_TELEMETRY_INSTALL_SOURCE", "aws_marketplace")
	ctx := testutil.Context(t, testutil.WaitMedium)
	db, _ := dbtestutil.NewDB(t)
	deployment, _ := collectSnapshot(ctx, t, db, nil)
