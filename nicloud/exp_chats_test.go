...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/aibridge"
	...
	nicloudpubsub "github.com/NeuralInverse/cloud/v2/nicloud/pubsub"
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chatprovider"
	"github.com/NeuralInverse/cloud/v2/nicloud/x/chatd/chattest"
...
func chatDeploymentValues(t testing.TB) *nicloudsdk.DeploymentValues {
	t.Helper()

	values := nicloudtest.DeploymentValues(t)
	return values
}

// newChatTestOptions builds nicloudtest options for chat runtime tests. Unless
// a test sets ChatProviderAPIKeys explicitly, it installs a fake
// OpenAI-compatible provider before nicloud starts so background chat work stays
// local, and the fake server outlives chatd during cleanup.
func newChatTestOptions(
	t testing.TB,
	values *nicloudsdk.DeploymentValues,
	overrides ...func(*nicloudtest.Options),
) *nicloudtest.Options {
	t.Helper()

	opts := &nicloudtest.Options{
	...
	if opts.ChatProviderAPIKeys == nil {
		providerKeys := nicloudtest.FakeOpenAICompatProviderAPIKeys(t)
		opts.ChatProviderAPIKeys = &providerKeys
	}
	return opts
}

func newChatClient(t testing.TB, overrides ...func(*nicloudtest.Options)) *nicloudsdk.ExperimentalClient {
	t.Helper()

	opts := newChatTestOptions(t, chatDeploymentValues(t), overrides...)
	client := nicloudtest.New(t, opts)
	return nicloudsdk.NewExperimentalClient(client)
}

func newChatClientWithAPI(t testing.TB, overrides ...func(*nicloudtest.Options)) (*nicloudsdk.ExperimentalClient, *nicloud.API) {
	t.Helper()

	opts := newChatTestOptions(t, chatDeploymentValues(t), overrides...)
	client, _, api := nicloudtest.NewWithAPI(t, opts)
	return nicloudsdk.NewExperimentalClient(client), api
}

func newChatClientWithDeploymentValues(
	t testing.TB,
	values *nicloudsdk.DeploymentValues,
) *nicloudsdk.ExperimentalClient {
	t.Helper()
...
func newChatClientWithDatabase(t testing.TB, overrides ...func(*nicloudtest.Options)) (*nicloudsdk.ExperimentalClient, database.Store) {
	t.Helper()

	opts := newChatTestOptions(t, chatDeploymentValues(t), overrides...)
	client, db := nicloudtest.NewWithDatabase(t, opts)
	return nicloudsdk.NewExperimentalClient(client), db
}

func newChatClientWithAPIAndDatabase(t testing.TB, overrides ...func(*nicloudtest.Options)) (*nicloudsdk.ExperimentalClient, database.Store, *nicloud.API) {
	t.Helper()

	opts := newChatTestOptions(t, chatDeploymentValues(t), overrides...)
	client, _, api := nicloudtest.NewWithAPI(t, opts)
	return nicloudsdk.NewExperimentalClient(client), api.Database, api
}
...
) *nicloudsdk.ChatUsageLimitExceededResponse {
	t.Helper()
...
func enableDailyChatUsageLimit(
	...
	_, err := db.UpsertChatUsageLimitConfig(
		...
		database.UpsertChatUsageLimitConfigParams{
			...
			Period:             string(nicloudsdk.ChatUsageLimitPeriodDay),
		},
	)
	require.NoError(t, err)

	_, periodEnd := chatd.ComputeUsagePeriodBounds(time.Now(), nicloudsdk.ChatUsageLimitPeriodDay)
	return periodEnd
}
...
func insertAssistantCostMessage(
	...
	assistantContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
		nicloudsdk.ChatMessageText("assistant"),
	})
	require.NoError(t, err)
...
func TestPostChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		memberClientRaw, member := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)

		chat, err := memberClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID, Content: []nicloudsdk.ChatInputPart{
				{
		...
		for _, message := range messagesResult.Messages {
			if message.Role != nicloudsdk.ChatMessageRoleUser {
				continue
			}
			for _, part := range message.Content {
				if part.Type == nicloudsdk.ChatMessagePartTypeText &&
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		memberClientRaw, _ := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)

		_, err := memberClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
	...
	t.Run("HidesSystemPromptMessages", func(t *testing.T) {
		...
		require.NoError(t, err)
		for _, message := range messagesResult.Messages {
			require.NotEqual(t, nicloudsdk.ChatMessageRoleSystem, message.Role)
		}
	})
	...
	t.Run("WithPerChatSystemPrompt", func(t *testing.T) {
		...
		user := nicloudtest.CreateFirstUser(t, client.Client)
		_ = createChatModelConfig(t, client)
	...
	t.Run("PerChatSystemPromptTooLong", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID, Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: "hello",
				},
	...
	t.Run("WorkspaceNotAccessible", func(t *testing.T) {
		...
		firstUser := nicloudtest.CreateFirstUser(t, adminClient.Client)
		memberClientRaw, _ := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
	...
	t.Run("WorkspaceAccessibleButNoSSH", func(t *testing.T) {
		...
		adminClient, db := newChatClientWithDatabase(t)
		firstUser := nicloudtest.CreateFirstUser(t, adminClient.Client)
		orgAdminClientRaw, _ := nicloudtest.CreateAnotherUser(
			t,
			adminClient.Client,
		...
		orgAdminClient := nicloudsdk.NewExperimentalClient(orgAdminClientRaw)
		...
		_, err := orgAdminClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("UnsupportedPartType", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			...
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartType("image"),
					Text: "hello",
				},
			},
		})
	...
	t.Run("NilOrganizationID", func(t *testing.T) {
		...
		memberClientRaw, _ := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
...
func TestPostChats_ClientType(t *testing.T) {
	...
	newChat := func(t *testing.T, clientType nicloudsdk.ChatClientType) nicloudsdk.Chat {
		t.Helper()
		ctx := testutil.Context(t, testutil.WaitLong)
		chat, err := memberClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
	...
	t.Run("DefaultIsAPI", func(t *testing.T) {
		...
		chat := newChat(t, "")
		require.Equal(t, nicloudsdk.ChatClientTypeAPI, chat.ClientType)

		got, err := memberClient.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ChatClientTypeAPI, got.ClientType)
	})
	...
	t.Run("ExplicitAPI", func(t *testing.T) {
		...
		chat := newChat(t, nicloudsdk.ChatClientTypeAPI)
		require.Equal(t, nicloudsdk.ChatClientTypeAPI, chat.ClientType)
	...
	t.Run("ExplicitUI", func(t *testing.T) {
		...
		chat := newChat(t, nicloudsdk.ChatClientTypeUI)
		require.Equal(t, nicloudsdk.ChatClientTypeUI, chat.ClientType)

		got, err := memberClient.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ChatClientTypeUI, got.ClientType)
	})
...
func TestListChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		firstChatA, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		firstChatB, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		chatsByID := make(map[uuid.UUID]nicloudsdk.Chat, len(chats))
		for i, chat := range chats {
			...
			require.Contains(t, []nicloudsdk.ChatStatus{
				nicloudsdk.ChatStatusPending,
				nicloudsdk.ChatStatusRunning,
				nicloudsdk.ChatStatusError,
				nicloudsdk.ChatStatusWaiting,
				nicloudsdk.ChatStatusCompleted,
	...
	t.Run("OrgMemberWithoutAgentsAccessCannotAccessOwnChats", func(t *testing.T) {
		...
		memberClientRaw, member := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
		...
		err = memberClient.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = nicloudtest.CreateFirstUser(t, client.Client)

		unauthenticatedClient := nicloudsdk.NewExperimentalClient(nicloudsdk.New(client.URL))
	...
	t.Run("Pagination", func(t *testing.T) {
		...
		page1, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
		...
		page2, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
		...
		page3, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
			Pagination: nicloudsdk.Pagination{
		...
		offsetPage, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
			Pagination: nicloudsdk.Pagination{Offset: 3, Limit: 2},
		})
		require.NoError(t, err)
	...
	t.Run("PinnedOnFirstPage", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, pinnedDBChat.ID, nicloudsdk.UpdateChatRequest{
		...
		page1, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
			Pagination: nicloudsdk.Pagination{Limit: 50},
		})
		require.NoError(t, err)
	...
	t.Run("CursorWithPins", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, createdChatIDs[0], nicloudsdk.UpdateChatRequest{
			PinOrder: ptr.Ref(int32(1)),
		})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, createdChatIDs[1], nicloudsdk.UpdateChatRequest{
			PinOrder: ptr.Ref(int32(1)),
		})
		require.NoError(t, err)
		...
		var allPaginated []nicloudsdk.Chat
		var afterID uuid.UUID
		for range maxPages {
			opts := &nicloudsdk.ListChatsOptions{
				Pagination: nicloudsdk.Pagination{Limit: pageSize},
			}
			if afterID != uuid.Nil {
	...
	t.Run("ChildChatsEmbeddedNotStandalone", func(t *testing.T) {
		...
		parentChat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		standalone, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: user.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("PaginationCountsOnlyRootChats", func(t *testing.T) {
		...
		for i := range 3 {
			parent, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
				OrganizationID: user.OrganizationID,
				Content: []nicloudsdk.ChatInputPart{
					{
						Type: nicloudsdk.ChatInputPartTypeText,
		...
		chats, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
			Pagination: nicloudsdk.Pagination{Limit: 2},
		})
		require.NoError(t, err)
	...
	t.Run("DiffURLFilter", func(t *testing.T) {
		...
		require.NoError(t, client.UpdateChat(ctx, archivedWithPR.ID, nicloudsdk.UpdateChatRequest{
		...
		t.Run("InvalidURL", func(t *testing.T) {
			_, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
...
func TestListChatModels(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var openAIProvider *nicloudsdk.ChatModelProvider
	...
	t.Run("UserOnlyProviderRequiresUserKey", func(t *testing.T) {
		...
		_, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
		...
		var anthropicProvider *nicloudsdk.ChatModelProvider
		for i := range models.Providers {
		...
		require.Equal(t, nicloudsdk.ChatModelProviderUnavailableReasonUserAPIKeyRequired, anthropicProvider.UnavailableReason)

		_, err = client.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{
			APIKey: "user-api-key",
		})
		require.NoError(t, err)
	...
	t.Run("CentralAndUserWithFallback", func(t *testing.T) {
		...
		var googleProvider *nicloudsdk.ChatModelProvider
		for i := range models.Providers {
	...
	t.Run("DisabledProvidersAndModelsAreFilteredOut", func(t *testing.T) {
		...
		enabled := false
		_, err = client.UpdateAIProvider(ctx, provider.ID.String(), nicloudsdk.UpdateAIProviderRequest{
			Enabled: &enabled,
		})
		require.NoError(t, err)
...
func TestWatchChats(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		createdChat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		for {
			...
			if payload.Kind == nicloudsdk.ChatWatchEventKindCreated &&
	...
	t.Run("CreatedEventIncludesAllChatFields", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ChatStatusPending, got.Status)
	...
	t.Run("DiffStatusChangeIncludesDiffStatus", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		rawClient, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
			DeploymentValues: chatDeploymentValues(t),
		})
		client := nicloudsdk.NewExperimentalClient(rawClient)
		db := api.Database
		user := nicloudtest.CreateFirstUser(t, client.Client)
		modelConfig := createChatModelConfig(t, client)
		...
		event := nicloudsdk.ChatWatchEvent{
			Kind: nicloudsdk.ChatWatchEventKindDiffStatusChange,
			Chat: nicloudsdk.Chat{
				ID:         chat.ID,
				OwnerID:    chat.OwnerID,
				Title:      chat.Title,
				Status:     nicloudsdk.ChatStatus(chat.Status),
		...
		err = api.Pubsub.Publish(nicloudpubsub.ChatWatchEventChannel(user.UserID), payload)
		require.NoError(t, err)

		var received nicloudsdk.ChatWatchEvent
		for {
			err = wsjson.Read(ctx, conn, &received)
			require.NoError(t, err)

			if received.Kind == nicloudsdk.ChatWatchEventKindDiffStatusChange &&
	...
	t.Run("ArchiveAndUnarchiveEmitEventsForDescendants", func(t *testing.T) {
		...
		collectLifecycleEvents := func(expectedKind nicloudsdk.ChatWatchEventKind) map[uuid.UUID]nicloudsdk.ChatWatchEvent {
			t.Helper()

			events := make(map[uuid.UUID]nicloudsdk.ChatWatchEvent, 3)
			for len(events) < 3 {
				var payload nicloudsdk.ChatWatchEvent
				err = wsjson.Read(ctx, conn, &payload)
				require.NoError(t, err)
		...
		assertLifecycleEvents := func(events map[uuid.UUID]nicloudsdk.ChatWatchEvent, archived bool) {
			t.Helper()
		...
		err = client.UpdateChat(ctx, parentChat.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		deletedEvents := collectLifecycleEvents(nicloudsdk.ChatWatchEventKindDeleted)
		assertLifecycleEvents(deletedEvents, true)

		err = client.UpdateChat(ctx, parentChat.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		require.NoError(t, err)
		createdEvents := collectLifecycleEvents(nicloudsdk.ChatWatchEventKindCreated)
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		unauthenticatedClient := nicloudsdk.New(client.URL)
...
func TestUserAIProviderKeys(t *testing.T) {
	...
	createOpenAIProvider := func(t *testing.T, client *nicloudsdk.ExperimentalClient, name string, enabled bool, apiKeys ...string) nicloudsdk.AIProvider {
		t.Helper()

		provider, err := client.CreateAIProvider(testutil.Context(t, testutil.WaitLong), nicloudsdk.CreateAIProviderRequest{
	...
	findUserAIProviderKeyConfig := func(
		t *testing.T,
		configs []nicloudsdk.UserAIProviderKeyConfig,
		providerID uuid.UUID,
	) *nicloudsdk.UserAIProviderKeyConfig {
		t.Helper()
	...
	t.Run("SelfServiceLifecycle", func(t *testing.T) {
		...
		firstUser := nicloudtest.CreateFirstUser(t, adminClient.Client)
		memberClientRaw, _ := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
		...
		cfgValue, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		require.NoError(t, err)
		...
		cfgValue, err = memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: "replacement-user-api-key"})
		require.NoError(t, err)
		require.Equal(t, provider.ID, cfgValue.Provider.ID)
		require.True(t, cfgValue.HasUserAPIKey)
	...
	t.Run("ListsDisabledProviderWithSavedUserKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		require.NoError(t, err)

		enabled := false
		_, err = adminClient.UpdateAIProvider(ctx, provider.ID.String(), nicloudsdk.UpdateAIProviderRequest{Enabled: &enabled})
		require.NoError(t, err)
	...
	t.Run("RejectsLargeAPIKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: strings.Repeat("x", 10241)})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("RejectsWhitespaceAPIKey", func(t *testing.T) {
		...
		_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: "  "})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("BYOKDisabledRejectsUpsertAndAllowsDelete", func(t *testing.T) {
		...
		_, err := client.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{APIKey: "test-user-api-key"})
		sdkErr := requireSDKError(t, err, http.StatusForbidden)
...
func TestListChatProviders(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		var openAIProvider *nicloudsdk.ChatProviderConfig
		...
		require.NotNil(t, openAIProvider)
		require.Equal(t, nicloudsdk.ChatProviderConfigSourceDatabase, openAIProvider.Source)
	...
	t.Run("IgnoresDeploymentKeyWhenCentralKeyDisabled", func(t *testing.T) {
		...
		provider, err := client.CreateChatProvider(ctx, nicloudsdk.CreateChatProviderConfigRequest{
			Provider:             "openai",
...
func TestCreateChatProvider(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ChatProviderConfigSourceDatabase, provider.Source)
	})
	...
	t.Run("RejectsMissingCentralAPIKeyForNonBedrock", func(t *testing.T) {
		...
		_, err := client.CreateChatProvider(ctx, nicloudsdk.CreateChatProviderConfigRequest{
			Provider:             "openai",
			DisplayName:          "OpenAI",
			CentralAPIKeyEnabled: ptr.Ref(true),
		})
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		_, err := memberClient.CreateChatProvider(ctx, nicloudsdk.CreateChatProviderConfigRequest{
			Provider: "openai",
			APIKey:   "member-key",
		})
		requireSDKError(t, err, http.StatusForbidden)
...
func TestUpdateChatProvider(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		updated, err := client.UpdateChatProvider(ctx, provider.ID, nicloudsdk.UpdateChatProviderConfigRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.UpdateChatProvider(ctx, uuid.New(), nicloudsdk.UpdateChatProviderConfigRequest{
			DisplayName: "missing",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("InvalidProviderID", func(t *testing.T) {
		...
		res, err := client.Request(
			...
			nicloudsdk.UpdateChatProviderConfigRequest{DisplayName: "ignored"},
		)
		require.NoError(t, err)
		defer res.Body.Close()

		err = nicloudsdk.ReadBodyAsError(res)
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		provider, err := adminClient.CreateChatProvider(ctx, nicloudsdk.CreateChatProviderConfigRequest{
			Provider: "openai",
			APIKey:   "test-api-key",
		})
		require.NoError(t, err)

		_, err = memberClient.UpdateChatProvider(ctx, provider.ID, nicloudsdk.UpdateChatProviderConfigRequest{
	...
	t.Run("RejectsDeploymentBackedCentralKey", func(t *testing.T) {
		...
		_, err = client.UpdateChatProvider(ctx, provider.ID, nicloudsdk.UpdateChatProviderConfigRequest{
			CentralAPIKeyEnabled: ptr.Ref(true),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, missingCentralKeyMessage, sdkErr.Message)
...
func TestChatProviderAPIKeysFromDeploymentValues(t *testing.T) {
	...
	t.Run("DoesNotReuseBridgeConfig", func(t *testing.T) {
		...
		keys := nicloud.ChatProviderAPIKeysFromDeploymentValues(values)
	...
	t.Run("NilDeploymentValues", func(t *testing.T) {
		...
		keys := nicloud.ChatProviderAPIKeysFromDeploymentValues(nil)
		require.Equal(t, chatprovider.ProviderAPIKeys{}, keys)
	})
}
...
func TestUserChatProviderConfigs(t *testing.T) {
	...
	requireUserProviderConfig := func(t *testing.T, configs []nicloudsdk.UserChatProviderConfig, provider string) nicloudsdk.UserChatProviderConfig {
		t.Helper()
		...
		return nicloudsdk.UserChatProviderConfig{}
	}

	requireNoUserProviderConfig := func(t *testing.T, configs []nicloudsdk.UserChatProviderConfig, provider string) {
		t.Helper()
	...
	t.Run("ListOnlyUserKeyProviders", func(t *testing.T) {
		...
		anthropicProvider, err := client.CreateChatProvider(ctx, nicloudsdk.CreateChatProviderConfigRequest{
			Provider:             "anthropic",
			CentralAPIKeyEnabled: ptr.Ref(false),
			AllowUserAPIKey:      ptr.Ref(true),
		})
	...
	t.Run("ListHidesDisabledProviderEvenWithSavedKey", func(t *testing.T) {
		...
		_, err = client.UpsertUserChatProviderKey(ctx, provider.ID, nicloudsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		require.NoError(t, err)
	...
	t.Run("UpsertCreatesKey", func(t *testing.T) {
		...
		config, err := client.UpsertUserChatProviderKey(ctx, provider.ID, nicloudsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		require.NoError(t, err)
	...
	t.Run("UpsertRejectsMissingProvider", func(t *testing.T) {
		...
		_, err := client.UpsertUserChatProviderKey(ctx, uuid.New(), nicloudsdk.CreateUserChatProviderKeyRequest{
			APIKey: "user-key",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("OtherUserDoesNotSeeKey", func(t *testing.T) {
		...
		_, err = adminClient.UpsertUserChatProviderKey(ctx, provider.ID, nicloudsdk.CreateUserChatProviderKeyRequest{
...
func TestListChatModelConfigs(t *testing.T) {
	...
	t.Run("AdminIncludesDisabledModelConfigs", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		disabledConfig, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			Provider:     "openai",
	...
	t.Run("NonAdminExcludesDisabledModelConfigs", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		_, err := adminClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
	...
	t.Run("DeserializesLegacyPricingJSON", func(t *testing.T) {
		...
		requireChatModelPricing(t, configs[0].ModelConfig, &nicloudsdk.ChatModelCallConfig{
...
func TestCreateChatModelConfig(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		isDefault := true
		pricing := &nicloudsdk.ChatModelCallConfig{
			Cost: &nicloudsdk.ModelCostConfig{
				InputPricePerMillionTokens:      decRef("0.15"),
				OutputPricePerMillionTokens:     decRef("0.6"),
				CacheReadPricePerMillionTokens:  decRef("0.03"),
		...
		modelConfig, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			Provider:     "openai",
			AIProviderID: &aiProvider.ID,
			Model:        "gpt-4o-mini",
			ContextLimit: &contextLimit,
	...
	t.Run("RejectsNegativePricing", func(t *testing.T) {
		...
		_, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			...
			Model:        "gpt-4o-mini",
			ContextLimit: &contextLimit,
			ModelConfig: &nicloudsdk.ChatModelCallConfig{
				Cost: &nicloudsdk.ModelCostConfig{
	...
	t.Run("WithAIProviderID", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = nicloudtest.CreateFirstUser(t, client.Client)
		provider, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeOpenAI,
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		_, err := memberClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			Provider:     "openai",
			AIProviderID: &aiProvider.ID,
			Model:        "gpt-4o-mini",
			ContextLimit: &contextLimit,
...
func TestUpdateChatModelConfig(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		updated, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, nicloudsdk.UpdateChatModelConfigRequest{
	...
	t.Run("DisablePreservesRecordAndHidesItFromNonAdmins", func(t *testing.T) {
		...
		enabled := false
		updated, err := adminClient.UpdateChatModelConfig(ctx, modelConfig.ID, nicloudsdk.UpdateChatModelConfigRequest{
			Enabled: &enabled,
		})
		require.NoError(t, err)
		require.Equal(t, modelConfig.ID, updated.ID)
	...
	t.Run("ReEnableRestoresVisibilityForNonAdmins", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		enabled := false
		modelConfig, err := adminClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			Provider:     "openai",
			AIProviderID: &aiProvider.ID,
	...
	t.Run("RejectsNegativePricing", func(t *testing.T) {
		...
		_, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, nicloudsdk.UpdateChatModelConfigRequest{
			ModelConfig: &nicloudsdk.ChatModelCallConfig{
				Cost: &nicloudsdk.ModelCostConfig{
	...
	t.Run("UpdateAIProviderID", func(t *testing.T) {
		...
		modelConfig := createChatModelConfig(t, client)
		provider, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeAnthropic,
	...
	t.Run("NotFoundWhenTargetRowDisappearsInTx", func(t *testing.T) {
		...
		client := nicloudsdk.NewExperimentalClient(nicloudtest.New(t, &nicloudtest.Options{
			Database:         store,
			Pubsub:           pubsub,
			DeploymentValues: chatDeploymentValues(t),
		}))
	...
	t.Run("InternalServerErrorWhenDefaultCandidateDisappearsInTx", func(t *testing.T) {
		...
		contextLimit := int64(4096)
		isDefault := false
		candidateConfig, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
			Provider:     "anthropic",
			AIProviderID: &aiProvider.ID,
		...
		_, err = client.UpdateChatModelConfig(ctx, defaultConfig.ID, nicloudsdk.UpdateChatModelConfigRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.UpdateChatModelConfig(ctx, uuid.New(), nicloudsdk.UpdateChatModelConfigRequest{
			DisplayName: "missing",
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("InvalidModelConfigID", func(t *testing.T) {
		...
		res, err := client.Request(
			...
			nicloudsdk.UpdateChatModelConfigRequest{DisplayName: "ignored"},
		)
		require.NoError(t, err)
		defer res.Body.Close()
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		modelConfig := createChatModelConfig(t, adminClient)
		_, err := memberClient.UpdateChatModelConfig(ctx, modelConfig.ID, nicloudsdk.UpdateChatModelConfigRequest{
			DisplayName: "member update",
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func TestGetChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		for _, message := range messagesResult.Messages {
			...
			for _, part := range message.Content {
				if message.Role == nicloudsdk.ChatMessageRoleUser &&
					part.Type == nicloudsdk.ChatMessagePartTypeText &&
	...
	t.Run("NotFoundForDifferentUser", func(t *testing.T) {
		...
		otherClientRaw, _ := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID, rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID))
		otherClient := nicloudsdk.NewExperimentalClient(otherClientRaw)
	...
	t.Run("FilesHydrated", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "check file hydration"}, {Type: nicloudsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("ToolCreatedFilesLinked", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "tool file test"},
			},
		})
		require.NoError(t, err)
		...
		rejected, err := store.LinkChatFiles(chatdCtx, database.LinkChatFilesParams{
			ChatID:       chat.ID,
			MaxFileLinks: int32(nicloudsdk.MaxChatFileIDs),
		...
		for i := 1; i < nicloudsdk.MaxChatFileIDs; i++ {
		...
		chatResult, err = client.GetChat(ctx, chat.ID)
		require.NoError(t, err)
		require.Len(t, chatResult.Files, nicloudsdk.MaxChatFileIDs)
...
func TestGetChatUserPrompts(t *testing.T) {
	...
	t.Run("NewestFirstFiltering", func(t *testing.T) {
		...
		want1 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "first "},
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "prompt"},
			},
		...
		want2 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "hello "},
				{Type: nicloudsdk.ChatMessagePartTypeFile, MediaType: "text/plain", Data: []byte("x")},
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "world"},
			},
			database.ChatMessageVisibilityBoth, false,
		)
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "   \n\t  "},
			},
			database.ChatMessageVisibilityBoth, false,
		)
		...
		// the response.
		assistantContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			{Type: nicloudsdk.ChatMessagePartTypeText, Text: "assistant reply"},
		})
		require.NoError(t, err)
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "deleted prompt"},
			},
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "model only"},
			},
		...
		want3 := insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "newest prompt"},
			},
	...
	t.Run("LimitClampsResults", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, user.UserID,
				[]nicloudsdk.ChatMessagePart{
					{Type: nicloudsdk.ChatMessagePartTypeText, Text: fmt.Sprintf("prompt %d", i)},
				},
				database.ChatMessageVisibilityBoth, false,
			)
		}

		resp, err := client.GetChatPrompts(ctx, chat.ID, &nicloudsdk.ChatPromptsOptions{Limit: 2})
		require.NoError(t, err)
	...
	t.Run("InvalidLimitRejected", func(t *testing.T) {
		...
		_, err = client.GetChatPrompts(ctx, chat.ID, &nicloudsdk.ChatPromptsOptions{Limit: 5000})
		require.Error(t, err)
	...
	t.Run("NotFoundForOtherUsers", func(t *testing.T) {
		...
		insertUserMessage(t, ctx, db, chat.ID, modelConfig.ID, firstUser.UserID,
			[]nicloudsdk.ChatMessagePart{
				{Type: nicloudsdk.ChatMessagePartTypeText, Text: "private prompt"},
			},
			database.ChatMessageVisibilityBoth, false,
		)

		memberClient, _ := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
		memberExp := nicloudsdk.NewExperimentalClient(memberClient)
...
func TestPatchChat(t *testing.T) {
	...
	createChat := func(ctx context.Context, t *testing.T, client *nicloudsdk.ExperimentalClient, orgID uuid.UUID, text string) nicloudsdk.Chat {
		t.Helper()
	...
	getChat := func(ctx context.Context, t *testing.T, client *nicloudsdk.ExperimentalClient, chatID uuid.UUID) nicloudsdk.Chat {
		t.Helper()
	...
	t.Run("PlanMode", func(t *testing.T) {
		...
		t.Run("SetToPlan", func(t *testing.T) {
			...
			err := client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(nicloudsdk.ChatPlanModePlan),
			})
			require.NoError(t, err)

			updated := getChat(ctx, t, client, chat.ID)
			require.Equal(t, nicloudsdk.ChatPlanModePlan, updated.PlanMode)
			require.True(t, mAudit.Contains(t, database.AuditLog{
		...
		t.Run("Clear", func(t *testing.T) {
			...
			err = client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(nicloudsdk.ChatPlanMode("")),
			})
			require.NoError(t, err)
		...
		t.Run("RejectsInvalidValue", func(t *testing.T) {
			...
			err := client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
				PlanMode: ptr.Ref(nicloudsdk.ChatPlanMode("invalid")),
			})
			sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("Title", func(t *testing.T) {
		...
		t.Run("Rename", func(t *testing.T) {
			...
			nicloudtest.WaitForChatSettled(ctx, t, api, chat.ID)
		...
		t.Run("PreservesUpdatedAt", func(t *testing.T) {
			...
			providerKeys := nicloudtest.FakeOpenAICompatProviderAPIKeys(t)
			clientRaw, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
				DeploymentValues:    chatDeploymentValues(t),
				Database:            db,
				Pubsub:              ps,
				ChatProviderAPIKeys: &providerKeys,
			})
			client := nicloudsdk.NewExperimentalClient(clientRaw)
			firstUser := nicloudtest.CreateFirstUser(t, client.Client)
			_ = createChatModelConfig(t, client)
		...
		t.Run("PublishesWatchEvent", func(t *testing.T) {
			...
			go func() {
				_ = client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
			...
			for {
				...
				if received.Kind == nicloudsdk.ChatWatchEventKindTitleChange &&
					received.Chat.ID == chat.ID {
...
func TestArchiveChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		mAudit := audit.NewMock()
		client := newChatClient(t, func(o *nicloudtest.Options) {
			o.Auditor = mAudit
		})
		firstUser := nicloudtest.CreateFirstUser(t, client.Client)
		_ = createChatModelConfig(t, client)

		chatToArchive, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		chatToKeep, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		err = client.UpdateChat(ctx, chatToArchive.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		...
		activeChats, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
		...
		archivedChats, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{
			Query: "archived:true",
		})
		require.NoError(t, err)
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, uuid.New(), nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("ArchivesChildren", func(t *testing.T) {
		...
		require.NoError(t, err)
		var foundParent *nicloudsdk.Chat
	...
	t.Run("AllowsChildChatArchiveIndividually", func(t *testing.T) {
		...
		err = client.UpdateChat(ctx, child.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		...
		activeChats, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{Query: "archived:false"})
		require.NoError(t, err)
		var activeParent *nicloudsdk.Chat
		...
		archivedChats, err := client.ListChats(ctx, &nicloudsdk.ListChatsOptions{Query: "archived:true"})
		require.NoError(t, err)
...
func TestUnarchiveChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		err = client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
		...
		require.True(t, archivedChats[0].Archived)
		// Unarchive the chat.
		err = client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		require.NoError(t, err)
	...
	t.Run("RejectsChildChatWhenParentArchived", func(t *testing.T) {
		...
		// is also excluded).
		err = client.UpdateChat(ctx, child.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, uuid.New(), nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(false)})
		requireSDKError(t, err, http.StatusNotFound)
	})
}
...
func TestChatPinOrder(t *testing.T) {
	...
	createChat := func(ctx context.Context, t *testing.T, client *nicloudsdk.ExperimentalClient, orgID uuid.UUID, title string) nicloudsdk.Chat {
		t.Helper()
	...
	t.Run("PinReorderAndUnpin", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, first.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, second.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		err = client.UpdateChat(ctx, third.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
		require.NoError(t, err)
		...
		err = client.UpdateChat(ctx, first.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(0))})
		require.NoError(t, err)
	...
	t.Run("ArchiveClearsPinOrder", func(t *testing.T) {
		...
		err = client.UpdateChat(ctx, first.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
	...
	t.Run("RejectsNegative", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(-1))})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("RejectsChildChat", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, child.ID, nicloudsdk.UpdateChatRequest{PinOrder: ptr.Ref(int32(1))})
...
func TestPostChatMessages(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		hasTextPart := func(parts []nicloudsdk.ChatMessagePart, want string) bool {
			for _, part := range parts {
				if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text == want {
					return true
				}
			}
			return false
		...
		created, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: messageText,
		...
		if created.Queued {
			...
			require.Eventually(t, func() bool {
				...
				for _, message := range messagesResult.Messages {
					if message.Role == nicloudsdk.ChatMessageRoleUser && hasTextPart(message.Content, messageText) {
						return true
					}
				}
				return false
			...
			require.Equal(t, nicloudsdk.ChatMessageRoleUser, created.Message.Role)
			...
			require.Eventually(t, func() bool {
				...
				for _, message := range messagesResult.Messages {
					if message.ID == created.Message.ID &&
						message.Role == nicloudsdk.ChatMessageRoleUser &&
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		_, err := memberClient.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: "this should fail",
	...
	t.Run("EmptyText", func(t *testing.T) {
		...
		_, err = client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: "   ",
	...
	t.Run("ChatNotFound", func(t *testing.T) {
		...
		_, err := client.CreateChatMessage(ctx, uuid.New(), nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: "hello",
...
func waitForChatWatchStatusChangeEvent(
	...
	for {
		...
		err := wsjson.Read(ctx, conn, &payload)
		require.NoError(t, err)
		if payload.Kind == nicloudsdk.ChatWatchEventKindStatusChange && payload.Chat.ID == chatID {
			return payload
		}
	}
}
...
func TestSendMessageWithModelOverrideUpdatesLastModelConfigID(t *testing.T) {
	...
	resp, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
		Content: []nicloudsdk.ChatInputPart{{
			Type: nicloudsdk.ChatInputPartTypeText,
...
func TestSendMessageQueuesEffectiveModelConfigID(t *testing.T) {
	...
	resp, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
		...
		ModelConfigID: ptr.Ref(modelConfigB.ID),
		BusyBehavior:  nicloudsdk.ChatBusyBehaviorQueue,
	})
	require.NoError(t, err)
...
func TestSubsequentSendWithoutOverrideUsesPersistedModel(t *testing.T) {
	...
	user := nicloudtest.CreateFirstUser(t, client.Client)
	_ = createChatModelConfig(t, client)
	modelConfigB := createAdditionalChatModelConfig(t, client, nicloudtest.TestChatProviderOpenAICompat, "gpt-4o-mini-persisted-"+uuid.NewString())
...
func TestWatchChatsStatusChangeCarriesUpdatedLastModelConfigID(t *testing.T) {
	...
	t.Run("QueuedPromotion", func(t *testing.T) {
		...
		queuedResp, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
...
func TestChatMessageWithFileReferences(t *testing.T) {
	...
	createChatForTest := func(t *testing.T, client *nicloudsdk.ExperimentalClient, orgID uuid.UUID) nicloudsdk.Chat {
		t.Helper()
		ctx := testutil.Context(t, testutil.WaitLong)
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: orgID,
	...
	t.Run("FileReferenceOnly", func(t *testing.T) {
		...
		created, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type:      nicloudsdk.ChatInputPartTypeFileReference,
				FileName:  "main.go",
				StartLine: 10,
				EndLine:   15,
				Content:   "func broken() {}",
		...
		checkFileRef := func(part nicloudsdk.ChatMessagePart) bool {
			return part.Type == nicloudsdk.ChatMessagePartTypeFileReference &&
	...
	t.Run("InterleavedTextAndFileReferences", func(t *testing.T) {
		...
		type wantPart struct {
			typ       nicloudsdk.ChatMessagePartType
			text      string
			fileName  string
			startLine int
			endLine   int
		...
		want := []wantPart{
			{typ: nicloudsdk.ChatMessagePartTypeText, text: "Please review these two issues:"},
			{typ: nicloudsdk.ChatMessagePartTypeFileReference, fileName: "a.go", startLine: 1, endLine: 3, content: "line1\nline2\nline3"},
			{typ: nicloudsdk.ChatMessagePartTypeText, text: "first issue"},
			{typ: nicloudsdk.ChatMessagePartTypeText, text: "and also:"},
			{typ: nicloudsdk.ChatMessagePartTypeFileReference, fileName: "b.go", startLine: 10, endLine: 10, content: "return nil"},
			{typ: nicloudsdk.ChatMessagePartTypeText, text: "second issue"},
		}
		...
		require.Eventually(t, func() bool {
			...
			checkParts := func(parts []nicloudsdk.ChatMessagePart) bool {
				...
				for i, w := range want {
					...
					switch w.typ {
					case nicloudsdk.ChatMessagePartTypeText:
						if p.Text != w.text {
							return false
						}
					case nicloudsdk.ChatMessagePartTypeFileReference:
			...
			for _, msg := range messagesResult.Messages {
				if msg.Role == nicloudsdk.ChatMessageRoleUser && checkParts(msg.Content) {
					return true
				}
			}
			if created.Queued && created.QueuedMessage != nil {
...
func TestChatMessageWithFiles(t *testing.T) {
	...
	t.Run("FileOnly", func(t *testing.T) {
		...
		if resp.Queued {
			...
			require.NotNil(t, resp.Message)
			require.Equal(t, nicloudsdk.ChatMessageRoleUser, resp.Message.Role)
		}
	})
	...
	t.Run("TextAndFile", func(t *testing.T) {
		...
		for _, msg := range messagesResult.Messages {
			for _, part := range msg.Content {
				if part.Type == nicloudsdk.ChatMessagePartTypeFile {
	...
	t.Run("FilesLinkedOnSend", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "no files yet"},
			},
		})
		require.NoError(t, err)
		...
		_, err = client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "here is a file"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("DedupFileIDs", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "first mention"}, {Type: nicloudsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)

		// Send another message with the SAME file.
		msgResp, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "same file again"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("FileCapExceeded", func(t *testing.T) {
		...
		fileIDs := make([]uuid.UUID, 0, nicloudsdk.MaxChatFileIDs)
		for i := range nicloudsdk.MaxChatFileIDs {
		...
		parts := []nicloudsdk.ChatInputPart{
			{Type: nicloudsdk.ChatInputPartTypeText, Text: "max files"},
		}
		for _, fid := range fileIDs {
			parts = append(parts, nicloudsdk.ChatInputPart{Type: nicloudsdk.ChatInputPartTypeFile, FileID: fid})
		}
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{OrganizationID: firstUser.OrganizationID, Content: parts})
		require.NoError(t, err)
		require.Empty(t, chat.Warnings, "creating a chat at exactly the cap should not warn")
		require.Len(t, chat.Files, nicloudsdk.MaxChatFileIDs, "all files should be linked on creation")
		...
		msgResp, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "one too many"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: extraResp.ID},
			},
		})
		require.NoError(t, err)
		...
		msgResp2, err := client.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "re-reference existing"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: fileIDs[0]},
			},
		})
		require.NoError(t, err)
	...
	t.Run("FileCapOnCreate", func(t *testing.T) {
		...
		fileIDs := make([]uuid.UUID, 0, nicloudsdk.MaxChatFileIDs+1)
		for i := range nicloudsdk.MaxChatFileIDs + 1 {
		...
		parts := []nicloudsdk.ChatInputPart{
			{Type: nicloudsdk.ChatInputPartTypeText, Text: "over cap on create"},
		}
		for _, fid := range fileIDs {
			parts = append(parts, nicloudsdk.ChatInputPart{Type: nicloudsdk.ChatInputPartTypeFile, FileID: fid})
		}
...
func TestPatchChatMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		edited, err := client.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
		...
		require.Equal(t, nicloudsdk.ChatMessageRoleUser, edited.Message.Role)

		foundEditedText := false
		for _, part := range edited.Message.Content {
			if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text == "hello after edit" {
	...
	t.Run("PreservesFileID", func(t *testing.T) {
		...
		for _, part := range edited.Message.Content {
			if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text == "after edit with file" {
				foundText = true
			}
			if part.Type == nicloudsdk.ChatMessagePartTypeFile && part.FileID.Valid && part.FileID.UUID == uploadResp.ID {
				foundFile = true
				require.Nil(t, part.Data, "file data should not be sent when file_id is present")
			}
		}
	...
	t.Run("UsageLimitExceeded", func(t *testing.T) {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("MessageNotFound", func(t *testing.T) {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, 999999, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{
					Type: nicloudsdk.ChatInputPartTypeText,
					Text: "edited",
	...
	t.Run("FilesLinkedOnEdit", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "before file edit"},
			},
		})
		require.NoError(t, err)
		...
		var userMessageID int64
		for _, msg := range messagesResult.Messages {
			if msg.Role == nicloudsdk.ChatMessageRoleUser {
		...
		_, err = client.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "after file edit"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: uploadResp.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("CapExceededOnEdit", func(t *testing.T) {
		...
		parts := []nicloudsdk.ChatInputPart{
			{Type: nicloudsdk.ChatInputPartTypeText, Text: "fill to cap"},
		}
		pngData := append([]byte{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, make([]byte, 64)...)
		for i := range nicloudsdk.MaxChatFileIDs {
			up, err := client.UploadChatFile(ctx, firstUser.OrganizationID, "image/png", fmt.Sprintf("cap-%d.png", i), bytes.NewReader(pngData))
			require.NoError(t, err)
			parts = append(parts, nicloudsdk.ChatInputPart{Type: nicloudsdk.ChatInputPartTypeFile, FileID: up.ID})
		}
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{OrganizationID: firstUser.OrganizationID, Content: parts})
		require.NoError(t, err)
		...
		edited, err := client.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "edit with extra file"},
				{Type: nicloudsdk.ChatInputPartTypeFile, FileID: extra.ID},
			},
		})
		require.NoError(t, err)
	...
	t.Run("ChangesModel", func(t *testing.T) {
		...
		testutil.Eventually(ctx, t, func(ctx context.Context) bool {
			...
			return c.Status != nicloudsdk.ChatStatusPending &&
				c.Status != nicloudsdk.ChatStatusRunning
		...
		for !foundInitialUserMessage {
			...
			case event, ok := <-events:
				...
				if event.Type == nicloudsdk.ChatStreamEventTypeMessage &&
					event.Message != nil &&
					event.Message.Role == nicloudsdk.ChatMessageRoleUser &&
...
func TestInterruptChat(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ChatStatusWaiting, interrupted.Status)
...
func TestRegenerateChatTitle(t *testing.T) {
	...
	t.Run("UpdateDenied", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		clientRaw, db := nicloudtest.NewWithDatabase(t, &nicloudtest.Options{
			Authorizer: &nicloudtest.FakeAuthorizer{
...
func TestProposeChatTitle(t *testing.T) {
	...
	t.Run("DoesNotPersistTitleOrBumpUpdatedAt", func(t *testing.T) {
		...
		chat, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{
				{Type: nicloudsdk.ChatInputPartTypeText, Text: "test chat"},
			},
		})
		require.NoError(t, err)
...
func TestManualTitleEndpointsPassCallerAPIKeyToAIGateway(t *testing.T) {
	...
	for _, tt := range []struct {
		name string
		call func(context.Context, *nicloudsdk.ExperimentalClient, uuid.UUID) error
	}{
		{
			name: "RegenerateChatTitle",
			call: func(ctx context.Context, client *nicloudsdk.ExperimentalClient, chatID uuid.UUID) error {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			client, db, api := newChatClientWithAPIAndDatabase(t, func(opts *nicloudtest.Options) {
			...
			require.NoError(t, client.UpdateChatModelOverride(ctx, nicloudsdk.ChatModelOverrideContextTitleGeneration, nicloudsdk.UpdateChatModelOverrideRequest{
			...
			content, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText("manual title source"),
			})
			require.NoError(t, err)
			_ = dbgen.ChatMessage(t, db, database.ChatMessage{
				ChatID:        chat.ID,
...
func TestDeleteChatQueuedMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		deleteContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("queued message for delete route"),
		})
		require.NoError(t, err)
	...
	t.Run("InvalidQueuedMessageID", func(t *testing.T) {
		...
		err = nicloudsdk.ReadBodyAsError(invalidRes)
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestPromoteChatQueuedMessage(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText(queuedText),
		})
		require.NoError(t, err)
		queuedMessage, err := db.InsertChatQueuedMessage(
			dbauthz.AsSystemRestricted(ctx),
		...
		for _, msg := range messagesResult.Messages {
			...
			for _, part := range msg.Content {
				if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text == queuedText {
					foundPromoted = true
				}
			}
		}
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("queued message no agents access"),
		})
		require.NoError(t, err)
		queuedMessage, err := db.InsertChatQueuedMessage(
			dbauthz.AsSystemRestricted(ctx),
	...
	t.Run("ArchivedChat", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("queued"),
		})
		require.NoError(t, err)
		queuedMessage, err := db.InsertChatQueuedMessage(
			dbauthz.AsSystemRestricted(ctx),
		...
		promoteErr := nicloudsdk.ReadBodyAsError(promoteRes)
		var promoteSDKErr *nicloudsdk.Error
		...
		for _, msg := range messages {
			...
			for _, part := range parts {
				if msg.Role == database.ChatMessageRoleTool &&
					part.Type == nicloudsdk.ChatMessagePartTypeToolResult &&
	...
	t.Run("WhileRunning", func(t *testing.T) {
		...
		queuedContent, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("running-promote"),
		})
		require.NoError(t, err)
		queuedMessage, err := db.InsertChatQueuedMessage(
			dbauthz.AsSystemRestricted(ctx),
...
func TestChatUsageLimitOverrideRoutes(t *testing.T) {
	...
	t.Run("UpsertUserOverrideRequiresPositiveSpendLimit", func(t *testing.T) {
		...
		firstUser := nicloudtest.CreateFirstUser(t, client.Client)
		_, member := nicloudtest.CreateAnotherUser(t, client.Client, firstUser.OrganizationID)
	...
	t.Run("UpsertUserOverrideMissingUser", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitOverride(ctx, uuid.New(), nicloudsdk.UpsertChatUsageLimitOverrideRequest{
	...
	t.Run("UpdateUserOverride", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitOverride(ctx, member.ID, nicloudsdk.UpsertChatUsageLimitOverrideRequest{
			SpendLimitMicros: 5_000_000,
		})
		require.NoError(t, err)

		override, err := client.UpsertChatUsageLimitOverride(ctx, member.ID, nicloudsdk.UpsertChatUsageLimitOverrideRequest{
	...
	t.Run("UpsertGroupOverrideIncludesMemberCount", func(t *testing.T) {
		...
		override, err := client.UpsertChatUsageLimitGroupOverride(ctx, group.ID, nicloudsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 7_000_000,
		})
		require.NoError(t, err)
		...
		var listed *nicloudsdk.ChatUsageLimitGroupOverride
	...
	t.Run("UpdateGroupOverride", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitGroupOverride(ctx, group.ID, nicloudsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 5_000_000,
		})
		require.NoError(t, err)
	...
	t.Run("UpsertGroupOverrideMissingGroup", func(t *testing.T) {
		...
		_, err := client.UpsertChatUsageLimitGroupOverride(ctx, uuid.New(), nicloudsdk.UpsertChatUsageLimitGroupOverrideRequest{
			SpendLimitMicros: 7_000_000,
		})
		sdkErr := requireSDKError(t, err, http.StatusNotFound)
...
`)
	...
	t.Run("MissingOrganization", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		nicloudtest.CreateFirstUser(t, client.Client)
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		unauthed := nicloudsdk.NewExperimentalClient(nicloudsdk.New(client.URL))
		data := append([]byte{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, make([]byte, 64)...)
...
func (f chatCostTestFixture) safeOptions() nicloudsdk.ChatCostSummaryOptions {
	return nicloudsdk.ChatCostSummaryOptions{
...
func assertChatCostSummary(t *testing.T, summary nicloudsdk.ChatCostSummary, modelConfigID, chatID uuid.UUID) {
	t.Helper()
...
func TestChatCostSummary_AdminDrilldown(t *testing.T) {
	...
	options := nicloudsdk.ChatCostSummaryOptions{
...
func TestChatCostUsers(t *testing.T) {
	...
	t.Run("AdminCanListUsers", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		resp, err := client.GetChatCostUsers(ctx, nicloudsdk.ChatCostUsersOptions{})
		require.NoError(t, err)
	...
	t.Run("MemberCannotListUsers", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		_, err := memberClient.GetChatCostUsers(ctx, nicloudsdk.ChatCostUsersOptions{})
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
		require.ErrorAs(t, err, &sdkErr)
...
func TestChatCostSummary_DateRange(t *testing.T) {
	...
	t.Run("MessageInRange", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		summary, err := client.GetChatCostSummary(ctx, "me", nicloudsdk.ChatCostSummaryOptions{
...
func requireChatModelPricing(
	t *testing.T,
	actual *nicloudsdk.ChatModelCallConfig,
	expected *nicloudsdk.ChatModelCallConfig,
) {
	t.Helper()
...
// in nicloud/workspaceagents_internal_test.go.
func TestWatchChatGitAuthz(t *testing.T) {
	...
	secondAdminClient, _ := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID, rbac.RoleOwner())
	...
	_, workspaceOwner := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	...
	chat, err := adminClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
		OrganizationID: firstUser.OrganizationID,
		Content: []nicloudsdk.ChatInputPart{
			{Type: nicloudsdk.ChatInputPartTypeText, Text: "codagt-184"},
		},
	})
	require.NoError(t, err)
	...
	err = adminClient.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
		WorkspaceID: &workspaceBuild.Workspace.ID,
	})
	require.NoError(t, err)
	...
	_, err = secondAdminClient.UpdateUserRoles(ctx, firstUser.UserID.String(), nicloudsdk.UpdateRoles{
		Roles: []string{rbac.RoleTemplateAdmin().String()},
	})
	require.NoError(t, err)

	_, err = secondAdminClient.UpdateOrganizationMemberRoles(ctx, firstUser.OrganizationID, firstUser.UserID.String(), nicloudsdk.UpdateRoles{
...
func createAIProviderForTest(
	...
	req := nicloudsdk.CreateAIProviderRequest{
		Type:    nicloudsdk.AIProviderType(provider),
...
func createChatModelConfig(t testing.TB, client *nicloudsdk.ExperimentalClient) nicloudsdk.ChatModelConfig {
	t.Helper()
	return nicloudtest.CreateOpenAICompatChatModelConfig(t, client, "")
}

func createChatModelConfigWithBaseURL(t testing.TB, client *nicloudsdk.ExperimentalClient, baseURL string) nicloudsdk.ChatModelConfig {
	t.Helper()
	return nicloudtest.CreateOpenAICompatChatModelConfig(t, client, baseURL)
}
...
func createChatModelConfigWithTitleFailure(t testing.TB, client *nicloudsdk.ExperimentalClient) nicloudsdk.ChatModelConfig {
	t.Helper()
...
func enableUserChatProviderKey(
	t testing.TB,
	adminClient *nicloudsdk.ExperimentalClient,
	userClient *nicloudsdk.ExperimentalClient,
	providerName string,
) nicloudsdk.AIProvider {
	t.Helper()
	...
	var provider nicloudsdk.AIProvider
	for _, candidate := range providers {
		if candidate.Type == nicloudsdk.AIProviderType(providerName) {
			provider = candidate
			break
		}
	}
	...
	_, err = userClient.UpsertUserAIProviderKey(ctx, "me", provider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{
...
//nolint:tparallel,paralleltest // Subtests share a single nicloudtest instance.
...
If a workspace is needed, use list_templates and read_template as needed before create_workspace.`
	...
	updateChatSystemPrompt := func(t *testing.T, ctx context.Context, req nicloudsdk.UpdateChatSystemPromptRequest) {
		t.Helper()
	...
	getChatSystemPrompt := func(t *testing.T, ctx context.Context) nicloudsdk.ChatSystemPromptResponse {
		t.Helper()
	...
	assertInjectedSystemMessages := func(t *testing.T, ctx context.Context, wantResolvedPrompt string) {
		...
		for _, message := range messages {
			...
			require.Equal(t, nicloudsdk.ChatMessagePartTypeText, parts[0].Type)
	...
	t.Run("AdminCanSet", func(t *testing.T) {
		...
		updateChatSystemPrompt(t, ctx, nicloudsdk.UpdateChatSystemPromptRequest{
	...
	t.Run("PreservesIncludeDefaultWhenOmitted", func(t *testing.T) {
		...
		err := client.UpdateChatSystemPrompt(ctx, nicloudsdk.UpdateChatSystemPromptRequest{
			SystemPrompt:               "",
			IncludeDefaultSystemPrompt: ptr.Ref(false),
		})
		require.NoError(t, err)
	...
	t.Run("ExistingCustomPromptDefaultsIncludeDefaultOff", func(t *testing.T) {
		...
		firstUser := nicloudtest.CreateFirstUser(t, legacyClient.Client)
		...
		chat, err := legacyClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			OrganizationID: firstUser.OrganizationID,
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("NonAdminFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatSystemPrompt(ctx, nicloudsdk.UpdateChatSystemPromptRequest{
	...
	t.Run("UnauthenticatedFails", func(t *testing.T) {
		...
		anonClient := nicloudsdk.NewExperimentalClient(nicloudsdk.New(adminClient.URL))
	...
	t.Run("TooLong", func(t *testing.T) {
		...
		err := adminClient.UpdateChatSystemPrompt(ctx, nicloudsdk.UpdateChatSystemPromptRequest{
...
func TestChatPlanModeInstructions(t *testing.T) {
	...
	updateChatPlanModeInstructions := func(t *testing.T, ctx context.Context, req nicloudsdk.UpdateChatPlanModeInstructionsRequest) {
		t.Helper()
	...
	getChatPlanModeInstructions := func(t *testing.T, ctx context.Context) nicloudsdk.ChatPlanModeInstructionsResponse {
		t.Helper()
	...
	for _, tt := range roundTripTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, instructions := range tt.updates {
				updateChatPlanModeInstructions(t, ctx, nicloudsdk.UpdateChatPlanModeInstructionsRequest{
	...
	t.Run("OversizedPayloadReturns400", func(t *testing.T) {
		...
		err := adminClient.UpdateChatPlanModeInstructions(ctx, nicloudsdk.UpdateChatPlanModeInstructionsRequest{
...
//nolint:tparallel,paralleltest // Setting subtests share per-setting nicloudtest instances.
func TestChatModelOverrides(t *testing.T) {
	t.Parallel()

	type overrideResponse struct {
		context       nicloudsdk.ChatModelOverrideContext
	...
	settingPath := func(overrideContext nicloudsdk.ChatModelOverrideContext) string {
	...
	getOverride := func(
		ctx context.Context,
		client *nicloudsdk.ExperimentalClient,
		overrideContext nicloudsdk.ChatModelOverrideContext,
	...
	putOverride := func(
		...
		return client.UpdateChatModelOverride(
			ctx,
			overrideContext,
			nicloudsdk.UpdateChatModelOverrideRequest{ModelConfigID: modelConfigID},
		)
	}
	...
	settings := []settingTest{
		{
			name:    "General",
			context: nicloudsdk.ChatModelOverrideContextGeneral,
		...
		{
			name:    "Explore",
			context: nicloudsdk.ChatModelOverrideContextExplore,
			dbGet: func(ctx context.Context, db database.Store) (string, error) {
		...
		},
		{
			...
			context: nicloudsdk.ChatModelOverrideContextTitleGeneration,
			dbGet: func(ctx context.Context, db database.Store) (string, error) {
	...
	t.Run("UnknownContextReturns400", func(t *testing.T) {
		...
		adminClient := newChatClient(t)
		nicloudtest.CreateFirstUser(t, adminClient.Client)
		unknownContext := nicloudsdk.ChatModelOverrideContext("not-a-context")
...
//nolint:tparallel,paralleltest // Subtests share nicloudtest instances.
func TestChatPersonalModelOverridesAdminSettings(t *testing.T) {
	...
	err = adminClient.UpdateChatPersonalModelOverridesAdminSettings(ctx, nicloudsdk.UpdateChatPersonalModelOverridesAdminSettingsRequest{
		AllowUsers: true,
	})
	require.NoError(t, err)
	resp, err = adminClient.GetChatPersonalModelOverridesAdminSettings(ctx)
	...
	err = memberClient.UpdateChatPersonalModelOverridesAdminSettings(ctx, nicloudsdk.UpdateChatPersonalModelOverridesAdminSettingsRequest{
		AllowUsers: true,
	})
	requireSDKError(t, err, http.StatusForbidden)
...
func TestUserChatPersonalModelOverrides(t *testing.T) {
	...
	adminClient, db := newChatClientWithDatabase(t)
	firstUser := nicloudtest.CreateFirstUser(t, adminClient.Client)
	memberClientRaw, member := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
	noKeyClientRaw, noKeyUser := nicloudtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
	noKeyClient := nicloudsdk.NewExperimentalClient(noKeyClientRaw)
	...
	_, err := memberClient.UpsertUserAIProviderKey(ctx, "me", modelProvider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{
		APIKey: "test-user-api-key-" + uuid.NewString(),
	})
	require.NoError(t, err)
	contextLimit := int64(4096)
	...
	require.NoError(t, err)
	err = adminClient.UpdateChatModelOverride(ctx, nicloudsdk.ChatModelOverrideContextGeneral, nicloudsdk.UpdateChatModelOverrideRequest{
		ModelConfigID: modelConfig.ID.String(),
	})
	require.NoError(t, err)
	err = adminClient.UpdateChatModelOverride(ctx, nicloudsdk.ChatModelOverrideContextExplore, nicloudsdk.UpdateChatModelOverrideRequest{
	...
	contextLimit = int64(4096)
	disabledProviderModelConfig, err := adminClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
		Provider:     "google",
	...
	require.NoError(t, err)
	enabled := false
	disabledProvider, err = adminClient.UpdateAIProvider(ctx, disabledProvider.ID.String(), nicloudsdk.UpdateAIProviderRequest{
		Enabled: &enabled,
	})
	require.NoError(t, err)
	require.NotEqual(t, uuid.Nil, provider.ID)
	...
	personalOverride := func(
		resp nicloudsdk.UserChatPersonalModelOverridesResponse,
		overrideContext nicloudsdk.ChatPersonalModelOverrideContext,
	) nicloudsdk.ChatPersonalModelOverride {
		t.Helper()
		switch overrideContext {
		case nicloudsdk.ChatPersonalModelOverrideContextRoot:
			return resp.Root
		case nicloudsdk.ChatPersonalModelOverrideContextGeneral:
			return resp.General
		case nicloudsdk.ChatPersonalModelOverrideContextExplore:
			return resp.Explore
		default:
			t.Fatalf("unexpected personal model override context %q", overrideContext)
			return nicloudsdk.ChatPersonalModelOverride{}
		}
	}
	assertOverride := func(
		resp nicloudsdk.UserChatPersonalModelOverridesResponse,
	...
	assertDeploymentDefault := func(
		...
		t.Helper()
		var override nicloudsdk.ChatModelOverrideResponse
		switch overrideContext {
		case nicloudsdk.ChatModelOverrideContextGeneral:
			override = resp.DeploymentDefaults.General
		case nicloudsdk.ChatModelOverrideContextExplore:
	...
	getRawFor := func(userID uuid.UUID, overrideContext nicloudsdk.ChatPersonalModelOverrideContext) string {
		t.Helper()
	...
	getRaw := func(overrideContext nicloudsdk.ChatPersonalModelOverrideContext) string {
		t.Helper()
	...
	t.Run("GETDisabledReturnsMissingDefaults", func(t *testing.T) {
		...
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.ChatPersonalModelOverrideModeChatDefault, "", false, false)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextGeneral, nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", false, false)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextExplore, nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", false, false)
	})

	upsertRaw(nicloudsdk.ChatPersonalModelOverrideContextRoot, string(nicloudsdk.ChatPersonalModelOverrideModeChatDefault))
	upsertRaw(nicloudsdk.ChatPersonalModelOverrideContextGeneral, string(nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault))
	upsertRaw(nicloudsdk.ChatPersonalModelOverrideContextExplore, "model:"+modelConfig.ID.String())

	t.Run("GETDisabledReturnsSavedValues", func(t *testing.T) {
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		require.False(t, resp.Enabled)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.ChatPersonalModelOverrideModeChatDefault, "", true, false)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextGeneral, nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextExplore, nicloudsdk.ChatPersonalModelOverrideModeModel, modelConfig.ID.String(), true, false)
	})

	t.Run("GETIncludesDeploymentDefaults", func(t *testing.T) {
		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertDeploymentDefault(resp, nicloudsdk.ChatModelOverrideContextGeneral, modelConfig.ID.String(), false)
		assertDeploymentDefault(resp, nicloudsdk.ChatModelOverrideContextExplore, defaultModelConfig.ID.String(), false)
	})

	t.Run("PUTDisabledReturns403AndPreservesRows", func(t *testing.T) {
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode:          nicloudsdk.ChatPersonalModelOverrideModeModel,
			ModelConfigID: modelConfig.ID.String(),
		})
		requireSDKError(t, err, http.StatusForbidden)
		require.Equal(t, string(nicloudsdk.ChatPersonalModelOverrideModeChatDefault), getRaw(nicloudsdk.ChatPersonalModelOverrideContextRoot))
	})
	...
	contexts := []nicloudsdk.ChatPersonalModelOverrideContext{
		nicloudsdk.ChatPersonalModelOverrideContextRoot,
		nicloudsdk.ChatPersonalModelOverrideContextGeneral,
		nicloudsdk.ChatPersonalModelOverrideContextExplore,
	}

	t.Run("PUTRejectsUnknownMode", func(t *testing.T) {
		rawBefore := getRaw(nicloudsdk.ChatPersonalModelOverrideContextGeneral)
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, nicloudsdk.ChatPersonalModelOverrideContextGeneral, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode: nicloudsdk.ChatPersonalModelOverrideMode("banana"),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Contains(t, sdkErr.Message, "Invalid personal model override mode.")
		require.Equal(t, rawBefore, getRaw(nicloudsdk.ChatPersonalModelOverrideContextGeneral))
	})

	t.Run("PUTChatDefaultRoundTrips", func(t *testing.T) {
		for _, overrideContext := range contexts {
			err := memberClient.UpdateUserChatPersonalModelOverride(ctx, overrideContext, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
		...
		for _, overrideContext := range contexts {
			assertOverride(resp, overrideContext, nicloudsdk.ChatPersonalModelOverrideModeChatDefault, "", true, false)
		}
	})

	t.Run("PUTChatDefaultRejectsNonEmptyModelConfigID", func(t *testing.T) {
		rawBefore := getRaw(nicloudsdk.ChatPersonalModelOverrideContextRoot)
		err := memberClient.UpdateUserChatPersonalModelOverride(ctx, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
			Mode:          nicloudsdk.ChatPersonalModelOverrideModeChatDefault,
			ModelConfigID: modelConfig.ID.String(),
		})
		...
		require.Equal(t, rawBefore, getRaw(nicloudsdk.ChatPersonalModelOverrideContextRoot))
	})

	t.Run("PUTDeploymentDefaultRoundTripsForAgentContexts", func(t *testing.T) {
		for _, overrideContext := range []nicloudsdk.ChatPersonalModelOverrideContext{
			nicloudsdk.ChatPersonalModelOverrideContextGeneral,
			nicloudsdk.ChatPersonalModelOverrideContextExplore,
		} {
			err := memberClient.UpdateUserChatPersonalModelOverride(ctx, overrideContext, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
		...
		require.NoError(t, err)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextGeneral, nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextExplore, nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault, "", true, false)
	})
	...
	t.Run("PUTModelRoundTrips", func(t *testing.T) {
		...
		require.NoError(t, err)
		for _, overrideContext := range contexts {
			assertOverride(resp, overrideContext, nicloudsdk.ChatPersonalModelOverrideModeModel, modelConfig.ID.String(), true, false)
		}
	})
	...
	t.Run("PUTModelRejectsInvalidModels", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				rawBefore := getRawFor(tc.userID, nicloudsdk.ChatPersonalModelOverrideContextGeneral)
				err := tc.client.UpdateUserChatPersonalModelOverride(ctx, nicloudsdk.ChatPersonalModelOverrideContextGeneral, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
					Mode:          nicloudsdk.ChatPersonalModelOverrideModeModel,
					ModelConfigID: tc.modelConfigID,
				})
				sdkErr := requireSDKError(t, err, http.StatusBadRequest)
				require.Contains(t, sdkErr.Message, tc.wantMessageSubstring)
				rawAfter := getRawFor(tc.userID, nicloudsdk.ChatPersonalModelOverrideContextGeneral)
	...
	t.Run("GETMalformedStoredValueFallsBackToContextDefault", func(t *testing.T) {
		upsertRaw(nicloudsdk.ChatPersonalModelOverrideContextRoot, "model:not-a-uuid")

		resp, err := memberClient.GetUserChatPersonalModelOverrides(ctx)
		require.NoError(t, err)
		assertOverride(resp, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.ChatPersonalModelOverrideModeChatDefault, "", true, true)
	})

	t.Run("GETRootDeploymentDefaultIsMalformed", func(t *testing.T) {
		upsertRaw(
			nicloudsdk.ChatPersonalModelOverrideContextRoot,
			string(nicloudsdk.ChatPersonalModelOverrideModeDeploymentDefault),
		)
...
func TestCreateChatPersonalModelOverrideRoot(t *testing.T) {
	...
	_, err := adminClient.UpsertUserAIProviderKey(ctx, "me", overrideProvider.ID, nicloudsdk.CreateUserAIProviderKeyRequest{
		APIKey: "test-user-api-key-" + uuid.NewString(),
	})
	require.NoError(t, err)
	contextLimit := int64(4096)
	overrideModel, err := adminClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
		Provider:     "anthropic",
	...
	memberClientRaw, member := nicloudtest.CreateAnotherUser(
		t,
		adminClient.Client,
		firstUser.OrganizationID,
		rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID),
	...
	upsertRootRaw := func(userID uuid.UUID, value string) {
		...
		err := db.UpsertUserChatPersonalModelOverride(dbauthz.AsSystemRestricted(ctx), database.UpsertUserChatPersonalModelOverrideParams{
			UserID: userID,
			Key:    chatd.ChatPersonalModelOverrideKey(nicloudsdk.ChatPersonalModelOverrideContextRoot),
			Value:  value,
		})
		require.NoError(t, err)
	}
	...
	require.NoError(t, err)
	err = adminClient.UpdateUserChatPersonalModelOverride(ctx, nicloudsdk.ChatPersonalModelOverrideContextRoot, nicloudsdk.UpdateUserChatPersonalModelOverrideRequest{
		Mode:          nicloudsdk.ChatPersonalModelOverrideModeModel,
...
func TestChatDesktopEnabled(t *testing.T) {
	...
	t.Run("AdminCanSetTrue", func(t *testing.T) {
		...
		err := adminClient.UpdateChatDesktopEnabled(ctx, nicloudsdk.UpdateChatDesktopEnabledRequest{
			EnableDesktop: true,
		})
		require.NoError(t, err)
	...
	t.Run("NonAdminWriteFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatDesktopEnabled(ctx, nicloudsdk.UpdateChatDesktopEnabledRequest{
			EnableDesktop: true,
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func TestChatComputerUseProvider(t *testing.T) {
	...
	t.Run("AdminCanSetAnthropic", func(t *testing.T) {
		...
		err := adminClient.UpdateChatComputerUseProvider(ctx, nicloudsdk.UpdateChatComputerUseProviderRequest{
			Provider: "anthropic",
		})
		require.NoError(t, err)
	...
	t.Run("NonAdminWriteFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatComputerUseProvider(ctx, nicloudsdk.UpdateChatComputerUseProviderRequest{
			Provider: "openai",
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func TestChatDebugLoggingSettings(t *testing.T) {
	...
	t.Run("AdminAllowsUsersToOptIn", func(t *testing.T) {
		...
		err := adminClient.UpdateChatDebugLogging(ctx, nicloudsdk.UpdateChatDebugLoggingAllowUsersRequest{
			AllowUsers: true,
		})
		require.NoError(t, err)
		...
		err = memberClient.UpdateUserChatDebugLogging(ctx, nicloudsdk.UpdateUserChatDebugLoggingRequest{
	...
	t.Run("NonAdminCannotManageAdminSetting", func(t *testing.T) {
		...
		err = memberClient.UpdateChatDebugLogging(ctx, nicloudsdk.UpdateChatDebugLoggingAllowUsersRequest{
			AllowUsers: true,
		})
		requireSDKError(t, err, http.StatusForbidden)
	})
...
func seedChatDebugRun(
	...
	run, err := db.InsertChatDebugRun(dbauthz.AsSystemRestricted(ctx), database.InsertChatDebugRunParams{
		ChatID:    chatID,
		Kind:      string(nicloudsdk.ChatDebugRunKindChatTurn),
		Status:    string(nicloudsdk.ChatDebugStatusInProgress),
...
func seedChatDebugStep(
	...
	step, err := db.InsertChatDebugStep(dbauthz.AsSystemRestricted(ctx), database.InsertChatDebugStepParams{
		...
		Operation:  string(nicloudsdk.ChatDebugStepOperationStream),
		Status:     string(nicloudsdk.ChatDebugStatusCompleted),
	})
	require.NoError(t, err)
	return step
}
...
func TestChatDebugRuns(t *testing.T) {
	...
	t.Run("ListReturnsRunsNewestFirst", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ChatDebugRunKindChatTurn, runs[0].Kind)
		require.Equal(t, nicloudsdk.ChatDebugStatusInProgress, runs[0].Status)
	})
...
func TestChatDebugRun(t *testing.T) {
	...
	t.Run("ReturnsRunWithSteps", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ChatDebugRunKindChatTurn, got.Kind)
		require.Equal(t, nicloudsdk.ChatDebugStatusInProgress, got.Status)
		...
		require.Equal(t, nicloudsdk.ChatDebugStepOperationStream, got.Steps[0].Operation)
	})
...
func TestChatAdvisorConfig_GetDefault(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.AdvisorConfig{}, resp)
}
...
func TestChatAdvisorConfig_MemberCannotWriteButCanRead(t *testing.T) {
	...
	err = memberClient.UpdateChatAdvisorConfig(ctx, nicloudsdk.UpdateAdvisorConfigRequest{
		Enabled: true,
	})
	requireSDKError(t, err, http.StatusForbidden)
...
func TestChatAdvisorConfig_NegativeMaxUsesPerRunRejected(t *testing.T) {
	...
	err := adminClient.UpdateChatAdvisorConfig(ctx, nicloudsdk.UpdateAdvisorConfigRequest{
		MaxUsesPerRun: -1,
	})
	sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestChatAdvisorConfig_OverwriteClearsPreviousValues(t *testing.T) {
	...
	sparse := nicloudsdk.AdvisorConfig{Enabled: true}
...
func TestChatAdvisorConfig_CanBeDisabledAfterEnabled(t *testing.T) {
	...
	err := adminClient.UpdateChatAdvisorConfig(ctx, nicloudsdk.AdvisorConfig{
		Enabled:       true,
		MaxUsesPerRun: 2,
	})
	require.NoError(t, err)
...
func TestChatAdvisorConfig_ClampsNegativeStoredValues(t *testing.T) {
	...
	resp, err := adminClient.GetChatAdvisorConfig(ctx)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.AdvisorConfig{
		Enabled:         true,
		MaxUsesPerRun:   0,
		MaxOutputTokens: 0,
	}, resp)
...
func TestChatAdvisorConfig_UnauthenticatedFails(t *testing.T) {
	...
	err = anonClient.UpdateChatAdvisorConfig(ctx, nicloudsdk.UpdateAdvisorConfigRequest{
		Enabled: true,
	})
	require.ErrorAs(t, err, &sdkErr)
	require.Equal(t, http.StatusUnauthorized, sdkErr.StatusCode())
...
func TestChatWorkspaceTTL(t *testing.T) {
	...
	err = adminClient.UpdateChatWorkspaceTTL(ctx, nicloudsdk.UpdateChatWorkspaceTTLRequest{
	...
	err = memberClient.UpdateChatWorkspaceTTL(ctx, nicloudsdk.UpdateChatWorkspaceTTLRequest{
...
func TestChatRetentionDays(t *testing.T) {
	...
	err = adminClient.UpdateChatRetentionDays(ctx, nicloudsdk.UpdateChatRetentionDaysRequest{
		RetentionDays: 90,
	})
	...
	err = memberClient.UpdateChatRetentionDays(ctx, nicloudsdk.UpdateChatRetentionDaysRequest{RetentionDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	err = adminClient.UpdateChatRetentionDays(ctx, nicloudsdk.UpdateChatRetentionDaysRequest{
		RetentionDays: 3651, // retentionDaysMaximum + 1; keep in sync with nicloud/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestChatDebugRetentionDays(t *testing.T) {
	...
	require.NoError(t, err, "get default")
	require.Equal(t, nicloudsdk.DefaultChatDebugRetentionDays, resp.DebugRetentionDays, "default should match DefaultChatDebugRetentionDays")

	// Admin can set debug retention days to 14.
	err = adminClient.UpdateChatDebugRetentionDays(ctx, nicloudsdk.UpdateChatDebugRetentionDaysRequest{
	...
	// Non-admin member cannot write.
	err = memberClient.UpdateChatDebugRetentionDays(ctx, nicloudsdk.UpdateChatDebugRetentionDaysRequest{DebugRetentionDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	// Validation: exceeding the 3650-day maximum is rejected.
	err = adminClient.UpdateChatDebugRetentionDays(ctx, nicloudsdk.UpdateChatDebugRetentionDaysRequest{
		DebugRetentionDays: 3651, // chatDebugRetentionDaysMaximum + 1; keep in sync with nicloud/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestChatAutoArchiveDays(t *testing.T) {
	...
	require.NoError(t, err, "get default")
	require.Equal(t, nicloudsdk.DefaultChatAutoArchiveDays, resp.AutoArchiveDays, "default should match DefaultChatAutoArchiveDays")

	// Admin can set auto-archive days to 45.
	err = adminClient.UpdateChatAutoArchiveDays(ctx, nicloudsdk.UpdateChatAutoArchiveDaysRequest{
		AutoArchiveDays: 45,
	})
	...
	// Non-admin member cannot write.
	err = memberClient.UpdateChatAutoArchiveDays(ctx, nicloudsdk.UpdateChatAutoArchiveDaysRequest{AutoArchiveDays: 7})
	requireSDKError(t, err, http.StatusForbidden)
	...
	// Validation: exceeding the 3650-day maximum is rejected.
	err = adminClient.UpdateChatAutoArchiveDays(ctx, nicloudsdk.UpdateChatAutoArchiveDaysRequest{
		AutoArchiveDays: 3651, // autoArchiveDaysMaximum + 1; keep in sync with nicloud/exp_chats.go.
	})
	requireSDKError(t, err, http.StatusBadRequest)
}
...
func TestUserChatCompactionThresholds(t *testing.T) {
	...
	t.Run("PutAndGet", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		override, err := client.UpdateUserChatCompactionThreshold(ctx, modelConfig.ID, nicloudsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 75,
		})
		require.NoError(t, err)
	...
	t.Run("UpsertChangesValue", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		_, err := client.UpdateUserChatCompactionThreshold(ctx, modelConfig.ID, nicloudsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 50,
		})
		require.NoError(t, err)
	...
	t.Run("NonExistentModelConfig", func(t *testing.T) { //nolint:paralleltest // subtests share parent state
		...
		fakeID := uuid.New()
		_, err := client.UpdateUserChatCompactionThreshold(ctx, fakeID, nicloudsdk.UpdateUserChatCompactionThresholdRequest{
			ThresholdPercent: 50,
		})
		requireSDKError(t, err, http.StatusNotFound)
	})
...
//nolint:tparallel // Subtests share a single nicloudtest instance and run sequentially.
func TestChatTemplateAllowlist(t *testing.T) {
	t.Parallel()

	// Shared setup: one nicloudtest instance with two real templates.
	// Subtests that need valid template IDs use these.
	client, store := newChatClientWithDatabase(t)
	admin := nicloudtest.CreateFirstUser(t, client.Client)
	...
	//nolint:paralleltest // Sequential: subtests share a single nicloudtest instance.
	t.Run("ReturnsEmptyWhenUnset", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
	...
	t.Run("AdminCanSet", func(t *testing.T) {
		...
		err := client.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: ids})
		require.NoError(t, err)
		resp, err := client.GetChatTemplateAllowlist(ctx)
		require.NoError(t, err)
	...
	t.Run("AdminCanClear", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: []string{}})
		require.NoError(t, err)
		resp, err := client.GetChatTemplateAllowlist(ctx)
		require.NoError(t, err)
		require.Empty(t, resp.TemplateIDs)
	...
	t.Run("NonAdminReadFails", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		memberClientRaw, _ := nicloudtest.CreateAnotherUser(t, client.Client, admin.OrganizationID)
		memberClient := nicloudsdk.NewExperimentalClient(memberClientRaw)
	...
	t.Run("NonAdminWriteFails", func(t *testing.T) {
		...
		err := memberClient.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusNotFound)
	})
	...
	t.Run("UnauthenticatedFails", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		anonClient := nicloudsdk.NewExperimentalClient(nicloudsdk.New(client.URL))
		// Uses a random UUID — hits 401 before template validation.
		err := anonClient.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusUnauthorized)
	})
	...
	t.Run("InvalidUUIDRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: []string{"not-a-uuid"}})
		requireSDKError(t, err, http.StatusBadRequest)
	})
	...
	t.Run("NonexistentTemplateRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{TemplateIDs: []string{uuid.NewString()}})
		requireSDKError(t, err, http.StatusBadRequest)
	})
	...
	t.Run("DeprecatedTemplateRejected", func(t *testing.T) {
		ctx := testutil.Context(t, testutil.WaitLong)
		err := client.UpdateChatTemplateAllowlist(ctx, nicloudsdk.ChatTemplateAllowlist{
...
func TestGetChatsByWorkspace(t *testing.T) {
	...
	t.Run("ReturnsLatestNonArchivedChat", func(t *testing.T) {
		...
		err := client.UpdateChat(ctx, olderChat.ID, nicloudsdk.UpdateChatRequest{Archived: ptr.Ref(true)})
		require.NoError(t, err)
...
func TestSubmitToolResults(t *testing.T) {
	...
	setupRequiresAction := func(
		...
		parts := make([]nicloudsdk.ChatMessagePart, 0, len(toolCallIDs))
	...
	t.Run("Success", func(t *testing.T) {
		...
		err := client.SubmitToolResults(ctx, chat.ID, nicloudsdk.SubmitToolResultsRequest{
			Results: []nicloudsdk.ToolResult{
		...
		require.NoError(t, err)
		require.NotEqual(t, nicloudsdk.ChatStatusRequiresAction, gotChat.Status,
		...
		for _, msg := range msgsResp.Messages {
			if msg.Role == nicloudsdk.ChatMessageRoleTool {
				toolResultCount++
			}
		}
	...
	t.Run("NotFoundForDifferentUser", func(t *testing.T) {
		...
		// to user A's chat.
		otherClientRaw, _ := nicloudtest.CreateAnotherUser(
			t, client.Client, user.OrganizationID,
			rbac.ScopedRoleAgentsAccess(user.OrganizationID),
		)
		otherClient := nicloudsdk.NewExperimentalClient(otherClientRaw)

		err := otherClient.SubmitToolResults(ctx, chat.ID, nicloudsdk.SubmitToolResultsRequest{
			Results: []nicloudsdk.ToolResult{
	...
	t.Run("MemberWithoutAgentsAccess", func(t *testing.T) {
		...
		err := memberClient.SubmitToolResults(ctx, chat.ID, nicloudsdk.SubmitToolResultsRequest{
			Results: []nicloudsdk.ToolResult{
...
func TestPostChats_DynamicToolValidation(t *testing.T) {
	...
	t.Run("TooManyTools", func(t *testing.T) {
		...
		tools := make([]nicloudsdk.DynamicTool, 251)
		for i := range tools {
			tools[i] = nicloudsdk.DynamicTool{
	...
	t.Run("EmptyToolName", func(t *testing.T) {
		...
		_, err := client.CreateChat(ctx, nicloudsdk.CreateChatRequest{
			...
			UnsafeDynamicTools: []nicloudsdk.DynamicTool{
				{Name: ""},
			},
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func TestChatStartWorkspace_RequireActiveVersion(t *testing.T) {
	...
	db := api.Database
	user := nicloudtest.CreateFirstUser(t, rawClient)
	...
	build, err := nicloud.ChatStartWorkspace(api, ctx, user.UserID, wsResp.Workspace.ID,
...
func TestChatStopWorkspace_BypassesRequireActiveVersion(t *testing.T) {
	...
	build, err := nicloud.ChatStopWorkspace(api, ctx, user.UserID, wsResp.Workspace.ID,
		nicloudsdk.CreateWorkspaceBuildRequest{})

	require.NoError(t, err)
	require.Equal(t, nicloudsdk.WorkspaceTransitionStop, build.Transition)
...
func TestGetChatMessages_Pagination(t *testing.T) {
	...
	seedChat := func(
		...
		for i := range count {
			content, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
				nicloudsdk.ChatMessageText(fmt.Sprintf("msg %d", i)),
			})
			require.NoError(t, err)
	...
	seedQueuedMessage := func(
		...
		content, err := json.Marshal([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("queued"),
		})
		require.NoError(t, err)
	...
	t.Run("BeforeIDReturnsOlderAndSuppressesQueued", func(t *testing.T) {
		...
		resp, err := client.GetChatMessages(ctx, chat.ID, &nicloudsdk.ChatMessagesPaginationOptions{
			BeforeID: ids[2],
		})
		require.NoError(t, err)
		require.False(t, resp.HasMore)
	...
	t.Run("NegativeAfterIDReturns400", func(t *testing.T) {
		...
		var sdkResp nicloudsdk.Response
		...
		require.True(t,
			slices.ContainsFunc(sdkResp.Validations, func(v nicloudsdk.ValidationError) bool {
	...
	t.Run("AfterIDGreaterThanOrEqualBeforeIDReturns400", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				_, err := client.GetChatMessages(ctx, chat.ID, &nicloudsdk.ChatMessagesPaginationOptions{
					AfterID:  tc.after,
					BeforeID: tc.before,
				})
				sdkErr := requireSDKError(t, err, http.StatusBadRequest)
...
func requireSDKError(t *testing.T, err error, expectedStatus int) *nicloudsdk.Error {
	t.Helper()
...
func TestChatReadOnlySharedWriteHandlers(t *testing.T) {
	...
	setup := func(t *testing.T) (
		ctx context.Context,
		ownerClient *nicloudsdk.ExperimentalClient,
		sharedClient *nicloudsdk.ExperimentalClient,
		chat nicloudsdk.Chat,
		db database.Store,
	) {
		t.Helper()
		...
		owner := nicloudtest.CreateFirstUser(t, ownerClient.Client)
		_ = createChatModelConfig(t, ownerClient)
		sharedRaw, sharedUser := nicloudtest.CreateAnotherUser(
			t,
			ownerClient.Client,
			owner.OrganizationID,
			rbac.ScopedRoleAgentsAccess(owner.OrganizationID),
		)
		sharedClient = nicloudsdk.NewExperimentalClient(sharedRaw)

		var err error
		chat, err = ownerClient.CreateChat(ctx, nicloudsdk.CreateChatRequest{
	...
	t.Run("GetChatAndMessages", func(t *testing.T) {
		...
		for _, message := range messagesResult.Messages {
			...
			for _, part := range message.Content {
				if part.Type == nicloudsdk.ChatMessagePartTypeText && part.Text == sharedChatText {
					foundUserMessage = true
					break
				}
			}
	...
	t.Run("PatchChat", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		err := sharedClient.UpdateChat(ctx, chat.ID, nicloudsdk.UpdateChatRequest{
			Archived: ptr.Ref(true),
		})
	...
	t.Run("PatchChatMessage", func(t *testing.T) {
		...
		_, err = sharedClient.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("PostChatMessages", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		_, err := sharedClient.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("PostChatToolResults", func(t *testing.T) {
		...
		ctx, _, sharedClient, chat, _ := setup(t)
		err := sharedClient.SubmitToolResults(ctx, chat.ID, nicloudsdk.SubmitToolResultsRequest{
			Results: []nicloudsdk.ToolResult{{
...
func TestChatOwnerOnlyWriteHandlers(t *testing.T) {
	...
	setupOrgAdminAndOwnerChat := func(t *testing.T) (
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		ownerClient, db = newChatClientWithDatabase(t)
		firstUser := nicloudtest.CreateFirstUser(t, ownerClient.Client)
		_ = createChatModelConfig(t, ownerClient)
		...
		orgAdminRaw, _ := nicloudtest.CreateAnotherUser(
			t,
			ownerClient.Client,
			firstUser.OrganizationID,
			rbac.ScopedRoleOrgAdmin(firstUser.OrganizationID),
		...
		adminClient = nicloudsdk.NewExperimentalClient(orgAdminRaw)
	...
	t.Run("PostChatMessages", func(t *testing.T) {
		...
		_, err := adminClient.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("PatchChatMessage", func(t *testing.T) {
		...
		_, err = adminClient.EditChatMessage(ctx, chat.ID, userMessageID, nicloudsdk.EditChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	t.Run("SubmitToolResults", func(t *testing.T) {
		...
		err := adminClient.SubmitToolResults(ctx, chat.ID, nicloudsdk.SubmitToolResultsRequest{
			Results: []nicloudsdk.ToolResult{{
	...
	t.Run("OwnerCanSendMessages", func(t *testing.T) {
		...
		_, err := ownerClient.CreateChatMessage(ctx, chat.ID, nicloudsdk.CreateChatMessageRequest{
			Content: []nicloudsdk.ChatInputPart{{
				Type: nicloudsdk.ChatInputPartTypeText,
