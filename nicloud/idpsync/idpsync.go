...
// claims to the internal representation of a user in Neural Inverse Cloud.
...
type DeploymentSyncSettings struct {
	...
	// to site roles within Neural Inverse Cloud.
...
func FromDeploymentValues(dv *nicloudsdk.DeploymentValues) DeploymentSyncSettings {
	if dv == nil {
...
func (e HTTPError) Write(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(r.Context(), rw, e.Code, nicloudsdk.Response{
		Message: e.Msg,
		Detail:  e.Detail,
	})
}
