	...
	if err != nil {
		...
		if s.DeploymentSyncSettings.Legacy.GroupField != "" {
			...
			if defaultOrganization.ID == orgID {
				settings = ptr.Ref(GroupSyncSettings(nicloudsdk.GroupSyncSettings{
...
type GroupSyncSettings nicloudsdk.GroupSyncSettings
...
// the external IDP group name matches the Neural Inverse Cloud one.
