...
// @Success 200 {object} nicloudsdk.UploadResponse "Returns existing file if duplicate"
// @Success 201 {object} nicloudsdk.UploadResponse "Returns newly created file"
...
func (api *API) postFile(rw http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.UploadResponse{
			ID: file.ID,
		})
		return
	...
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.UploadResponse{
		ID: file.ID,
	})
}
...
func (api *API) fileByID(rw http.ResponseWriter, r *http.Request) {
	...
	switch format {
	case nicloudsdk.FormatZip:
		if file.Mimetype != nicloudsdk.ContentTypeTar {
			httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
		...
		rw.Header().Set("Content-Type", nicloudsdk.ContentTypeZip)
