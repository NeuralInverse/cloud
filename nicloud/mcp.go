...
func (api *API) listMCPServerConfigs(rw http.ResponseWriter, r *http.Request) {
	...
	resp := make([]nicloudsdk.MCPServerConfig, 0, len(configs))
	for _, config := range configs {
		var sdkConfig nicloudsdk.MCPServerConfig
		if isAdmin {
...
func (api *API) createMCPServerConfig(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateMCPServerConfigRequest
	...
	case "oauth2":
		...
		if req.OAuth2ClientID == "" && req.OAuth2AuthURL == "" && req.OAuth2TokenURL == "" {
			...
			inserted, err := api.Database.InsertMCPServerConfig(ctx, database.InsertMCPServerConfigParams{
				...
				ForwardNIHeaders:     req.ForwardNIHeaders,
			...
			updated, err := api.Database.UpdateMCPServerConfig(ctx, database.UpdateMCPServerConfigParams{
				...
				ForwardNIHeaders:     inserted.ForwardNIHeaders,
				UpdatedBy:               apiKey.UserID,
			})
			if err != nil {
				httpapi.Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
...
func (api *API) updateMCPServerConfig(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateMCPServerConfigRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		forwardNIHeaders := existing.ForwardNIHeaders
		if req.ForwardNIHeaders != nil {
			forwardNIHeaders = *req.ForwardNIHeaders
		}
...
func (api *API) mcpServerOAuth2Callback(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusBadGateway, nicloudsdk.Response{
...
func convertMCPServerConfig(config database.MCPServerConfig) nicloudsdk.MCPServerConfig {
	return nicloudsdk.MCPServerConfig{
		ID:          config.ID,
		...
		ForwardNIHeaders: config.ForwardNIHeaders,
...
// non-admin callers.
func convertMCPServerConfigRedacted(config database.MCPServerConfig) nicloudsdk.MCPServerConfig {
...
func discoverAndRegisterMCPOAuth2(ctx context.Context, httpClient *http.Client, mcpServerURL, callbackURL string) (*mcpOAuth2Discovery, error) {
	...
	clientID, clientSecret, err := registerOAuth2Client(
		ctx, httpClient, asMeta.RegistrationEndpoint, callbackURL, "Neural Inverse Cloud",
	)
	if err != nil {
		return nil, xerrors.Errorf(
