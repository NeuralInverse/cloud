...
func GetAppSecrets(db database.Store) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		secrets := []nicloudsdk.OAuth2ProviderAppSecret{}
		for _, secret := range dbSecrets {
			secrets = append(secrets, nicloudsdk.OAuth2ProviderAppSecret{
				ID:                    secret.ID,
				LastUsedAt:            nicloudsdk.NullTime{NullTime: secret.LastUsedAt},
...
func CreateAppSecret(db database.Store, auditor *audit.Auditor, logger slog.Logger) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		aReq.New = dbSecret
		httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.OAuth2ProviderAppSecretFull{
			ID:               dbSecret.ID,
