...
func extractTokenRequest(r *http.Request, callbackURL *url.URL) (nicloudsdk.OAuth2TokenRequest, []nicloudsdk.ValidationError, error) {
	p := httpapi.NewQueryParamParser()
	err := r.ParseForm()
	if err != nil {
		return nicloudsdk.OAuth2TokenRequest{}, nil, xerrors.Errorf("parse form: %w", err)
	}

	vals := r.Form
	p.RequiredNotEmpty("grant_type")
	grantType := httpapi.ParseCustom(p, vals, "", "grant_type", httpapi.ParseEnum[nicloudsdk.OAuth2ProviderGrantType])

	// Grant-type specific validation - must be called before parsing values.
	switch grantType {
	case nicloudsdk.OAuth2ProviderGrantTypeRefreshToken:
		p.RequiredNotEmpty("refresh_token")
	case nicloudsdk.OAuth2ProviderGrantTypeAuthorizationCode:
		p.RequiredNotEmpty("code")
	}

	req := nicloudsdk.OAuth2TokenRequest{
		GrantType:    grantType,
	...
	if user, pass, ok := r.BasicAuth(); ok && user != "" {
		if req.ClientID != "" && req.ClientID != user {
			return nicloudsdk.OAuth2TokenRequest{}, nil, errConflictingClientAuth
		}
	...
	if req.GrantType == nicloudsdk.OAuth2ProviderGrantTypeAuthorizationCode {
		if req.ClientID == "" {
	...
	if len(p.Errors) > 0 {
		return nicloudsdk.OAuth2TokenRequest{}, p.Errors, xerrors.Errorf("invalid query params: %w", p.Errors)
	}
	return req, nil, nil
}
...
func Tokens(db database.Store, lifetimes nicloudsdk.SessionLifetime) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if errors.Is(err, errConflictingClientAuth) {
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidRequest, "Conflicting client credentials between Authorization header and request body")
				return
			}

			// Check for specific validation errors in priority order
			if slices.ContainsFunc(validationErrs, func(validationError nicloudsdk.ValidationError) bool {
				return validationError.Field == "grant_type"
			}) {
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeUnsupportedGrantType, "The grant type is missing or unsupported")
				return
			}
			...
			for _, field := range []string{"code", "client_id", "client_secret"} {
				if slices.ContainsFunc(validationErrs, func(validationError nicloudsdk.ValidationError) bool {
					...
					httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidRequest, fmt.Sprintf("Missing required parameter: %s", field))
					return
				}
			}
			// Generic invalid request for other validation errors
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidRequest, "The request is missing required parameters or is otherwise malformed")
			return
		}

		var token nicloudsdk.OAuth2TokenResponse
		...
		default:
			...
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeUnsupportedGrantType, fmt.Sprintf("The grant type %q is not supported", req.GrantType))
			return
		}

		if errors.Is(err, errBadSecret) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusUnauthorized, nicloudsdk.OAuth2ErrorCodeInvalidClient, "The client credentials are invalid")
			return
		}
		if errors.Is(err, errBadCode) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidGrant, "The authorization code is invalid or expired")
			return
		}
		if errors.Is(err, errInvalidPKCE) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidGrant, "The PKCE code verifier is invalid")
			return
		}
		if errors.Is(err, errInvalidResource) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidTarget, "The resource parameter is invalid")
			return
		}
		if errors.Is(err, errBadToken) {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidGrant, "The refresh token is invalid or expired")
			return
		}
		if err != nil {
			httpapi.Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
...
func authorizationCodeGrant(ctx context.Context, db database.Store, app database.OAuth2ProviderApp, lifetimes nicloudsdk.SessionLifetime, req nicloudsdk.OAuth2TokenRequest) (nicloudsdk.OAuth2TokenResponse, error) {
	// Validate the client secret.
	secret, err := ParseFormattedSecret(req.ClientSecret)
	if err != nil {
		return nicloudsdk.OAuth2TokenResponse{}, errBadSecret
	}
	...
	if err != nil {
		return nicloudsdk.OAuth2TokenResponse{}, err
	}
	...
	if err != nil {
		return nicloudsdk.OAuth2TokenResponse{}, errBadCode
	}
	...
	if dbCode.ResourceUri.Valid && dbCode.ResourceUri.String != "" {
		...
		if req.Resource == "" {
			return nicloudsdk.OAuth2TokenResponse{}, errInvalidResource
		}
	...
	if err != nil {
		return nicloudsdk.OAuth2TokenResponse{}, xerrors.Errorf("fetch user actor: %w", err)
	}
	...
	return nicloudsdk.OAuth2TokenResponse{
		AccessToken:  sessionToken,
		TokenType:    nicloudsdk.OAuth2TokenTypeBearer,
...
func refreshTokenGrant(ctx context.Context, db database.Store, app database.OAuth2ProviderApp, lifetimes nicloudsdk.SessionLifetime, req nicloudsdk.OAuth2TokenRequest) (nicloudsdk.OAuth2TokenResponse, error) {
	// Validate the token.
	token, err := ParseFormattedSecret(req.RefreshToken)
	if err != nil {
		return nicloudsdk.OAuth2TokenResponse{}, errBadToken
	}
