...
func TestExtractAuthorizeParams_TokenResponseTypeDoesNotRequirePKCE(t *testing.T) {
	...
	query := url.Values{}
	query.Set("response_type", string(nicloudsdk.OAuth2ProviderResponseTypeToken))
	query.Set("client_id", "test-client")
	query.Set("redirect_uri", "http://localhost:3000/callback")
	...
	require.NoError(t, err)
	require.Empty(t, validationErrs)
	require.Equal(t, nicloudsdk.OAuth2ProviderResponseTypeToken, params.responseType)
}
...
func TestRefreshTokenGrant_Scopes(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, validationErrs)
	require.Equal(t, nicloudsdk.OAuth2ProviderGrantTypeRefreshToken, tokenReq.GrantType)
