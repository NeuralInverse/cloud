...
func extractRevocationRequest(r *http.Request) (nicloudsdk.OAuth2TokenRevocationRequest, error) {
	if err := r.ParseForm(); err != nil {
		return nicloudsdk.OAuth2TokenRevocationRequest{}, xerrors.Errorf("invalid form data: %w", err)
	}

	req := nicloudsdk.OAuth2TokenRevocationRequest{
		Token:         r.Form.Get("token"),
		TokenTypeHint: nicloudsdk.OAuth2RevocationTokenTypeHint(r.Form.Get("token_type_hint")),
	...
	if req.Token == "" {
		return nicloudsdk.OAuth2TokenRevocationRequest{}, xerrors.New("missing token parameter")
	}
...
func RevokeToken(db database.Store, logger slog.Logger) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodPost {
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusMethodNotAllowed, nicloudsdk.OAuth2ErrorCodeInvalidRequest, "Method not allowed")
			return
		}
		...
		if err != nil {
			...
			if errors.Is(err, ErrInvalidTokenFormat) {
				...
				logger.Debug(ctx, "token revocation failed: invalid token format",
					slog.F("client_id", app.ID.String()),
					slog.F("app_name", app.Name))
				httpapi.WriteOAuth2Error(ctx, rw, http.StatusBadRequest, nicloudsdk.OAuth2ErrorCodeInvalidRequest, "Invalid token format")
				return
			}
			logger.Error(ctx, "token revocation failed with internal server error",
				...
				slog.F("client_id", app.ID.String()),
				slog.F("app_name", app.Name))
			httpapi.WriteOAuth2Error(ctx, rw, http.StatusInternalServerError, nicloudsdk.OAuth2ErrorCodeServerError, "Internal server error")
			return
		}
