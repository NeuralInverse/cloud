...
func CalculateAutostop(ctx context.Context, params CalculateAutostopParams) (AutostopTime, error) {
	...
	if ttl > 0 {
		...
		if params.WorkspaceAutostart != "" {
			...
			// 2. Neural Inverse Cloud stops workspace at 9pm
...
func WeeksSinceEpoch(now time.Time) (int64, error) {
	...
	if now.Before(epoch) {
		return 0, xerrors.New("neuralinverse server system clock is incorrect, cannot calculate template autostop requirement")
	}
