...
func TestDeploymentInsights(t *testing.T) {
	...
	daus, err := client.DeploymentDAUs(ctx, nicloudsdk.TimezoneOffsetHour(clientTz))
	require.NoError(t, err)

	_ = agenttest.New(t, client.URL, authToken)
	resources := nicloudtest.NewWorkspaceAgentWaiter(t, client, workspace.ID).Wait()
...
func TestUserActivityInsights_SanityCheck(t *testing.T) {
	...
	user := nicloudtest.CreateFirstUser(t, client)
	_, _ = nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
	authToken := uuid.NewString()
	version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse:          echo.ParseComplete,
		ProvisionPlan:  echo.PlanComplete,
	...
	var userActivities nicloudsdk.UserActivityInsightsResponse
	require.Eventuallyf(t, func() bool {
		...
		userActivities, err = client.UserActivityInsights(ctx, nicloudsdk.UserActivityInsightsRequest{
			StartTime:   today,
...
func TestUserLatencyInsights(t *testing.T) {
	...
	var userLatencies nicloudsdk.UserLatencyInsightsResponse
	require.Eventuallyf(t, func() bool {
		// Keep connection active.
		_, err := w.Write([]byte("hello world\n"))
		if !assert.NoError(t, err) {
		...
		userLatencies, err = client.UserLatencyInsights(ctx, nicloudsdk.UserLatencyInsightsRequest{
			StartTime:   today,
			EndTime:     time.Now().UTC().Truncate(time.Hour).Add(time.Hour), // Round up to include the current hour.
			TemplateIDs: []uuid.UUID{template.ID},
		})
...
func TestUserLatencyInsights_BadRequest(t *testing.T) {
	...
	_, err := client.UserLatencyInsights(ctx, nicloudsdk.UserLatencyInsightsRequest{
		StartTime: today,
...
func TestUserActivityInsights_BadRequest(t *testing.T) {
	...
	_, err = client.UserActivityInsights(ctx, nicloudsdk.UserActivityInsightsRequest{
		StartTime: today,
		EndTime:   today.AddDate(0, 0, -1),
	})
	assert.Error(t, err, "want error for end time before start time")
...
func TestTemplateInsights_Golden(t *testing.T) {
	...
	prepare := func(t *testing.T, templates []*testTemplate, users []*testUser, testData map[*testWorkspace]testDataGen, disableStorage bool) (*nicloudsdk.Client, chan dbrollup.Event) {
		logger := testutil.Logger(t)
		db, ps := dbtestutil.NewDB(t)
		...
		for _, user := range users {
			user.client, user.sdk = nicloudtest.CreateAnotherUserMutators(t, client, firstUser.OrganizationID, nil, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
		...
		for _, template := range templates {
			...
			for _, user := range users {
				for _, workspace := range user.workspaces {
					...
					var buildParameters []nicloudsdk.WorkspaceBuildParameter
					for _, buildParameter := range workspace.buildParameters {
						buildParameters = append(buildParameters, nicloudsdk.WorkspaceBuildParameter{
					...
					createWorkspaces = append(createWorkspaces, func(templateID uuid.UUID) {
						...
						createdWorkspace := nicloudtest.CreateWorkspace(t, user.client, templateID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
							cwr.RichParameterValues = buildParameters
						})
						workspace.id = createdWorkspace.ID
						waitWorkspaces = append(waitWorkspaces, func() {
							nicloudtest.AwaitWorkspaceBuildJobCompleted(t, user.client, createdWorkspace.LatestBuild.ID)
			...
			// 	createdTemplate := nicloudtest.CreateTemplate(t, client, firstUser.OrganizationID, version.ID)
			createdTemplate := dbgen.Template(t, db, database.Template{
				...
				GroupACL: database.TemplateACL{
					firstUser.OrganizationID.String(): db2sdk.TemplateRoleActions(nicloudsdk.TemplateRoleUse),
				},
			})
		...
		// NOTE(mafredri): Ideally we would pass batcher as a nicloud option and
	...
	type testRequest struct {
		name        string
		makeRequest func([]*testTemplate) nicloudsdk.TemplateInsightsRequest
		ignoreTimes bool
	}
	tests := []struct {
		name           string
			...
			requests: []testRequest{
				{
					...
					makeRequest: func(templates []*testTemplate) nicloudsdk.TemplateInsightsRequest {
						return nicloudsdk.TemplateInsightsRequest{
							StartTime: frozenWeekAgo,
							EndTime:   frozenWeekAgo.AddDate(0, 0, 7),
							Interval:  nicloudsdk.InsightsReportIntervalDay,
						}
					},
				},
				{
					...
					makeRequest: func(templates []*testTemplate) nicloudsdk.TemplateInsightsRequest {
						return nicloudsdk.TemplateInsightsRequest{
							...
							Interval:  nicloudsdk.InsightsReportIntervalWeek,
						}
					},
				},
				{
				...
				},
				{
					...
					makeRequest: func(templates []*testTemplate) nicloudsdk.TemplateInsightsRequest {
						return nicloudsdk.TemplateInsightsRequest{
							...
							EndTime:     frozenWeekAgo.AddDate(0, 0, 7),
							Interval:    nicloudsdk.InsightsReportIntervalWeek,
							Sections:    []nicloudsdk.TemplateInsightsSection{nicloudsdk.TemplateInsightsSectionReport},
						}
					},
				},
				{
					...
					makeRequest: func(templates []*testTemplate) nicloudsdk.TemplateInsightsRequest {
						return nicloudsdk.TemplateInsightsRequest{
							...
							EndTime:     frozenWeekAgo.AddDate(0, 0, 7),
							Interval:    nicloudsdk.InsightsReportIntervalWeek,
							Sections:    []nicloudsdk.TemplateInsightsSection{nicloudsdk.TemplateInsightsSectionIntervalReports},
						}
					},
				},
			},
			...
			requests: []testRequest{
				{
					...
					ignoreTimes: true,
					makeRequest: func(_ []*testTemplate) nicloudsdk.TemplateInsightsRequest {
						now := time.Now().UTC()
						return nicloudsdk.TemplateInsightsRequest{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, req := range tt.requests {
				t.Run(req.name, func(t *testing.T) {
					...
					defer f.Close()
					var want nicloudsdk.TemplateInsightsResponse
...
func TestUserActivityInsights_Golden(t *testing.T) {
	...
	prepare := func(t *testing.T, templates []*testTemplate, users []*testUser, testData map[*testWorkspace]testDataGen, disableStorage bool) (*nicloudsdk.Client, chan dbrollup.Event) {
		...
		for _, user := range users {
			...
			require.NoError(t, err)
			userClient := nicloudsdk.New(client.URL, nicloudsdk.WithSessionToken(token.Key))
		...
		for _, template := range templates {
			...
			for _, user := range users {
				for _, workspace := range user.workspaces {
					...
					createWorkspaces = append(createWorkspaces, func(templateID uuid.UUID) {
						// Create workspace using the users client.
						createdWorkspace := nicloudtest.CreateWorkspace(t, user.client, templateID)
						workspace.id = createdWorkspace.ID
						waitWorkspaces = append(waitWorkspaces, func() {
							nicloudtest.AwaitWorkspaceBuildJobCompleted(t, user.client, createdWorkspace.LatestBuild.ID)
							ctx := testutil.Context(t, testutil.WaitShort)
	...
	type testRequest struct {
		name        string
		makeRequest func([]*testTemplate) nicloudsdk.UserActivityInsightsRequest
		ignoreTimes bool
	}
	tests := []struct {
		name           string
	...
	}{
		{
			...
			requests: []testRequest{
				{
					name: "week deployment wide",
					makeRequest: func(templates []*testTemplate) nicloudsdk.UserActivityInsightsRequest {
						return nicloudsdk.UserActivityInsightsRequest{
							StartTime: frozenWeekAgo,
							EndTime:   frozenWeekAgo.AddDate(0, 0, 7),
						}
					},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, req := range tt.requests {
				t.Run(req.name, func(t *testing.T) {
					...
					require.NoError(t, err, "open golden file, run \"make gen/golden-files\" and commit the changes")
					defer f.Close()
					var want nicloudsdk.UserActivityInsightsResponse
					err = json.NewDecoder(f).Decode(&want)
					require.NoError(t, err, "want no error decoding golden file")
...
func TestTemplateInsights_BadRequest(t *testing.T) {
	...
	_, err := client.TemplateInsights(ctx, nicloudsdk.TemplateInsightsRequest{
		StartTime: today,
		EndTime:   today.AddDate(0, 0, -1),
	})
	assert.Error(t, err, "want error for end time before start time")
	...
	_, err = client.TemplateInsights(ctx, nicloudsdk.TemplateInsightsRequest{
		...
		EndTime:   today,
		Interval:  nicloudsdk.InsightsReportIntervalWeek,
		Sections:  []nicloudsdk.TemplateInsightsSection{"invalid"},
	})
...
func TestTemplateInsights_RBAC(t *testing.T) {
	...
	tests := []test{
		{nicloudsdk.InsightsReportIntervalDay, true},
		{nicloudsdk.InsightsReportIntervalDay, false},
		{"", true},
		{"", false},
	}
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("with interval=%q", tt.interval), func(t *testing.T) {
			...
			t.Run("AsTemplateAdmin", func(t *testing.T) {
				...
				_, err := templateAdmin.TemplateInsights(ctx, nicloudsdk.TemplateInsightsRequest{
					StartTime:   today.AddDate(0, 0, -1),
					EndTime:     today,
					Interval:    tt.interval,
					TemplateIDs: templateIDs,
			...
			t.Run("AsRegularUser", func(t *testing.T) {
				...
				regular, _ := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
...
func TestGenericInsights_RBAC(t *testing.T) {
	...
	type fetchInsightsFunc func(ctx context.Context, client *nicloudsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error
	...
	endpoints := map[string]fetchInsightsFunc{
		"UserLatency": func(ctx context.Context, client *nicloudsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error {
			_, err := client.UserLatencyInsights(ctx, nicloudsdk.UserLatencyInsightsRequest{
				StartTime:   startTime,
				EndTime:     endTime,
				TemplateIDs: templateIDs,
		...
		"UserActivity": func(ctx context.Context, client *nicloudsdk.Client, startTime, endTime time.Time, templateIDs ...uuid.UUID) error {
			_, err := client.UserActivityInsights(ctx, nicloudsdk.UserActivityInsightsRequest{
				StartTime:   startTime,
				EndTime:     endTime,
				TemplateIDs: templateIDs,
...
func TestGenericInsights_Disabled(t *testing.T) {
	...
	client := nicloudtest.New(t, &nicloudtest.Options{
		...
		DeploymentValues: nicloudtest.DeploymentValues(t, func(dv *nicloudsdk.DeploymentValues) {
			dv.StatsCollection = nicloudsdk.StatsCollectionConfig{
				UsageStats: nicloudsdk.UsageStatsConfig{
					Enable: false,
				},
			}
		}),
	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(ctx context.Context) error {
				_, err := client.GetUserStatusCounts(ctx, nicloudsdk.GetUserStatusCountsRequest{
...
func TestGetUserStatusCounts(t *testing.T) {
	...
	type testCase struct {
		name          string
		request       nicloudsdk.GetUserStatusCountsRequest
		checkError    func(t *testing.T, err error)
		checkResponse func(t *testing.T, resp nicloudsdk.GetUserStatusCountsResponse)
	}

	happyResponseCheck := func(t *testing.T, resp nicloudsdk.GetUserStatusCountsResponse) {
		require.Len(t, resp.StatusCounts, 1)
		require.NotNil(t, resp.StatusCounts[nicloudsdk.UserStatusActive])
		require.Len(t, resp.StatusCounts[nicloudsdk.UserStatusActive], 61)
		...
		counts := resp.StatusCounts[nicloudsdk.UserStatusActive]
