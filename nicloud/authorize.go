...
// @Produce json
// @Tags Authorization
// @Param request body nicloudsdk.AuthorizationRequest true "Authorization request"
// @Success 200 {object} nicloudsdk.AuthorizationResponse
...
func (api *API) checkAuthorization(rw http.ResponseWriter, r *http.Request) {
	...
	var params nicloudsdk.AuthorizationRequest
		...
		if v.Object.ResourceID != "" {
			...
			if err != nil {
				httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
					...
					Validations: []nicloudsdk.ValidationError{{Field: "resource_id", Detail: err.Error()}},
				})
				return
			}
			...
			default:
				...
				httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
					Message:     msg,
					Validations: []nicloudsdk.ValidationError{{Field: "resource_type", Detail: msg}},
				})
				return
			}
			if dbErr != nil {
