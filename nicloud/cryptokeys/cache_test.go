...
func TestCryptoKeyCache(t *testing.T) {
	...
	t.Run("Signing", func(t *testing.T) {
		...
		t.Run("HitsCache", func(t *testing.T) {
			...
			now := clock.Now().UTC()
			expected := nicloudsdk.CryptoKey{
				Feature:  nicloudsdk.CryptoKeyFeatureTailnetResume,
				Secret:   generateKey(t, 64),
				Sequence: 2,
				StartsAt: now,
			}
			olderKey := nicloudsdk.CryptoKey{
				Feature:  nicloudsdk.CryptoKeyFeatureTailnetResume,
				Secret:   generateKey(t, 64),
				Sequence: 1,
				StartsAt: now,
			...
			ff := &fakeFetcher{
				keys: []nicloudsdk.CryptoKey{expected, olderKey},
			}

			cache, err := cryptokeys.NewSigningCache(ctx, logger, ff, nicloudsdk.CryptoKeyFeatureTailnetResume, cryptokeys.WithCacheClock(clock))
		...
		t.Run("MissesCache", func(t *testing.T) {
			...
			ff.keys = []nicloudsdk.CryptoKey{expected}
		...
		t.Run("KeyNotFound", func(t *testing.T) {
			...
			cache, err := cryptokeys.NewSigningCache(ctx, logger, ff, nicloudsdk.CryptoKeyFeatureTailnetResume)
			require.NoError(t, err)
	...
	t.Run("CacheRefreshes", func(t *testing.T) {
		...
		newKey := nicloudsdk.CryptoKey{
			Feature:  nicloudsdk.CryptoKeyFeatureTailnetResume,
			Secret:   generateKey(t, 64),
			Sequence: 13,
			StartsAt: now,
		}
		ff.keys = []nicloudsdk.CryptoKey{newKey}

		// The ticker should fire and cause a request to nicloud.
...
func (f *fakeFetcher) Fetch(_ context.Context, _ nicloudsdk.CryptoKeyFeature) ([]nicloudsdk.CryptoKey, error) {
	f.called++
	return f.keys, nil
}

func keyID(key nicloudsdk.CryptoKey) string {
	return strconv.FormatInt(int64(key.Sequence), 10)
}

func decodedSecret(t *testing.T, key nicloudsdk.CryptoKey) []byte {
	t.Helper()
