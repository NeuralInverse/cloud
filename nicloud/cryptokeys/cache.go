...
type Fetcher interface {
	Fetch(ctx context.Context, feature nicloudsdk.CryptoKeyFeature) ([]nicloudsdk.CryptoKey, error)
}
...
func (d *DBFetcher) Fetch(ctx context.Context, feature nicloudsdk.CryptoKeyFeature) ([]nicloudsdk.CryptoKey, error) {
...
type cache struct {
	...
	fetcher Fetcher
	logger  slog.Logger
	feature nicloudsdk.CryptoKeyFeature

	mu        sync.Mutex
	keys      map[int32]nicloudsdk.CryptoKey
	lastFetch time.Time
...
func NewSigningCache(ctx context.Context, logger slog.Logger, fetcher Fetcher,
	feature nicloudsdk.CryptoKeyFeature, opts ...func(*cache),
...
func newCache(ctx context.Context, logger slog.Logger, fetcher Fetcher, feature nicloudsdk.CryptoKeyFeature, opts ...func(*cache)) *cache {
	cache := &cache{
...
func isEncryptionKeyFeature(feature nicloudsdk.CryptoKeyFeature) bool {
	return feature == nicloudsdk.CryptoKeyFeatureWorkspaceAppsAPIKey
}

func isSigningKeyFeature(feature nicloudsdk.CryptoKeyFeature) bool {
	switch feature {
	case nicloudsdk.CryptoKeyFeatureTailnetResume, nicloudsdk.CryptoKeyFeatureOIDCConvert, nicloudsdk.CryptoKeyFeatureWorkspaceAppsToken:
		return true
	default:
		return false
	}
...
func idSecret(k nicloudsdk.CryptoKey) (string, []byte, error) {
...
func (c *cache) key(sequence int32) (nicloudsdk.CryptoKey, bool) {
...
func checkKey(key nicloudsdk.CryptoKey, sequence int32, now time.Time) (string, []byte, error) {
	if sequence == latestSequence {
		if !key.CanSign(now) {
...
func (c *cache) cryptoKeys(ctx context.Context) (map[int32]nicloudsdk.CryptoKey, error) {
...
func toKeyMap(keys []nicloudsdk.CryptoKey, now time.Time) map[int32]nicloudsdk.CryptoKey {
	m := make(map[int32]nicloudsdk.CryptoKey)
	var latest nicloudsdk.CryptoKey
...
func toSDKKeys(keys []database.CryptoKey) []nicloudsdk.CryptoKey {
	into := make([]nicloudsdk.CryptoKey, 0, len(keys))
	for _, key := range keys {
...
func toSDK(key database.CryptoKey) nicloudsdk.CryptoKey {
