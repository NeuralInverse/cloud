...
// @Success 200 {object} nicloudsdk.AuditLogResponse
...
func (api *API) auditLogs(rw http.ResponseWriter, r *http.Request) {
	...
	if count == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.AuditLogResponse{
			AuditLogs: []nicloudsdk.AuditLog{},
			Count:     0,
...
// @Accept json
// @Tags Audit
// @Param request body nicloudsdk.CreateTestAuditLogRequest true "Audit log request"
// @Success 204
...
func (api *API) generateFakeAuditLog(rw http.ResponseWriter, r *http.Request) {
	...
	diff, err := json.Marshal(nicloudsdk.AuditDiff{
		"foo": nicloudsdk.AuditDiffField{Old: "bar", New: "baz"},
	})
	if err != nil {
		httpapi.InternalServerError(rw, err)
		return
	...
	var params nicloudsdk.CreateTestAuditLogRequest
	if !httpapi.Read(ctx, rw, r, &params) {
		return
	}
	if params.Action == "" {
		params.Action = nicloudsdk.AuditActionWrite
	}
	if params.ResourceType == "" {
		params.ResourceType = nicloudsdk.ResourceTypeUser
	}
...
func (api *API) convertAuditLogs(ctx context.Context, dblogs []database.GetAuditLogsOffsetRow) []nicloudsdk.AuditLog {
	alogs := make([]nicloudsdk.AuditLog, 0, len(dblogs))
...
func (api *API) convertAuditLog(ctx context.Context, dblog database.GetAuditLogsOffsetRow) nicloudsdk.AuditLog {
	ip, _ := netip.AddrFromSlice(dblog.AuditLog.Ip.IPNet.IP)

	diff := nicloudsdk.AuditDiff{}
	...
	if dblog.AuditLog.OrganizationID != uuid.Nil {
		alog.Organization = &nicloudsdk.MinimalOrganization{
...
func auditLogDescription(alog database.GetAuditLogsOffsetRow) string {
	...
	case alog.AuditLog.StatusCode >= 400:
		...
		_, _ = b.WriteString(string(alog.AuditLog.Action))
	default:
		_, _ = b.WriteString(nicloudsdk.AuditAction(alog.AuditLog.Action).Friendly())
	}
	...
	if alog.AuditLog.ResourceType == database.ResourceTypeGitSshKey {
		...
		_, _ = b.WriteString(nicloudsdk.ResourceType(alog.AuditLog.ResourceType).FriendlyString())
		return b.String()
	}
