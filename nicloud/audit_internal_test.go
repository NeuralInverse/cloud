...
func TestAuditLogDescription(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "chat_archived",
			alog: chatAuditLogRow(t, nicloudsdk.AuditDiff{
		...
		},
		{
			...
			alog: func() database.GetAuditLogsOffsetRow {
				row := chatAuditLogRow(t, nicloudsdk.AuditDiff{
					"archived": {Old: false, New: true},
				})
...
func chatAuditLogRow(t *testing.T, diff nicloudsdk.AuditDiff) database.GetAuditLogsOffsetRow {
	t.Helper()
