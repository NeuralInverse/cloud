...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/taskname"
...
// @Param request body nicloudsdk.CreateTaskRequest true "Create task request"
// @Success 201 {object} nicloudsdk.Task
...
func (api *API) tasksCreate(rw http.ResponseWriter, r *http.Request) {
	...
	taskName := req.Name
	if taskName != "" {
		if err := nicloudsdk.NameValid(taskName); err != nil {
			httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	createReq := nicloudsdk.CreateWorkspaceRequest{
		Name:                    taskName,
	...
	workspace, err := createWorkspace(ctx, aReqWS, apiKey.UserID, api, owner, createReq, &createWorkspaceOptions{
		...
		preCreateInTX: func(ctx context.Context, tx database.Store) error {
			...
			if err != nil {
				return httperror.NewResponseError(http.StatusInternalServerError, nicloudsdk.Response{
...
// taskFromDBTaskAndWorkspace creates a nicloudsdk.Task response from the task
// database record and workspace.
func taskFromDBTaskAndWorkspace(dbTask database.Task, ws nicloudsdk.Workspace) nicloudsdk.Task {
	var taskAgentLifecycle *nicloudsdk.WorkspaceAgentLifecycle
	var taskAgentHealth *nicloudsdk.WorkspaceAgentHealth
	var taskAppHealth *nicloudsdk.WorkspaceAppHealth

	if dbTask.WorkspaceAgentLifecycleState.Valid {
		taskAgentLifecycle = ptr.Ref(nicloudsdk.WorkspaceAgentLifecycle(dbTask.WorkspaceAgentLifecycleState.WorkspaceAgentLifecycleState))
	}
	if dbTask.WorkspaceAppHealth.Valid {
		taskAppHealth = ptr.Ref(nicloudsdk.WorkspaceAppHealth(dbTask.WorkspaceAppHealth.WorkspaceAppHealth))
	}
	...
	return nicloudsdk.Task{
		...
		Status:                  nicloudsdk.TaskStatus(dbTask.Status),
...
func appStatusStateToTaskState(s nicloudsdk.WorkspaceAppStatusState) nicloudsdk.TaskState {
	switch s {
	case nicloudsdk.WorkspaceAppStatusStateFailure:
		return nicloudsdk.TaskStateFailed
	default:
		return nicloudsdk.TaskState(s)
	}
}
...
func deriveTaskCurrentState(
	...
	taskAgentLifecycle *nicloudsdk.WorkspaceAgentLifecycle,
	taskAppHealth *nicloudsdk.WorkspaceAppHealth,
) *nicloudsdk.TaskStateEntry {
	var currentState *nicloudsdk.TaskStateEntry
	...
	if ws.LatestAppStatus != nil {
		if ws.LatestBuild.Transition != nicloudsdk.WorkspaceTransitionStart || ws.LatestAppStatus.CreatedAt.After(ws.LatestBuild.CreatedAt) {
	...
	if currentState == nil && dbTask.Status == database.TaskStatusInitializing {
		...
		switch {
		case ws.LatestBuild.Status == nicloudsdk.WorkspaceStatusPending ||
			ws.LatestBuild.Status == nicloudsdk.WorkspaceStatusStarting:
			message = fmt.Sprintf("Workspace is %s", ws.LatestBuild.Status)
		case taskAgentLifecycle != nil:
			switch {
			case *taskAgentLifecycle == nicloudsdk.WorkspaceAgentLifecycleCreated:
				message = "Agent is connecting"
			case *taskAgentLifecycle == nicloudsdk.WorkspaceAgentLifecycleStarting:
				message = "Agent is starting"
			case *taskAgentLifecycle == nicloudsdk.WorkspaceAgentLifecycleReady:
				if taskAppHealth != nil && *taskAppHealth == nicloudsdk.WorkspaceAppHealthInitializing {
...
// @Success 200 {object} nicloudsdk.TasksListResponse
...
func (api *API) tasksList(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.TasksListResponse{
		Tasks: tasks,
		Count: len(tasks),
	})
}
...
func (api *API) convertTasks(ctx context.Context, requesterID uuid.UUID, dbTasks []database.Task) ([]nicloudsdk.Task, error) {
	if len(dbTasks) == 0 {
		return []nicloudsdk.Task{}, nil
	}
	...
	workspacesByID := make(map[uuid.UUID]nicloudsdk.Workspace)
	for _, ws := range apiWorkspaces {
		workspacesByID[ws.ID] = ws
	}

	// Convert tasks to SDK format.
	result := make([]nicloudsdk.Task, 0, len(dbTasks))
...
// @Success 200 {object} nicloudsdk.Task
...
func (api *API) taskDelete(rw http.ResponseWriter, r *http.Request) {
	...
	if task.WorkspaceID.Valid {
		...
		// initiate deletion.
		buildReq := nicloudsdk.CreateWorkspaceBuildRequest{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Param request body nicloudsdk.UpdateTaskInputRequest true "Update task input request"
// @Success 204
...
func (api *API) taskUpdateInput(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateTaskInputRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	if err := api.Database.InTx(func(tx database.Store) error {
		...
		if task.Status != database.TaskStatusPaused {
			return httperror.NewResponseError(http.StatusConflict, nicloudsdk.Response{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Param request body nicloudsdk.TaskSendRequest true "Task input request"
// @Success 204
...
func (api *API) taskSend(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.TaskSendRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	if req.Input == "" {
	...
	if err := api.authAndDoWithTaskAppClient(r, task, func(ctx context.Context, client *http.Client, appURL *url.URL) error {
		...
		if err != nil {
			return httperror.NewResponseError(http.StatusBadGateway, nicloudsdk.Response{
...
// TaskLogEntry format.
func convertAgentAPIMessagesToLogEntries(messages []agentapisdk.Message) ([]nicloudsdk.TaskLogEntry, error) {
	logs := make([]nicloudsdk.TaskLogEntry, 0, len(messages))
	for _, m := range messages {
		var typ nicloudsdk.TaskLogType
		switch m.Role {
		case agentapisdk.RoleUser:
			typ = nicloudsdk.TaskLogTypeInput
		case agentapisdk.RoleAgent:
			typ = nicloudsdk.TaskLogTypeOutput
		default:
			return nil, xerrors.Errorf("invalid agentapi message role %q", m.Role)
		}
		logs = append(logs, nicloudsdk.TaskLogEntry{
			ID:      int(m.Id),
			Content: m.Content,
			Type:    typ,
			Time:    m.Time,
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID, or task name"
// @Success 200 {object} nicloudsdk.TaskLogsResponse
...
func (api *API) fetchLiveTaskLogs(r *http.Request, task database.Task) (nicloudsdk.TaskLogsResponse, error) {
	var out nicloudsdk.TaskLogsResponse
	err := api.authAndDoWithTaskAppClient(r, task, func(ctx context.Context, client *http.Client, appURL *url.URL) error {
		...
		out = nicloudsdk.TaskLogsResponse{
			Logs: logs,
		}
		return nil
	})
...
func (api *API) fetchSnapshotTaskLogs(ctx context.Context, taskID uuid.UUID) (nicloudsdk.TaskLogsResponse, error) {
	snapshot, err := api.Database.GetTaskSnapshot(ctx, taskID)
	if err != nil {
		if httpapi.IsUnauthorizedError(err) {
			return nicloudsdk.TaskLogsResponse{}, httperror.NewResponseError(http.StatusNotFound, nicloudsdk.Response{
		...
		if errors.Is(err, sql.ErrNoRows) {
			...
			return nicloudsdk.TaskLogsResponse{
				Logs:     []nicloudsdk.TaskLogEntry{},
				Snapshot: true,
			}, nil
		}
		return nicloudsdk.TaskLogsResponse{}, httperror.NewResponseError(http.StatusInternalServerError, nicloudsdk.Response{
...
func (api *API) authAndDoWithTaskAppClient(
	...
	if task.Status != database.TaskStatusActive {
		...
		default:
			...
			return httperror.NewResponseError(http.StatusBadRequest, nicloudsdk.Response{
				Message: "Task must be active.",
				Detail:  fmt.Sprintf("Task status is %q, it must be %q to interact with the task.", task.Status, nicloudsdk.TaskStatusActive),
			})
		}
	}
	if !task.WorkspaceID.Valid {
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID" format(uuid)
// @Success 202 {object} nicloudsdk.PauseTaskResponse
...
func (api *API) pauseTask(rw http.ResponseWriter, r *http.Request) {
	...
	buildReq := nicloudsdk.CreateWorkspaceBuildRequest{
		...
		Reason:     nicloudsdk.CreateWorkspaceBuildReasonTaskManualPause,
	}
	...
	httpapi.Write(ctx, rw, http.StatusAccepted, nicloudsdk.PauseTaskResponse{
...
// @Param user path string true "Username, user ID, or 'me' for the authenticated user"
// @Param task path string true "Task ID" format(uuid)
// @Success 202 {object} nicloudsdk.ResumeTaskResponse
...
func (api *API) resumeTask(rw http.ResponseWriter, r *http.Request) {
	...
	workspaceStatus := nicloudsdk.ConvertWorkspaceStatus(
		nicloudsdk.ProvisionerJobStatus(job.JobStatus),
		nicloudsdk.WorkspaceTransition(latestBuild.Transition),
	)
	if workspaceStatus == nicloudsdk.WorkspaceStatusRunning {
		httpapi.Write(ctx, rw, http.StatusConflict, nicloudsdk.Response{
	...
	buildReq := nicloudsdk.CreateWorkspaceBuildRequest{
		...
		Reason:     nicloudsdk.CreateWorkspaceBuildReasonTaskResume,
	}
	build, err := api.postWorkspaceBuildsInternal(
		ctx,
		apiKey,
	...
	httpapi.Write(ctx, rw, http.StatusAccepted, nicloudsdk.ResumeTaskResponse{
		WorkspaceBuild: &build,
	})
}
