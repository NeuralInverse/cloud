...
func New() *Set {
	s := &Set{
		...
		// These will be updated when nicloud is initialized.
		entitlements: nicloudsdk.Entitlements{
	...
	for _, featureName := range nicloudsdk.FeatureNames {
		s.entitlements.AddFeature(featureName, nicloudsdk.Feature{
...
var ErrLicenseRequiresTelemetry = xerrors.New(nicloudsdk.LicenseTelemetryRequiredErrorText)

func (l *Set) Update(ctx context.Context, fetch func(context.Context) (nicloudsdk.Entitlements, error)) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-l.right2Update:
	...
	if xerrors.Is(err, ErrLicenseRequiresTelemetry) {
		...
		l.Modify(func(entitlements *nicloudsdk.Entitlements) {
...
func (l *Set) Feature(name nicloudsdk.FeatureName) (nicloudsdk.Feature, bool) {
	l.entitlementsMu.RLock()
	defer l.entitlementsMu.RUnlock()
...
func (l *Set) Enabled(feature nicloudsdk.FeatureName) bool {
	l.entitlementsMu.RLock()
	defer l.entitlementsMu.RUnlock()
...
func (l *Set) Modify(do func(entitlements *nicloudsdk.Entitlements)) {
	l.entitlementsMu.Lock()
	defer l.entitlementsMu.Unlock()
...
func (l *Set) FeatureChanged(featureName nicloudsdk.FeatureName, newFeature nicloudsdk.Feature) (initial, changed, enabled bool) {
	l.entitlementsMu.RLock()
	defer l.entitlementsMu.RUnlock()
...
func (l *Set) WriteEntitlementWarningHeaders(header http.Header) {
	...
	for _, warning := range l.entitlements.Warnings {
		header.Add(nicloudsdk.EntitlementsWarningHeader, warning)
	}
}
