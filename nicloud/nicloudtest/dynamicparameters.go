...
func DynamicParameterTemplate(t *testing.T, client *nicloudsdk.Client, org uuid.UUID, args DynamicParameterTemplateParams) (nicloudsdk.Template, nicloudsdk.TemplateVersion) {
	t.Helper()
	...
	userVars := make([]nicloudsdk.VariableValue, 0, len(args.Variables))
	...
	for _, argv := range args.Variables {
		...
		userVars = append(userVars, nicloudsdk.VariableValue{
			Name:  argv.Name,
			Value: argv.Value,
		})
	}
	...
	mime := nicloudsdk.ContentTypeTar
	if args.Zip {
		mime = nicloudsdk.ContentTypeZip
	}
	version := CreateTemplateVersionMimeType(t, client, mime, org, files, func(request *nicloudsdk.CreateTemplateVersionRequest) {
	...
	var tpl nicloudsdk.Template
	var err error

	if args.TemplateID == uuid.Nil {
		tpl = CreateTemplate(t, client, org, version.ID, func(request *nicloudsdk.CreateTemplateRequest) {
			request.UseClassicParameterFlow = ptr.Ref(false)
		})
	} else {
		tpl, err = client.UpdateTemplateMeta(t.Context(), args.TemplateID, nicloudsdk.UpdateTemplateMeta{
			UseClassicParameterFlow: ptr.Ref(false),
		})
		require.NoError(t, err)
	}

	err = client.UpdateActiveTemplateVersion(t.Context(), tpl.ID, nicloudsdk.UpdateActiveTemplateVersion{
		ID: version.ID,
	})
	require.NoError(t, err)
...
type ParameterAsserter struct {
	Name   string
	Params []nicloudsdk.PreviewParameter
	t      *testing.T
}

func AssertParameter(t *testing.T, name string, params []nicloudsdk.PreviewParameter) *ParameterAsserter {
...
func (a *ParameterAsserter) find(name string) *nicloudsdk.PreviewParameter {
	a.t.Helper()
...
func (a *ParameterAsserter) NotExists() *ParameterAsserter {
	...
	names := slice.Convert(a.Params, func(p nicloudsdk.PreviewParameter) string {
		return p.Name
	})
...
func (a *ParameterAsserter) Options(expected ...string) *ParameterAsserter {
	...
	optValues := slice.Convert(p.Options, func(p nicloudsdk.PreviewParameterOption) string {
