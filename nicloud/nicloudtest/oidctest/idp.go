...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/util/syncmap"
...
type FakeIDP struct {
	...
	// callbackPath allows changing where the callback path to nicloud is expected.
	// This only affects using the Login helper functions.
	callbackPath string
	// clientID to be used by nicloud
	clientID     string
	clientSecret string
	pkce         bool // TODO(Emyrk): Implement for refresh token flow as well
	// externalProviderID is optional to match the provider in nicloud for
	// redirectURLs.
...
func NewFakeIDP(t testing.TB, opts ...FakeIDPOpt) *FakeIDP {
	...
	if idp.locked.Issuer() == "" {
		idp.locked.SetIssuer("https://cloud.neuralinverse.com")
	}
...
// The client argument is just to get the URL of the Neural Inverse Cloud instance.
//
// The client passed in is just to get the url of the Neural Inverse Cloud instance.
// The actual client that is used is 100% unauthenticated and fresh.
func (f *FakeIDP) Login(t testing.TB, client *nicloudsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*nicloudsdk.Client, *http.Response) {
	t.Helper()
...
func (f *FakeIDP) AttemptLogin(t testing.TB, client *nicloudsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*nicloudsdk.Client, *http.Response) {
	t.Helper()
	var err error
	...
	unauthenticated := nicloudsdk.New(client.URL)
...
func (f *FakeIDP) LoginWithClient(t testing.TB, client *nicloudsdk.Client, idTokenClaims jwt.MapClaims, opts ...func(r *http.Request)) (*nicloudsdk.Client, *http.Response) {
	t.Helper()
	...
	// If the neuralinverse session token exists, return the new authed client!
	var user *nicloudsdk.Client
	cookies := cli.Jar.Cookies(client.URL)
	for _, cookie := range cookies {
		if cookie.Name == nicloudsdk.SessionTokenCookie {
			user = nicloudsdk.New(client.URL)
...
// an authenticated neuralinverse client.
func (f *FakeIDP) ExternalLogin(t testing.TB, client *nicloudsdk.Client, opts ...nicloudsdk.RequestOption) {
	...
	opts = append([]nicloudsdk.RequestOption{client.SessionTokenProvider.AsRequestOption()}, opts...)
	if cli.Jar == nil {
		cli.Jar, err = cookiejar.New(nil)
		require.NoError(t, err, "failed to create cookie jar")
	}
...
func (*FakeIDP) DeviceLogin(t testing.TB, client *nicloudsdk.Client, externalAuthID string) {
	// First we need to initiate the device flow. This will have Neural Inverse Cloud hit the
	...
	err = client.ExternalAuthDeviceExchange(context.Background(), externalAuthID, nicloudsdk.ExternalAuthDeviceExchange{
...
// OIDCCallback will emulate the IDP redirecting back to the Neural Inverse Cloud callback.
// This is helpful if no Neural Inverse Cloudd exists because the IDP needs to redirect to
// something.
// Essentially this is used to fake the Neural Inverse Cloudd side of the exchange.
...
// SetNeural Inverse ClouddCallback is optional and only works if not using the IsServing.
// It will setup a fake "Neural Inverse Cloudd" for the IDP to call when the IDP redirects
...
func (f *FakeIDP) OIDCConfigSkipIssuerChecks(t testing.TB, scopes []string, opts ...func(cfg *nicloud.OIDCConfig)) *nicloud.OIDCConfig {
...
func (f *FakeIDP) OIDCConfig(t testing.TB, scopes []string, opts ...func(cfg *nicloud.OIDCConfig)) *nicloud.OIDCConfig {
	return f.internalOIDCConfig(context.Background(), t, scopes, nil, opts...)
}

// OIDCConfig returns the OIDC config to use for Neural Inverse Cloudd.
func (f *FakeIDP) internalOIDCConfig(ctx context.Context, t testing.TB, scopes []string, verifierOpt func(config *oidc.Config), opts ...func(cfg *nicloud.OIDCConfig)) *nicloud.OIDCConfig {
	t.Helper()
	...
	cfg := &nicloud.OIDCConfig{
		...
		SecondaryClaims: nicloud.MergedClaimsSourceUserInfo,
	}
