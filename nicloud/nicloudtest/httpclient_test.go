...
func TestNewIsolatedHTTPClient(t *testing.T) {
	...
	client := nicloudtest.NewIsolatedHTTPClient(testutil.MustURL(t, "http://example.com"))
...
func TestNewIsolatedHTTPSClient(t *testing.T) {
	...
	client := nicloudtest.NewIsolatedHTTPClient(testutil.MustURL(t, "https://example.com"))
	require.NotSame(t, http.DefaultTransport, client.Transport)
...
func TestNewIsolatedHTTPClientNilURL(t *testing.T) {
	...
	client := nicloudtest.NewIsolatedHTTPClient(nil)
	require.NotNil(t, client.Transport)
	require.NotSame(t, http.DefaultTransport, client.Transport)
...
func TestCreateAnotherUserHTTPClientDefaultTransport(t *testing.T) {
	...
	base := nicloudsdk.New(
		...
		nicloudsdk.WithHTTPClient(&http.Client{Timeout: time.Second}),
	)

	other, _ := nicloudtest.CreateAnotherUser(t, base, first.OrganizationID)
