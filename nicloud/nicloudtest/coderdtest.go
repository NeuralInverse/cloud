...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/agentapi/metadatabatcher"
	"github.com/NeuralInverse/cloud/v2/nicloud/audit"
	"github.com/NeuralInverse/cloud/v2/nicloud/autobuild"
	"github.com/NeuralInverse/cloud/v2/nicloud/awsidentity"
	"github.com/NeuralInverse/cloud/v2/nicloud/azureidentity"
	"github.com/NeuralInverse/cloud/v2/nicloud/connectionlog"
	"github.com/NeuralInverse/cloud/v2/nicloud/cryptokeys"
	"github.com/NeuralInverse/cloud/v2/nicloud/database"
	"github.com/NeuralInverse/cloud/v2/nicloud/database/dbauthz"
	"github.com/NeuralInverse/cloud/v2/nicloud/database/dbrollup"
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/usage"
...
type Options struct {
	...
	GithubOAuth2Config             *nicloud.GithubOAuth2Config
	...
	TrialGenerator                 func(ctx context.Context, body nicloudsdk.LicensorTrialRequest) error
	...
	DeploymentValues              *nicloudsdk.DeploymentValues

	// Set update check options to enable update check.
	UpdateCheckOptions *updatecheck.Options

	// Overriding the database is heavily discouraged.
	// It should only be used in cases where multiple Neural Inverse Cloud
...
// New constructs a nicloudsdk client connected to an in-memory API instance.
func New(t testing.TB, options *Options) *nicloudsdk.Client {
	client, _ := newWithCloser(t, options)
	return client
}

// NewWithDatabase constructs a nicloudsdk client connected to an in-memory API instance.
// The database is returned to provide direct data manipulation for tests.
func NewWithDatabase(t testing.TB, options *Options) (*nicloudsdk.Client, database.Store) {
...
// standardize how provisioners are registered with nicloud. The option
// to include a provisioner is set to true for convenience.
func NewWithProvisionerCloser(t testing.TB, options *Options) (*nicloudsdk.Client, io.Closer) {
	if options == nil {
		options = &Options{}
	}
...
// newWithCloser constructs a nicloudsdk client connected to an in-memory API instance.
// The returned closer closes a provisioner if it was provided
// The API is intentionally not returned here because nicloud tests should not
...
// out.
func newWithCloser(t testing.TB, options *Options) (*nicloudsdk.Client, io.Closer) {
	client, closer, _ := NewWithAPI(t, options)
	return client, closer
}

func NewOptions(t testing.TB, options *Options) (func(http.Handler), context.CancelFunc, *url.URL, *nicloud.Options) {
	t.Helper()
	...
	if options.Logger == nil {
		logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug).Named("nicloud")
	...
	ctx, cancelFunc := context.WithCancel(context.Background())
	experiments := nicloud.ReadExperiments(*options.Logger, options.DeploymentValues.Experiments)
	...
	t.Logf("nicloudtest server listening on %s", srv.Listener.Addr().String())
	...
	t.Logf("nicloudtest server started on %s", srv.URL)
	t.Cleanup(func() {
		t.Logf("closing nicloudtest server on %s", srv.Listener.Addr().String())
		srv.Close()
		t.Logf("closed nicloudtest server on %s", srv.Listener.Addr().String())
	})
	...
	// Technically AGPL nicloud servers don't set this value, but it doesn't
	...
	return func(h http.Handler) {
			...
			defer mutex.Unlock()
			handler = h
		}, cancelFunc, serverURL, &nicloud.Options{
			...
			DeploymentOptions:                  nicloudsdk.DeploymentOptionsWithoutSecrets(options.DeploymentValues.Options()),
...
func NewWithAPI(t testing.TB, options *Options) (*nicloudsdk.Client, io.Closer, *nicloud.API) {
	if options == nil {
		options = &Options{}
	}
	...
	if options.IncludeProvisionerDaemon {
		provisionerCloser = NewTaggedProvisionerDaemon(t, niAPI, defaultTestDaemonName, options.ProvisionerDaemonTags, nicloud.MemoryProvisionerWithVersionOverride(options.ProvisionerDaemonVersion))
	}
	client := nicloudsdk.New(serverURL, nicloudsdk.WithHTTPClient(NewIsolatedHTTPClient(serverURL)))
	t.Cleanup(func() {
		cancelFunc()
	...
	return client, provisionerCloser, niAPI
}
...
func NewIsolatedHTTPClient(serverURL *url.URL) *http.Client {
	...
	//nolint:gosec // The nicloudtest server uses test-only TLS certificates.
...
// well with nicloud testing. It registers the "echo" provisioner for
// quick testing.
func NewProvisionerDaemon(t testing.TB, niAPI *nicloud.API) io.Closer {
	return NewTaggedProvisionerDaemon(t, niAPI, defaultTestDaemonName, nil)
}

func NewTaggedProvisionerDaemon(t testing.TB, niAPI *nicloud.API, name string, provisionerTags map[string]string, opts ...nicloud.MemoryProvisionerDaemonOption) io.Closer {
	t.Helper()
	...
	go func() {
		err := echo.Serve(ctx, &provisionersdk.ServeOptions{
			...
			Logger:        niAPI.Logger.Named("echo").Leveled(slog.LevelDebug),
		})
	...
	daemon := provisionerd.New(func(dialCtx context.Context) (provisionerdproto.DRPCProvisionerDaemonClient, error) {
		return niAPI.CreateInMemoryTaggedProvisionerDaemon(dialCtx, name, []nicloudsdk.ProvisionerType{nicloudsdk.ProvisionerTypeEcho}, provisionerTags, opts...)
	}, &provisionerd.Options{
		Logger:              niAPI.Logger.Named("provisionerd").Leveled(slog.LevelDebug),
...
var FirstUserParams = nicloudsdk.CreateFirstUserRequest{
...
var TrialUserParams = nicloudsdk.CreateFirstUserTrialInfo{
	FirstName:   "John",
	LastName:    "Doe",
...
// with the passed in nicloudsdk client.
func CreateFirstUser(t testing.TB, client *nicloudsdk.Client) nicloudsdk.CreateFirstUserResponse {
	resp, err := client.CreateFirstUser(context.Background(), FirstUserParams)
	require.NoError(t, err)

	login, err := client.LoginWithPassword(context.Background(), nicloudsdk.LoginWithPasswordRequest{
...
func CreateAnotherUser(t testing.TB, client *nicloudsdk.Client, organizationID uuid.UUID, roles ...rbac.RoleIdentifier) (*nicloudsdk.Client, nicloudsdk.User) {
	return createAnotherUserRetry(t, client, []uuid.UUID{organizationID}, 5, roles)
}

func CreateAnotherUserMutators(t testing.TB, client *nicloudsdk.Client, organizationID uuid.UUID, roles []rbac.RoleIdentifier, mutators ...func(r *nicloudsdk.CreateUserRequestWithOrgs)) (*nicloudsdk.Client, nicloudsdk.User) {
...
func AuthzUserSubject(user nicloudsdk.User) rbac.Subject {
...
func AuthzUserSubjectWithDB(ctx context.Context, t testing.TB, db database.Store, user nicloudsdk.User) rbac.Subject {
	t.Helper()
...
func createAnotherUserRetry(t testing.TB, client *nicloudsdk.Client, organizationIDs []uuid.UUID, retries int, roles []rbac.RoleIdentifier, mutators ...func(r *nicloudsdk.CreateUserRequestWithOrgs)) (*nicloudsdk.Client, nicloudsdk.User) {
	req := nicloudsdk.CreateUserRequestWithOrgs{
		Email:           namesgenerator.UniqueName() + "@cloud.neuralinverse.com",
	...
	if req.ServiceAccount {
		req.Password = ""
		req.Email = ""
		req.UserLoginType = nicloudsdk.LoginTypeNone
	}
	...
	case nicloudsdk.LoginTypeNone, nicloudsdk.LoginTypeGithub, nicloudsdk.LoginTypeOIDC:
		// Cannot log in with a non-password user. So make it an api key from the
		// client making this user.
		token, err := client.CreateToken(context.Background(), user.ID.String(), nicloudsdk.CreateTokenRequest{
	...
	if user.Status == nicloudsdk.UserStatusDormant {
	...
	other := nicloudsdk.New(
		client.URL,
		nicloudsdk.WithSessionToken(sessionToken),
		nicloudsdk.WithHTTPClient(newHTTPClientWithTransportFrom(client.HTTPClient)),
	)
	...
	if len(roles) > 0 {
		...
		user, err = client.UpdateUserRoles(context.Background(), user.ID.String(), nicloudsdk.UpdateRoles{Roles: slice.List(siteRoles, onlyName)})
		...
		for orgID, roles := range orgRoles {
			...
			_, err = client.UpdateOrganizationMemberRoles(context.Background(), orgID, user.ID.String(),
				nicloudsdk.UpdateRoles{Roles: slice.List(roles, onlyName)})
...
func CreateTemplateVersionMimeType(t testing.TB, client *nicloudsdk.Client, mimeType string, organizationID uuid.UUID, res *echo.Responses, mutators ...func(*nicloudsdk.CreateTemplateVersionRequest)) nicloudsdk.TemplateVersion {
	t.Helper()
	data, err := echo.TarWithOptions(context.Background(), client.Logger(), res)
	require.NoError(t, err)

	switch mimeType {
	case nicloudsdk.ContentTypeTar:
		// do nothing
	case nicloudsdk.ContentTypeZip:
...
// with testing.
func CreateTemplateVersion(t testing.TB, client *nicloudsdk.Client, organizationID uuid.UUID, res *echo.Responses, mutators ...func(*nicloudsdk.CreateTemplateVersionRequest)) nicloudsdk.TemplateVersion {
	return CreateTemplateVersionMimeType(t, client, nicloudsdk.ContentTypeTar, organizationID, res, mutators...)
}
...
func CreateWorkspaceBuild(
	...
	mutators ...func(*nicloudsdk.CreateWorkspaceBuildRequest),
) nicloudsdk.WorkspaceBuild {
	t.Helper()

	req := nicloudsdk.CreateWorkspaceBuildRequest{
		Transition: nicloudsdk.WorkspaceTransition(transition),
	}
	for _, mut := range mutators {
		mut(&req)
	}
...
func CreateTemplate(t testing.TB, client *nicloudsdk.Client, organization uuid.UUID, version uuid.UUID, mutators ...func(*nicloudsdk.CreateTemplateRequest)) nicloudsdk.Template {
	req := nicloudsdk.CreateTemplateRequest{
...
func CreateGroup(t testing.TB, client *nicloudsdk.Client, organizationID uuid.UUID, name string, members ...nicloudsdk.User) nicloudsdk.Group {
	t.Helper()
	group, err := client.CreateGroup(context.Background(), organizationID, nicloudsdk.CreateGroupRequest{
		Name: name,
	})
	...
	group, err = client.PatchGroup(context.Background(), group.ID, nicloudsdk.PatchGroupRequest{
		AddUsers: memberIDs,
	})
...
func UpdateTemplateVersion(t testing.TB, client *nicloudsdk.Client, organizationID uuid.UUID, res *echo.Responses, templateID uuid.UUID) nicloudsdk.TemplateVersion {
	ctx := context.Background()
	data, err := echo.Tar(res)
	require.NoError(t, err)
	file, err := client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(data))
	require.NoError(t, err)
	templateVersion, err := client.CreateTemplateVersion(ctx, organizationID, nicloudsdk.CreateTemplateVersionRequest{
...
func UpdateActiveTemplateVersion(t testing.TB, client *nicloudsdk.Client, templateID, versionID uuid.UUID) {
	err := client.UpdateActiveTemplateVersion(context.Background(), templateID, nicloudsdk.UpdateActiveTemplateVersion{
		ID: versionID,
	})
	require.NoError(t, err)
}
...
func UpdateTemplateMeta(t testing.TB, client *nicloudsdk.Client, templateID uuid.UUID, meta nicloudsdk.UpdateTemplateMeta) nicloudsdk.Template {
	t.Helper()
...
func AwaitTemplateVersionJobRunning(t testing.TB, client *nicloudsdk.Client, version uuid.UUID) nicloudsdk.TemplateVersion {
	t.Helper()
	...
	require.Eventually(t, func() bool {
		...
		case nicloudsdk.ProvisionerJobPending:
			return false
...
func AwaitTemplateVersionJobCompleted(t testing.TB, client *nicloudsdk.Client, version uuid.UUID) nicloudsdk.TemplateVersion {
	t.Helper()
...
func AwaitWorkspaceBuildJobCompleted(t testing.TB, client *nicloudsdk.Client, build uuid.UUID) nicloudsdk.WorkspaceBuild {
	t.Helper()
	...
	var workspaceBuild nicloudsdk.WorkspaceBuild
	require.Eventually(t, func() bool {
		var err error
...
func AwaitWorkspaceAgents(t testing.TB, client *nicloudsdk.Client, workspaceID uuid.UUID, agentNames ...string) []nicloudsdk.WorkspaceResource {
...
type WorkspaceAgentWaiter struct {
	...
	agentNames       []string
	resourcesMatcher func([]nicloudsdk.WorkspaceResource) bool
	ctx              context.Context
}

// NewWorkspaceAgentWaiter returns an object that waits for agents to connect when
// you call Wait() on it.
func NewWorkspaceAgentWaiter(t testing.TB, client *nicloudsdk.Client, workspaceID uuid.UUID) WorkspaceAgentWaiter {
...
func RequireWorkspaceAgentByName(t testing.TB, resources []nicloudsdk.WorkspaceResource, name string) nicloudsdk.WorkspaceAgent {
	t.Helper()
...
func (w WorkspaceAgentWaiter) MatchResources(m func([]nicloudsdk.WorkspaceResource) bool) WorkspaceAgentWaiter {
	//nolint: revive // returns modified struct
...
//
// waiter := nicloudtest.NewWorkspaceAgentWaiter(t, client, r.Workspace.ID)
// waiter.WaitFor(nicloudtest.AgentsReady)
type WaitForAgentFn func(agent nicloudsdk.WorkspaceAgent) bool

// AgentsReady checks that the latest lifecycle state of an agent is "Ready".
func AgentsReady(agent nicloudsdk.WorkspaceAgent) bool {
	return agent.LifecycleState == nicloudsdk.WorkspaceAgentLifecycleReady
}

// AgentsNotReady checks that the latest lifecycle state of an agent is anything except "Ready".
func AgentsNotReady(agent nicloudsdk.WorkspaceAgent) bool {
...
// waiter's context is canceled.
func (w WorkspaceAgentWaiter) Wait() []nicloudsdk.WorkspaceResource {
	w.t.Helper()
	...
	w.t.Logf("waiting for workspace agents (workspace %s)", w.workspaceID)
	var resources []nicloudsdk.WorkspaceResource
	testutil.Eventually(ctx, w.t, func(ctx context.Context) bool {
		var err error
		workspace, err := w.client.Workspace(ctx, w.workspaceID)
		if err != nil {
...
func CreateWorkspace(t testing.TB, client *nicloudsdk.Client, templateID uuid.UUID, mutators ...func(*nicloudsdk.CreateWorkspaceRequest)) nicloudsdk.Workspace {
	t.Helper()
	...
	workspace, err := client.CreateUserWorkspace(context.Background(), nicloudsdk.Me, req)
	require.NoError(t, err)
	return workspace
}

// TransitionWorkspace is a convenience method for transitioning a workspace from one state to another.
func MustTransitionWorkspace(t testing.TB, client *nicloudsdk.Client, workspaceID uuid.UUID, from, to nicloudsdk.WorkspaceTransition, muts ...func(req *nicloudsdk.CreateWorkspaceBuildRequest)) nicloudsdk.Workspace {
	t.Helper()
	ctx := context.Background()
...
func MustWorkspace(t testing.TB, client *nicloudsdk.Client, workspaceID uuid.UUID) nicloudsdk.Workspace {
	t.Helper()
	ctx := context.Background()
...
func RequestExternalAuthCallback(t testing.TB, providerID string, client *nicloudsdk.Client, opts ...func(*http.Request)) *http.Response {
...
func SDKError(t testing.TB, err error) *nicloudsdk.Error {
	var cerr *nicloudsdk.Error
	require.True(t, errors.As(err, &cerr), "should be SDK error, got %s", err)
	return cerr
}

func DeploymentValues(t testing.TB, mut ...func(*nicloudsdk.DeploymentValues)) *nicloudsdk.DeploymentValues {
...
func MustWaitForProvisionersUnavailable(t *testing.T, db database.Store, workspace nicloudsdk.Workspace, tags map[string]string, checkTime time.Time) {
	t.Helper()
...
func MustWaitForProvisionersAvailable(t *testing.T, db database.Store, workspace nicloudsdk.Workspace, ts time.Time) uuid.UUID {
	t.Helper()
