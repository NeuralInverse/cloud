...
func TestTasks(t *testing.T) {
	...
	createAITemplate := func(t *testing.T, client *nicloudsdk.Client, user nicloudsdk.CreateFirstUserResponse, opts ...aiTemplateOpt) nicloudsdk.Template {
		t.Helper()
	...
	t.Run("List", func(t *testing.T) {
		...
		tasks, err := client.Tasks(ctx, &nicloudsdk.TasksFilter{Owner: nicloudsdk.Me})
		require.NoError(t, err)

		got, ok := slice.Find(tasks, func(t nicloudsdk.Task) bool { return t.ID == task.ID })
	...
	t.Run("Get", func(t *testing.T) {
		...
		var (
			...
			ctx            = testutil.Context(t, testutil.WaitLong)
			user           = nicloudtest.CreateFirstUser(t, client)
			anotherUser, _ = nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
			template       = createAITemplate(t, client, user)
		...
		ws = nicloudtest.MustWorkspace(t, client, task.WorkspaceID.UUID)
		...
		byName, err := client.TaskByOwnerAndName(ctx, nicloudsdk.Me, task.Name)
		require.NoError(t, err)
		...
		// Stop the workspace
		nicloudtest.MustTransitionWorkspace(t, client, task.WorkspaceID.UUID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
		...
		assert.Equal(t, nicloudsdk.TaskStateComplete, updated.CurrentState.State)
		previousCurrentState := updated.CurrentState

		// Start the workspace again
		nicloudtest.MustTransitionWorkspace(t, client, task.WorkspaceID.UUID, nicloudsdk.WorkspaceTransitionStop, nicloudsdk.WorkspaceTransitionStart)
		...
		assert.Equal(t, nicloudsdk.TaskStateWorking, updated.CurrentState.State)
	...
	t.Run("Delete", func(t *testing.T) {
		...
		t.Run("OK", func(t *testing.T) {
			...
			testutil.Eventually(ctx, t, func(ctx context.Context) (done bool) {
				...
				return dws.LatestBuild.Status == nicloudsdk.WorkspaceStatusDeleted
		...
		t.Run("SnapshotCleanupOnDeletion", func(t *testing.T) {
			...
			require.NoError(t, err)
			userSubject := nicloudtest.AuthzUserSubject(userObj)
		...
		t.Run("PreservesOtherTaskSnapshots", func(t *testing.T) {
			...
			// Create task A.
			taskA, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "task A",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, wsA.LatestBuild.ID)

			// Create task B.
			taskB, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "task B",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, wsB.LatestBuild.ID)
		...
		t.Run("DeletingTaskWorkspaceDeletesTask", func(t *testing.T) {
			...
			nicloudtest.MustTransitionWorkspace(t, client, ws.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionDelete)
			...
			var sdkErr *nicloudsdk.Error
			require.ErrorAs(t, err, &sdkErr, "expected a nicloudsdk.Error")
			require.Equal(t, http.StatusNotFound, sdkErr.StatusCode())
		})
	})
	...
	t.Run("Send", func(t *testing.T) {
		...
		t.Run("IntegrationOK", func(t *testing.T) {
			...
			task, err := userClient.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
				Input:             "send me food",
			})
			require.NoError(t, err)
			...
			require.NoError(t, err)
			nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, ws.LatestBuild.ID)
			...
			nicloudtest.NewWorkspaceAgentWaiter(t, userClient, ws.ID).WaitFor(nicloudtest.AgentsReady)
			...
			err = client.TaskSend(ctx, "me", task.ID, nicloudsdk.TaskSendRequest{
				Input: "Hello, Agent!",
			})
		...
		t.Run("TaskNotFound", func(t *testing.T) {
			...
			err := client.TaskSend(ctx, "me", uuid.New(), nicloudsdk.TaskSendRequest{
				Input: "hi",
			})
		...
		t.Run("SendToNonActiveStates", func(t *testing.T) {
			...
			_, user := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
	...
	t.Run("Logs", func(t *testing.T) {
		...
		nicloudtest.NewWorkspaceAgentWaiter(t, client, ws.ID).WaitFor(nicloudtest.AgentsReady)
		...
		t.Run("OK", func(t *testing.T) {
			...
			assert.Equal(t, nicloudsdk.TaskLogTypeOutput, resp.Logs[0].Type)
			assert.Equal(t, "Welcome, user!", resp.Logs[0].Content)

			assert.Equal(t, 1, resp.Logs[1].ID)
			assert.Equal(t, nicloudsdk.TaskLogTypeInput, resp.Logs[1].Type)
			assert.Equal(t, "Hello, agent!", resp.Logs[1].Content)

			assert.Equal(t, 2, resp.Logs[2].ID)
			assert.Equal(t, nicloudsdk.TaskLogTypeOutput, resp.Logs[2].Type)
	...
	t.Run("LogsWithSnapshot", func(t *testing.T) {
		...
		verifySnapshotLogs := func(t *testing.T, got nicloudsdk.TaskLogsResponse) {
			t.Helper()
			want := nicloudsdk.TaskLogsResponse{
				Snapshot:   true,
				SnapshotAt: &snapshotTime,
				Logs: []nicloudsdk.TaskLogEntry{
					{
						ID:      0,
						Type:    nicloudsdk.TaskLogTypeOutput,
						Content: "First message",
						Time:    snapshotMessages[0].Time,
					},
					{
						ID:      1,
						Type:    nicloudsdk.TaskLogTypeInput,
						Content: "Second message",
		...
		t.Run("InvalidSnapshotFormat", func(t *testing.T) {
			...
			invalidEnvelope := nicloud.TaskLogSnapshotEnvelope{
	...
	t.Run("UpdateInput", func(t *testing.T) {
		tests := []struct {
			...
			deleteTask         bool
			taskInput          string
			wantStatus         nicloudsdk.TaskStatus
			wantErr            string
		...
		}{
			{
				...
				wantStatus:         nicloudsdk.TaskStatusPending,
			...
			},
			{
				...
				transition: database.WorkspaceTransitionStop,
				taskInput:  "Valid prompt",
				wantStatus: nicloudsdk.TaskStatusPaused,
			},
			{
				...
				taskInput:         "Valid prompt",
				wantStatus:        nicloudsdk.TaskStatusError,
				wantErr:           "Unable to update",
				wantErrStatusCode: http.StatusConflict,
			},
			{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				client, provisioner := nicloudtest.NewWithProvisionerCloser(t, &nicloudtest.Options{IncludeProvisionerDaemon: true})
				user := nicloudtest.CreateFirstUser(t, client)
				ctx := testutil.Context(t, testutil.WaitLong)
				...
				if !tt.disableProvisioner {
					...
					build := nicloudtest.CreateWorkspaceBuild(t, client, workspace, tt.transition)
					if tt.cancelTransition {
						...
						require.NoError(t, err)
						require.Equal(t, nicloudsdk.WorkspaceStatusCanceled, build.Status)
					} else {
						nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, build.ID)
					}
				}
				...
				if tt.deleteTask {
					err = client.DeleteTask(ctx, nicloudsdk.Me, task.ID)
					require.NoError(t, err)
				} else {
				...
				err = client.UpdateTaskInput(ctx, task.OwnerName, task.ID, nicloudsdk.UpdateTaskInputRequest{
					Input: tt.taskInput,
				})
				if tt.wantErr != "" {
		...
		t.Run("NonExistentTask", func(t *testing.T) {
			...
			err := client.UpdateTaskInput(ctx, user.UserID.String(), uuid.New(), nicloudsdk.UpdateTaskInputRequest{
				Input: "Should fail",
			})
			require.Error(t, err)
			var apiErr *nicloudsdk.Error
		...
		t.Run("UnauthorizedUser", func(t *testing.T) {
			...
			err = anotherUser.UpdateTaskInput(ctx, task.OwnerName, task.ID, nicloudsdk.UpdateTaskInputRequest{
...
func TestTasksCreate(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if tt.expectError == "" {
					...
					err = nicloudsdk.NameValid(task.Name)
	...
	t.Run("FailsOnNonTaskTemplate", func(t *testing.T) {
		...
		// When: We attempt to create a Task.
		_, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             taskPrompt,
		})
		...
		var sdkErr *nicloudsdk.Error
		require.Error(t, err)
		require.ErrorAsf(t, err, &sdkErr, "error should be of type *nicloudsdk.Error")
	...
	t.Run("MultipleTasksForSameUser", func(t *testing.T) {
		...
		task1, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "First task",
			Name:              "task-1",
		})
		...
		task2, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
			TemplateVersionID: template.ActiveVersionID,
			Input:             "Second task",
			Name:              "task-2",
		})
	...
	t.Run("TaskLinkedToCorrectTemplateVersion", func(t *testing.T) {
		...
		version1 := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:          echo.ParseComplete,
			ProvisionApply: echo.ApplyComplete,
			ProvisionGraph: []*proto.Response{
				{Type: &proto.Response_Graph{Graph: &proto.GraphComplete{
		...
		version2 := nicloudtest.UpdateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse:          echo.ParseComplete,
			ProvisionApply: echo.ApplyComplete,
			ProvisionGraph: []*proto.Response{
				{Type: &proto.Response_Graph{Graph: &proto.GraphComplete{
...
func TestTasksNotification(t *testing.T) {
	...
	for _, tc := range []struct {
		name                 string
		latestAppStatuses    []nicloudsdk.WorkspaceAppStatusState
		newAppStatus         nicloudsdk.WorkspaceAppStatusState
		isAITask             bool
		...
		// Should not send a notification when the agent app is not an AI task.
		{
			...
			newAppStatus:       nicloudsdk.WorkspaceAppStatusStateWorking,
			isAITask:           false,
		...
		// Should not send a notification when the new app status is neither 'Working' nor 'Idle'.
		{
			...
			latestAppStatuses:  nil,
			newAppStatus:       nicloudsdk.WorkspaceAppStatusStateComplete,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should not send a notification when the new app status equals the latest status (Working).
		{
			...
			latestAppStatuses:  []nicloudsdk.WorkspaceAppStatusState{nicloudsdk.WorkspaceAppStatusStateWorking},
			newAppStatus:       nicloudsdk.WorkspaceAppStatusStateWorking,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should send TemplateTaskIdle when the AI task's FIRST status is 'Idle' (task completed immediately).
		{
			...
			latestAppStatuses:    nil,
			newAppStatus:         nicloudsdk.WorkspaceAppStatusStateIdle,
			isAITask:             true,
		...
		// Should send TemplateTaskWorking when the AI task transitions to 'Working' from 'Idle'.
		{
			...
			latestAppStatuses: []nicloudsdk.WorkspaceAppStatusState{
				nicloudsdk.WorkspaceAppStatusStateWorking,
				nicloudsdk.WorkspaceAppStatusStateIdle,
			}, // latest
			newAppStatus:         nicloudsdk.WorkspaceAppStatusStateWorking,
			isAITask:             true,
			isNotificationSent:   true,
		...
		// Should send TemplateTaskFailed when the AI task transitions to 'Failure'.
		{
			...
			latestAppStatuses:    []nicloudsdk.WorkspaceAppStatusState{nicloudsdk.WorkspaceAppStatusStateWorking},
			newAppStatus:         nicloudsdk.WorkspaceAppStatusStateFailure,
			isAITask:             true,
			isNotificationSent:   true,
		...
		// Should send TemplateTaskCompleted when the AI task transitions from 'Idle' to 'Complete'.
		{
			...
			latestAppStatuses:    []nicloudsdk.WorkspaceAppStatusState{nicloudsdk.WorkspaceAppStatusStateIdle},
			newAppStatus:         nicloudsdk.WorkspaceAppStatusStateComplete,
			isAITask:             true,
			isNotificationSent:   true,
			notificationTemplate: notifications.TemplateTaskCompleted,
		...
		// Should NOT send notification when transitioning from 'Complete' to 'Complete' (no change).
		{
			...
			latestAppStatuses:  []nicloudsdk.WorkspaceAppStatusState{nicloudsdk.WorkspaceAppStatusStateComplete},
			newAppStatus:       nicloudsdk.WorkspaceAppStatusStateComplete,
			isAITask:           true,
			isNotificationSent: false,
		...
		// Should NOT send notification when transitioning from 'Failure' to 'Failure' (no change).
		{
			...
			latestAppStatuses:  []nicloudsdk.WorkspaceAppStatusState{nicloudsdk.WorkspaceAppStatusStateFailure},
			newAppStatus:       nicloudsdk.WorkspaceAppStatusStateFailure,
			isAITask:           true,
			isNotificationSent: false,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Given: a member user
			ownerUser := nicloudtest.CreateFirstUser(t, client)
			client, memberUser := nicloudtest.CreateAnotherUser(t, client, ownerUser.OrganizationID)
			...
			if tc.agentLifecycle != "" {
				workspace := nicloudtest.MustWorkspace(t, client, workspaceBuild.Workspace.ID)
...
func TestPostWorkspaceAgentTaskSnapshot(t *testing.T) {
	...
	// Shared nicloud with mock clock for all tests.
	clock := quartz.NewMock(t)
	ownerClient, db := nicloudtest.NewWithDatabase(t, &nicloudtest.Options{
		Clock: clock,
	})
	...
	makeRequest := func(t *testing.T, taskID uuid.UUID, agentToken string, payload []byte, format string) *http.Response {
		...
		require.NoError(t, err)
		req.Header.Set(nicloudsdk.SessionTokenHeader, agentToken)
	...
	t.Run("MissingFormat", func(t *testing.T) {
		...
		var errResp nicloudsdk.Response
...
func TestPauseTask(t *testing.T) {
	...
	setupClient := func(t *testing.T, db database.Store, ps pubsub.Pubsub, authorizer rbac.Authorizer) *nicloudsdk.Client {
		t.Helper()
	...
	setupWorkspaceTask := func(t *testing.T, db database.Store, user nicloudsdk.CreateFirstUserResponse) (database.Task, uuid.UUID) {
		t.Helper()
		workspaceBuild := dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
	...
	t.Run("OK", func(t *testing.T) {
		...
		resp, err := client.PauseTask(ctx, nicloudsdk.Me, task.ID)
		...
		require.Equal(t, string(nicloudsdk.CreateWorkspaceBuildReasonTaskManualPause), string(build.Reason))
	...
	t.Run("Non-owner role access", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				userClient, _ := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, tc.roles...)

				resp, err := userClient.PauseTask(ctx, nicloudsdk.Me, task.ID)
	...
	t.Run("Task not found", func(t *testing.T) {
		...
		_, err := client.PauseTask(ctx, nicloudsdk.Me, uuid.New())
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Task lookup forbidden", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		db, ps := dbtestutil.NewDB(t)
		auth := &nicloudtest.FakeAuthorizer{
		...
		_, err := client.PauseTask(ctx, nicloudsdk.Me, task.ID)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Job already in progress", func(t *testing.T) {
		...
		_, err := client.PauseTask(ctx, nicloudsdk.Me, workspaceBuild.Task.ID)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Notification", func(t *testing.T) {
		...
		var (
			notifyEnq       = &notificationstest.FakeEnqueuer{}
			ownerClient, db = nicloudtest.NewWithDatabase(t, &nicloudtest.Options{NotificationsEnqueuer: notifyEnq})
			owner           = nicloudtest.CreateFirstUser(t, ownerClient)
		)
		...
		createTask := createTaskInState(db, nicloudtest.AuthzUserSubject(ownerUser), owner.OrganizationID, owner.UserID)
		...
		_, err = ownerClient.PauseTask(ctx, nicloudsdk.Me, task.ID)
		require.NoError(t, err)
...
func TestResumeTask(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		pauseResp, err := client.PauseTask(ctx, nicloudsdk.Me, task.ID)
		require.NoError(t, err)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, pauseResp.WorkspaceBuild.ID)

		resumeResp, err := client.ResumeTask(ctx, nicloudsdk.Me, task.ID)
		require.NoError(t, err)
		build := *resumeResp.WorkspaceBuild
		require.Equal(t, nicloudsdk.WorkspaceTransitionStart, build.Transition)
		require.Equal(t, task.WorkspaceID.UUID, build.WorkspaceID)
		require.Equal(t, workspace.LatestBuild.BuildNumber+2, build.BuildNumber)
		require.Equal(t, string(nicloudsdk.CreateWorkspaceBuildReasonTaskResume), string(build.Reason))
	...
	t.Run("Resume a task that is not paused", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, nicloudsdk.Me, workspaceBuild.Task.ID)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusConflict, apiErr.StatusCode())
	})
	...
	t.Run("Task not found", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, nicloudsdk.Me, uuid.New())
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Task lookup forbidden", func(t *testing.T) {
		...
		_, err := client.ResumeTask(ctx, nicloudsdk.Me, task.ID)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusNotFound, apiErr.StatusCode())
	})
	...
	t.Run("Notification", func(t *testing.T) {
		...
		_, err = ownerClient.ResumeTask(ctx, nicloudsdk.Me, task.ID)
		require.NoError(t, err)
