...
type agentChatContextTestSetup struct {
	...
	db          database.Store
	user        nicloudsdk.CreateFirstUserResponse
...
func TestAgentChatContext(t *testing.T) {
	...
	type addSuccessCase struct {
		...
		wantStored    [][]nicloudsdk.ChatMessagePart
		storedOrdered bool
		wantCached    []nicloudsdk.ChatMessagePart
		cachedOrdered bool
	}

	agentInstructionsPart := nicloudsdk.ChatMessagePart{
	...
	projectInstructionsPart := nicloudsdk.ChatMessagePart{
		Type:               nicloudsdk.ChatMessagePartTypeContextFile,
		ContextFilePath:    "/workspace/AGENTS.md",
		ContextFileContent: "project instructions",
	}
	cachedAgentInstructionsPart := nicloudsdk.ChatMessagePart{
		Type:            nicloudsdk.ChatMessagePartTypeContextFile,
		ContextFilePath: agentInstructionsPart.ContextFilePath,
	}
	cachedFileAPart := nicloudsdk.ChatMessagePart{
		Type:            nicloudsdk.ChatMessagePartTypeContextFile,
		ContextFilePath: fileAPart.ContextFilePath,
	}
	cachedFileBPart := nicloudsdk.ChatMessagePart{
		Type:            nicloudsdk.ChatMessagePartTypeContextFile,
		ContextFilePath: fileBPart.ContextFilePath,
	}
	cachedRepoHelperSkillPart := nicloudsdk.ChatMessagePart{
		Type:             nicloudsdk.ChatMessagePartTypeSkill,
		SkillName:        repoHelperSkillPart.SkillName,
		SkillDescription: repoHelperSkillPart.SkillDescription,
	}
	cachedProjectInstructionsPart := nicloudsdk.ChatMessagePart{
		Type:            nicloudsdk.ChatMessagePartTypeContextFile,
	...
	addSuccessCases := []addSuccessCase{
		{
			...
			steps:         []addSuccessStep{{req: agentsdk.AddChatContextRequest{Parts: []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("ignore this text part"), agentInstructionsPart}}, wantCount: 1}},
			wantStored:    [][]nicloudsdk.ChatMessagePart{{agentInstructionsPart}},
			storedOrdered: true,
			wantCached:    []nicloudsdk.ChatMessagePart{cachedAgentInstructionsPart},
			cachedOrdered: true,
		},
		{
			name:          "AddSuccessIsAdditive",
			steps:         []addSuccessStep{{req: agentsdk.AddChatContextRequest{Parts: []nicloudsdk.ChatMessagePart{fileAPart}}, wantCount: 1}, {req: agentsdk.AddChatContextRequest{Parts: []nicloudsdk.ChatMessagePart{fileBPart}}, wantCount: 1}},
			wantStored:    [][]nicloudsdk.ChatMessagePart{{fileAPart}, {fileBPart}},
			storedOrdered: false,
			wantCached:    []nicloudsdk.ChatMessagePart{cachedFileAPart, cachedFileBPart},
			cachedOrdered: false,
		},
		{
			name:  "AddSuccessWithSkillOnlyPartsGetsSentinel",
			steps: []addSuccessStep{{req: agentsdk.AddChatContextRequest{Parts: []nicloudsdk.ChatMessagePart{repoHelperSkillPart}}, wantCount: 1}},
			wantStored: [][]nicloudsdk.ChatMessagePart{{{
				Type:            nicloudsdk.ChatMessagePartTypeContextFile,
				ContextFilePath: chatd.AgentChatContextSentinelPath,
			}, repoHelperSkillPart}},
			storedOrdered: true,
			wantCached:    []nicloudsdk.ChatMessagePart{cachedRepoHelperSkillPart},
			cachedOrdered: true,
		},
		{
			name:          "AddSuccessWithMixedPartsNoSentinel",
			steps:         []addSuccessStep{{req: agentsdk.AddChatContextRequest{Parts: []nicloudsdk.ChatMessagePart{projectInstructionsPart, repoHelperSkillPart}}, wantCount: 2}},
			wantStored:    [][]nicloudsdk.ChatMessagePart{{projectInstructionsPart, repoHelperSkillPart}},
			storedOrdered: true,
			wantCached:    []nicloudsdk.ChatMessagePart{cachedProjectInstructionsPart, cachedRepoHelperSkillPart},
			cachedOrdered: true,
		},
	}
	...
	for _, tc := range addSuccessCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			model := nicloud.InsertAgentChatTestModelConfig(t, setup.db, setup.user.UserID)
	...
	t.Run("AddUsesLockedChatModelConfig", func(t *testing.T) {
		...
		originalModel := nicloud.InsertAgentChatTestModelConfig(t, baseDB, user.UserID)
	...
	t.Run("ClearDeletesSkillMessages", func(t *testing.T) {
		...
		skillPart := nicloudsdk.ChatMessagePart{
			Type:                     nicloudsdk.ChatMessagePartTypeSkill,
			SkillName:                "repo-helper",
			SkillDescription:         "Repository instructions",
			SkillDir:                 "/workspace/.agents/skills/repo-helper",
		...
		_, err := setup.agentClient.AddChatContext(ctx, agentsdk.AddChatContextRequest{
			Parts: []nicloudsdk.ChatMessagePart{skillPart},
		})
		require.NoError(t, err)
		...
		rawSkillOnly, err := json.Marshal([]nicloudsdk.ChatMessagePart{storedParts[1]})
		require.NoError(t, err)
	...
	t.Run("ClearDeletesSkillMessagesBeforeCompressedSummary", func(t *testing.T) {
		...
		summaryContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("compressed summary"),
		})
		require.NoError(t, err)
		...
		regularContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("keep this user message"),
		})
		require.NoError(t, err)
		_ = dbgen.ChatMessage(t, setup.db, database.ChatMessage{
			ChatID:         chat.ID,
		...
		require.Equal(t, nicloudsdk.ChatMessagePartTypeText, remainingParts[0].Type)
	...
	t.Run("ClearSuccessResetsProviderResponseChain", func(t *testing.T) {
		...
		assistantContent, err := chatprompt.MarshalParts([]nicloudsdk.ChatMessagePart{
			nicloudsdk.ChatMessageText("assistant reply"),
		})
		require.NoError(t, err)
		_ = dbgen.ChatMessage(t, setup.db, database.ChatMessage{
			ChatID:             chat.ID,
	...
	t.Run("AddRejectsChatOwnedByAnotherAgent", func(t *testing.T) {
		...
		user := nicloudtest.CreateFirstUser(t, client)
		model := nicloud.InsertAgentChatTestModelConfig(t, db, user.UserID)
	...
	t.Run("AddRejectsChatOwnedByAnotherUserOnSameAgent", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		setup := newAgentChatContextTestSetup(t)
		_, otherUser := nicloudtest.CreateAnotherUser(t, setup.client, setup.user.OrganizationID)
		model := nicloud.InsertAgentChatTestModelConfig(t, setup.db, setup.user.UserID)
	...
	t.Run("AddRejectsTooManyParts", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		setup := newAgentChatContextTestSetup(t)
		parts := make([]nicloudsdk.ChatMessagePart, 101)
		for i := range parts {
			parts[i] = nicloudsdk.ChatMessagePart{
				Type:               nicloudsdk.ChatMessagePartTypeContextFile,
...
func requireAgentChatContextCachedParts(ctx context.Context, t testing.TB, db database.Store, chatID uuid.UUID) []nicloudsdk.ChatMessagePart {
	t.Helper()
...
func requireAgentChatContextStoredMessages(t testing.TB, messages []database.ChatMessage) [][]nicloudsdk.ChatMessagePart {
	t.Helper()

	stored := make([][]nicloudsdk.ChatMessagePart, len(messages))
...
func agentChatContextExpectedMessages(agent database.WorkspaceAgent, messages [][]nicloudsdk.ChatMessagePart) [][]nicloudsdk.ChatMessagePart {
	expected := make([][]nicloudsdk.ChatMessagePart, len(messages))
...
func agentChatContextExpectedStoredParts(agent database.WorkspaceAgent, parts []nicloudsdk.ChatMessagePart) []nicloudsdk.ChatMessagePart {
	expected := make([]nicloudsdk.ChatMessagePart, len(parts))
...
func agentChatContextExpectedCachedParts(agent database.WorkspaceAgent, parts []nicloudsdk.ChatMessagePart) []nicloudsdk.ChatMessagePart {
	expected := make([]nicloudsdk.ChatMessagePart, len(parts))
	for i, part := range parts {
		part.ContextFileAgentID = uuid.NullUUID{UUID: agent.ID, Valid: true}
		expected[i] = part
...
func requireAgentChatContextParts(t testing.TB, raw json.RawMessage) []nicloudsdk.ChatMessagePart {
	t.Helper()
