...
func keyIDs(keys []nicloudsdk.AIProviderKey) []uuid.UUID {
...
func TestAIProvidersCRUD(t *testing.T) {
	...
	t.Run("CreatePreservesPresetProviderTypes", func(t *testing.T) {
		...
		tests := []struct {
			providerType nicloudsdk.AIProviderType
			baseURL      string
		}{
			{providerType: nicloudsdk.AIProviderTypeAzure, baseURL: "https://example.openai.azure.com/openai/v1"},
			{providerType: nicloudsdk.AIProviderTypeGoogle, baseURL: "https://generativelanguage.googleapis.com/v1beta/openai/"},
			{providerType: nicloudsdk.AIProviderTypeOpenAICompat, baseURL: "https://compat.example.com/v1"},
			{providerType: nicloudsdk.AIProviderTypeOpenrouter, baseURL: "https://openrouter.ai/api/v1"},
			{providerType: nicloudsdk.AIProviderTypeVercel, baseURL: "https://ai-gateway.vercel.sh/v1"},
		}
		for _, tt := range tests {
			t.Run(string(tt.providerType), func(t *testing.T) {
				created, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
	...
	t.Run("CreateGetUpdateDelete", func(t *testing.T) {
		...
		disabled := false
		updated, err := client.UpdateAIProvider(ctx, created.Name, nicloudsdk.UpdateAIProviderRequest{
	...
	t.Run("RequiredBaseURL", func(t *testing.T) {
		...
		//nolint:gocritic // Owner role is the audience for this endpoint.
		_, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeOpenAI,
		...
		require.Contains(t, sdkErr.Validations, nicloudsdk.ValidationError{Field: "base_url", Detail: "base_url is required"})
		...
		baseURL = ""
		_, err = client.UpdateAIProvider(ctx, created.Name, nicloudsdk.UpdateAIProviderRequest{
			BaseURL: &baseURL,
		})
		sdkErr = requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "Invalid AI provider request.", sdkErr.Message)
	...
	t.Run("ListExcludesDeletedProviderKeys", func(t *testing.T) {
		...
		// own keys.
		//nolint:gocritic // Owner role is the audience for this endpoint.
		deleted, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeOpenAI,
			Name:    "list-deleted",
			Enabled: true,
			BaseURL: "https://api.openai.com/v1",
		...
		live, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeOpenAI,
			Name:    "list-live",
			Enabled: true,
			BaseURL: "https://api.openai.com/v1",
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		anon := nicloudsdk.New(ownerClient.URL)
	...
	t.Run("BedrockSettingsRequireAnthropic", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			Settings: &nicloudsdk.AIProviderSettings{
				Bedrock: &nicloudsdk.AIProviderBedrockSettings{Region: "us-east-1"},
			},
		})
		require.Error(t, err)
		require.ErrorAs(t, err, &sdkErr)
...
func TestAIProvidersKeyManagement(t *testing.T) {
	...
	t.Run("UpdateReplacesKeys", func(t *testing.T) {
		...
		replacement := []nicloudsdk.AIProviderKeyMutation{
		...
		updated, err := client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &replacement,
		})
		require.NoError(t, err)
	...
	t.Run("UpdateKeepsExistingByID", func(t *testing.T) {
		...
		// implicitly removed.
		patch := []nicloudsdk.AIProviderKeyMutation{
			{ID: &keepID},
	...
	t.Run("UpdateClearsKeys", func(t *testing.T) {
		...
		empty := []nicloudsdk.AIProviderKeyMutation{}
		updated, err := client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &empty,
		})
		require.NoError(t, err)
	...
	t.Run("UpdateKeepOnlyIsNoOp", func(t *testing.T) {
		...
		mutations := []nicloudsdk.AIProviderKeyMutation{
	...
	t.Run("BedrockRejectsUpdateWithKeys", func(t *testing.T) {
		...
		rejected := []nicloudsdk.AIProviderKeyMutation{
	...
	t.Run("CopilotCreateWithoutKeys", func(t *testing.T) {
		...
		//nolint:gocritic // Owner role is the audience for this endpoint.
		provider, err := client.CreateAIProvider(ctx, nicloudsdk.CreateAIProviderRequest{
			Type:    nicloudsdk.AIProviderTypeCopilot,
			Name:    "keys-copilot",
			Enabled: true,
			BaseURL: "https://api.business.githubcopilot.com",
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.AIProviderTypeCopilot, provider.Type)
	...
	t.Run("WhitespaceKeyRejected", func(t *testing.T) {
		...
		muts := []nicloudsdk.AIProviderKeyMutation{{APIKey: &padded}}
		_, err = client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &muts,
		})
		require.Error(t, err)
	...
	t.Run("NonOwnerForbidden", func(t *testing.T) {
		...
		_, err = memberClient.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &patch,
		})
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
	...
	t.Run("MutationBothFieldsRejected", func(t *testing.T) {
		...
		muts := []nicloudsdk.AIProviderKeyMutation{
	...
	t.Run("PATCHKeysSurfacesOpsInAudit", func(t *testing.T) {
		...
		updatedProvider, err := client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &mutations,
		})
		require.NoError(t, err)
		...
		var added nicloudsdk.AIProviderKey
	...
	t.Run("MutationUnknownIDRejected", func(t *testing.T) {
		...
		bogus := uuid.New()
		muts := []nicloudsdk.AIProviderKeyMutation{{ID: &bogus}}
		_, err = client.UpdateAIProvider(ctx, provider.Name, nicloudsdk.UpdateAIProviderRequest{
			APIKeys: &muts,
		})
		require.Error(t, err)
