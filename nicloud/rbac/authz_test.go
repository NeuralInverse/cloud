...
func BenchmarkCacher(b *testing.B) {
	...
	authz := rbac.Cacher(&nicloudtest.FakeAuthorizer{})
		...
		b.Run(fmt.Sprintf("%v:1", rat), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				if i%rat == 0 {
					// Cache miss
					b.StopTimer()
					subj, obj, action = nicloudtest.RandomRBACSubject(), nicloudtest.RandomRBACObject(), nicloudtest.RandomRBACAction()
					b.StartTimer()
				}
...
func TestCache(t *testing.T) {
	...
	t.Run("DontCacheTransientErrors", func(t *testing.T) {
		...
		var (
			...
			rec               = &nicloudtest.RecordingAuthorizer{Wrapped: ma}
			authz             = rbac.Cacher(rec)
			subj, obj, action = nicloudtest.RandomRBACSubject(), nicloudtest.RandomRBACObject(), nicloudtest.RandomRBACAction()
		)
	...
	t.Run("MultipleSubjects", func(t *testing.T) {
		...
		authz := rbac.Cacher(rec)
		subj1, obj1, action1 := nicloudtest.RandomRBACSubject(), nicloudtest.RandomRBACObject(), nicloudtest.RandomRBACAction()
		...
		// Extra unique calls
		var pairs []nicloudtest.ActionObjectPair
		subj2, obj2, action2 := nicloudtest.RandomRBACSubject(), nicloudtest.RandomRBACObject(), nicloudtest.RandomRBACAction()
		_ = authz.Authorize(ctx, subj2, action2, obj2)
		pairs = append(pairs, rec.Pair(action2, obj2))

		obj3, action3 := nicloudtest.RandomRBACObject(), nicloudtest.RandomRBACAction()
