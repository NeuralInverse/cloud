...
These can be found in `nicloud/rbac/roles.go`.
...
These can be found in `nicloud/rbac/policy/policy.go`.
...
Let's now add our frobulator noun to `nicloud/rbac/policy/policy.go`:
...
`policy.go` is used to generate code in `nicloud/rbac/object_gen.go`, and we can
execute this by running `make gen`.

Now we have this change in `nicloud/rbac/object_gen.go`:
...
all organization's frobulators; we need to add it to `nicloud/rbac/roles.go`:
...
`go test github.com/coder/coder/v2/nicloud/rbac`
...
--- FAIL: TestRolePermissions (0.61s)
    --- FAIL: TestRolePermissions/frobulator-AllActions (0.00s)
        roles_test.go:705:
            	Error Trace:	/tmp/coder/nicloud/rbac/roles_test.go:705
            	Error:      	Not equal:
...
FAIL
FAIL	github.com/coder/coder/v2/nicloud/rbac	1.314s
FAIL
```
...
> Take a look at `nicloud/rbac/roles_test.go` in the
...
```bash
$ go test github.com/coder/coder/v2/nicloud/rbac -count=1
ok  	github.com/coder/coder/v2/nicloud/rbac	1.313s
```
...
--- FAIL: TestRolePermissions (0.79s)
    --- FAIL: TestRolePermissions/FrobulatorsReadOnly (0.01s)
        roles_test.go:737:
            	Error Trace:	/tmp/coder/nicloud/rbac/roles_test.go:737
...
FAIL
FAIL	github.com/coder/coder/v2/nicloud/rbac	1.390s
FAIL
```
...
Let's add a SQL query to `nicloud/database/queries/frobulators.sql`:
...
`nicloud/database/modelmethods.go` is where we implement this interface for all
RBAC objects:
...
> Take a look at `nicloud/frobulators.go` in the
> [reference PR](https://github.com/coder/coder/pull/14055) for a complete
> example
...
func (api *API) createFrobulator(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.InsertFrobulatorRequest
