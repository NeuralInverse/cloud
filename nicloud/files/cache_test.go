...
func TestCancelledConcurrentFetch(t *testing.T) {
	...
	cache := files.LeakCache{Cache: files.New(prometheus.NewRegistry(), &nicloudtest.FakeAuthorizer{})}
...
func cachePromMetricName(metric string) string {
	return "nicloud_file_cache_" + metric
}
...
func TestConcurrency(t *testing.T) {
	...
	c := files.New(reg, &nicloudtest.FakeAuthorizer{})
...
func cacheAuthzSetup(t *testing.T) (database.Store, *files.Cache, *nicloudtest.RecordingAuthorizer) {
	t.Helper()
	...
	// Dbauthz wrap the db
	db = dbauthz.New(db, rec, logger, nicloudtest.AccessControlStorePointer())
	c := files.New(reg, rec)
	return db, c, rec
}
