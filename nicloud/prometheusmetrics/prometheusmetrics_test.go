...
func TestWorkspaceLatestBuildTotals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		Total    int
		Status   map[nicloudsdk.ProvisionerJobStatus]int
	}{{
		Name: "None",
		Database: func() database.Store {
			db, _ := dbtestutil.NewDB(t)
		...
		Total: 7,
		Status: map[nicloudsdk.ProvisionerJobStatus]int{
			nicloudsdk.ProvisionerJobCanceled:  1,
			nicloudsdk.ProvisionerJobFailed:    2,
			nicloudsdk.ProvisionerJobSucceeded: 3,
			nicloudsdk.ProvisionerJobRunning:   1,
		},
	}, {
		...
		Status: map[nicloudsdk.ProvisionerJobStatus]int{
			nicloudsdk.ProvisionerJobCanceled:  1,
			nicloudsdk.ProvisionerJobFailed:    1,
			nicloudsdk.ProvisionerJobSucceeded: 1,
			nicloudsdk.ProvisionerJobRunning:   1,
		},
	}} {
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				for _, m := range metrics {
					if m.GetName() != "nicloud_api_workspace_latest_build" {
						continue
					}

					for _, metric := range m.Metric {
						count, ok := tc.Status[nicloudsdk.ProvisionerJobStatus(metric.Label[0].GetValue())]
...
func TestWorkspaceLatestBuildStatuses(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		ExpectedStatuses   map[nicloudsdk.ProvisionerJobStatus]int
	}{{
		Name: "None",
		Database: func() database.Store {
			db, _ := dbtestutil.NewDB(t)
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				stMap := map[nicloudsdk.ProvisionerJobStatus]int{}
				for _, m := range metrics {
					if m.GetName() != "nicloud_workspace_latest_build_status" {
						continue
					}
					...
					for _, metric := range m.Metric {
						for _, l := range metric.Label {
							...
							if l.GetName() == "status" {
								status := nicloudsdk.ProvisionerJobStatus(l.GetValue())
	...
	for _, tc := range []struct {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				sum := 0
				for _, m := range metrics {
					if m.GetName() != "nicloud_workspace_creation_total" {
						continue
					}
					for _, metric := range m.Metric {
	...
	require.Eventually(t, func() bool {
		...
		for _, metric := range metrics {
			...
			case "nicloud_agents_up":
				...
				agentsUp = true
			case "nicloud_agents_connections":
			...
			case "nicloud_agents_apps":
				...
				assert.Equal(t, 1, int(metric.Metric[0].Gauge.GetValue()))                // Metric value
				agentsApps = true
			case "nicloud_prometheusmetrics_agents_execution_seconds":
...
func TestAgentStats(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		...
		for _, metric := range metrics {
			switch metric.GetName() {
			case "nicloud_prometheusmetrics_agentstats_execution_seconds":
				executionSeconds = true
			case "nicloud_agentstats_connection_count",
				"nicloud_agentstats_connection_median_latency_seconds",
				"nicloud_agentstats_rx_bytes",
				"nicloud_agentstats_tx_bytes",
				"nicloud_agentstats_session_count_jetbrains",
				"nicloud_agentstats_session_count_reconnecting_pty",
				"nicloud_agentstats_session_count_ssh",
				"nicloud_agentstats_session_count_vscode":
...
func TestExperimentsMetric(t *testing.T) {
	...
	if len(nicloudsdk.ExperimentsSafe) == 0 {
	...
	tests := []struct {
		...
		expected    map[nicloudsdk.Experiment]float64
	}{
		{
			name: "Enabled experiment is exported in metrics",
			experiments: nicloudsdk.Experiments{
				nicloudsdk.ExperimentsSafe[0],
			},
			expected: map[nicloudsdk.Experiment]float64{
				nicloudsdk.ExperimentsSafe[0]: 1,
			},
		},
		{
			...
			experiments: nicloudsdk.Experiments{},
			expected: map[nicloudsdk.Experiment]float64{
				nicloudsdk.ExperimentsSafe[0]: 0,
			},
		},
		{
			name:        "Unknown experiment is not exported in metrics",
			experiments: nicloudsdk.Experiments{nicloudsdk.Experiment("bob")},
			expected:    map[nicloudsdk.Experiment]float64{},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			seen := make(map[nicloudsdk.Experiment]float64)

			for _, metric := range out[0].GetMetric() {
				require.Equal(t, "nicloud_experiments", out[0].GetName())

				labels := metric.GetLabel()
				require.Lenf(t, labels, 1, "unexpected number of labels")

				experiment := nicloudsdk.Experiment(labels[0].GetValue())
			...
			// We don't want to define the state of all experiments because nicloudsdk.ExperimentAll will change at some
...
func TestBuildInfo(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, out, 1)
	require.Equal(t, "nicloud_build_info", out[0].GetName())
...
func prepareWorkspaceAndAgent(ctx context.Context, t *testing.T, client *nicloudsdk.Client, user nicloudsdk.CreateFirstUserResponse, workspaceNum int) agentproto.DRPCAgentClient {
