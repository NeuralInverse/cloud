...
// @Success 200 {array} nicloudsdk.ProvisionerJob
...
func (api *API) provisionerJobResources(rw http.ResponseWriter, r *http.Request, job database.ProvisionerJob) {
	...
	for _, resource := range resources {
		agents := make([]nicloudsdk.WorkspaceAgent, 0)
		for _, agent := range resourceAgents {
...
func convertProvisionerJobLogs(provisionerJobLogs []database.ProvisionerJobLog) []nicloudsdk.ProvisionerJobLog {
	sdk := make([]nicloudsdk.ProvisionerJobLog, 0, len(provisionerJobLogs))
...
func convertProvisionerJobLog(provisionerJobLog database.ProvisionerJobLog) nicloudsdk.ProvisionerJobLog {
	return nicloudsdk.ProvisionerJobLog{
		ID:        provisionerJobLog.ID,
		CreatedAt: provisionerJobLog.CreatedAt,
		Source:    nicloudsdk.LogSource(provisionerJobLog.Source),
		Level:     nicloudsdk.LogLevel(provisionerJobLog.Level),
		Stage:     provisionerJobLog.Stage,
		Output:    provisionerJobLog.Output,
	}
}

func convertProvisionerJob(pj database.GetProvisionerJobsByIDsWithQueuePositionRow) nicloudsdk.ProvisionerJob {
	...
	job := nicloudsdk.ProvisionerJob{
		...
		Type:           nicloudsdk.ProvisionerJobType(provisionerJob.Type),
		Error:          provisionerJob.Error.String,
		ErrorCode:      nicloudsdk.JobErrorCode(provisionerJob.ErrorCode.String),
	...
	job.Status = nicloudsdk.ProvisionerJobStatus(pj.ProvisionerJob.JobStatus)
...
func convertProvisionerJobWithQueuePosition(pj database.GetProvisionerJobsByOrganizationAndStatusWithQueuePositionAndProvisionerRow) nicloudsdk.ProvisionerJob {
	...
	job.Metadata = nicloudsdk.ProvisionerJobMetadata{
	...
	if pj.WorkspaceBuildTransition.Valid {
		job.Metadata.WorkspaceBuildTransition = nicloudsdk.WorkspaceTransition(pj.WorkspaceBuildTransition.WorkspaceTransition)
	}
	return job
}
...
func jobIsComplete(logger slog.Logger, job database.ProvisionerJob) bool {
	status := nicloudsdk.ProvisionerJobStatus(job.JobStatus)
	switch status {
	...
	case nicloudsdk.ProvisionerJobPending:
		return false
	case nicloudsdk.ProvisionerJobCanceling:
		return false
...
type logFollower struct {
	...
	conn      *websocket.Conn
	enc       *wsjson.Encoder[nicloudsdk.ProvisionerJobLog]
...
func (f *logFollower) follow() {
	...
	if !f.complete {
		...
		if err != nil {
			httpapi.Write(f.ctx, f.rw, http.StatusInternalServerError, nicloudsdk.Response{
	...
	if err != nil {
		httpapi.Write(f.ctx, f.rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	f.enc = wsjson.NewEncoder[nicloudsdk.ProvisionerJobLog](f.conn, websocket.MessageText)
