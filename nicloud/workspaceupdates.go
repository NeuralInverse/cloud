...
func convertRows(rows []database.GetWorkspacesAndAgentsByOwnerIDRow) workspacesByID {
	...
	for _, row := range rows {
		...
		out[row.ID] = ownedWorkspace{
			...
			Status:        tailnet.WorkspaceStatusToProto(nicloudsdk.ConvertWorkspaceStatus(nicloudsdk.ProvisionerJobStatus(row.JobStatus), nicloudsdk.WorkspaceTransition(row.Transition))),
			Agents:        agents,
		}
	}
	return out
