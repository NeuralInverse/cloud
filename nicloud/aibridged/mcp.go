...
const (
	InternalMCPServerID = "neuralinverse"
)

// Deprecated: Injected MCP in AI Bridge is deprecated and will be removed in a future release.
type MCPProxyBuilder interface {
	// Build creates a [mcp.ServerProxier] for the given request initiator.
	// At minimum, the Neural Inverse Cloud MCP server will be proxied.
	// The SessionKey from [Request] is used to authenticate against the Neural Inverse Cloud MCP server.
	//
...
func (m *MCPProxyFactory) retrieveMCPServerConfigs(ctx context.Context, req Request) (map[string]mcp.ServerProxier, error) {
	...
	proxiers := make(map[string]mcp.ServerProxier, len(mcpSrvCfgs.GetExternalAuthMcpConfigs())+1) // Extra one for Neural Inverse Cloud MCP server.

	if mcpSrvCfgs.GetCoderMcpConfig() != nil {
		// Delegated callers (e.g., chatd) do not hold the user's API key
		// secret and so cannot authenticate against the Neural Inverse Cloud MCP server.
		...
		if req.SessionKey == "" {
			...
			// Setup the Neural Inverse Cloud MCP server proxy.
