...
import (
	...
	mock "github.com/NeuralInverse/cloud/v2/nicloud/aibridged/aibridgedmock"
...
func TestServeHTTP_FailureModes(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			reqHeaders: map[string]string{
				nicloudsdk.SessionTokenHeader: "key", // NI-Session-Token is not supported; requests originate with AI clients, not neuralinverse CLI.
			},
		...
		// TODO: nicloud connection-related failures.
	...
	tests := []struct {
		...
		},
		{
			...
			reqHeaders: map[string]string{
				...
				// ExtractAuthToken prefers HeaderNIToken.
				agplaibridge.HeaderNIToken: "coder-token-byok",
...
func TestServeHTTP_DelegatedAPIKey_BYOK_Integration(t *testing.T) {
	...
	require.NoError(t, err)
	// HeaderNIToken marks the request as BYOK. Its value is irrelevant on
	// the delegated path (identity comes from context) and it must be
	// stripped before forwarding upstream.
	req.Header.Set(agplaibridge.HeaderNIToken, "ignored-on-delegated-path")
	...
	require.Empty(t, mockH.headersReceived.Get(agplaibridge.HeaderNIToken),
...
func TestServeHTTP_StripNIToken(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			// but does not include HeaderNIToken.
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// HeaderNIToken should always be stripped
			require.Empty(t, mockH.headersReceived.Get(agplaibridge.HeaderNIToken),
				"header %q should be stripped", agplaibridge.HeaderNIToken)
		})
	}
}
...
func TestExtractAuthToken(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "byok/empty",
			headers: map[string]string{agplaibridge.HeaderNIToken: ""},
		},
		{
			name:        "byok/ok",
			headers:     map[string]string{agplaibridge.HeaderNIToken: "coder-token"},
...
func TestServeHTTP_StripInternalHeaders(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			header: agplaibridge.HeaderNIToken,
			value:  "coder-token",
		},
		{
			name:   "X-Coder-AI-Governance-Request-Id",
			header: agplaibridge.HeaderNIRequestID,
