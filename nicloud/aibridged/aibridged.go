...
connectLoop:
	for retrier := retry.New(50*time.Millisecond, 10*time.Second); retrier.Wait(s.lifecycleCtx); {
		...
		s.logger.Debug(s.lifecycleCtx, "dialing nicloud")
		...
		if err != nil {
			...
			if errors.As(err, &sdkErr) && sdkErr.StatusCode() == http.StatusForbidden {
				s.logger.Error(s.lifecycleCtx, "not authorized to dial nicloud", slog.Error(err))
				return
			}
			if s.isShutdown() {
				return
			}
			s.logger.Warn(s.lifecycleCtx, "nicloud client failed to dial", slog.Error(err))
			continue
		}

		// TODO: log this with INFO level when we implement external aibridge daemons.
		logConnect(s.lifecycleCtx, "successfully connected to nicloud")
		retrier.Reset()
		...
		for {
			...
			case <-client.DRPCConn().Closed():
				logConnect(s.lifecycleCtx, "connection to nicloud closed")
