...
func TestOAuth2(t *testing.T) {
	...
	t.Run("NotSetup", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(nil, nil, nicloudsdk.HTTPCookieConfig{}, nil, nil)(nil).ServeHTTP(res, req)
	...
	t.Run("RedirectWithoutCode", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(tp, nil, nicloudsdk.HTTPCookieConfig{}, nil, nil)(nil).ServeHTTP(res, req)
	...
	t.Run("ExchangeCodeAndState", func(t *testing.T) {
		...
		res := httptest.NewRecorder()
		tp := newTestOAuth2Provider(t, oauth2.AccessTypeOffline)
		httpmw.ExtractOAuth2(tp, nil, nicloudsdk.HTTPCookieConfig{}, nil, nil)(http.HandlerFunc(func(_ http.ResponseWriter, r *http.Request) {
	...
	t.Run("CustomAuthCodeOptions", func(t *testing.T) {
		...
		httpmw.ExtractOAuth2(tp, nil, nicloudsdk.HTTPCookieConfig{}, authOpts, nil)(nil).ServeHTTP(res, req)
		location := res.Header().Get("Location")
	...
	t.Run("PresetConvertState", func(t *testing.T) {
		...
		res := httptest.NewRecorder()
		tp := newTestOAuth2Provider(t, oauth2.AccessTypeOffline)
		httpmw.ExtractOAuth2(tp, nil, nicloudsdk.HTTPCookieConfig{
			Secure:   true,
			SameSite: "none",
		...
		for _, cookie := range res.Result().Cookies() {
			if cookie.Name == nicloudsdk.OAuth2StateCookie {
