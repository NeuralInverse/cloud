...
func CSRF(cookieCfg nicloudsdk.HTTPCookieConfig) func(next http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		mw := nosurf.New(next)
		mw.SetBaseCookie(*cookieCfg.Apply(&http.Cookie{Path: "/", HttpOnly: true}))
		mw.SetFailureHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			sessCookie, err := r.Cookie(nicloudsdk.SessionTokenCookie)
			if err == nil &&
				r.Header.Get(nicloudsdk.SessionTokenHeader) != "" &&
				r.Header.Get(nicloudsdk.SessionTokenHeader) != sessCookie.Value {
				...
				http.Error(w,
					fmt.Sprintf("CSRF error encountered. Authentication via %q cookie and %q header detected, but the values do not match. "+
						...
						nicloudsdk.SessionTokenCookie, nicloudsdk.SessionTokenHeader),
		...
		mw.ExemptFunc(func(r *http.Request) bool {
			...
			if token := r.Header.Get(nicloudsdk.SessionTokenHeader); token == sessCookie.Value {
			...
			if token := r.URL.Query().Get(nicloudsdk.SessionTokenCookie); token == sessCookie.Value {
				// If the auth is set in a url param and matches the cookie, it
				// is the same as just using the url param.
				return true
			}

			if r.Header.Get(nicloudsdk.ProvisionerDaemonPSK) != "" {
			...
			if r.Header.Get(nicloudsdk.ProvisionerDaemonKey) != "" {
				// If present, the provisioner daemon also is providing an api key
				// that will make them exempt from CSRF. But this is still useful
				// for enumerating the external auths.
				return true
