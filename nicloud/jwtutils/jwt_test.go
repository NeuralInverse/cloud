	...
	cases := []testcase{
		{
			...
			claims: jwt.Claims{
				Issuer:    "coder",
				Subject:   "user@cloud.neuralinverse.com",
		...
		},
		{
			...
			expectedClaims: jwt.Expected{
				Subject: "user2@cloud.neuralinverse.com",
			},
...
func TestJWS(t *testing.T) {
	...
	t.Run("WithKeycache", func(t *testing.T) {
		...
		cache, err := cryptokeys.NewSigningCache(ctx, log, fetcher, nicloudsdk.CryptoKeyFeatureOIDCConvert)
		require.NoError(t, err)
...
func TestJWE(t *testing.T) {
	...
	t.Run("WithKeycache", func(t *testing.T) {
		...
		cache, err := cryptokeys.NewEncryptionCache(ctx, log, fetcher, nicloudsdk.CryptoKeyFeatureWorkspaceAppsAPIKey)
		require.NoError(t, err)
