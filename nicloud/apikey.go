...
// @Param request body nicloudsdk.CreateTokenRequest true "Create token request"
// @Success 201 {object} nicloudsdk.GenerateAPIKeyResponse
...
func (api *API) postToken(rw http.ResponseWriter, r *http.Request) {
	...
	var createToken nicloudsdk.CreateTokenRequest
	...
	scopes := database.APIKeyScopes{database.ApiKeyScopeNIAll}
	...
	aReq.New = *key
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.GenerateAPIKeyResponse{Key: cookie.Value})
}
...
// @Success 200 {object} nicloudsdk.APIKey
...
// @Tags Users
// @Param user path string true "User ID, name, or me"
// @Success 200 {array} nicloudsdk.APIKey
...
func (api *API) tokens(rw http.ResponseWriter, r *http.Request) {
	...
	var apiKeys []nicloudsdk.APIKeyWithOwner
	for _, key := range keys {
		if user, exists := usersByID[key.UserID]; exists {
			apiKeys = append(apiKeys, nicloudsdk.APIKeyWithOwner{
...
// @Param keyid path string true "Key ID" format(string)
// @Success 204
// @Failure 404 {object} nicloudsdk.Response
// @Failure 500 {object} nicloudsdk.Response
...
// @Tags General
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} nicloudsdk.TokenConfig
...
func (api *API) getMaxTokenLifetime(ctx context.Context, userID uuid.UUID) (time.Duration, error) {
	...
	for _, role := range roles {
		if role.Identifier.Name == nicloudsdk.RoleOwner {
