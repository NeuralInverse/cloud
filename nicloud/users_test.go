...
func TestFirstUser(t *testing.T) {
	...
	t.Run("Create", func(t *testing.T) {
		...
		client := nicloudtest.New(t, nil)
		_ = nicloudtest.CreateFirstUser(t, client)
		u, err := client.User(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		assert.Equal(t, nicloudtest.FirstUserParams.Name, u.Name)
		assert.Equal(t, nicloudtest.FirstUserParams.Email, u.Email)
		assert.Equal(t, nicloudtest.FirstUserParams.Username, u.Username)
	})
	...
	t.Run("Trial", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			TrialGenerator: func(context.Context, nicloudsdk.LicensorTrialRequest) error {
		...
		req := nicloudsdk.CreateFirstUserRequest{
			Email:    "testuser@cloud.neuralinverse.com",
			Username: "testuser",
			Name:     "Test User",
			Password: "SomeSecurePassword!",
			Trial:    true,
...
func TestFirstUser_OnboardingTelemetry(t *testing.T) {
	...
	t.Run("OnboardingInfoFlowsToSnapshot", func(t *testing.T) {
		...
		_, err := client.CreateFirstUser(ctx, nicloudsdk.CreateFirstUserRequest{
			...
			Username: "admin",
			Password: "SomeSecurePassword!",
			OnboardingInfo: &nicloudsdk.CreateFirstUserOnboardingInfo{
	...
	t.Run("EmptyOnboardingInfoIsNonNilWithZeroFields", func(t *testing.T) {
		...
		_, err := client.CreateFirstUser(ctx, nicloudsdk.CreateFirstUserRequest{
			Email: "admin@cloud.neuralinverse.com", Username: "admin",
			Password:       "SomeSecurePassword!",
			OnboardingInfo: &nicloudsdk.CreateFirstUserOnboardingInfo{},
		})
		require.NoError(t, err)
...
func TestPostLogin(t *testing.T) {
	...
	t.Run("Suspended", func(t *testing.T) {
		...
		member, _ := nicloudtest.CreateAnotherUser(t, client, first.OrganizationID)
		numLogs++ // add an audit log for create user

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		memberUser, err := member.User(ctx, nicloudsdk.Me)
		require.NoError(t, err, "fetch member user")

		_, err = client.UpdateUserStatus(ctx, memberUser.Username, nicloudsdk.UserStatusSuspended)
		require.NoError(t, err, "suspend member")
		numLogs++ // add an audit log for update user

		// Test an existing session
		_, err = member.User(ctx, nicloudsdk.Me)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusUnauthorized, apiErr.StatusCode())
	...
	t.Run("DisabledPasswordAuth", func(t *testing.T) {
		...
		dc := nicloudtest.DeploymentValues(t)
		client := nicloudtest.New(t, &nicloudtest.Options{
			DeploymentValues: dc,
		})
		...
		user, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			Email:           "test+user-@cloud.neuralinverse.com",
			Username:        "user",
			Password:        password,
		...
		userClient := nicloudsdk.New(client.URL)
		_, err = userClient.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
			Email:    user.Email,
			Password: password,
		})
		require.Error(t, err)
	...
	t.Run("Lifetime&Expire", func(t *testing.T) {
		...
		token, err := client.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{})
...
func TestDeleteUser(t *testing.T) {
	...
	t.Run("Works", func(t *testing.T) {
		...
		anotherClient, another := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		another, err = client.CreateUserWithOrgs(context.Background(), nicloudsdk.CreateUserRequestWithOrgs{
			Email:           another.Email,
		...
		_, err = anotherClient.User(context.Background(), nicloudsdk.Me)
		require.Error(t, err)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
		require.Equal(t, http.StatusUnauthorized, apiErr.StatusCode())
	...
	t.Run("NoPermission", func(t *testing.T) {
		t.Parallel()
		api := nicloudtest.New(t, nil)
		firstUser := nicloudtest.CreateFirstUser(t, api)
		client, _ := nicloudtest.CreateAnotherUser(t, api, firstUser.OrganizationID)
	...
	t.Run("HasWorkspaces", func(t *testing.T) {
		t.Parallel()
		client, _ := nicloudtest.NewWithProvisionerCloser(t, nil)
		user := nicloudtest.CreateFirstUser(t, client)
		anotherClient, another := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		nicloudtest.CreateWorkspace(t, anotherClient, template.ID)
		err := client.DeleteUser(context.Background(), another.ID)
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Self", func(t *testing.T) {
		...
		require.ErrorAs(t, err, &apiErr, "should be a nicloud error")
	...
	t.Run("CountCheckIncludesAllWorkspaces", func(t *testing.T) {
		...
		targetUserClient, targetUser := nicloudtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

		// Create a User Admin who should not have permission to see the target user's workspace
		userAdminClient, userAdmin := nicloudtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

		// Grant User Admin role to the userAdmin
		userAdmin, err := client.UpdateUserRoles(context.Background(), userAdmin.ID.String(), nicloudsdk.UpdateRoles{
			Roles: []string{rbac.RoleUserAdmin().String()},
		})
		require.NoError(t, err)

		// Create a template and workspace owned by the target user
		version := nicloudtest.CreateTemplateVersion(t, client, firstUser.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, firstUser.OrganizationID, version.ID)
		_ = nicloudtest.CreateWorkspace(t, targetUserClient, template.ID)

		workspaces, err := userAdminClient.Workspaces(context.Background(), nicloudsdk.WorkspaceFilter{
...
func TestNotifyUserStatusChanged(t *testing.T) {
	...
	verifyNotificationDispatched := func(notifyEnq *notificationstest.FakeEnqueuer, expectedNotifications []expectedNotification, member nicloudsdk.User, label string) {
	...
	t.Run("Account suspended", func(t *testing.T) {
		...
		_, userAdmin := nicloudtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID, rbac.RoleUserAdmin())

		member, err := adminClient.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
		...
		// when
		_, err = adminClient.UpdateUserStatus(context.Background(), member.Username, nicloudsdk.UserStatusSuspended)
		require.NoError(t, err)
	...
	t.Run("Account reactivated", func(t *testing.T) {
		...
		// when
		_, err = adminClient.UpdateUserStatus(context.Background(), member.Username, nicloudsdk.UserStatusActive)
		require.NoError(t, err)
...
func TestNotifyDeletedUser(t *testing.T) {
	...
	t.Run("OwnerNotified", func(t *testing.T) {
		...
		firstUserResponse := nicloudtest.CreateFirstUser(t, adminClient)
		...
		user, err := adminClient.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
...
func TestPostLogout(t *testing.T) {
	...
	t.Run("Logout", func(t *testing.T) {
		...
		for _, cookie := range cookies {
			if cookie.Name == nicloudsdk.SessionTokenCookie {
				require.Equal(t, nicloudsdk.SessionTokenCookie, cookie.Name, "Cookie should be the auth cookie")
...
func TestPostUsers(t *testing.T) {
	...
	t.Run("CreateWithStatus", func(t *testing.T) {
		...
		user, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			...
			Username:        "someone-else",
			Password:        "SomeSecurePassword!",
			UserStatus:      ptr.Ref(nicloudsdk.UserStatusActive),
		})
		require.NoError(t, err)

		require.Equal(t, nicloudsdk.UserStatusActive, user.Status)
	...
	t.Run("LastSeenAt", func(t *testing.T) {
		...
		client := nicloudtest.New(t, nil)
		firstUserResp := nicloudtest.CreateFirstUser(t, client)

		firstUser, err := client.User(ctx, firstUserResp.UserID.String())
		require.NoError(t, err)

		_, _ = nicloudtest.CreateAnotherUser(t, client, firstUserResp.OrganizationID)

		allUsersRes, err := client.Users(ctx, nicloudsdk.UsersRequest{})
		require.NoError(t, err)
	...
	t.Run("CreateNoneLoginType", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, found.LoginType, nicloudsdk.LoginTypeNone)
	})
	...
	t.Run("CreateOIDCLoginType", func(t *testing.T) {
		...
		_, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			...
			Username:        "someone-else",
			Password:        "",
			UserLoginType:   nicloudsdk.LoginTypeOIDC,
		})
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Equal(t, found.LoginType, nicloudsdk.LoginTypeOIDC)
	})
...
func TestNotifyCreatedUser(t *testing.T) {
	...
	t.Run("UserAdminNotified", func(t *testing.T) {
		...
		userAdmin, err := adminClient.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			OrganizationIDs: []uuid.UUID{firstUser.OrganizationID},
		...
		_, err = adminClient.UpdateUserRoles(ctx, userAdmin.Username, nicloudsdk.UpdateRoles{
			Roles: []string{
...
func TestUpdateUserProfile(t *testing.T) {
	...
	t.Run("UserNotFound", func(t *testing.T) {
		...
		_, err := client.UpdateUserProfile(ctx, uuid.New().String(), nicloudsdk.UpdateUserProfileRequest{
	...
	t.Run("ConflictingUsername", func(t *testing.T) {
		...
		existentUser, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			Email:           "bruno@cloud.neuralinverse.com",
			Username:        "bruno",
			Password:        "SomeSecurePassword!",
			OrganizationIDs: []uuid.UUID{user.OrganizationID},
		})
		require.NoError(t, err)
		_, err = client.UpdateUserProfile(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserProfileRequest{
	...
	t.Run("UpdateSelf", func(t *testing.T) {
		...
		userProfile, err := client.UpdateUserProfile(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserProfileRequest{
	...
	t.Run("UpdateSelfAsMember_Name", func(t *testing.T) {
		...
		memberClient, memberUser := nicloudtest.CreateAnotherUser(t, client, firstUser.OrganizationID)
		numLogs++ // add an audit log for user creation

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		newName := nicloudtest.RandomName(t)
		userProfile, err := memberClient.UpdateUserProfile(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserProfileRequest{
			Name:     newName,
	...
	t.Run("UpdateSelfAsMember_Username", func(t *testing.T) {
		...
		newUsername := nicloudtest.RandomUsername(t)
		_, err := memberClient.UpdateUserProfile(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserProfileRequest{
	...
	t.Run("UpdateMemberAsAdmin_Username", func(t *testing.T) {
		...
		_, memberUser := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
		numLogs++ // add an audit log for user creation
		...
		newUsername := nicloudtest.RandomUsername(t)
		userProfile, err := adminClient.UpdateUserProfile(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserProfileRequest{
			Name:     memberUser.Name,
			Username: newUsername,
		})
	...
	t.Run("InvalidRealUserName", func(t *testing.T) {
		...
		_, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
			Email:           "john@cloud.neuralinverse.com",
			Username:        "john",
			Password:        "SomeSecurePassword!",
			OrganizationIDs: []uuid.UUID{user.OrganizationID},
		})
...
func TestUpdateUserPassword(t *testing.T) {
	...
	t.Run("MemberCantUpdateAdminPassword", func(t *testing.T) {
		...
		err := member.UpdateUserPassword(ctx, owner.UserID.String(), nicloudsdk.UpdateUserPasswordRequest{
	...
	t.Run("AdminCanUpdateMemberPassword", func(t *testing.T) {
		...
		member, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
		...
		err = client.UpdateUserPassword(ctx, member.ID.String(), nicloudsdk.UpdateUserPasswordRequest{
	...
	t.Run("AuditorCantUpdateOtherUserPassword", func(t *testing.T) {
		...
		auditor, _ := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleAuditor())
		...
		err = auditor.UpdateUserPassword(ctx, member.ID.String(), nicloudsdk.UpdateUserPasswordRequest{
			Password: "SomeNewStrongPassword!",
		})
	...
	t.Run("MemberCanUpdateOwnPassword", func(t *testing.T) {
		...
		err := member.UpdateUserPassword(ctx, "me", nicloudsdk.UpdateUserPasswordRequest{
	...
	t.Run("AuditorCantTellIfPasswordIncorrect", func(t *testing.T) {
		...
		auditorClient, _ := nicloudtest.CreateAnotherUser(t, adminClient,
		...
		err := auditorClient.UpdateUserPassword(ctx, memberUser.ID.String(), nicloudsdk.UpdateUserPasswordRequest{
			Password: "MySecurePassword!",
		})
		numLogs++ // add an audit log for user update

		require.Error(t, err, "auditors shouldn't be able to update passwords")
		var httpErr *nicloudsdk.Error
	...
	t.Run("AdminCantUpdateOwnPasswordWithoutOldPassword", func(t *testing.T) {
		...
		err := client.UpdateUserPassword(ctx, "me", nicloudsdk.UpdateUserPasswordRequest{
			Password: "MySecurePassword!",
		})
		numLogs++ // add an audit log for user update
	...
	t.Run("ValidateUserPassword", func(t *testing.T) {
		...
		resp, err := client.ValidateUserPassword(ctx, nicloudsdk.ValidateUserPasswordRequest{
			Password: "MySecurePassword!",
		})
	...
	t.Run("ChangingPasswordDeletesKeys", func(t *testing.T) {
		...
		apikey1, err := client.CreateToken(ctx, user.UserID.String(), nicloudsdk.CreateTokenRequest{})
		require.NoError(t, err)

		apikey2, err := client.CreateToken(ctx, user.UserID.String(), nicloudsdk.CreateTokenRequest{})
		require.NoError(t, err)
...
func TestInitialRoles(t *testing.T) {
	...
	roles, err := client.UserRoles(ctx, nicloudsdk.Me)
	require.NoError(t, err)
	require.ElementsMatch(t, roles.Roles, []string{
		nicloudsdk.RoleOwner,
...
func TestPutUserSuspend(t *testing.T) {
	...
	t.Run("SuspendAnOwner", func(t *testing.T) {
		t.Parallel()
		client := nicloudtest.New(t, nil)
		me := nicloudtest.CreateFirstUser(t, client)
		_, user := nicloudtest.CreateAnotherUser(t, client, me.OrganizationID, rbac.RoleOwner())

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		_, err := client.UpdateUserStatus(ctx, user.Username, nicloudsdk.UserStatusSuspended)
	...
	t.Run("SuspendAnotherUser", func(t *testing.T) {
		...
		_, user := nicloudtest.CreateAnotherUser(t, client, me.OrganizationID)
		numLogs++ // add an audit log for user create

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		user, err := client.UpdateUserStatus(ctx, user.Username, nicloudsdk.UserStatusSuspended)
		require.NoError(t, err)
		require.Equal(t, user.Status, nicloudsdk.UserStatusSuspended)
		numLogs++ // add an audit log for user update
	...
	t.Run("SuspendItSelf", func(t *testing.T) {
		...
		client.User(ctx, nicloudsdk.Me)
		_, err := client.UpdateUserStatus(ctx, nicloudsdk.Me, nicloudsdk.UserStatusSuspended)
...
func TestActivateDormantUser(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
	defer cancel()
	anotherUser, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
		Email:           "coder@cloud.neuralinverse.com",
		Username:        "coder",
		Password:        "SomeStrongPassword!",
	...
	_, err = client.UpdateUserStatus(ctx, anotherUser.Username, nicloudsdk.UserStatusActive)
	require.NoError(t, err)

	// Verify if the account is active now
	anotherUser, err = client.User(ctx, anotherUser.Username)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.UserStatusActive, anotherUser.Status)
}
...
func TestGetUsersFilter(t *testing.T) {
	...
	nicloudtest.UsersFilter(setupCtx, t, client, api.Database, nil, nil, func(testCtx context.Context, req nicloudsdk.UsersRequest) []nicloudsdk.ReducedUser {
		...
		require.NoError(t, err)
		reduced := make([]nicloudsdk.ReducedUser, len(res.Users))
...
func TestPostTokens(t *testing.T) {
	...
	apiKey, err := client.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{})
...
func TestUserTerminalFont(t *testing.T) {
	...
	t.Run("valid font", func(t *testing.T) {
		...
		client, _ := nicloudtest.CreateAnotherUser(t, adminClient, firstUser.OrganizationID)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitShort)
		defer cancel()

		// given
		initial, err := client.GetUserAppearanceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.TerminalFontName(""), initial.TerminalFont)

		// when
		updated, err := client.UpdateUserAppearanceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserAppearanceSettingsRequest{
		...
		// then
		require.Equal(t, nicloudsdk.TerminalFontFiraCode, updated.TerminalFont)
	})
	...
	t.Run("unsupported font", func(t *testing.T) {
		...
		// when
		_, err = client.UpdateUserAppearanceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserAppearanceSettingsRequest{
			ThemePreference: "light",
			TerminalFont:    "foobar",
		})
...
func TestUserThemeMode(t *testing.T) {
	...
	t.Run("defaults to empty", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ThemeModeUnset, initial.ThemeMode)
	...
	t.Run("sync mode roundtrip", func(t *testing.T) {
		...
		updated, err := client.UpdateUserAppearanceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserAppearanceSettingsRequest{
			...
			TerminalFont:    nicloudsdk.TerminalFontGeistMono,
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThemeModeSync, updated.ThemeMode)
		require.Equal(t, "light-tritan", updated.ThemeLight)
		require.Equal(t, "dark-tritan", updated.ThemeDark)

		// Fetched values should match.
		fetched, err := client.GetUserAppearanceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThemeModeSync, fetched.ThemeMode)
	...
	t.Run("empty theme_mode is accepted for back-compat", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ThemeModeUnset, updated.ThemeMode)
	...
	t.Run("omitted theme fields preserve sync settings", func(t *testing.T) {
		...
		require.Equal(t, "light-tritan", fetched.ThemeLight)
		require.Equal(t, "dark-tritan", fetched.ThemeDark)
		require.Equal(t, nicloudsdk.TerminalFontFiraCode, fetched.TerminalFont)
	})
	...
	t.Run("single mode with omitted slots preserves sync settings", func(t *testing.T) {
		...
		updated, err := client.UpdateUserAppearanceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserAppearanceSettingsRequest{
			ThemePreference: "dark",
			ThemeMode:       nicloudsdk.ThemeModeSingle,
			TerminalFont:    nicloudsdk.TerminalFontFiraCode,
		})
		require.NoError(t, err)
		require.Equal(t, "dark", updated.ThemePreference)
		require.Equal(t, nicloudsdk.ThemeModeSingle, updated.ThemeMode)
		require.Equal(t, "light-tritan", updated.ThemeLight)
		require.Equal(t, "dark-tritan", updated.ThemeDark)
		require.Equal(t, nicloudsdk.TerminalFontFiraCode, updated.TerminalFont)
		...
		require.NoError(t, err)
		require.Equal(t, "dark", fetched.ThemePreference)
		require.Equal(t, nicloudsdk.ThemeModeSingle, fetched.ThemeMode)
		require.Equal(t, "light-tritan", fetched.ThemeLight)
		require.Equal(t, "dark-tritan", fetched.ThemeDark)
		require.Equal(t, nicloudsdk.TerminalFontFiraCode, fetched.TerminalFont)
	})
	...
	t.Run("legacy auto with omitted theme_mode clears mode", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ThemeModeUnset, fetched.ThemeMode)
		require.Equal(t, "light-tritan", fetched.ThemeLight)
		require.Equal(t, "dark-tritan", fetched.ThemeDark)
		require.Equal(t, nicloudsdk.TerminalFontFiraCode, fetched.TerminalFont)
	})
	...
	t.Run("invalid theme_mode is rejected", func(t *testing.T) {
		...
		_, err := client.UpdateUserAppearanceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserAppearanceSettingsRequest{
			ThemePreference: "dark",
			ThemeMode:       nicloudsdk.ThemeMode("wizard"),
			TerminalFont:    nicloudsdk.TerminalFontGeistMono,
		})
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("invalid theme slots are rejected", func(t *testing.T) {
		...
		for _, tc := range []struct {
			name       string
			themeMode  nicloudsdk.ThemeMode
			themeLight string
			themeDark  string
		}{
			{
...
func TestUserTaskNotificationAlertDismissed(t *testing.T) {
	...
	t.Run("defaults to false", func(t *testing.T) {
		...
		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
	...
	t.Run("update to true", func(t *testing.T) {
		...
		updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
	...
	t.Run("update to false", func(t *testing.T) {
		...
		_, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			TaskNotificationAlertDismissed: ptr.Ref(true),
		})
		require.NoError(t, err)
...
func TestThinkingDisplayMode(t *testing.T) {
	...
	t.Run("defaults to auto", func(t *testing.T) {
		...
		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModeAuto, settings.ThinkingDisplayMode)
	})
	...
	t.Run("round-trips a valid mode", func(t *testing.T) {
		...
		updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			ThinkingDisplayMode: nicloudsdk.ThinkingDisplayModeAlwaysCollapsed,
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModeAlwaysCollapsed, updated.ThinkingDisplayMode)

		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModeAlwaysCollapsed, settings.ThinkingDisplayMode)
	})
	...
	t.Run("empty mode preserves stored value", func(t *testing.T) {
		...
		_, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			ThinkingDisplayMode: nicloudsdk.ThinkingDisplayModePreview,
		})
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModePreview, updated.ThinkingDisplayMode)
	})
}
...
func TestAgentChatSendShortcutPreference(t *testing.T) {
	...
	t.Run("defaults to enter", func(t *testing.T) {
		...
		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.AgentChatSendShortcutEnter, settings.AgentChatSendShortcut)
	})
	...
	t.Run("round-trips shortcut", func(t *testing.T) {
		...
		updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			AgentChatSendShortcut: nicloudsdk.AgentChatSendShortcutModifierEnter,
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.AgentChatSendShortcutModifierEnter, updated.AgentChatSendShortcut)

		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.AgentChatSendShortcutModifierEnter, settings.AgentChatSendShortcut)
	})
	...
	t.Run("rejects invalid shortcut", func(t *testing.T) {
		...
		_, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			AgentChatSendShortcut: nicloudsdk.AgentChatSendShortcut("bogus"),
		})
	...
	t.Run("updates preserve stored shortcut", func(t *testing.T) {
		...
		updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			ThinkingDisplayMode: nicloudsdk.ThinkingDisplayModeAlwaysExpanded,
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModeAlwaysExpanded, updated.ThinkingDisplayMode)
		require.Equal(t, nicloudsdk.AgentChatSendShortcutModifierEnter, updated.AgentChatSendShortcut)
	})
}
...
func TestAgentDisplayModePreferences(t *testing.T) {
	...
	t.Run("defaults shell tools to always collapsed", func(t *testing.T) {
		...
		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.AgentDisplayModeAlwaysCollapsed, settings.ShellToolDisplayMode)
	...
	t.Run("round-trips shell tool display mode", func(t *testing.T) {
		...
		for _, mode := range []nicloudsdk.AgentDisplayMode{
			nicloudsdk.AgentDisplayModeAlwaysExpanded,
			nicloudsdk.AgentDisplayModeAlwaysCollapsed,
		} {
			updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
	...
	t.Run("updates preserve stored display modes", func(t *testing.T) {
		...
		_, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			ThinkingDisplayMode:  nicloudsdk.ThinkingDisplayModePreview,
			ShellToolDisplayMode: nicloudsdk.AgentDisplayModeAlwaysCollapsed,
			CodeDiffDisplayMode:  nicloudsdk.AgentDisplayModeAlwaysExpanded,
		})
		require.NoError(t, err)

		updated, err := client.UpdateUserPreferenceSettings(ctx, nicloudsdk.Me, nicloudsdk.UpdateUserPreferenceSettingsRequest{
			ShellToolDisplayMode: nicloudsdk.AgentDisplayModeAlwaysExpanded,
		})
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModePreview, updated.ThinkingDisplayMode)
		require.Equal(t, nicloudsdk.AgentDisplayModeAlwaysExpanded, updated.ShellToolDisplayMode)
		require.Equal(t, nicloudsdk.AgentDisplayModeAlwaysExpanded, updated.CodeDiffDisplayMode)

		settings, err := client.GetUserPreferenceSettings(ctx, nicloudsdk.Me)
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.ThinkingDisplayModePreview, settings.ThinkingDisplayMode)
		require.Equal(t, nicloudsdk.AgentDisplayModeAlwaysExpanded, settings.ShellToolDisplayMode)
		require.Equal(t, nicloudsdk.AgentDisplayModeAlwaysExpanded, settings.CodeDiffDisplayMode)
	})
	...
	t.Run("rejects invalid shell tool display mode", func(t *testing.T) {
		...
		for _, tt := range []struct {
			name string
			mode nicloudsdk.AgentDisplayMode
		}{
			{
				name: "bogus",
				mode: nicloudsdk.AgentDisplayMode("bogus"),
			},
			{
				name: "thinking preview",
				mode: nicloudsdk.AgentDisplayMode(nicloudsdk.ThinkingDisplayModePreview),
			},
		} {
...
func TestWorkspacesByUser(t *testing.T) {
	...
	t.Run("Access", func(t *testing.T) {
		...
		newUser, err := client.CreateUserWithOrgs(ctx, nicloudsdk.CreateUserRequestWithOrgs{
		...
		require.NoError(t, err)
		auth, err := client.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
			Email:    newUser.Email,
			Password: "MySecurePassword!",
		})
		require.NoError(t, err)

		newUserClient := nicloudsdk.New(client.URL)
		...
		res, err := newUserClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{Owner: nicloudsdk.Me})
		require.NoError(t, err)
		require.Len(t, res.Workspaces, 0)

		res, err = client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{Owner: nicloudsdk.Me})
		require.NoError(t, err)
...
func TestDormantUser(t *testing.T) {
	...
	users, err := client.Users(ctx, nicloudsdk.UsersRequest{Search: newUser.Username})
	require.NoError(t, err)
	require.Len(t, users.Users, 1)
	require.Equal(t, nicloudsdk.UserStatusDormant, users.Users[0].Status)
	...
	require.NoError(t, err)
	require.Len(t, users.Users, 1)
	require.Equal(t, nicloudsdk.UserStatusActive, users.Users[0].Status)
}
...
func TestSuspendedPagination(t *testing.T) {
	...
	total := 10
	users := make([]nicloudsdk.User, 0, total)
	// Create users
	for i := 0; i < total; i++ {
		email := fmt.Sprintf("%d@cloud.neuralinverse.com", i)
	...
	deletedUser := users[2]
	expected := users[3:8]
	_, err = client.UpdateUserStatus(ctx, deletedUser.ID.String(), nicloudsdk.UserStatusSuspended)
...
func TestUserAutofillParameters(t *testing.T) {
	...
	t.Run("NotSelf", func(t *testing.T) {
		t.Parallel()
		client1, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{})

		u1 := nicloudtest.CreateFirstUser(t, client1)

		client2, u2 := nicloudtest.CreateAnotherUser(t, client1, u1.OrganizationID)
...
func TestPaginatedUsers(t *testing.T) {
	...
	gmailSearch := func(request nicloudsdk.UsersRequest) nicloudsdk.UsersRequest {
		request.Search = "gmail"
		return request
	}
	usernameSearch := func(request nicloudsdk.UsersRequest) nicloudsdk.UsersRequest {
	...
	tests := []struct {
		...
		opt      func(request nicloudsdk.UsersRequest) nicloudsdk.UsersRequest
	}{
...
// against.
func assertPagination(ctx context.Context, t *testing.T, client *nicloudsdk.Client, limit int, allUsers []database.User,
	opt func(request nicloudsdk.UsersRequest) nicloudsdk.UsersRequest,
) {
	var count int
	if opt == nil {
	...
	page, err := client.Users(ctx, opt(nicloudsdk.UsersRequest{
		Pagination: nicloudsdk.Pagination{
			Limit: limit,
		},
	}))
	...
	for {
		...
		offsetPage, err := client.Users(ctx, opt(nicloudsdk.UsersRequest{
			Pagination: nicloudsdk.Pagination{
				Limit:  limit,
				Offset: count,
			},
		...
		prevPage, err := client.Users(ctx, opt(nicloudsdk.UsersRequest{
			Pagination: nicloudsdk.Pagination{
				Offset: count - limit,
				Limit:  limit,
			},
...
func sortUsers(users []nicloudsdk.User) {
...
func onlyUsernames[U nicloudsdk.User | database.User](users []U) []string {
	var out []string
	for _, u := range users {
		switch u := (any(u)).(type) {
		case nicloudsdk.User:
...
func BenchmarkUsersMe(b *testing.B) {
	client := nicloudtest.New(b, nil)
	_ = nicloudtest.CreateFirstUser(b, client)
