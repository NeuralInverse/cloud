...
func TestProvisionerDaemonReport(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			expectedItems: []healthsdk.ProvisionerDaemonsReportItem{
				{
					ProvisionerDaemon: nicloudsdk.ProvisionerDaemon{
						ID:           uuid.Nil,
						Name:         "pd-ok",
						CreatedAt:    now,
						LastSeenAt:   nicloudsdk.NewNullTime(now, true),
						Version:      "v1.2.3",
						APIVersion:   "1.0",
						Provisioners: []nicloudsdk.ProvisionerType{nicloudsdk.ProvisionerTypeEcho, nicloudsdk.ProvisionerTypeTerraform},
