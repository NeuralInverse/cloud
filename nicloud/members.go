...
// @Success 200 {object} nicloudsdk.OrganizationMember
...
func (api *API) deleteOrganizationMember(rw http.ResponseWriter, r *http.Request) {
	...
	if member.UserID == apiKey.UserID {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{Message: "cannot remove self from an organization"})
		return
	}
...
// @Param organization path string true "Organization ID"
// @Param user path string true "User ID, name, or me"
// @Success 200 {object} nicloudsdk.OrganizationMemberWithUserData
// @Produce json
...
// @Success 200 {object} []nicloudsdk.PaginatedMembersResponse
...
func (api *API) paginatedMembers(rw http.ResponseWriter, r *http.Request) {
	...
	if len(paginatedMemberRows) == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.PaginatedMembersResponse{
			Members: []nicloudsdk.OrganizationMemberWithUserData{},
			Count:   0,
		})
		return
	}
	...
	resp := nicloudsdk.PaginatedMembersResponse{
		Members: members,
...
// We
func convertOrganizationMembers(ctx context.Context, db database.Store, mems []database.OrganizationMember) ([]nicloudsdk.OrganizationMember, error) {
	converted := make([]nicloudsdk.OrganizationMember, 0, len(mems))
	roleLookup := make([]database.NameOrganizationPair, 0)

	for _, m := range mems {
		converted = append(converted, nicloudsdk.OrganizationMember{
			UserID:         m.UserID,
			OrganizationID: m.OrganizationID,
			CreatedAt:      m.CreatedAt,
			UpdatedAt:      m.UpdatedAt,
			Roles: slice.List(m.Roles, func(r string) nicloudsdk.SlimRole {
...
func convertOrganizationMembersWithUserData(ctx context.Context, db database.Store, rows []database.OrganizationMembersRow, aiSeatSet map[uuid.UUID]struct{}) ([]nicloudsdk.OrganizationMemberWithUserData, error) {
	...
	converted := make([]nicloudsdk.OrganizationMemberWithUserData, 0)
	for i := range convertedMembers {
		_, hasAISeat := aiSeatSet[rows[i].OrganizationMember.UserID]
		converted = append(converted, nicloudsdk.OrganizationMemberWithUserData{
			...
			Status:             nicloudsdk.UserStatus(rows[i].Status),
			IsServiceAccount:   rows[i].IsServiceAccount,
			LoginType:          nicloudsdk.LoginType(rows[i].LoginType),
