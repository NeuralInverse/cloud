...
func (b Builder) DeploymentValues(dv *nicloudsdk.DeploymentValues) Builder {
	// nolint: revive
	b.deploymentValues = dv
	return b
}

func (b Builder) Experiments(exp nicloudsdk.Experiments) Builder {
	// nolint: revive
	cpy := make(nicloudsdk.Experiments, len(exp))
	copy(cpy, exp)
	b.experiments = cpy
	return b
}
...
func (b Builder) RichParameterValues(p []nicloudsdk.WorkspaceBuildParameter) Builder {
	// nolint: revive
...
func (e BuildError) Response() (int, nicloudsdk.Response) {
	...
	return e.Status, nicloudsdk.Response{
		Message: e.Message,
		Detail:  e.Error(),
	}
}
...
func (b *Builder) findNewBuildParameterValue(name string, presets []database.TemplateVersionPresetParameter) *nicloudsdk.WorkspaceBuildParameter {
...
func (b *Builder) checkTemplateJobStatus() error {
	...
	templateVersionJobStatus := nicloudsdk.ProvisionerJobStatus(templateVersionJob.JobStatus)
...
func (b *Builder) checkRunningBuild() error {
	...
	if nicloudsdk.ProvisionerJobStatus(job.JobStatus).Active() {
