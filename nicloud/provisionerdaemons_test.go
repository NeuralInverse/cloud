...
func TestProvisionerDaemons(t *testing.T) {
	...
	pd3 := dbgen.ProvisionerDaemon(t, nicloudAPI.Database, database.ProvisionerDaemon{
		Name:       "provisioner-3",
		CreatedAt:  dbtime.Now().Add(3 * time.Second),
		LastSeenAt: sql.NullTime{Time: nicloudAPI.Clock.Now().Add(time.Hour), Valid: true},
		KeyID:      nicloudsdk.ProvisionerKeyUUIDBuiltIn,
	...
	for i := range 50 {
		userDaemons = append(userDaemons, dbgen.ProvisionerDaemon(t, nicloudAPI.Database, database.ProvisionerDaemon{
			Name:      "user-provisioner-" + strconv.Itoa(i),
			CreatedAt: dbtime.Now().Add(3 * time.Second),
			KeyID:     nicloudsdk.ProvisionerKeyUUIDUserAuth,
	...
	t.Run("Default limit", func(t *testing.T) {
		...
		daemons, err := templateAdminClient.OrganizationProvisionerDaemons(ctx, owner.OrganizationID, &nicloudsdk.OrganizationProvisionerDaemonsOptions{
			Offline: true,
		})
	...
	t.Run("Busy", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.ProvisionerDaemonBusy, *daemons[0].Status)
		require.NotNil(t, daemons[0].CurrentJob)
		require.Nil(t, daemons[0].PreviousJob)
		// Verify job.
		require.Equal(t, job1.ID, daemons[0].CurrentJob.ID)
		require.Equal(t, nicloudsdk.ProvisionerJobRunning, daemons[0].CurrentJob.Status)
	...
	t.Run("Offline", func(t *testing.T) {
		...
		// Verify status.
		require.NotNil(t, daemons[0].Status)
		require.Equal(t, nicloudsdk.ProvisionerDaemonOffline, *daemons[0].Status)
		require.Nil(t, daemons[0].CurrentJob)
		require.NotNil(t, daemons[0].PreviousJob)
		// Verify job.
		require.Equal(t, job2.ID, daemons[0].PreviousJob.ID)
		require.Equal(t, nicloudsdk.ProvisionerJobSucceeded, daemons[0].PreviousJob.Status)
	...
	t.Run("Idle", func(t *testing.T) {
		...
		// Verify status.
		require.NotNil(t, daemons[0].Status)
		require.Equal(t, nicloudsdk.ProvisionerDaemonIdle, *daemons[0].Status)
		require.Nil(t, daemons[0].CurrentJob)
		require.Nil(t, daemons[0].PreviousJob)
	})
