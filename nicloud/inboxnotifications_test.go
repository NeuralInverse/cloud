...
func TestInboxNotification_Watch(t *testing.T) {
	...
	t.Run("Failure Modes", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				client, _ = nicloudtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

				ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
				defer cancel()

				resp, err := client.Request(ctx, http.MethodGet, "/api/v2/notifications/inbox/watch", nil,
					nicloudsdk.ListInboxNotificationsRequestToQueryParams(nicloudsdk.ListInboxNotificationsRequest{
	...
	t.Run("OK", func(t *testing.T) {
		...
		firstClient, _, _ := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
			Pubsub:   ps,
			Database: db,
		})
		firstUser := nicloudtest.CreateFirstUser(t, firstClient)
		member, memberClient := nicloudtest.CreateAnotherUser(t, firstClient, firstUser.OrganizationID, rbac.RoleTemplateAdmin())
		...
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconWorkspace, notif.Notification.Icon)
	})
...
func TestInboxNotifications_List(t *testing.T) {
	...
	t.Run("Failure Modes", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				firstUser := nicloudtest.CreateFirstUser(t, client)
				client, member := nicloudtest.CreateAnotherUser(t, client, firstUser.OrganizationID)

				ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
				defer cancel()

				notifs, err := client.ListInboxNotifications(ctx, nicloudsdk.ListInboxNotificationsRequest{})
				require.NoError(t, err)
	...
	t.Run("OK check icons", func(t *testing.T) {
		...
		for i := range 10 {
			dbgen.NotificationInbox(t, api.Database, database.InsertInboxNotificationParams{
				...
				Icon: func() string {
					if i == 9 {
						return "https://dev.cloud.neuralinverse.com/icon.png"
					}
		...
		require.Equal(t, "https://dev.cloud.neuralinverse.com/icon.png", notifs.Notifications[0].Icon)
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[9].Icon)
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[8].Icon)
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconAccount, notifs.Notifications[7].Icon)
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconTemplate, notifs.Notifications[6].Icon)
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconOther, notifs.Notifications[4].Icon)
	})
	...
	t.Run("OK with template filter", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.InboxNotificationFallbackIconWorkspace, notifs.Notifications[0].Icon)
	})
...
func TestInboxNotifications_ReadStatus(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, notifs.Notifications[19].ID.String(), nicloudsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
		require.NoError(t, err)
	...
	t.Run("NOK - wrong id", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, "xxx-xxx-xxx", nicloudsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
	...
	t.Run("NOK - unknown id", func(t *testing.T) {
		...
		updatedNotif, err := client.UpdateInboxNotificationReadStatus(ctx, failingPaginationUUID.String(), nicloudsdk.UpdateInboxNotificationReadStatusRequest{
			IsRead: true,
		})
