...
func TestWebpushSubscribeUnsubscribe(t *testing.T) {
	...
	_, anotherMember := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
	...
	err = memberClient.PostWebpushSubscription(ctx, "me", nicloudsdk.WebpushSubscription{
		Endpoint:  endpoint,
	...
	err = memberClient.DeleteWebpushSubscription(ctx, "me", nicloudsdk.DeleteWebpushSubscription{
		Endpoint: endpoint,
	})
	...
	err = memberClient.PostWebpushSubscription(ctx, anotherMember.ID.String(), nicloudsdk.WebpushSubscription{
		Endpoint:  endpoint,
		AuthKey:   validEndpointAuthKey,
		P256DHKey: validEndpointP256dhKey,
	})
	...
	err = memberClient.DeleteWebpushSubscription(ctx, anotherMember.ID.String(), nicloudsdk.DeleteWebpushSubscription{
		Endpoint: endpoint,
	})
...
func (d *testWebpushDispatcher) Dispatch(_ context.Context, _ uuid.UUID, _ nicloudsdk.WebpushMessage) error {
	d.dispatchCalls.Add(1)
	return nil
}

func (d *testWebpushDispatcher) Test(_ context.Context, _ nicloudsdk.WebpushSubscription) error {
	d.testCalls.Add(1)
	return nil
}
