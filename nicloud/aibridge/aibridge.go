...
// HeaderNeural Inverse CloudToken is a header set by clients opting into BYOK
// (Bring Your Own Key) mode. It carries the Neural Inverse Cloud token so
...
const HeaderNIToken = "X-Coder-AI-Governance-Token" //nolint:gosec // This is a header name, not a credential.

// HeaderNeural Inverse CloudRequestID is a header set by aibridgeproxyd on each
// request forwarded to aibridged for cross-service log correlation.
const HeaderNIRequestID = "X-Coder-AI-Governance-Request-Id"
...
// by the presence of the X-Neural Inverse Cloud-AI-Governance-Token header.
func IsBYOK(header http.Header) bool {
	return strings.TrimSpace(header.Get(HeaderNIToken)) != ""
}
...
func ExtractAuthToken(header http.Header) string {
	if token := strings.TrimSpace(header.Get(HeaderNIToken)); token != "" {
		return token
	}
