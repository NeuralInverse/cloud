...
func TestOAuth2ProviderTokenExchange(t *testing.T) {
	...
	tests := []struct {
		name string
		app  nicloudsdk.OAuth2ProviderApp
		// The flow is setup(ctx, client, user) -> preAuth(cfg) -> cfg.AuthCodeURL() -> preToken(cfg) -> cfg.Exchange()
		setup      func(context.Context, *nicloudsdk.Client, nicloudsdk.User) error
		...
		},
		{
			...
			tokenError:  "The authorization code is invalid or expired",
			setup: func(ctx context.Context, client *nicloudsdk.Client, user nicloudsdk.User) error {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.tokenError != "" {
				...
				newClient := nicloudsdk.New(userClient.URL)
				newClient.SetSessionToken(token.AccessToken)

				gotUser, err := newClient.User(ctx, nicloudsdk.Me)
				require.NoError(t, err)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.error != "" {
				...
				_, err = newClient.User(ctx, nicloudsdk.Me)
				require.Error(t, err)
...
type exchangeSetup struct {
	cfg      *oauth2.Config
	app      nicloudsdk.OAuth2ProviderApp
	secret   nicloudsdk.OAuth2ProviderAppSecretFull
	code     string
	verifier string
}
...
func TestOAuth2ProviderRevoke(t *testing.T) {
	...
	tests := []struct {
		...
		fn func(context.Context, *nicloudsdk.Client, exchangeSetup)
		...
		{
			name: "DeleteApp",
			fn: func(ctx context.Context, _ *nicloudsdk.Client, s exchangeSetup) {
				//nolint:gocritic // OAauth2 app management requires owner permission.
		...
		{
			name: "DeleteApp",
			fn: func(ctx context.Context, client *nicloudsdk.Client, s exchangeSetup) {
	...
	setup := func(ctx context.Context, testClient *nicloudsdk.Client, name string) exchangeSetup {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			ctx := testutil.Context(t, testutil.WaitLong)
			testClient, testUser := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
			...
			newClient := nicloudsdk.New(client.URL)
			newClient.SetSessionToken(token.AccessToken)
			...
			filter := nicloudsdk.OAuth2ProviderAppFilter{UserID: testUser.ID}
			...
			apps, err = client.OAuth2ProviderApps(ctx, nicloudsdk.OAuth2ProviderAppFilter{UserID: owner.UserID})
			require.NoError(t, err)
...
func authorizationFlow(ctx context.Context, client *nicloudsdk.Client, cfg *oauth2.Config) (code, codeVerifier string, err error) {
...
func TestOAuth2ProviderCrossResourceAudienceValidation(t *testing.T) {
	...
	server1 := nicloudtest.New(t, &nicloudtest.Options{
		Database: db,
		Pubsub:   pubsub,
	})
	owner := nicloudtest.CreateFirstUser(t, server1)

	// Set up second Coder instance (resource server 2) - simulate different host
	server2 := nicloudtest.New(t, &nicloudtest.Options{
		Database: db,
		Pubsub:   pubsub,
	})
	...
	require.NoError(t, err)
	userClient, user := nicloudtest.CreateAnotherUser(t, server1, owner.OrganizationID)
	...
	client1 := nicloudsdk.New(server1.URL)
	client1.SetSessionToken(token.AccessToken)
	gotUser, err := client1.User(ctx, nicloudsdk.Me)
	require.NoError(t, err)
	require.Equal(t, user.ID, gotUser.ID)
	...
	client2 := nicloudsdk.New(server2.URL)
	...
	if _, err := client2.User(ctx, nicloudsdk.Me); err != nil {
...
func TestOAuth2RefreshExpiryOutlivesAccess(t *testing.T) {
	...
	db, pubsub := dbtestutil.NewDB(t)
	dv := nicloudtest.DeploymentValues(t, func(d *nicloudsdk.DeploymentValues) {
...
func TestOAuth2DynamicClientRegistration(t *testing.T) {
	...
	t.Run("BasicRegistration", func(t *testing.T) {
		...
		require.Contains(t, resp.GrantTypes, nicloudsdk.OAuth2ProviderGrantTypeAuthorizationCode)
		require.Contains(t, resp.GrantTypes, nicloudsdk.OAuth2ProviderGrantTypeRefreshToken)
		require.Contains(t, resp.ResponseTypes, nicloudsdk.OAuth2ProviderResponseTypeCode)
		require.Equal(t, nicloudsdk.OAuth2TokenEndpointAuthMethodClientSecretBasic, resp.TokenEndpointAuthMethod)
...
func TestOAuth2ClientConfiguration(t *testing.T) {
	...
	t.Run("UpdateConfiguration", func(t *testing.T) {
		...
		updateReq := nicloudsdk.OAuth2ClientRegistrationRequest{
...
func TestOAuth2RegistrationAccessToken(t *testing.T) {
	...
	t.Run("ManuallyCreatedClient", func(t *testing.T) {
		...
		appReq := nicloudsdk.PostOAuth2ProviderAppRequest{
	...
	t.Run("TokenPasswordComparison", func(t *testing.T) {
		...
		req1 := nicloudsdk.OAuth2ClientRegistrationRequest{
			RedirectURIs: []string{"https://client1.com/callback"},
			ClientName:   fmt.Sprintf("client-1-%d", timestamp),
		}
		req2 := nicloudsdk.OAuth2ClientRegistrationRequest{
...
// TestOAuth2CoderClient verfies a nicloudsdk client can be used with an oauth client.
func TestOAuth2CoderClient(t *testing.T) {
	...
	// Setup an oauth app
	ctx := testutil.Context(t, testutil.WaitLong)
	app, err := owner.PostOAuth2ProviderApp(ctx, nicloudsdk.PostOAuth2ProviderAppRequest{
		Name:        "new-app",
		CallbackURL: "http://localhost",
	})
	require.NoError(t, err)
	...
	// Make a new user
	client, user := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID)
	...
	usingOauth := nicloudsdk.New(owner.URL)
	usingOauth.HTTPClient = oauthClient

	me, err := usingOauth.User(ctx, nicloudsdk.Me)
	require.NoError(t, err)
	...
	_, err = usingOauth.User(ctx, nicloudsdk.Me)
	require.Error(t, err)
}
