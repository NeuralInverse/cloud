...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/nicloudtest/testjar"
...
func TestOIDCOauthLoginWithExisting(t *testing.T) {
	...
	cfg := fake.OIDCConfig(t, nil, func(cfg *nicloud.OIDCConfig) {
		...
		cfg.SecondaryClaims = nicloud.MergedClaimsSourceNone
	})
	...
	client, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
		...
		DeploymentValues: nicloudtest.DeploymentValues(t, func(values *nicloudsdk.DeploymentValues) {
			values.HTTPCookies = nicloudsdk.HTTPCookieConfig{
				Secure:   true,
				SameSite: "none",
			}
		}),
	...
	// Signup alice
	freshClient := func() *nicloudsdk.Client {
		cli := nicloudsdk.New(client.URL)
...
func TestUserLogin(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		_, err := anotherClient.LoginWithPassword(context.Background(), nicloudsdk.LoginWithPasswordRequest{
	...
	t.Run("LoginTypeNone", func(t *testing.T) {
		t.Parallel()
		anotherClient, anotherUser := nicloudtest.CreateAnotherUserMutators(t, client, user.OrganizationID, nil, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
			r.Password = ""
			r.UserLoginType = nicloudsdk.LoginTypeNone
		})
...
func TestUserOAuth2Github(t *testing.T) {
	...
	t.Run("NotInAllowedTeam", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				...
				AllowTeams:         []nicloud.GithubOAuth2Team{{"another", "something"}, {"coder", "frontend"}},
				OAuth2Config:       &testutil.OAuth2Config{},
				ListOrganizationMemberships: func(ctx context.Context, client *http.Client) ([]*github.Membership, error) {
					return []*github.Membership{{
						State: &stateActive,
			...
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				...
				ListEmails: func(ctx context.Context, client *http.Client) ([]*github.UserEmail, error) {
					return []*github.UserEmail{{
						Email:    github.String("testuser@cloud.neuralinverse.com"),
	...
	t.Run("BlockSignups", func(t *testing.T) {
		...
		email := atomic.NewString("testuser@cloud.neuralinverse.com")
		...
		id.Store(101)
		email.Store("someotheruser@cloud.neuralinverse.com")
	...
	t.Run("Signup", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			...
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				...
				ListEmails: func(ctx context.Context, client *http.Client) ([]*github.UserEmail, error) {
					return []*github.UserEmail{{
						Email:    github.String("kyle@cloud.neuralinverse.com"),
						Verified: github.Bool(true),
						Primary:  github.Bool(true),
					}}, nil
				},
		...
		require.NoError(t, err)
		require.Equal(t, "kyle@cloud.neuralinverse.com", user.Email)
	...
	t.Run("SignupAllowedTeam", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			...
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				AllowSignups:       true,
				AllowOrganizations: []string{"coder"},
				AllowTeams:         []nicloud.GithubOAuth2Team{{"coder", "frontend"}},
				OAuth2Config:       &testutil.OAuth2Config{},
				ListOrganizationMemberships: func(ctx context.Context, client *http.Client) ([]*github.Membership, error) {
					return []*github.Membership{{
						State: &stateActive,
	...
	t.Run("SignupAllowedTeamInFirstOrganization", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			...
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				...
				AllowTeams:         []nicloud.GithubOAuth2Team{{"coder", "backend"}},
				OAuth2Config:       &testutil.OAuth2Config{},
				ListOrganizationMemberships: func(ctx context.Context, client *http.Client) ([]*github.Membership, error) {
					return []*github.Membership{
						{
				...
				ListEmails: func(ctx context.Context, client *http.Client) ([]*github.UserEmail, error) {
					return []*github.UserEmail{{
						Email:    github.String("mathias@cloud.neuralinverse.com"),
						Verified: github.Bool(true),
						Primary:  github.Bool(true),
					}}, nil
				},
		...
		user, err := client.User(context.Background(), "me")
		require.NoError(t, err)
		require.Equal(t, "mathias@cloud.neuralinverse.com", user.Email)
	...
	t.Run("SignupAllowedTeamInSecondOrganization", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			...
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				AllowSignups:       true,
				AllowOrganizations: []string{"coder", "nil"},
				AllowTeams:         []nicloud.GithubOAuth2Team{{"nil", "null"}},
				OAuth2Config:       &testutil.OAuth2Config{},
				ListOrganizationMemberships: func(ctx context.Context, client *http.Client) ([]*github.Membership, error) {
					return []*github.Membership{
						{
	...
	t.Run("ChangedEmail", func(t *testing.T) {
		...
		coderEmail := &github.UserEmail{
			Email:    github.String("alice@cloud.neuralinverse.com"),
			Verified: github.Bool(true),
			Primary:  github.Bool(true),
		}
		...
		nicloudtest.CreateFirstUser(t, owner)
		...
		client := nicloudsdk.New(owner.URL)
		...
		links, err := db.GetUserLinksByUserID(dbauthz.As(ctx, nicloudtest.AuthzUserSubject(ownerUser)), deleted.ID)
		require.NoError(t, err)
	...
	t.Run("DeviceFlow", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{
			GithubOAuth2Config: &nicloud.GithubOAuth2Config{
				...
				AuthorizeDevice: func(_ context.Context) (*nicloudsdk.ExternalAuthDevice, error) {
					return &nicloudsdk.ExternalAuthDevice{
		...
		req.AddCookie(&http.Cookie{
			Name:  nicloudsdk.OAuth2PKCEVerifier,
		...
		var resp nicloudsdk.OAuth2DeviceFlowCallbackResponse
...
func TestUserOIDC(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		AllowSignups        bool
		EmailDomain         []string
		AssertUser          func(t testing.TB, u nicloudsdk.User)
		StatusCode          int
		...
		},
		{
			...
			IDTokenClaims: jwt.MapClaims{
				"email":          "cian@cloud.neuralinverse.com",
				"email_verified": true,
				"sub":            uuid.NewString(),
			},
			AllowSignups: true,
		...
		},
		{
			...
			IDTokenClaims: jwt.MapClaims{
				"email":  "coolin@cloud.neuralinverse.com",
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			cfg := fake.OIDCConfig(t, nil, func(cfg *nicloud.OIDCConfig) {
				...
				cfg.SecondaryClaims = nicloud.MergedClaimsSourceUserInfo
				if tc.IgnoreUserInfo {
					cfg.SecondaryClaims = nicloud.MergedClaimsSourceNone
				}
				if tc.UseAccessToken {
					cfg.SecondaryClaims = nicloud.MergedClaimsSourceAccessToken
				}
				cfg.NameField = "name"
			})
			...
			if tc.PrecreateFirstUser {
				owner.CreateFirstUser(ctx, nicloudsdk.CreateFirstUserRequest{
					Email:    "precreated@cloud.neuralinverse.com",
					Username: "precreated",
					Password: "SomeSecurePassword!",
				})
			}
	...
	t.Run("OIDCDormancy", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.UserStatusActive, me.Status)
	})
	...
	t.Run("OIDCConvert", func(t *testing.T) {
		...
		user, userData := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
		require.Equal(t, nicloudsdk.LoginTypePassword, userData.LoginType)
		...
		convertResponse, err := user.ConvertLoginType(ctx, nicloudsdk.ConvertLoginRequest{
			ToType:   nicloudsdk.LoginTypeOIDC,
			Password: "SomeSecurePassword!",
		})
		require.NoError(t, err)
		...
		_, _ = fake.LoginWithClient(t, user, claims, func(r *http.Request) {
			...
			r.Header.Set(nicloudsdk.SessionTokenHeader, user.SessionToken())
		...
		require.NoError(t, err)
		require.Equal(t, nicloudsdk.LoginTypeOIDC, info.LoginType)
	})
	...
	t.Run("BadJWT", func(t *testing.T) {
		...
		kc, err := cryptokeys.NewSigningCache(ctx, logger, fetcher, nicloudsdk.CryptoKeyFeatureOIDCConvert)
		require.NoError(t, err)
		...
		_, resp := fake.AttemptLogin(t, user, claims, func(r *http.Request) {
			...
			for i, cookie := range cookies {
				if cookie.Name != nicloud.OAuthConvertCookieValue {
					continue
				}

				jwt := cookie.Value
				var claims nicloud.OAuthConvertStateClaims
		...
		require.Equal(t, http.StatusBadRequest, resp.StatusCode)
		var respErr nicloudsdk.Response
	...
	t.Run("AlternateUsername", func(t *testing.T) {
		...
		numLogs := len(auditor.AuditLogs())
		claims := jwt.MapClaims{
			"email": "jon@cloud.neuralinverse.com",
			"sub":   uuid.NewString(),
		}
...
func TestUserLogout(t *testing.T) {
	...
	const (
		email    = "dean.was.here@test.cloud.neuralinverse.com"
		username = "dean"
		//nolint:gosec
	...
	loginRes1, err := userClient.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
		Email:    email,
		Password: password,
	})
	require.NoError(t, err)
	...
	loginRes2, err := userClient.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
		Email:    email,
		Password: password,
	})
	require.NoError(t, err)
	...
	for i := range 3 {
		key, _ := dbgen.APIKey(t, db, database.APIKey{
			UserID: newUser.ID,
			Scopes: database.APIKeyScopes{database.ApiKeyScopeNIApplicationConnect},
		})
	...
	_, err = userClient.User(ctx, nicloudsdk.Me)
	require.Error(t, err)
...
// TestOIDCSkipIssuer verifies nicloud can run without checking the issuer url
// in the OIDC exchange. This means the NEURALINVERSE_OIDC_ISSUER_URL does not need
...
func TestOIDCDomainErrorMessage(t *testing.T) {
	...
	setup := func() (*oidctest.FakeIDP, *nicloudsdk.Client) {
...
func TestOIDCSkipIssuer(t *testing.T) {
	...
	owner := nicloudtest.New(t, &nicloudtest.Options{
		OIDCConfig: fake.OIDCConfigSkipIssuerChecks(t, nil, func(cfg *nicloud.OIDCConfig) {
			cfg.AllowSignups = true
		}),
	})
...
func TestUserForgotPassword(t *testing.T) {
	...
	requireCanLogin := func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, email string, password string) {
	...
	requireCannotLogin := func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, email string, password string) {
		_, err := client.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
			Email:    email,
			Password: password,
		})
	...
	requireRequestOneTimePasscode := func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, notifyEnq *notificationstest.FakeEnqueuer, email string, userID uuid.UUID) string {
		notifyEnq.Clear()
		err := client.RequestOneTimePasscode(ctx, nicloudsdk.RequestOneTimePasscodeRequest{Email: email})
		require.NoError(t, err)
		sent := notifyEnq.Sent()
	...
	requireChangePasswordWithOneTimePasscode := func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, email string, passcode string, password string) {
		err := client.ChangePasswordWithOneTimePasscode(ctx, nicloudsdk.ChangePasswordWithOneTimePasscodeRequest{
			Email:           email,
	...
	t.Run("CanChangePassword", func(t *testing.T) {
		...
		err := anotherClient.ChangePasswordWithOneTimePasscode(ctx, nicloudsdk.ChangePasswordWithOneTimePasscodeRequest{
			Email:           anotherUser.Email,
	...
	t.Run("CannotChangePasswordOfAnotherUser", func(t *testing.T) {
		...
		anotherClient, anotherUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		thirdClient, thirdUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		err := thirdClient.ChangePasswordWithOneTimePasscode(ctx, nicloudsdk.ChangePasswordWithOneTimePasscodeRequest{
	...
	t.Run("GivenOKResponseWithInvalidEmail", func(t *testing.T) {
		...
		err := anotherClient.RequestOneTimePasscode(ctx, nicloudsdk.RequestOneTimePasscodeRequest{
			Email: "not-a-member@cloud.neuralinverse.com",
		})
		require.NoError(t, err)
...
func oauth2Callback(t *testing.T, client *nicloudsdk.Client, opts ...func(*http.Request)) *http.Response {
