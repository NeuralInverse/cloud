...
// be used by a user.
func (api *API) PrimaryRegion(ctx context.Context) (nicloudsdk.Region, error) {
	...
	if xerrors.Is(err, sql.ErrNoRows) {
		...
		return nicloudsdk.Region{}, xerrors.Errorf("get deployment ID: %w", err)
	}
	...
	if err != nil {
		return nicloudsdk.Region{}, xerrors.Errorf("get default proxy config: %w", err)
	}
...
// @Success 200 {object} nicloudsdk.RegionsResponse[nicloudsdk.Region]
...
func (api *API) regions(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.RegionsResponse[nicloudsdk.Region]{
		Regions: []nicloudsdk.Region{region},
	})
}
