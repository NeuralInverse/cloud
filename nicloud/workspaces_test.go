...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/nicloudtest/promhelp"
...
func TestWorkspace(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		authz := nicloudtest.AssertRBAC(t, api, client)
		...
		require.Equal(t, nicloudsdk.BuildReasonInitiator, ws.LatestBuild.Reason)
	...
	t.Run("Rename", func(t *testing.T) {
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		ws1 := nicloudtest.CreateWorkspace(t, client, template.ID)
		ws2 := nicloudtest.CreateWorkspace(t, client, template.ID)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws1.LatestBuild.ID)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws2.LatestBuild.ID)
		...
		err := client.UpdateWorkspace(ctx, ws1.ID, nicloudsdk.UpdateWorkspaceRequest{
			Name: want,
		})
	...
	t.Run("TemplateProperties", func(t *testing.T) {
		...
		assert.Equal(t, nicloudsdk.BuildReasonInitiator, ws.LatestBuild.Reason)
	...
	t.Run("Health", func(t *testing.T) {
		...
		t.Run("Sub-agent excluded", func(t *testing.T) {
			...
			var parentAgent nicloudsdk.WorkspaceAgent
			testutil.Eventually(ctx, t, func(ctx context.Context) bool {
				var err error
				workspace, err = client.Workspace(ctx, workspace.ID)
				if err != nil {
	...
	t.Run("Archived", func(t *testing.T) {
		...
		active := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, active.ID)
		template := nicloudtest.CreateTemplate(t, client, owner.OrganizationID, active.ID)
		// We need another version because the active template version cannot be
		// archived.
		version := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, nil, func(request *nicloudsdk.CreateTemplateVersionRequest) {
		...
		_, err = client.CreateWorkspace(ctx, owner.OrganizationID, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
	...
	t.Run("WorkspaceBan", func(t *testing.T) {
		t.Parallel()
		owner, _, _ := nicloudtest.NewWithAPI(t, &nicloudtest.Options{IncludeProvisionerDaemon: true})
		first := nicloudtest.CreateFirstUser(t, owner)

		version := nicloudtest.CreateTemplateVersion(t, owner, first.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, owner, version.ID)
		template := nicloudtest.CreateTemplate(t, owner, first.OrganizationID, version.ID)

		goodClient, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID)

		// When a user with workspace-creation-ban
		client, user := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgWorkspaceCreationBan(first.OrganizationID))

		// Ensure a similar user can create a workspace
		nicloudtest.CreateWorkspace(t, goodClient, template.ID)

		ctx := testutil.Context(t, testutil.WaitLong)
		// Then: Cannot create a workspace
		_, err := client.CreateUserWorkspace(ctx, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
		...
		wrk, err := owner.CreateUserWorkspace(ctx, user.ID.String(), nicloudsdk.CreateWorkspaceRequest{
			TemplateID:        template.ID,
			TemplateVersionID: uuid.UUID{},
			Name:              "random",
		})
		require.NoError(t, err)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, wrk.LatestBuild.ID)

		// Then: They cannot delete said workspace
		_, err = client.CreateWorkspaceBuild(ctx, wrk.ID, nicloudsdk.CreateWorkspaceBuildRequest{
			Transition:       nicloudsdk.WorkspaceTransitionDelete,
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				for _, createdPreset := range createdPresets {
					...
					for _, expectedPresetParam := range tc.presets[presetIndex].Parameters {
						paramFoundAtIndex := slices.IndexFunc(createdPreset.Parameters, func(createdPresetParam nicloudsdk.PresetParameter) bool {
				...
				if tc.selectedPresetIndex != nil {
					...
					workspace = nicloudtest.CreateWorkspace(t, client, template.ID, func(request *nicloudsdk.CreateWorkspaceRequest) {
				...
				builds, err := client.WorkspaceBuilds(ctx, nicloudsdk.WorkspaceBuildsRequest{
					WorkspaceID: ws.ID,
				})
				require.NoError(t, err)
				...
				parametersSetByPreset := slice.CountMatchingPairs(
					...
					func(gotParameter nicloudsdk.WorkspaceBuildParameter, presetParameter *proto.PresetParameter) bool {
...
func TestResolveAutostart(t *testing.T) {
	...
	client, member := nicloudtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
...
func TestWorkspacesSortOrder(t *testing.T) {
	...
	secondUserClient, secondUser := nicloudtest.CreateAnotherUserMutators(t, client, firstUser.OrganizationID, []rbac.RoleIdentifier{rbac.RoleOwner()}, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
		r.Username = "zzz"
	})
	...
	workspacesResponse, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	...
	workspacesResponse, err = secondUserClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
...
func TestPostWorkspacesByOrganization(t *testing.T) {
	...
	t.Run("CreateSendsNotification", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true, NotificationsEnqueuer: &enqueuer})
		user := nicloudtest.CreateFirstUser(t, client)
		templateAdminClient, templateAdmin := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin())
		memberClient, memberUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)

		version := nicloudtest.CreateTemplateVersion(t, templateAdminClient, user.OrganizationID, nil)
		template := nicloudtest.CreateTemplate(t, templateAdminClient, user.OrganizationID, version.ID)
		...
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, memberClient, workspace.LatestBuild.ID)
	...
	t.Run("CreateSendsNotificationToCorrectUser", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true, NotificationsEnqueuer: &enqueuer})
		user := nicloudtest.CreateFirstUser(t, client)
		templateAdminClient, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin(), rbac.RoleOwner())
		_, memberUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		workspace, err := templateAdminClient.CreateUserWorkspace(ctx, memberUser.Username, nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
	...
	t.Run("CreateFromVersionWithAuditLogs", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		user := nicloudtest.CreateFirstUser(t, client)
		versionDefault := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, versionDefault.ID)
		versionTest := nicloudtest.UpdateTemplateVersion(t, client, user.OrganizationID, nil, template.ID)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionDefault.ID)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionTest.ID)
		defaultWorkspace := nicloudtest.CreateWorkspace(t, client, uuid.Nil,
			func(c *nicloudsdk.CreateWorkspaceRequest) { c.TemplateVersionID = versionDefault.ID },
		)
		testWorkspace := nicloudtest.CreateWorkspace(t, client, uuid.Nil,
			func(c *nicloudsdk.CreateWorkspaceRequest) { c.TemplateVersionID = versionTest.ID },
		)
		defaultWorkspaceBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, defaultWorkspace.LatestBuild.ID)
		testWorkspaceBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, testWorkspace.LatestBuild.ID)
	...
	t.Run("InvalidCombinationOfTemplateAndTemplateVersion", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		user := nicloudtest.CreateFirstUser(t, client)
		versionTest := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		versionDefault := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, versionDefault.ID)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionTest.ID)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionDefault.ID)
		...
		_, err := client.CreateWorkspace(context.Background(), user.OrganizationID, nicloudsdk.Me, req)
	...
	t.Run("InvalidTTL", func(t *testing.T) {
		...
		t.Run("BelowMin", func(t *testing.T) {
			...
			_, err := client.CreateWorkspace(ctx, template.OrganizationID, nicloudsdk.Me, req)
			require.Error(t, err)
			var apiErr *nicloudsdk.Error
			require.ErrorAs(t, err, &apiErr)
			require.Equal(t, http.StatusBadRequest, apiErr.StatusCode())
	...
	t.Run("TemplateDefaultTTL", func(t *testing.T) {
		...
		ws, err := client.CreateWorkspace(ctx, template.OrganizationID, nicloudsdk.Me, req)
		require.NoError(t, err)
	...
	t.Run("NoProvisionersAvailable", func(t *testing.T) {
		...
		ws, err := client.CreateUserWorkspace(ctx, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "testing",
		})
		...
		require.Equal(t, nicloudsdk.ProvisionerJobPending, ws.LatestBuild.Job.Status)
	...
	t.Run("AllProvisionersStale", func(t *testing.T) {
		...
		provisioner := nicloudtest.NewTaggedProvisionerDaemon(t, api, "test-provisioner", nil, nicloud.MemoryProvisionerWithHeartbeatOverride(func(ctx context.Context) error {
...
func TestWorkspaceByOwnerAndName(t *testing.T) {
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		_, err := client.WorkspaceByOwnerAndName(ctx, nicloudsdk.Me, "something", nicloudsdk.WorkspaceOptions{})
		var apiErr *nicloudsdk.Error
		require.ErrorAs(t, err, &apiErr)
	...
	t.Run("Get", func(t *testing.T) {
		...
		_, err := client.WorkspaceByOwnerAndName(ctx, nicloudsdk.Me, workspace.Name, nicloudsdk.WorkspaceOptions{})
		require.NoError(t, err)
	})
	t.Run("Deleted", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, nicloudsdk.WorkspaceOptions{})
		require.ErrorContains(t, err, "404")

		// Then:
		// When we call with includes_deleted, we should get the workspace back
		workspaceNew, err := client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, nicloudsdk.WorkspaceOptions{IncludeDeleted: true})
		require.NoError(t, err)
		require.Equal(t, workspace.ID, workspaceNew.ID)

		// Given:
		// We recreate the workspace with the same name
		workspace, err = client.CreateWorkspace(ctx, user.OrganizationID, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
		...
		workspaceNew, err = client.WorkspaceByOwnerAndName(ctx, workspace.OwnerName, workspace.Name, nicloudsdk.WorkspaceOptions{})
		require.NoError(t, err)
		require.Equal(t, workspace.ID, workspaceNew.ID)
...
func TestWorkspaceFilterAllStatus(t *testing.T) {
	...
	defer cancel()
	workspaces, err := client.Workspaces(apiCtx, nicloudsdk.WorkspaceFilter{})
	require.NoError(t, err)

	// Make sure all workspaces have the correct status
	var statuses []nicloudsdk.WorkspaceStatus
...
func TestWorkspaceFilter(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		userClient, user := nicloudtest.CreateAnotherUser(t, client, first.OrganizationID, rbac.RoleOwner())

		if i%3 == 0 {
			var err error
			user, err = client.UpdateUserProfile(ctx, user.ID.String(), nicloudsdk.UpdateUserProfileRequest{
				Username: strings.ToUpper(user.Username),
			})
			require.NoError(t, err, "uppercase username")
		}

		org, err := userClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
	...
	type madeWorkspace struct {
		Owner     nicloudsdk.User
		Workspace nicloudsdk.Workspace
		Template  nicloudsdk.Template
	}

	availTemplates := make([]nicloudsdk.Template, 0)
	...
	for i, user := range users {
		version := nicloudtest.CreateTemplateVersion(t, client, user.Org.ID, nil)
		...
		if i%3 == 0 {
			template = nicloudtest.CreateTemplate(t, client, user.Org.ID, version.ID, func(request *nicloudsdk.CreateTemplateRequest) {
				request.Name = strings.ToUpper(request.Name)
			})
			upperTemplates = append(upperTemplates, template.Name)
		} else {
			template = nicloudtest.CreateTemplate(t, client, user.Org.ID, version.ID)
		}

		availTemplates = append(availTemplates, template)
		workspace := nicloudtest.CreateWorkspace(t, user.Client, template.ID, func(request *nicloudsdk.CreateWorkspaceRequest) {
			if count%3 == 0 {
				request.Name = strings.ToUpper(request.Name)
			}
		})
		...
		randWorkspace := nicloudtest.CreateWorkspace(t, user.Client, randTemplate.ID)
		allWorkspaces = append(allWorkspaces, madeWorkspace{
	...
	for i, w := range allWorkspaces {
		latest := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, w.Workspace.LatestBuild.ID)
	...
	testCases := []struct {
		Name   string
		Filter nicloudsdk.WorkspaceFilter
		// If FilterF is true, we include it in the expected results
		FilterF func(f nicloudsdk.WorkspaceFilter, workspace madeWorkspace) bool
	}{
		{
			Name:   "All",
			Filter: nicloudsdk.WorkspaceFilter{},
			FilterF: func(_ nicloudsdk.WorkspaceFilter, _ madeWorkspace) bool {
				return true
			},
		},
		{
	...
	for _, c := range testCases {
		t.Run(c.Name, func(t *testing.T) {
			...
			exp := make([]nicloudsdk.Workspace, 0)
	...
	t.Run("Shared", func(t *testing.T) {
		...
		var (
			...
			_, workspaceOwner = nicloudtest.CreateAnotherUser(t, client, orgOwner.OrganizationID, rbac.ScopedRoleOrgAuditor(orgOwner.OrganizationID))
...
func TestWorkspaceFilterManual(t *testing.T) {
	...
	expectIDs := func(t *testing.T, exp []nicloudsdk.Workspace, got []nicloudsdk.Workspace) {
		t.Helper()
	...
	t.Run("Owner", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true})
		user := nicloudtest.CreateFirstUser(t, client)
		otherUser, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleOwner())
		version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)

		// Add a non-matching workspace
		nicloudtest.CreateWorkspace(t, otherUser, template.ID)
		...
		sdkUser, err := client.User(ctx, nicloudsdk.Me)
		require.NoError(t, err)
	...
	t.Run("IDs", func(t *testing.T) {
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		alpha := nicloudtest.CreateWorkspace(t, client, template.ID)
		bravo := nicloudtest.CreateWorkspace(t, client, template.ID)
		...
		require.True(t, slices.ContainsFunc(res.Workspaces, func(workspace nicloudsdk.Workspace) bool {
	...
	t.Run("Template", func(t *testing.T) {
		...
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		template2 := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version2.ID)
		workspace := nicloudtest.CreateWorkspace(t, client, template.ID)
		_ = nicloudtest.CreateWorkspace(t, client, template2.ID)
	...
	t.Run("Status", func(t *testing.T) {
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		workspace1 := nicloudtest.CreateWorkspace(t, client, template.ID)
		workspace2 := nicloudtest.CreateWorkspace(t, client, template.ID)

		// wait for workspaces to be "running"
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace1.LatestBuild.ID)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace2.LatestBuild.ID)

		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()

		// filter finds both running workspaces
		ws1, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
		require.NoError(t, err)
		require.Len(t, ws1.Workspaces, 2)

		// stop workspace1
		build1 := nicloudtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStop)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, build1.ID)

		// filter finds one running workspace
		ws2, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
			Status: "running",
		})
		require.NoError(t, err)
		...
		// stop workspace2
		build2 := nicloudtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStop)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, build2.ID)

		// filter finds no running workspaces
		ws3, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
			Status: "running",
		})
		require.NoError(t, err)
	...
	t.Run("Dormant", func(t *testing.T) {
		// this test has a licensed counterpart in enterprise/nicloud/workspaces_test.go: FilterQueryHasDeletingByAndLicensed
		t.Parallel()
		client, db := nicloudtest.NewWithDatabase(t, nil)
		user := nicloudtest.CreateFirstUser(t, client)
		...
		err := client.UpdateWorkspaceDormancy(ctx, dormantWorkspace.ID, nicloudsdk.UpdateWorkspaceDormancy{
			Dormant: true,
		})
		require.NoError(t, err)
	...
	t.Run("LastUsed", func(t *testing.T) {
		...
		now := dbtime.Now()
		before := nicloudtest.CreateWorkspace(t, client, template.ID)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, before.LatestBuild.ID)

		after := nicloudtest.CreateWorkspace(t, client, template.ID)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, after.LatestBuild.ID)
		...
		beforeRes, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
		...
		afterRes, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
	...
	t.Run("Updated", func(t *testing.T) {
		...
		newTv := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(request *nicloudsdk.CreateTemplateVersionRequest) {
			request.TemplateID = template.ID
		})
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, newTv.ID)
	...
	t.Run("HealthyFilter", func(t *testing.T) {
		...
		t.Run("Unhealthy", func(t *testing.T) {
			...
			res, err := client.Workspaces(testCtx, nicloudsdk.WorkspaceFilter{
	...
	t.Run("Params", func(t *testing.T) {
		...
		client := nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true})
		user := nicloudtest.CreateFirstUser(t, client)
		version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, makeParameters(&proto.RichParameter{Name: paramOptional, Description: "", Mutable: true, Type: "string"}))
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
		noOptionalVersion := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, makeParameters(), func(request *nicloudsdk.CreateTemplateVersionRequest) {
			request.TemplateID = template.ID
		})
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, noOptionalVersion.ID)

		// foo :: one=foo, two=bar, one=baz, optional=optional
		foo := nicloudtest.CreateWorkspace(t, client, uuid.Nil, func(request *nicloudsdk.CreateWorkspaceRequest) {
		...
		bar := nicloudtest.CreateWorkspace(t, client, uuid.Nil, func(request *nicloudsdk.CreateWorkspaceRequest) {
			request.TemplateVersionID = version.ID
			request.RichParameterValues = []nicloudsdk.WorkspaceBuildParameter{
				{
					Name:  paramOneName,
		...
		baz := nicloudtest.CreateWorkspace(t, client, uuid.Nil, func(request *nicloudsdk.CreateWorkspaceRequest) {
		...
		t.Run("has_param", func(t *testing.T) {
			...
			// all match
			all, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s", paramOneName),
			})
			require.NoError(t, err)
			expectIDs(t, []nicloudsdk.Workspace{foo, bar, baz}, all.Workspaces)

			// Some match
			optional, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s", paramOptional),
			})
			require.NoError(t, err)
			expectIDs(t, []nicloudsdk.Workspace{foo, bar}, optional.Workspaces)

			// None match
			none, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
		...
		t.Run("exact_param", func(t *testing.T) {
			...
			// Two match
			two, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s=%s", paramTwoName, "bar"),
			})
			require.NoError(t, err)
			expectIDs(t, []nicloudsdk.Workspace{foo, bar}, two.Workspaces)

			// Only 1 matches
			one, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
				FilterQuery: fmt.Sprintf("param:%s=%s", paramOneName, "foo"),
			})
			require.NoError(t, err)
			expectIDs(t, []nicloudsdk.Workspace{foo}, one.Workspaces)
		})
...
func TestOffsetLimit(t *testing.T) {
	...
	ws, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	require.NoError(t, err)
...
func TestWorkspaceUpdateAutostart(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			var (
				...
				workspace = nicloudtest.CreateWorkspace(t, client, project.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
			...
			err := client.UpdateWorkspaceAutostart(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceAutostartRequest{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		var (
			...
			wsid   = uuid.New()
			req    = nicloudsdk.UpdateWorkspaceAutostartRequest{
		...
		require.IsType(t, err, &nicloudsdk.Error{}, "expected nicloudsdk.Error")
		coderSDKErr, _ := err.(*nicloudsdk.Error) //nolint:errorlint
...
func TestWorkspaceUpdateTTL(t *testing.T) {
	...
	testCases := []struct {
		...
		ttlMillis      *int64
		expectedError  string
		modifyTemplate func(*nicloudsdk.CreateTemplateRequest)
	}{
		{
			name:          "disable ttl",
			ttlMillis:     nil,
			expectedError: "",
			modifyTemplate: func(ctr *nicloudsdk.CreateTemplateRequest) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			mutators := make([]func(*nicloudsdk.CreateTemplateRequest), 0)
			...
			var (
				...
				version   = nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
				_         = nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
				project   = nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID, mutators...)
				workspace = nicloudtest.CreateWorkspace(t, client, project.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
					cwr.AutostartSchedule = nil
					cwr.TTLMillis = nil
				})
			...
			err := client.UpdateWorkspaceTTL(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceTTLRequest{
	...
	t.Run("ModifyAutostopWithRunningWorkspace", func(t *testing.T) {
		...
		testCases := []struct {
			...
			fromTTL     *int64
			toTTL       *int64
			afterUpdate func(t *testing.T, before, after nicloudsdk.NullTime)
		}{
			{
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		var (
			...
			_      = nicloudtest.CreateFirstUser(t, client)
			wsid   = uuid.New()
			req    = nicloudsdk.UpdateWorkspaceTTLRequest{
				TTLMillis: ptr.Ref(time.Hour.Milliseconds()),
			}
		)
...
func TestWorkspaceExtend(t *testing.T) {
	...
	req := nicloudsdk.PutExtendWorkspaceRequest{
	...
	err = client.PutExtendWorkspace(ctx, workspace.ID, nicloudsdk.PutExtendWorkspaceRequest{
		Deadline: time.Time{},
	})
...
func TestWorkspaceUpdateAutomaticUpdates_OK(t *testing.T) {
	...
	var (
		auditor      = audit.NewMock()
		adminClient  = nicloudtest.New(t, &nicloudtest.Options{IncludeProvisionerDaemon: true, Auditor: auditor})
		admin        = nicloudtest.CreateFirstUser(t, adminClient)
		client, user = nicloudtest.CreateAnotherUser(t, adminClient, admin.OrganizationID)
		version      = nicloudtest.CreateTemplateVersion(t, adminClient, admin.OrganizationID, nil)
		_            = nicloudtest.AwaitTemplateVersionJobCompleted(t, adminClient, version.ID)
		project      = nicloudtest.CreateTemplate(t, adminClient, admin.OrganizationID, version.ID)
		workspace    = nicloudtest.CreateWorkspace(t, client, project.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = nil
			cwr.TTLMillis = nil
			cwr.AutomaticUpdates = nicloudsdk.AutomaticUpdatesNever
		})
	)
	...
	require.Equal(t, nicloudsdk.AutomaticUpdatesNever, workspace.AutomaticUpdates, "expected newly-minted workspace to automatic updates set to never")

	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
	defer cancel()

	err := client.UpdateWorkspaceAutomaticUpdates(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceAutomaticUpdatesRequest{
		AutomaticUpdates: nicloudsdk.AutomaticUpdatesAlways,
	})
	require.NoError(t, err)

	updated, err := client.Workspace(ctx, workspace.ID)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.AutomaticUpdatesAlways, updated.AutomaticUpdates)
...
func TestUpdateWorkspaceAutomaticUpdates_NotFound(t *testing.T) {
	...
	var (
		...
		_      = nicloudtest.CreateFirstUser(t, client)
		wsid   = uuid.New()
		req    = nicloudsdk.UpdateWorkspaceAutomaticUpdatesRequest{
...
func TestWorkspaceWatcher(t *testing.T) {
	t.Parallel()
	client, closeFunc := nicloudtest.NewWithProvisionerCloser(t, &nicloudtest.Options{
		IncludeProvisionerDaemon: true,
		AllowWorkspaceRenames:    true,
	})
	...
	wait := func(event string, ready func(w nicloudsdk.Workspace) bool) {
		for {
			select {
			case <-ctx.Done():
	...
	nicloudtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	...
	//
	// Note that the first timeout is from `nicloudtest.CreateWorkspace` and
	// the latter is from `nicloudtest.CreateWorkspaceBuild`.
	...
	wait("agent connected/ready", func(w nicloudsdk.Workspace) bool {
		return w.LatestBuild.Resources[0].Agents[0].Status == nicloudsdk.WorkspaceAgentConnected &&
			w.LatestBuild.Resources[0].Agents[0].LifecycleState == nicloudsdk.WorkspaceAgentLifecycleReady
	})
	agt.Close()
	wait("agent disconnected", func(w nicloudsdk.Workspace) bool {
		return w.LatestBuild.Resources[0].Agents[0].Status == nicloudsdk.WorkspaceAgentDisconnected
	})

	err = client.UpdateWorkspace(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceRequest{
		Name: "another",
	})
	require.NoError(t, err)
	...
	badVersion := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse:          echo.ParseComplete,
		ProvisionPlan:  echo.PlanComplete,
	...
	}, func(req *nicloudsdk.CreateTemplateVersionRequest) {
		req.TemplateID = template.ID
	})
	nicloudtest.AwaitTemplateVersionJobCompleted(t, client, badVersion.ID)
	err = client.UpdateActiveTemplateVersion(ctx, template.ID, nicloudsdk.UpdateActiveTemplateVersion{
		ID: badVersion.ID,
	})
	require.NoError(t, err)
	...
	_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart, func(req *nicloudsdk.CreateWorkspaceBuildRequest) {
	...
	sawFailed := false
	wait("workspace build pending or failed", func(w nicloudsdk.Workspace) bool {
	...
	if !sawFailed {
		wait("workspace build failed", func(w nicloudsdk.Workspace) bool {
			return w.LatestBuild.Status == nicloudsdk.WorkspaceStatusFailed
		})
	}
...
func TestWatchAllWorkspaceBuilds(t *testing.T) {
	...
	client, closer := nicloudtest.NewWithProvisionerCloser(t, &nicloudtest.Options{
		IncludeProvisionerDaemon: true,
	...
	waitForUpdate := func(event string, workspaceID uuid.UUID, expectedTransition, expectedStatus string) nicloudsdk.WorkspaceBuildUpdate {
		t.Helper()
		for {
			select {
			case <-ctx.Done():
				require.FailNow(t, "timed out waiting for event", event)
				return nicloudsdk.WorkspaceBuildUpdate{}
	...
	// Stop workspace 1.
	_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStop)
	update = waitForUpdate("workspace1 stop", workspace1.ID, "stop", "succeeded")
	require.Equal(t, workspace1.ID, update.WorkspaceID)

	// Stop workspace 2.
	_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStop)
	update = waitForUpdate("workspace2 stop", workspace2.ID, "stop", "succeeded")
	require.Equal(t, workspace2.ID, update.WorkspaceID)

	// Start workspace 1 again.
	_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace1, database.WorkspaceTransitionStart)
	update = waitForUpdate("workspace1 start", workspace1.ID, "start", "succeeded")
	require.Equal(t, workspace1.ID, update.WorkspaceID)

	// Start workspace 2 again.
	_ = nicloudtest.CreateWorkspaceBuild(t, client, workspace2, database.WorkspaceTransitionStart)
...
func TestWorkspaceResource(t *testing.T) {
	...
	t.Run("Apps", func(t *testing.T) {
		...
		require.EqualValues(t, nicloudsdk.WorkspaceAppHealthDisabled, got.Health)
		...
		require.EqualValues(t, app.Icon, got.Icon)
		require.EqualValues(t, app.DisplayName, got.DisplayName)
		require.EqualValues(t, nicloudsdk.WorkspaceAppHealthInitializing, got.Health)
	...
	t.Run("Metadata", func(t *testing.T) {
		...
		require.Equal(t, []nicloudsdk.WorkspaceResourceMetadata{{
			Key:   "foo",
			Value: "bar",
		}, {
			Key: "empty",
...
func TestWorkspaceWithRichParameters(t *testing.T) {
	...
	const (
		...
		secondParameterValidationMonotonic = nicloudsdk.MonotonicOrderIncreasing
	...
	require.Equal(t, nicloudsdk.ValidationMonotonicOrder(""), templateRichParameters[0].ValidationMonotonic) // no validation for string
	...
	expectedBuildParameters := []nicloudsdk.WorkspaceBuildParameter{
	...
	workspaceBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
	require.Equal(t, nicloudsdk.WorkspaceStatusRunning, workspaceBuild.Status)
...
func TestWorkspaceWithMultiSelectFailure(t *testing.T) {
	...
	_, err = client.CreateUserWorkspace(context.Background(), nicloudsdk.Me, req)
	require.Error(t, err)
	var apiError *nicloudsdk.Error
	require.ErrorAs(t, err, &apiError)
...
func TestWorkspaceWithOptionalRichParameters(t *testing.T) {
	...
	workspace := nicloudtest.CreateWorkspace(t, client, template.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
		cwr.RichParameterValues = []nicloudsdk.WorkspaceBuildParameter{
			// First parameter is optional, so neuralinverse will pick the default value.
			{Name: secondParameterName, Value: secondParameterValue},
		}
	})
...
func TestWorkspaceWithEphemeralRichParameters(t *testing.T) {
	...
	workspaceBuild, err = client.CreateWorkspaceBuild(ctx, workspaceBuild.WorkspaceID, nicloudsdk.CreateWorkspaceBuildRequest{
		Transition: nicloudsdk.WorkspaceTransitionStart,
		RichParameterValues: []nicloudsdk.WorkspaceBuildParameter{
			{
				Name:  ephemeralParameterName,
	...
	require.NoError(t, err)
	workspaceBuild = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceBuild.ID)
	require.Equal(t, nicloudsdk.WorkspaceStatusRunning, workspaceBuild.Status)
...
func TestWorkspaceDormant(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		err := client.UpdateWorkspaceDormancy(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceDormancy{
			Dormant: true,
		})
		require.NoError(t, err)
	...
	t.Run("StartWakesUpDormantWorkspace", func(t *testing.T) {
		...
		nicloudtest.MustTransitionWorkspace(t, client, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
		...
		wb, err := client.CreateWorkspaceBuild(ctx, workspace.ID, nicloudsdk.CreateWorkspaceBuildRequest{
			TemplateVersionID: template.ActiveVersionID,
			Transition:        nicloudsdk.WorkspaceTransition(database.WorkspaceTransitionStart),
		})
		require.NoError(t, err)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, wb.ID)
...
func TestWorkspaceUsageTracking(t *testing.T) {
	...
	t.Run("NoExperiment", func(t *testing.T) {
		...
		require.NoError(t, err)
		err = client.PostWorkspaceUsageWithBody(ctx, r.Workspace.ID, nicloudsdk.PostWorkspaceUsageRequest{})
		require.NoError(t, err)
	})
	...
	t.Run("Dormant", func(t *testing.T) {
		...
		t.Run("InitiatorNotOwner", func(t *testing.T) {
			...
			var (
				...
				user            = nicloudtest.CreateFirstUser(t, client)
				memberClient, _ = nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleOwner())
				version         = nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
				_               = nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
				template        = nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID)
				workspace       = nicloudtest.CreateWorkspace(t, client, template.ID)
...
func TestWorkspaceTimings(t *testing.T) {
	...
	t.Run("LatestBuild", func(t *testing.T) {
		...
		for _, script := range workspace.LatestBuild.Resources[0].Agents[0].Scripts {
			...
			require.Equal(t, *script.Status, nicloudsdk.WorkspaceAgentScriptStatus(timing.Status))
		}
	})
...
func TestOIDCRemoved(t *testing.T) {
	...
	owner, db := nicloudtest.NewWithDatabase(t, &nicloudtest.Options{
		IncludeProvisionerDaemon: true,
	})
	first := nicloudtest.CreateFirstUser(t, owner)

	user, userData := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.ScopedRoleOrgAdmin(first.OrganizationID))
	...
	version := nicloudtest.CreateTemplateVersion(t, owner, first.OrganizationID, nil)
	_ = nicloudtest.AwaitTemplateVersionJobCompleted(t, owner, version.ID)
	template := nicloudtest.CreateTemplate(t, owner, first.OrganizationID, version.ID)

	wrk := nicloudtest.CreateWorkspace(t, user, template.ID)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, owner, wrk.LatestBuild.ID)

	deleteBuild, err := owner.CreateWorkspaceBuild(ctx, wrk.ID, nicloudsdk.CreateWorkspaceBuildRequest{
		Transition: nicloudsdk.WorkspaceTransitionDelete,
	})
	require.NoError(t, err, "delete the workspace")
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, owner, deleteBuild.ID)
}
...
func TestWorkspaceListTasks(t *testing.T) {
	...
	workspaceWithoutTask, err := client.CreateUserWorkspace(ctx, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
		TemplateID: template.ID,
		Name:       "user-workspace",
	})
	require.NoError(t, err)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceWithoutTask.LatestBuild.ID)

	// Given: a workspace associated with a task
	task, err := client.CreateTask(ctx, nicloudsdk.Me, nicloudsdk.CreateTaskRequest{
		TemplateVersionID: template.ActiveVersionID,
	...
	require.NoError(t, err)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspaceWithTask.LatestBuild.ID)
...
func TestWorkspaceAppUpsertRestart(t *testing.T) {
	...
	// Stop the workspace
	stopBuild := nicloudtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStop)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, stopBuild.ID)

	// Restart the workspace (this will trigger upsert for the app)
	startBuild := nicloudtest.CreateWorkspaceBuild(t, client, workspace, database.WorkspaceTransitionStart)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, startBuild.ID)

	// Verify the workspace restarted successfully
	workspace, err = client.Workspace(ctx, workspace.ID)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.WorkspaceStatusRunning, workspace.LatestBuild.Status)
...
func TestUpdateWorkspaceACL(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		_, friend := nicloudtest.CreateAnotherUser(t, adminClient, orgID)

		tv := nicloudtest.CreateTemplateVersion(t, adminClient, orgID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, adminClient, tv.ID)
		template := nicloudtest.CreateTemplate(t, adminClient, orgID, tv.ID)
		...
		err := client.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				friend.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)
		...
		require.Equal(t, workspaceACL.Users[0].Role, nicloudsdk.WorkspaceRoleAdmin)
	})
	...
	t.Run("DeletedUser", func(t *testing.T) {
		...
		orgID := adminUser.OrganizationID
		client, _ := nicloudtest.CreateAnotherUser(t, adminClient, orgID)
		_, mike := nicloudtest.CreateAnotherUser(t, adminClient, orgID)
		...
		err = client.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				mike.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
			},
		})
		require.Error(t, err)
		cerr, ok := nicloudsdk.AsError(err)
	...
	t.Run("CannotChangeOwnRole", func(t *testing.T) {
		...
		adminUser := nicloudtest.CreateFirstUser(t, adminClient)
		orgID := adminUser.OrganizationID
		workspaceOwnerClient, workspaceOwner := nicloudtest.CreateAnotherUser(t, adminClient, orgID)
		sharedAdminClient, sharedAdminUser := nicloudtest.CreateAnotherUser(t, adminClient, orgID)

		tv := nicloudtest.CreateTemplateVersion(t, adminClient, orgID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, adminClient, tv.ID)
		template := nicloudtest.CreateTemplate(t, adminClient, orgID, tv.ID)

		ws := nicloudtest.CreateWorkspace(t, workspaceOwnerClient, template.ID)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, workspaceOwnerClient, ws.LatestBuild.ID)
		...
		err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				sharedAdminUser.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)

		// The shared admin user should not be able to change their own role.
		err = sharedAdminClient.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				sharedAdminUser.ID.String(): nicloudsdk.WorkspaceRoleUse,
			},
		})
		require.Error(t, err)
		cerr, ok := nicloudsdk.AsError(err)
		...
		err = workspaceOwnerClient.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				workspaceOwner.ID.String(): nicloudsdk.WorkspaceRoleUse,
			},
		})
		require.Error(t, err)
		cerr, ok = nicloudsdk.AsError(err)
...
func TestDeleteWorkspaceACL(t *testing.T) {
	...
	t.Run("WorkspaceOwnerCanDelete", func(t *testing.T) {
		...
		var (
			...
			workspaceOwnerClient, workspaceOwner = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
			_, toShareWithUser                   = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		...
		err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				toShareWithUser.ID.String(): nicloudsdk.WorkspaceRoleUse,
			},
		})
	...
	t.Run("SharedUsersCannot", func(t *testing.T) {
		...
		var (
			...
			admin                                = nicloudtest.CreateFirstUser(t, client)
			workspaceOwnerClient, workspaceOwner = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
			sharedUseClient, toShareWithUser     = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
			workspace                            = dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
				OwnerID:        workspaceOwner.ID,
				OrganizationID: admin.OrganizationID,
			}).Do().Workspace
...
func TestWorkspaceReadCanListACL(t *testing.T) {
	...
	var (
		...
		admin                                = nicloudtest.CreateFirstUser(t, client)
		workspaceOwnerClient, workspaceOwner = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		sharedUserClientA, sharedUserA       = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		_, sharedUserB                       = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
	...
	err := workspaceOwnerClient.UpdateWorkspaceACL(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceACL{
		UserRoles: map[string]nicloudsdk.WorkspaceRole{
			sharedUserA.ID.String(): nicloudsdk.WorkspaceRoleUse,
			sharedUserB.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
		},
		GroupRoles: map[string]nicloudsdk.WorkspaceRole{
			sharedGroup.ID.String(): nicloudsdk.WorkspaceRoleUse,
		},
	})
	require.NoError(t, err)
	...
	gotRoles := make(map[uuid.UUID]nicloudsdk.WorkspaceRole, len(acl.Users))
	for _, u := range acl.Users {
		gotRoles[u.ID] = u.Role
	}
	require.Equal(t, nicloudsdk.WorkspaceRoleUse, gotRoles[sharedUserA.ID])
	require.Equal(t, nicloudsdk.WorkspaceRoleAdmin, gotRoles[sharedUserB.ID])

	gotGroupRoles := make(map[uuid.UUID]nicloudsdk.WorkspaceRole, len(acl.Groups))
	for _, g := range acl.Groups {
		gotGroupRoles[g.ID] = g.Role
	}
	require.Equal(t, nicloudsdk.WorkspaceRoleUse, gotGroupRoles[sharedGroup.ID])
}
...
func TestWorkspaceSharingDisabled(t *testing.T) {
	t.Run("CanAccessWhenEnabled", func(t *testing.T) {
		var (
			...
			admin            = nicloudtest.CreateFirstUser(t, client)
			_, wsOwner       = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
			userClient, user = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		)
...
func TestWorkspaceAvailableUsers(t *testing.T) {
	...
	t.Run("OrgAdminCanListUsers", func(t *testing.T) {
		...
		_, user1 := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
		_, user2 := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
...
func TestWorkspaceCreateWithImplicitPreset(t *testing.T) {
	...
	createTemplateWithPresets := func(t *testing.T, client *nicloudsdk.Client, user nicloudsdk.CreateFirstUserResponse, presets []*proto.Preset) (nicloudsdk.Template, nicloudsdk.TemplateVersion) {
		version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
			Parse: echo.ParseComplete,
			ProvisionGraph: []*proto.Response{
				{
	...
	createWorkspaceAndVerifyPreset := func(t *testing.T, client *nicloudsdk.Client, template nicloudsdk.Template, expectedPresetID *uuid.UUID, params []nicloudsdk.WorkspaceBuildParameter) nicloudsdk.Workspace {
		wsName := testutil.GetRandomNameHyphenated(t)
		var ws nicloudsdk.Workspace
		if len(params) > 0 {
	...
	t.Run("NoPresets", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, nil, []nicloudsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})
	})
	...
	t.Run("SinglePresetNoParameters", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, &presetID, []nicloudsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})
	})
	...
	t.Run("MultiplePresets", func(t *testing.T) {
		...
		createWorkspaceAndVerifyPreset(t, client, template, &singleParamPresetID, []nicloudsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
		})

		// Test workspace creation with multiple parameters - should match multi param preset
		createWorkspaceAndVerifyPreset(t, client, template, &multiParamPresetID, []nicloudsdk.WorkspaceBuildParameter{
			{Name: "param1", Value: "value1"},
			{Name: "param2", Value: "value2"},
		})

		// Test workspace creation with non-matching parameters - should not match any preset
		createWorkspaceAndVerifyPreset(t, client, template, &emptyPresetID, []nicloudsdk.WorkspaceBuildParameter{
	...
	t.Run("PresetSpecifiedExplicitly", func(t *testing.T) {
		...
		ws2 := nicloudtest.CreateWorkspace(t, client, template.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
...
func TestProvisionerJobQueueWaitMetric(t *testing.T) {
	...
	importMetric := promhelp.MetricValue(t, reg, "nicloud_provisioner_job_queue_wait_seconds", prometheus.Labels{
	...
	buildMetric := promhelp.MetricValue(t, reg, "nicloud_provisioner_job_queue_wait_seconds", prometheus.Labels{
		"provisioner_type": string(database.ProvisionerTypeEcho),
...
func TestWorkspaceBuildsEnqueuedMetric(t *testing.T) {
	...
	workspace = nicloudtest.MustTransitionWorkspace(t, client, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
	...
	p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, map[string]string{})
	require.NoError(t, err)

	go func() {
		tickTime := sched.Next(workspace.LatestBuild.CreatedAt)
		nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, tickTime)
		tickCh <- tickTime
		close(tickCh)
	}()
	...
	workspace = nicloudtest.MustWorkspace(t, client, workspace.ID)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
	require.Equal(t, nicloudsdk.BuildReasonAutostart, workspace.LatestBuild.Reason)

	// Now check the autostart metric was recorded.
	autostartCount := promhelp.CounterValue(t, reg, "nicloud_workspace_builds_enqueued_total", prometheus.Labels{
		"provisioner_type": string(database.ProvisionerTypeEcho),
