...
import (
	...
	provProto "github.com/NeuralInverse/cloud/v2/provisionerd/proto"
...
func TestDynamicParametersWithTerraformValues(t *testing.T) {
	...
	t.Run("OK_Modules", func(t *testing.T) {
		...
		nicloudtest.AssertParameter(t, "jetbrains_ide", preview.Parameters).
			Exists().Value("CL")
		nicloudtest.AssertParameter(t, "region", preview.Parameters).
			Exists().Value("na")
	})
	...
	t.Run("RebuildParameters", func(t *testing.T) {
		...
		wrk := nicloudtest.CreateWorkspace(t, setup.client, setup.template.ID, func(request *nicloudsdk.CreateWorkspaceRequest) {
		...
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, setup.client, wrk.LatestBuild.ID)

		params, err := setup.client.WorkspaceBuildParameters(ctx, wrk.LatestBuild.ID)
		require.NoError(t, err)
		require.ElementsMatch(t, []nicloudsdk.WorkspaceBuildParameter{
		...
		doTransition := func(t *testing.T, trans nicloudsdk.WorkspaceTransition) {
			t.Helper()

			regionVal := regionOptions[0]
			regionOptions = regionOptions[1:] // Choose the next region on the next build

			bld, err := setup.client.CreateWorkspaceBuild(ctx, wrk.ID, nicloudsdk.CreateWorkspaceBuildRequest{
			...
			require.NoError(t, err)
			nicloudtest.AwaitWorkspaceBuildJobCompleted(t, setup.client, bld.ID)

			latestParams, err := setup.client.WorkspaceBuildParameters(ctx, bld.ID)
			require.NoError(t, err)
			require.ElementsMatch(t, latestParams, []nicloudsdk.WorkspaceBuildParameter{
		...
		doTransition(t, nicloudsdk.WorkspaceTransitionStop)
		doTransition(t, nicloudsdk.WorkspaceTransitionStart)
		doTransition(t, nicloudsdk.WorkspaceTransitionDelete)
	})
	...
	t.Run("TemplateVariables", func(t *testing.T) {
		...
		setup := setupDynamicParamsTest(t, setupDynamicParamsTestParams{
			provisionerDaemonVersion: provProto.CurrentVersion.String(),
			mainTF:                   dynamicParametersTerraformSource,
			variables: []nicloudsdk.TemplateVersionVariable{
		...
		require.Len(t, preview.Parameters, 1)
		nicloudtest.AssertParameter(t, "variable_values", preview.Parameters).
			Exists().Value("austin")
	})
}
...
type dynamicParamsTest struct {
	...
	api      *nicloud.API
	stream   *wsjson.Stream[nicloudsdk.DynamicParametersResponse, nicloudsdk.DynamicParametersRequest]
	template nicloudsdk.Template
}

func setupDynamicParamsTest(t *testing.T, args setupDynamicParamsTestParams) dynamicParamsTest {
	ownerClient, _, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
		Database:                 args.db,
		Pubsub:                   args.ps,
	...
	tpl, version := nicloudtest.DynamicParameterTemplate(t, templateAdmin, owner.OrganizationID, nicloudtest.DynamicParameterTemplateParams{
