...
func TestTokenCRUD(t *testing.T) {
	...
	res, err := client.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{})
	require.NoError(t, err)
	...
	require.Equal(t, nicloudsdk.APIKeyScopeAll, keys[0].Scope)
	require.Len(t, keys[0].AllowList, 1)
	require.Equal(t, "*:*", keys[0].AllowList[0].String())

	// no update

	err = client.DeleteAPIKey(ctx, nicloudsdk.Me, keys[0].ID)
	require.NoError(t, err)
...
func TestTokensFilterExpired(t *testing.T) {
	...
	_ = nicloudtest.CreateFirstUser(t, adminClient)

	// Create a token.
	res, err := adminClient.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{
	...
	keys, err := adminClient.Tokens(ctx, nicloudsdk.Me, nicloudsdk.TokensFilter{})
	require.NoError(t, err)
	require.Len(t, keys, 1)

	// Expire the token.
	err = adminClient.ExpireAPIKey(ctx, nicloudsdk.Me, keyID)
	require.NoError(t, err)
...
func TestTokenScoped(t *testing.T) {
	...
	require.EqualValues(t, len(keys), 1)
	require.Contains(t, res.Key, keys[0].ID)
	require.Equal(t, keys[0].Scope, nicloudsdk.APIKeyScopeApplicationConnect)
	require.Len(t, keys[0].AllowList, 1)
	require.Equal(t, "*:*", keys[0].AllowList[0].String())
}
...
func TestTokenLegacySingularScopeCompat(t *testing.T) {
	...
	cases := []struct {
		...
		scopes []nicloudsdk.APIKeyScope
	}{
		{
			name:   "all",
			scope:  nicloudsdk.APIKeyScopeAll,
			scopes: []nicloudsdk.APIKeyScope{nicloudsdk.APIKeyScopeNIAll},
		},
		{
			name:   "application_connect",
			scope:  nicloudsdk.APIKeyScopeApplicationConnect,
			scopes: []nicloudsdk.APIKeyScope{nicloudsdk.APIKeyScopeNIApplicationConnect},
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := client.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{
				Scope: tc.scope,
			})
			require.NoError(t, err)
...
func TestTokenAdminSetMaxLifetime(t *testing.T) {
	...
	nonAdminClient, _ := nicloudtest.CreateAnotherUser(t, client, adminUser.OrganizationID)
	...
	_, err = nonAdminClient.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{
		Lifetime: time.Hour * 24 * 8,
	})
	require.Error(t, err)
	require.Contains(t, err.Error(), "lifetime must be less")
...
func TestTokenCustomDefaultLifetime(t *testing.T) {
	...
	tokens, err := client.Tokens(ctx, nicloudsdk.Me, nicloudsdk.TokensFilter{})
	require.NoError(t, err)
...
func TestSessionExpiry(t *testing.T) {
	...
	userClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
...
func TestSessionCookieMaxAge(t *testing.T) {
	...
	res, err := client.Request(ctx, http.MethodPost, loginURL.String(), nicloudsdk.LoginWithPasswordRequest{
		Email:    req.Email,
...
func TestAPIKey_OK(t *testing.T) {
	...
	res, err := client.CreateAPIKey(ctx, nicloudsdk.Me)
	require.NoError(t, err)
	require.Greater(t, len(res.Key), 2)
...
func TestAPIKey_Deleted(t *testing.T) {
	...
	_, anotherUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
...
func TestAPIKey_PrebuildsNotAllowed(t *testing.T) {
	...
	_, err = client.CreateToken(setupCtx, database.PrebuildsSystemUserID.String(), nicloudsdk.CreateTokenRequest{})
	// Then: also denied.
	require.ErrorContains(t, err, httpapi.ResourceForbiddenResponse.Message)
}

//nolint:tparallel,paralleltest // Subtests share the same nicloudtest instance and auditor.
func TestExpireAPIKey(t *testing.T) {
	...
	memberClient, member := nicloudtest.CreateAnotherUser(t, adminClient, admin.OrganizationID)
	...
	t.Run("OwnerCanExpireOwnToken", func(t *testing.T) {
		...
		key, err := adminClient.APIKeyByID(ctx, nicloudsdk.Me, keyID)
		require.NoError(t, err)
	...
	t.Run("AdminCanExpireOtherUsersToken", func(t *testing.T) {
		...
		res, err := memberClient.CreateToken(ctx, nicloudsdk.Me, nicloudsdk.CreateTokenRequest{
			Lifetime: time.Hour * 24 * 7,
		})
		require.NoError(t, err)
		keyID := strings.Split(res.Key, "-")[0]
		...
		// Verify the token is expired.
		key, err := memberClient.APIKeyByID(ctx, nicloudsdk.Me, keyID)
		require.NoError(t, err)
		require.True(t, key.ExpiresAt.Before(dbtime.Now()))
	})
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		err := adminClient.ExpireAPIKey(ctx, nicloudsdk.Me, "nonexistent")
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusNotFound, sdkErr.StatusCode())
	...
	t.Run("ExpiringAlreadyExpiredTokenSucceeds", func(t *testing.T) {
		...
		keyAgain, err := adminClient.APIKeyByID(ctx, nicloudsdk.Me, keyID)
		require.NoError(t, err)
	...
	t.Run("DeletingExpiredTokenSucceeds", func(t *testing.T) {
		...
		err = adminClient.DeleteAPIKey(ctx, nicloudsdk.Me, keyID)
		require.NoError(t, err)

		// Verify it's gone.
		_, err = adminClient.APIKeyByID(ctx, nicloudsdk.Me, keyID)
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusNotFound, sdkErr.StatusCode())
