...
// @Success 200 {object} nicloudsdk.ExternalAuth
...
func (api *API) externalAuthByID(w http.ResponseWriter, r *http.Request) {
	...
	res := nicloudsdk.ExternalAuth{
		Authenticated:    false,
		Device:           config.DeviceAuth != nil,
		AppInstallURL:    config.AppInstallURL,
		DisplayName:      config.DisplayName,
		AppInstallations: []nicloudsdk.ExternalAuthAppInstallation{},
	}
	...
	if res.AppInstallations == nil {
		res.AppInstallations = []nicloudsdk.ExternalAuthAppInstallation{}
	}
	httpapi.Write(ctx, w, http.StatusOK, res)
}

// deleteExternalAuthByID only deletes the link on the Neural Inverse Cloud side, does not revoke the token on the provider side.
//
...
// @Produce json
// @Param externalauth path string true "Git Provider ID" format(string)
// @Success 200 {object} nicloudsdk.DeleteExternalAuthByIDResponse
...
func (api *API) deleteExternalAuthByID(w http.ResponseWriter, r *http.Request) {
	...
	resp := nicloudsdk.DeleteExternalAuthByIDResponse{TokenRevoked: ok}
...
func (api *API) postExternalAuthDeviceByID(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.ExternalAuthDeviceExchange
...
// @Produce json
// @Tags Git
// @Success 200 {object} nicloudsdk.ExternalAuthLink
...
func (api *API) listUserExternalAuths(rw http.ResponseWriter, r *http.Request) {
	...
	// call.
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.ListUserExternalAuthResponse{
		Providers: ExternalAuthConfigs(api.ExternalAuthConfigs),
		Links:     db2sdk.ExternalAuths(links, linkMeta),
	})
}

func ExternalAuthConfigs(auths []*externalauth.Config) []nicloudsdk.ExternalAuthLinkProvider {
	out := make([]nicloudsdk.ExternalAuthLinkProvider, 0, len(auths))
...
func ExternalAuthConfig(cfg *externalauth.Config) nicloudsdk.ExternalAuthLinkProvider {
	return nicloudsdk.ExternalAuthLinkProvider{
		ID:                            cfg.ID,
		Type:                          cfg.Type,
