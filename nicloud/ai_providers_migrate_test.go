...
func TestSeedAIProvidersFromEnv(t *testing.T) {
	...
	t.Run("EmptyConfigNoOp", func(t *testing.T) {
		...
		err := nicloud.SeedAIProvidersFromEnv(ctx, db, nicloudsdk.AIBridgeConfig{}, testLogger(t))
	...
	t.Run("LegacyOpenAI", func(t *testing.T) {
		...
		cfg := nicloudsdk.AIBridgeConfig{
			LegacyOpenAI: nicloudsdk.AIBridgeOpenAIConfig{
				BaseURL: serpent.String("https://api.openai.com/v1"),
				Key:     serpent.String("sk-legacy"),
			},
		}
		var firstSeedLogs bytes.Buffer
		err := nicloud.SeedAIProvidersFromEnv(ctx, db, cfg, capturedLogger(&firstSeedLogs))
		require.NoError(t, err)
		...
		err = nicloud.SeedAIProvidersFromEnv(ctx, db, cfg, capturedLogger(&rerunLogs))
		require.NoError(t, err)
	...
	t.Run("DriftFailsStartup", func(t *testing.T) {
		...
		require.NoError(t, nicloud.SeedAIProvidersFromEnv(ctx, db, cfg, testLogger(t)))
		...
		err := nicloud.SeedAIProvidersFromEnv(ctx, db, cfg, testLogger(t))
		require.Error(t, err)
