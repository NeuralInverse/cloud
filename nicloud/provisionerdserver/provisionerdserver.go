...
func (s *server) acquireProtoJob(ctx context.Context, job database.ProvisionerJob) (*proto.AcquiredJob, error) {
	...
	case database.ProvisionerJobTypeWorkspaceBuild:
		...
		if input.PrebuiltWorkspaceBuildStage == sdkproto.PrebuiltWorkspaceBuildStage_CLAIM {
			...
			// the provisioner (and ultimately to the `ni_agent` resource in the Terraform provider) where they will be
...
func (s *server) includeLastVariableValues(ctx context.Context, templateVersionID uuid.UUID, userVariableValues []nicloudsdk.VariableValue) ([]nicloudsdk.VariableValue, error) {
	var values []nicloudsdk.VariableValue
	...
	for _, templateVariable := range templateVariables {
		...
		values = append(values, nicloudsdk.VariableValue{
...
func (s *server) FailJob(ctx context.Context, failJob *proto.FailedJob) (*proto.Empty, error) {
	...
	case *proto.FailedJob_WorkspaceBuild_:
		...
		if s.Experiments.Enabled(nicloudsdk.ExperimentWorkspaceBuildUpdates) {
			err = wspubsub.PublishWorkspaceBuildUpdate(ctx, s.Pubsub, nicloudsdk.WorkspaceBuildUpdate{
				WorkspaceID:   workspace.ID,
				WorkspaceName: workspace.Name,
				BuildID:       build.ID,
...
func (s *server) prepareForNotifyWorkspaceManualBuildFailed(ctx context.Context, workspace database.Workspace, build database.WorkspaceBuild) ([]database.GetUsersRow,
	...
	users, err := s.Database.GetUsers(ctx, database.GetUsersParams{
		RbacRole: []string{nicloudsdk.RoleTemplateAdmin},
	})
	if err != nil {
...
func (s *server) CompleteJob(ctx context.Context, completed *proto.CompletedJob) (*proto.Empty, error) {
	...
	default:
		...
		return nil, xerrors.Errorf("unknown job type %q; ensure nicloud and provisionerd versions match",
...
func (s *server) completeWorkspaceBuildJob(ctx context.Context, job database.ProvisionerJob, jobID uuid.UUID, jobType *proto.CompletedJob_WorkspaceBuild_, telemetrySnapshot *telemetry.Snapshot) error {
	...
	err = s.Database.InTx(func(db database.Store) error {
		...
		if unknownAppID != "" && workspaceBuild.Transition == database.WorkspaceTransitionStart {
			...
			// 2. Count=0 on the agent due to stop transition, meaning the associated ni_app was not inserted.
			...
			if _, err := db.InsertProvisionerJobLogs(ctx, database.InsertProvisionerJobLogsParams{
				...
				Output: []string{
					...
					"  - You have associated the coder_ai_task with a valid ni_app in your template (ref: https://registry.terraform.io/providers/coder/coder/latest/docs/resources/ai_task).",
					"  - You have associated the ni_agent with at least one other compute resource. Agents with no other associated resources are not inserted into the database.",
				},
			}); err != nil {
		...
		_, hasExternalAgent := slice.Find(jobType.WorkspaceBuild.Resources, func(resource *sdkproto.Resource) bool {
			return resource.Type == "ni_external_agent"
		})
...
func convertVariableValues(variableValues []nicloudsdk.VariableValue) []*sdkproto.VariableValue {
...
type TemplateVersionImportJob struct {
	...
	UserVariableValues []nicloudsdk.VariableValue `json:"user_variable_values"`
}
