		...
		for _, prebuiltWorkspaceBuildStage := range []sdkproto.PrebuiltWorkspaceBuildStage{
			...
			t.Run(tc.name+"_WorkspaceBuildJob_Stage"+prebuiltWorkspaceBuildStage.String(), func(t *testing.T) {
				...
				dv := &nicloudsdk.DeploymentValues{
					Sessions: nicloudsdk.SessionLifetime{
		...
		t.Run("TemplateImport", func(t *testing.T) {
			...
			for _, tc := range []testcase{
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					tvID := uuid.New()
					templateAdminUser := dbgen.User(t, db, database.User{RBACRoles: []string{nicloudsdk.RoleTemplateAdmin}})
					template := dbgen.Template(t, db, database.Template{
						Name:           "template",
		...
		t.Run("WorkspaceBuild", func(t *testing.T) {
			...
			for _, tc := range []testcase{
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					importJobID := uuid.New()
					tvID := uuid.New()
					templateUser := dbgen.User(t, db, database.User{RBACRoles: []string{nicloudsdk.RoleTemplateAdmin}})
					template := dbgen.Template(t, db, database.Template{
						Name:           "template",
...
func TestNotifications(t *testing.T) {
	...
	t.Run("Manual build failed, template admins notified", func(t *testing.T) {
		...
		templateAdmin := dbgen.User(t, db, database.User{RBACRoles: []string{nicloudsdk.RoleTemplateAdmin}})
		_ /* other template admin, should not receive notification */ = dbgen.User(t, db, database.User{RBACRoles: []string{nicloudsdk.RoleTemplateAdmin}})
...
func setup(t *testing.T, ignoreLogErrors bool, ov *overrides) (proto.DRPCProvisionerDaemonServer, database.Store, pubsub.Pubsub, database.ProvisionerDaemon) {
	...
	serverDB := dbauthz.New(db, authorizer, logger, nicloudtest.AccessControlStorePointer())
	srv, err := provisionerdserver.NewServer(
		...
		nicloud.ReadExperiments(logger, deploymentValues.Experiments),
	)
	require.NoError(t, err)
...
func newFakeUsageInserter() (*nicloudtest.UsageInserter, *atomic.Pointer[usage.Inserter]) {
	poitr := &atomic.Pointer[usage.Inserter]{}
	fake := nicloudtest.NewUsageInserter()
