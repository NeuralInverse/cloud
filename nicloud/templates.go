...
// @Success 200 {object} nicloudsdk.Template
...
// @Produce json
// @Tags Templates
// @Param request body nicloudsdk.CreateTemplateRequest true "Request body"
...
func (api *API) postTemplateByOrganization(rw http.ResponseWriter, r *http.Request) {
	var (
		...
		createTemplate                     nicloudsdk.CreateTemplateRequest
	...
	if createTemplate.AutostartRequirement != nil {
		...
		autostartRequirementDaysOfWeek = nicloudsdk.BitmapToWeekdays(0b01111111)
	}
	...
	var (
		validErrs                            []nicloudsdk.ValidationError
	...
	if defaultTTL < 0 {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "default_ttl_ms", Detail: "Must be a positive integer."})
	}
	if activityBump < 0 {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "activity_bump_ms", Detail: "Must be a positive integer."})
	}

	if len(autostopRequirementDaysOfWeek) > 0 {
		autostopRequirementDaysOfWeekParsed, err = nicloudsdk.WeekdaysToBitmap(autostopRequirementDaysOfWeek)
		if err != nil {
			validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "autostop_requirement.days_of_week", Detail: err.Error()})
		}
	}
	if len(autostartRequirementDaysOfWeek) > 0 {
		autostartRequirementDaysOfWeekParsed, err = nicloudsdk.WeekdaysToBitmap(autostartRequirementDaysOfWeek)
		if err != nil {
			validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "autostart_requirement.days_of_week", Detail: err.Error()})
		}
	}
	if createTemplate.MaxPortShareLevel != nil {
		err = portSharer.ValidateTemplateMaxLevel(*createTemplate.MaxPortShareLevel)
		if err != nil {
			validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "max_port_share_level", Detail: err.Error()})
		} else {
	...
	if !val.Valid() {
		validErrs = append(validErrs, nicloudsdk.ValidationError{
			Field:  "cors_behavior",
	...
	if autostopRequirementWeeks < 0 {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "autostop_requirement.weeks", Detail: "Must be a positive integer."})
	}
	if autostopRequirementWeeks > schedule.MaxTemplateAutostopRequirementWeeks {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "autostop_requirement.weeks", Detail: fmt.Sprintf("Must be less than %d.", schedule.MaxTemplateAutostopRequirementWeeks)})
	}
	if failureTTL < 0 {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "failure_ttl_ms", Detail: "Must be a positive integer."})
	}
	if dormantTTL < 0 {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "time_til_dormant_autodeletion_ms", Detail: "Must be a positive integer."})
	}
	...
	if !createTemplate.DisableEveryoneGroupAccess {
		...
		defaultsGroups[organization.ID.String()] = db2sdk.TemplateRoleActions(nicloudsdk.TemplateRoleUse)
	}
...
// @Success 200 {array} nicloudsdk.Template
...
// @Tags Templates
// @Param template path string true "Template ID" format(uuid)
// @Param request body nicloudsdk.UpdateTemplateMeta true "Patch template settings request"
// @Success 200 {object} nicloudsdk.Template
...
func (api *API) patchTemplateMeta(rw http.ResponseWriter, r *http.Request) {
	...
	if resolved.failureTTLMillis < 0 || (resolved.failureTTLMillis > 0 && resolved.failureTTLMillis < minTTL) {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "failure_ttl_ms", Detail: "Value must be at least one minute."})
	}
	if resolved.timeTilDormantMillis < 0 || (resolved.timeTilDormantMillis > 0 && resolved.timeTilDormantMillis < minTTL) {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "time_til_dormant_ms", Detail: "Value must be at least one minute."})
	}
	if resolved.timeTilDormantAutoDeleteMillis < 0 || (resolved.timeTilDormantAutoDeleteMillis > 0 && resolved.timeTilDormantAutoDeleteMillis < minTTL) {
		validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "time_til_dormant_autodelete_ms", Detail: "Value must be at least one minute."})
	}
	...
	if req.MaxPortShareLevel != nil && *req.MaxPortShareLevel != portSharer.ConvertMaxLevel(template.MaxPortSharingLevel) {
		...
		if err != nil {
			validErrs = append(validErrs, nicloudsdk.ValidationError{Field: "max_port_sharing_level", Detail: err.Error()})
		} else {
...
// @Tags Templates
// @Param organization path string true "Organization ID" format(uuid)
// @Success 200 {array} nicloudsdk.TemplateExample
...
func (api *API) convertTemplates(templates []database.Template) []nicloudsdk.Template {
	apiTemplates := make([]nicloudsdk.Template, 0, len(templates))
...
func (api *API) convertTemplate(
	...
	return nicloudsdk.Template{
		...
		Provisioner:                    nicloudsdk.ProvisionerType(template.Provisioner),
		...
		AutostopRequirement: nicloudsdk.TemplateAutostopRequirement{
			DaysOfWeek: nicloudsdk.BitmapToWeekdays(uint8(template.AutostopRequirementDaysOfWeek)), // #nosec G115 - Safe conversion as AutostopRequirementDaysOfWeek is a 7-bit bitmap
			Weeks:      autostopRequirementWeeks,
		},
		AutostartRequirement: nicloudsdk.TemplateAutostartRequirement{
			DaysOfWeek: nicloudsdk.BitmapToWeekdays(template.AutostartAllowedDays()),
		},
		...
		CORSBehavior:            nicloudsdk.CORSBehavior(template.CorsBehavior),
...
func findTemplateAdmins(ctx context.Context, store database.Store) ([]database.GetUsersRow, error) {
	templateAdmins, err := store.GetUsers(ctx, database.GetUsersParams{
		RbacRole: []string{nicloudsdk.RoleTemplateAdmin, nicloudsdk.RoleOwner},
	})
	if err != nil {
