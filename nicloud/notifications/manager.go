...
//
// The notifier dequeues messages from the store _NEURALINVERSE_NOTIFICATIONS_LEASE_COUNT_ at a time and concurrently "dispatches"
...
// of size NEURALINVERSE_NOTIFICATIONS_STORE_SYNC_INTERVAL in the Manager, and updates are sent to the store about which messages
// succeeded or failed every NEURALINVERSE_NOTIFICATIONS_STORE_SYNC_INTERVAL seconds.
...
type Manager struct {
	cfg nicloudsdk.NotificationsConfig
...
// access URL etc.
func NewManager(cfg nicloudsdk.NotificationsConfig, store Store, ps pubsub.Pubsub, helpers template.FuncMap, metrics *Metrics, log slog.Logger, opts ...ManagerOption) (*Manager, error) {
...
func defaultHandlers(cfg nicloudsdk.NotificationsConfig, log slog.Logger, store Store, ps pubsub.Pubsub) map[database.NotificationMethod]Handler {
