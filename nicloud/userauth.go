...
type OAuthConvertStateClaims struct {
	...
	FromLoginType nicloudsdk.LoginType `json:"from_login_type"`
	ToLoginType   nicloudsdk.LoginType `json:"to_login_type"`
}
...
// @Produce json
// @Tags Authorization
// @Param request body nicloudsdk.ConvertLoginRequest true "Convert request"
// @Param user path string true "User ID, name, or me"
// @Success 201 {object} nicloudsdk.OAuthConversionResponse
...
func (api *API) postConvertLoginType(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.ConvertLoginRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}

	switch req.ToType {
	case nicloudsdk.LoginTypeGithub, nicloudsdk.LoginTypeOIDC:
		// Allowed!
	case nicloudsdk.LoginTypeNone, nicloudsdk.LoginTypePassword, nicloudsdk.LoginTypeToken:
	...
	user, _, ok := api.loginRequest(ctx, rw, nicloudsdk.LoginWithPasswordRequest{
		Email:    user.Email,
	...
	// When the user does the oauth flow, this jwt will be sent back to nicloud.
	// The included information in this payload links it to a state string, so
	// this request is tied 1:1 with an oauth state.
	// This JWT also includes information to tie it 1:1 with a neuralinverse deployment
	// and user account. This is mainly to inform the user if they are accidentally
	// switching between neuralinverse deployments if the OIDC is misconfigured.
	...
	claims := &OAuthConvertStateClaims{
		...
		UserID:        user.ID,
		State:         stateString,
		FromLoginType: nicloudsdk.LoginType(user.LoginType),
	...
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.OAuthConversionResponse{
...
// @Accept json
// @Tags Authorization
// @Param request body nicloudsdk.RequestOneTimePasscodeRequest true "One-time passcode request"
// @Success 204
...
func (api *API) postRequestOneTimePasscode(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.RequestOneTimePasscodeRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Accept json
// @Tags Authorization
// @Param request body nicloudsdk.ChangePasswordWithOneTimePasscodeRequest true "Change password request"
// @Success 204
...
func (api *API) postChangePasswordWithOneTimePasscode(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.ChangePasswordWithOneTimePasscodeRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Accept json
// @Tags Authorization
// @Param request body nicloudsdk.ValidateUserPasswordRequest true "Validate user password request"
// @Success 200 {object} nicloudsdk.ValidateUserPasswordResponse
...
func (*API) validateUserPassword(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.ValidateUserPasswordRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.ValidateUserPasswordResponse{
		Valid:   valid,
		Details: details,
	})
}
...
// @Produce json
// @Tags Authorization
// @Param request body nicloudsdk.LoginWithPasswordRequest true "Login request"
// @Success 201 {object} nicloudsdk.LoginWithPasswordResponse
...
func (api *API) postLogin(rw http.ResponseWriter, r *http.Request) {
	...
	var loginWithPassword nicloudsdk.LoginWithPasswordRequest
	...
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.LoginWithPasswordResponse{
...
func (api *API) loginRequest(ctx context.Context, rw http.ResponseWriter, req nicloudsdk.LoginWithPasswordRequest) (database.User, rbac.Subject, bool) {
	logger := api.Logger.Named(userAuthLoggerName)
...
func (c *GithubOAuth2Config) AuthCodeURL(state string, opts ...oauth2.AuthCodeOption) string {
	...
	// This is an absolute path in the Neural Inverse Cloud app. The device flow is orchestrated
	// by the Neural Inverse Cloud frontend, so we need to redirect the user to the device flow page.
...
// @Produce json
// @Tags Users
// @Success 200 {object} nicloudsdk.AuthMethods
...
func (api *API) userAuthMethods(rw http.ResponseWriter, r *http.Request) {
	...
	httpapi.Write(r.Context(), rw, http.StatusOK, nicloudsdk.AuthMethods{
		TermsOfServiceURL: api.DeploymentValues.TermsOfServiceURL.Value(),
		Password: nicloudsdk.AuthMethod{
			Enabled: !api.DeploymentValues.DisablePasswordAuth.Value(),
		},
		Github: nicloudsdk.GithubAuthMethod{
		...
		OIDC: nicloudsdk.OIDCAuthMethod{
			AuthMethod: nicloudsdk.AuthMethod{Enabled: api.OIDCConfig != nil},
...
// @Produce json
// @Tags Users
// @Success 200 {object} nicloudsdk.ExternalAuthDevice
...
func (api *API) userOAuth2Github(rw http.ResponseWriter, r *http.Request) {
	...
	normName := nicloudsdk.NormalizeRealUsername(ghName)
	...
	if api.GithubOAuth2Config.DeviceFlowEnabled {
		...
		httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.OAuth2DeviceFlowCallbackResponse{
...
func (api *API) userOIDC(rw http.ResponseWriter, r *http.Request) {
	...
	// The username is a required property in Neural Inverse Cloud. We make a best-effort
	// attempt at using what the claims provide, but if that fails we will
	// generate a random username.
	usernameValid := nicloudsdk.NameValid(username)
	if usernameValid != nil {
		...
		username = nicloudsdk.UsernameFrom(username)
	}
	...
	// The 'name' is an optional property in Neural Inverse Cloud. If not specified,
	// it will be left blank.
	var name string
	nameRaw, ok := mergedClaims[api.OIDCConfig.NameField]
	if ok {
		name, _ = nameRaw.(string)
		name = nicloudsdk.NormalizeRealUsername(name)
	}
...
func (api *API) oauthLogin(r *http.Request, params *oauthLoginParams) ([]*http.Cookie, database.User, database.APIKey, error) {
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		if user.ID == uuid.Nil {
			...
			if err == nil {
				...
				for i := 0; i < 10; i++ {
					...
					params.Username = nicloudsdk.UsernameFrom(alternate)
		...
		// that in Neural Inverse Cloud. Many enterprises use a user's email/username as
...
func (api *API) convertUserToOauth(ctx context.Context, r *http.Request, db database.Store, params *oauthLoginParams) (database.User, error) {
	...
	if xerrors.Is(err, cryptokeys.ErrKeyNotFound) || xerrors.Is(err, cryptokeys.ErrKeyInvalid) || xerrors.Is(err, jose.ErrCryptoFailure) || xerrors.Is(err, jwtutils.ErrMissingKeyID) {
		// These errors are probably because the user is mixing 2 neuralinverse deployments.
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusBadRequest,
			Msg:  "Using an invalid jwt to authorize this action. Ensure there is only 1 neuralinverse deployment and try again.",
		}
	}
	if err != nil {
		return database.User{}, idpsync.HTTPError{
	...
	if claims.Issuer != api.DeploymentID {
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusForbidden,
			Msg:  "Request to convert login type failed. Issuer mismatch. Found a cookie from another neuralinverse deployment, please try again.",
		}
	}
	...
	if user.ID != claims.UserID ||
		nicloudsdk.LoginType(user.LoginType) != claims.FromLoginType ||
		nicloudsdk.LoginType(params.LoginType) != claims.ToLoginType {
		return database.User{}, idpsync.HTTPError{
			Code: http.StatusForbidden,
