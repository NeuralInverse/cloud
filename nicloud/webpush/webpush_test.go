...
func TestPush(t *testing.T) {
	...
	t.Run("NoSubscriptions", func(t *testing.T) {
		...
		userID := uuid.New()
		notification := nicloudsdk.WebpushMessage{
			Title: "Test Title",
			Body:  "Test Body",
		}
...
func randomWebpushMessage(t testing.TB) nicloudsdk.WebpushMessage {
	t.Helper()
	return nicloudsdk.WebpushMessage{
		Title: testutil.GetRandomName(t),
		Body:  testutil.GetRandomName(t),

		Actions: []nicloudsdk.WebpushMessageAction{
...
func TestNoopWebpusher(t *testing.T) {
	...
	dispatchErr := noop.Dispatch(context.Background(), uuid.New(), nicloudsdk.WebpushMessage{})
	require.Error(t, dispatchErr)
	require.Contains(t, dispatchErr.Error(), "push disabled")

	testErr := noop.Test(context.Background(), nicloudsdk.WebpushSubscription{})
...
func TestSSRFPrevention(t *testing.T) {
	...
	err = manager.Test(ctx, nicloudsdk.WebpushSubscription{
		Endpoint:  server.URL,
	...
	err = manager.Dispatch(ctx, user.ID, nicloudsdk.WebpushMessage{
		Title: "SSRF test",
