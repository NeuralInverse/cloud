...
import (
	...
	"github.com/NeuralInverse/cloud/v2/archive/archivetest"
...
func TestPostFiles(t *testing.T) {
	...
	t.Run("Insert", func(t *testing.T) {
		...
		_, err := client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(make([]byte, 1024)))
		require.NoError(t, err)
	})
	...
	t.Run("InsertAlreadyExists", func(t *testing.T) {
		...
		data := make([]byte, 1024)
		_, err := client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
		_, err = client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
	})
	t.Run("InvalidZipMetadata", func(t *testing.T) {
		...
		_, err := client.Upload(ctx, nicloudsdk.ContentTypeZip, bytes.NewReader(zipBytes))
	...
	t.Run("OversizedZipExpansion", func(t *testing.T) {
		...
		// file size limit.
		size := int64(nicloud.HTTPFileMaxBytes - 1024)
...
func TestDownload(t *testing.T) {
	...
	t.Run("InsertTar_DownloadTar", func(t *testing.T) {
		...
		// when
		resp, err := client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(tarball))
		require.NoError(t, err)
		...
		require.Equal(t, nicloudsdk.ContentTypeTar, contentType)
	...
	t.Run("InsertZip_DownloadTar", func(t *testing.T) {
		...
		// when
		resp, err := client.Upload(ctx, nicloudsdk.ContentTypeZip, bytes.NewReader(zipContent))
		require.NoError(t, err)
		data, contentType, err := client.Download(ctx, resp.ID)
		require.NoError(t, err)
	...
	t.Run("InsertTar_DownloadZip", func(t *testing.T) {
		...
		resp, err := client.Upload(ctx, nicloudsdk.ContentTypeTar, bytes.NewReader(tarball))
		require.NoError(t, err)
		data, contentType, err := client.DownloadWithFormat(ctx, resp.ID, nicloudsdk.FormatZip)
		require.NoError(t, err)

		// then
		require.Equal(t, nicloudsdk.ContentTypeZip, contentType)
