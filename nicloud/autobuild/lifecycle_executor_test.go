...
func TestExecutorAutostartOK(t *testing.T) {
	...
	var (
		...
		workspace = mustProvisionWorkspace(t, client, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
	...
	assert.Equal(t, nicloudsdk.BuildReasonAutostart, workspace.LatestBuild.Reason)
...
func TestMultipleLifecycleExecutors(t *testing.T) {
	...
	var (
		...
		clientA  = nicloudtest.New(t, &nicloudtest.Options{
			IncludeProvisionerDaemon: true,
			AutobuildTicker:          tickCh,
		...
		_        = nicloudtest.New(t, &nicloudtest.Options{
			IncludeProvisionerDaemon: true,
			AutobuildTicker:          tickCh,
		...
		workspace = mustProvisionWorkspace(t, clientA, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = ptr.Ref(sched.String())
		})
	)

	// Have the workspace stopped so we can perform an autostart
	workspace = nicloudtest.MustTransitionWorkspace(t, clientA, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
...
func TestExecutorBuildNumberRaceIsHandled(t *testing.T) {
	...
	p, err := nicloudtest.GetProvisionerForTags(realDB, time.Now(), workspace.OrganizationID, nil)
	require.NoError(t, err)
	next := sched.Next(workspace.LatestBuild.CreatedAt)
	nicloudtest.UpdateProvisionerLastSeenAt(t, realDB, p.ID, next)
...
func TestExecutorAutostartTemplateUpdated(t *testing.T) {
	...
	testCases := []struct {
		name                 string
		automaticUpdates     nicloudsdk.AutomaticUpdates
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			workspace = nicloudtest.MustTransitionWorkspace(
				t, client, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
			...
			newVersion := nicloudtest.UpdateTemplateVersion(t, client, orgs[0].ID, res, workspace.TemplateID)
			nicloudtest.AwaitTemplateVersionJobCompleted(t, client, newVersion.ID)
			require.NoError(t, client.UpdateActiveTemplateVersion(
				ctx, workspace.TemplateID, nicloudsdk.UpdateActiveTemplateVersion{
					ID: newVersion.ID,
				},
			))
...
func TestExecutorAutostartUserSuspended(t *testing.T) {
	...
	workspace := nicloudtest.CreateWorkspace(t, userClient, template.ID, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)
	workspace = nicloudtest.MustWorkspace(t, userClient, workspace.ID)

	// Given: workspace is stopped, and the user is suspended.
	workspace = nicloudtest.MustTransitionWorkspace(t, userClient, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)

	ctx := testutil.Context(t, testutil.WaitShort)

	_, err := client.UpdateUserStatus(ctx, user.ID.String(), nicloudsdk.UserStatusSuspended)
...
func TestExecutorAutostopOK(t *testing.T) {
	...
	workspace = nicloudtest.MustWorkspace(t, client, workspace.ID)
	assert.Equal(t, nicloudsdk.BuildReasonAutostop, workspace.LatestBuild.Reason)
}
...
func TestExecutorAutostopAIAgentActivity(t *testing.T) {
	...
	_, err := client.UpdateTemplateMeta(ctx, r.Template.ID, nicloudsdk.UpdateTemplateMeta{
	...
	p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), r.Workspace.OrganizationID, nil)
	require.NoError(t, err)
...
func TestExecutorWorkspaceDeleted(t *testing.T) {
	...
	workspace = nicloudtest.MustTransitionWorkspace(t, client, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionDelete)
...
func TestExecuteAutostopSuspendedUser(t *testing.T) {
	...
	nicloudtest.MustWaitForAnyProvisioner(t, db)
	version := nicloudtest.CreateTemplateVersion(t, client, admin.OrganizationID, nil)
	nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
	template := nicloudtest.CreateTemplate(t, client, admin.OrganizationID, version.ID)
	userClient, user := nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
	...
	assert.Equal(t, nicloudsdk.WorkspaceStatusStopped, workspaceBuild.Status)
}
...
func TestExecutorWorkspaceAutostopNoWaitChangedMyMind(t *testing.T) {
	...
	err := client.UpdateWorkspaceTTL(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceTTLRequest{TTLMillis: nil})
	require.NoError(t, err)

	// Then: the deadline should be set to zero
	updated := nicloudtest.MustWorkspace(t, client, workspace.ID)
...
func TestExecutorAutostartMultipleOK(t *testing.T) {
	...
	var (
		...
		_, _ = nicloudtest.NewWithDatabase(t, &nicloudtest.Options{
...
func TestExecutorAutostartWithParameters(t *testing.T) {
	...
	var (
		...
		// Given: we have a user with a workspace that has autostart enabled
		workspace = mustProvisionWorkspaceWithParameters(t, client, richParameters, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
			cwr.AutostartSchedule = ptr.Ref(sched.String())
...
func TestExecutorRequireActiveVersion(t *testing.T) {
	...
	me, err := ownerClient.User(ctx, nicloudsdk.Me)
	require.NoError(t, err)
	...
	activeVersion := nicloudtest.CreateTemplateVersion(t, ownerClient, owner.OrganizationID, nil)
	nicloudtest.AwaitTemplateVersionJobCompleted(t, ownerClient, activeVersion.ID)
	template := nicloudtest.CreateTemplate(t, ownerClient, owner.OrganizationID, activeVersion.ID)
	...
	err = db.UpdateTemplateAccessControlByID(dbauthz.As(ctx, nicloudtest.AuthzUserSubject(me)), database.UpdateTemplateAccessControlByIDParams{
		ID:                   template.ID,
		RequireActiveVersion: true,
	})
	require.NoError(t, err)
	inactiveVersion := nicloudtest.CreateTemplateVersion(t, ownerClient, owner.OrganizationID, nil, func(ctvr *nicloudsdk.CreateTemplateVersionRequest) {
		ctvr.TemplateID = template.ID
	})
	nicloudtest.AwaitTemplateVersionJobCompleted(t, ownerClient, inactiveVersion.ID)
	memberClient, _ := nicloudtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
	ws := nicloudtest.CreateWorkspace(t, memberClient, uuid.Nil, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
		cwr.TemplateVersionID = inactiveVersion.ID
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, ownerClient, ws.LatestBuild.ID)
	ws = nicloudtest.MustTransitionWorkspace(t, memberClient, ws.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop, func(req *nicloudsdk.CreateWorkspaceBuildRequest) {
	...
	ws = nicloudtest.MustWorkspace(t, memberClient, ws.ID)
	require.Equal(t, inactiveVersion.ID, ws.LatestBuild.TemplateVersionID)
}
...
// For enterprise functionality see enterprise/nicloud/workspaces_test.go
...
func TestNotifications(t *testing.T) {
	...
	t.Run("Dormancy", func(t *testing.T) {
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, admin.OrganizationID, version.ID, func(ctr *nicloudsdk.CreateTemplateRequest) {
			ctr.TimeTilDormantMillis = ptr.Ref(timeTilDormant.Milliseconds())
		})
		userClient, _ := nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		workspace := nicloudtest.CreateWorkspace(t, userClient, template.ID)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)

		// Stop workspace
		workspace = nicloudtest.MustTransitionWorkspace(t, client, workspace.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
		_ = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, workspace.LatestBuild.ID)
...
// For enterprise-related functionality, see enterprise/nicloud/workspaces_test.go.
func TestExecutorPrebuilds(t *testing.T) {
	...
	t.Run("OnlyStopsAfterClaimed", func(t *testing.T) {
		...
		_, user := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleMember())
		...
		prebuild := nicloudtest.MustWorkspace(t, client, dbPrebuild.ID)
		...
		p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), prebuild.OrganizationID, nil)
		require.NoError(t, err)
		...
		dbWorkspace := dbgen.ClaimPrebuild(
			...
			sql.NullTime{},
			sql.NullInt64{})
		workspace := nicloudtest.MustWorkspace(t, client, dbWorkspace.ID)
...
func mustProvisionWorkspace(t *testing.T, client *nicloudsdk.Client, mut ...func(*nicloudsdk.CreateWorkspaceRequest)) nicloudsdk.Workspace {
	t.Helper()
	user := nicloudtest.CreateFirstUser(t, client)
	...
	ws := nicloudtest.CreateWorkspace(t, client, template.ID, mut...)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
	return nicloudtest.MustWorkspace(t, client, ws.ID)
}

// mustProvisionWorkspaceWithProvisionerTags creates a workspace with a template version that has specific provisioner tags
func mustProvisionWorkspaceWithProvisionerTags(t *testing.T, client *nicloudsdk.Client, provisionerTags map[string]string, mut ...func(*nicloudsdk.CreateWorkspaceRequest)) nicloudsdk.Workspace {
	t.Helper()
	user := nicloudtest.CreateFirstUser(t, client)

	// Create template version with specific provisioner tags
	version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(request *nicloudsdk.CreateTemplateVersionRequest) {
...
func mustProvisionWorkspaceWithParameters(t *testing.T, client *nicloudsdk.Client, richParameters []*proto.RichParameter, mut ...func(*nicloudsdk.CreateWorkspaceRequest)) nicloudsdk.Workspace {
	t.Helper()
	user := nicloudtest.CreateFirstUser(t, client)
	version := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, &echo.Responses{
		Parse: echo.ParseComplete,
...
func mustWorkspaceParameters(t *testing.T, client *nicloudsdk.Client, workspaceID uuid.UUID) {
	ctx := testutil.Context(t, testutil.WaitShort)
...
func TestExecutorAutostartSkipsWhenNoProvisionersAvailable(t *testing.T) {
	...
	daemon1Closer := nicloudtest.NewTaggedProvisionerDaemon(t, api, "name", provisionerDaemonTags)
	t.Cleanup(func() {
		_ = daemon1Closer.Close()
	})

	// Create workspace with autostart enabled and matching provisioner tags
	workspace := mustProvisionWorkspaceWithProvisionerTags(t, client, provisionerDaemonTags, func(cwr *nicloudsdk.CreateWorkspaceRequest) {
		cwr.AutostartSchedule = ptr.Ref(sched.String())
	})
	...
	p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, provisionerDaemonTags)
	...
	nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, staleTime)
	...
	nicloudtest.MustWaitForProvisionersUnavailable(t, db, workspace, provisionerDaemonTags, next)
	...
	daemon2Closer := nicloudtest.NewTaggedProvisionerDaemon(t, api, "name", provisionerDaemonTags)
	t.Cleanup(func() {
	...
	nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, notStaleTime)
...
func TestExecutorTaskWorkspace(t *testing.T) {
	...
	createTaskTemplate := func(t *testing.T, client *nicloudsdk.Client, orgID uuid.UUID, ctx context.Context, defaultTTL time.Duration) nicloudsdk.Template {
		t.Helper()
	...
	createTaskWorkspace := func(t *testing.T, client *nicloudsdk.Client, template nicloudsdk.Template, ctx context.Context, input string) nicloudsdk.Workspace {
		t.Helper()

		task, err := client.CreateTask(ctx, "me", nicloudsdk.CreateTaskRequest{
	...
	t.Run("Autostop", func(t *testing.T) {
		...
		workspace = nicloudtest.MustWorkspace(t, client, workspace.ID)
		assert.Equal(t, nicloudsdk.BuildReasonTaskAutoPause, workspace.LatestBuild.Reason, "task workspace should use TaskAutoPause build reason")
	})
