...
import (
	...
	mcpserver "github.com/NeuralInverse/cloud/v2/nicloud/mcp"
...
func TestMCPHTTP_E2E_ClientIntegration(t *testing.T) {
	...
	niClient, closer, api := nicloudtest.NewWithAPI(t, nil)
	defer closer.Close()

	_ = nicloudtest.CreateFirstUser(t, niClient)
	...
	mcpClient := newIsolatedMCPClient(t, mcpURL,
		transport.WithHTTPHeaders(map[string]string{
			"Authorization": "Bearer " + niClient.SessionToken(),
		}))
	defer func() {
...
func TestMCPHTTP_E2E_UnauthenticatedAccess(t *testing.T) {
	...
	// Setup Coder server
	_, closer, api := nicloudtest.NewWithAPI(t, nil)
	defer closer.Close()
...
func TestMCPHTTP_E2E_ToolWithWorkspace(t *testing.T) {
	...
	user := nicloudtest.CreateFirstUser(t, niClient)
	...
	_ = agenttest.New(t, niClient.URL, r.AgentToken, func(opts *agent.Options) {
		opts.Filesystem = fs
	})
	nicloudtest.NewWorkspaceAgentWaiter(t, niClient, r.Workspace.ID).Wait()
...
func TestMCPHTTP_E2E_ErrorHandling(t *testing.T) {
	...
	// Setup Coder server
	niClient, closer, api := nicloudtest.NewWithAPI(t, &nicloudtest.Options{
...
func TestMCPHTTP_E2E_OAuth2_EndToEnd(t *testing.T) {
	...
	_, err := niClient.PostOAuth2ProviderApp(ctx, nicloudsdk.PostOAuth2ProviderAppRequest{
		Name:        "test-mcp-app",
	...
	t.Run("MCPWithOAuth2BearerToken", func(t *testing.T) {
		...
		sessionToken := niClient.SessionToken()
	...
	t.Run("OAuth2FullFlowWithTokenRefresh", func(t *testing.T) {
		...
		app, err := niClient.PostOAuth2ProviderApp(ctx, nicloudsdk.PostOAuth2ProviderAppRequest{
		...
		secret, err := niClient.PostOAuth2ProviderAppSecret(ctx, app.ID)
		require.NoError(t, err)
		...
		req.Header.Set("Authorization", "Bearer "+niClient.SessionToken())
		...
		if resp.StatusCode == http.StatusOK {
			...
			require.NoError(t, err)
			consentReq.Header.Set("Authorization", "Bearer "+niClient.SessionToken())
	...
	t.Run("DynamicClientRegistrationWithMCPFlow", func(t *testing.T) {
		...
		require.NoError(t, err)
		authReq.Header.Set("Cookie", fmt.Sprintf("coder_session_token=%s", niClient.SessionToken()))
		authReq.Header.Set("Authorization", "Bearer "+niClient.SessionToken())
		...
		if authResp.StatusCode == http.StatusOK {
			...
			consentReq, err := http.NewRequestWithContext(ctx, "POST", authURL, nil)
			require.NoError(t, err)
			consentReq.Header.Set("Cookie", fmt.Sprintf("coder_session_token=%s", niClient.SessionToken()))
			consentReq.Header.Set("Authorization", "Bearer "+niClient.SessionToken())
			consentReq.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func TestMCPHTTP_E2E_ChatGPTEndpoint(t *testing.T) {
	...
	version := nicloudtest.CreateTemplateVersion(t, niClient, user.OrganizationID, nil)
	nicloudtest.AwaitTemplateVersionJobCompleted(t, niClient, version.ID)
	template := nicloudtest.CreateTemplate(t, niClient, user.OrganizationID, version.ID)
...
func TestMCPHTTP_E2E_WorkspaceSSHAuthz(t *testing.T) {
	...
	admin := nicloudtest.CreateFirstUser(t, niClient)
	...
	tmplAdminClient, _ := nicloudtest.CreateAnotherUser(
		t, niClient, admin.OrganizationID, rbac.RoleTemplateAdmin(),
	)
	...
	workspaceIdent := nicloudtest.FirstUserParams.Username + "/" + r.Workspace.Name
	toolResult, err := mcpClient.CallTool(ctx, mcp.CallToolRequest{
		Params: mcp.CallToolParams{
...
func newIsolatedMCPClient(t *testing.T, mcpURL string, opts ...transport.StreamableHTTPCOption) *mcpclient.Client {
	t.Helper()
	isolated := nicloudtest.NewIsolatedHTTPClient(nil)
