...
// How many NINauts appear per round.
...
// ---------------------------------------------------------------------------
// NINauts - little astronauts stranded on the landing pads
// ---------------------------------------------------------------------------
...
function updateNINauts(nauts: NINaut[], pads: LandingPad[], dt: number) {
...
/**
 * Draw a single NINaut at their position on the pad surface.
...
// ---------------------------------------------------------------------------
// Disembark animation - NINauts walking into the base
// ---------------------------------------------------------------------------
...
export const LunarLander: FC = () => {
	...
	useEffect(() => {
		...
		function startNewRound() {
			...
			ninauts = createNINauts(terrain.pads, pick);
			disembarks = [];
			resetLander();
		...
		function onClick(e: MouseEvent) {
			...
			if (dialogVisible) {
				...
				if (
					...
					// Yes - travel to another base with this NINaut.
		...
		function loop(ts: number) {
			...
			if (victory) {
				...
				cx.textAlign = "center";
				cx.fillText("All NINauts have been saved!", dashW / 2, playH * 0.6);
			...
			// -- update ninauts --
			if (terrain) updateNINauts(ninauts, terrain.pads, dt);
					...
					if (
						...
						if (pad) {
							...
							if (pad.isStation) {
								...
								const aboardCount = ninauts.filter((c) => c.aboard).length;
			...
			if (terrain) {
				...
				for (const naut of ninauts) {
					if (!naut.aboard && !naut.saved) {
						drawNINaut(cx, naut, terrain.pads[naut.padIdx].y);
					}
				}
			}
			...
			for (const a of disembarks) {
				...
				drawNINaut(cx, tmpNaut, a.groundY);
			}
			...
			const currentNautNames = new Set(
				ninauts.filter((c) => !c.aboard && !c.saved).map((c) => c.name),
			);
			drawSidebar(
				cx,
				logicalW,
			...
			const cargo = ninauts.filter((c) => c.aboard);
			const savedPct = (savedNames.size / ROSTER.length) * 100;
