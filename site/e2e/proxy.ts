...
export const startWorkspaceProxy = async (
	...
	const cp = spawn(coderBinary, ["wsproxy", "server"], {
		env: {
			...process.env,
			NEURALINVERSE_PRIMARY_ACCESS_URL: `http://127.0.0.1:${coderPort}`,
			NEURALINVERSE_PROXY_SESSION_TOKEN: token,
			NEURALINVERSE_HTTP_ADDRESS: `localhost:${workspaceProxyPort}`,
		},
	});
