...
func TestInterruptTask_FinishInterruptionOnly(t *testing.T) {
	...
	f.requireNoLegacyStreamEvents(t)
...
func (f *taskTestFixture) requireNoLegacyStreamEvents(t *testing.T) {
	t.Helper()
	for _, event := range f.pubsub.events() {
		require.NotContains(t, event.channel, "chat:stream:")
	}
}
