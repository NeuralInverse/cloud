...
func TestGenerateAssistant_StartupTimeoutRetryClassification(t *testing.T) {
	t.Parallel()
	...
	t.Run("timeout while opening stream", func(t *testing.T) {
		...
		trap := clock.Trap().AfterFunc("startupGuard")
		defer trap.Close()
	...
	t.Run("timeout before first part", func(t *testing.T) {
		...
		require.Error(t, err)
		classified := chaterror.Classify(err)
		require.Equal(t, codersdk.ChatErrorKindStartupTimeout, classified.Kind)
		require.Equal(t, "openai", classified.Provider)
		require.True(t, classified.Retryable)
		require.Equal(t, int32(1), calls.Load())
	})
