...
func GenerateAssistant(ctx context.Context, opts GenerateAssistantOptions) (AssistantOutcome, error) {
	...
	if streamErr != nil {
		classified := chaterror.Classify(streamErr).WithProvider(provider)
		if classified.Retryable {
			opts.Metrics.RecordStreamRetry(provider, modelName, classified)
		}
		return AssistantOutcome{}, wrapProviderStreamError(provider, streamErr)
	}
	...
	if err := attempt.finish(processErr); err != nil {
		...
		return AssistantOutcome{}, wrapProviderStreamError(provider, err)
	}
