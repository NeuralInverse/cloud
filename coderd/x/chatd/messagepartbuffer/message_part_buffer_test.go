...
func TestBuffer_SubscribeBeforeCreateWaitsWithoutNotFound(t *testing.T) {
	t.Parallel()
	...
	type subscription struct {
		ch     <-chan messagepartbuffer.Part
		cancel func()
		err    error
	}
	...
	go func() {
		ch, cancel, err := buffer.SubscribeToEpisode(ctx, key)
		resultCh <- subscription{ch: ch, cancel: cancel, err: err}
	}()

	select {
	case result := <-resultCh:
		t.Fatalf("subscription returned before episode create: %v", result.err)
	default:
	}

	require.NoError(t, buffer.CreateEpisode(key))
	result := receiveSubscription(t, resultCh)
	require.NoError(t, result.err)
	defer result.cancel()
	require.NoError(t, buffer.AddPart(key, codersdk.ChatMessageRoleAssistant, codersdk.ChatMessageText("live")))
	require.Equal(t, "live", receivePart(t, result.ch).MessagePart.Text)
}
...
func TestBuffer_GCClosedEpisodeAfterGraceAndNoSubscribers(t *testing.T) {
	...
	clock.Advance(time.Minute + time.Second).MustWait(ctx)
...
func TestBuffer_SubscribeCanceledBeforeCreateCanCreateEpisode(t *testing.T) {
	...
	cancel()
	_, _, err := buffer.SubscribeToEpisode(ctx, key)
...
func TestBuffer_CloseClosesPendingSubscriptionAndRejectsOperations(t *testing.T) {
	...
	go func() {
		_, _, err := buffer.SubscribeToEpisode(ctx, key)
		resultCh <- subscriptionResult{err: err}
	}()
	buffer.Close()
	result := receiveSubscription(t, resultCh)
	require.ErrorIs(t, result.err, messagepartbuffer.ErrMessagePartBufferClosed)
...
func receiveSubscription[T any](t *testing.T, ch <-chan T) T {
	t.Helper()
	select {
	case value := <-ch:
		return value
