...
func TestActiveServer_ChainBrokenRecovery(t *testing.T) {
	...
	chat := createChatThroughServer(ctx, t, server, org.ID, user.ID, model.ID, "first user")
...
func TestActiveServer_ChainBrokenWithoutChainModeIsSafe(t *testing.T) {
	...
	server := newActiveTestServer(t, db, ps)
	chat := createChatThroughServer(ctx, t, server, org.ID, user.ID, model.ID, "only user")
	waitForChatStatus(ctx, t, db, chat.ID, database.ChatStatusWaiting)
