...
func (server *Server) prepareGeneration(
	...
	g.Go(func() error {
		var err error
		model, modelConfig, providerKeys, debugEnabled, debugProvider, debugModel, err = server.resolveChatModel(ctx, chat)
		if err != nil {
			return err
		}
	...
	if advisorCfg.Enabled && isRootChat && !isPlanModeTurn && !isExploreSubagent {
		advisorRuntime = server.newAdvisorRuntime(
			ctx,
			chat,
			advisorCfg,
			model,
	...
	if isComputerUse {
		...
		computerUseProviderKeys, keyErr := server.resolveUserProviderAPIKeysForProviderType(ctx, chat.OwnerID, computerUseModelProvider)
		if keyErr != nil {
			cleanup()
			return generationPrepared{}, xerrors.Errorf("resolve computer use provider API keys: %w", keyErr)
		}
...
func (server *Server) deriveFinalTurnRunResult(
	...
	model, _, providerKeys, _, resolvedProvider, resolvedModel, err := server.resolveChatModel(ctx, chat)
	if err != nil {
