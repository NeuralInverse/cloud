...
func TestActiveServer_RetryStartupTimeoutAndClassification(t *testing.T) {
	t.Parallel()
	...
	t.Run("startup timeout retry recovers", func(t *testing.T) {
		t.Parallel()
		...
		requireRetryCounter(t, reg, "coderd_chatd_stream_retries_total", 1, map[string]string{
			"provider":     "openai",
			"model":        "gpt-4o-mini",
			"kind":         string(codersdk.ChatErrorKindStartupTimeout),
			"chain_broken": "false",
		})
	})
}
