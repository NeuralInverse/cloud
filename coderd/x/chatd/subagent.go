...
func (p *Server) createChildSubagentChatWithOptions(
	...
	if len(copiedContextParts) > 0 {
		...
		initialMessages = append(initialMessages, chatstate.Message{
			Role:           database.ChatMessageRoleUser,
			Content:        filteredContent,
			Visibility:     database.ChatMessageVisibilityBoth,
			ModelConfigID:  uuid.NullUUID{UUID: modelConfigID, Valid: modelConfigID != uuid.Nil},
			CreatedBy:      uuid.NullUUID{UUID: parent.OwnerID, Valid: parent.OwnerID != uuid.Nil},
	...
	initialMessages = append(initialMessages, userMessage(userContent, modelConfigID, parent.OwnerID))
