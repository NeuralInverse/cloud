...
// ExternalAgentClient is the subset of *nicloudsdk.Client the Manager
// uses to enumerate external-agent workspaces under a template and
// fetch each agent's auth token. *nicloudsdk.Client satisfies this
// interface, so production callers pass their client directly; tests
// substitute a fake without standing up a real nicloud.
type ExternalAgentClient interface {
	Workspaces(ctx context.Context, filter nicloudsdk.WorkspaceFilter) (nicloudsdk.WorkspacesResponse, error)
...
// TokenInfo is a single workspace-agent auth token retrieved for a neuralinverse external agent, along with the identifying
...
// ManagerOptions configures a Manager. Authentication is supplied via the *nicloudsdk.Client passed to NewManager rather
...
// Manager supervises a set of fake Agents in one process. It enumerates the agents it owns from nicloud at Run time
// (via ni_external_agent tokens on workspaces matching opts.Template), then opens a dRPC stream per agent and keeps
...
// (template-admin or higher; FeatureWorkspaceExternalAgent must be enabled). niURL is the URL the spawned
// fake agents will dial.
func NewManager(niURL *url.URL, client ExternalAgentClient, logger slog.Logger, opts ManagerOptions) *Manager {
	return &Manager{
		niURL: niURL,
		client:   client,
		logger:   logger,
...
// Run enumerates the Manager's external agents from nicloud, constructs one Agent per token, and runs the "fake agent"
...
func (m *Manager) Run(ctx context.Context) error {
	...
	for i, ti := range tokens {
		agents = append(agents, NewAgent(m.niURL, ti.Token,
...
// EnumerateExternalAgents asks nicloud for the list of workspaces matching the configured template, walks each
...
func (m *Manager) EnumerateExternalAgents(ctx context.Context) ([]TokenInfo, error) {
	...
	for _, ws := range workspaces {
		...
		// enterprise/nicloud/workspaceagents.go:48). Skip workspaces whose latest build
		// doesn't carry the flag rather than 404 our way through every workspace in nicloud.
...
// IsFatalEnumerationError reports whether err from a nicloud API call indicates an unrecoverable misconfiguration that
