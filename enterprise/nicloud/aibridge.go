...
// @Success 200 {object} nicloudsdk.AIBridgeListInterceptionsResponse
...
func (api *API) aiBridgeListInterceptions(rw http.ResponseWriter, r *http.Request) {
	...
	page, ok := nicloud.ParsePagination(rw, r)
	if !ok {
		return
	}
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.AIBridgeListInterceptionsResponse{
		Count:   count,
		Results: items,
	})
}
...
// @Success 200 {object} nicloudsdk.AIBridgeListSessionsResponse
...
func (api *API) aiBridgeListSessions(rw http.ResponseWriter, r *http.Request) {
	...
	sessions := make([]nicloudsdk.AIBridgeSession, len(rows))
	for i, row := range rows {
		sessions[i] = db2sdk.AIBridgeSession(row)
	}

	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.AIBridgeListSessionsResponse{
		Count:    count,
		Sessions: sessions,
	})
}
...
// @Success 200 {object} nicloudsdk.AIBridgeSessionThreadsResponse
...
func populatedAndConvertAIBridgeInterceptions(ctx context.Context, db database.Store, dbInterceptions []database.ListAIBridgeInterceptionsRow) ([]nicloudsdk.AIBridgeInterception, error) {
	if len(dbInterceptions) == 0 {
		return []nicloudsdk.AIBridgeInterception{}, nil
	}
	...
	items := make([]nicloudsdk.AIBridgeInterception, len(dbInterceptions))
	for i, row := range dbInterceptions {
...
// @Success 200 {object} nicloudsdk.GroupAIBudget
...
// @Tags Enterprise
// @Param group path string true "Group ID" format(uuid)
// @Param request body nicloudsdk.UpsertGroupAIBudgetRequest true "Upsert group AI budget request"
// @Success 200 {object} nicloudsdk.GroupAIBudget
...
func (api *API) upsertGroupAIBudget(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpsertGroupAIBudgetRequest
...
// @Success 200 {object} nicloudsdk.UserAIBudgetOverride
...
// @Tags Enterprise
// @Param user path string true "User ID, username, or me"
// @Param request body nicloudsdk.UpsertUserAIBudgetOverrideRequest true "Upsert user AI budget override request"
// @Success 200 {object} nicloudsdk.UserAIBudgetOverride
...
func (api *API) upsertUserAIBudgetOverride(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpsertUserAIBudgetOverrideRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
