...
func (api *API) shouldBlockNonBrowserConnections(rw http.ResponseWriter) bool {
	if api.Entitlements.Enabled(nicloudsdk.FeatureBrowserOnly) {
		httpapi.Write(context.Background(), rw, http.StatusConflict, nicloudsdk.Response{
...
// @Success 200 {object} nicloudsdk.ExternalAgentCredentials
...
func (api *API) workspaceExternalAgentCredentials(rw http.ResponseWriter, r *http.Request) {
	...
	command := fmt.Sprintf("curl -fsSL %q | NEURALINVERSE_AGENT_TOKEN=%q sh", initScriptURL, agent.AuthToken.String())
	if agent.OperatingSystem == "windows" {
		command = fmt.Sprintf("$env:NEURALINVERSE_AGENT_TOKEN=%q; iwr -useb %q | iex", agent.AuthToken.String(), initScriptURL)
	}

	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.ExternalAgentCredentials{
