...
// replicas returns the number of replicas that are active in Neural Inverse Cloud.
//
...
// @Produce json
// @Tags Enterprise
// @Success 200 {array} nicloudsdk.Replica
...
func (api *API) replicas(rw http.ResponseWriter, r *http.Request) {
	...
	res := make([]nicloudsdk.Replica, 0, len(replicas))
...
func convertReplica(replica database.Replica) nicloudsdk.Replica {
	return nicloudsdk.Replica{
		ID:              replica.ID,
