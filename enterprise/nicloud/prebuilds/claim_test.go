		...
		for _, useDefaultOrg := range []bool{true, false} {
			t.Run(name, func(t *testing.T) {
				...
				reconciler := prebuilds.NewStoreReconciler(
					spy, pubsub, cache, nicloudsdk.PrebuildsConfig{}, logger,
					quartz.NewMock(t),
				...
				version := nicloudtest.CreateTemplateVersion(t, client, orgID, templateWithAgentAndPresetsWithPrebuilds(desiredInstances))
				...
				userClient, user := nicloudtest.CreateAnotherUser(t, client, orgID, rbac.RoleMember())
				...
				userWorkspace, err := userClient.CreateUserWorkspace(ctx, user.Username, nicloudsdk.CreateWorkspaceRequest{
				...
				case tc.claimingErr != nil && (isNoPrebuiltWorkspaces || isUnsupported):
					require.NoError(t, err)
					nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, userWorkspace.LatestBuild.ID)
				...
				stopBuild, err := userClient.CreateWorkspaceBuild(ctx, workspace.ID, nicloudsdk.CreateWorkspaceBuildRequest{
					TemplateVersionID: version.ID,
					Transition:        nicloudsdk.WorkspaceTransitionStop,
				})
				require.NoError(t, err)
				build := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, stopBuild.ID)
				require.Equal(t, build.Job.Status, nicloudsdk.ProvisionerJobSucceeded)

				startBuild, err := userClient.CreateWorkspaceBuild(ctx, workspace.ID, nicloudsdk.CreateWorkspaceBuildRequest{
					TemplateVersionID:   version.ID,
					Transition:          nicloudsdk.WorkspaceTransitionStart,
					RichParameterValues: wp,
				})
				require.NoError(t, err)
				build = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, userClient, startBuild.ID)
				require.Equal(t, build.Job.Status, nicloudsdk.ProvisionerJobSucceeded)
