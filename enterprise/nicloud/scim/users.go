...
// Create implements scim.ResourceHandler. Creates a new Neural Inverse Cloud user from
...
func (ru *ResourceUser) Create(r *http.Request, attributes scim.ResourceAttributes) (scim.Resource, error) {
	...
	usernameValidErr := nicloudsdk.NameValid(username)
	...
	if err == nil {
		...
		// However, Neural Inverse Cloud never deletes a user. So suspended **is** deleted.
...
// Delete implements scim.ResourceHandler. Suspends the user (Neural Inverse Cloud does
...
// scimUserStatus maps the SCIM "active" boolean to Neural Inverse Cloud's internal user status.
