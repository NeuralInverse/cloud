...
func TestProvisionerKeys(t *testing.T) {
	...
	outsideOrgAdmin, _ := nicloudtest.CreateAnotherUser(t, client, otherOrg.ID, rbac.ScopedRoleOrgAdmin(otherOrg.ID))

	// member cannot create a provisioner key
	_, err := member.CreateProvisionerKey(ctx, otherOrg.ID, nicloudsdk.CreateProvisionerKeyRequest{
		Name: "key",
	})
	...
	_, err = outsideOrgAdmin.CreateProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.CreateProvisionerKeyRequest{
		Name: "key",
	})
	require.ErrorContains(t, err, "Resource not found")
	...
	_, err = orgAdmin.CreateProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.CreateProvisionerKeyRequest{
		Name: nicloudsdk.ProvisionerKeyNameBuiltIn,
	})
	...
	require.ErrorContains(t, err, "reserved")
	_, err = orgAdmin.CreateProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.CreateProvisionerKeyRequest{
		Name: nicloudsdk.ProvisionerKeyNamePSK,
	})
	require.ErrorContains(t, err, "reserved")
	...
	err = orgAdmin.DeleteProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.ProvisionerKeyNameBuiltIn)
	require.ErrorContains(t, err, "reserved")
	err = orgAdmin.DeleteProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.ProvisionerKeyNameUserAuth)
	require.ErrorContains(t, err, "reserved")
	err = orgAdmin.DeleteProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.ProvisionerKeyNamePSK)
	require.ErrorContains(t, err, "reserved")
}
...
func TestGetProvisionerKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			key, err := client.CreateProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.CreateProvisionerKeyRequest{
				Name: "my-test-key",
			...
			if !tt.success {
				...
				require.Equal(t, fetchedKey.Tags, nicloudsdk.ProvisionerKeyTags{"key1": "value1", "key2": "value2"})
			}
		})
	}
	...
	t.Run("TestPSK", func(t *testing.T) {
		...
		//nolint:gocritic // ignore This client is operating as the owner user, which has unrestricted permissions
		_, err := client.CreateProvisionerKey(ctx, owner.OrganizationID, nicloudsdk.CreateProvisionerKeyRequest{
			Name: "my-test-key",
			Tags: map[string]string{"key1": "value1", "key2": "value2"},
		})
		require.NoError(t, err)
