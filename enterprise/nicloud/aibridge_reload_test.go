...
func startTestAIBridgeDaemon(t *testing.T, api *nicloud.API) *aibridged.Metrics {
	t.Helper()
...
// reload -> routing path: every provider mutation made through nicloudsdk
...
func TestAIBridgeProviderHotReload(t *testing.T) {
	...
	client, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
		Options: &nicloudtest.Options{DeploymentValues: dv},
		LicenseOptions: &nicloudenttest.LicenseOptions{
			Features: license.Features{nicloudsdk.FeatureAIBridge: 1},
		},
	})
	...
	_, err = client.UpdateAIProvider(ctx, "primary", nicloudsdk.UpdateAIProviderRequest{
		BaseURL: &newBaseURL,
	})
	require.NoError(t, err)
