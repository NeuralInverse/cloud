...
func TestCustomLogoAndCompanyName(t *testing.T) {
	...
	adminClient, adminUser := nicloudenttest.New(t, &nicloudenttest.Options{DontAddLicense: true})
	nicloudenttest.AddLicense(t, adminClient, nicloudenttest.LicenseOptions{
		Features: license.Features{
			nicloudsdk.FeatureAppearance: 1,
		},
	})
	...
	uac := nicloudsdk.UpdateAppearanceConfig{
...
func TestAnnouncementBanners(t *testing.T) {
	...
	t.Run("User", func(t *testing.T) {
		...
		adminClient, adminUser := nicloudenttest.New(t, &nicloudenttest.Options{DontAddLicense: true})
		basicUserClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
		...
		uac := nicloudsdk.UpdateAppearanceConfig{
			AnnouncementBanners: []nicloudsdk.BannerConfig{{Enabled: true}},
		}
		...
		// But an admin can
		wantBanner := nicloudsdk.UpdateAppearanceConfig{
	...
	t.Run("Agent", func(t *testing.T) {
		...
		lic := nicloudenttest.AddLicense(t, client, nicloudenttest.LicenseOptions{
			Features: license.Features{
				nicloudsdk.FeatureAppearance: 1,
			},
		})
		cfg := nicloudsdk.UpdateAppearanceConfig{
			AnnouncementBanners: []nicloudsdk.BannerConfig{{
				Enabled:         true,
				Message:         "The beep-bop will be boop-beeped on Saturday at 12AM PST.",
				BackgroundColor: "#00FF00",
		...
		agplClient := nicloudtest.New(t, &nicloudtest.Options{Database: store, Pubsub: ps})
		...
		require.Equal(t, []nicloudsdk.BannerConfig{}, banners)
...
func requireGetAnnouncementBanners(ctx context.Context, t *testing.T, client *agentsdk.Client) []nicloudsdk.BannerConfig {
...
func TestCustomSupportLinks(t *testing.T) {
	...
	cfg.Support.Links = serpent.Struct[[]nicloudsdk.LinkConfig]{
		Value: supportLinks,
	}
...
func TestCustomDocsURL(t *testing.T) {
	...
	adminClient, adminUser := nicloudenttest.New(t, &nicloudenttest.Options{DontAddLicense: true, Options: &nicloudtest.Options{DeploymentValues: cfg}})
	anotherClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
...
func TestDefaultSupportLinksWithCustomDocsUrl(t *testing.T) {
	...
	appr, err := anotherClient.Appearance(ctx)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.DefaultSupportLinks(testURLRawString), appr.SupportLinks)
}
...
func TestDefaultSupportLinks(t *testing.T) {
	...
	appr, err := anotherClient.Appearance(ctx)
	require.NoError(t, err)
	require.Equal(t, nicloudsdk.DefaultSupportLinks(nicloudsdk.DefaultDocsURL()), appr.SupportLinks)
}
