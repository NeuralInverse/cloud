...
// key20220812 is the Neural Inverse Cloud license public key with id 2022-08-12 used to validate licenses signed
...
//
//  1. Upgrades --- if the license format changes from one version of Neural Inverse Cloud to the next, during a
//     rolling update you will have different Neural Inverse Cloud servers that need different licenses to function.
...
// @Produce json
// @Tags Enterprise
// @Param request body nicloudsdk.AddLicenseRequest true "Add license request"
// @Success 201 {object} nicloudsdk.License
...
func (api *API) postLicense(rw http.ResponseWriter, r *http.Request) {
	...
	var addLicense nicloudsdk.AddLicenseRequest
...
// @Produce json
// @Tags Enterprise
// @Success 201 {object} nicloudsdk.Response
...
// @Produce json
// @Tags Enterprise
// @Success 200 {array} nicloudsdk.License
...
func (api *API) licenses(rw http.ResponseWriter, r *http.Request) {
	...
	if xerrors.Is(err, sql.ErrNoRows) {
		httpapi.Write(ctx, rw, http.StatusOK, []nicloudsdk.License{})
		return
	}
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
	...
	licenses, err = nicloud.AuthorizeFilter(api.AGPL.HTTPAuth, r, policy.ActionRead, licenses)
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, nicloudsdk.Response{
			Message: "Internal error fetching licenses.",
			Detail:  err.Error(),
...
func convertLicense(dl database.License, c jwt.MapClaims) nicloudsdk.License {
	return nicloudsdk.License{
		ID:         dl.ID,
		UUID:       dl.UUID,
...
func convertLicenses(licenses []database.License) ([]nicloudsdk.License, error) {
	out := make([]nicloudsdk.License, 0, len(licenses))
...
// if they are expired, or signed by a key this version of Neural Inverse Cloud no longer considers valid.
//
