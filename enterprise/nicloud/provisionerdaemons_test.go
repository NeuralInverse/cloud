...
func TestProvisionerDaemonServe(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		srv, err := templateAdminClient.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []nicloudsdk.ProvisionerType{
				nicloudsdk.ProvisionerTypeEcho,
			},
	...
	t.Run("NoVersion", func(t *testing.T) {
		...
		require.NoError(t, err)
		req.Header.Set(nicloudsdk.ProvisionerDaemonPSK, "provisionersftw")
	...
	t.Run("NoLicense", func(t *testing.T) {
		t.Parallel()
		client, user := nicloudenttest.New(t, &nicloudenttest.Options{DontAddLicense: true})
		templateAdminClient, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin())
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		daemonName := testutil.MustRandString(t, 63)
		_, err := templateAdminClient.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []nicloudsdk.ProvisionerType{
				nicloudsdk.ProvisionerTypeEcho,
	...
	t.Run("Organization", func(t *testing.T) {
		...
		another, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.ScopedRoleOrgAdmin(user.OrganizationID))
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		_, err := another.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
	...
	t.Run("OrganizationNoPerms", func(t *testing.T) {
		...
		another, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		_, err := another.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         testutil.MustRandString(t, 63),
	...
	t.Run("UserLocal", func(t *testing.T) {
		...
		closer := nicloudenttest.NewExternalProvisionerDaemon(t, client, user.OrganizationID, map[string]string{
		...
		file, err := client.Upload(context.Background(), nicloudsdk.ContentTypeTar, bytes.NewReader(data))
		require.NoError(t, err)
		...
		version, err := client.CreateTemplateVersion(context.Background(), user.OrganizationID, nicloudsdk.CreateTemplateVersionRequest{
			Name:          "example",
		...
		another, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		_ = closer.Close()
		closer = nicloudenttest.NewExternalProvisionerDaemon(t, another, user.OrganizationID, map[string]string{
			provisionersdk.TagScope: provisionersdk.ScopeUser,
		})
		defer closer.Close()
		workspace := nicloudtest.CreateWorkspace(t, another, template.ID)
	...
	t.Run("PSK", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		another := nicloudsdk.New(client.URL)
		daemonName := testutil.MustRandString(t, 63)
		srv, err := another.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: user.OrganizationID,
			Provisioners: []nicloudsdk.ProvisionerType{
				nicloudsdk.ProvisionerTypeEcho,
	...
	t.Run("ChangeTags", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitLong)
		defer cancel()
		req := nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         testutil.MustRandString(t, 63),
			Organization: user.OrganizationID,
			Provisioners: []nicloudsdk.ProvisionerType{
				nicloudsdk.ProvisionerTypeEcho,
	...
	t.Run("PSK_daily_cost", func(t *testing.T) {
		...
		pd := provisionerd.New(func(ctx context.Context) (proto.DRPCProvisionerDaemonClient, error) {
			return another.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
				Name:         testutil.MustRandString(t, 63),
				Organization: user.OrganizationID,
				Provisioners: []nicloudsdk.ProvisionerType{
					nicloudsdk.ProvisionerTypeEcho,
		...
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.multiOrgFeatureEnabled {
					features[nicloudsdk.FeatureMultipleOrganizations] = 1
				}
...
func TestGetProvisionerDaemons(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		org := nicloudenttest.CreateOrganization(t, client, nicloudenttest.CreateOrganizationOptions{})
		orgAdmin, _ := nicloudtest.CreateAnotherUser(t, client, org.ID, rbac.ScopedRoleOrgAdmin(org.ID))
		outsideOrg, _ := nicloudtest.CreateAnotherUser(t, client, first.OrganizationID)

		res, err := orgAdmin.CreateProvisionerKey(context.Background(), org.ID, nicloudsdk.CreateProvisionerKeyRequest{
			Name: "my-key",
		})
		require.NoError(t, err)
		...
		defer cancel()
		daemonName := testutil.MustRandString(t, 63)
		srv, err := orgAdmin.ServeProvisionerDaemon(ctx, nicloudsdk.ServeProvisionerDaemonRequest{
			Name:         daemonName,
			Organization: org.ID,
			Provisioners: []nicloudsdk.ProvisionerType{
				nicloudsdk.ProvisionerTypeEcho,
		...
		assert.Equal(t, nicloudsdk.ProvisionerKeyUUIDUserAuth, pkDaemons[1].Key.ID)
		assert.Equal(t, nicloudsdk.ProvisionerKeyNameUserAuth, pkDaemons[1].Key.Name)
	...
	t.Run("filtered by tags", func(t *testing.T) {
		...
		for _, tt := range testCases {
			t.Run(tt.name, func(t *testing.T) {
				t.Parallel()
				dv := nicloudtest.DeploymentValues(t)
				client, db, _ := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
					Options: &nicloudtest.Options{
						DeploymentValues: dv,
					},
					ProvisionerDaemonPSK: "provisionersftw",
				...
				orgTemplateAdmin, _ := nicloudtest.CreateAnotherUser(t, client, org.ID, rbac.ScopedRoleOrgTemplateAdmin(org.ID))
				...
				daemonsAsFound, err := orgTemplateAdmin.OrganizationProvisionerDaemons(ctx, org.ID, &nicloudsdk.OrganizationProvisionerDaemonsOptions{
