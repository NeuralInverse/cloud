...
import (
	...
	agplschedule "github.com/NeuralInverse/cloud/v2/nicloud/schedule"
...
func TestCreateWorkspace(t *testing.T) {
	...
	t.Run("NoTemplateAccess", func(t *testing.T) {
		...
		other, _ := nicloudtest.CreateAnotherUser(t, client, first.OrganizationID, rbac.RoleMember(), rbac.RoleOwner())

		ctx := testutil.Context(t, testutil.WaitLong)

		org, err := other.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
			Name: "another",
		})
		require.NoError(t, err)
		version := nicloudtest.CreateTemplateVersion(t, other, org.ID, nil)
		template := nicloudtest.CreateTemplate(t, other, org.ID, version.ID)

		ctx = testutil.Context(t, testutil.WaitLong) // Reset the context to avoid timeouts.

		_, err = client.CreateWorkspace(ctx, first.OrganizationID, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
	...
	t.Run("Unauthorized", func(t *testing.T) {
		...
		templateAdminClient, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID, rbac.RoleTemplateAdmin())
		...
		err = templateAdminClient.UpdateTemplateACL(ctx, template.ID, nicloudsdk.UpdateTemplateACL{
			GroupPerms: map[string]nicloudsdk.TemplateRole{
				acl.Groups[0].ID.String(): nicloudsdk.TemplateRoleDeleted,
			},
		})
		require.NoError(t, err)
	...
	t.Run("NoTemplateAccess", func(t *testing.T) {
		...
		user, _ := nicloudtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID, rbac.RoleMember())
		...
		err := templateAdmin.UpdateTemplateACL(ctx, template.ID, nicloudsdk.UpdateTemplateACL{
			UserPerms: map[string]nicloudsdk.TemplateRole{},
			GroupPerms: map[string]nicloudsdk.TemplateRole{
				owner.OrganizationID.String(): nicloudsdk.TemplateRoleDeleted,
			},
		})
		require.NoError(t, err)
		...
		_, err = user.CreateUserWorkspace(ctx, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
			TemplateID:        template.ID,
			Name:              "random",
			AutostartSchedule: ptr.Ref("CRON_TZ=US/Central 30 9 * * 1-5"),
			TTLMillis:         ptr.Ref((8 * time.Hour).Milliseconds()),
	...
	t.Run("Auditor", func(t *testing.T) {
		...
		auditor, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleAuditor())
		...
		err := owner.UpdateTemplateACL(ctx, template.ID, nicloudsdk.UpdateTemplateACL{
			UserPerms: nil,
			GroupPerms: map[string]nicloudsdk.TemplateRole{
		...
		_, err = auditor.CreateUserWorkspace(ctx, nicloudsdk.Me, nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "workspace",
		})
		require.Error(t, err)
...
func TestCreateUserWorkspace(t *testing.T) {
	...
	t.Run("ForAnotherUser", func(t *testing.T) {
		...
		r, err := owner.CreateOrganizationRole(ctx, nicloudsdk.Role{
			Name:           "creator",
			OrganizationID: first.OrganizationID.String(),
			DisplayName:    "Creator",
			OrganizationPermissions: nicloudsdk.CreatePermissions(map[nicloudsdk.RBACResource][]nicloudsdk.RBACAction{
				nicloudsdk.ResourceWorkspace:          {nicloudsdk.ActionCreate, nicloudsdk.ActionWorkspaceStart, nicloudsdk.ActionUpdate, nicloudsdk.ActionRead},
				nicloudsdk.ResourceOrganizationMember: {nicloudsdk.ActionRead},
			}),
		})
		require.NoError(t, err)

		// use admin for setting up test
		admin, adminID := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleTemplateAdmin())

		// try the test action with this user & custom role
		creator, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleMember(), rbac.RoleIdentifier{
			Name:           r.Name,
			OrganizationID: first.OrganizationID,
		})

		template, _ := nicloudtest.DynamicParameterTemplate(t, admin, first.OrganizationID, nicloudtest.DynamicParameterTemplateParams{
			Zip: true,
		})

		ctx = testutil.Context(t, testutil.WaitLong)

		wrk, err := creator.CreateUserWorkspace(ctx, adminID.ID.String(), nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "workspace",
		})
		require.NoError(t, err)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, admin, wrk.LatestBuild.ID)

		_, err = creator.WorkspaceByOwnerAndName(ctx, adminID.Username, wrk.Name, nicloudsdk.WorkspaceOptions{
	...
	t.Run("ForANonOrgMember", func(t *testing.T) {
		...
		_, forUser := nicloudtest.CreateAnotherUser(t, owner, secondOrg.ID)

		// try the test action with this user & custom role
		creator, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleMember(),
		...
		template, _ := nicloudtest.DynamicParameterTemplate(t, creator, first.OrganizationID, nicloudtest.DynamicParameterTemplateParams{})

		ctx = testutil.Context(t, testutil.WaitLong)

		wrk, err := creator.CreateUserWorkspace(ctx, forUser.ID.String(), nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "workspace",
		})
		require.NoError(t, err)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, creator, wrk.LatestBuild.ID)

		_, err = creator.WorkspaceByOwnerAndName(ctx, forUser.Username, wrk.Name, nicloudsdk.WorkspaceOptions{
			IncludeDeleted: false,
		})
		require.NoError(t, err)
	})
	...
	t.Run("AuthzStory", func(t *testing.T) {
		t.Parallel()
		owner, _, api, first := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
			Options: &nicloudtest.Options{
				IncludeProvisionerDaemon: true,
			},
			LicenseOptions: &nicloudenttest.LicenseOptions{
		...
		//nolint:gocritic // using owner to setup roles
		creatorRole, err := owner.CreateOrganizationRole(ctx, nicloudsdk.Role{
			Name:           "creator",
			OrganizationID: first.OrganizationID.String(),
			OrganizationPermissions: nicloudsdk.CreatePermissions(map[nicloudsdk.RBACResource][]nicloudsdk.RBACAction{
				nicloudsdk.ResourceWorkspace:          {nicloudsdk.ActionCreate, nicloudsdk.ActionWorkspaceStart, nicloudsdk.ActionUpdate, nicloudsdk.ActionRead},
		...
		template := nicloudtest.CreateTemplate(t, owner, first.OrganizationID, version.ID)
		_, userID := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID)
		creator, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID, rbac.RoleIdentifier{
			Name:           creatorRole.Name,
			OrganizationID: first.OrganizationID,
		})

		// Create a workspace with the current api using an org admin.
		authz := nicloudtest.AssertRBAC(t, api.AGPL, creator)
		authz.Reset() // Reset all previous checks done in setup.
		_, err = creator.CreateUserWorkspace(ctx, userID.ID.String(), nicloudsdk.CreateWorkspaceRequest{
			TemplateID: template.ID,
			Name:       "test-user",
		})
		require.NoError(t, err)
	...
	t.Run("ClaimPrebuild", func(t *testing.T) {
		...
		_, err = client.CreateUserWorkspace(ctx, user.UserID.String(), nicloudsdk.CreateWorkspaceRequest{
			TemplateVersionID:       tv.TemplateVersion.ID,
...
func TestWorkspaceAutobuild(t *testing.T) {
	...
	t.Run("FailureTTLOK", func(t *testing.T) {
		...
		client, db, user := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
			...
			LicenseOptions: &nicloudenttest.LicenseOptions{
				Features: license.Features{nicloudsdk.FeatureAdvancedTemplateScheduling: 1},
			},
		})
		...
		build := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
		require.Equal(t, nicloudsdk.WorkspaceStatusFailed, build.Status)
		tickTime := build.Job.CompletedAt.Add(failureTTL * 2)

		p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), ws.OrganizationID, nil)
		require.NoError(t, err)
	...
	t.Run("TemplateTTLsUnset", func(t *testing.T) {
		...
		ws := nicloudtest.CreateWorkspace(t, client, template.ID)
		build := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
		require.Equal(t, nicloudsdk.WorkspaceStatusRunning, build.Status)
		ticker <- time.Now()
		stats := <-statCh
	...
	t.Run("DormancyThresholdOK", func(t *testing.T) {
		...
		template := nicloudtest.UpdateTemplateMeta(t, client, tpl.ID, nicloudsdk.UpdateTemplateMeta{
		...
		p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, nil)
		require.NoError(t, err)
		nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, tickTime)
		ticker <- tickTime
		stats := <-statCh
		...
		ws := nicloudtest.MustWorkspace(t, client, workspace.ID)
		// Should be dormant now.
		require.NotNil(t, ws.DormantAt)
		// Should be transitioned to stop.
		require.Equal(t, nicloudsdk.WorkspaceTransitionStop, ws.LatestBuild.Transition)
		...
		err = client.UpdateWorkspaceDormancy(ctx, ws.ID, nicloudsdk.UpdateWorkspaceDormancy{Dormant: false})
		require.NoError(t, err)

		// Assert that we updated our last_used_at so that we don't immediately
		// retrigger another lock action.
		ws = nicloudtest.MustWorkspace(t, client, ws.ID)
	...
	t.Run("DormantAutostop", func(t *testing.T) {
		...
		// This simulates a template without `count = data.ni_workspace.me.start_count`.
		...
		// agents without `count = data.ni_workspace.me.start_count`.
		...
		latestBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, ws.LatestBuild.ID)
		require.Equal(t, nicloudsdk.WorkspaceStatusStopped, latestBuild.Status)
	})
	...
	t.Run("NoDeadlock", func(t *testing.T) {
		...
		// initialize our nicloud with a real auditor and limit DB connections
		...
		p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), workspaces[0].OrganizationID, nil)
		require.NoError(t, err)
		nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, tickTime)
		ticker <- tickTime
		stats := <-statCh
	...
	t.Run("InactiveStoppedWorkspaceNoTransition", func(t *testing.T) {
		...
		ws = nicloudtest.MustTransitionWorkspace(t, client, ws.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
	...
	t.Run("WorkspaceInactiveDeleteTransition", func(t *testing.T) {
		...
		nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, tickTime2)
		ticker <- tickTime2
		stats = <-statCh
		require.Len(t, stats.Transitions, 1)
	...
	t.Run("DormantTTLTooEarly", func(t *testing.T) {
		...
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		ws := nicloudtest.CreateWorkspace(t, anotherClient, template.ID)
		build := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, anotherClient, ws.LatestBuild.ID)
		require.Equal(t, nicloudsdk.WorkspaceStatusRunning, build.Status)

		ctx := testutil.Context(t, testutil.WaitMedium)
		err := anotherClient.UpdateWorkspaceDormancy(ctx, ws.ID, nicloudsdk.UpdateWorkspaceDormancy{
			Dormant: true,
		})
		require.NoError(t, err)
	...
	t.Run("FailedDeleteRetryDaily", func(t *testing.T) {
		...
		ws := nicloudtest.CreateWorkspace(t, templateAdmin, template.ID)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, templateAdmin, ws.LatestBuild.ID)
		...
		build, err := templateAdmin.CreateWorkspaceBuild(ctx, ws.ID, nicloudsdk.CreateWorkspaceBuildRequest{
			Transition:        nicloudsdk.WorkspaceTransitionDelete,
			TemplateVersionID: version.ID,
		})
		require.NoError(t, err)

		build = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, build.ID)
		require.NotEmpty(t, build.Job.Error)

		ctx = testutil.Context(t, testutil.WaitLong) // Reset the context to avoid timeouts.

		// Update our workspace to be dormant so that it qualifies for auto-deletion.
		err = templateAdmin.UpdateWorkspaceDormancy(ctx, ws.ID, nicloudsdk.UpdateWorkspaceDormancy{
			Dormant: true,
		})
		require.NoError(t, err)

		// Enable auto-deletion for the template.
		_, err = templateAdmin.UpdateTemplateMeta(ctx, template.ID, nicloudsdk.UpdateTemplateMeta{
	...
	t.Run("RequireActiveVersion", func(t *testing.T) {
		...
		client, db, user := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
			...
			LicenseOptions: &nicloudenttest.LicenseOptions{
				Features: license.Features{nicloudsdk.FeatureAccessControl: 1},
			},
		})
		...
		version1 := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(ctvr *nicloudsdk.CreateTemplateVersionRequest) {
			ctvr.Name = "v1"
		})
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version1.ID)

		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version1.ID)
		...
		version2 := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil, func(ctvr *nicloudsdk.CreateTemplateVersionRequest) {
			ctvr.Name = "v2"
			ctvr.TemplateID = template.ID
		})
		...
		started := nicloudtest.MustWorkspace(t, client, ws.ID)
		firstBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, started.LatestBuild.ID)
		...
		// to autostart again.
		nicloudtest.MustTransitionWorkspace(t, client, ws.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop, func(req *nicloudsdk.CreateWorkspaceBuildRequest) {
	...
	t.Run("NextStartAtIsValid", func(t *testing.T) {
		...
		version1 := nicloudtest.CreateTemplateVersion(t, client, user.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version1.ID)

		// First create a template that only supports Monday-Friday
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version1.ID, func(ctr *nicloudsdk.CreateTemplateRequest) {
			ctr.AutostartRequirement = &nicloudsdk.TemplateAutostartRequirement{DaysOfWeek: nicloudsdk.BitmapToWeekdays(0b00011111)}
		})
		require.Equal(t, version1.ID, template.ActiveVersionID)
		...
		for range 7 {
			...
			p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), ws.OrganizationID, nil)
			require.NoError(t, err)
			nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, next)
			tickCh <- next
			stats := <-statsCh
			ws = nicloudtest.MustWorkspace(t, client, ws.ID)
	...
	t.Run("NextStartAtIsUpdatedWhenTemplateAutostartRequirementsChange", func(t *testing.T) {
		...
		nicloudtest.UpdateTemplateMeta(t, client, template.ID, nicloudsdk.UpdateTemplateMeta{
			AutostartRequirement: &nicloudsdk.TemplateAutostartRequirement{
				DaysOfWeek: nicloudsdk.BitmapToWeekdays(0b00011110),
			},
		})
	...
	t.Run("NextStartAtIsNullifiedOnScheduleChange", func(t *testing.T) {
		...
		template := nicloudtest.CreateTemplate(t, client, user.OrganizationID, version.ID, func(ctr *nicloudsdk.CreateTemplateRequest) {
			ctr.AutostartRequirement = &nicloudsdk.TemplateAutostartRequirement{DaysOfWeek: nicloudsdk.AllDaysOfWeek}
		})
...
func TestTemplateDoesNotAllowUserAutostop(t *testing.T) {
	...
	t.Run("TTLSetByTemplate", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitShort)
		template = nicloudtest.UpdateTemplateMeta(t, client, template.ID, nicloudsdk.UpdateTemplateMeta{
...
func TestPrebuildsAutobuild(t *testing.T) {
	...
	claimPrebuild := func(
		...
		assert.Equal(t, nicloudsdk.WorkspaceTransitionStart, workspace.LatestBuild.Transition)
	...
	t.Run("DefaultTTLOnlyTriggersAfterClaim", func(t *testing.T) {
		...
		ttlTime := 2 * time.Hour
		userClient, user := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleMember())
		version := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, templateWithAgentAndPresetsWithPrebuilds(prebuildInstances))
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		// Set a template level TTL to trigger the autostop
		// Template level TTL can only be set if autostop is disabled for users
		nicloudtest.CreateTemplate(t, client, owner.OrganizationID, version.ID, func(ctr *nicloudsdk.CreateTemplateRequest) {
		...
		require.Equal(t, nicloudsdk.WorkspaceTransitionStart, prebuild.LatestBuild.Transition)
		prebuild = nicloudtest.MustWorkspace(t, client, prebuild.ID)
		require.Equal(t, nicloudsdk.BuildReasonInitiator, prebuild.LatestBuild.Reason)
		require.Zero(t, prebuild.LatestBuild.Deadline)
		require.Zero(t, prebuild.LatestBuild.MaxDeadline)
		...
		require.Equal(t, nicloudsdk.BuildReasonAutostop, workspace.LatestBuild.Reason)
	})
	...
	t.Run("AutostopScheduleOnlyTriggersAfterClaim", func(t *testing.T) {
		...
		nicloudtest.CreateTemplate(t, client, owner.OrganizationID, version.ID, func(ctr *nicloudsdk.CreateTemplateRequest) {
			ctr.AutostopRequirement = ptr.Ref[nicloudsdk.TemplateAutostopRequirement](
	...
	t.Run("AutostartScheduleOnlyTriggersAfterClaim", func(t *testing.T) {
		...
		prebuild = nicloudtest.MustTransitionWorkspace(t, client, prebuild.ID, nicloudsdk.WorkspaceTransitionStart, nicloudsdk.WorkspaceTransitionStop)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, prebuild.LatestBuild.ID)
		...
		require.Len(t, prebuildStats.Errors, 0)
		require.Len(t, prebuildStats.Transitions, 0)
		require.Equal(t, nicloudsdk.WorkspaceTransitionStop, prebuild.LatestBuild.Transition)
		prebuild = nicloudtest.MustWorkspace(t, client, prebuild.ID)
		require.Equal(t, nicloudsdk.BuildReasonInitiator, prebuild.LatestBuild.Reason)
		require.Nil(t, prebuild.AutostartSchedule)
		require.Nil(t, prebuild.NextStartAt)
		...
		prebuild = nicloudtest.MustTransitionWorkspace(t, client, prebuild.ID, nicloudsdk.WorkspaceTransitionStop, nicloudsdk.WorkspaceTransitionStart)
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, prebuild.LatestBuild.ID)
		...
		p, err := nicloudtest.GetProvisionerForTags(db, time.Now(), workspace.OrganizationID, nil)
		nicloudtest.UpdateProvisionerLastSeenAt(t, db, p.ID, sched.Next(prebuild.LatestBuild.CreatedAt))

		// Wait for provisioner to be available for this specific workspace
		nicloudtest.MustWaitForProvisionersAvailable(t, db, prebuild, sched.Next(prebuild.LatestBuild.CreatedAt))
	...
	t.Run("DormantOnlyAfterClaimed", func(t *testing.T) {
		...
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
		workspace = nicloudtest.MustWorkspace(t, client, workspace.ID)
		require.Equal(t, nicloudsdk.BuildReasonDormancy, workspace.LatestBuild.Reason)
		require.Equal(t, nicloudsdk.WorkspaceStatusStopped, workspace.LatestBuild.Status)
	...
	t.Run("FailureTTLOnlyAfterClaimed", func(t *testing.T) {
		...
		version := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, templateWithFailedResponseAndPresetsWithPrebuilds(prebuildInstances))
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		...
		prebuild := nicloudtest.MustWorkspace(t, client, failedWorkspaceBuilds[0].WorkspaceID)
		require.Equal(t, nicloudsdk.WorkspaceStatusFailed, prebuild.LatestBuild.Status)
...
func TestPrebuildUpdateLifecycleParams(t *testing.T) {
	...
	cases := []struct {
		name         string
		endpoint     func(*testing.T, context.Context, *nicloudsdk.Client, uuid.UUID) error
		apiErrorMsg  string
		assertUpdate func(*testing.T, *quartz.Mock, *nicloudsdk.Client, uuid.UUID)
	}{
		{
			name: "AutostartUpdatePrebuildAfterClaim",
			endpoint: func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, workspaceID uuid.UUID) error {
				err = client.UpdateWorkspaceAutostart(ctx, workspaceID, nicloudsdk.UpdateWorkspaceAutostartRequest{
					Schedule: ptr.Ref(autostartSchedule.String()),
				})
				return err
			},
			apiErrorMsg: "Autostart is not supported for prebuilt workspaces",
			assertUpdate: func(t *testing.T, clock *quartz.Mock, client *nicloudsdk.Client, workspaceID uuid.UUID) {
				...
				updatedWorkspace := nicloudtest.MustWorkspace(t, client, workspaceID)
		...
		},
		{
			...
			endpoint: func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, workspaceID uuid.UUID) error {
				err := client.UpdateWorkspaceTTL(ctx, workspaceID, nicloudsdk.UpdateWorkspaceTTLRequest{
					TTLMillis: ttlMillis,
				})
				return err
			},
		...
		},
		{
			...
			endpoint: func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, workspaceID uuid.UUID) error {
				err := client.UpdateWorkspaceDormancy(ctx, workspaceID, nicloudsdk.UpdateWorkspaceDormancy{
					Dormant: true,
				})
				return err
			},
		...
		},
		{
			...
			endpoint: func(t *testing.T, ctx context.Context, client *nicloudsdk.Client, workspaceID uuid.UUID) error {
				err := client.PutExtendWorkspace(ctx, workspaceID, nicloudsdk.PutExtendWorkspaceRequest{
					Deadline: deadline,
				})
				return err
			},
...
func TestPrebuildActivityBump(t *testing.T) {
	...
	prebuild := nicloudtest.MustWorkspace(t, client, wb.Workspace.ID)
...
func TestWorkspaceProvisionerdServerMetrics(t *testing.T) {
	...
	versionPrebuild := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, templateWithAgentAndPresetsWithPrebuilds(1))
	nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionPrebuild.ID)
	templatePrebuild := nicloudtest.CreateTemplate(t, client, owner.OrganizationID, versionPrebuild.ID)
	...
	versionNoPrebuild := nicloudtest.CreateTemplateVersion(t, client, owner.OrganizationID, templateWithAgentAndPresetsWithPrebuilds(0))
	nicloudtest.AwaitTemplateVersionJobCompleted(t, client, versionNoPrebuild.ID)
	templateNoPrebuild := nicloudtest.CreateTemplate(t, client, owner.OrganizationID, versionNoPrebuild.ID)
	...
	prebuildCreationMetric := promhelp.MetricValue(t, reg, "nicloud_workspace_creation_duration_seconds", prometheus.Labels{
	...
	// Given: reconciliation loop runs and starts prebuilt workspace
	nicloudenttest.MustRunReconciliationLoopForPreset(ctx, t, db, reconciler, presetsPrebuild[0])
	runningPrebuilds := nicloudenttest.GetRunningPrebuilds(ctx, t, db, 1)
	...
	require.Eventually(t, func() bool {
		return promhelp.MetricValue(t, reg, "nicloud_workspace_creation_duration_seconds", prebuildCreationLabels) != nil
	}, testutil.WaitShort, testutil.IntervalFast)
	prebuildCreationHistogram := promhelp.HistogramValue(t, reg, "nicloud_workspace_creation_duration_seconds", prebuildCreationLabels)
	...
	prebuildClaimMetric := promhelp.MetricValue(t, reg, "nicloud_prebuilt_workspace_claim_duration_seconds", prometheus.Labels{
		"organization_name": organizationName.Name,
		"template_name":     templatePrebuild.Name,
		"preset_name":       presetsPrebuild[0].Name,
	})
	...
	// Given: the prebuilt workspace is claimed by a user
	workspace := nicloudenttest.MustClaimPrebuild(ctx, t, client, userClient, user.Username, versionPrebuild, presetsPrebuild[0].ID)
	require.Equal(t, prebuild.ID, workspace.ID)
	...
	require.Eventually(t, func() bool {
		return promhelp.MetricValue(t, reg, "nicloud_prebuilt_workspace_claim_duration_seconds", prebuildClaimLabels) != nil
	}, testutil.WaitShort, testutil.IntervalFast)
	prebuildClaimHistogram := promhelp.HistogramValue(t, reg, "nicloud_prebuilt_workspace_claim_duration_seconds", prebuildClaimLabels)
	require.Equal(t, uint64(1), prebuildClaimHistogram.GetSampleCount())

	// Given: no histogram value for regular workspaces creation
	regularWorkspaceHistogramMetric := promhelp.MetricValue(t, reg, "nicloud_workspace_creation_duration_seconds", prometheus.Labels{
		"organization_name": organizationName.Name,
	...
	regularWorkspace, err := client.CreateUserWorkspace(ctx, user.ID.String(), nicloudsdk.CreateWorkspaceRequest{
		TemplateVersionID:       versionNoPrebuild.ID,
		TemplateVersionPresetID: presetsNoPrebuild[0].ID,
		Name:                    nicloudtest.RandomUsername(t),
	})
	require.NoError(t, err)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, regularWorkspace.LatestBuild.ID)
	...
	require.Eventually(t, func() bool {
		return promhelp.MetricValue(t, reg, "nicloud_workspace_creation_duration_seconds", regularWorkspaceLabels) != nil
	}, testutil.WaitShort, testutil.IntervalFast)
	regularWorkspaceHistogram := promhelp.HistogramValue(t, reg, "nicloud_workspace_creation_duration_seconds", regularWorkspaceLabels)
...
func TestWorkspaceTemplateParamsChange(t *testing.T) {
	...
	mainTfTemplate := fmt.Sprintf(`
		...
		data "ni_parameter" "param_min" {
			name = "param_min"
			type = "number"
			default = 10
		}

		data "ni_parameter" "param" {
			name    = "param"
			type    = "number"
			default = 12
			validation {
				min = data.ni_parameter.param_min.value
			}
		}
	...
	_ = nicloudenttest.NewExternalProvisionerDaemonTerraform(t, client, owner.OrganizationID, nil)
	...
	tv, err := templateAdmin.CreateTemplateVersion(ctx, owner.OrganizationID, nicloudsdk.CreateTemplateVersionRequest{
	...
	nicloudtest.AwaitTemplateVersionJobCompleted(t, templateAdmin, tv.ID)
	tpl := nicloudtest.CreateTemplate(t, templateAdmin, owner.OrganizationID, tv.ID)

	// Set to dynamic params
	tpl, err = client.UpdateTemplateMeta(ctx, tpl.ID, nicloudsdk.UpdateTemplateMeta{
	...
	ws, err := member.CreateUserWorkspace(ctx, memberUser.Username, nicloudsdk.CreateWorkspaceRequest{
		TemplateID: tpl.ID,
		Name:       nicloudtest.RandomUsername(t),
	...
	createBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, member, ws.LatestBuild.ID)
	require.Equal(t, createBuild.Status, nicloudsdk.WorkspaceStatusRunning)
	...
	build, err := member.CreateWorkspaceBuild(ctx, ws.ID, nicloudsdk.CreateWorkspaceBuildRequest{
		Transition: nicloudsdk.WorkspaceTransitionDelete,
	})
	require.NoError(t, err)
	build = nicloudtest.AwaitWorkspaceBuildJobCompleted(t, member, build.ID)
...
type testWorkspaceTagsTerraformCase struct {
	...
	// the ni_workspace_tags bit of main.tf.
	// you can add more stuff here if you need
	tfWorkspaceTags                  string
	templateImportUserVariableValues []nicloudsdk.VariableValue
	// if we need to set parameters on workspace build
	workspaceBuildParameters []nicloudsdk.WorkspaceBuildParameter
	...
	for _, tc := range []testWorkspaceTagsTerraformCase{
		...
		},
		{
			...
			tfWorkspaceTags: `
				...
				data "ni_workspace_tags" "tags" {
					tags = {
						"foo" = data.ni_parameter.foo.value
					}
				}`,
		},
		{
		...
		},
		{
			...
			templateImportUserVariableValues: []nicloudsdk.VariableValue{{Name: "default_foo", Value: "baz"}, {Name: "foo", Value: "bar,baz"}},
			workspaceBuildParameters:         []nicloudsdk.WorkspaceBuildParameter{{Name: "foo", Value: "bar"}},
			tfWorkspaceTags: `
...
func workspaceTagsTerraform(t *testing.T, tc testWorkspaceTagsTerraformCase, dynamic bool) {
	...
	emptyTv, err := templateAdmin.CreateTemplateVersion(ctx, owner.OrganizationID, nicloudsdk.CreateTemplateVersionRequest{
		Name:          testutil.GetRandomName(t),
		FileID:        emptyFi.ID,
		StorageMethod: nicloudsdk.ProvisionerStorageMethodFile,
		Provisioner:   nicloudsdk.ProvisionerTypeTerraform,
	...
	tpl := nicloudtest.CreateTemplate(t, templateAdmin, owner.OrganizationID, emptyTv.ID, func(request *nicloudsdk.CreateTemplateRequest) {
		request.UseClassicParameterFlow = ptr.Ref(!dynamic)
	})

	// The provisioner for the next template version
	_ = nicloudenttest.NewExternalProvisionerDaemonTerraform(t, client, owner.OrganizationID, tc.provisionerTags)
	...
	err = templateAdmin.UpdateActiveTemplateVersion(ctx, tpl.ID, nicloudsdk.UpdateActiveTemplateVersion{
		ID: tv.ID,
	})
	...
	if !tc.skipCreateWorkspace {
		...
		nicloudtest.AwaitWorkspaceBuildJobCompleted(t, member, ws.LatestBuild.ID)
	}
}
...
func TestExecutorAutostartBlocked(t *testing.T) {
	...
	var (
		...
		template = nicloudtest.CreateTemplate(t, client, owner.OrganizationID, version.ID, func(request *nicloudsdk.CreateTemplateRequest) {
			request.AutostartRequirement = &nicloudsdk.TemplateAutostartRequirement{
				DaysOfWeek: allowed,
			}
		})
		_         = nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
...
func TestWorkspacesFiltering(t *testing.T) {
	...
	t.Run("Dormant", func(t *testing.T) {
		...
		templateAdminClient, templateAdmin := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID, rbac.RoleTemplateAdmin())
		...
		err := templateAdminClient.UpdateWorkspaceDormancy(ctx, dormantWS1.ID, nicloudsdk.UpdateWorkspaceDormancy{Dormant: true})
		require.NoError(t, err)

		err = templateAdminClient.UpdateWorkspaceDormancy(ctx, dormantWS2.ID, nicloudsdk.UpdateWorkspaceDormancy{Dormant: true})
		require.NoError(t, err)

		workspaces, err := templateAdminClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
	...
	t.Run("SharedWithGroup", func(t *testing.T) {
		...
		_, workspaceOwner := nicloudtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID, rbac.ScopedRoleOrgAuditor(owner.OrganizationID))
		...
		group, err := ownerClient.CreateGroup(ctx, owner.OrganizationID, nicloudsdk.CreateGroupRequest{
			Name: "wibble",
		})
		require.NoError(t, err, "create group")

		err = ownerClient.UpdateWorkspaceACL(ctx, sharedWorkspace.ID, nicloudsdk.UpdateWorkspaceACL{
		...
		workspaces, err := ownerClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
			Shared: ptr.Ref(true),
		})
	...
	t.Run("SharedWithUserAndGroup", func(t *testing.T) {
		...
		var (
			...
			_, toShareWithUser = nicloudtest.CreateAnotherUser(t, ownerClient, owner.OrganizationID)
			ctx                = testutil.Context(t, testutil.WaitMedium)
		)
	...
	t.Run("SharedWithGroupFilter", func(t *testing.T) {
		...
		workspacesByID, err := ownerClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
			SharedWithGroup: group.ID.String(),
		})
		require.NoError(t, err)
		...
		workspacesByName, err := ownerClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
		...
		workspacesByOrgAndName, err := ownerClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
...
func TestWorkspacesWithoutTemplatePerms(t *testing.T) {
	...
	user, _ := nicloudtest.CreateAnotherUser(t, client, first.OrganizationID)
	workspace := nicloudtest.CreateWorkspace(t, user, template.ID)
	...
	_, err = user.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	require.NoError(t, err, "fetch workspaces should not fail")

	// Now create another workspace the user can read.
	version2 := nicloudtest.CreateTemplateVersion(t, client, first.OrganizationID, nil)
	nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version2.ID)
	template2 := nicloudtest.CreateTemplate(t, client, first.OrganizationID, version2.ID)
	_ = nicloudtest.CreateWorkspace(t, user, template2.ID)

	workspaces, err := user.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	require.NoError(t, err, "fetch workspaces should not fail")
...
func TestResolveAutostart(t *testing.T) {
	...
	_, err := ownerClient.UpdateTemplateMeta(ctx, version1.Template.ID, nicloudsdk.UpdateTemplateMeta{
		RequireActiveVersion: ptr.Ref(true),
	})
	require.NoError(t, err)
...
func TestAdminViewAllWorkspaces(t *testing.T) {
	...
	otherOrg, err := client.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
		Name: "default-test",
	})
	...
	otherOwner, _ := nicloudtest.CreateAnotherUser(t, client, otherOrg.ID, rbac.RoleOwner())
	otherWorkspaces, err := otherOwner.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	require.NoError(t, err, "(other) fetch workspaces")

	firstWorkspaces, err := client.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
	...
	memberView, _ := nicloudtest.CreateAnotherUser(t, client, otherOrg.ID)
	memberViewWorkspaces, err := memberView.Workspaces(ctx, nicloudsdk.WorkspaceFilter{})
...
func TestWorkspaceByOwnerAndName(t *testing.T) {
	...
	t.Run("Matching Provisioner", func(t *testing.T) {
		...
		client, db, userResponse := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
			LicenseOptions: &nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureExternalProvisionerDaemons: 1,
				},
		...
		_ = nicloudenttest.NewExternalProvisionerDaemon(t, client, userResponse.OrganizationID, map[string]string{
			provisionersdk.TagScope: provisionersdk.ScopeOrganization,
		})

		version := nicloudtest.CreateTemplateVersion(t, client, userResponse.OrganizationID, nil)
		nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, userResponse.OrganizationID, version.ID)
		workspace := nicloudtest.CreateWorkspace(t, client, template.ID)

		// Pending builds should show matching provisioners
		require.Equal(t, workspace.LatestBuild.Status, nicloudsdk.WorkspaceStatusPending)
		...
		completedBuild := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, workspace.LatestBuild.ID)
		require.Equal(t, completedBuild.Status, nicloudsdk.WorkspaceStatusRunning)
		require.Equal(t, completedBuild.MatchedProvisioners.Count, 0)
		require.Equal(t, completedBuild.MatchedProvisioners.Available, 0)

		ws, err := client.WorkspaceByOwnerAndName(ctx, username, workspace.Name, nicloudsdk.WorkspaceOptions{})
		require.NoError(t, err)
	...
	t.Run("No Matching Provisioner", func(t *testing.T) {
		...
		closer := nicloudenttest.NewExternalProvisionerDaemon(t, client, userResponse.OrganizationID, map[string]string{
			provisionersdk.TagScope: provisionersdk.ScopeOrganization,
		})
		...
		_, err = client.WorkspaceByOwnerAndName(dbauthz.As(ctx, userSubject), username, workspace.Name, nicloudsdk.WorkspaceOptions{})
		require.NoError(t, err)
		require.Equal(t, workspace.LatestBuild.Status, nicloudsdk.WorkspaceStatusPending)
		require.Equal(t, workspace.LatestBuild.MatchedProvisioners.Count, 0)
		require.Equal(t, workspace.LatestBuild.MatchedProvisioners.Available, 0)
...
func TestUpdateWorkspaceACL(t *testing.T) {
	...
	t.Run("OKWithGroup", func(t *testing.T) {
		...
		group := nicloudtest.CreateGroup(t, adminClient, orgID, "bloob")
		...
		err := client.UpdateWorkspaceACL(ctx, ws.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				friend.ID.String(): nicloudsdk.WorkspaceRoleUse,
			},
			GroupRoles: map[string]nicloudsdk.WorkspaceRole{
				group.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)
		...
		require.Equal(t, workspaceACL.Users[0].Role, nicloudsdk.WorkspaceRoleUse)
		require.Len(t, workspaceACL.Groups, 1)
		require.Equal(t, workspaceACL.Groups[0].ID, group.ID)
		require.Equal(t, workspaceACL.Groups[0].Role, nicloudsdk.WorkspaceRoleAdmin)
	})
...
func TestDeleteWorkspaceACL(t *testing.T) {
	...
	t.Run("WorkspaceOwnerCanDelete_Groups", func(t *testing.T) {
		...
		var (
			client, db, admin = nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
				Options: &nicloudtest.Options{
			...
			workspaceOwnerClient, workspaceOwner = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID, rbac.ScopedRoleOrgAuditor(admin.OrganizationID))
			workspace                            = dbfake.WorkspaceBuild(t, db, database.WorkspaceTable{
				OwnerID:        workspaceOwner.ID,
		...
		group, err := client.CreateGroup(ctx, admin.OrganizationID, nicloudsdk.CreateGroupRequest{
			Name: "wibble",
		})
		require.NoError(t, err)
	...
	t.Run("SharedGroupUsersCannotDelete", func(t *testing.T) {
		...
		var (
			...
			sharedClient, toShareWithUser = nicloudtest.CreateAnotherUser(t, client, admin.OrganizationID)
		)
		...
		require.NoError(t, err)
		group, err = client.PatchGroup(ctx, group.ID, nicloudsdk.PatchGroupRequest{
...
func TestWorkspacesSharedWith(t *testing.T) {
	...
	t.Run("ContainsActorsWithFullData", func(t *testing.T) {
		...
		_, workspaceOwner := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		_, sharedWithUser := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)
		...
		sharedWithGroup, err := client.CreateGroup(ctx, user.OrganizationID, nicloudsdk.CreateGroupRequest{
		...
		err = client.UpdateWorkspaceACL(ctx, workspace.ID, nicloudsdk.UpdateWorkspaceACL{
			UserRoles: map[string]nicloudsdk.WorkspaceRole{
				sharedWithUser.ID.String(): nicloudsdk.WorkspaceRoleUse,
			},
			GroupRoles: map[string]nicloudsdk.WorkspaceRole{
				sharedWithGroup.ID.String(): nicloudsdk.WorkspaceRoleAdmin,
			},
		})
		require.NoError(t, err)
		...
		var userActor, groupActor *nicloudsdk.SharedWorkspaceActor
		for i := range sharedWith {
			if sharedWith[i].ActorType == nicloudsdk.SharedWorkspaceActorTypeUser {
				userActor = &sharedWith[i]
			} else if sharedWith[i].ActorType == nicloudsdk.SharedWorkspaceActorTypeGroup {
		...
		assert.Contains(t, userActor.Roles, nicloudsdk.WorkspaceRoleUse)
		...
		assert.Contains(t, groupActor.Roles, nicloudsdk.WorkspaceRoleAdmin)
...
func TestWorkspaceAITask(t *testing.T) {
	...
	usage := nicloudtest.NewUsageInserter()
	owner, _, first := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
		Options: &nicloudtest.Options{
			UsageInserter:            usage,
			IncludeProvisionerDaemon: true,
		},
	...
	client, _ := nicloudtest.CreateAnotherUser(t, owner, first.OrganizationID,
	...
	t.Run("CreateWorkspaceWithTaskNormally", func(t *testing.T) {
		...
		_ = nicloudtest.AwaitTemplateVersionJobCompleted(t, client, version.ID)
		template := nicloudtest.CreateTemplate(t, client, first.OrganizationID, version.ID)
		wrk := nicloudtest.CreateWorkspace(t, client, template.ID)
		build := nicloudtest.AwaitWorkspaceBuildJobCompleted(t, client, wrk.LatestBuild.ID)
		require.Equal(t, nicloudsdk.WorkspaceStatusRunning, build.Status)
	...
	t.Run("CreateTaskWorkspace", func(t *testing.T) {
		...
		build, err = client.CreateWorkspaceBuild(ctx, wrk.ID, nicloudsdk.CreateWorkspaceBuildRequest{
