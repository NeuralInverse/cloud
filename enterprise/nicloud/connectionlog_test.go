...
func TestConnectionLogs(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		logs, err := client.ConnectionLogs(ctx, nicloudsdk.ConnectionLogsRequest{})
		require.NoError(t, err)

		require.Len(t, logs.ConnectionLogs, 1)
		require.EqualValues(t, 1, logs.Count)
		require.Equal(t, nicloudsdk.ConnectionTypeSSH, logs.ConnectionLogs[0].Type)
	})
	...
	t.Run("Empty", func(t *testing.T) {
		...
		ctx := context.Background()
		client, _, _ := nicloudenttest.NewWithDatabase(t, &nicloudenttest.Options{
			ConnectionLogging: true,
			LicenseOptions: &nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureAuditLog:      1,
