...
import (
	...
	agplproxyhealth "github.com/NeuralInverse/cloud/v2/nicloud/proxyhealth"
...
// workspace proxies. This information is stored in memory, so each nicloud
	...
	for _, proxy := range proxies {
		...
		grp.Go(func() error {
			...
			case err == nil && resp.StatusCode == http.StatusOK:
				...
				if err != nil {
					isCoderErr := xerrors.Errorf("proxy url %q is not a neuralinverse proxy instance, verify the url is correct", reqURL)
					if resp.Header.Get(nicloudsdk.BuildVersionHeader) != "" {
						isCoderErr = xerrors.Errorf("proxy url %q is a neuralinverse instance, but unable to decode the response payload. Could this be a primary nicloud and not a proxy?", reqURL)
					}
					...
					if notJSONErr := nicloudsdk.ExpectJSONMime(resp); notJSONErr != nil {
						err = errors.Join(
							isCoderErr,
