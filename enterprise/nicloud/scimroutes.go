...
import (
	...
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/scim"
)
...
func (api *API) mountScimRoute(opt *Options, r chi.Router) error {
	if len(opt.SCIMAPIKey) == 0 {
		...
		r.Mount("/", http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			httpapi.Write(r.Context(), w, http.StatusNotFound, nicloudsdk.Response{
				...
				Detail:  "SCIM endpoints are disabled if no SCIM is configured. Configure 'NEURALINVERSE_SCIM_AUTH_HEADER' to enable.",
			})
		})))
		return nil
	}
	...
	if opt.UseLegacySCIM {
		...
		r.Mount("/v2", chi.Chain(
			api.RequireFeatureMW(nicloudsdk.FeatureSCIM),
