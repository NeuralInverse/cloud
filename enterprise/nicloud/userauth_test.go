...
import (
	...
	niclouden "github.com/NeuralInverse/cloud/v2/enterprise/nicloud"
...
func TestUserOIDC(t *testing.T) {
	...
	t.Run("OrganizationSync", func(t *testing.T) {
		...
		t.Run("SingleOrgDeployment", func(t *testing.T) {
			...
			runner := setupOIDCTest(t, oidcTestConfig{
				Config: func(cfg *nicloud.OIDCConfig) {
					cfg.AllowSignups = true
				},
				DeploymentValues: func(dv *nicloudsdk.DeploymentValues) {
		...
		t.Run("MultiOrgNoSync", func(t *testing.T) {
			...
			second, err := runner.AdminClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
				Name:        "second",
				DisplayName: "",
				Description: "",
				Icon:        "",
		...
		t.Run("MultiOrgWithDefault", func(t *testing.T) {
			...
			ctx := testutil.Context(t, testutil.WaitMedium)
			orgOne, err := runner.AdminClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
				Name:        "one",
				DisplayName: "One",
				Description: "",
				Icon:        "",
			...
			orgTwo, err := runner.AdminClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
				Name:        "two",
				DisplayName: "two",
				Description: "",
				Icon:        "",
			...
			orgThree, err := runner.AdminClient.CreateOrganization(ctx, nicloudsdk.CreateOrganizationRequest{
				Name:        "three",
				DisplayName: "three",
			})
			require.NoError(t, err)
			...
			user, err := userClient.User(ctx, nicloudsdk.Me)
			require.NoError(t, err)
	...
	t.Run("RoleSync", func(t *testing.T) {
		...
		t.Run("BlockAssignRoles", func(t *testing.T) {
			...
			_, err := runner.AdminClient.UpdateUserRoles(ctx, "alice", nicloudsdk.UpdateRoles{
				Roles: []string{
	...
	t.Run("Groups", func(t *testing.T) {
		...
		t.Run("Assigns", func(t *testing.T) {
			...
			ctx := testutil.Context(t, testutil.WaitShort)
			group, err := runner.AdminClient.CreateGroup(ctx, runner.AdminUser.OrganizationIDs[0], nicloudsdk.CreateGroupRequest{
				Name: groupName,
			})
			require.NoError(t, err)
	...
	t.Run("Refresh", func(t *testing.T) {
		...
		t.Run("FailedRefresh", func(t *testing.T) {
			...
			_, err := client.User(context.Background(), nicloudsdk.Me)
			require.Error(t, err)
...
func TestGroupSync(t *testing.T) {
	...
	testCases := []struct {
		name   string
		modCfg func(cfg *nicloud.OIDCConfig)
		modDV  func(dv *nicloudsdk.DeploymentValues)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			initialGroups := make(map[string]nicloudsdk.Group)
			for _, group := range tc.initialOrgGroups {
				newGroup, err := runner.AdminClient.CreateGroup(ctx, org, nicloudsdk.CreateGroupRequest{
					Name: group,
				})
				require.NoError(t, err)
			...
			_, user := nicloudtest.CreateAnotherUser(t, runner.AdminClient, org)
			for _, group := range tc.initialUserGroups {
				_, err := runner.AdminClient.PatchGroup(ctx, initialGroups[group].ID, nicloudsdk.PatchGroupRequest{
			...
			for _, group := range orgGroups {
				if slice.Contains(tc.initialOrgGroups, group.Name) || group.IsEveryone() {
					require.Equal(t, group.Source, nicloudsdk.GroupSourceUser)
				} else {
					require.Equal(t, group.Source, nicloudsdk.GroupSourceOIDC)
				}
			}
			...
			for _, group := range orgGroups {
				userInGroup := slice.ContainsCompare(group.Members, nicloudsdk.ReducedUser{Email: user.Email}, func(a, b nicloudsdk.ReducedUser) bool {
...
func TestEnterpriseUserLogin(t *testing.T) {
	...
	t.Run("CustomRole", func(t *testing.T) {
		...
		customRole, err := ownerClient.CreateOrganizationRole(ctx, nicloudsdk.Role{
			...
			OrganizationPermissions: []nicloudsdk.Permission{},
		})
		require.NoError(t, err, "create custom role")

		anotherClient, anotherUser := nicloudtest.CreateAnotherUserMutators(t, ownerClient, owner.OrganizationID, []rbac.RoleIdentifier{
			{
				Name:           customRole.Name,
				OrganizationID: owner.OrganizationID,
			},
		}, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
			r.Password = "SomeSecurePassword!"
			r.UserLoginType = nicloudsdk.LoginTypePassword
		})

		_, err = anotherClient.LoginWithPassword(ctx, nicloudsdk.LoginWithPasswordRequest{
	...
	t.Run("DeletedRole", func(t *testing.T) {
		...
		anotherClient, anotherUser := nicloudtest.CreateAnotherUserMutators(t, ownerClient, owner.OrganizationID, nil, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
			r.Password = "SomeSecurePassword!"
			r.UserLoginType = nicloudsdk.LoginTypePassword
		})
...
type oidcTestRunner struct {
	...
	AdminUser   nicloudsdk.User
	API         *niclouden.API

	// Login will call the OIDC flow with an unauthenticated client.
	// The IDP will return the idToken claims.
	Login        func(t *testing.T, idToken jwt.MapClaims) (*nicloudsdk.Client, *http.Response)
	AttemptLogin func(t *testing.T, idToken jwt.MapClaims) (*nicloudsdk.Client, *http.Response)
	// ForceRefresh will use an authenticated nicloudsdk.Client, and force their
	...
	ForceRefresh     func(t *testing.T, client *nicloudsdk.Client, idToken jwt.MapClaims)
	ExpireOauthToken func(t *testing.T, client *nicloudsdk.Client)
}
...
func (r *oidcTestRunner) AssertOrganizations(t *testing.T, userIdent string, includeDefault bool, expected []uuid.UUID) {
	...
	hasDefault := false
	userOrgIDs := slice.List(userOrgs, func(o nicloudsdk.Organization) uuid.UUID {
		if o.IsDefault {
			hasDefault = true
			cpy = append(cpy, o.ID)
		}
...
func setupOIDCTest(t *testing.T, settings oidcTestConfig) *oidcTestRunner {
	...
	owner, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
		Options: &nicloudtest.Options{
			OIDCConfig:       cfg,
			DeploymentValues: dv,
		},
		LicenseOptions: &nicloudenttest.LicenseOptions{
			Features: license.Features{
				nicloudsdk.FeatureUserRoleManagement:    1,
