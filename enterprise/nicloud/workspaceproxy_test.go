...
func TestRegions(t *testing.T) {
	...
	t.Run("RequireAuth", func(t *testing.T) {
		...
		unauthedClient := nicloudsdk.New(client.URL)
...
func TestWorkspaceProxyCRUD(t *testing.T) {
	...
	t.Run("CreateAndUpdate", func(t *testing.T) {
		...
		_, err = client.PatchWorkspaceProxy(ctx, nicloudsdk.PatchWorkspaceProxy{
...
func TestProxyRegisterDeregister(t *testing.T) {
	...
	setupWithDeploymentValues := func(t *testing.T, dv *nicloudsdk.DeploymentValues) (*nicloudsdk.Client, database.Store) {
		db, pubsub := dbtestutil.NewDB(t)
		client, _ := nicloudenttest.New(t, &nicloudenttest.Options{
			Options: &nicloudtest.Options{
				DeploymentValues:         dv,
	...
	setup := func(t *testing.T) (*nicloudsdk.Client, database.Store) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		createRes, err := client.CreateWorkspaceProxy(ctx, nicloudsdk.CreateWorkspaceProxyRequest{
			Name:        proxyName,
	...
	t.Run("ReturnSiblings", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		createRes1, err := client.CreateWorkspaceProxy(ctx, nicloudsdk.CreateWorkspaceProxyRequest{
			Name: "one",
		})
		require.NoError(t, err)
		createRes2, err := client.CreateWorkspaceProxy(ctx, nicloudsdk.CreateWorkspaceProxyRequest{
			Name: "two",
		})
		require.NoError(t, err)
	...
	t.Run("RegisterWithDisabledBuiltInDERP/DerpEnabled", func(t *testing.T) {
		...
		require.Equal(t, registerRes.DERPMeshKey, nicloudtest.DefaultDERPMeshKey)
	})
...
func TestIssueSignedAppToken(t *testing.T) {
	...
	client, user := nicloudenttest.New(t, &nicloudenttest.Options{
		...
		LicenseOptions: &nicloudenttest.LicenseOptions{
			Features: license.Features{
				nicloudsdk.FeatureWorkspaceProxy: 1,
				nicloudsdk.FeatureConnectionLog:  1,
			},
		},
	})
	...
	proxyRes, err := client.CreateWorkspaceProxy(createProxyCtx, nicloudsdk.CreateWorkspaceProxyRequest{
		Name: testutil.GetRandomName(t),
		Icon: "/emojis/flag.png",
	})
	require.NoError(t, err)
...
func TestReconnectingPTYSignedToken(t *testing.T) {
	...
	t.Run("Validate", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		res, err := client.IssueReconnectingPTYSignedToken(ctx, nicloudsdk.IssueReconnectingPTYSignedTokenRequest{
			URL:     "",
			AgentID: uuid.Nil,
		})
		require.Error(t, err)
	...
	t.Run("NoToken", func(t *testing.T) {
		...
		ctx := testutil.Context(t, testutil.WaitLong)
		res, err := unauthedClient.IssueReconnectingPTYSignedToken(ctx, nicloudsdk.IssueReconnectingPTYSignedTokenRequest{
			URL:     u.String(),
			AgentID: agentID,
		})
		require.Error(t, err)
	...
	t.Run("NoPermissions", func(t *testing.T) {
		...
		userClient, _ := nicloudtest.CreateAnotherUser(t, client, user.OrganizationID)

		ctx := testutil.Context(t, testutil.WaitLong)
		res, err := userClient.IssueReconnectingPTYSignedToken(ctx, nicloudsdk.IssueReconnectingPTYSignedTokenRequest{
			URL:     u.String(),
			AgentID: agentID,
		})
		require.Error(t, err)
...
func TestGetCryptoKeys(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		db, pubsub := dbtestutil.NewDB(t)
		cclient, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
			Options: &nicloudtest.Options{
				Database:                 db,
				Pubsub:                   pubsub,
				IncludeProvisionerDaemon: true,
		...
		proxy := nicloudenttest.NewWorkspaceProxyReplica(t, api, cclient, &nicloudenttest.ProxyOptions{
			Name: testutil.GetRandomName(t),
		})

		keys, err := proxy.SDKClient.CryptoKeys(ctx, nicloudsdk.CryptoKeyFeatureWorkspaceAppsAPIKey)
		require.NoError(t, err)
		...
		keys, err = proxy.SDKClient.CryptoKeys(ctx, nicloudsdk.CryptoKeyFeatureWorkspaceAppsToken)
		require.NoError(t, err)
		require.NotEmpty(t, keys)
		// 1 key is generated on startup, the other we manually generated.
		require.Equal(t, 2, len(keys.CryptoKeys))
	...
	t.Run("InvalidFeature", func(t *testing.T) {
		...
		_, err := proxy.SDKClient.CryptoKeys(ctx, nicloudsdk.CryptoKeyFeatureOIDCConvert)
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusBadRequest, sdkErr.StatusCode())
		_, err = proxy.SDKClient.CryptoKeys(ctx, nicloudsdk.CryptoKeyFeatureTailnetResume)
		require.Error(t, err)
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusBadRequest, sdkErr.StatusCode())
	...
	t.Run("Unauthorized", func(t *testing.T) {
		...
		_ = nicloudenttest.NewWorkspaceProxyReplica(t, api, cclient, &nicloudenttest.ProxyOptions{
			Name: testutil.GetRandomName(t),
		})

		client := wsproxysdk.New(cclient.URL, cclient.SessionToken())

		_, err := client.CryptoKeys(ctx, nicloudsdk.CryptoKeyFeatureWorkspaceAppsAPIKey)
		require.Error(t, err)
		var sdkErr *nicloudsdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusUnauthorized, sdkErr.StatusCode())
...
func requireNotContainsKeys(t *testing.T, keys []nicloudsdk.CryptoKey, unexpected ...nicloudsdk.CryptoKey) {
	t.Helper()
...
func requireContainsKeys(t *testing.T, keys []nicloudsdk.CryptoKey, expected ...nicloudsdk.CryptoKey) {
	t.Helper()
