...
func TestCollectLicenseMetrics(t *testing.T) {
	...
	sut.Entitlements.Modify(func(entitlements *nicloudsdk.Entitlements) {
		entitlements.Features[nicloudsdk.FeatureUserLimit] = nicloudsdk.Feature{
			Enabled: true,
...
func TestCollectLicenseMetrics_WarningsAndErrors(t *testing.T) {
	...
	t.Run("NoWarningsOrErrors", func(t *testing.T) {
		...
		warnings, ok := findMetric(metrics, "nicloud_license_warnings")
		require.True(t, ok)
		require.Zero(t, warnings)

		errors, ok := findMetric(metrics, "nicloud_license_errors")
		require.True(t, ok)
