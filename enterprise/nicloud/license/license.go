...
func Entitlements(
	...
	if err != nil {
		return nicloudsdk.Entitlements{}, xerrors.Errorf("query active user count: %w", err)
	}
	...
	if err != nil {
		return nicloudsdk.Entitlements{}, xerrors.Errorf("query active AI seat count: %w", err)
	}
	...
	if err != nil {
		return nicloudsdk.Entitlements{}, xerrors.Errorf("query external templates: %w", err)
	}
...
func LicensesEntitlements(
	...
	for _, featureName := range nicloudsdk.FeatureNames {
		entitlements.AddFeature(featureName, nicloudsdk.Feature{
			Entitlement: nicloudsdk.EntitlementNotEntitled,
	...
	for _, license := range licenses {
		...
		if now.After(claims.LicenseExpires.Time) {
			...
			entitlement = nicloudsdk.EntitlementGracePeriod
		}

		// 'claims.AllFeature' is the legacy way to set 'claims.FeatureSet = nicloudsdk.FeatureSetEnterprise'
		// If both are set, ignore the legacy 'claims.AllFeature'
		if claims.AllFeatures && claims.FeatureSet == "" {
			claims.FeatureSet = nicloudsdk.FeatureSetEnterprise
		}
		...
		if claims.FeatureSet == nicloudsdk.FeatureSetPremium {
			var (
			...
			entitlements.AddFeature(nicloudsdk.FeatureManagedAgentLimit, nicloudsdk.Feature{
				Enabled:     true,
				Entitlement: entitlement,
				Limit:       &defaultManagedAgentsLimit,
				UsagePeriod: &nicloudsdk.UsagePeriod{
		...
		if slices.Contains(claims.Addons, nicloudsdk.AddonAIGovernance) {
		...
		for featureName, featureValue := range claims.Features {
			...
			if _, ok := nicloudsdk.FeatureNamesMap[featureName]; !ok {
			...
			case featureName.UsesLimit():
				...
				if featureName == nicloudsdk.FeatureAIGovernanceUserLimit {
			...
			default:
				...
				entitlements.Features[featureName] = nicloudsdk.Feature{
					Entitlement: entitlement,
					Enabled:     enablements[featureName] || featureName.AlwaysEnable(),
				}
			}
		...
		addonFeatures := make(map[nicloudsdk.FeatureName]nicloudsdk.Feature)
		...
		for _, addon := range claims.Addons {
			...
			for _, featureName := range addon.Features() {
				if _, exists := addonFeatures[featureName]; !exists {
					addonFeatures[featureName] = nicloudsdk.Feature{
						Entitlement: entitlement,
						Enabled:     enablements[featureName] || featureName.AlwaysEnable(),
					}
				}
	...
	if featureArguments.ReplicaCount > 1 {
		feature := entitlements.Features[nicloudsdk.FeatureHighAvailability]

		switch feature.Entitlement {
		case nicloudsdk.EntitlementNotEntitled:
		...
		case nicloudsdk.EntitlementGracePeriod:
	...
	if featureArguments.ExternalAuthCount > 1 {
		feature := entitlements.Features[nicloudsdk.FeatureMultipleExternalAuth]
	...
	if featureArguments.ExternalTemplateCount > 0 {
		feature := entitlements.Features[nicloudsdk.FeatureWorkspaceExternalAgent]
		switch feature.Entitlement {
		case nicloudsdk.EntitlementNotEntitled:
			entitlements.Errors = append(entitlements.Errors,
	...
	agentLimit := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
	if entitlements.HasLicense && agentLimit.UsagePeriod != nil {
		...
		if err != nil {
			...
			entitlements.AddFeature(nicloudsdk.FeatureManagedAgentLimit, agentLimit)
			...
			if agentLimit.Enabled && agentLimit.Limit != nil && managedAgentCount >= *agentLimit.Limit {
				entitlements.Warnings = append(entitlements.Warnings,
					nicloudsdk.LicenseManagedAgentLimitExceededWarningText)
			}
		}
	}

	if entitlements.HasLicense {
		userLimit := entitlements.Features[nicloudsdk.FeatureUserLimit]
		...
		} else if userLimit.Limit != nil && userLimit.Entitlement == nicloudsdk.EntitlementGracePeriod {
			entitlements.Warnings = append(entitlements.Warnings, fmt.Sprintf(
		...
		if featureArguments.ActiveAISeatCount > 0 {
			actual := featureArguments.ActiveAISeatCount
			feature := entitlements.Features[nicloudsdk.FeatureAIGovernanceUserLimit]
			switch {
			case feature.Entitlement == nicloudsdk.EntitlementNotEntitled:
			...
			case feature.Entitlement == nicloudsdk.EntitlementGracePeriod && feature.Limit != nil:
				entitlements.Warnings = append(entitlements.Warnings,
					fmt.Sprintf(
		...
		for _, featureName := range nicloudsdk.FeatureNames {
			...
			if featureName == nicloudsdk.FeatureHighAvailability {
				continue
			}
			// External Auth Providers auth has it's own warnings based on the number configured!
			if featureName == nicloudsdk.FeatureMultipleExternalAuth {
				continue
			}
		...
		if aiBridgeFeature.Enabled && aiBridgeFeature.Entitlement.Entitled() && !hasExplicitAIBridgeEntitlement {
			entitlements.Warnings = append(entitlements.Warnings,
				"The AI Governance add-on is required to use AI Bridge. Please reach out to your account team or sales@cloud.neuralinverse.com to learn more.")
		}
	}
...
func appendAIGovernanceSeatLimitWarning(warnings []string, actual int64, limit int64) []string {
	...
	if actual > limit {
		...
		return append(warnings, fmt.Sprintf(
			nicloudsdk.LicenseAIGovernanceOverLimitWarningText,
			actual,
			limit,
			overLimitSeats,
		))
		...
		return append(warnings, fmt.Sprintf(nicloudsdk.LicenseAIGovernance90PercentWarningText, usedPercent))
	}
...
type Features map[nicloudsdk.FeatureName]int64
...
type Claims struct {
	...
	FeatureSet    nicloudsdk.FeatureSet `json:"feature_set"`
	...
	Addons           []nicloudsdk.Addon `json:"addons,omitempty"`
...
func (p *licenseValidityPeriod) LicenseExpirationWarning(entitlements *nicloudsdk.Entitlements, now time.Time) {
	merged := p.merged()
	if len(merged) == 0 {
		// No licenses
		return
