...
func TestEntitlements(t *testing.T) {
	t.Parallel()
	all := make(map[nicloudsdk.FeatureName]bool)
	for _, n := range nicloudsdk.FeatureNames {
		all[n] = true
	}

	empty := map[nicloudsdk.FeatureName]bool{}

	t.Run("Defaults", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), db, 1, 1, nicloudenttest.Keys, all)
		...
		for _, featureName := range nicloudsdk.FeatureNames {
			...
			require.Equal(t, nicloudsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
		}
	})
	t.Run("Always return the current user count", func(t *testing.T) {
		...
		require.False(t, entitlements.HasLicense)
		require.False(t, entitlements.Trial)
		require.Equal(t, *entitlements.Features[nicloudsdk.FeatureUserLimit].Actual, int64(0))
	})
	t.Run("SingleLicenseNothing", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{}),
			Exp: dbtime.Now().Add(time.Hour),
		})
		entitlements, err := license.Entitlements(context.Background(), db, 1, 1, nicloudenttest.Keys, empty)
		require.NoError(t, err)
	...
	t.Run("SingleLicenseAll", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: func() license.Features {
					...
					for _, name := range nicloudsdk.FeatureNames {
						if name == nicloudsdk.FeatureManagedAgentLimit {
							f[nicloudsdk.FeatureManagedAgentLimit] = 100
							continue
						}
						f[name] = 1
					}
		...
		require.False(t, entitlements.Trial)
		for _, featureName := range nicloudsdk.FeatureNames {
			require.Equal(t, nicloudsdk.EntitlementEntitled, entitlements.Features[featureName].Entitlement, featureName)
		}
	})
	t.Run("SingleLicenseGrace", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureUserLimit: 100,
		...
		require.Equal(t, nicloudsdk.EntitlementGracePeriod, entitlements.Features[nicloudsdk.FeatureAuditLog].Entitlement)
		require.Contains(
			t, entitlements.Warnings,
			fmt.Sprintf("%s is enabled but your license for this feature is expired.", nicloudsdk.FeatureAuditLog.Humanize()),
		)
	})
	t.Run("Expiration warning", func(t *testing.T) {
		...
		require.Equal(t, nicloudsdk.EntitlementEntitled, entitlements.Features[nicloudsdk.FeatureAuditLog].Entitlement)
		require.Contains(
			t, entitlements.Warnings,
		...
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureUserLimit:             100,
					nicloudsdk.FeatureAuditLog:              1,
					nicloudsdk.FeatureAIGovernanceUserLimit: 100,
				},
				FeatureSet: nicloudsdk.FeatureSetPremium,
				GraceAt:    graceDate,
				ExpiresAt:  dbtime.Now().AddDate(0, 0, 5),
				Addons:     []nicloudsdk.Addon{nicloudsdk.AddonAIGovernance},
			}),
			Exp: time.Now().AddDate(0, 0, 5),
		})
		require.NoError(t, err)
	...
	t.Run("SingleLicenseNotEntitled", func(t *testing.T) {
		...
		require.False(t, entitlements.Trial)
		for _, featureName := range nicloudsdk.FeatureNames {
			if featureName == nicloudsdk.FeatureUserLimit ||
				featureName == nicloudsdk.FeatureHighAvailability ||
				featureName == nicloudsdk.FeatureMultipleExternalAuth ||
				featureName == nicloudsdk.FeatureManagedAgentLimit ||
				featureName == nicloudsdk.FeatureAIGovernanceUserLimit ||
				featureName == nicloudsdk.FeatureBoundary {
	...
	t.Run("TooManyUsers", func(t *testing.T) {
		...
		activeUser1, err := db.InsertUser(context.Background(), database.InsertUserParams{
			ID:        uuid.New(),
			Username:  "test1",
			Email:     "test1@cloud.neuralinverse.com",
		...
		_, err = db.InsertUser(context.Background(), database.InsertUserParams{
			...
			Email:     "dormant-user@cloud.neuralinverse.com",
			LoginType: database.LoginTypePassword,
			RBACRoles: []string{},
		})
		require.NoError(t, err)
	...
	t.Run("MaximizeUserLimit", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureUserLimit: 10,
				},
	...
	t.Run("Enterprise", func(t *testing.T) {
		...
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			Exp: time.Now().Add(time.Hour),
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				FeatureSet: nicloudsdk.FeatureSetEnterprise,
			}),
		})
		require.NoError(t, err)
		entitlements, err := license.Entitlements(context.Background(), db, 1, 1, nicloudenttest.Keys, all)
		...
		enterpriseFeatures := nicloudsdk.FeatureSetEnterprise.Features()
		for _, featureName := range nicloudsdk.FeatureNames {
			if featureName == nicloudsdk.FeatureUserLimit {
				continue
			}
			if featureName == nicloudsdk.FeatureManagedAgentLimit {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				...
				require.Equal(t, nicloudsdk.EntitlementEntitled, entitlements.Features[featureName].Entitlement)
			} else {
	...
	t.Run("Premium", func(t *testing.T) {
		...
		licenseOptions := nicloudenttest.LicenseOptions{
			...
			FeatureSet: nicloudsdk.FeatureSetPremium,
			Features: license.Features{
				nicloudsdk.FeatureUserLimit: userLimit,
			},
		}
		_, err := db.InsertLicense(context.Background(), database.InsertLicenseParams{
			Exp: time.Now().Add(time.Hour),
			JWT: nicloudenttest.GenerateLicense(t, licenseOptions),
		})
		require.NoError(t, err)
		entitlements, err := license.Entitlements(context.Background(), db, 1, 1, nicloudenttest.Keys, all)
		require.NoError(t, err)
		...
		enterpriseFeatures := nicloudsdk.FeatureSetPremium.Features()
		for _, featureName := range nicloudsdk.FeatureNames {
			if featureName == nicloudsdk.FeatureUserLimit {
				continue
			}
			if featureName == nicloudsdk.FeatureManagedAgentLimit {
				...
				require.Equal(t, nicloudsdk.EntitlementEntitled, agentEntitlement.Entitlement)
	...
	t.Run("AllFeaturesAlwaysEnable", func(t *testing.T) {
		...
		for _, featureName := range nicloudsdk.FeatureNames {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				...
				require.Equal(t, nicloudsdk.EntitlementEntitled, feature.Entitlement)
			} else {
				require.False(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, nicloudsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
			}
	...
	t.Run("AllFeaturesGrace", func(t *testing.T) {
		...
		for _, featureName := range nicloudsdk.FeatureNames {
			...
			if slices.Contains(enterpriseFeatures, featureName) {
				require.True(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, nicloudsdk.EntitlementGracePeriod, entitlements.Features[featureName].Entitlement)
			} else {
				require.False(t, entitlements.Features[featureName].Enabled, featureName)
				require.Equal(t, nicloudsdk.EntitlementNotEntitled, entitlements.Features[featureName].Entitlement)
			}
	...
	t.Run("MultipleReplicasNoLicense", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), db, 2, 1, nicloudenttest.Keys, all)
		require.NoError(t, err)
		require.False(t, entitlements.HasLicense)
	...
	t.Run("MultipleReplicasNotEntitled", func(t *testing.T) {
		...
		entitlements, err := license.Entitlements(context.Background(), db, 2, 1, nicloudenttest.Keys, map[nicloudsdk.FeatureName]bool{
			nicloudsdk.FeatureHighAvailability: true,
		})
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthNoLicense", func(t *testing.T) {
		t.Parallel()
		db, _ := dbtestutil.NewDB(t)
		entitlements, err := license.Entitlements(context.Background(), db, 1, 2, nicloudenttest.Keys, all)
		require.NoError(t, err)
		require.False(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthNotEntitled", func(t *testing.T) {
		...
		entitlements, err := license.Entitlements(context.Background(), db, 1, 2, nicloudenttest.Keys, map[nicloudsdk.FeatureName]bool{
			nicloudsdk.FeatureMultipleExternalAuth: true,
		})
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		require.Len(t, entitlements.Errors, 1)
	...
	t.Run("MultipleGitAuthGrace", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				...
				ExpiresAt: time.Now().Add(time.Hour),
				Features: license.Features{
					nicloudsdk.FeatureMultipleExternalAuth: 1,
				},
			}),
			Exp: time.Now().Add(time.Hour),
		})
	...
	t.Run("ManagedAgentLimitHasValue", func(t *testing.T) {
		...
		licenseOpts := (&nicloudenttest.LicenseOptions{
			FeatureSet: nicloudsdk.FeatureSetPremium,
		...
		lic := database.License{
			ID:  1,
			JWT: nicloudenttest.GenerateLicense(t, *licenseOpts),
		...
		entitlements, err := license.Entitlements(context.Background(), mDB, 1, 0, nicloudenttest.Keys, all)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)

		managedAgentLimit, ok := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
		require.True(t, ok)
		...
		aiGovernanceSeatLimit, ok := entitlements.Features[nicloudsdk.FeatureAIGovernanceUserLimit]
		require.True(t, ok)
		...
		require.Len(t, entitlements.Warnings, 1)
		require.Equal(t, nicloudsdk.LicenseManagedAgentLimitExceededWarningText, entitlements.Warnings[0])
	})
	...
	t.Run("AIGovernanceSeatWarnings", func(t *testing.T) {
		...
		testCases := []struct {
		...
		}{
			{
				...
				limit:           100,
				activeSeatCount: 90,
				expectedWarning: fmt.Sprintf(nicloudsdk.LicenseAIGovernance90PercentWarningText, 90),
			},
			{
				name:            "Below90Percent",
				limit:           100,
			...
			},
			{
				...
				limit:           100,
				activeSeatCount: 110,
				expectedWarning: fmt.Sprintf(nicloudsdk.LicenseAIGovernanceOverLimitWarningText, 110, 100, 10),
			},
			{
				name:            "AtLimit",
				limit:           100,
				activeSeatCount: 100,
				expectedWarning: fmt.Sprintf(nicloudsdk.LicenseAIGovernance90PercentWarningText, 100),
			},
			{
				name:            "OverLimitRoundingDown",
				limit:           101,
				activeSeatCount: 106,
				expectedWarning: fmt.Sprintf(nicloudsdk.LicenseAIGovernanceOverLimitWarningText, 106, 101, 5),
			},
			{
				name:            "TinyOverage",
				limit:           1000,
				activeSeatCount: 1001,
				expectedWarning: fmt.Sprintf(nicloudsdk.LicenseAIGovernanceOverLimitWarningText, 1001, 1000, 1),
			},
			{
				name:            "ZeroLimitGuard",
				limit:           0,
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				licenseOpts := (&nicloudenttest.LicenseOptions{
					...
					Addons:     []nicloudsdk.Addon{nicloudsdk.AddonAIGovernance},
					Features: license.Features{
						nicloudsdk.FeatureAIGovernanceUserLimit: tc.limit,
					},
				}).
					UserLimit(100)
		...
		t.Run("GracePeriodOverLimit", func(t *testing.T) {
			...
			licenseOpts := &nicloudenttest.LicenseOptions{
				...
				Addons:    []nicloudsdk.Addon{nicloudsdk.AddonAIGovernance},
				Features: license.Features{
					nicloudsdk.FeatureAIGovernanceUserLimit: limit,
				},
			}
			...
			enablements := map[nicloudsdk.FeatureName]bool{
				nicloudsdk.FeatureAIGovernanceUserLimit: true,
			}

			entitlements, err := license.Entitlements(context.Background(), mDB, 1, 0, nicloudenttest.Keys, enablements)
			require.NoError(t, err)
			require.True(t, entitlements.HasLicense)

			feature, ok := entitlements.Features[nicloudsdk.FeatureAIGovernanceUserLimit]
			require.True(t, ok)
			require.Equal(t, nicloudsdk.EntitlementGracePeriod, feature.Entitlement)
			...
			require.Contains(t, entitlements.Warnings,
				fmt.Sprintf(nicloudsdk.LicenseAIGovernanceOverLimitWarningText, activeSeatCount, limit, 27),
			)
		})
		...
		t.Run("GracePeriod90Percent", func(t *testing.T) {
			...
			require.Contains(t, entitlements.Warnings,
				fmt.Sprintf(nicloudsdk.LicenseAIGovernance90PercentWarningText, 95))
...
func TestLicenseEntitlements(t *testing.T) {
	...
	// This list comes from nicloud.go on launch. This list is a bit arbitrary,
	// maybe some should be moved to "AlwaysEnabled" instead.
	defaultEnablements := map[nicloudsdk.FeatureName]bool{
		nicloudsdk.FeatureAuditLog:                   true,
		nicloudsdk.FeatureConnectionLog:              true,
		nicloudsdk.FeatureBrowserOnly:                true,
		nicloudsdk.FeatureSCIM:                       true,
		nicloudsdk.FeatureMultipleExternalAuth:       true,
		nicloudsdk.FeatureTemplateRBAC:               true,
		nicloudsdk.FeatureExternalTokenEncryption:    true,
		nicloudsdk.FeatureExternalProvisionerDaemons: true,
		nicloudsdk.FeatureAdvancedTemplateScheduling: true,
		nicloudsdk.FeatureWorkspaceProxy:             true,
		nicloudsdk.FeatureUserRoleManagement:         true,
		nicloudsdk.FeatureAccessControl:              true,
		nicloudsdk.FeatureControlSharedPorts:         true,
		nicloudsdk.FeatureWorkspaceExternalAgent:     true,
		nicloudsdk.FeatureAIBridge:                   true,
		nicloudsdk.FeatureBoundary:                   true,
	}

	legacyLicense := func() *nicloudenttest.LicenseOptions {
		return (&nicloudenttest.LicenseOptions{
	...
	enterpriseLicense := func() *nicloudenttest.LicenseOptions {
		return (&nicloudenttest.LicenseOptions{
			AccountType:   "salesforce",
			AccountID:     "Bob",
			DeploymentIDs: nil,
	...
	premiumLicense := func() *nicloudenttest.LicenseOptions {
		return (&nicloudenttest.LicenseOptions{
			AccountType:   "salesforce",
			AccountID:     "Charlie",
			DeploymentIDs: nil,
	...
	testCases := []struct {
		Name        string
		Licenses    []*nicloudenttest.LicenseOptions
		Enablements map[nicloudsdk.FeatureName]bool
		Arguments   license.FeatureArguments

		ExpectedErrorContains string
		AssertEntitlements    func(t *testing.T, entitlements nicloudsdk.Entitlements)
	}{
		{
			Name: "NoLicenses",
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				...
				assertNoErrors(t, entitlements)
				assertNoWarnings(t, entitlements)
				userFeature := entitlements.Features[nicloudsdk.FeatureUserLimit]
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				...
				assert.Equalf(t, int64(500), *userFeature.Limit, "user limit")
				assert.Equalf(t, int64(200), *userFeature.Actual, "user count")
				assert.Equal(t, userFeature.Entitlement, nicloudsdk.EntitlementGracePeriod)
			},
		},
		{
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				assert.False(t, entitlements.Features[nicloudsdk.FeatureMultipleOrganizations].Enabled, "multi-org only enabled for premium")
				assert.False(t, entitlements.Features[nicloudsdk.FeatureCustomRoles].Enabled, "custom-roles only enabled for premium")
			},
		},
		{
			...
			ExpectedErrorContains: "",
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				assert.True(t, entitlements.Features[nicloudsdk.FeatureMultipleOrganizations].Enabled, "multi-org enabled for premium")
				assert.True(t, entitlements.Features[nicloudsdk.FeatureCustomRoles].Enabled, "custom-roles enabled for premium")
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				...
				userFeature := entitlements.Features[nicloudsdk.FeatureUserLimit]
				assert.Equalf(t, int64(100), *userFeature.Limit, "user limit")
				assert.Equal(t, nicloudsdk.EntitlementNotEntitled,
					entitlements.Features[nicloudsdk.FeatureMultipleOrganizations].Entitlement)
				assert.Equal(t, nicloudsdk.EntitlementNotEntitled,
					entitlements.Features[nicloudsdk.FeatureCustomRoles].Entitlement)
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				assertNoErrors(t, entitlements)
				assertNoWarnings(t, entitlements)
				feature := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
				assert.Equal(t, nicloudsdk.EntitlementEntitled, feature.Entitlement)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				...
				assertNoWarnings(t, entitlements)
				feature := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
				assert.Equal(t, nicloudsdk.EntitlementGracePeriod, feature.Entitlement)
				assert.True(t, feature.Enabled)
				assert.Equal(t, int64(100), *feature.Limit)
				assert.Equal(t, int64(74), *feature.Actual)
			},
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				feature := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
				assert.Equal(t, nicloudsdk.EntitlementNotEntitled, feature.Entitlement)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				...
				assert.Equal(t, nicloudsdk.LicenseManagedAgentLimitExceededWarningText, entitlements.Warnings[0])
				assertNoErrors(t, entitlements)
		...
		},
		{
			...
			AssertEntitlements: func(t *testing.T, entitlements nicloudsdk.Entitlements) {
				assert.Equal(t, nicloudsdk.EntitlementEntitled, entitlements.Features[nicloudsdk.FeatureWorkspaceExternalAgent].Entitlement)
				assert.True(t, entitlements.Features[nicloudsdk.FeatureWorkspaceExternalAgent].Enabled)
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, tc.Enablements, nicloudenttest.Keys, tc.Arguments)
...
func TestAIBridgeSoftWarning(t *testing.T) {
	...
	aiBridgeEnabledEnablements := map[nicloudsdk.FeatureName]bool{
		nicloudsdk.FeatureAIBridge: true,
	}

	aiBridgeDisabledEnablements := map[nicloudsdk.FeatureName]bool{
		nicloudsdk.FeatureAIBridge: false,
	}

	aiBridgeWarningMessage := "The AI Governance add-on is required to use AI Bridge. Please reach out to your account team or sales@cloud.neuralinverse.com to learn more."

	t.Run("NoAddon_AIBridgeOff", func(t *testing.T) {
		t.Parallel()
		// License without addon and AI Bridge disabled should NOT show warning.
		lo := (&nicloudenttest.LicenseOptions{
			AccountType: "salesforce",
			AccountID:   "test",
			FeatureSet:  nicloudsdk.FeatureSetPremium,
		}).Valid(time.Now())
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, aiBridgeDisabledEnablements, nicloudenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[nicloudsdk.FeatureAIBridge]
	...
	t.Run("NoAddon_AIBridgeOn", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), generatedLicenses, aiBridgeEnabledEnablements, nicloudenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[nicloudsdk.FeatureAIBridge]
		assert.True(t, aiBridgeFeature.Enabled)
		assert.Equal(t, nicloudsdk.EntitlementEntitled, aiBridgeFeature.Entitlement)
	...
	t.Run("NoLicense_AIBridgeOn", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), []database.License{}, aiBridgeEnabledEnablements, nicloudenttest.Keys, license.FeatureArguments{})
		require.NoError(t, err)

		aiBridgeFeature := entitlements.Features[nicloudsdk.FeatureAIBridge]
		assert.Equal(t, nicloudsdk.EntitlementNotEntitled, aiBridgeFeature.Entitlement)
		require.NotContains(t, entitlements.Warnings, aiBridgeWarningMessage)
	})
}
	...
	t.Run("IssuedAtRanking", func(t *testing.T) {
		...
		lic1 := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				...
				Features: license.Features{
					nicloudsdk.FeatureManagedAgentLimit: 100,
				},
			}),
		}
		...
		lic2 := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				...
				ExpiresAt: lic2Exp,
				Features: license.Features{
					nicloudsdk.FeatureManagedAgentLimit: 50,
				},
			}),
		}
		...
		for _, order := range [][]database.License{
			...
			entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), order, map[nicloudsdk.FeatureName]bool{}, nicloudenttest.Keys, arguments)
			require.NoError(t, err)

			feature, ok := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
			require.True(t, ok, "feature %s not found", nicloudsdk.FeatureManagedAgentLimit)
			require.Equal(t, nicloudsdk.EntitlementEntitled, feature.Entitlement)
...
func TestOldStyleManagedAgentLicenses(t *testing.T) {
	...
	t.Run("SoftAndHard", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureName("managed_agent_limit_soft"): 100,
					nicloudsdk.FeatureName("managed_agent_limit_hard"): 200,
				},
			}),
		}
		...
		entitlements, err := license.LicensesEntitlements(
			...
			map[nicloudsdk.FeatureName]bool{}, nicloudenttest.Keys, arguments,
		)
		require.NoError(t, err)
	...
	t.Run("OnlySoft", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureName("managed_agent_limit_soft"): 75,
				},
			}),
		}
	...
	t.Run("OnlyHard", func(t *testing.T) {
		...
		feature := entitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
		require.Equal(t, nicloudsdk.EntitlementNotEntitled, feature.Entitlement)
	})
	...
	t.Run("ExplicitZero", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				...
				Features: license.Features{
					nicloudsdk.FeatureUserLimit:                        100,
					nicloudsdk.FeatureName("managed_agent_limit_soft"): 0,
					nicloudsdk.FeatureName("managed_agent_limit_hard"): 0,
				},
			}),
		}
...
func TestManagedAgentLimitDefault(t *testing.T) {
	...
	t.Run("Enterprise", func(t *testing.T) {
		...
		entitlements, err := license.LicensesEntitlements(context.Background(), time.Now(), []database.License{lic}, map[nicloudsdk.FeatureName]bool{}, nicloudenttest.Keys, arguments)
		require.NoError(t, err)
	...
	t.Run("PremiumExplicitZero", func(t *testing.T) {
		...
		lic := database.License{
			...
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				...
				Features: license.Features{
					nicloudsdk.FeatureUserLimit:         100,
					nicloudsdk.FeatureManagedAgentLimit: 0,
				},
			}),
		}
...
func TestAIGovernanceAddon(t *testing.T) {
	...
	t.Run("AIGovernanceAddon enables AI Governance features when enablements are set", func(t *testing.T) {
		...
		db.InsertLicense(context.Background(), database.InsertLicenseParams{
			JWT: nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
				FeatureSet: nicloudsdk.FeatureSetPremium,
				Features: license.Features{
					nicloudsdk.FeatureAIGovernanceUserLimit: 1000,
					nicloudsdk.FeatureManagedAgentLimit:     1000,
				},
				Addons: []nicloudsdk.Addon{nicloudsdk.AddonAIGovernance},
			}),
			Exp: dbtime.Now().Add(time.Hour),
		...
		entitlements, err := license.Entitlements(context.Background(), db, 1, 1, nicloudenttest.Keys, enablements)
		require.NoError(t, err)
		require.True(t, entitlements.HasLicense)
		...
		// require.Equal(t, nicloudsdk.EntitlementEntitled, aibridgeFeature.Entitlement, "AI Bridge should be entitled when addon is present")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[nicloudsdk.FeatureBoundary]
		// require.True(t, boundaryFeature.Enabled, "Boundary should be enabled when addon is present and enablements are set")
		// require.Equal(t, nicloudsdk.EntitlementEntitled, boundaryFeature.Entitlement, "Boundary should be entitled when addon is present")
	})
	...
	t.Run("AIGovernanceAddon not present disables AI Governance features", func(t *testing.T) {
		...
		// require.Equal(t, nicloudsdk.EntitlementNotEntitled, aibridgeFeature.Entitlement, "AI Bridge should not be entitled when addon is absent")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[nicloudsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled when addon is absent")
		// require.Equal(t, nicloudsdk.EntitlementNotEntitled, boundaryFeature.Entitlement, "Boundary should not be entitled when addon is absent")
	})
	...
	t.Run("AIGovernanceAddon respects grace period entitlement", func(t *testing.T) {
		...
		// require.Equal(t, nicloudsdk.EntitlementGracePeriod, aibridgeFeature.Entitlement, "AI Bridge should be in grace period")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[nicloudsdk.FeatureBoundary]
		// require.True(t, boundaryFeature.Enabled, "Boundary should be enabled during grace period")
		// require.Equal(t, nicloudsdk.EntitlementGracePeriod, boundaryFeature.Entitlement, "Boundary should be in grace period")
	})
	...
	t.Run("AIGovernanceAddon requires enablements to enable features", func(t *testing.T) {
		...
		// require.Equal(t, nicloudsdk.EntitlementEntitled, aibridgeFeature.Entitlement, "AI Bridge should still be entitled")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[nicloudsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled without enablements")
		// require.Equal(t, nicloudsdk.EntitlementEntitled, boundaryFeature.Entitlement, "Boundary should still be entitled")
	})
	...
	t.Run("AIGovernanceAddon missing dependencies", func(t *testing.T) {
		...
		// require.Equal(t, nicloudsdk.EntitlementNotEntitled, aibridgeFeature.Entitlement, "AI Bridge should not be entitled when addon validation fails")

		// TODO: Readd this test once Boundary is enforced as an add-on license.
		// boundaryFeature := entitlements.Features[nicloudsdk.FeatureBoundary]
		// require.False(t, boundaryFeature.Enabled, "Boundary should not be enabled when addon validation fails")
		// require.Equal(t, nicloudsdk.EntitlementNotEntitled, boundaryFeature.Entitlement, "Boundary should not be entitled when addon validation fails")
	})
}

func assertNoErrors(t *testing.T, entitlements nicloudsdk.Entitlements) {
	t.Helper()
	assert.Empty(t, entitlements.Errors, "no errors")
}

func assertNoWarnings(t *testing.T, entitlements nicloudsdk.Entitlements) {
	t.Helper()
	assert.Empty(t, entitlements.Warnings, "no warnings")
}

func assertEnterpriseFeatures(t *testing.T, entitlements nicloudsdk.Entitlements) {
	t.Helper()
	for _, expected := range nicloudsdk.FeatureSetEnterprise.Features() {
		f := entitlements.Features[expected]
		assert.Equalf(t, nicloudsdk.EntitlementEntitled, f.Entitlement, "%s entitled", expected)
