...
func createOpenAIModelConfigForTest(
	...
	model, err := client.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
...
func TestChatStreamRelay(t *testing.T) {
	...
	t.Run("RelayMessagePartsAcrossReplicas", func(t *testing.T) {
		...
		expClient := nicloudsdk.NewExperimentalClient(firstClient)
		...
		var localClient *nicloudsdk.ExperimentalClient
		var relayClient *nicloudsdk.ExperimentalClient
		switch runningChat.WorkerID.UUID {
		case firstReplicaID:
			localClient = nicloudsdk.NewExperimentalClient(firstClient)
			relayClient = nicloudsdk.NewExperimentalClient(secondClient)
		case secondReplicaID:
			localClient = nicloudsdk.NewExperimentalClient(secondClient)
			relayClient = nicloudsdk.NewExperimentalClient(firstClient)
		default:
			require.FailNowf(
				t,
		...
		require.Equal(t, nicloudsdk.ChatMessageRoleAssistant, firstEvent.MessagePart.Role)
		...
		require.Equal(t, nicloudsdk.ChatMessageRoleAssistant, secondSnapshotEvent.MessagePart.Role)
	...
	//
	// The bug: nicloudsdk.Client.Dial() does not propagate c.HTTPClient to
	...
	t.Run("RelayWithTLSAndCookieAuth", func(t *testing.T) {
		...
		// headers (like NI-Session-Token) on WebSocket upgrades;
		// they rely on cookies for authentication.
		//
		// We intentionally do NOT call secondClient.SetSessionToken()
		// because that would set the NI-Session-Token header,
		// which masks the bug.
		...
		// which includes Cookie but the NI-Session-Token header
	...
	// set custom headers (like NI-Session-Token) on WebSocket
	...
	// NI-Session-Token header, masking this code path.
	...
	// and sets the NI-Session-Token header so the worker
	...
	t.Run("RelayCookieOnlyAuthWithHostPrefix", func(t *testing.T) {
		...
		db, pubsub := dbtestutil.NewDB(t)
		hostPrefixValues := nicloudtest.DeploymentValues(t, func(dv *nicloudsdk.DeploymentValues) {
			require.NoError(t, dv.AI.Chat.AIGatewayRoutingEnabled.Set("false"))
		...
		// NI-Session-Token header, causing a 401.
		secondEvents, secondStream, err := relayClient.StreamChat(ctx, chat.ID, nil)
		require.NoError(t, err)
		defer secondStream.Close()
	...
	t.Run("RelaySnapshotIncludesBufferedParts", func(t *testing.T) {
		...
		for _, text := range bufferedTexts {
			...
			require.Equal(t, nicloudsdk.ChatMessageRoleAssistant, event.MessagePart.Role)
		}
...
func waitForStreamTextPart(
	...
	for {
		...
		case event, ok := <-events:
			...
			if event.Type == nicloudsdk.ChatStreamEventTypeError {
			...
			if event.Type != nicloudsdk.ChatStreamEventTypeMessagePart || event.MessagePart == nil {
				continue
			}
			if event.MessagePart.Part.Type != nicloudsdk.ChatMessagePartTypeText {
				continue
			}
...
func TestChatModelConfigDefault(t *testing.T) {
	...
	client, _ := nicloudenttest.New(t, nil)
	...
	firstModel, err := expClient.CreateChatModelConfig(
		ctx,
		nicloudsdk.CreateChatModelConfigRequest{
			Provider:             string(provider.Type),
			AIProviderID:         &provider.ID,
			Model:                "gpt-5-a",
			DisplayName:          "GPT 5 A",
	...
	updatedFirst, err := expClient.UpdateChatModelConfig(
		ctx,
		firstModel.ID,
		nicloudsdk.UpdateChatModelConfigRequest{
			IsDefault: &trueValue,
		},
	)
	require.NoError(t, err)
...
func findChatModelConfigByID(
	t *testing.T,
	modelConfigs []nicloudsdk.ChatModelConfig,
	id uuid.UUID,
) nicloudsdk.ChatModelConfig {
	t.Helper()
...
// NI-Session-Token header (for regular API calls) but
...
func (p cookieOnlySessionTokenProvider) AsRequestOption() nicloudsdk.RequestOption {
	return func(req *http.Request) {
		req.Header.Set(nicloudsdk.SessionTokenHeader, p.token)
	}
}
...
func (p cookieOnlySessionTokenProvider) SetDialOption(opts *websocket.DialOptions) {
	...
	cookieName := nicloudsdk.SessionTokenCookie
	if p.hostPrefix {
...
func TestCreateChatNonDefaultOrg(t *testing.T) {
	...
	client, firstUser := nicloudenttest.New(t, &nicloudenttest.Options{
		Options: &nicloudtest.Options{
			DeploymentValues: func() *nicloudsdk.DeploymentValues {
				v := nicloudtest.DeploymentValues(t)
				return v
			}(),
		},
		LicenseOptions: &nicloudenttest.LicenseOptions{
	...
	_, err := expClient.CreateChatModelConfig(ctx, nicloudsdk.CreateChatModelConfigRequest{
		Provider:             string(provider.Type),
		AIProviderID:         &provider.ID,
		Model:                "gpt-4o-mini",
...
func TestListChats_OrgAdminOnlySeesOwnChats(t *testing.T) {
	...
	// Create a member with agents-access in both orgs.
	memberClientRaw, _ := nicloudtest.CreateAnotherUser(
		t, client, firstUser.OrganizationID,
		rbac.ScopedRoleAgentsAccess(firstUser.OrganizationID),
		rbac.ScopedRoleAgentsAccess(secondOrg.ID),
	)
	memberExp := nicloudsdk.NewExperimentalClient(memberClientRaw)
	// Member creates a chat in the second org.
	memberChat, err := memberExp.CreateChat(ctx, nicloudsdk.CreateChatRequest{
		OrganizationID: secondOrg.ID,
		Content: []nicloudsdk.ChatInputPart{
			{
				Type: nicloudsdk.ChatInputPartTypeText,
	...
	adminClientRaw, _ := nicloudtest.CreateAnotherUser(
		t, client, firstUser.OrganizationID,
		rbac.ScopedRoleOrgAdmin(secondOrg.ID), rbac.ScopedRoleAgentsAccess(secondOrg.ID),
	)
	adminExp := nicloudsdk.NewExperimentalClient(adminClientRaw)

	// Admin creates a chat in the second org.
	adminChat, err := adminExp.CreateChat(ctx, nicloudsdk.CreateChatRequest{
		OrganizationID: secondOrg.ID,
		Content: []nicloudsdk.ChatInputPart{
			{
				Type: nicloudsdk.ChatInputPartTypeText,
