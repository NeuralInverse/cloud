...
func TestEntitlements(t *testing.T) {
	...
	t.Run("NoLicense", func(t *testing.T) {
		t.Parallel()
		adminClient, _, api, adminUser := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
			DontAddLicense: true,
		})
		anotherClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
	...
	t.Run("FullLicense", func(t *testing.T) {
		...
		features[nicloudsdk.FeatureUserLimit] = 100
		nicloudenttest.AddLicense(t, adminClient, nicloudenttest.LicenseOptions{
			Features: features,
		...
		ul := res.Features[nicloudsdk.FeatureUserLimit]
		assert.Equal(t, nicloudsdk.EntitlementEntitled, ul.Entitlement)
		...
		al := res.Features[nicloudsdk.FeatureAuditLog]
		assert.Equal(t, nicloudsdk.EntitlementEntitled, al.Entitlement)
	...
	t.Run("MultiplePrebuildsLicenseUpdates", func(t *testing.T) {
		t.Parallel()
		adminClient, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
			DontAddLicense: true,
		})
		...
		license1 := nicloudenttest.AddLicense(t, adminClient, nicloudenttest.LicenseOptions{
			Features: features,
		})
		res, err := adminClient.Entitlements(context.Background())
		require.NoError(t, err)
		require.True(t, res.HasLicense)
		require.Equal(t, nicloudsdk.EntitlementEntitled, res.Features[nicloudsdk.FeatureWorkspacePrebuilds].Entitlement)
	...
	t.Run("FullLicenseToNone", func(t *testing.T) {
		...
		anotherClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
		license := nicloudenttest.AddLicense(t, adminClient, nicloudenttest.LicenseOptions{
			Features: license.Features{
				nicloudsdk.FeatureUserLimit: 100,
		...
		al = res.Features[nicloudsdk.FeatureAuditLog]
		assert.Equal(t, nicloudsdk.EntitlementNotEntitled, al.Entitlement)
		assert.False(t, al.Enabled)
	})
	t.Run("Pubsub", func(t *testing.T) {
		t.Parallel()
		adminClient, _, api, adminUser := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{DontAddLicense: true})
		anotherClient, _ := nicloudtest.CreateAnotherUser(t, adminClient, adminUser.OrganizationID)
		...
		require.NoError(t, err)
		err = api.Pubsub.Publish(nicloud.PubsubEventLicenses, []byte{})
		require.NoError(t, err)
...
func TestEntitlements_HeaderWarnings(t *testing.T) {
	...
	t.Run("ExistForAdmin", func(t *testing.T) {
		...
		require.NotEmpty(t, res.Header.Values(nicloudsdk.EntitlementsWarningHeader))
	})
	t.Run("NoneForNormalUser", func(t *testing.T) {
		...
		defer res.Body.Close()
		require.Equal(t, http.StatusOK, res.StatusCode)
		require.Empty(t, res.Header.Values(nicloudsdk.EntitlementsWarningHeader))
	})
}
...
func TestEntitlements_Prebuilds(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{
				...
				LicenseOptions: &nicloudenttest.LicenseOptions{
					Features: license.Features{
						nicloudsdk.FeatureWorkspacePrebuilds: prebuildsEntitled,
					},
				},
			})
...
func TestAuditLogging(t *testing.T) {
	...
	t.Run("Disabled", func(t *testing.T) {
		t.Parallel()
		_, _, api, _ := nicloudenttest.NewWithAPI(t, &nicloudenttest.Options{DontAddLicense: true})
		auditor := *api.AGPL.Auditor.Load()
		ea := agplaudit.NewNop()
		t.Logf("%T = %T", auditor, ea)
	...
	t.Run("FullBuild", func(t *testing.T) {
		...
		build := nicloudtest.CreateWorkspaceBuild(t, client, r.workspace, database.WorkspaceTransitionStop)
...
func TestExternalTokenEncryption(t *testing.T) {
	...
	t.Run("Enabled", func(t *testing.T) {
		...
		client, _ := nicloudenttest.New(t, &nicloudenttest.Options{
			...
			LicenseOptions: &nicloudenttest.LicenseOptions{
				Features: license.Features{
					nicloudsdk.FeatureExternalTokenEncryption: 1,
				},
			},
			Options: &nicloudtest.Options{
				Database: db,
		...
		require.Eventually(t, func() bool {
			...
			assert.NoError(t, err)
			feature := entitlements.Features[nicloudsdk.FeatureExternalTokenEncryption]
			entitled := feature.Entitlement == nicloudsdk.EntitlementEntitled
			var warningExists bool
			for _, warning := range entitlements.Warnings {
				if strings.Contains(warning, nicloudsdk.FeatureExternalTokenEncryption.Humanize()) {
					warningExists = true
					break
				}
			}
	...
	t.Run("PreviouslyEnabledButMissingFromLicense", func(t *testing.T) {
		...
		// actively been using this feature is now unable _start nicloud_
...
func TestMultiReplica_EmptyRelayAddress(t *testing.T) {
	...
	_, _ = nicloudenttest.New(t, &nicloudenttest.Options{
		EntitlementsUpdateInterval: 25 * time.Millisecond,
	...
	// Send a bunch of updates to see if the nicloud will log errors.
	{
...
func TestSCIMDisabled(t *testing.T) {
	...
	cli, _ := nicloudenttest.New(t, &nicloudenttest.Options{})
	...
	for _, p := range checkPaths {
		t.Run(p, func(t *testing.T) {
			...
			var apiError nicloudsdk.Response
...
func TestManagedAgentLimit(t *testing.T) {
	...
	cli, owner := nicloudenttest.New(t, &nicloudenttest.Options{
		Options: &nicloudtest.Options{
			IncludeProvisionerDaemon: true,
		},
		LicenseOptions: (&nicloudenttest.LicenseOptions{
	...
	agentLimit := sdkEntitlements.Features[nicloudsdk.FeatureManagedAgentLimit]
	...
	aiVersion := nicloudtest.CreateTemplateVersion(t, cli, uuid.Nil, echoRes)
	nicloudtest.AwaitTemplateVersionJobCompleted(t, cli, aiVersion.ID)
	aiTemplate := nicloudtest.CreateTemplate(t, cli, uuid.Nil, aiVersion.ID)
	noAiVersion := nicloudtest.CreateTemplateVersion(t, cli, uuid.Nil, nil) // use default responses
	nicloudtest.AwaitTemplateVersionJobCompleted(t, cli, noAiVersion.ID)
	noAiTemplate := nicloudtest.CreateTemplate(t, cli, uuid.Nil, noAiVersion.ID)

	// Create one AI workspace, which should succeed.
	task, err := cli.CreateTask(ctx, owner.UserID.String(), nicloudsdk.CreateTaskRequest{
	...
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, cli, workspace.LatestBuild.ID)
	...
	// workspace creation.
	task2, err := cli.CreateTask(ctx, owner.UserID.String(), nicloudsdk.CreateTaskRequest{
		Name:                    namesgenerator.UniqueNameWith("-"),
		TemplateVersionID:       aiTemplate.ActiveVersionID,
		TemplateVersionPresetID: uuid.Nil,
		Input:                   "hi",
	...
	require.NoError(t, err, "fetching AI workspace must succeed")
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, cli, workspace2.LatestBuild.ID)

	// Create a third workspace using the same template, which should succeed.
	workspace = nicloudtest.CreateWorkspace(t, cli, aiTemplate.ID)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, cli, workspace.LatestBuild.ID)

	// Create a fourth non-AI workspace, which should also succeed.
	workspace = nicloudtest.CreateWorkspace(t, cli, noAiTemplate.ID)
	nicloudtest.AwaitWorkspaceBuildJobCompleted(t, cli, workspace.LatestBuild.ID)
}
...
func TestCheckBuildUsage_NeverBlocksOnManagedAgentLimit(t *testing.T) {
	...
	entSet.Modify(func(e *nicloudsdk.Entitlements) {
		e.HasLicense = true
		limit := int64(1)
		issuedAt := time.Now().Add(-2 * time.Hour)
		start := time.Now().Add(-time.Hour)
		end := time.Now().Add(time.Hour)
		e.Features[nicloudsdk.FeatureManagedAgentLimit] = nicloudsdk.Feature{
			Enabled:     true,
			Limit:       &limit,
			UsagePeriod: &nicloudsdk.UsagePeriod{IssuedAt: issuedAt, Start: start, End: end},
		}
	})

	// Enterprise API instance with entitlements injected.
	agpl := &agplnicloud.API{
		Options: &agplnicloud.Options{
			Entitlements: entSet,
		},
	}
	eapi := &nicloud.API{
		AGPL:    agpl,
		Options: &nicloud.Options{Options: agpl.Options},
	}
...
func TestCheckBuildUsage_BlocksWithoutManagedAgentEntitlement(t *testing.T) {
	...
	tests := []struct {
		name      string
		setupEnts func(e *nicloudsdk.Entitlements)
	}{
		{
			name: "FeatureAbsent",
			setupEnts: func(e *nicloudsdk.Entitlements) {
...
type restartableTestServer struct {
	options *nicloudenttest.Options
...
func newRestartableTestServer(t *testing.T, options *nicloudenttest.Options) (*nicloudsdk.Client, nicloudsdk.CreateFirstUserResponse, *restartableTestServer) {
	t.Helper()
	if options == nil {
		options = &nicloudenttest.Options{}
	}
...
func (s *restartableTestServer) startWithFirstUser(t *testing.T) (client *nicloudsdk.Client, firstUser nicloudsdk.CreateFirstUserResponse) {
	t.Helper()
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	// used in this test.
	client, s.closer, s.api, firstUser = nicloudenttest.NewWithAPI(t, s.options)
...
func TestConn_CoordinatorRollingRestart(t *testing.T) {
	...
	for _, direct := range []bool{true, false} {
		...
		t.Run(name, func(t *testing.T) {
			...
			dv := nicloudtest.DeploymentValues(t, func(dv *nicloudsdk.DeploymentValues) {
				dv.DERP.Config.BlockDirect = serpent.Bool(!direct)
			})
			logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)

			// Create two restartable test servers with the same database.
			client1, user, s1 := newRestartableTestServer(t, &nicloudenttest.Options{
			...
			client2, _, s2 := newRestartableTestServer(t, &nicloudenttest.Options{
			...
			resources := nicloudtest.NewWorkspaceAgentWaiter(t, client1, workspace.Workspace.ID).Wait()
