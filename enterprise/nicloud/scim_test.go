...
import (
	...
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/legacyscim"
...
func makeScimUser(t testing.TB) legacyscim.SCIMUser {
	...
	return legacyscim.SCIMUser{
		...
		Emails: []struct {
			...
			{Primary: true, Value: fmt.Sprintf("%s@cloud.neuralinverse.com", rstr)},
		},
		Active: ptr.Ref(true),
	}
}
...
// enterprise/nicloud/scim/scimusers_test.go.
//
...
func TestLegacyScim(t *testing.T) {
	...
	t.Run("disabled", func(t *testing.T) {
		...
		client, _ := nicloudenttest.New(t, &nicloudenttest.Options{
			...
			LicenseOptions: &nicloudenttest.LicenseOptions{
				AccountID: "coolin",
				Features:  license.Features{nicloudsdk.FeatureSCIM: 0},
			},
		})
	...
	t.Run("noAuth", func(t *testing.T) {
		...
		client, _ := nicloudenttest.New(t, &nicloudenttest.Options{
			...
			LicenseOptions: &nicloudenttest.LicenseOptions{
				AccountID: "coolin",
				Features:  license.Features{nicloudsdk.FeatureSCIM: 1},
			},
		})
	...
	t.Run("postUser", func(t *testing.T) {
		...
		// Verify user exists.
		userRes, err := client.Users(ctx, nicloudsdk.UsersRequest{Search: createdUser.UserName})
		require.NoError(t, err)
		require.Len(t, userRes.Users, 1)
		assert.Equal(t, nicloudsdk.LoginTypeOIDC, userRes.Users[0].LoginType)
	...
	t.Run("Duplicate", func(t *testing.T) {
		...
		userRes, err := client.Users(ctx, nicloudsdk.UsersRequest{Search: sUser.UserName})
		require.NoError(t, err)
		require.Len(t, userRes.Users, 1)
	})
	...
	t.Run("patchUser", func(t *testing.T) {
		...
		mockAudit := audit.NewMock()
		client, _ := nicloudenttest.New(t, &nicloudenttest.Options{
			Options:       &nicloudtest.Options{Auditor: mockAudit},
			SCIMAPIKey:    scimAPIKey,
			UseLegacySCIM: true,
			AuditLogging:  true,
			LicenseOptions: &nicloudenttest.LicenseOptions{
		...
		require.NoError(t, err)
		assert.Equal(t, nicloudsdk.UserStatusSuspended, userRes.Status)
	})
...
func TestScim(t *testing.T) {
	...
	t.Run("postUser", func(t *testing.T) {
		...
		// Verify user exists.
		userRes, err := client.Users(ctx, nicloudsdk.UsersRequest{Search: created.UserName})
		require.NoError(t, err)
		require.Len(t, userRes.Users, 1)
		assert.Equal(t, nicloudsdk.LoginTypeOIDC, userRes.Users[0].LoginType)
	...
	t.Run("postUserReactivatesSuspended", func(t *testing.T) {
		...
		userRes, err = client.User(ctx, created.ID)
		require.NoError(t, err)
		assert.Equal(t, nicloudsdk.UserStatusDormant, userRes.Status)
	})
