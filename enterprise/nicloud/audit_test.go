...
func TestEnterpriseAuditLogs(t *testing.T) {
	...
	t.Run("IncludeOrganization", func(t *testing.T) {
		...
		require.Equal(t, &nicloudsdk.MinimalOrganization{
			ID:          o.ID,
			Name:        o.Name,
		...
		err = client.CreateTestAuditLog(ctx, nicloudsdk.CreateTestAuditLogRequest{
			ResourceType: nicloudsdk.ResourceTypeAPIKey,
			ResourceID:   user.UserID,
		})
		require.NoError(t, err)
		...
		require.Equal(t, (*nicloudsdk.MinimalOrganization)(nil), alogs.AuditLogs[0].Organization)
