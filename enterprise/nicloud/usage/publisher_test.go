...
func TestIntegration(t *testing.T) {
	...
	// enough permissions.
	authzDB := dbauthz.New(db, rbac.NewAuthorizer(prometheus.NewRegistry()), log, nicloudtest.AccessControlStorePointer())
	publisher := usage.NewTallymanPublisher(ctx, log, authzDB, nicloudenttest.Keys,
...
func TestPublisherNoEligibleLicenses(t *testing.T) {
	...
	publisher := usage.NewTallymanPublisher(ctx, log, db, nicloudenttest.Keys,
		usage.PublisherWithClock(clock),
		usage.PublisherWithIngestURL(ingestURL),
	)
	defer publisher.Close()
	...
	licenseJWT := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
...
func TestPublisherLicenseSelection(t *testing.T) {
	...
	badLicense1 := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: false,
		IssuedAt:         now.Add(-30 * time.Minute),
	})
	badLicense2 := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-1 * time.Hour),
		AccountType:      "trial",
	})
	badLicense3 := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-30 * time.Minute),
		ExpiresAt:        now.Add(-10 * time.Minute),
	})
	badLicense4 := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-1 * time.Hour),
	})
	expectedLicense := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: true,
		IssuedAt:         now.Add(-30 * time.Minute),
	})
...
func configureDeployment(ctx context.Context, t *testing.T, db database.Store) (uuid.UUID, string) {
	...
	licenseRaw := nicloudenttest.GenerateLicense(t, nicloudenttest.LicenseOptions{
		PublishUsageData: true,
	})
