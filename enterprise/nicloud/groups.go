...
// @Produce json
// @Tags Enterprise
// @Param request body nicloudsdk.CreateGroupRequest true "Create group request"
// @Param organization path string true "Organization ID"
// @Success 201 {object} nicloudsdk.Group
...
func (api *API) postGroupByOrganization(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateGroupRequest
	...
	if req.Name == database.EveryoneGroup {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
			...
			Validations: []nicloudsdk.ValidationError{{Field: "name", Detail: fmt.Sprintf("%q is a reserved group name", req.Name)}},
		})
		return
	}
	...
	if database.IsUniqueViolation(err) {
		httpapi.Write(ctx, rw, http.StatusConflict, nicloudsdk.Response{
			...
			Validations: []nicloudsdk.ValidationError{{Field: "name", Detail: "Group names must be unique"}},
		})
		return
	}
	if err != nil {
...
// @Param request body nicloudsdk.PatchGroupRequest true "Patch group request"
// @Success 200 {object} nicloudsdk.Group
...
func (api *API) patchGroup(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.PatchGroupRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
...
// @Success 200 {object} nicloudsdk.GroupMembersResponse
...
func (api *API) groupMembers(rw http.ResponseWriter, r *http.Request) {
	...
	if len(members) == 0 {
		httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.GroupMembersResponse{
			Users: nil,
			Count: 0,
		})
		return
...
// @Tags Enterprise
// @Param organization path string true "Organization ID" format(uuid)
// @Success 200 {array} nicloudsdk.Group
...
func (api *API) groups(rw http.ResponseWriter, r *http.Request) {
	...
	resp := make([]nicloudsdk.Group, 0, len(groups))
