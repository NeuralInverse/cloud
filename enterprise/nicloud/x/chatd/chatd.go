...
type MultiReplicaSubscribeConfig struct {
	...
	DialerFn              func(
		...
		parts <-chan nicloudsdk.ChatStreamEvent,
		cancel func(),
		err error,
	)
...
func NewMultiReplicaSubscribeFn(
	...
	return func(ctx context.Context, params osschatd.SubscribeFnParams) <-chan nicloudsdk.ChatStreamEvent {
		chatID := params.ChatID
		requestHeader := params.RequestHeader
		logger := params.Logger

		var relayCancel func()
		var relayParts <-chan nicloudsdk.ChatStreamEvent
		...
		var initialRelaySnapshot []nicloudsdk.ChatStreamEvent
		...
		openRelayAsync := func(workerID uuid.UUID) {
			...
			go func() {
				...
				wrappedParts := make(chan nicloudsdk.ChatStreamEvent, 128)
				go func() {
		...
		sendRelayTerminalError := func(msg string) {
			...
			case mergedEvents <- nicloudsdk.ChatStreamEvent{
				...
				ChatID: chatID,
				Error:  &nicloudsdk.ChatError{Message: msg},
			}:
			case <-ctx.Done():
			}
		}
...
func dialRelay(
	...
	headers.Set(nicloudsdk.SessionTokenHeader, extractSessionToken(requestHeader))
	...
	// Match the server's 4 MiB read limit in nicloudsdk.StreamChat so
	// large message_part batches don't trip the default 32 KiB cap.
	conn.SetReadLimit(1 << 22)

	snapshot = make([]nicloudsdk.ChatStreamEvent, 0, 100)
	...
	sourceEvents := make(chan nicloudsdk.ChatStreamEvent, 128)
	go func() {
		defer close(sourceEvents)
		for {
			var batch []nicloudsdk.ChatStreamEvent
...
drainInitial:
	...
	events := make(chan nicloudsdk.ChatStreamEvent, 128)
...
// apiKeyMiddleware: cookie, then Neural Inverse Cloud-Session-Token header, then
...
func extractSessionToken(header http.Header) string {
	...
	if raw := header.Get(cookieHeader); raw != "" {
		...
		if c, err := r.Cookie(nicloudsdk.SessionTokenCookie); err == nil && c.Value != "" {
			return c.Value
		}
	}
	// Neural Inverse Cloud-Session-Token header (SDK / CLI callers).
	if v := header.Get(nicloudsdk.SessionTokenHeader); v != "" {
		return v
	}
