...
import (
	...
	osschatd "github.com/NeuralInverse/cloud/v2/nicloud/x/chatd"
...
func TestSubscribeRelayReconnectsOnDrop(t *testing.T) {
	...
	provider := func(ctx context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		[]nicloudsdk.ChatStreamEvent, <-chan nicloudsdk.ChatStreamEvent, func(), error,
	) {
		call := callCount.Add(1)
		ch := make(chan nicloudsdk.ChatStreamEvent, 10)
		if call == 1 {
			// First relay: send a part then close to simulate a drop.
			ch <- nicloudsdk.ChatStreamEvent{
				Type: nicloudsdk.ChatStreamEventTypeMessagePart,
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("first-relay"),
				},
			}
			close(ch)
		} else {
			...
			ch <- nicloudsdk.ChatStreamEvent{
				...
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("second-relay"),
				},
			}
...
func TestSubscribeRelayAsyncDoesNotBlock(t *testing.T) {
	...
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), payload)
	require.NoError(t, err)
	...
	statusNotify := nicloudpubsub.ChatStreamNotifyMessage{
		Status: string(database.ChatStatusWaiting),
	}
	statusPayload, err := json.Marshal(statusNotify)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), statusPayload)
	require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		select {
		case event := <-events:
			return event.Type == nicloudsdk.ChatStreamEventTypeStatus &&
				event.Status != nil &&
				event.Status.Status == nicloudsdk.ChatStatusWaiting
		default:
			return false
		}
...
func TestSubscribeRelaySnapshotDelivered(t *testing.T) {
	...
	provider := func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		...
		snapshot := []nicloudsdk.ChatStreamEvent{
			{
				Type: nicloudsdk.ChatStreamEventTypeMessagePart,
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("snap-one"),
				},
			},
			{
				Type: nicloudsdk.ChatStreamEventTypeMessagePart,
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("snap-two"),
				},
			},
		}
		ch := make(chan nicloudsdk.ChatStreamEvent, 10)
		...
		ch <- nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
				Part: nicloudsdk.ChatMessageText("live-part"),
			},
		}
	...
	var snapshotStatus nicloudsdk.ChatStatus
	...
	require.Equal(t, nicloudsdk.ChatStatusRunning, snapshotStatus)
}
...
func TestSubscribeRetryEventAcrossInstances(t *testing.T) {
	...
	var retryEvent *nicloudsdk.ChatStreamRetry
	...
	require.Eventually(t, func() bool {
		...
		case event, ok := <-events:
			...
			case nicloudsdk.ChatStreamEventTypeStatus:
				if event.Status != nil && event.Status.Status == nicloudsdk.ChatStatusWaiting {
					if retryEvent == nil {
	...
	require.Equal(t, nicloudsdk.ChatErrorKindRateLimit, retryEvent.Kind)
...
func TestSubscribeRelayStaleDialDiscardedAfterInterrupt(t *testing.T) {
	...
	provider := func(ctx context.Context, _ uuid.UUID, workerID uuid.UUID, _ http.Header) (
		...
		if call == 1 {
			...
			ch <- nicloudsdk.ChatStreamEvent{
				...
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("stale-part"),
				},
			}
			close(ch)
			return nil, ch, func() {}, nil
		...
		ch <- nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
				Part: nicloudsdk.ChatMessageText("new-worker-part"),
			},
		}
		return nil, ch, func() {}, nil
	}
	...
	require.NoError(t, err)
	oldRunningNotify := nicloudpubsub.ChatStreamNotifyMessage{
		Status:   string(database.ChatStatusRunning),
		WorkerID: oldWorkerID.String(),
	}
	oldRunningPayload, err := json.Marshal(oldRunningNotify)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), oldRunningPayload)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	waitingNotify := nicloudpubsub.ChatStreamNotifyMessage{
		Status: string(database.ChatStatusWaiting),
	}
	waitingPayload, err := json.Marshal(waitingNotify)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), waitingPayload)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	runningNotify := nicloudpubsub.ChatStreamNotifyMessage{
		Status:   string(database.ChatStatusRunning),
		WorkerID: newWorkerID.String(),
	}
	runningPayload, err := json.Marshal(runningNotify)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), runningPayload)
	require.NoError(t, err)
	...
	require.Never(t, func() bool {
		select {
		case event := <-events:
			return event.Type == nicloudsdk.ChatStreamEventTypeMessagePart &&
				event.MessagePart != nil &&
				event.MessagePart.Part.Text == "stale-part"
		default:
			return false
...
func TestSubscribeRelayRunningToRunningSwitch(t *testing.T) {
	...
	provider := func(ctx context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		...
		ch <- nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
				Part: nicloudsdk.ChatMessageText("worker-b-part"),
			},
		}
		return nil, ch, func() {}, nil
	}
	...
	notifyA := nicloudpubsub.ChatStreamNotifyMessage{
		Status:   string(database.ChatStatusRunning),
		WorkerID: workerA.String(),
	}
	payloadA, err := json.Marshal(notifyA)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), payloadA)
	require.NoError(t, err)
	...
	notifyB := nicloudpubsub.ChatStreamNotifyMessage{
		Status:   string(database.ChatStatusRunning),
		WorkerID: workerB.String(),
	}
	payloadB, err := json.Marshal(notifyB)
	require.NoError(t, err)
	err = ps.Publish(nicloudpubsub.ChatStreamNotifyChannel(chat.ID), payloadB)
	require.NoError(t, err)
...
func TestSubscribeRelayFailedDialRetries(t *testing.T) {
	...
	provider := func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		...
		ch <- nicloudsdk.ChatStreamEvent{
			...
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
				Part: nicloudsdk.ChatMessageText("retry-success"),
			},
		}
		return nil, ch, func() {}, nil
	}
	...
	provider := func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		...
		if call == 1 {
			...
			ch <- nicloudsdk.ChatStreamEvent{
				...
				MessagePart: &nicloudsdk.ChatStreamMessagePart{
					Role: "assistant",
					Part: nicloudsdk.ChatMessageText("remote-part"),
				},
			}
...
func TestSubscribeRelayMultipleReconnects(t *testing.T) {
	...
	provider := func(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ http.Header) (
		...
		call := callCount.Add(1)
		ch := make(chan nicloudsdk.ChatStreamEvent, 10)
		part := nicloudsdk.ChatStreamEvent{
			Type: nicloudsdk.ChatStreamEventTypeMessagePart,
			MessagePart: &nicloudsdk.ChatStreamMessagePart{
				Role: "assistant",
...
func TestSubscribeRelayDialCanceledOnFastCompletion(t *testing.T) {
	...
	_, err := worker.SendMessage(ctx, osschatd.SendMessageOptions{
		ChatID:    chat.ID,
		CreatedBy: user.ID,
		Content:   []nicloudsdk.ChatMessagePart{nicloudsdk.ChatMessageText("hello")},
	})
	require.NoError(t, err)
...
waitForStream:
	...
	require.Eventually(t, func() bool {
		...
		case event := <-events:
			return event.Type == nicloudsdk.ChatStreamEventTypeStatus &&
				event.Status != nil &&
				event.Status.Status == nicloudsdk.ChatStatusRunning
		default:
			return false
		}
	}, testutil.WaitMedium, testutil.IntervalFast)
