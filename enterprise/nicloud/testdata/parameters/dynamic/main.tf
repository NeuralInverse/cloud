...
locals {
  isAdmin = contains(data.ni_workspace_owner.me.groups, "admin")
}

data "ni_parameter" "isAdmin" {
  name      = "isAdmin"
  type      = "bool"
  form_type = "switch"
...
data "ni_parameter" "groups" {
  name      = "groups"
  type      = "list(string)"
  form_type = "multi-select"
  default   = jsonencode([data.ni_workspace_owner.me.groups[0]])
  order     = 50
...
locals {
  selected = jsondecode(data.ni_parameter.colors.value)
  things = flatten([
    for color in local.selected : local.colors[color]
  ])
}

data "ni_parameter" "thing" {
  name      = "thing"
  type      = "string"
  form_type = "dropdown"
  order     = 101
