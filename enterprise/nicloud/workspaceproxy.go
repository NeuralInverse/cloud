...
func (api *API) fetchRegions(ctx context.Context) (nicloudsdk.RegionsResponse[nicloudsdk.Region], error) {
	...
	if err != nil {
		return nicloudsdk.RegionsResponse[nicloudsdk.Region]{}, err
	}

	regions := make([]nicloudsdk.Region, 0, len(proxies.Regions))
	...
	return nicloudsdk.RegionsResponse[nicloudsdk.Region]{
		Regions: regions,
	}, nil
}
...
// @Param request body nicloudsdk.PatchWorkspaceProxy true "Update workspace proxy request"
// @Success 200 {object} nicloudsdk.WorkspaceProxy
...
func (api *API) patchWorkspaceProxy(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.PatchWorkspaceProxy
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.UpdateWorkspaceProxyResponse{
...
func (api *API) patchPrimaryWorkspaceProxy(req nicloudsdk.PatchWorkspaceProxy, rw http.ResponseWriter, r *http.Request) (database.WorkspaceProxy, bool) {
	var (
		ctx   = r.Context()
		proxy = httpmw.WorkspaceProxyParam(r)
	)
...
// @Produce json
// @Tags Enterprise
// @Param request body nicloudsdk.CreateWorkspaceProxyRequest true "Create workspace proxy request"
// @Success 201 {object} nicloudsdk.WorkspaceProxy
...
func (api *API) postWorkspaceProxy(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateWorkspaceProxyRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	aReq.New = proxy
	httpapi.Write(ctx, rw, http.StatusCreated, nicloudsdk.UpdateWorkspaceProxyResponse{
...
// @Produce json
// @Tags Enterprise
// @Success 200 {array} nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy]
...
func (api *API) fetchWorkspaceProxies(ctx context.Context) (nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy], error) {
	proxies, err := api.Database.GetWorkspaceProxies(ctx)
	if err != nil && !xerrors.Is(err, sql.ErrNoRows) {
		return nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy]{}, err
	}
	...
	return nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy]{
...
func (api *API) workspaceProxyIssueSignedAppToken(rw http.ResponseWriter, r *http.Request) {
	...
	userReq.Header.Set(nicloudsdk.SessionTokenHeader, req.SessionToken)
	userReq.RemoteAddr = r.Header.Get(wsproxysdk.NIWorkspaceProxyRealIPHeader)
...
func (api *API) workspaceProxyRegister(rw http.ResponseWriter, r *http.Request) {
	...
	//
	// The mesh key is always generated at startup by an enterprise nicloud
	// server.
	var meshKey string
	if req.DerpEnabled {
		var err error
	...
	siblingsRes := make([]nicloudsdk.Replica, 0, len(siblings))
...
// @Accept json
// @Produce json
// @Param request body nicloudsdk.IssueReconnectingPTYSignedTokenRequest true "Issue reconnecting PTY signed token request"
// @Success 200 {object} nicloudsdk.IssueReconnectingPTYSignedTokenResponse
...
func (api *API) reconnectingPTYSignedToken(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.IssueReconnectingPTYSignedTokenRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	httpapi.Write(ctx, rw, http.StatusOK, nicloudsdk.IssueReconnectingPTYSignedTokenResponse{
...
func convertProxies(p []database.WorkspaceProxy, statuses map[uuid.UUID]proxyhealth.ProxyStatus) []nicloudsdk.WorkspaceProxy {
	resp := make([]nicloudsdk.WorkspaceProxy, 0, len(p))
	for _, proxy := range p {
...
func convertRegion(proxy database.WorkspaceProxy, status proxyhealth.ProxyStatus) nicloudsdk.Region {
	return nicloudsdk.Region{
		ID:               proxy.ID,
		Name:             proxy.Name,
...
func convertProxy(p database.WorkspaceProxy, status proxyhealth.ProxyStatus) nicloudsdk.WorkspaceProxy {
	now := dbtime.Now()
	if p.IsPrimary() {
		status = proxyhealth.ProxyStatus{
			Proxy:     p,
			Status:    proxyhealth.Healthy,
			Report:    nicloudsdk.ProxyHealthReport{},
			CheckedAt: now,
		}
	...
	return nicloudsdk.WorkspaceProxy{
		...
		Deleted:     p.Deleted,
		Version:     p.Version,
		Status: nicloudsdk.WorkspaceProxyStatus{
			Status:    nicloudsdk.ProxyHealthStatus(status.Status),
...
type workspaceProxiesFetchUpdater struct {
	fetchFunc  func(context.Context) (nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy], error)
	updateFunc func(context.Context) error
}

func (w *workspaceProxiesFetchUpdater) Fetch(ctx context.Context) (nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy], error) {
