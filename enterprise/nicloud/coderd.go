...
import (
	...
	agplaudit "github.com/NeuralInverse/cloud/v2/nicloud/audit"
	"github.com/NeuralInverse/cloud/v2/nicloud/boundaryusage"
	agplconnectionlog "github.com/NeuralInverse/cloud/v2/nicloud/connectionlog"
	"github.com/NeuralInverse/cloud/v2/nicloud/database"
	agpldbauthz "github.com/NeuralInverse/cloud/v2/nicloud/database/dbauthz"
	...
	agplportsharing "github.com/NeuralInverse/cloud/v2/nicloud/portsharing"
	...
	agplusage "github.com/NeuralInverse/cloud/v2/nicloud/usage"
	"github.com/NeuralInverse/cloud/v2/nicloud/wsbuilder"
	"github.com/NeuralInverse/cloud/v2/nicloudsdk"
	"github.com/NeuralInverse/cloud/v2/enterprise/aiseats"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/connectionlog"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/dbauthz"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/enidpsync"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/license"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/portsharing"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/prebuilds"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/proxyhealth"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/schedule"
	"github.com/NeuralInverse/cloud/v2/enterprise/nicloud/usage"
	entchatd "github.com/NeuralInverse/cloud/v2/enterprise/nicloud/x/chatd"
	"github.com/NeuralInverse/cloud/v2/enterprise/dbcrypt"
	"github.com/NeuralInverse/cloud/v2/enterprise/derpmesh"
	"github.com/NeuralInverse/cloud/v2/enterprise/replicasync"
	"github.com/NeuralInverse/cloud/v2/enterprise/tailnet"
	"github.com/NeuralInverse/cloud/v2/provisionerd/proto"
	agpltailnet "github.com/NeuralInverse/cloud/v2/tailnet"
	"github.com/coder/quartz"
)

// New constructs an Enterprise nicloud API instance.
// This handler is designed to wrap the AGPL Neural Inverse Cloud code and
...
func New(ctx context.Context, options *Options) (_ *API, err error) {
	...
	if options.Options == nil {
		options.Options = &nicloud.Options{}
	}
	...
	if err != nil {
		...
		if xerrors.As(err, &derr) {
			return nil, xerrors.Errorf("database encrypted with unknown key, either add the key or see https://cloud.neuralinverse.com/docs/admin/encryption#disabling-encryption: %w", derr)
		}
	...
	// This must happen before nicloud initialization!
	...
	// and pubsub coordination. Must be set before nicloud.New so the
	...
	// api.AGPL.ID after nicloud.New is called. The SubscribeFn is
	...
	api.AGPL = nicloud.New(options.Options)
	...
	apiKeyMiddleware := httpmw.ExtractAPIKeyMW(httpmw.ExtractAPIKeyConfig{
		DB:                            options.Database,
		ActivateDormantUser:           nicloud.ActivateDormantUser(options.Logger, &api.AGPL.Auditor, options.Database),
	...
	api.AGPL.APIHandler.Group(func(r chi.Router) {
		r.Route("/aibridge/keys", func(r chi.Router) {
			r.Use(
				apiKeyMiddleware,
				api.RequireFeatureMW(nicloudsdk.FeatureAIBridge),
			)
	...
	api.AGPL.APIHandler.Group(func(r chi.Router) {
		...
		r.Route("/connectionlog", func(r chi.Router) {
			r.Use(
				apiKeyMiddleware,
				api.RequireFeatureMW(nicloudsdk.FeatureConnectionLog),
			)
		...
		r.Route("/workspaceproxies", func(r chi.Router) {
			r.Use(
				api.RequireFeatureMW(nicloudsdk.FeatureWorkspaceProxy),
			)
			r.Group(func(r chi.Router) {
				r.Use(
					apiKeyMiddleware,
		...
		r.Group(func(r chi.Router) {
			r.Use(
				apiKeyMiddleware,
				api.RequireFeatureMW(nicloudsdk.FeatureMultipleOrganizations),
			)
		...
		r.Group(func(r chi.Router) {
			r.Use(
				apiKeyMiddleware,
				api.RequireFeatureMW(nicloudsdk.FeatureCustomRoles),
				httpmw.ExtractOrganizationParam(api.Database),
			)
		...
		r.Route("/organizations/{organization}/provisionerkeys", func(r chi.Router) {
			r.Use(
				apiKeyMiddleware,
				httpmw.ExtractOrganizationParam(api.Database),
				api.RequireFeatureMW(nicloudsdk.FeatureExternalProvisionerDaemons),
			)
		...
		r.Route("/templates/{template}/prebuilds", func(r chi.Router) {
			r.Use(
				api.RequireFeatureMW(nicloudsdk.FeatureWorkspacePrebuilds),
				apiKeyMiddleware,
				httpmw.ExtractTemplateParam(api.Database),
			)
		...
		r.Route("/workspaces/{workspace}/external-agent", func(r chi.Router) {
			r.Use(
				...
				api.RequireFeatureMW(nicloudsdk.FeatureWorkspaceExternalAgent),
			)
	...
	// enabled, since it's used to detect other neuralinverse servers for licensing.
...
type API struct {
	...
	*Options
	...
	// Detects multiple Neural Inverse Cloud replicas running at the same time.
...
func (api *API) Close() error {
	...
	// This tells other Neural Inverse Cloudds that it is now offline.
...
func (api *API) updateEntitlements(ctx context.Context) error {
	return api.Entitlements.Update(ctx, func(ctx context.Context) (nicloudsdk.Entitlements, error) {
		...
		for _, replica := range replicas {
			...
			// want to display a warning. In the open-source version of Neural Inverse Cloud,
		...
		reloadedEntitlements, err := license.Entitlements(
			ctx, api.Database,
			len(agedReplicas), len(api.ExternalAuthConfigs), api.LicenseKeys, map[nicloudsdk.FeatureName]bool{
				nicloudsdk.FeatureAuditLog:                   api.AuditLogging,
				nicloudsdk.FeatureConnectionLog:              api.ConnectionLogging,
				nicloudsdk.FeatureBrowserOnly:                api.BrowserOnly,
				nicloudsdk.FeatureSCIM:                       len(api.SCIMAPIKey) != 0,
				nicloudsdk.FeatureMultipleExternalAuth:       len(api.ExternalAuthConfigs) > 1,
				nicloudsdk.FeatureTemplateRBAC:               api.RBAC,
				nicloudsdk.FeatureExternalTokenEncryption:    len(api.ExternalTokenEncryption) > 0,
				...
				nicloudsdk.FeatureAIBridge:                   api.DeploymentValues.AI.BridgeConfig.Enabled.Value(),
			})
		if err != nil {
			return nicloudsdk.Entitlements{}, err
		}
		...
		featureChanged := func(featureName nicloudsdk.FeatureName) (initial, changed, enabled bool) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureAuditLog); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureConnectionLog); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureBrowserOnly); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureTemplateRBAC); shouldUpdate(initial, changed, enabled) {
			if enabled {
				committer := committer{
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureAdvancedTemplateScheduling); shouldUpdate(initial, changed, enabled) {
			if enabled {
				...
				if api.DefaultQuietHoursSchedule == "" {
					api.Logger.Warn(ctx, "template autostop requirement will default to UTC midnight as the default user quiet hours schedule. Set a custom default quiet hours schedule using NEURALINVERSE_QUIET_HOURS_DEFAULT_SCHEDULE to avoid this warning")
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureHighAvailability); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureWorkspaceProxy); shouldUpdate(initial, changed, enabled) {
			if enabled {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureAccessControl); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureAppearance); shouldUpdate(initial, changed, enabled) {
			if enabled {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureControlSharedPorts); shouldUpdate(initial, changed, enabled) {
		...
		if initial, changed, enabled := featureChanged(nicloudsdk.FeatureWorkspacePrebuilds); shouldUpdate(initial, changed, enabled) {
		...
		featureExternalTokenEncryption := reloadedEntitlements.Features[nicloudsdk.FeatureExternalTokenEncryption]
		featureExternalTokenEncryption.Enabled = len(api.ExternalTokenEncryption) > 0
		if featureExternalTokenEncryption.Enabled && featureExternalTokenEncryption.Entitlement != nicloudsdk.EntitlementEntitled {
			msg := fmt.Sprintf("%s is enabled (due to setting external token encryption keys) but your license is not entitled to this feature.", nicloudsdk.FeatureExternalTokenEncryption.Humanize())
			api.Logger.Warn(ctx, msg)
			reloadedEntitlements.Warnings = append(reloadedEntitlements.Warnings, msg)
		}
		reloadedEntitlements.Features[nicloudsdk.FeatureExternalTokenEncryption] = featureExternalTokenEncryption
...
func (api *API) CheckBuildUsage(
	...
	if templateVersion.HasExternalAgent.Valid && templateVersion.HasExternalAgent.Bool {
		feature, ok := api.Entitlements.Feature(nicloudsdk.FeatureWorkspaceExternalAgent)
	...
	managedAgentLimit, ok := api.Entitlements.Feature(nicloudsdk.FeatureManagedAgentLimit)
...
// @Produce json
// @Tags Enterprise
// @Success 200 {object} nicloudsdk.Entitlements
...
func (api *API) runEntitlementsLoop(ctx context.Context) {
	...
	defer func() {
		...
		// nicloud is shutting down. In this case, post a pubsub message to
		// tell other nicloud's to resync their entitlements. This is required to
