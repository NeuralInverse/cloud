...
// enterprise/nicloud/scim. Once that implementation has run in production
// for a while and the NEURALINVERSE_SCIM_USE_LEGACY default is flipped, remove
// this package in its entirety.
//
// Enabled via the UseLegacySCIM option.
//
// Deprecated: Use the enterprise/nicloud/scim package instead.
package legacyscim
...
func (s *LegacyServer) Handler() http.Handler {
	...
	r.NotFound(func(w http.ResponseWriter, r *http.Request) {
		u := r.URL.String()
		httpapi.Write(r.Context(), w, http.StatusNotFound, nicloudsdk.Response{
...
func (s *LegacyServer) scimServiceProviderConfig(rw http.ResponseWriter, _ *http.Request) {
	...
	_ = enc.Encode(ServiceProviderConfig{
		...
		DocURI:  "https://cloud.neuralinverse.com/docs/admin/users/oidc-auth#scim",
		Patch: Supported{
			Supported: true,
		},
		Bulk: BulkSupported{
...
func (s *LegacyServer) scimPostUser(rw http.ResponseWriter, r *http.Request) {
	...
	usernameValid := nicloudsdk.NameValid(sUser.UserName)
	if usernameValid != nil {
		...
		sUser.UserName = nicloudsdk.UsernameFrom(sUser.UserName)
	}
...
func (s *LegacyServer) scimPutUser(rw http.ResponseWriter, r *http.Request) {
	...
	// TODO: Currently ignoring a lot of the SCIM fields. Neural Inverse Cloud's SCIM implementation
