...
// @Param request body nicloudsdk.CustomRoleRequest true "Insert role request"
// @Tags Members
// @Success 200 {array} nicloudsdk.Role
...
func (api *API) postOrgRoles(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CustomRoleRequest
...
// @Produce json
// @Param organization path string true "Organization ID" format(uuid)
// @Param request body nicloudsdk.CustomRoleRequest true "Update role request"
// @Tags Members
// @Success 200 {array} nicloudsdk.Role
...
func (api *API) putOrgRoles(rw http.ResponseWriter, r *http.Request) {
	...
	updated, err := db.UpdateCustomRole(ctx, database.UpdateCustomRoleParams{
		...
		// now being invalid has to be addressed. Neural Inverse Cloud can change permissions,
...
func (api *API) deleteOrgRole(rw http.ResponseWriter, r *http.Request) {
	...
	if !validOrganizationRoleRequest(ctx, nicloudsdk.CustomRoleRequest{Name: rolename}, rw) {
		return
	}
...
func filterInvalidPermissions(permissions []nicloudsdk.Permission) []nicloudsdk.Permission {
	// Filter out any invalid permissions
	var validPermissions []nicloudsdk.Permission
...
func sdkPermissionToDB(p nicloudsdk.Permission) database.CustomRolePermission {
...
func validOrganizationRoleRequest(ctx context.Context, req nicloudsdk.CustomRoleRequest, rw http.ResponseWriter) bool {
	...
	if err := nicloudsdk.NameValid(req.Name); err != nil {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
