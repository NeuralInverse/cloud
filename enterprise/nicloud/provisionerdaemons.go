...
func (api *API) provisionerDaemonsEnabledMW(next http.Handler) http.Handler {
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		if !api.Entitlements.Enabled(nicloudsdk.FeatureExternalProvisionerDaemons) {
...
func (p *provisionerDaemonAuth) authorize(r *http.Request, org database.Organization, tags map[string]string) (provisiionerDaemonAuthResponse, error) {
	...
	if provAuth {
		...
		pskKey, err := uuid.Parse(nicloudsdk.ProvisionerKeyIDPSK)
		if err != nil {
	...
	userKey, err := uuid.Parse(nicloudsdk.ProvisionerKeyIDUserAuth)
	if err != nil {
...
func (api *API) provisionerDaemonServe(rw http.ResponseWriter, r *http.Request) {
	...
	provisionersMap := map[nicloudsdk.ProvisionerType]struct{}{}
	for _, provisioner := range r.URL.Query()["provisioner"] {
		switch provisioner {
		case string(nicloudsdk.ProvisionerTypeEcho):
			provisionersMap[nicloudsdk.ProvisionerTypeEcho] = struct{}{}
		case string(nicloudsdk.ProvisionerTypeTerraform):
			provisionersMap[nicloudsdk.ProvisionerTypeTerraform] = struct{}{}
		default:
			httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
	...
	authCtx := ctx
	if r.Header.Get(nicloudsdk.ProvisionerDaemonPSK) != "" || r.Header.Get(nicloudsdk.ProvisionerDaemonKey) != "" {
		//nolint:gocritic // PSK auth means no actor in request,
		// so use system restricted.
		authCtx = dbauthz.AsSystemRestricted(ctx)
	}

	versionHdrVal := r.Header.Get(nicloudsdk.BuildVersionHeader)
