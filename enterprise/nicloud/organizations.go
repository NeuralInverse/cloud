...
// @Param request body nicloudsdk.UpdateOrganizationRequest true "Patch organization request"
...
func (api *API) patchOrganization(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.UpdateOrganizationRequest
	...
	// use "me" for users).
	if req.Name == nicloudsdk.DefaultOrganization {
		httpapi.Write(ctx, rw, http.StatusBadRequest, nicloudsdk.Response{
			Message: fmt.Sprintf("Organization name %q is reserved.", nicloudsdk.DefaultOrganization),
		})
		return
	}
...
// @Produce json
// @Tags Organizations
// @Param request body nicloudsdk.CreateOrganizationRequest true "Create organization request"
// @Success 201 {object} nicloudsdk.Organization
...
func (api *API) postOrganizations(rw http.ResponseWriter, r *http.Request) {
	...
	var req nicloudsdk.CreateOrganizationRequest
	if !httpapi.Read(ctx, rw, r, &req) {
		return
	}
	...
	err = api.Database.InTx(func(tx database.Store) error {
		...
		// system role across nicloud instances (e.g. during rolling
		// restarts).
