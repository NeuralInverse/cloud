...
All Neural Inverse Cloud services log to standard output, which can be critical for identifying
errors and monitoring Neural Inverse Cloud's deployment health. Like any service, logs can be
captured via Splunk, Datadog, Grafana Loki, or other ingestion tools.

## `nicloud` Logs

By default, the Neural Inverse Cloud server exports human-readable logs to standard output. You
can access these logs via `kubectl logs deployment/coder -n <coder-namespace>`
on Kubernetes or `journalctl -u coder` if you deployed Neural Inverse Cloud on a host
machine/VM.

- To change the log format/location, you can set
  [`NEURALINVERSE_LOGGING_HUMAN`](../../reference/cli/server.md#--log-human) and
  [`NEURALINVERSE_LOGGING_JSON`](../../reference/cli/server.md#--log-json) server config.
  options.
- To only display certain types of logs, use
  the[`NEURALINVERSE_LOG_FILTER`](../../reference/cli/server.md#-l---log-filter) server
  config. Using `.*` will result in the `DEBUG` log level being used.

> [!NOTE]
> To disable human-readable logging, set `--log-human` (or
> `NEURALINVERSE_LOGGING_HUMAN`) to `/dev/null`. An empty string does not disable
> logging.

Events such as server errors, audit logs, user activities, and SSO & OpenID
Connect logs are all captured in the `nicloud` logs.
...
to `nicloud` logs. Use these logs to troubleshoot and monitor the Terraform
operations behind workspaces and templates.

## Workspace Logs

The [Neural Inverse Cloud agent](../infrastructure/architecture.md#agents) inside workspaces
...
- Windows: Refer to the template code (e.g.
  [azure-windows](https://github.com/NeuralInverse/cloud/blob/2cfadad023cb7f4f85710cff0b21ac46bdb5a845/examples/templates/azure-windows/Initialize.ps1.tftpl#L64))
...
events to the Neural Inverse Cloud UI.

![Kubernetes logs in Neural Inverse Cloud dashboard](../../images/admin/monitoring/logstream-kube.png)
