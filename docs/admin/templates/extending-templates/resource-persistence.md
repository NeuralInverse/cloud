...
By default, all Neural Inverse Cloud resources are persistent, but production templates
**must** use the practices laid out in this document to prevent accidental
deletion.

Neural Inverse Cloud templates have full control over workspace ephemerality. In a completely
...
The Terraform
[`ni_workspace` data source](https://registry.terraform.io/providers/coder/coder/latest/docs/data-sources/workspace)
...
In this example, Neural Inverse Cloud will provision or tear down the `docker_container`
resource:
...
resource "docker_container" "workspace" {
  ...
  count = data.ni_workspace.me.start_count # 0 (stopped), 1 (started)
  # ... other config
}
```
...
resource "docker_volume" "home_volume" {
  name = "coder-${data.ni_workspace.me.owner}-home"
}
```

Because we depend on `ni_workspace.me.owner`, if the owner changes their
username, Terraform will recreate the volume (wiping its data!) the next time
that Neural Inverse Cloud starts the workspace.

To prevent this, use immutable IDs:

- `ni_workspace.me.owner_id`
- `ni_workspace.me.id`

You should also avoid using `ni_workspace.me.name` if your deployment allows workspace renaming via `NEURALINVERSE_ALLOW_WORKSPACE_RENAMES` or `--allow-workspace-renames`.
...
resource "docker_volume" "home_volume" {
  ...
  name = "coder-${data.ni_workspace.id}-home"
}
```
