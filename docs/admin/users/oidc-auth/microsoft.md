...
This guide shows how to configure Neural Inverse Cloud to authenticate users with Microsoft Entra ID using OpenID Connect (OIDC)
...
9. Now, click on the tab labeled "Authentication" and click on "Add a platform", select "Web" and for the redirect URI enter your Neural Inverse Cloud callback URL, and then hit "Configure" at the bottom:
   - `https://coder.example.com/api/v2/users/oidc/callback`

## Step 2: Configure Neural Inverse Cloud OIDC for Microsoft Entra ID

Set the following environment variables on your Neural Inverse Cloud deployment and restart Neural Inverse Cloud:

```env
NEURALINVERSE_OIDC_ISSUER_URL=https://login.microsoftonline.com/{tenant-id}/v2.0 # Replace {tenant-id} with your Azure tenant ID
NEURALINVERSE_OIDC_CLIENT_ID=<client id, located in "Overview">
NEURALINVERSE_OIDC_CLIENT_SECRET=<client secret, saved from step 6>
# Restrict to one or more email domains (comma-separated)
NEURALINVERSE_OIDC_EMAIL_DOMAIN="example.com"
NEURALINVERSE_OIDC_EMAIL_FIELD="upn" # This is set because EntraID typically uses .onmicrosoft.com domains by default, this should pull the user's username@domain email.
NEURALINVERSE_OIDC_GROUP_FIELD="groups" # This is for group sync / IdP Sync, a premium feature.
# Optional: customize the login button
NEURALINVERSE_OIDC_SIGN_IN_TEXT="Sign in with Microsoft Entra ID"
NEURALINVERSE_OIDC_ICON_URL=/icon/microsoft.svg
```
