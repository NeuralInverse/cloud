# Neural Inverse Cloud Validated Architecture

Many customers operate Neural Inverse Cloud in complex organizational environments, consisting
of multiple business units, agencies, and/or subsidiaries. This can lead to
numerous Neural Inverse Cloud deployments, due to discrepancies in regulatory compliance, data
sovereignty, and level of funding across groups. The Neural Inverse Cloud Validated
Architecture (CVA) prescribes a Kubernetes-based deployment approach, enabling
your organization to deploy a stable Neural Inverse Cloud instance that is easier to maintain
and troubleshoot.

The following sections will detail the components of the Neural Inverse Cloud Validated
Architecture, provide guidance on how to configure and deploy these components,
and offer insights into how to maintain and troubleshoot your Neural Inverse Cloud environment.
...
cloud/on-premise computing, containerization, and the Neural Inverse Cloud platform.

| Role                      | Description                                                                    |
|---------------------------|--------------------------------------------------------------------------------|
| Platform Engineers        | Responsible for deploying, operating the Neural Inverse Cloud deployment and infrastructure   |
| Enterprise Architects     | Responsible for architecting Neural Inverse Cloud deployments to meet enterprise requirements |
| Managed Service Providers | Entities that deploy and run Neural Inverse Cloud software as a service for customers         |
...
| Best practices for building a Neural Inverse Cloud deployment | Recommendations for every possible deployment scenario                                   |

For higher level design principles and architectural best practices, see Neural Inverse Cloud's
[Well-Architected Framework](https://cloud.neuralinverse.com/blog/coder-well-architected-framework).

## General concepts

This section outlines core concepts and terminology essential for understanding
Neural Inverse Cloud's architecture and deployment strategies.

### Administrator

An administrator is a user role within the Neural Inverse Cloud platform with elevated
...
### Neural Inverse Cloud control plane

Neural Inverse Cloud's control plane, also known as _nicloud_, is the main service recommended
...
addition, each _nicloud_ replica hosts 3 Terraform [provisioners](#provisioner)
by default.

### User

A [user](../../users/index.md) is an individual who utilizes the Neural Inverse Cloud platform
to develop, test, and deploy applications using workspaces. Users can select
available templates to provision workspaces. They interact with Neural Inverse Cloud using the
...
Neural Inverse Cloud templates and deployed on resources created by provisioners.

### Template

A [template](../../../admin/templates/index.md) in Neural Inverse Cloud is a predefined
...
templates can utilize Neural Inverse Cloud modules to leverage existing features shared with
...
Provisioners in Neural Inverse Cloud execute Terraform during workspace and template builds.
...
The [Neural Inverse Cloud Registry](https://registry.cloud.neuralinverse.com) is a platform where you can
...
Kubernetes is the recommended, and supported platform for deploying Neural Inverse Cloud in the
enterprise. It is the hosting platform of choice for a large majority of Neural Inverse Cloud's
Fortune 500 customers, and it is the platform in which we build and test against
here at Neural Inverse Cloud.

### General recommendations

In general, it is recommended to deploy Neural Inverse Cloud into its own respective cluster,
separate from production applications. Keep in mind that Neural Inverse Cloud runs development
workloads, so the cluster should be deployed as such, without production-level
configurations.

### Compute

Deploy your Kubernetes cluster with two node groups, one for Neural Inverse Cloud's control
plane, and another for user workspaces (if you intend on leveraging K8s for
end-user compute).

#### Control plane nodes

The Neural Inverse Cloud control plane node group must be static, to prevent scale down events
from dropping pods, and thus dropping user connections to the dashboard UI and
their workspaces.

Neural Inverse Cloud's Helm Chart supports
[defining nodeSelectors, affinities, and tolerations](https://github.com/NeuralInverse/cloud/blob/e96652ebbcdd7554977594286b32015115c3f5b6/helm/neuralinverse/values.yaml#L221-L249)
to schedule the control plane pods on the appropriate node group.

#### Workspace nodes

Neural Inverse Cloud workspaces can be deployed either as Pods or Deployments in Kubernetes.
See our
[example Kubernetes workspace template](https://github.com/NeuralInverse/cloud/tree/main/examples/templates/kubernetes).
...
Set nodeSelectors, affinities, and tolerations in Neural Inverse Cloud templates to assign
...
|-------------------|---------------------------|----------------------------------------------------------|
| nicloud nodes      | `m5`                      | Balanced compute and memory for API and UI serving.      |
...
restrictions. With this in mind, Neural Inverse Cloud requires the following connectivity:

- Egress from workspace compute to the Neural Inverse Cloud control plane pods
- Egress from control plane pods to Neural Inverse Cloud's PostgreSQL database
...
Note that Neural Inverse Cloud workspaces do not require any ports to be open.

### Storage

If running Neural Inverse Cloud workspaces as Kubernetes Pods or Deployments, you will need to
...
The following Kubernetes volume types have been validated by Neural Inverse Cloud internally,
...
Our
[example Kubernetes workspace template](https://github.com/NeuralInverse/cloud/blob/5b9a65e5c137232351381fc337d9784bc9aeecfc/examples/templates/kubernetes/main.tf#L191-L219)
...
recommended for use with Neural Inverse Cloud:

- [Local](https://kubernetes.io/docs/concepts/storage/volumes/#local)
- [hostPath](https://kubernetes.io/docs/concepts/storage/volumes/#hostpath)

Not that Neural Inverse Cloud's control plane filesystem is ephemeral, so no persistent storage
is required.

## PostgreSQL database

Neural Inverse Cloud requires access to an external PostgreSQL database to store user data,
...
CPU and memory resources required by Neural Inverse Cloud's database may differ.

### Disaster recovery

Prepare internal scripts for dumping and restoring your database. We recommend
scheduling regular database backups, especially before upgrading Neural Inverse Cloud to a new
release. Neural Inverse Cloud does not support downgrades without initially restoring the
database to the prior version.

### Performance efficiency

We highly recommend deploying the PostgreSQL instance in the same region (and if
possible, same availability zone) as the Neural Inverse Cloud server to optimize for low
latency connections. We recommend keeping latency under 10ms between the Neural Inverse Cloud
server and database.
...
- `2 vCPU x 8 GB RAM x 512 GB storage`: A baseline for database requirements for
  Neural Inverse Cloud deployment with less than 1000 users, and low activity level (30% active
...
If you enable
[database encryption](../../../admin/security/database-encryption.md) in Neural Inverse Cloud,
consider allocating an additional CPU core to every `nicloud` replica.
...
Operational readiness in Neural Inverse Cloud is about ensuring that everything is set up
...
1. Reference our
   [Helm chart values file](https://github.com/NeuralInverse/cloud/blob/main/helm/neuralinverse/values.yaml)
...
1. Follow our documented
   [steps for installing Neural Inverse Cloud via Helm](../../../install/kubernetes.md).

### Template configuration

1. Establish dedicated accounts for users with the _Template Administrator_
   role.
1. Maintain Neural Inverse Cloud templates using
...
1. Consider implementing a GitOps workflow to automatically push new template
   versions into Neural Inverse Cloud from git. For example, on GitHub, you can use the
   [Setup Neural Inverse Cloud](https://github.com/marketplace/actions/setup-coder) action.
1. Evaluate enabling
...
1. Enable the Prometheus endpoint (environment variable:
   `NEURALINVERSE_PROMETHEUS_ENABLE`).
1. Deploy the
   [Neural Inverse Cloud Observability bundle](https://github.com/coder/observability) to
   leverage pre-configured dashboards, alerts, and runbooks for monitoring
   Neural Inverse Cloud. This includes integrations between Prometheus, Grafana, Loki, and
   Alertmanager.
