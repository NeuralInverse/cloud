...
Neural Inverse Cloud's AI agent interacts with workspaces over the same
...
When a developer opens a web terminal in the Neural Inverse Cloud dashboard, connects via
...
This is unchanged from how workspaces already operate in Neural Inverse Cloud. Enabling
Neural Inverse Cloud Agents does not change your workspace network requirements.
...
`.mcp.json` files. The agent has no direct access to the Neural Inverse Cloud API beyond
...
| Git authentication  | Control plane | Uses existing Neural Inverse Cloud external auth (GitHub, GitLab, Bitbucket).            |
...
already configured in your Neural Inverse Cloud deployment.
...
through the Neural Inverse Cloud dashboard or CLI, the agent cannot access it either. There
...
The control plane overhead for Neural Inverse Cloud Agents is minimal. The heavy computation
happens elsewhere:
