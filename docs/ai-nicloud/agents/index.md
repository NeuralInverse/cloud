...
Neural Inverse Cloud Agents is a chat interface and API for delegating development work and research to coding agents in your Neural Inverse Cloud deployment. Developers describe the work they want done, and Neural Inverse Cloud Agents handles selecting a template, provisioning a workspace, and executing the task.

Neural Inverse Cloud Agents includes its own self-hosted AI coding
agent that runs the agent loop directly within the Neural Inverse Cloud control plane.
...
## What Neural Inverse Cloud Agents is and isn't
...
Neural Inverse Cloud Agents is not a replacement for your text editor or IDE. It is the
...
## Who Neural Inverse Cloud Agents is for

Neural Inverse Cloud Agents is designed for organizations that need to self-host their AI
...
- **Organizations with existing Neural Inverse Cloud deployments** that want to add agentic
  capabilities using their current templates, workspaces, and identity
  providers rather than adopting a separate SaaS product.

Neural Inverse Cloud Agents runs entirely self-hosted. There is no SaaS or managed component — the agent
loop, chat history, and all tool execution happen within your Neural Inverse Cloud deployment.
...
1. Executes tool calls by connecting to a Neural Inverse Cloud workspace over the existing
...
Neural Inverse Cloud Agents supports sub-agent delegation. The root agent can spawn child
...
All chat state is stored in the Neural Inverse Cloud database, not in the workspace. If a
...
Neural Inverse Cloud Agents eliminates this by moving the agent loop out of the workspace
entirely:
...
- **No privilege escalation.** The agent operates with the exact same
  ...
  access a template, workspace, or resource through the Neural Inverse Cloud dashboard,
...
Neural Inverse Cloud Agents works with any LLM provider. Administrators configure providers
and models from the Neural Inverse Cloud dashboard or API. Supported providers include:
...
## Comparison to Neural Inverse Cloud Tasks

Neural Inverse Cloud Agents is a new approach that differs from
[Neural Inverse Cloud Tasks](../tasks.md) in several ways:

| Aspect              | Neural Inverse Cloud Agents                         | Neural Inverse Cloud Tasks                                                    |
|---------------------|--------------------------------------|----------------------------------------------------------------|
...
Neural Inverse Cloud Agents is in Beta. The feature is under active development and
