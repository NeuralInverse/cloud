...
> [!NOTE]
> The Chats API is experimental in current Neural Inverse Cloud releases. Endpoints live under `/api/experimental/chats` and may change without notice until the feature graduates to GA.

## When to migrate

Neural Inverse Cloud Tasks is being deprecated. Support continues on the ESR release and
through Neural Inverse Cloud v2.36. See the deprecation notice on the [Neural Inverse Cloud Tasks](../tasks.md) page for the full timeline.

If you currently run workflows on the Tasks API, you should plan to
migrate to the Chats API and [Neural Inverse Cloud Agents](./index.md). Neural Inverse Cloud Agents
runs the agent loop in the Neural Inverse Cloud control plane rather than inside the
...
| Chat state             | Stored in the workspace (AgentAPI state file)                                    | Persisted in the Neural Inverse Cloud database                            |
...
variables (e.g. `ANTHROPIC_API_KEY`). With Neural Inverse Cloud Agents, credentials are
...
You no longer pass API keys in template variables or workspace environment. See https://cloud.neuralinverse.com/docs/ai-nicloud/agents/getting-started for more information.
...
defined in `nicloudsdk.ChatStatus`:
...
+     - name: Neural Inverse Cloud Create Agent Chat
...
- LLM credentials are no longer passed through the template. They are
  configured in the Neural Inverse Cloud control plane.
- Identify the user with `github-user-id` (the action resolves it to a
  Neural Inverse Cloud user via the GitHub OAuth link) or with `coder-username`
  directly.
...
<!-- NEEDS REVIEW: This section contains initial recommendations based on
     the current Neural Inverse Cloud Agents architecture. Platform teams should validate
     these against their own deployment patterns before adopting them. -->

> [!NOTE]
> This section contains recommendations that may evolve as Neural Inverse Cloud Agents
> matures. Review these against your deployment requirements.

With Neural Inverse Cloud Tasks, every task-capable template requires specific Terraform
resources (`coder_ai_task`, `coder_task`, agent modules, and LLM API
keys). With Neural Inverse Cloud Agents, templates no longer need any of these. The
...
- **Network boundaries.** Agent workspaces inherit whatever network access
  ...
  agent templates to only reach the Neural Inverse Cloud control plane and your git
...
If you are converting an existing task template for use with Neural Inverse Cloud Agents,
...
+   # The Neural Inverse Cloud Agents control plane handles the agent loop.
  }
```
...
curl -s https://coder.example.com/api/experimental/chats/models \
  -H "Neural Inverse Cloud-Session-Token: $NEURALINVERSE_SESSION_TOKEN" | jq '.[].display_name'
```
...
```sh
websocat -H "Neural Inverse Cloud-Session-Token: $NEURALINVERSE_SESSION_TOKEN" \
...
standard Neural Inverse Cloud UI, then attach it to a new chat from the chat composer:

1. In the Neural Inverse Cloud dashboard, create a workspace from the agent template
   you migrated.
...
curl -s "https://coder.example.com/api/experimental/chats/$CHAT_ID" \
  -H "Neural Inverse Cloud-Session-Token: $NEURALINVERSE_SESSION_TOKEN" | jq '{workspace_id, status}'
```
...
```sh
curl -s "https://coder.example.com/api/experimental/chats/$CHAT_ID" \
  -H "Neural Inverse Cloud-Session-Token: $NEURALINVERSE_SESSION_TOKEN" | jq '.status'
```
...
The Tasks CLI (`coder task`) remains separate from the Neural Inverse Cloud Agents Chats API.
Neural Inverse Cloud no longer ships an interactive Neural Inverse Cloud Agents TUI. Use the web UI for
