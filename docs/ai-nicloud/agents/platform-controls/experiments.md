...
- The [portabledesktop](https://registry.cloud.neuralinverse.com/modules/coder/portabledesktop)
  module installed in the workspace template.
- An API key for the selected provider configured under the **Providers**
  tab.

The Anthropic and OpenAI computer-use models are fixed by Neural Inverse Cloud per provider
...
1. **Deployment override.** Setting `NEURALINVERSE_CHAT_DEBUG_LOGGING_ENABLED=true`
...
curl -fsS \
  ...
  "$NEURALINVERSE_URL/api/experimental/chats/$CHAT_ID/debug/runs/$RUN_ID" \
  | jq . > "coder-agents-debug-run-$RUN_ID.json"
```

Fetch every run returned by the list endpoint and save a chat-level export.
Using the same `NEURALINVERSE_URL`, `NEURALINVERSE_SESSION_TOKEN`, and `CHAT_ID` variables
from above:

```sh
RUN_IDS=$(curl -fsS \
  -H "Neural Inverse Cloud-Session-Token: $NEURALINVERSE_SESSION_TOKEN" \
  "$NEURALINVERSE_URL/api/experimental/chats/$CHAT_ID/debug/runs" \
  | jq -r '.[].id') || {
