...
Not every chat with Neural Inverse Cloud Agents requires a workspace. A workspace is only provisioned when the
...
and useful to Neural Inverse Cloud Agents.
...
metadata shown on the templates page in the Neural Inverse Cloud dashboard, sorted by number
...
- **The Neural Inverse Cloud control plane** — required for the workspace daemon to function.
...
access a resource through the Neural Inverse Cloud dashboard, the agent cannot access it
either.
...
- **Configure external auth at the template level.** Use Neural Inverse Cloud's
...
existing git authentication configured in your Neural Inverse Cloud deployment. There is no
shared bot account.
...
preset, Neural Inverse Cloud assigns an already-running prebuilt workspace instead of
provisioning from scratch. The agent can begin working immediately.

## Checklist

Use this as a quick reference when creating or updating templates for Neural Inverse Cloud
Agents:
