...
| `forward_coder_headers` | No       | When enabled, forwards Neural Inverse Cloud identity headers on every outgoing MCP request. See [Neural Inverse Cloud identity headers](#coder-identity-headers). |
...
Neural Inverse Cloud via OIDC. Users who logged in with password or GitHub will see
...
## Neural Inverse Cloud identity headers
...
| Header                 | Description                                                                                                  |
|------------------------|--------------------------------------------------------------------------------------------------------------|
| `X-Neural Inverse Cloud-Owner-Id`     | Neural Inverse Cloud user who owns the chat that issued the tool call.                                                      |
| `X-Neural Inverse Cloud-Chat-Id`      | Top-level (parent) chat ID. For root chats this is the chat's own ID; for subchats it is the parent chat ID. |
| `X-Neural Inverse Cloud-Subchat-Id`   | Subchat ID. Only present when the request originates from a child chat.                                      |
| `X-Neural Inverse Cloud-Workspace-Id` | Workspace associated with the chat, if any.                                                                  |

Neural Inverse Cloud sends the same identity headers to LLM providers, so a first-party
