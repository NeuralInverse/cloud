...
Neural Inverse Cloud Agents models from the **Agents** settings page. Providers, models, and
...
Neural Inverse Cloud supports the following provider types:
...
Neural Inverse Cloud Agents route model requests through AI Gateway automatically by using
the provider configuration stored in Neural Inverse Cloud's database.
...
implements the APIs Neural Inverse Cloud sends.

For the default Agents path through AI Gateway, set the endpoint/base URL to
the upstream provider or proxy endpoint. Do not set it to Neural Inverse Cloud's public AI
...
the endpoint/base URL because Neural Inverse Cloud appends request suffixes such as
...
Provider API keys entered in the dashboard are stored encrypted in the Neural Inverse Cloud
...
Neural Inverse Cloud Agents use the AI providers configured by administrators. Provider API
keys entered by administrators are centralized credentials for the deployment.

BYOK for Neural Inverse Cloud Agents is controlled by the
...
For each provider request, Neural Inverse Cloud selects credentials in this order:

1. If BYOK is enabled and the user has saved a personal key for the selected
   provider, Neural Inverse Cloud uses the user's key.
1. Otherwise, Neural Inverse Cloud uses centralized provider credentials when they are
   configured.
...
Beyond the chat-level model picker, Neural Inverse Cloud Agents supports two override
layers:
...
1. Navigate to the **Agents** page in the Neural Inverse Cloud dashboard.
...
1. Each enabled provider is listed with a status indicator:
   - **Key saved**, your personal key is active and will be used for requests to
     that provider.
   - **Using shared key**, no personal key is set and Neural Inverse Cloud is using
...
limiting, and audit logging while using Neural Inverse Cloud Agents as the developer interface.
