...
This guide walks platform teams and administrators through setting up Neural Inverse Cloud
Agents, preparing your deployment, and running your first Neural Inverse Cloud Agent.

> [!NOTE]
> Neural Inverse Cloud Agents is in Beta. APIs, behavior, and configuration may change
> between releases without notice; pin a release before broad rollout.
> Use **Neural Inverse Cloud version 2.33.1 or greater**.

## Prerequisites

Before you begin, confirm the following:

- **Neural Inverse Cloud deployment** running the latest release.
...
- **Admin access** to the Neural Inverse Cloud deployment for configuring providers.
- **Neural Inverse Cloud Agents User role** assigned to each user who needs to interact with Neural Inverse Cloud Agents.
  ...
  **Members**. See [Grant Neural Inverse Cloud Agents User](#step-2-grant-coder-agents-user)
  below.
...
> **Owner** role (Neural Inverse Cloud administrator). Non-admin users cannot access the
> admin Settings panel or modify deployment-level Agents configuration.

To configure Neural Inverse Cloud Agents:
...
## Step 2: Grant Neural Inverse Cloud Agents User

The **Neural Inverse Cloud Agents User** role controls which users can interact with Neural Inverse Cloud
...
1. Open **Admin settings** > **Organizations** in the Neural Inverse Cloud dashboard, then
...
1. Toggle on **Neural Inverse Cloud Agents User** and save.
...
You can also set the organization with the `NEURALINVERSE_ORGANIZATION` environment
variable instead of `-O`.

## Step 3: Start your first Neural Inverse Cloud Agent

1. Go to the **Agents** page in the Neural Inverse Cloud dashboard.
...
Neural Inverse Cloud Agents is under active development. APIs, behavior, and
...
Neural Inverse Cloud Agents use browser push notifications to alert you when a task
...
Administrators can set a system prompt that applies to all Neural Inverse Cloud Agents across the
...
The [Chats API](../../reference/api/chats.md) enables programmatic access to Neural Inverse Cloud Agents.
This is useful for building automations such as:

- Triggering Neural Inverse Cloud Agents from CI/CD pipelines when builds fail.
- Creating Neural Inverse Cloud Agents from GitHub webhooks on new issues or PRs.
- Building internal tools or dashboards on top of the API.
- Scripting batch operations across repositories.

**Quick example — create a Neural Inverse Cloud Agent via the API:**
...
for every conversation with a Neural Inverse Cloud Agent that uses that workspace.
...
Neural Inverse Cloud Agents is a collaborative evaluation between your team and Neural Inverse Cloud.
...
channel** with the Neural Inverse Cloud team.
