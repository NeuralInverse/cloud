...
Chat sharing lets you give other users or groups read-only access to a Neural Inverse Cloud Agents conversation.
...
Neural Inverse Cloud does not create a separate share link or notify recipients. They must open the chat from the URL you send them.
...
Administrators can disable chat sharing for a deployment with `--disable-chat-sharing`, `NEURALINVERSE_DISABLE_CHAT_SHARING`, or `disableChatSharing`. When disabled, only chat owners can access their chats.
