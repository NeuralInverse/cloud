...
Neural Inverse Cloud Workspaces already lets teams run AI tools like
[Cursor](https://registry.cloud.neuralinverse.com/modules/coder/cursor) and
[Claude Code](https://registry.cloud.neuralinverse.com/modules/coder/claude-code) inside their
...
> [!NOTE]
> As of Neural Inverse Cloud v2.32, the AI Governance Add-On is required to use AI Gateway and Agent Firewall.
> Deployments without the add-on cannot access these features.

## Who should use the AI Governance Add-On

The AI Governance Add-On is for teams that want to extend the Neural Inverse Cloud platform to
...
- Rolling out AI-powered IDEs like Cursor and AI coding agents like Claude Code
  across teams
- Looking to centrally observe, audit, and govern AI activity in Neural Inverse Cloud
  Workspaces
...
or vendor-managed policies, you can continue using them. Neural Inverse Cloud Workspaces can
...
authentication so users authenticate through Neural Inverse Cloud, eliminating the need to
...
Starting with Neural Inverse Cloud v2.30 (February 2026), AI Gateway and Agent Firewall are
...
[Neural Inverse Cloud account team](https://cloud.neuralinverse.com/contact/sales).

## How Neural Inverse Cloud Tasks usage is measured
...
The usage metric used to measure Neural Inverse Cloud Tasks consumption is called **Agent
Workspace Builds** (prev. "managed agents").

An Agent Workspace Build is counted each time a workspace is started
specifically for a coding agent to independently work on a Neural Inverse Cloud Task. Most of
the work in this workspace is performed by the agent, not a human developer.
Each Neural Inverse Cloud Task starts its own workspace, and the usage meter counts one Agent
Workspace Build.

Traditional Neural Inverse Cloud Workspaces started manually by developers or scheduled to
...
|---------------------------------------------------------------------------------------------------|--------------------------------|
| Developer creates a Neural Inverse Cloud Task to write end-to-end tests                                          | Yes                            |
| Automated pipeline creates a task via Neural Inverse Cloud Tasks CLI (with Claude Code) to review a pull request | Yes                            |
| Developer resumes an old Neural Inverse Cloud Task order to continue prototyping                                 | Yes                            |
...
| Developer creates a workspace for use with Neural Inverse Cloud AI Gateway and Agent Firewall                    | No                             |

In the future, additional capabilities for managing agents (beyond Neural Inverse Cloud Tasks)
may also consume agent workspace builds.

### Agent Workspace Build Limits

Without proper controls and sandboxing, it is not recommended to open up Neural Inverse Cloud
...
proportionately with user count. Usage counts are measured and sent to Neural Inverse Cloud via
[usage data reporting](./usage-data-reporting.md). Neural Inverse Cloud Tasks and other AI
features continue to function normally even if the limit is breached. Admins
will receive a warning to [contact their account team](https://cloud.neuralinverse.com/contact)
to remediate.

### Tracking Agent Workspace Builds

Admins can monitor Agent Workspace Build usage from the Neural Inverse Cloud dashboard.
