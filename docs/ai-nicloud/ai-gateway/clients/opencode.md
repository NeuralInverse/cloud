...
To authenticate with AI Gateway, get your **[Neural Inverse Cloud API token](../../../admin/users/sessions-tokens.md#generate-a-long-lived-api-token-on-behalf-of-yourself)** and replace `<your-coder-api-token>` in `~/.local/share/opencode/auth.json`
...
Set the following in `~/.config/opencode/opencode.json`, including the `X-Neural Inverse Cloud-AI-Governance-Token` header with your Neural Inverse Cloud API token:
