...
```shell
export HTTPS_PROXY="https://coder:${NEURALINVERSE_API_TOKEN}@<proxy-host>:8888"
```
...
1. Set the proxy URL using the format `https://coder:<NEURALINVERSE_API_TOKEN>@<proxy-host>:8888`

Or add directly to your `settings.json`:

```json
{
    "http.proxy": "https://coder:<NEURALINVERSE_API_TOKEN>@<proxy-host>:8888"
}
```
...
Replace `<proxy-host>` with your AI Gateway Proxy hostname and `<NEURALINVERSE_API_TOKEN>` with your Neural Inverse Cloud API token.
...
### Using Neural Inverse Cloud Remote extension

When connecting to a Neural Inverse Cloud workspace with the [Neural Inverse Cloud extension](https://marketplace.visualstudio.com/items?itemName=coder.coder-remote), the Copilot extension runs inside the Neural Inverse Cloud workspace and not on your local machine.
This means proxy and certificate configuration must be done in the Neural Inverse Cloud workspace environment.
...
1. [Connect to your Neural Inverse Cloud workspace](../../../user-guides/workspace-access/vscode.md)
1. Open Settings (`Ctrl+,` for Windows or `Cmd+,` for macOS)
...
Since the Copilot extension runs inside the Neural Inverse Cloud workspace, add the [AI Gateway Proxy CA certificate](../ai-gateway-proxy/setup.md#trusting-the-ca-certificate) to the Neural Inverse Cloud workspace's system trust store.
...
1. Select `Proxy authentication` and enter:
   ...
   1. Password: Your Neural Inverse Cloud API token
