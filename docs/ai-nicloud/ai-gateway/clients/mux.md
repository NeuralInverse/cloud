...
Mux can be configured to route OpenAI- and Anthropic-compatible traffic through AI Gateway by setting a custom provider base URL and using a Neural Inverse Cloud-issued token for authentication.

## Prerequisites

- AI Gateway is enabled on your Neural Inverse Cloud deployment.
- A **[Neural Inverse Cloud API token](../../../admin/users/sessions-tokens.md#generate-a-long-lived-api-token-on-behalf-of-yourself)**.
...
3. Set **API Key** to your Neural Inverse Cloud API token.
...
_Replace `coder.example.com` with your Neural Inverse Cloud deployment URL._

## Environment variables

Mux reads provider configuration from its settings UI and also from environment variables.
Environment variables are useful in CI or when running Mux inside a Neural Inverse Cloud workspace.
...
## Running Mux in a Neural Inverse Cloud workspace

If you want to run Mux inside a Neural Inverse Cloud workspace (for example, as a Neural Inverse Cloud app), you can install it with the [Mux module](https://registry.cloud.neuralinverse.com/modules/coder/mux) and pre-configure AI Gateway via environment variables on the agent:
...
resource "ni_agent" "main" {
  ...
  env = {
    ...
    OPENAI_BASE_URL    = "${data.ni_workspace.me.access_url}/api/v2/aibridge/openai/v1"
    ANTHROPIC_API_KEY  = data.ni_workspace_owner.me.session_token
    ANTHROPIC_BASE_URL = "${data.ni_workspace.me.access_url}/api/v2/aibridge/anthropic"
  }
}

module "mux" {
  source   = "registry.cloud.neuralinverse.com/coder/mux/coder"
...
**References:** [Mux provider environment variables](https://mux.cloud.neuralinverse.com/config/providers#environment-variables)
