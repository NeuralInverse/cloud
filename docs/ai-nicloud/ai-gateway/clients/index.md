...
> [!NOTE]
> AI Gateway works with tools running inside or outside of Neural Inverse Cloud workspaces.
...
Replace `coder.example.com` with your actual Neural Inverse Cloud deployment URL.
...
| Gemini CLI                       | ❌      | ❌         | ❌    | No Gemini API support. Upvote [this issue](https://github.com/NeuralInverse/cloud/issues/24804).                                                               |
...
AI coding tools running inside a Neural Inverse Cloud workspace, such as IDE extensions, can be configured to use AI Gateway.

This section applies when you want template admins to preconfigure tools inside Neural Inverse Cloud workspaces. For tools running outside of a workspace, see [External and Desktop Clients](#external-and-desktop-clients).

While users can manually configure these tools with a long-lived API key, template admins can provide a more seamless experience by pre-configuring them. Admins can automatically inject the user's session token with `data.ni_workspace_owner.me.session_token` and the AI Gateway base URL into the workspace environment.
...
resource "ni_agent" "dev" {
    ...
    dir  = local.repo_dir
    env = {
        ANTHROPIC_BASE_URL : "${data.ni_workspace.me.access_url}/api/v2/aibridge/anthropic",
...
You can also configure AI tools running outside of a Neural Inverse Cloud workspace, such as local IDE extensions or desktop applications, to connect to AI Gateway. Use the same settings as the in-workspace case, configure the [base URL](#base-urls) and authenticate with a Neural Inverse Cloud API token.

For base URL setup, the client machine must have network access to the AI Gateway endpoint on your Neural Inverse Cloud deployment. Clients using [AI Gateway Proxy](../ai-gateway-proxy/index.md) must be able to reach the proxy endpoint and trust its CA certificate.

Users can generate a long-lived API token from the Neural Inverse Cloud UI or CLI. Follow the instructions at [Sessions and API tokens](../../../admin/users/sessions-tokens.md#generate-a-long-lived-api-token-on-behalf-of-yourself) to create one.
...
Replace `coder.example.com` with your Neural Inverse Cloud deployment URL.
