...
NEURALINVERSE_EXTERNAL_AUTH_0_CLIENT_ID=...
NEURALINVERSE_EXTERNAL_AUTH_0_CLIENT_SECRET=...
# Tell AI Gateway where it can find this service's remote MCP server.
NEURALINVERSE_EXTERNAL_AUTH_0_MCP_URL=https://api.githubcopilot.com/mcp/
```
...
```env
NEURALINVERSE_EXTERNAL_AUTH_0_MCP_TOOL_ALLOW_REGEX=(.+_gist.*)
NEURALINVERSE_EXTERNAL_AUTH_0_MCP_TOOL_DENY_REGEX=(create_gist)
```
...
If you have [Neural Inverse Cloud MCP Server](../mcp-server.md) enabled, as well as have `NEURALINVERSE_AI_GATEWAY_INJECT_NEURALINVERSE_MCP_TOOLS=true` set, Neural Inverse Cloud's MCP tools will be injected into intercepted requests.
...
- **Neural Inverse Cloud MCP tools not being injected**: in order for Neural Inverse Cloud MCP tools to be injected, the internal MCP server needs to be active. Follow the instructions in the [MCP Server](../mcp-server.md) page to enable it and ensure `NEURALINVERSE_AI_GATEWAY_INJECT_NEURALINVERSE_MCP_TOOLS` is set to `true`.
