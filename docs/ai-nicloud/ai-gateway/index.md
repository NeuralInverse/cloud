...
AI Gateway supports clients running inside or outside Neural Inverse Cloud workspaces.
...
1. **Centralized authn/z management**: no more issuing & managing API tokens for OpenAI/Anthropic usage.
   Users use their Neural Inverse Cloud session or API tokens to authenticate with `nicloud` (Neural Inverse Cloud control plane), and
   `nicloud` securely communicates with the upstream APIs on their behalf.
...
- [Set up AI Gateway](./setup.md) on your Neural Inverse Cloud deployment
