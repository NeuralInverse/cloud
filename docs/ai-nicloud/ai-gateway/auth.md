...
AI Gateway authenticates clients with the same Neural Inverse Cloud API token
that a user already uses against the rest of the Neural Inverse Cloud API.
No separate AI Gateway login or credential is required.

Authenticating with a Neural Inverse Cloud token avoids distributing provider-specific API keys
...
> [!NOTE]
> Only Neural Inverse Cloud-issued tokens can authenticate users to AI Gateway.
...
## Create a Neural Inverse Cloud API token

You can generate a token from the Neural Inverse Cloud dashboard or the CLI.
...
If you're logged in with the Neural Inverse Cloud CLI, you can retrieve your current session token
...
from the Neural Inverse Cloud dashboard.
...
The Neural Inverse Cloud token is supplied in the proxy URL:
...
- A **Neural Inverse Cloud API token** to authenticate with AI Gateway.
...
Neural Inverse Cloud Agents requests routed through AI Gateway are in-process control plane
requests, not external client requests that send their own AI Gateway bearer
token. Neural Inverse Cloud Agents use this same global BYOK setting. When BYOK is enabled,
...
Administrators can disable it using `--ai-gateway-allow-byok=false` or `NEURALINVERSE_AI_GATEWAY_ALLOW_BYOK=false`:
