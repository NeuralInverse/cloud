...
> `NEURALINVERSE_AI_GATEWAY_PROVIDER_<N>_*`, `NEURALINVERSE_AI_GATEWAY_OPENAI_*`,
> `NEURALINVERSE_AI_GATEWAY_ANTHROPIC_*`, and their `--aibridge/ai-gateway-*`
...
- **AWS SDK default credential chain (recommended).** When no explicit
  ...
  Attaching an IAM Role to the compute running Neural Inverse Cloud follows
...
restarting `nicloud`. AI Gateway records the timestamp of each reload
