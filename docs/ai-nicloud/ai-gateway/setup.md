...
AI Gateway runs inside the Neural Inverse Cloud control plane (`nicloud`), requiring no separate compute to deploy or scale. Once enabled, `nicloud` runs the `aibridged` in-memory and brokers traffic to your configured AI providers on behalf of authenticated users.
...
```sh
export NEURALINVERSE_AI_GATEWAY_ENABLED=true
coder server
# or
...
disable it without restarting `nicloud`.
...
`NEURALINVERSE_AI_GATEWAY_DUMP_DIR`:
...
Configure retention using `--ai-gateway-retention` or `NEURALINVERSE_AI_GATEWAY_RETENTION`:
...
Enable with `--ai-gateway-structured-logging` or `NEURALINVERSE_AI_GATEWAY_STRUCTURED_LOGGING`:
...
These logs are written to the same output stream as all other `nicloud` logs,
