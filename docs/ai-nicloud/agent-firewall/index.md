...
Agent Firewall also streams audit logs to Neural Inverse Cloud's control plane for centralized
monitoring of HTTP requests.

## Getting Started with Agent Firewall

The easiest way to use Agent Firewall is through existing Neural Inverse Cloud modules, such
as the
[Claude Code module](https://registry.cloud.neuralinverse.com/modules/coder/claude-code). It
...
```yaml
allowlist:
  - "domain=coder.example.com" # Required - use your Neural Inverse Cloud deployment domain
...
the
[Neural Inverse Cloud dogfood policy example](https://github.com/NeuralInverse/cloud/blob/main/dogfood/neuralinverse/boundary-config.yaml).
...
Agent Firewall streams audit logs to the Neural Inverse Cloud control plane, providing
...
> [!NOTE]
> Requires Neural Inverse Cloud v2.30+ and Agent Firewall v0.5.2+.
...
Agent Firewall audit logs are emitted as structured log entries from the Neural Inverse Cloud
...
```console
2026-01-16 00:11:40.564 [info]  nicloud.agentrpc: boundary_request owner=joe  workspace_name=some-task-c88d agent_name=dev  decision=allow  workspace_id=f2bd4e9f-7e27-49fc-961e-be4d1c2aa987  http_method=GET http_url=https://coder.example.com  event_time=2026-01-16T00:11:39.388607657Z  matched_rule=domain=coder.example.com request_id=9f30d667-1fc9-47ba-b9e5-8eac46e0abef trace=478b2b45577307c4fd1bcfc64fad6ffb span=9ece4bc70c311edb
```
