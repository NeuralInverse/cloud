# Guide: Create a GitHub to Neural Inverse Cloud Tasks Workflow
...
Neural Inverse Cloud Tasks provides a method for automating these repeatable workflows. With a Task, you can direct an agent like Claude Code to update your documentation or even diagnose and address a bug. By connecting GitHub to Neural Inverse Cloud Tasks, you can build out a GitHub workflow that will for example:

1. Trigger an automation to take a pre-existing issue
1. Automatically spin up a Neural Inverse Cloud Task with the context from that issue and direct an agent to work on it
...
This guide walks you through how to configure GitHub and Neural Inverse Cloud together so that you can tag Neural Inverse Cloud in a GitHub issue comment, and securely delegate work to coding agents in a Neural Inverse Cloud Task.

## Implementing the GHA

The below steps outline how to use the Neural Inverse Cloud [Create Task Action GHA](https://github.com/coder/create-task-action) in a GitHub workflow to solve a bug. The guide makes the following assumptions:

- You have access to a Neural Inverse Cloud Server that is running. If you don't have a Neural Inverse Cloud Server running, follow our [Quickstart Guide](https://cloud.neuralinverse.com/docs/tutorials/quickstart)
- Your Neural Inverse Cloud Server is accessible from GitHub
- You have an AI-enabled Task Template that can successfully create a Neural Inverse Cloud Task. If you don't have a Task Template available, follow our [Getting Started with Tasks Guide](https://cloud.neuralinverse.com/docs/ai-nicloud/tasks#getting-started-with-tasks)
- Check the [Requirements section of the GHA](https://github.com/coder/create-task-action?tab=readme-ov-file#requirements) for specific version requirements for your Neural Inverse Cloud deployment and the following
  - GitHub OAuth is configured in your Neural Inverse Cloud Deployment
  - Users have linked their GitHub account to Neural Inverse Cloud via `/settings/external-auth`

This guide can be followed for other use cases beyond bugs like updating documentation or implementing a small feature, but may require minor changes to file names and the prompts provided to the Neural Inverse Cloud Task.
...
```yaml
name: Start Neural Inverse Cloud Task
...
jobs:
  coder-create-task:
    ...
    steps:
      - name: Neural Inverse Cloud Create Task
        uses: coder/create-task-action@v0
        with:
          coder-url: ${{ secrets.NEURALINVERSE_URL }}
          coder-token: ${{ secrets.NEURALINVERSE_TOKEN }}
...
- Create a Neural Inverse Cloud Task when you apply the `coder` label to an existing GitHub issue
- Pass as a prompt to the Neural Inverse Cloud Task:
...
- `coder-url`: the URL of your Neural Inverse Cloud deployment, e.g. https://coder.example.com
- `coder-token`: follow our [API Tokens documentation](https://cloud.neuralinverse.com/docs/admin/users/sessions-tokens#long-lived-tokens-api-tokens) to generate a token. Note that the token must be an admin/org-level with the "Read users in organization" and "Create tasks for any user" permissions
...
data "coder_external_auth" "github" {
  id = "github" # Must match your NEURALINVERSE_EXTERNAL_AUTH_0_ID
}
...
For more information, see our [External Authentication documentation](https://cloud.neuralinverse.com/docs/admin/external-auth#configure-a-github-oauth-app).

### Step 3: Test Your Setup

Create a new GitHub issue for a bug in your codebase. We recommend a basic bug, for this test, like “The sidebar color needs to be red” or “The text ‘Neural Inverse Cloud Tasks are Awesome’ needs to appear in the top left corner of the screen”. You should adapt the phrasing to be specific to your codebase.
...
- A Neural Inverse Cloud Task will spin up, and you'll receive a Tasks notification to that effect
- You can click the link to follow the Task's progress in creating a plan to solve your bug

Depending on the complexity of the task and the size of your repository, the Neural Inverse Cloud Task may take minutes or hours to complete. Our recommendation is to rely on Task Notifications to know when the Task completes, and further action is required.
...
This guide shows you how to automatically delegate routine engineering work to AI coding agents by connecting GitHub issues to Neural Inverse Cloud Tasks. When you label an issue (like a bug report or documentation update), a coding agent spins up in a secure Neural Inverse Cloud workspace, reads the issue context, and works on solving it while you focus on higher-priority tasks. The agent reports back with a proposed solution for you to review and approve, turning hours of repetitive work into minutes of oversight. This same pattern can be adapted to handle documentation updates, test writing, code reviews, and other automatable workflows across your development process.

## Troubleshooting

### "No Neural Inverse Cloud user found with GitHub user ID X"

**Cause:** The user who triggered the workflow hasn't linked their GitHub account to Neural Inverse Cloud.

**Solution:**

1. Ensure GitHub OAuth is configured in your Neural Inverse Cloud deployment (see [External Authentication docs](https://cloud.neuralinverse.com/docs/admin/external-auth#configure-a-github-oauth-app))
...
Generate a new token with these permissions at `https://<your-coder-url>/deployment/general`. See the [Neural Inverse Cloud Create Task GHA requirements](https://github.com/coder/create-task-action?tab=readme-ov-file#requirements) for more specific information.
