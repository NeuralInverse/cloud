# Speed up your Neural Inverse Cloud templates and workspaces
...
changes to your Neural Inverse Cloud templates to help speed things up.

## Monitoring

You can monitor [Neural Inverse Cloud logs](../../admin/monitoring/logs.md) through the
...
pre-built set of dashboards to monitor your Neural Inverse Cloud deployments over time. It
...
### Enable Prometheus metrics for Neural Inverse Cloud

Neural Inverse Cloud exposes a variety of
[application metrics](../../admin/integrations/prometheus.md#available-metrics),
such as `nicloud_provisionerd_job_timings_seconds` and
`nicloud_agentstats_startup_script_seconds`, which measure how long the
...
Neural Inverse Cloud installation.

## Provisioners

`coder server` by default provides three built-in provisioner daemons
(controlled by the
[`NEURALINVERSE_PROVISIONER_DAEMONS`](../../reference/cli/server.md#--provisioner-daemons)
...
Provisioners are queue-based to reduce unpredictable load to the Neural Inverse Cloud server.
If you require a higher bandwidth of provisioner jobs, you can do so by
increasing the
[`NEURALINVERSE_PROVISIONER_DAEMONS`](../../reference/cli/server.md#--provisioner-daemons)
config option.

You risk overloading Neural Inverse Cloud if you use too many built-in provisioners, so we
recommend a maximum of five built-in provisioners per `nicloud` replica. For more
...
We recommend that you deploy Neural Inverse Cloud to its own respective Kubernetes cluster,
...
[Helm provisioner values.yaml](https://github.com/NeuralInverse/cloud/blob/main/helm/provisioner/values.yaml#L134-L141):
...
`terraform init` generates a `.terraform.lock.hcl` which instructs Neural Inverse Cloud
...
1. Push the templates back to your Neural Inverse Cloud deployment:

   ```shell
   coder templates push <template>
   ```

This bundles up your template and the lock file and uploads it to Neural Inverse Cloud. The
next time the template is used, Terraform will attempt to cache the specific
provider versions.

### Cache directory

Neural Inverse Cloud will instruct Terraform to cache its downloaded providers in the
configured [`NEURALINVERSE_CACHE_DIRECTORY`](../../reference/cli/server.md#--cache-dir)
directory.

Ensure that this directory is set to a location on disk which will persist
across restarts of Neural Inverse Cloud or
