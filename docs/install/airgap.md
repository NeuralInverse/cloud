...
All Neural Inverse Cloud features are supported in air-gapped / behind firewalls / disconnected / offline.
This is a general comparison. Keep reading for a full tutorial running Neural Inverse Cloud
air-gapped with Kubernetes or Docker.

|                           | Public deployments                                                                                                                                                                                                                                                 | Air-gapped deployments                                                                                                                                                                                                                                                                               |
|---------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Terraform binary          | By default, Neural Inverse Cloud downloads Terraform binary from [releases.hashicorp.com](https://releases.hashicorp.com)                                                                                                                                                         | Terraform binary must be included in `PATH` for the VM or container image. [Supported versions](https://github.com/NeuralInverse/cloud/blob/main/provisioner/terraform/install.go#L23-L24)                                                                                                                   |
| Terraform registry        | Neural Inverse Cloud templates will attempt to download providers from [registry.terraform.io](https://registry.terraform.io) or [custom source addresses](https://developer.hashicorp.com/terraform/language/providers/requirements#source-addresses) specified in each template | [Custom source addresses](https://developer.hashicorp.com/terraform/language/providers/requirements#source-addresses) can be specified in each Neural Inverse Cloud template, or a custom registry/mirror can be used. More details below                                                                           |
| STUN                      | By default, Neural Inverse Cloud uses Google's public STUN server for direct workspace connections                                                                                                                                                                                | STUN can be safely [disabled](../reference/cli/server.md#--derp-server-stun-addresses) users can still connect via [relayed connections](../admin/networking/index.md#-geo-distribution). Alternatively, you can set a [custom DERP server](../reference/cli/server.md#--derp-server-stun-addresses) |
| DERP                      | By default, Neural Inverse Cloud's built-in DERP relay can be used, or [Tailscale's public relays](../admin/networking/index.md#relayed-connections).                                                                                                                             | By default, Neural Inverse Cloud's built-in DERP relay can be used, or [custom relays](../admin/networking/index.md#custom-relays).                                                                                                                                                                                 |
| PostgreSQL                | If no [PostgreSQL connection URL](../reference/cli/server.md#--postgres-url) is specified, Neural Inverse Cloud will download Postgres from [repo1.maven.org](https://repo1.maven.org)                                                                                            | An external database is required, you must specify a [PostgreSQL connection URL](../reference/cli/server.md#--postgres-url)                                                                                                                                                                          |
...
| Update check              | By default, Neural Inverse Cloud checks for updates from [GitHub releases](https://github.com/NeuralInverse/cloud/releases)                                                                                                                                                               | Update checks [can be disabled](../reference/cli/server.md#--update-check)                                                                                                                                                                                                                           |
| License validation        | License keys are validated locally using cryptographic signatures. No outbound connection to Neural Inverse Cloud is required                                                                                                                                                     | No changes needed. See [offline license validation](../admin/licensing/index.md#offline-license-validation)                                                                                                                                                                                          |
| AI Governance Usage Count | By default, deployments with the [AI Governance Add On](../ai-coder/ai-governance.md) report usage data                                                                                                                                                            | [Contact us](https://cloud.neuralinverse.com/contact) to request a license with usage reporting off.                                                                                                                                                                                                               |

## Air-gapped container images

The following instructions walk you through how to build a custom Neural Inverse Cloud server
...
> [!NOTE]
> Neural Inverse Cloud includes the latest
> [supported version](https://github.com/NeuralInverse/cloud/blob/main/provisioner/terraform/install.go#L23-L24)
...
# Terraform is already included in the official Neural Inverse Cloud image.
# See https://github.com/NeuralInverse/cloud/blob/main/scripts/Dockerfile.base#L15
# If you need to install a different version of Terraform, you can do so here.
# The below step is optional if you wish to keep the existing version.
# See https://github.com/NeuralInverse/cloud/blob/main/provisioner/terraform/install.go#L23-L24
...
# to download the Terraform providers used in Neural Inverse Cloud templates.
...
ARG NEURALINVERSE_PROVIDER_VERSION=2.2.0
RUN echo "Adding coder/coder v${NEURALINVERSE_PROVIDER_VERSION}" \
    && mkdir -p coder/coder && cd coder/coder \
    && curl -LOs https://github.com/coder/terraform-provider-coder/releases/download/v${NEURALINVERSE_PROVIDER_VERSION}/terraform-provider-coder_${NEURALINVERSE_PROVIDER_VERSION}_linux_amd64.zip
...
> [!NOTE]
> If you are bundling Terraform providers into your Neural Inverse Cloud image, be sure the
...
documentation and modify the docker-compose file to specify your custom Neural Inverse Cloud
...
services:
  coder:
    ...
    # ...
  environment:
    NEURALINVERSE_TELEMETRY_ENABLE: "false" # Disable telemetry
    NEURALINVERSE_BLOCK_DIRECT: "true" # force SSH traffic through control plane's DERP proxy
    NEURALINVERSE_DERP_SERVER_STUN_ADDRESSES: "disable" # Only use relayed connections
    NEURALINVERSE_UPDATE_CHECK: "false" # Disable automatic update checks
  database:
...
command can be used to download the required plugins for a Neural Inverse Cloud template.
This can be uploaded into the `plugins` directory on your offline server.

### Kubernetes

We publish the Helm chart for download on
[GitHub Releases](https://github.com/NeuralInverse/cloud/releases/latest). Follow our
[Kubernetes](./kubernetes.md) documentation and modify the Helm values to
specify your custom Neural Inverse Cloud image.
...
coder:
  ...
  env:
    # Disable telemetry
    - name: "NEURALINVERSE_TELEMETRY_ENABLE"
      value: "false"
    # Disable automatic update checks
    - name: "NEURALINVERSE_UPDATE_CHECK"
      value: "false"
    # force SSH traffic through control plane's DERP proxy
    - name: NEURALINVERSE_BLOCK_DIRECT
      value: "true"
    # Only use relayed connections
    - name: "NEURALINVERSE_DERP_SERVER_STUN_ADDRESSES"
      value: "disable"
...
Neural Inverse Cloud also provides air-gapped documentation in case you want to host it on your
...
1. Go to the release page. In this case, we want to use the
   [latest version](https://github.com/NeuralInverse/cloud/releases/latest).
...
5. Set the [NEURALINVERSE_DOCS_URL](../reference/cli/server.md#--docs-url) environment
   variable to use the URL of your hosted docs. This way, the Neural Inverse Cloud UI will
   reference the documentation from your specified URL.

With these steps, you'll have the Neural Inverse Cloud documentation hosted on your server and
accessible for your team to use.

## Neural Inverse Cloud Modules

To use Neural Inverse Cloud modules in offline installations, you can either:

- [Mirror the Neural Inverse Cloud Registry with JFrog Artifactory](./registry-mirror-artifactory.md) (recommended)
...
In restricted internet networks, Neural Inverse Cloud may require connection to internet.
Ensure that the following web addresses are accessible from the machine where
Neural Inverse Cloud is installed.
...
- v2-licensor.cloud.neuralinverse.com (developing Neural Inverse Cloud in Neural Inverse Cloud)

## JetBrains IDEs

Gateway, JetBrains' remote development product that works with Neural Inverse Cloud,
...
(for connecting a local VS Code to a remote Neural Inverse Cloud workspace) requires that your
