# Install Neural Inverse Cloud on Kubernetes

You can install Neural Inverse Cloud on Kubernetes (K8s) using Helm. We run on most Kubernetes
...
Create a namespace for the Neural Inverse Cloud control plane. In this tutorial, we'll call it
`coder`.
...
Neural Inverse Cloud does not manage a database server for you. This is required for storing
data about your Neural Inverse Cloud deployment and resources.
...
## 4. Install Neural Inverse Cloud with Helm
...
```yaml
coder:
  # You can specify any environment variables you'd like to pass to Neural Inverse Cloud
  # here. Neural Inverse Cloud consumes environment variables listed in
  # `coder server --help`, and these environment variables are also passed
  # to the workspace provisioner (so you can consume them in your Terraform
  # templates for auth keys etc.).
  #
  # Please keep in mind that you should not set `NEURALINVERSE_HTTP_ADDRESS`,
  # `NEURALINVERSE_TLS_ENABLE`, `NEURALINVERSE_TLS_CERT_FILE` or `NEURALINVERSE_TLS_KEY_FILE` as
  ...
  env:
    ...
    - name: NEURALINVERSE_OAUTH2_GITHUB_DEFAULT_PROVIDER_ENABLE
      value: "false"

    # (Optional) For production deployments the access URL should be set.
    # If you're just trying Neural Inverse Cloud, access the dashboard via the service IP.
...
You can view our
[Helm README](https://github.com/NeuralInverse/cloud/blob/main/helm/neuralinverse#readme) for
details on the values that are available, or you can view the
[values.yaml](https://github.com/NeuralInverse/cloud/blob/main/helm/neuralinverse/values.yaml)
file directly.

We support two release channels: mainline and stable - read the
[Releases](./releases/index.md) page to learn more about which best suits your team.

- **Mainline** Neural Inverse Cloud release:
...
- **Stable** Neural Inverse Cloud release:
...
You can watch Neural Inverse Cloud start up by running `kubectl get pods -n coder`. Once Neural Inverse Cloud
has started, the `coder-*` pods should enter the `Running` state.

## 5. Log in to Neural Inverse Cloud 🎉
...
If you do not have a domain, you should set `NEURALINVERSE_ACCESS_URL` to this URL in
the Helm chart and upgrade Neural Inverse Cloud (see below). This allows workspaces to connect
to the proper Neural Inverse Cloud URL.

## Upgrading Neural Inverse Cloud via Helm

To upgrade Neural Inverse Cloud in the future or change values, you can run the following
command:
...
## Neural Inverse Cloud Observability Chart
...
1. **All container images must be sourced from an internal container registry.**
   ...
   - Control plane - To pull the control plane image from the appropriate
     registry,
     [update this Helm chart value](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/helm/neuralinverse/values.yaml#L43-L50).
   - Workspaces - To pull the workspace image from your registry,
     [update the Terraform template code here](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/examples/templates/kubernetes/main.tf#L271).
...
2. **All containers must run as non-root user**
   ...
   - Control plane - Our control plane pod
     [runs as non-root by default](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/helm/neuralinverse/values.yaml#L124-L127).
   - Workspaces - Workspace pod UID is
     [set in the Terraform template here](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/examples/templates/kubernetes/main.tf#L274-L276),
     and are not required to run as `root`.

3. **Containers cannot run privileged**

   - Neural Inverse Cloud's control plane does not run as privileged.
     [We disable](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/helm/neuralinverse/values.yaml#L141)
     `allowPrivilegeEscalation`
     [by default](https://github.com/NeuralInverse/cloud/blob/f57ce97b5aadd825ddb9a9a129bb823a3725252b/helm/neuralinverse/values.yaml#L141).
...
If you are deploying Neural Inverse Cloud on AWS EKS and service is set to `LoadBalancer`, AWS
...
By default, Neural Inverse Cloud will set the `externalTrafficPolicy` to `Cluster` which will
...
coder:
  env:
    - name: NEURALINVERSE_PROXY_TRUSTED_HEADERS
      value: X-Forwarded-For
    - name: NEURALINVERSE_PROXY_TRUSTED_ORIGINS
...
Follow our doc on
[how to deploy Neural Inverse Cloud on Azure with an Application Gateway](./kubernetes/kubernetes-azure-app-gateway.md)
for an example.

## Troubleshooting

You can view Neural Inverse Cloud's logs by getting the pod name from `kubectl get pods` and
...
Ensure you have an externally-reachable `NEURALINVERSE_ACCESS_URL` set in your helm
chart. If you do not have a domain set up, this should be the IP address of
Neural Inverse Cloud's LoadBalancer (`kubectl get svc -n coder`).
