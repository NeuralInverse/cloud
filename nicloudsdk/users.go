...
type ReducedUser struct {
	...
	// `nicloudsdk.UserPreferenceSettings` instead.
	ThemePreference string `json:"theme_preference,omitempty"`
}

// User represents a user in Neural Inverse Cloud.
type User struct {
...
func (c *Client) HasFirstUser(ctx context.Context) (bool, error) {
	...
	if res.StatusCode == http.StatusNotFound {
		// ensure we are talking to neuralinverse and not
		// some other service that returns 404
		v := res.Header.Get(BuildVersionHeader)
		if v == "" {
			return false, xerrors.Errorf("missing build version header, not a neuralinverse instance")
		}
...
// CreateFirstUser attempts to create the first user on a Neural Inverse Cloud deployment.
