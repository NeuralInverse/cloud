...
//     budget (nicloudsdk/drpcsdk.MaxMessageSize).
//
...
//     (NEURALINVERSE_*, PATH, HOME, ...), and template-defined env. Not
...
// 400. TestUserSecretLimits in nicloud/usersecrets_test.go exercises
...
var (
	...
	reservedEnvNames = map[string]struct{}{
		...
		// SSH session variables. The Neural Inverse Cloud agent sets
		...
		// Editor/pager. The Neural Inverse Cloud agent sets these so that git
	...
	// exact-name map. The CODER / NEURALINVERSE_* namespace is handled
	// separately with its own error message (see below).
	reservedEnvPrefixes = []string{
		// The Neural Inverse Cloud agent sets GIT_SSH_COMMAND, GIT_ASKPASS,
...
func UserSecretEnvNameValid(s string) error {
	...
	if upper == "CODER" || strings.HasPrefix(upper, "NEURALINVERSE_") {
		return xerrors.New("environment variable names starting with NEURALINVERSE_ are reserved for internal use")
	}
