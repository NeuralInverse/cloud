...
func TestNullTime_MarshalJSON(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tr := nicloudsdk.NewNullTime(tt.input.Time, tt.input.Valid)
...
func TestNullTime_UnmarshalJSON(t *testing.T) {
	...
	type request struct {
		Time nicloudsdk.NullTime `json:"time"`
	}

	tests := []struct {
		name    string
		data    string
		want    nicloudsdk.NullTime
		wantErr bool
	}{
		{
			name: "null",
		...
		},
		{
			...
			want: nicloudsdk.NewNullTime(t1, true),
		},
		{
			name:    "invalid time",
...
func TestNullTime_IsZero(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tr := nicloudsdk.NullTime{NullTime: tt.input}
