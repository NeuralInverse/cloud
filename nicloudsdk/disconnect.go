...
const (
	...
	// nicloud is shutting down and is closing connections as part of that
	...
	// stopped or deleted while the connection was open, so nicloud closed
	...
	// its coordination RPC to nicloud. The data plane (peer-to-peer or
...
const (
	...
	// extension, JetBrains plugin, Neural Inverse Cloud Desktop) closed the connection.
	...
	// DisconnectInitiatorServer means nicloud (or a workspace proxy)
...
const (
	...
	// between nicloud and the workspace agent (coordination RPC, DERP map
	...
	// client to nicloud (e.g. the CLI's WebSocket to the coordinator).
