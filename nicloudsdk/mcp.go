...
type MCPServerConfig struct {
	...
	// ForwardNIHeaders forwards the same Neural Inverse Cloud identity headers we
	// send to LLM providers (X-Coder-Owner-Id, X-Neural Inverse Cloud-Chat-Id, and the
	// optional X-Coder-Subchat-Id and X-Neural Inverse Cloud-Workspace-Id) to this
	// MCP server on every request. Off by default to avoid leaking
	// chat identity to third-party servers.
	ForwardNIHeaders bool      `json:"forward_coder_headers"`
...
type CreateMCPServerConfigRequest struct {
	...
	// ForwardNIHeaders, when true, forwards Neural Inverse Cloud identity
...
type UpdateMCPServerConfigRequest struct {
	...
	// ForwardNIHeaders, when set, updates whether Neural Inverse Cloud identity
	// headers are forwarded on every outgoing MCP request.
	ForwardNIHeaders *bool `json:"forward_coder_headers,omitempty"`
}
