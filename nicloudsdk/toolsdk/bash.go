...
func findWorkspaceAndAgent(ctx context.Context, client *nicloudsdk.Client, workspaceName string) (nicloudsdk.Workspace, nicloudsdk.WorkspaceAgent, error) {
	...
	if err != nil {
		return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, err
	}
	...
	if workspace.LatestBuild.Transition != nicloudsdk.WorkspaceTransitionStart {
		if workspace.LatestBuild.Transition == nicloudsdk.WorkspaceTransitionDelete {
			return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q is deleted", workspace.Name)
		}
		if workspace.LatestBuild.Job.Status == nicloudsdk.ProvisionerJobFailed {
			return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q is in failed state", workspace.Name)
		}
		if workspace.LatestBuild.Status != nicloudsdk.WorkspaceStatusStopped {
			return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("workspace must be started; was unable to autostart as the last build job is %q, expected %q",
				workspace.LatestBuild.Status, nicloudsdk.WorkspaceStatusStopped)
		}
		...
		if err != nil {
			return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("failed to start workspace: %w", err)
		}
		...
		if build.Job.CompletedAt == nil {
			...
			if err != nil {
				return nicloudsdk.Workspace{}, nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("failed to wait for build completion: %w", err)
			}
		}
...
func getWorkspaceAgent(workspace nicloudsdk.Workspace, agentName string) (nicloudsdk.WorkspaceAgent, error) {
	...
	if len(agents) == 0 {
		return nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("workspace %q has no agents", workspace.Name)
	}
	...
	if agentName != "" {
		...
		return nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("agent not found by name %q, available agents: %v", agentName, availableNames)
	}
	...
	return nicloudsdk.WorkspaceAgent{}, xerrors.Errorf("multiple agents found, please specify the agent name, available agents: %v", availableNames)
}
