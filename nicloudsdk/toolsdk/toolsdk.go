...
const (
	...
	ToolNameWorkspaceBash               = "ni_workspace_bash"
	ToolNameChatGPTSearch               = "search"
	ToolNameChatGPTFetch                = "fetch"
	ToolNameWorkspaceLS                 = "ni_workspace_ls"
	ToolNameWorkspaceReadFile           = "ni_workspace_read_file"
	ToolNameWorkspaceWriteFile          = "ni_workspace_write_file"
	ToolNameWorkspaceEditFile           = "ni_workspace_edit_file"
	ToolNameWorkspaceEditFiles          = "ni_workspace_edit_files"
	ToolNameWorkspacePortForward        = "ni_workspace_port_forward"
	ToolNameWorkspaceListApps           = "ni_workspace_list_apps"
...
func NewDeps(client *nicloudsdk.Client, opts ...func(*Deps)) (Deps, error) {
	d := Deps{
		niClient: client,
	}
	...
	if d.agentConnFn == nil && d.niClient != nil {
		workspaceClient := workspacesdk.New(d.niClient)
...
func openAgentConn(ctx context.Context, deps Deps, workspace string) (workspacesdk.AgentConn, error) {
	...
	if err := cliui.Agent(ctx, io.Discard, workspaceAgent.ID, cliui.AgentOptions{
		FetchInterval: 0,
		Fetch:         deps.niClient.WorkspaceAgent,
		FetchLogs:     deps.niClient.WorkspaceAgentLogsAfter,
...
type Tool[Arg, Ret any] struct {
	...
	// classification. Both the nicloud-hosted MCP server and the CLI MCP
...
// All is a list of all tools that can be used in the Neural Inverse Cloud CLI.
...
var ReportTask = Tool[ReportTaskArgs, nicloudsdk.Response]{
	Tool: aisdk.Tool{
...
`,
		Schema: aisdk.Schema{
			Properties: map[string]any{
				...
				"state": map[string]any{
					...
					"enum": []string{
						string(nicloudsdk.WorkspaceAppStatusStateWorking),
						string(nicloudsdk.WorkspaceAppStatusStateComplete),
						string(nicloudsdk.WorkspaceAppStatusStateIdle),
						string(nicloudsdk.WorkspaceAppStatusStateFailure),
					},
				},
			},
	...
	Handler: func(_ context.Context, deps Deps, args ReportTaskArgs) (nicloudsdk.Response, error) {
		if len(args.Summary) > 160 {
			return nicloudsdk.Response{}, xerrors.New("summary must be less than 160 characters")
		}
		// Check if task reporting is available to prevent nil pointer dereference
		if deps.report == nil {
			return nicloudsdk.Response{}, xerrors.New("task reporting not available. Please ensure a task reporter is configured.")
		}
		err := deps.report(args)
		if err != nil {
			return nicloudsdk.Response{}, err
		}
		return nicloudsdk.Response{
...
var GetWorkspace = Tool[GetWorkspaceArgs, nicloudsdk.Workspace]{
	Tool: aisdk.Tool{
...
This returns more data than list_workspaces to reduce token usage.`,
	...
	Handler: func(ctx context.Context, deps Deps, args GetWorkspaceArgs) (nicloudsdk.Workspace, error) {
		return deps.niClient.ResolveWorkspace(ctx, NormalizeWorkspaceInput(args.WorkspaceID))
	},
}
...
func richParametersFromMap(m map[string]string) []nicloudsdk.WorkspaceBuildParameter {
	if len(m) == 0 {
		return nil
	}
	out := make([]nicloudsdk.WorkspaceBuildParameter, 0, len(m))
	for k, v := range m {
		out = append(out, nicloudsdk.WorkspaceBuildParameter{Name: k, Value: v})
	}
	return out
}

var CreateWorkspace = Tool[CreateWorkspaceArgs, nicloudsdk.Workspace]{
	Tool: aisdk.Tool{
...
`,
	...
	MCPAnnotations: mcpMutationAnnotations,
	Handler: func(ctx context.Context, deps Deps, args CreateWorkspaceArgs) (nicloudsdk.Workspace, error) {
		...
		if (args.TemplateID == "") == (args.TemplateVersionID == "") {
			return nicloudsdk.Workspace{}, xerrors.New("exactly one of template_id or template_version_id must be provided")
		}
		var (
			tID  uuid.UUID
			tvID uuid.UUID
		...
		if args.TemplateID != "" {
			...
			if err != nil {
				return nicloudsdk.Workspace{}, xerrors.New("template_id must be a valid UUID")
			}
		}
		if args.TemplateVersionID != "" {
			tvID, err = uuid.Parse(args.TemplateVersionID)
			if err != nil {
				return nicloudsdk.Workspace{}, xerrors.New("template_version_id must be a valid UUID")
			}
		}
		...
		if args.TemplateVersionPresetID != "" {
			...
			if err != nil {
				return nicloudsdk.Workspace{}, xerrors.New("template_version_preset_id must be a valid UUID")
			}
		}
		if args.User == "" {
			args.User = nicloudsdk.Me
		}
		...
		workspace, err := deps.niClient.CreateUserWorkspace(ctx, args.User, req)
		if err != nil {
...
var ListWorkspaces = Tool[ListWorkspacesArgs, []MinimalWorkspace]{
	...
	Handler: func(ctx context.Context, deps Deps, args ListWorkspacesArgs) ([]MinimalWorkspace, error) {
		...
		workspaces, err := deps.niClient.Workspaces(ctx, nicloudsdk.WorkspaceFilter{
			Owner: owner,
		})
		if err != nil {
			return nil, err
...
var ListTemplates = Tool[NoArgs, []MinimalTemplate]{
	...
	Handler: func(ctx context.Context, deps Deps, _ NoArgs) ([]MinimalTemplate, error) {
		templates, err := deps.niClient.Templates(ctx, nicloudsdk.TemplateFilter{})
		if err != nil {
			return nil, err
		}
...
var ListTemplateVersionParameters = Tool[ListTemplateVersionParametersArgs, []nicloudsdk.TemplateVersionParameter]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations: mcpReadOnlyAnnotations,
	Handler: func(ctx context.Context, deps Deps, args ListTemplateVersionParametersArgs) ([]nicloudsdk.TemplateVersionParameter, error) {
		...
		parameters, err := deps.niClient.TemplateVersionRichParameters(ctx, templateVersionID)
		if err != nil {
			return nil, err
		}
...
type TemplateDetail struct {
	MinimalTemplate
	Parameters []nicloudsdk.TemplateVersionParameter `json:"parameters"`
	Presets    []presetView                        `json:"presets,omitempty"`
}

// presetView is a tool-local projection of nicloudsdk.Preset with
// snake_case JSON keys that match the field names referenced in
// the create_workspace tool description. nicloudsdk.Preset has no
...
func toPresetView(p nicloudsdk.Preset) presetView {
...
When selecting a preset: if a preset is marked default and the user has not specified preferences, prefer that preset. Presets with desired_prebuild_instances > 0 may have prebuilt workspaces available for faster startup; prefer those when startup speed matters.`,
	...
	Handler: func(ctx context.Context, deps Deps, args GetTemplateArgs) (TemplateDetail, error) {
		...
		template, err := deps.niClient.Template(ctx, templateID)
		if err != nil {
		...
		parameters, err := deps.niClient.TemplateVersionRichParameters(ctx, template.ActiveVersionID)
		if err != nil {
			return TemplateDetail{}, xerrors.Errorf("get template parameters: %w", err)
		}
		presets, err := deps.niClient.TemplateVersionPresets(ctx, template.ActiveVersionID)
		if err != nil {
...
var GetAuthenticatedUser = Tool[NoArgs, nicloudsdk.User]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations: mcpReadOnlyAnnotations,
	Handler: func(ctx context.Context, deps Deps, _ NoArgs) (nicloudsdk.User, error) {
		return deps.niClient.User(ctx, "me")
	},
}
...
var CreateWorkspaceBuild = Tool[CreateWorkspaceBuildArgs, nicloudsdk.WorkspaceBuild]{
	Tool: aisdk.Tool{
...
`,
	...
	Handler: func(ctx context.Context, deps Deps, args CreateWorkspaceBuildArgs) (nicloudsdk.WorkspaceBuild, error) {
		workspaceID, err := uuid.Parse(args.WorkspaceID)
		if err != nil {
			return nicloudsdk.WorkspaceBuild{}, xerrors.Errorf("workspace_id must be a valid UUID: %w", err)
		}
		transition := nicloudsdk.WorkspaceTransition(args.Transition)
		...
		if transition != nicloudsdk.WorkspaceTransitionStart {
			var errs []error
			if args.TemplateVersionPresetID != "" {
			...
			if len(errs) > 0 {
				return nicloudsdk.WorkspaceBuild{}, errors.Join(errs...)
			}
		}
		cbr := nicloudsdk.CreateWorkspaceBuildRequest{
		...
		if args.TemplateVersionID != "" {
			...
			if err != nil {
				return nicloudsdk.WorkspaceBuild{}, xerrors.Errorf("template_version_id must be a valid UUID: %w", err)
			}
		}
		if args.TemplateVersionPresetID != "" {
			cbr.TemplateVersionPresetID, err = uuid.Parse(args.TemplateVersionPresetID)
			if err != nil {
				return nicloudsdk.WorkspaceBuild{}, xerrors.Errorf("template_version_preset_id must be a valid UUID: %w", err)
			}
		}
		return deps.niClient.CreateWorkspaceBuild(ctx, workspaceID, cbr)
	},
}
...
var CreateTemplateVersion = Tool[CreateTemplateVersionArgs, nicloudsdk.TemplateVersion]{
	Tool: aisdk.Tool{
...
- token: Set the environment variable NEURALINVERSE_AGENT_TOKEN to this value to authenticate the agent.
...
data "cloudinit_config" "user_data" {
  ...
  part {
	...
	//     content: |
	//       [Unit]
	//       Description=Neural Inverse Cloud Agent
    ...
    content = templatefile("${path.module}/cloud-init/cloud-config.yaml.tftpl", {
      username    = "neuralinverse" # Ensure this user/group does not exist in your VM image
...
resource "docker_container" "workspace" {
  ...
  volumes {
    container_path = "/home/neuralinverse"
...
`,
	...
	MCPAnnotations: mcpMutationAnnotations,
	Handler: func(ctx context.Context, deps Deps, args CreateTemplateVersionArgs) (nicloudsdk.TemplateVersion, error) {
		me, err := deps.niClient.User(ctx, "me")
		if err != nil {
			return nicloudsdk.TemplateVersion{}, err
		}
		fileID, err := uuid.Parse(args.FileID)
		if err != nil {
			return nicloudsdk.TemplateVersion{}, xerrors.Errorf("file_id must be a valid UUID: %w", err)
		}
		var templateID uuid.UUID
		if args.TemplateID != "" {
			tid, err := uuid.Parse(args.TemplateID)
			if err != nil {
				return nicloudsdk.TemplateVersion{}, xerrors.Errorf("template_id must be a valid UUID: %w", err)
			}
			templateID = tid
		}
		templateVersion, err := deps.niClient.CreateTemplateVersion(ctx, me.OrganizationIDs[0], nicloudsdk.CreateTemplateVersionRequest{
...
var GetWorkspaceAgentLogs = Tool[GetWorkspaceAgentLogsArgs, []string]{
	...
	Handler: func(ctx context.Context, deps Deps, args GetWorkspaceAgentLogsArgs) ([]string, error) {
		...
		logs, closer, err := deps.niClient.WorkspaceAgentLogsAfter(ctx, workspaceAgentID, 0, false)
		if err != nil {
			return nil, err
		}
		defer closer.Close()
...
var GetWorkspaceBuildLogs = Tool[GetWorkspaceBuildLogsArgs, []string]{
	...
	Handler: func(ctx context.Context, deps Deps, args GetWorkspaceBuildLogsArgs) ([]string, error) {
		...
		logs, closer, err := deps.niClient.WorkspaceBuildLogsAfter(ctx, workspaceBuildID, 0)
		if err != nil {
			return nil, err
		}
		defer closer.Close()
...
var GetTemplateVersionLogs = Tool[GetTemplateVersionLogsArgs, []string]{
	...
	Handler: func(ctx context.Context, deps Deps, args GetTemplateVersionLogsArgs) ([]string, error) {
		...
		logs, closer, err := deps.niClient.TemplateVersionLogsAfter(ctx, templateVersionID, 0)
		if err != nil {
			return nil, err
		}
		defer closer.Close()
...
var UpdateTemplateActiveVersion = Tool[UpdateTemplateActiveVersionArgs, string]{
	...
	Handler: func(ctx context.Context, deps Deps, args UpdateTemplateActiveVersionArgs) (string, error) {
		...
		err = deps.niClient.UpdateActiveTemplateVersion(ctx, templateID, nicloudsdk.UpdateActiveTemplateVersion{
...
var UploadTarFile = Tool[UploadTarFileArgs, nicloudsdk.UploadResponse]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations: mcpMutationAnnotations,
	Handler: func(ctx context.Context, deps Deps, args UploadTarFileArgs) (nicloudsdk.UploadResponse, error) {
		...
		resp, err := deps.niClient.Upload(ctx, nicloudsdk.ContentTypeTar, pipeReader)
		if err != nil {
			_ = pipeReader.CloseWithError(err)
			<-done
			return nicloudsdk.UploadResponse{}, err
		}
		<-done
		return resp, nil
	},
...
var CreateTemplate = Tool[CreateTemplateArgs, nicloudsdk.Template]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations: mcpMutationAnnotations,
	Handler: func(ctx context.Context, deps Deps, args CreateTemplateArgs) (nicloudsdk.Template, error) {
		me, err := deps.niClient.User(ctx, "me")
		if err != nil {
			return nicloudsdk.Template{}, err
		}
		versionID, err := uuid.Parse(args.VersionID)
		if err != nil {
			return nicloudsdk.Template{}, xerrors.Errorf("version_id must be a valid UUID: %w", err)
		}
		template, err := deps.niClient.CreateTemplate(ctx, me.OrganizationIDs[0], nicloudsdk.CreateTemplateRequest{
			Name:        args.Name,
...
var DeleteTemplate = Tool[DeleteTemplateArgs, nicloudsdk.Response]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations: mcpDestructiveAnnotations,
	Handler: func(ctx context.Context, deps Deps, args DeleteTemplateArgs) (nicloudsdk.Response, error) {
		templateID, err := uuid.Parse(args.TemplateID)
		if err != nil {
			return nicloudsdk.Response{}, xerrors.Errorf("template_id must be a valid UUID: %w", err)
		}
		err = deps.niClient.DeleteTemplate(ctx, templateID)
		if err != nil {
			return nicloudsdk.Response{}, err
		}
		return nicloudsdk.Response{
...
var WorkspaceWriteFile = Tool[WorkspaceWriteFileArgs, nicloudsdk.Response]{
	Tool: aisdk.Tool{
...
`,
	...
	MCPAnnotations:     mcpDestructiveAnnotations,
	UserClientOptional: true,
	Handler: func(ctx context.Context, deps Deps, args WorkspaceWriteFileArgs) (nicloudsdk.Response, error) {
		conn, err := openAgentConn(ctx, deps, args.Workspace)
		if err != nil {
			return nicloudsdk.Response{}, err
		}
...
var WorkspacePortForward = Tool[WorkspacePortForwardArgs, WorkspacePortForwardResponse]{
	...
	Handler: func(ctx context.Context, deps Deps, args WorkspacePortForwardArgs) (WorkspacePortForwardResponse, error) {
		...
		workspace, workspaceAgent, err := findWorkspaceAndAgent(ctx, deps.niClient, workspaceName)
		if err != nil {
			return WorkspacePortForwardResponse{}, xerrors.Errorf("failed to find workspace: %w", err)
		}
		res, err := deps.niClient.AppHost(ctx)
		if err != nil {
		...
		return WorkspacePortForwardResponse{
			URL: deps.niClient.URL.Scheme + "://" + strings.Replace(res.Host, "*", url.String(), 1),
		}, nil
	},
}
...
var CreateTask = Tool[CreateTaskArgs, nicloudsdk.Task]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations:     mcpMutationAnnotations,
	UserClientOptional: true,
	Handler: func(ctx context.Context, deps Deps, args CreateTaskArgs) (nicloudsdk.Task, error) {
		if args.Input == "" {
			return nicloudsdk.Task{}, xerrors.New("input is required")
		}

		tvID, err := uuid.Parse(args.TemplateVersionID)
		if err != nil {
			return nicloudsdk.Task{}, xerrors.New("template_version_id must be a valid UUID")
		}
		...
		if args.TemplateVersionPresetID != "" {
			tvPresetID, err = uuid.Parse(args.TemplateVersionPresetID)
			if err != nil {
				return nicloudsdk.Task{}, xerrors.New("template_version_preset_id must be a valid UUID")
			}
		}
		...
		task, err := deps.niClient.CreateTask(ctx, args.User, nicloudsdk.CreateTaskRequest{
			Input:                   args.Input,
			TemplateVersionID:       tvID,
			TemplateVersionPresetID: tvPresetID,
		})
		if err != nil {
			return nicloudsdk.Task{}, xerrors.Errorf("create task: %w", err)
		}
...
var DeleteTask = Tool[DeleteTaskArgs, nicloudsdk.Response]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations:     mcpDestructiveAnnotations,
	UserClientOptional: true,
	Handler: func(ctx context.Context, deps Deps, args DeleteTaskArgs) (nicloudsdk.Response, error) {
		if args.TaskID == "" {
			return nicloudsdk.Response{}, xerrors.New("task_id is required")
		}

		task, err := deps.niClient.TaskByIdentifier(ctx, args.TaskID)
		if err != nil {
			return nicloudsdk.Response{}, xerrors.Errorf("resolve task: %w", err)
		}

		err = deps.niClient.DeleteTask(ctx, task.OwnerName, task.ID)
		if err != nil {
			return nicloudsdk.Response{}, xerrors.Errorf("delete task: %w", err)
		}
...
type ListTasksArgs struct {
	Status nicloudsdk.TaskStatus `json:"status"`
	User   string              `json:"user"`
}
...
var ListTasks = Tool[ListTasksArgs, ListTasksResponse]{
	...
	Handler: func(ctx context.Context, deps Deps, args ListTasksArgs) (ListTasksResponse, error) {
		...
		tasks, err := deps.niClient.Tasks(ctx, &nicloudsdk.TasksFilter{
			Owner:  args.User,
			Status: args.Status,
		})
		if err != nil {
...
type GetTaskStatusResponse struct {
	Status nicloudsdk.TaskStatus      `json:"status"`
	State  *nicloudsdk.TaskStateEntry `json:"state"`
}
...
var SendTaskInput = Tool[SendTaskInputArgs, nicloudsdk.Response]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations:     mcpMutationAnnotations,
	UserClientOptional: true,
	Handler: func(ctx context.Context, deps Deps, args SendTaskInputArgs) (nicloudsdk.Response, error) {
		if args.TaskID == "" {
			return nicloudsdk.Response{}, xerrors.New("task_id is required")
		}

		if args.Input == "" {
			return nicloudsdk.Response{}, xerrors.New("input is required")
		}

		task, err := deps.niClient.TaskByIdentifier(ctx, args.TaskID)
		if err != nil {
			return nicloudsdk.Response{}, xerrors.Errorf("resolve task %q: %w", args.TaskID, err)
		}

		err = deps.niClient.TaskSend(ctx, task.OwnerName, task.ID, nicloudsdk.TaskSendRequest{
			Input: args.Input,
		})
		if err != nil {
			return nicloudsdk.Response{}, xerrors.Errorf("send task input %q: %w", args.TaskID, err)
		}
...
var GetTaskLogs = Tool[GetTaskLogsArgs, nicloudsdk.TaskLogsResponse]{
	Tool: aisdk.Tool{
	...
	MCPAnnotations:     mcpReadOnlyAnnotations,
	UserClientOptional: true,
	Handler: func(ctx context.Context, deps Deps, args GetTaskLogsArgs) (nicloudsdk.TaskLogsResponse, error) {
		if args.TaskID == "" {
			return nicloudsdk.TaskLogsResponse{}, xerrors.New("task_id is required")
		}

		task, err := deps.niClient.TaskByIdentifier(ctx, args.TaskID)
		if err != nil {
			return nicloudsdk.TaskLogsResponse{}, err
		}

		logs, err := deps.niClient.TaskLogs(ctx, task.OwnerName, task.ID)
		if err != nil {
			return nicloudsdk.TaskLogsResponse{}, xerrors.Errorf("get task logs %q: %w", args.TaskID, err)
		}
...
//   - owner--workspace.agent       → owner/workspace.agent
//   - agent.workspace.owner        → owner/workspace.agent (Neural Inverse Cloud Connect format)
func NormalizeWorkspaceInput(input string) string {
	// Handle the special Neural Inverse Cloud Connect format: agent.workspace.owner
