...
func setupWorkspaceForAgent(t *testing.T, opts *nicloudtest.Options) (*nicloudsdk.Client, database.WorkspaceTable, string) {
	t.Helper()
...
// nolint:gocritic // This is in a test package and does not end up in the build
func setupWorkspaceForAgentWithName(t *testing.T, opts *nicloudtest.Options, workspaceName string) (*nicloudsdk.Client, database.WorkspaceTable, string) {
	t.Helper()

	client, store := nicloudtest.NewWithDatabase(t, opts)
...
// These tests are dependent on the state of the neuralinverse server.
...
func TestTools(t *testing.T) {
	// Given: a running nicloud instance using SSH test setup pattern
	...
	t.Run("ListTemplates", func(t *testing.T) {
		...
		expected, err := memberClient.Templates(context.Background(), nicloudsdk.TemplateFilter{})
		require.NoError(t, err)
	...
	t.Run("CreateWorkspaceBuild", func(t *testing.T) {
		t.Run("Stop", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, nicloudsdk.WorkspaceTransitionStop, result.Transition)
			...
			require.NoError(t, client.CancelWorkspaceBuild(ctx, result.ID, nicloudsdk.CancelWorkspaceBuildParams{}))
		})
		...
		t.Run("TemplateVersionChange", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, nicloudsdk.WorkspaceTransitionStart, updateBuild.Transition)
			...
			require.NoError(t, client.CancelWorkspaceBuild(ctx, updateBuild.ID, nicloudsdk.CancelWorkspaceBuildParams{}))
			...
			require.NoError(t, err)
			require.Equal(t, nicloudsdk.WorkspaceTransitionStart, rollbackBuild.Transition)
			...
			// Cancel the build so it doesn't remain in the 'pending' state indefinitely.
			require.NoError(t, client.CancelWorkspaceBuild(ctx, rollbackBuild.ID, nicloudsdk.CancelWorkspaceBuildParams{}))
		})
		...
		t.Run("Start_WithPreset", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, nicloudsdk.WorkspaceTransitionStart, result.Transition)
			require.Equal(t, r.Workspace.ID, result.WorkspaceID)
	...
	t.Run("WorkspaceSSHExec", func(t *testing.T) {
		...
		nicloudtest.NewWorkspaceAgentWaiter(t, uuidClient, uuidWorkspace.ID).Wait()
	...
	t.Run("WorkspacePortForward", func(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				port:      1234,
				host:      "*.test.cloud.neuralinverse.com",
				expect:    "%s://1234--dev--myworkspace--myuser.test.cloud.neuralinverse.com:%s",
			},
			{
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				t.Parallel()
				client, workspace, agentToken := setupWorkspaceForAgent(t, &nicloudtest.Options{
					AppHostname: tt.host,
				})
				_ = agenttest.New(t, client.URL, agentToken)
				nicloudtest.NewWorkspaceAgentWaiter(t, client, workspace.ID).Wait()
	...
	t.Run("ListTasks", func(t *testing.T) {
		...
		taskClient, _ := nicloudtest.CreateAnotherUser(t, client, owner.OrganizationID)
		...
		_, err := client.CreateTask(ctx, member.Username, nicloudsdk.CreateTaskRequest{
		...
		for i := range 5 {
			...
			task, err := taskClient.CreateTask(ctx, nicloudsdk.Me, nicloudsdk.CreateTaskRequest{
				TemplateVersionID: template.ActiveVersionID,
			...
			if i == 0 {
				...
				require.NoError(t, err)
				nicloudtest.AwaitWorkspaceBuildJobCompleted(t, taskClient, ws.LatestBuild.ID)

				// Stop the workspace to set task status to paused.
				build, err := taskClient.CreateWorkspaceBuild(ctx, task.WorkspaceID.UUID, nicloudsdk.CreateWorkspaceBuildRequest{
					Transition: nicloudsdk.WorkspaceTransitionStop,
				})
				require.NoError(t, err)
				nicloudtest.AwaitWorkspaceBuildJobCompleted(t, taskClient, build.ID)
			}
		}
		...
		tests := []struct {
			...
			},
			{
				name: "ListFiltered",
				args: toolsdk.ListTasksArgs{
					Status: nicloudsdk.TaskStatusPaused,
				},
				expected: []string{
					"list-task-workspace-0",
				},
	...
	t.Run("GetTask", func(t *testing.T) {
		...
		tests := []struct {
			...
			expected nicloudsdk.TaskStatus
			error    string
		}{
			{
				name: "ByUUID",
				...
				expected: nicloudsdk.TaskStatusInitializing,
			},
			{
				name: "ByIdentifier",
				args: toolsdk.GetTaskStatusArgs{
	...
	t.Run("WorkspaceListApps", func(t *testing.T) {
		...
		_ = dbfake.WorkspaceBuild(t, store, database.WorkspaceTable{
			...
			agents[0].Apps = []*proto.App{
				{
					Slug: "zero",
					Url:  "http://zero.dev.cloud.neuralinverse.com",
				},
			}
			return agents
		}).Do()
		...
		_ = dbfake.WorkspaceBuild(t, store, database.WorkspaceTable{
			...
			agents[0].Apps = []*proto.App{
				{
					Slug: "one",
					Url:  "http://one.dev.cloud.neuralinverse.com",
				},
				{
					Slug: "two",
					Url:  "http://two.dev.cloud.neuralinverse.com",
				},
				{
					Slug: "three",
					Url:  "http://three.dev.cloud.neuralinverse.com",
				},
			}
			agents = append(agents, &proto.Agent{
				...
				Apps: []*proto.App{
					{
						Slug: "four",
						Url:  "http://four.dev.cloud.neuralinverse.com",
					},
				},
			})
			return agents
	...
	t.Run("SendTaskInput", func(t *testing.T) {
		...
		nicloudtest.NewWorkspaceAgentWaiter(t, client, ws.Workspace.ID).
				...
				if tt.error != "" {
					...
					for i, msg := range tt.expected {
						...
						if msg.Role == agentapi.RoleUser {
							require.Equal(t, nicloudsdk.TaskLogTypeInput, res.Logs[i].Type)
						} else {
							require.Equal(t, nicloudsdk.TaskLogTypeOutput, res.Logs[i].Type)
						}
...
func TestReportTaskNilPointerDeref(t *testing.T) {
	...
	client, _ := nicloudtest.NewWithDatabase(t, nil)
	...
	args := toolsdk.ReportTaskArgs{
		Summary: "Test task",
		Link:    "https://example.com",
		State:   string(nicloudsdk.WorkspaceAppStatusStateWorking),
	}
...
func TestReportTaskWithReporter(t *testing.T) {
	...
	reporter := func(args toolsdk.ReportTaskArgs) error {
		...
		require.Equal(t, string(nicloudsdk.WorkspaceAppStatusStateWorking), args.State)
		return nil
	}
