...
// Template is the JSON representation of a Neural Inverse Cloud template. This type matches the
...
type TemplateAutostopRequirement struct {
	...
	// across all workspaces (and Neural Inverse Cloud deployments) using modulo math on a
...
type UpdateTemplateACL struct {
	...
	// (available as a constant named nicloudsdk.TemplateRoleDeleted)
...
// parameters applied server-side; pass nicloudsdk.UsersRequest{} when no
...
// AgentStatsReportRequest is a WebSocket request by nicloud
...
// TemplateExamples lists example templates available in Neural Inverse Cloud.
//
...
// StarterTemplates lists example templates available in Neural Inverse Cloud.
