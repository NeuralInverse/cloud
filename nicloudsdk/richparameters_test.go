...
func TestParameterResolver_ValidateResolve_New(t *testing.T) {
	t.Parallel()
	uut := nicloudsdk.ParameterResolver{}
	p := nicloudsdk.TemplateVersionParameter{
		Name: "n",
		Type: "number",
	}
	v, err := uut.ValidateResolve(p, &nicloudsdk.WorkspaceBuildParameter{
		Name:  "n",
		Value: "1",
	})
...
func TestParameterResolver_ValidateResolve_PrevRequired(t *testing.T) {
	t.Parallel()
	uut := nicloudsdk.ParameterResolver{
		Rich: []nicloudsdk.WorkspaceBuildParameter{{Name: "n", Value: "5"}},
	}
	p := nicloudsdk.TemplateVersionParameter{
		Name:     "n",
		Type:     "number",
...
func TestParameterResolver_ValidateResolve_PrevInvalid(t *testing.T) {
	t.Parallel()
	uut := nicloudsdk.ParameterResolver{
		Rich: []nicloudsdk.WorkspaceBuildParameter{{Name: "n", Value: "11"}},
	}
	p := nicloudsdk.TemplateVersionParameter{
		Name:          "n",
		Type:          "number",
...
func TestParameterResolver_ValidateResolve_Immutable(t *testing.T) {
	t.Parallel()
	uut := nicloudsdk.ParameterResolver{
		Rich: []nicloudsdk.WorkspaceBuildParameter{{Name: "n", Value: "old"}},
	}
	p := nicloudsdk.TemplateVersionParameter{
		Name:     "n",
		Type:     "string",
...
func TestRichParameterValidation(t *testing.T) {
	...
	initialBuildParameters := []nicloudsdk.WorkspaceBuildParameter{
	...
	t.Run("Validation", func(t *testing.T) {
		...
		numberRichParameters := []nicloudsdk.TemplateVersionParameter{
		...
		numberRichParametersMinOnly := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true, ValidationMin: ptr.Ref(int32(5))},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		numberRichParametersMaxOnly := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true, ValidationMax: ptr.Ref(int32(5))},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		monotonicIncreasingNumberRichParameters := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true, ValidationMin: ptr.Ref(int32(3)), ValidationMax: ptr.Ref(int32(100)), ValidationMonotonic: nicloudsdk.MonotonicOrderIncreasing},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		monotonicDecreasingNumberRichParameters := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true, ValidationMin: ptr.Ref(int32(3)), ValidationMax: ptr.Ref(int32(100)), ValidationMonotonic: nicloudsdk.MonotonicOrderDecreasing},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		stringRichParameters := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		boolRichParameters := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true},
			{Name: numberParameterName, Type: "number", Mutable: true},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		regexRichParameters := []nicloudsdk.TemplateVersionParameter{
			{Name: stringParameterName, Type: "string", Mutable: true, ValidationRegex: "^[a-z]+$", ValidationError: "this is error"},
			{Name: numberParameterName, Type: "number", Mutable: true},
			{Name: boolParameterName, Type: "bool", Mutable: true},
		}

		listOfStringsRichParameters := []nicloudsdk.TemplateVersionParameter{
		...
		tests := []struct {
			...
			value          string
			valid          bool
			richParameters []nicloudsdk.TemplateVersionParameter
		}{
...
func TestParameterResolver_ValidateResolve_EmptyString_Monotonic(t *testing.T) {
	t.Parallel()
	uut := nicloudsdk.ParameterResolver{
		Rich: []nicloudsdk.WorkspaceBuildParameter{{Name: "n", Value: ""}},
	}
	p := nicloudsdk.TemplateVersionParameter{
		Name:                "n",
		Type:                "number",
		Mutable:             true,
		DefaultValue:        "0",
		ValidationMonotonic: nicloudsdk.MonotonicOrderIncreasing,
	}
	v, err := uut.ValidateResolve(p, &nicloudsdk.WorkspaceBuildParameter{
		Name:  "n",
		Value: "1",
...
func TestParameterResolver_ValidateResolve_Number_CustomError(t *testing.T) {
	...
	_, err := uut.ValidateResolve(p, &nicloudsdk.WorkspaceBuildParameter{
		Name:  "n",
		Value: "8",
	})
	require.Error(t, err)
