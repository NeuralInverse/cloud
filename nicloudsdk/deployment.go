...
import (
	...
	"github.com/NeuralInverse/cloud/v2/nicloud/agentmetrics"
...
const (
	...
	// enterprise/nicloud/license/license.go for the license format.
...
type Feature struct {
	...
	// from, see enterprise/nicloud/license/license.go.
	//
...
// DeploymentValues is the central configuration values the neuralinverse server.
...
// SessionLifetime refers to "sessions" authenticating into Coderd. Neural Inverse Cloud has
...
// TODO: These config options were created back when neuralinverse only had api keys.
...
type ExternalAuthConfig struct {
	...
	// a string (e.g. cloud.neuralinverse.com) instead of by it's type.
	//
	// Git clone makes use of this by parsing the URL from:
	// 'Username for "https://github.com":'
	// And sending it to the Neural Inverse Cloud server to match against the Regex.
	...
	// The only one supported by Neural Inverse Cloud is "S256".
...
func DefaultCacheDir() string {
	...
	return filepath.Join(defaultCacheDir, "neuralinverse")
}
...
func DefaultDocsURL() string {
	...
	if version == "v0.0.0" {
		return "https://cloud.neuralinverse.com/docs"
	}
	return "https://cloud.neuralinverse.com/docs/@" + version
}
...
func (c *DeploymentValues) Options() serpent.OptionSet {
 ...
 when required by your organization's security policy.`,
	...
	httpAddress := serpent.Option{
		...
		Flag:        "http-address",
		Env:         "NEURALINVERSE_HTTP_ADDRESS",
	...
	redirectToAccessURL := serpent.Option{
		...
		Env:         "NEURALINVERSE_REDIRECT_TO_ACCESS_URL",
	...
	emailSmarthost := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_SMARTHOST",
	...
	emailForceTLS := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_FORCE_TLS",
		Default:     "false",
	...
	emailAuthIdentity := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_AUTH_IDENTITY",
	...
	emailAuthUsername := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_AUTH_USERNAME",
	...
	emailAuthPassword := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_AUTH_PASSWORD",
	...
	emailAuthPasswordFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_AUTH_PASSWORD_FILE",
	...
	emailTLSStartTLS := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_STARTTLS",
	...
	emailTLSServerName := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_SERVERNAME",
	...
	emailTLSSkipCertVerify := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_SKIPVERIFY",
	...
	emailTLSCertAuthorityFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_CACERTFILE",
	...
	emailTLSCertFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_CERTFILE",
	...
	emailTLSCertKeyFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_EMAIL_TLS_CERTKEYFILE",
	...
	telemetryEnable := serpent.Option{
		...
		Flag:        "telemetry",
		Env:         "NEURALINVERSE_TELEMETRY_ENABLE",
		Default:     strconv.FormatBool(flag.Lookup("test.v") == nil || os.Getenv("NEURALINVERSE_TEST_TELEMETRY_DEFAULT_ENABLE") == "true"),
	...
	workspaceHostnameSuffix := serpent.Option{
		...
		Env:         "NEURALINVERSE_WORKSPACE_HOSTNAME_SUFFIX",
		YAML:        "workspaceHostnameSuffix",
		Group:       &deploymentGroupClient,
		Value:       &c.WorkspaceHostnameSuffix,
		Hidden:      false,
		Default:     "neuralinverse",
	}
	...
	aiGatewayEnabled := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_ENABLED",
	...
	aiGatewayOpenAIBaseURL := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_OPENAI_BASE_URL",
	...
	aiGatewayOpenAIKey := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_OPENAI_KEY",
	...
	aiGatewayAnthropicBaseURL := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_ANTHROPIC_BASE_URL",
	...
	aiGatewayAnthropicKey := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_ANTHROPIC_KEY",
	...
	aiGatewayBedrockBaseURL := serpent.Option{
		...
		Description: aiGatewayProviderSeedingDeprecated + "The base URL to use for the AWS Bedrock API. Use this setting to specify an exact URL to use. Takes precedence over NEURALINVERSE_AI_GATEWAY_BEDROCK_REGION.",
		Flag:        "ai-gateway-bedrock-base-url",
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_BASE_URL",
	...
	aiGatewayBedrockRegion := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_REGION",
	...
	aiGatewayBedrockAccessKey := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_ACCESS_KEY",
	...
	aiGatewayBedrockAccessKeySecret := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_ACCESS_KEY_SECRET",
	...
	aiGatewayBedrockModel := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_MODEL",
	...
	aiGatewayBedrockSmallFastModel := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_BEDROCK_SMALL_FAST_MODEL",
	...
	aiGatewayInjectCoderMCPTools := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_INJECT_NEURALINVERSE_MCP_TOOLS",
	...
	aiGatewayRetention := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_RETENTION",
	...
	aiGatewayMaxConcurrency := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_MAX_CONCURRENCY",
	...
	aiGatewayRateLimit := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_RATE_LIMIT",
	...
	aiGatewayStructuredLogging := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_STRUCTURED_LOGGING",
	...
	aiGatewayAPIDumpDir := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_DUMP_DIR",
	...
	aiGatewaySendActorHeaders := serpent.Option{
		...
		Env:     "NEURALINVERSE_AI_GATEWAY_SEND_ACTOR_HEADERS",
	...
	aiGatewayAllowBYOK := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_ALLOW_BYOK",
	...
	aiGatewayCircuitBreakerEnabled := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_ENABLED",
	...
	aiGatewayCircuitBreakerFailureThreshold := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
	...
	aiGatewayCircuitBreakerInterval := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_INTERVAL",
	...
	aiGatewayCircuitBreakerTimeout := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_TIMEOUT",
	...
	aiGatewayCircuitBreakerMaxRequests := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_MAX_REQUESTS",
	...
	aiGatewayProxyEnabled := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_ENABLED",
	...
	aiGatewayProxyListenAddr := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_LISTEN_ADDR",
	...
	aiGatewayProxyTLSCertFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_TLS_CERT_FILE",
	...
	aiGatewayProxyTLSKeyFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_TLS_KEY_FILE",
	...
	aiGatewayProxyMITMCertFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_CERT_FILE",
	...
	aiGatewayProxyMITMKeyFile := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_KEY_FILE",
	...
	aiGatewayProxyDomainAllowlist := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_DOMAIN_ALLOWLIST",
	...
	aiGatewayProxyUpstreamProxy := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_UPSTREAM",
	...
	aiGatewayProxyUpstreamProxyCA := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_UPSTREAM_CA",
	...
	aiGatewayProxyAllowedPrivateCIDRs := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_ALLOWED_PRIVATE_CIDRS",
	...
	aiGatewayProxyAPIDumpDir := serpent.Option{
		...
		Env:         "NEURALINVERSE_AI_GATEWAY_PROXY_DUMP_DIR",
	...
	opts := serpent.OptionSet{
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WILDCARD_ACCESS_URL",
		...
		redirectToAccessURL,
		{
			...
			Env:         "NEURALINVERSE_AUTOBUILD_POLL_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_JOB_HANG_DETECTOR_INTERVAL",
			Hidden:      true,
			Default:     time.Minute.String(),
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_REDIRECT_HTTP_TO_HTTPS",
			Default:     "true",
			Hidden:      true,
		...
		},
		{
			...
			Flag:        "tls-cert-file",
			Env:         "NEURALINVERSE_TLS_CERT_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_CLIENT_CA_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_CLIENT_AUTH",
			Default:     "none",
		...
		},
		{
			...
			Flag:        "tls-key-file",
			Env:         "NEURALINVERSE_TLS_KEY_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_MIN_VERSION",
			Default:     "tls12",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_CLIENT_CERT_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_CLIENT_KEY_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TLS_ALLOW_INSECURE_CIPHERS",
			Default:     "false",
		...
		// Derp settings
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_ENABLE",
			Default:     "true",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_REGION_ID",
			Default:     "999",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_REGION_CODE",
			Default:     "neuralinverse",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_REGION_NAME",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_STUN_ADDRESSES",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_SERVER_RELAY_URL",
		...
		},
		{
			...
			Env:   "NEURALINVERSE_BLOCK_DIRECT",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_FORCE_WEBSOCKETS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_CONFIG_URL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DERP_CONFIG_PATH",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_STATS_COLLECTION_USAGE_STATS_ENABLE",
			Default:     "true",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROMETHEUS_COLLECT_AGENT_STATS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROMETHEUS_AGGREGATE_AGENT_STATS_BY",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_PROMETHEUS_COLLECT_DB_METRICS",
		...
		// Pprof settings
		{
			...
			Flag:        "pprof-enable",
			Env:         "NEURALINVERSE_PPROF_ENABLE",
		...
		},
		{
			...
			Flag:        "pprof-address",
			Env:         "NEURALINVERSE_PPROF_ADDRESS",
		...
		// oAuth settings
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_CLIENT_ID",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_CLIENT_SECRET",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_DEVICE_FLOW",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_DEFAULT_PROVIDER_ENABLE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_ALLOWED_ORGS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_ALLOWED_TEAMS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_ALLOW_SIGNUPS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_ALLOW_EVERYONE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OAUTH2_GITHUB_ENTERPRISE_BASE_URL",
		...
		// OIDC settings.
		{
			...
			Env:         "NEURALINVERSE_OIDC_ALLOW_SIGNUPS",
			Default:     "true",
		...
		},
		{
			...
			Flag:        "oidc-client-id",
			Env:         "NEURALINVERSE_OIDC_CLIENT_ID",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_CLIENT_SECRET",
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		},
		{
			...
			Env:   "NEURALINVERSE_OIDC_CLIENT_KEY_FILE",
		...
		},
		{
			...
			Env:   "NEURALINVERSE_OIDC_CLIENT_CERT_FILE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_EMAIL_DOMAIN",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_ISSUER_URL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_IGNORE_EMAIL_VERIFIED",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_USERNAME_FIELD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_NAME_FIELD",
			Default:     "name",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_EMAIL_FIELD",
			Default:     "email",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_AUTH_URL_PARAMS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_IGNORE_USERINFO",
			Default:     "false",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_OIDC_ACCESS_TOKEN_CLAIMS",
			Default: "false",
		...
		},
		{
			...
			Env:  "NEURALINVERSE_OIDC_ORGANIZATION_FIELD",
		...
		},
		{
			...
			Env:  "NEURALINVERSE_OIDC_ORGANIZATION_ASSIGN_DEFAULT",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_OIDC_ORGANIZATION_MAPPING",
			Default: "{}",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_GROUP_FIELD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_GROUP_MAPPING",
			Default:     "{}",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_GROUP_AUTO_CREATE",
			Default:     "false",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_GROUP_REGEX_FILTER",
			Default:     ".*",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_ALLOWED_GROUPS",
			Default:     "",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_USER_ROLE_FIELD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_USER_ROLE_MAPPING",
			Default:     "{}",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_USER_ROLE_DEFAULT",
			Default:     "",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_SIGN_IN_TEXT",
		...
		},
		{
			...
			Flag:        "oidc-icon-url",
			Env:         "NEURALINVERSE_OIDC_ICON_URL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_OIDC_SIGNUPS_DISABLED_TEXT",
		...
		},
		{
			...
			Env:   "NEURALINVERSE_DANGEROUS_OIDC_SKIP_ISSUER_CHECKS",
		...
		},
		{
			...
			Env:        "NEURALINVERSE_OIDC_REDIRECT_URL",
		...
		telemetryEnable,
		{
			...
			// was renamed to NEURALINVERSE_TELEMETRY_ENABLE in the past, but
			// the flag was not renamed -enable.
			Flag:       "telemetry-enable",
			Env:        "NEURALINVERSE_TELEMETRY",
			Value:      &c.Telemetry.Enable,
			Group:      &deploymentGroupTelemetry,
		...
		// provides a mock server that prints received telemetry as JSON.
		{
			...
			Flag:        "telemetry-url",
			Env:         "NEURALINVERSE_TELEMETRY_URL",
			Hidden:      true,
			Default:     "https://telemetry.cloud.neuralinverse.com",
		...
		// Trace settings
		{
			...
			Flag:        "trace",
			Env:         "NEURALINVERSE_TRACE_ENABLE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TRACE_HONEYCOMB_API_KEY",
		...
		},
		{
			...
			Flag:        "trace-datadog",
			Env:         "NEURALINVERSE_TRACE_DATADOG",
		...
		// Provisioner settings
		{
			...
			Env:         "NEURALINVERSE_PROVISIONER_DAEMONS",
			Default:     "3",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_PROVISIONER_TYPES",
			Hidden:  true,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROVISIONER_DAEMON_POLL_INTERVAL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROVISIONER_DAEMON_POLL_JITTER",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROVISIONER_FORCE_CANCEL_INTERVAL",
		...
		// RateLimit settings
		{
			...
			Env:         "NEURALINVERSE_DANGEROUS_DISABLE_RATE_LIMITS",
		...
		},
		{
			...
			// Change the env from the auto-generated NEURALINVERSE_RATE_LIMIT_API to the
			// old value to avoid breaking existing deployments.
			Env:         "NEURALINVERSE_API_RATE_LIMIT",
			Flag:        "api-rate-limit",
			Default:     "512",
		...
		logFilter,
		{
			...
			Flag:        "log-human",
			Env:         "NEURALINVERSE_LOGGING_HUMAN",
			Default:     "/dev/stderr",
		...
		},
		{
			...
			Flag:        "log-json",
			Env:         "NEURALINVERSE_LOGGING_JSON",
			Default:     "",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_LOGGING_STACKDRIVER",
			Default:     "",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_ENABLE_TERRAFORM_DEBUG_MODE",
			Default:     "false",
		...
		},
		{
			...
			Env:   "NEURALINVERSE_ADDITIONAL_CSP_POLICY",
		...
		// ☢️ Dangerous settings
		{
			...
			Env:         "NEURALINVERSE_DANGEROUS_ALLOW_CORS_REQUESTS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DANGEROUS_ALLOW_PATH_APP_SHARING",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DANGEROUS_ALLOW_PATH_APP_SITE_OWNER_ACCESS",
		...
		},
		{
			...
			Flag:        "update-check",
			Env:         "NEURALINVERSE_UPDATE_CHECK",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_MAX_TOKEN_LIFETIME",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_MAX_ADMIN_TOKEN_LIFETIME",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DEFAULT_TOKEN_LIFETIME",
			Default:     (7 * 24 * time.Hour).String(),
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DEFAULT_OAUTH_REFRESH_LIFETIME",
		...
		},
		{
			...
			Flag:        "swagger-enable",
			Env:         "NEURALINVERSE_SWAGGER_ENABLE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROXY_TRUSTED_HEADERS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PROXY_TRUSTED_ORIGINS",
		...
		},
		{
			...
			Description: "The directory to cache temporary files. If unspecified and $CACHE_DIRECTORY is set, it will be used for compatibility with systemd. " +
				"This directory is NOT safe to be configured as a shared directory across nicloud/provisionerd replicas.",
			Flag:    "cache-dir",
		...
		},
		{
			...
			Description: "URL of a PostgreSQL database. If empty, PostgreSQL binaries will be downloaded from Maven (https://repo1.maven.org/maven2) and store all data in the config root. Access the built-in database with \"neuralinverse server postgres-builtin-url\". Note that any special characters in the URL must be URL-encoded.",
			Flag:        "postgres-url",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_PG_CONN_MAX_OPEN",
			Default:     "10",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_PG_CONN_MAX_IDLE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SECURE_AUTH_COOKIE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SAMESITE_AUTH_COOKIE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_HOST_PREFIX_COOKIE",
			Value:       serpent.BoolOf(&c.HTTPCookies.EnableHostPrefix),
			// Ideally this is true, however any frontend interactions with the neuralinverse api would be broken.
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TERMS_OF_SERVICE_URL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_STRICT_TRANSPORT_SECURITY",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_STRICT_TRANSPORT_SECURITY_OPTIONS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_METRICS_CACHE_REFRESH_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_AGENT_STATS_REFRESH_INTERVAL",
			Hidden:      true,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_AGENT_FALLBACK_TROUBLESHOOTING_URL",
			Hidden:      true,
			Default:     "https://cloud.neuralinverse.com/docs/admin/templates/troubleshooting",
		...
		},
		{
			...
			Flag:        "browser-only",
			Env:         "NEURALINVERSE_BROWSER_ONLY",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SCIM_AUTH_HEADER",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SCIM_USE_LEGACY",
			Hidden:      true,
		...
		},
		{
			...
			Description: "Encrypt OIDC and Git authentication tokens with AES-256-GCM in the database. The value must be a comma-separated list of base64-encoded keys. Each key, when base64-decoded, must be exactly 32 bytes in length. The first key will be used to encrypt new values. Subsequent keys will be used as a fallback when decrypting. During normal operation it is recommended to only set one key unless you are in the process of rotating keys with the `neuralinverse server dbcrypt rotate` command.",
			Flag:        "external-token-encryption-keys",
			Env:         "NEURALINVERSE_EXTERNAL_TOKEN_ENCRYPTION_KEYS",
			Annotations: serpent.Annotations{}.Mark(annotationEnterpriseKey, "true").Mark(annotationSecretKey, "true"),
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_PATH_APPS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_OWNER_WORKSPACE_ACCESS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_WORKSPACE_SHARING",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_CHAT_SHARING",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SESSION_DURATION",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_SESSION_EXPIRY_REFRESH",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SSH_HOSTNAME_PREFIX",
		...
		workspaceHostnameSuffix,
		{
			...
			Env:    "NEURALINVERSE_SSH_CONFIG_OPTIONS",
		...
		},
		{
			...
			Description: "The upgrade message to display to users when a client/server mismatch is detected. By default it instructs users to update using 'curl -L https://cloud.neuralinverse.com/install.sh | sh'.",
			Flag:        "cli-upgrade-message",
			Env:         "NEURALINVERSE_CLI_UPGRADE_MESSAGE",
...
Write out the current server config as YAML to stdout.`,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_SUPPORT_LINKS",
			Flag:        "support-links",
			YAML:        "supportLinks",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_EXTERNAL_AUTH_GITHUB_DEFAULT_PROVIDER_ENABLE",
		...
		},
		{
			...
			Description: "The interval in which nicloud should be checking the status of workspace proxies.",
			Flag:        "proxy-health-interval",
			Env:         "NEURALINVERSE_PROXY_HEALTH_INTERVAL",
			Default:     (time.Minute).String(),
		...
		},
		{
			...
			Env:         "NEURALINVERSE_QUIET_HOURS_DEFAULT_SCHEDULE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_ALLOW_CUSTOM_QUIET_HOURS",
			Default:     "true",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WEB_TERMINAL_RENDERER",
			Default:     "canvas",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_ALLOW_WORKSPACE_RENAMES",
			Default: "false",
		...
		// Healthcheck Options
		{
			...
			Env:         "NEURALINVERSE_HEALTH_CHECK_REFRESH",
			Default:     (10 * time.Minute).String(),
		...
		},
		{
			...
			Env:         "NEURALINVERSE_HEALTH_CHECK_THRESHOLD_DATABASE",
		...
		// Notifications Options
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_METHOD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_DISPATCH_TIMEOUT",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_FROM",
			Value:       &c.Notifications.SMTP.From,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_SMARTHOST",
			Value:       &c.Notifications.SMTP.Smarthost,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "smarthost",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_HELLO",
			Value:       &c.Notifications.SMTP.Hello,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "hello",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_FORCE_TLS",
			Value:       &c.Notifications.SMTP.ForceTLS,
			Group:       &deploymentGroupNotificationsEmail,
			YAML:        "forceTLS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_AUTH_IDENTITY",
			Value:       &c.Notifications.SMTP.Auth.Identity,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_AUTH_USERNAME",
			Value:       &c.Notifications.SMTP.Auth.Username,
			Group:       &deploymentGroupNotificationsEmailAuth,
			YAML:        "username",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_AUTH_PASSWORD",
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
			Value:       &c.Notifications.SMTP.Auth.Password,
			Group:       &deploymentGroupNotificationsEmailAuth,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_AUTH_PASSWORD_FILE",
			Value:       &c.Notifications.SMTP.Auth.PasswordFile,
			Group:       &deploymentGroupNotificationsEmailAuth,
			YAML:        "passwordFile",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_STARTTLS",
			Value:       &c.Notifications.SMTP.TLS.StartTLS,
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_SERVERNAME",
			Value:       &c.Notifications.SMTP.TLS.ServerName,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "serverName",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_SKIPVERIFY",
			Value:       &c.Notifications.SMTP.TLS.InsecureSkipVerify,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "insecureSkipVerify",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_CACERTFILE",
			Value:       &c.Notifications.SMTP.TLS.CAFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "caCertFile",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_CERTFILE",
			Value:       &c.Notifications.SMTP.TLS.CertFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "certFile",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_EMAIL_TLS_CERTKEYFILE",
			Value:       &c.Notifications.SMTP.TLS.KeyFile,
			Group:       &deploymentGroupNotificationsEmailTLS,
			YAML:        "certKeyFile",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_WEBHOOK_ENDPOINT",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_INBOX_ENABLED",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_MAX_SEND_ATTEMPTS",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_RETRY_INTERVAL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_STORE_SYNC_INTERVAL",
		...
		},
		{
			...
			Env:     "NEURALINVERSE_NOTIFICATIONS_STORE_SYNC_BUFFER_SIZE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_LEASE_PERIOD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_LEASE_COUNT",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_NOTIFICATIONS_FETCH_INTERVAL",
		...
		// Workspace Prebuilds Options
		{
			...
			Env:         "NEURALINVERSE_WORKSPACE_PREBUILDS_RECONCILIATION_INTERVAL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WORKSPACE_PREBUILDS_RECONCILIATION_BACKOFF_INTERVAL",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WORKSPACE_PREBUILDS_RECONCILIATION_BACKOFF_LOOKBACK_PERIOD",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WORKSPACE_PREBUILDS_FAILURE_HARD_LIMIT",
		...
		},
		{
			...
			Flag:        "hide-ai-tasks",
			Env:         "NEURALINVERSE_HIDE_AI_TASKS",
			Default:     "false",
			Value:       &c.HideAITasks,
			Group:       &deploymentGroupClient,
			YAML:        "hideAITasks",
		...
		// Chat Options
		{
			...
			Env:         "NEURALINVERSE_CHAT_ACQUIRE_BATCH_SIZE",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_CHAT_DEBUG_LOGGING_ENABLED",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_CHAT_AI_GATEWAY_ROUTING_ENABLED",
		...
		// AI Bridge Options (deprecated in favor of AI Gateway options)
		{
			...
			Description: "Deprecated: use --ai-gateway-enabled or NEURALINVERSE_AI_GATEWAY_ENABLED instead. Whether to start an in-memory aibridged instance.",
			Flag:        "aibridge-enabled",
			Env:         "NEURALINVERSE_AIBRIDGE_ENABLED",
			Value:       &c.AI.BridgeConfig.Enabled,
			Default:     "true",
		...
		aiGatewayEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-openai-base-url or NEURALINVERSE_AI_GATEWAY_OPENAI_BASE_URL instead. The base URL of the OpenAI API.",
			Flag:        "aibridge-openai-base-url",
			Env:         "NEURALINVERSE_AIBRIDGE_OPENAI_BASE_URL",
			Value:       &c.AI.BridgeConfig.LegacyOpenAI.BaseURL,
			Default:     "https://api.openai.com/v1/",
			Group:       &deploymentGroupAIBridge,
			YAML:        "openai_base_url",
		...
		aiGatewayOpenAIBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-openai-key or NEURALINVERSE_AI_GATEWAY_OPENAI_KEY instead. The key to authenticate against the OpenAI API.",
			Flag:        "aibridge-openai-key",
			Env:         "NEURALINVERSE_AIBRIDGE_OPENAI_KEY",
			Value:       &c.AI.BridgeConfig.LegacyOpenAI.Key,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayOpenAIKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-anthropic-base-url or NEURALINVERSE_AI_GATEWAY_ANTHROPIC_BASE_URL instead. The base URL of the Anthropic API.",
			Flag:        "aibridge-anthropic-base-url",
			Env:         "NEURALINVERSE_AIBRIDGE_ANTHROPIC_BASE_URL",
			Value:       &c.AI.BridgeConfig.LegacyAnthropic.BaseURL,
			Default:     "https://api.anthropic.com/",
			Group:       &deploymentGroupAIBridge,
			YAML:        "anthropic_base_url",
		...
		aiGatewayAnthropicBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-anthropic-key or NEURALINVERSE_AI_GATEWAY_ANTHROPIC_KEY instead. The key to authenticate against the Anthropic API.",
			Flag:        "aibridge-anthropic-key",
			Env:         "NEURALINVERSE_AIBRIDGE_ANTHROPIC_KEY",
			Value:       &c.AI.BridgeConfig.LegacyAnthropic.Key,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayAnthropicKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-base-url or NEURALINVERSE_AI_GATEWAY_BEDROCK_BASE_URL instead. The base URL to use for the AWS Bedrock API. Use this setting to specify an exact URL to use. Takes precedence " +
				"over NEURALINVERSE_AIBRIDGE_BEDROCK_REGION.",
			Flag:       "aibridge-bedrock-base-url",
			Env:        "NEURALINVERSE_AIBRIDGE_BEDROCK_BASE_URL",
			Value:      &c.AI.BridgeConfig.LegacyBedrock.BaseURL,
			Default:    "",
			Group:      &deploymentGroupAIBridge,
			YAML:       "bedrock_base_url",
		...
		aiGatewayBedrockBaseURL,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-region or NEURALINVERSE_AI_GATEWAY_BEDROCK_REGION instead. The AWS Bedrock API region to use. Constructs a base URL to use for the AWS Bedrock API in the form of " +
				"'https://bedrock-runtime.<region>.amazonaws.com'.",
			Flag:       "aibridge-bedrock-region",
			Env:        "NEURALINVERSE_AIBRIDGE_BEDROCK_REGION",
			Value:      &c.AI.BridgeConfig.LegacyBedrock.Region,
			Default:    "",
			Group:      &deploymentGroupAIBridge,
			YAML:       "bedrock_region",
		...
		aiGatewayBedrockRegion,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-access-key or NEURALINVERSE_AI_GATEWAY_BEDROCK_ACCESS_KEY instead. The access key to authenticate against the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-access-key",
			Env:         "NEURALINVERSE_AIBRIDGE_BEDROCK_ACCESS_KEY",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.AccessKey,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayBedrockAccessKey,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-access-key-secret or NEURALINVERSE_AI_GATEWAY_BEDROCK_ACCESS_KEY_SECRET instead. The access key secret to use with the access key to authenticate against the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-access-key-secret",
			Env:         "NEURALINVERSE_AIBRIDGE_BEDROCK_ACCESS_KEY_SECRET",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.AccessKeySecret,
			Default:     "",
			Group:       &deploymentGroupAIBridge,
			Annotations: serpent.Annotations{}.Mark(annotationSecretKey, "true"),
		...
		aiGatewayBedrockAccessKeySecret,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-model or NEURALINVERSE_AI_GATEWAY_BEDROCK_MODEL instead. The model to use when making requests to the AWS Bedrock API.",
			Flag:        "aibridge-bedrock-model",
			Env:         "NEURALINVERSE_AIBRIDGE_BEDROCK_MODEL",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.Model,
			Default:     "global.anthropic.claude-sonnet-4-5-20250929-v1:0", // See https://docs.claude.com/en/api/claude-on-amazon-bedrock#accessing-bedrock.
			Group:       &deploymentGroupAIBridge,
			YAML:        "bedrock_model",
		...
		aiGatewayBedrockModel,
		{
			...
			Description: "Deprecated: use --ai-gateway-bedrock-small-fastmodel or NEURALINVERSE_AI_GATEWAY_BEDROCK_SMALL_FAST_MODEL instead. The small fast model to use when making requests to the AWS Bedrock API. Claude Code uses Haiku-class models to perform background tasks. See https://docs.claude.com/en/docs/claude-code/settings#environment-variables.",
			Flag:        "aibridge-bedrock-small-fastmodel",
			Env:         "NEURALINVERSE_AIBRIDGE_BEDROCK_SMALL_FAST_MODEL",
			Value:       &c.AI.BridgeConfig.LegacyBedrock.SmallFastModel,
			Default:     "global.anthropic.claude-haiku-4-5-20251001-v1:0", // See https://docs.claude.com/en/api/claude-on-amazon-bedrock#accessing-bedrock.
			Group:       &deploymentGroupAIBridge,
			YAML:        "bedrock_small_fast_model",
		...
		aiGatewayBedrockSmallFastModel,
		{
			...
			Env:         "NEURALINVERSE_AIBRIDGE_INJECT_NEURALINVERSE_MCP_TOOLS",
			Value:       &c.AI.BridgeConfig.InjectCoderMCPTools,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "inject_coder_mcp_tools",
		...
		aiGatewayInjectCoderMCPTools,
		{
			...
			Description: "Deprecated: use --ai-gateway-retention or NEURALINVERSE_AI_GATEWAY_RETENTION instead. Length of time to retain data such as interceptions and all related records (token, prompt, tool use).",
			Flag:        "aibridge-retention",
			Env:         "NEURALINVERSE_AIBRIDGE_RETENTION",
			Value:       &c.AI.BridgeConfig.Retention,
			Default:     "60d",
			Group:       &deploymentGroupAIBridge,
			YAML:        "retention",
		...
		aiGatewayRetention,
		{
			...
			Description: "Deprecated: use --ai-gateway-max-concurrency or NEURALINVERSE_AI_GATEWAY_MAX_CONCURRENCY instead. Maximum number of concurrent AI Bridge requests per replica. Set to 0 to disable (unlimited).",
			Flag:        "aibridge-max-concurrency",
			Env:         "NEURALINVERSE_AIBRIDGE_MAX_CONCURRENCY",
			Value:       &c.AI.BridgeConfig.MaxConcurrency,
			Default:     "0",
			Group:       &deploymentGroupAIBridge,
			YAML:        "max_concurrency",
		...
		aiGatewayMaxConcurrency,
		{
			...
			Description: "Deprecated: use --ai-gateway-rate-limit or NEURALINVERSE_AI_GATEWAY_RATE_LIMIT instead. Maximum number of AI Bridge requests per second per replica. Set to 0 to disable (unlimited).",
			Flag:        "aibridge-rate-limit",
			Env:         "NEURALINVERSE_AIBRIDGE_RATE_LIMIT",
			Value:       &c.AI.BridgeConfig.RateLimit,
			Default:     "0",
			Group:       &deploymentGroupAIBridge,
			YAML:        "rate_limit",
		...
		aiGatewayRateLimit,
		{
			...
			Description: "Deprecated: use --ai-gateway-structured-logging or NEURALINVERSE_AI_GATEWAY_STRUCTURED_LOGGING instead. Emit structured logs for AI Bridge interception records. Use this for exporting these records to external SIEM or observability systems.",
			Flag:        "aibridge-structured-logging",
			Env:         "NEURALINVERSE_AIBRIDGE_STRUCTURED_LOGGING",
			Value:       &c.AI.BridgeConfig.StructuredLogging,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "structured_logging",
		...
		aiGatewayStructuredLogging,
		{
			...
			Description: "Deprecated: use --ai-gateway-send-actor-headers or NEURALINVERSE_AI_GATEWAY_SEND_ACTOR_HEADERS instead. Once enabled, extra headers will be added to upstream requests to identify the user (actor) making requests to AI Bridge. " +
			...
			Env:        "NEURALINVERSE_AIBRIDGE_SEND_ACTOR_HEADERS",
			Value:      &c.AI.BridgeConfig.SendActorHeaders,
			Default:    "false",
			Group:      &deploymentGroupAIBridge,
			YAML:       "send_actor_headers",
		...
		aiGatewayAPIDumpDir,
		{
			...
			Description: "Deprecated: use --ai-gateway-allow-byok or NEURALINVERSE_AI_GATEWAY_ALLOW_BYOK instead. Allow users to provide their own LLM API keys or subscriptions. When disabled, only centralized key authentication is permitted.",
			Flag:        "aibridge-allow-byok",
			Env:         "NEURALINVERSE_AIBRIDGE_ALLOW_BYOK",
			Value:       &c.AI.BridgeConfig.AllowBYOK,
			Default:     "true",
			Group:       &deploymentGroupAIBridge,
			YAML:        "allow_byok",
		...
		aiGatewayAllowBYOK,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-enabled or NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_ENABLED instead. Enable the circuit breaker to protect against cascading failures from upstream AI provider overload (503, 529).",
			Flag:        "aibridge-circuit-breaker-enabled",
			Env:         "NEURALINVERSE_AIBRIDGE_CIRCUIT_BREAKER_ENABLED",
			Value:       &c.AI.BridgeConfig.CircuitBreakerEnabled,
			Default:     "false",
			Group:       &deploymentGroupAIBridge,
			YAML:        "circuit_breaker_enabled",
		...
		aiGatewayCircuitBreakerEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-failure-threshold or NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_FAILURE_THRESHOLD instead. Number of consecutive failures that triggers the circuit breaker to open.",
			Flag:        "aibridge-circuit-breaker-failure-threshold",
			Env:         "NEURALINVERSE_AIBRIDGE_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
			Value:       serpent.Validate(&c.AI.BridgeConfig.CircuitBreakerFailureThreshold, validateCircuitBreakerPercent),
			Default:     "5",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerFailureThreshold,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-interval or NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_INTERVAL instead. Cyclic period of the closed state for clearing internal failure counts.",
			Flag:        "aibridge-circuit-breaker-interval",
			Env:         "NEURALINVERSE_AIBRIDGE_CIRCUIT_BREAKER_INTERVAL",
			Value:       &c.AI.BridgeConfig.CircuitBreakerInterval,
			Default:     "10s",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerInterval,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-timeout or NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_TIMEOUT instead. How long the circuit breaker stays open before transitioning to half-open state.",
			Flag:        "aibridge-circuit-breaker-timeout",
			Env:         "NEURALINVERSE_AIBRIDGE_CIRCUIT_BREAKER_TIMEOUT",
			Value:       &c.AI.BridgeConfig.CircuitBreakerTimeout,
			Default:     "30s",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerTimeout,
		{
			...
			Description: "Deprecated: use --ai-gateway-circuit-breaker-max-requests or NEURALINVERSE_AI_GATEWAY_CIRCUIT_BREAKER_MAX_REQUESTS instead. Maximum number of requests allowed in half-open state before deciding to close or re-open the circuit.",
			Flag:        "aibridge-circuit-breaker-max-requests",
			Env:         "NEURALINVERSE_AIBRIDGE_CIRCUIT_BREAKER_MAX_REQUESTS",
			Value:       serpent.Validate(&c.AI.BridgeConfig.CircuitBreakerMaxRequests, validateCircuitBreakerPercent),
			Default:     "3",
			Hidden:      true,
			Group:       &deploymentGroupAIBridge,
		...
		aiGatewayCircuitBreakerMaxRequests,
		{
			...
			Env:         "NEURALINVERSE_AI_BUDGET_POLICY",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_AI_BUDGET_PERIOD",
		...
		// AI Gateway Proxy Options
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-enabled or NEURALINVERSE_AI_GATEWAY_PROXY_ENABLED instead. Enable the AI Bridge MITM Proxy for intercepting and decrypting AI provider requests.",
			Flag:        "aibridge-proxy-enabled",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_ENABLED",
			Value:       &c.AI.BridgeProxyConfig.Enabled,
			Default:     "false",
		...
		aiGatewayProxyEnabled,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-listen-addr or NEURALINVERSE_AI_GATEWAY_PROXY_LISTEN_ADDR instead. The address the AI Bridge Proxy will listen on.",
			Flag:        "aibridge-proxy-listen-addr",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_LISTEN_ADDR",
			Value:       &c.AI.BridgeProxyConfig.ListenAddr,
			Default:     ":8888",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "listen_addr",
		...
		aiGatewayProxyListenAddr,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-tls-cert-file or NEURALINVERSE_AI_GATEWAY_PROXY_TLS_CERT_FILE instead. Path to the TLS certificate file for the AI Bridge Proxy listener. Must be set together with AI Bridge Proxy TLS Key File.",
			Flag:        "aibridge-proxy-tls-cert-file",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_TLS_CERT_FILE",
			Value:       &c.AI.BridgeProxyConfig.TLSCertFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "tls_cert_file",
		...
		aiGatewayProxyTLSCertFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-tls-key-file or NEURALINVERSE_AI_GATEWAY_PROXY_TLS_KEY_FILE instead. Path to the TLS private key file for the AI Bridge Proxy listener. Must be set together with AI Bridge Proxy TLS Certificate File.",
			Flag:        "aibridge-proxy-tls-key-file",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_TLS_KEY_FILE",
			Value:       &c.AI.BridgeProxyConfig.TLSKeyFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "tls_key_file",
		...
		aiGatewayProxyTLSKeyFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-cert-file or NEURALINVERSE_AI_GATEWAY_PROXY_CERT_FILE instead. Path to the CA certificate file used to intercept (MITM) HTTPS traffic from AI clients. This CA must be trusted by AI clients for the proxy to decrypt their requests.",
			Flag:        "aibridge-proxy-cert-file",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_CERT_FILE",
			Value:       &c.AI.BridgeProxyConfig.MITMCertFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "cert_file",
		...
		aiGatewayProxyMITMCertFile,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-key-file or NEURALINVERSE_AI_GATEWAY_PROXY_KEY_FILE instead. Path to the CA private key file used to intercept (MITM) HTTPS traffic from AI clients.",
			Flag:        "aibridge-proxy-key-file",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_KEY_FILE",
			Value:       &c.AI.BridgeProxyConfig.MITMKeyFile,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "key_file",
		...
		aiGatewayProxyMITMKeyFile,
		{
			...
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_DOMAIN_ALLOWLIST",
			Value:       &c.AI.BridgeProxyConfig.DomainAllowlist,
			Default:     "",
			Hidden:      true,
			Group:       &deploymentGroupAIBridgeProxy,
		...
		aiGatewayProxyDomainAllowlist,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-upstream or NEURALINVERSE_AI_GATEWAY_PROXY_UPSTREAM instead. URL of an upstream HTTP proxy to chain tunneled (non-allowlisted) requests through. Format: http://[user:pass@]host:port or https://[user:pass@]host:port.",
			Flag:        "aibridge-proxy-upstream",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_UPSTREAM",
			Value:       &c.AI.BridgeProxyConfig.UpstreamProxy,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "upstream_proxy",
		...
		aiGatewayProxyUpstreamProxy,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-upstream-ca or NEURALINVERSE_AI_GATEWAY_PROXY_UPSTREAM_CA instead. Path to a PEM-encoded CA certificate to trust for the upstream proxy's TLS connection. Only needed for HTTPS upstream proxies with certificates not trusted by the system. If not provided, the system certificate pool is used.",
			Flag:        "aibridge-proxy-upstream-ca",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_UPSTREAM_CA",
			Value:       &c.AI.BridgeProxyConfig.UpstreamProxyCA,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "upstream_proxy_ca",
		...
		aiGatewayProxyUpstreamProxyCA,
		{
			...
			Description: "Deprecated: use --ai-gateway-proxy-allowed-private-cidrs or NEURALINVERSE_AI_GATEWAY_PROXY_ALLOWED_PRIVATE_CIDRS instead. Comma-separated list of CIDR ranges that are permitted even though they fall within blocked private/reserved IP ranges. By default all private ranges are blocked to prevent SSRF attacks. Use this to allow access to specific internal networks.",
			Flag:        "aibridge-proxy-allowed-private-cidrs",
			Env:         "NEURALINVERSE_AIBRIDGE_PROXY_ALLOWED_PRIVATE_CIDRS",
			Value:       &c.AI.BridgeProxyConfig.AllowedPrivateCIDRs,
			Default:     "",
			Group:       &deploymentGroupAIBridgeProxy,
			YAML:        "allowed_private_cidrs",
		...
		// Retention settings
		{
			...
			Env:         "NEURALINVERSE_AUDIT_LOGS_RETENTION",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_CONNECTION_LOGS_RETENTION",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_API_KEYS_RETENTION",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_WORKSPACE_AGENT_LOGS_RETENTION",
		...
		},
		{
			...
			Env:      "NEURALINVERSE_ENABLE_AUTHZ_RECORDINGS",
			Default:  "false",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_DISABLE_TEMPLATE_BUILDER",
		...
		},
		{
			...
			Env:         "NEURALINVERSE_TEMPLATE_BUILDER_REGISTRY_URL",
			Value:       &c.TemplateBuilder.RegistryURL,
			Default:     "https://registry.cloud.neuralinverse.com",
			Group:       &deploymentGroupTemplateBuilder,
			YAML:        "registryURL",
		},
	}
...
type AIBridgeConfig struct {
	Enabled serpent.Bool `json:"enabled" typescript:",notnull"`
	// Deprecated: Use Providers with indexed NEURALINVERSE_AI_GATEWAY_PROVIDER_<N>_* env vars instead.
	...
	// Providers holds provider instances populated from NEURALINVERSE_AI_GATEWAY_PROVIDER_<N>_<KEY>
...
// parsed from NEURALINVERSE_AI_GATEWAY_PROVIDER_<N>_<KEY> environment variables.
// NEURALINVERSE_AIBRIDGE_PROVIDER_<N>_<KEY> is also accepted as a deprecated alias.
...
// DeploymentConfig returns the deployment config for the neuralinverse server.
...
type BuildInfoResponse struct {
	// ExternalURL references the current Neural Inverse Cloud version.
	// For production builds, this will link directly to a release. For development builds, this will link to a commit.
	ExternalURL string `json:"external_url"`
	// Version returns the semantic version of the build.
	Version string `json:"version"`
	// DashboardURL is the URL to hit the deployment's dashboard.
	// For external workspace proxies, this is the nicloud they are connected
	// to.
...
type WorkspaceProxyBuildInfo struct {
	...
	WorkspaceProxy bool `json:"workspace_proxy"`
	// DashboardURL is the URL of the nicloud this proxy is connected to.
	DashboardURL string `json:"dashboard_url"`
}
...
// BuildInfo returns build information for this instance of Neural Inverse Cloud.
...
type AppHostResponse struct {
	// Host is the externally accessible URL for the Neural Inverse Cloud instance.
	Host string `json:"host"`
}

// AppHost returns the site-wide application wildcard hostname
// e.g. "*--apps.cloud.neuralinverse.com". Apps are accessible at:
// "<app-name>--<agent-name>--<workspace-name>--<username><app-host>", e.g.
// "my-app--agent--workspace--username--apps.cloud.neuralinverse.com".
//
