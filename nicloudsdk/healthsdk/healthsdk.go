...
func New(c *nicloudsdk.Client) *HealthClient {
...
func (c *HealthClient) DebugHealth(ctx context.Context) (HealthcheckReport, error) {
	...
	if res.StatusCode != http.StatusOK {
		return HealthcheckReport{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *HealthClient) HealthSettings(ctx context.Context) (HealthSettings, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return HealthSettings{}, nicloudsdk.ReadBodyAsError(res)
	}
...
// HealthcheckReport contains information about the health status of a Neural Inverse Cloud deployment.
type HealthcheckReport struct {
	...
	// Severity indicates the status of Neural Inverse Cloud health.
	...
	// The Neural Inverse Cloud version of the server that the report was generated on.
...
type ProvisionerDaemonsReportItem struct {
	nicloudsdk.ProvisionerDaemon `json:"provisioner_daemon"`
	Warnings                   []health.Message `json:"warnings"`
}
...
type WorkspaceProxyReport struct {
	...
	Healthy bool `json:"healthy"`
	BaseReport
	WorkspaceProxies nicloudsdk.RegionsResponse[nicloudsdk.WorkspaceProxy] `json:"workspace_proxies"`
}
