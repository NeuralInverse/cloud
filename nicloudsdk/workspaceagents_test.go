...
func TestProvisionerJobLogText(t *testing.T) {
	...
	log := nicloudsdk.ProvisionerJobLog{
		...
		Source:    nicloudsdk.LogSourceProvisioner,
		Stage:     "Planning",
...
func TestWorkspaceAgentLogText(t *testing.T) {
	...
	ts := time.Date(2024, 1, 28, 10, 30, 0, 0, time.UTC)
	log := nicloudsdk.WorkspaceAgentLog{
		CreatedAt: ts,
		Level:     nicloudsdk.LogLevelInfo,
...
func TestWorkspaceAgentLogTextSpecialChars(t *testing.T) {
	...
	log := nicloudsdk.WorkspaceAgentLog{
		CreatedAt: ts,
		Level:     nicloudsdk.LogLevelDebug,
...
func TestWorkspaceAgentDevcontainerEquals(t *testing.T) {
	...
	base := nicloudsdk.WorkspaceAgentDevcontainer{
		ID:              uuid.New(),
		Name:            "test-dc",
		WorkspaceFolder: "/workspace",
		Status:          nicloudsdk.WorkspaceAgentDevcontainerStatusRunning,
		Dirty:           false,
		Container:       &nicloudsdk.WorkspaceAgentContainer{ID: "container-123"},
		Agent:           &nicloudsdk.WorkspaceAgentDevcontainerAgent{ID: agentID, Name: "agent-1"},
		Error:           "",
	}

	tests := []struct {
		name      string
		modify    func(*nicloudsdk.WorkspaceAgentDevcontainer)
		wantEqual bool
	}{
		{
			name:      "identical",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) {},
			wantEqual: true,
		},
		{
			name:      "different ID",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.ID = uuid.New() },
			wantEqual: false,
		},
		{
			name:      "different Name",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.Name = "other-dc" },
			wantEqual: false,
		},
		{
			name:      "different WorkspaceFolder",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.WorkspaceFolder = "/other" },
			wantEqual: false,
		},
		{
		...
		},
		{
			...
			modify: func(d *nicloudsdk.WorkspaceAgentDevcontainer) {
				d.Status = nicloudsdk.WorkspaceAgentDevcontainerStatusStopped
			},
			wantEqual: false,
		},
		{
			name:      "different Dirty",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.Dirty = true },
			wantEqual: false,
		},
		{
			name:      "different Container (one nil)",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.Container = nil },
			wantEqual: false,
		},
		{
			name: "different Container IDs",
			modify: func(d *nicloudsdk.WorkspaceAgentDevcontainer) {
				d.Container = &nicloudsdk.WorkspaceAgentContainer{ID: "different-container"}
			},
			wantEqual: false,
		},
		{
			name:      "different Agent (one nil)",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.Agent = nil },
			wantEqual: false,
		},
		{
			name: "different Agent values",
			modify: func(d *nicloudsdk.WorkspaceAgentDevcontainer) {
				d.Agent = &nicloudsdk.WorkspaceAgentDevcontainerAgent{ID: agentID, Name: "agent-2"}
			},
			wantEqual: false,
		},
		{
			name:      "different Error",
			modify:    func(d *nicloudsdk.WorkspaceAgentDevcontainer) { d.Error = "some error" },
			wantEqual: false,
		},
	}
