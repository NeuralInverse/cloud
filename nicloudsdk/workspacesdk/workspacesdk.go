...
const (
	...
	// by the proxy applications endpoint. Neural Inverse Cloud consumes ports 1-4 at the
...
var AgentIgnoredListeningPorts = map[uint16]struct{}{
	0: {},
	// Ports 1-8 are reserved for future use by the Neural Inverse Cloud agent.
	1: {},
	2: {},
	3: {},
	4: {},
...
func New(c *nicloudsdk.Client) *Client {
...
func (c *Client) AgentConnectionInfoGeneric(ctx context.Context) (AgentConnectionInfo, error) {
	...
	if res.StatusCode != http.StatusOK {
		return AgentConnectionInfo{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *Client) DialAgent(dialCtx context.Context, agentID uuid.UUID, options *DialAgentOptions) (agentConn AgentConn, err error) {
	...
	if headerTransport, ok := c.client.HTTPClient.Transport.(*nicloudsdk.HeaderTransport); ok {
...
type WorkspaceAgentReconnectingPTYOpts struct {
	...
	// NEURALINVERSE_AGENT_DEVCONTAINERS_ENABLE set to "true".
	Container     string
...
func (c *Client) AgentReconnectingPTY(ctx context.Context, opts WorkspaceAgentReconnectingPTYOpts) (net.Conn, error) {
	...
	if opts.SignedToken != "" {
		q.Set(nicloudsdk.SignedAppTokenQueryParameter, opts.SignedToken)
	}
	serverURL.RawQuery = q.Encode()

	// Shallow-clone the HTTP client so we never inherit a caller-provided
	// cookie jar. Non-browser websocket auth uses the Neural Inverse Cloud-Session-Token
	...
	if opts.SignedToken == "" {
		headers.Set(nicloudsdk.SessionTokenHeader, c.client.SessionToken())
	}
	//nolint:bodyclose
...
type NIConnectQueryOptions struct {
	HostnameSuffix string
}

// IsNIConnectRunning checks if Neural Inverse Cloud Connect (OS level tunnel to workspaces) is running on the system. If you
// already know the hostname suffix your deployment uses, you can pass it in the Neural Inverse CloudConnectQueryOptions to avoid an
// API call to AgentConnectionInfoGeneric.
func (c *Client) IsNIConnectRunning(ctx context.Context, o NIConnectQueryOptions) (bool, error) {
	...
	domainName := fmt.Sprintf(tailnet.IsNIConnectEnabledFmtString, suffix)
	return ExistsViaNIConnect(ctx, domainName)
}
...
// ExistsViaNIConnect checks if the given hostname exists via Neural Inverse Cloud Connect. This doesn't guarantee the
// workspace is actually reachable, if, for example, its agent is unhealthy, but rather that Neural Inverse Cloud Connect knows about
// the workspace and advertises the hostname via DNS.
func ExistsViaNIConnect(ctx context.Context, hostname string) (bool, error) {
	...
	// The returned IP addresses are probably from the Neural Inverse Cloud Connect DNS server, but there are sometimes weird captive
	...
	for _, ip := range ips {
		...
		if tailnet.NIServicePrefix.AsNetip().Contains(addr) {
			return true, nil
		}
	}
	return false, nil
