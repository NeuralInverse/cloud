...
// to the WorkspaceAgentConn, or it may be shared in the case of nicloud. If the
...
const (
	// Neural Inverse CloudChatIDHeader is the HTTP header containing the current
	// chat ID. Set by nicloud on agentconn requests originating
	// from chatd.
	NIChatIDHeader = "Coder-Chat-Id"
	// Neural Inverse CloudAncestorChatIDsHeader is the HTTP header containing a
	// JSON array of ancestor chat UUIDs.
	NIAncestorChatIDsHeader = "Coder-Ancestor-Chat-Ids"
)
...
type AgentConn interface {
	...
	ListContainers(ctx context.Context) (nicloudsdk.WorkspaceAgentListContainersResponse, error)
	ListMCPTools(ctx context.Context) (ListMCPToolsResponse, error)
	ListProcesses(ctx context.Context) (ListProcessesResponse, error)
	ListeningPorts(ctx context.Context) (nicloudsdk.WorkspaceAgentListeningPortsResponse, error)
	...
	RecreateDevcontainer(ctx context.Context, devcontainerID string) (nicloudsdk.Response, error)
	...
	WatchContainers(ctx context.Context, logger slog.Logger) (<-chan nicloudsdk.WorkspaceAgentListContainersResponse, io.Closer, error)
	WatchGit(ctx context.Context, logger slog.Logger, chatID uuid.UUID) (*wsjson.Stream[nicloudsdk.WorkspaceAgentGitServerMessage, nicloudsdk.WorkspaceAgentGitClientMessage], error)
...
func (c *agentConn) ListeningPorts(ctx context.Context) (nicloudsdk.WorkspaceAgentListeningPortsResponse, error) {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
	res, err := c.apiRequest(ctx, http.MethodGet, "/api/v0/listening-ports", nil)
	if err != nil {
		return nicloudsdk.WorkspaceAgentListeningPortsResponse{}, xerrors.Errorf("do request: %w", err)
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return nicloudsdk.WorkspaceAgentListeningPortsResponse{}, nicloudsdk.ReadBodyAsError(res)
	}

	var resp nicloudsdk.WorkspaceAgentListeningPortsResponse
...
func (c *agentConn) Netcheck(ctx context.Context) (healthsdk.AgentNetcheckReport, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return healthsdk.AgentNetcheckReport{}, nicloudsdk.ReadBodyAsError(res)
	}
...
// DebugLogs returns up to the last 10MB of `/tmp/neuralinverse-agent.log`
...
func (c *agentConn) ListContainers(ctx context.Context) (nicloudsdk.WorkspaceAgentListContainersResponse, error) {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
	res, err := c.apiRequest(ctx, http.MethodGet, "/api/v0/containers", nil)
	if err != nil {
		return nicloudsdk.WorkspaceAgentListContainersResponse{}, xerrors.Errorf("do request: %w", err)
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return nicloudsdk.WorkspaceAgentListContainersResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
	var resp nicloudsdk.WorkspaceAgentListContainersResponse
	return resp, json.NewDecoder(res.Body).Decode(&resp)
}

func (c *agentConn) WatchContainers(ctx context.Context, logger slog.Logger) (<-chan nicloudsdk.WorkspaceAgentListContainersResponse, io.Closer, error) {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
	...
	if err != nil {
		...
		return nil, nil, nicloudsdk.ReadBodyAsError(res)
	}
	...
	d := wsjson.NewDecoder[nicloudsdk.WorkspaceAgentListContainersResponse](conn, websocket.MessageText, logger)
	return d.Chan(), d, nil
}
...
func (c *agentConn) WatchGit(ctx context.Context, logger slog.Logger, chatID uuid.UUID) (*wsjson.Stream[nicloudsdk.WorkspaceAgentGitServerMessage, nicloudsdk.WorkspaceAgentGitClientMessage], error) {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
...
func (c *agentConn) ExecuteDesktopAction(ctx context.Context, action DesktopAction) (DesktopActionResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return DesktopActionResponse{}, nicloudsdk.ReadBodyAsError(resp)
	}
...
func (c *agentConn) StopDesktopRecording(ctx context.Context, req StopDesktopRecordingRequest) (StopDesktopRecordingResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		defer res.Body.Close()
		return StopDesktopRecordingResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) RecreateDevcontainer(ctx context.Context, devcontainerID string) (nicloudsdk.Response, error) {
	ctx, span := tracing.StartSpan(ctx)
	defer span.End()
	res, err := c.apiRequest(ctx, http.MethodPost, "/api/v0/containers/devcontainers/"+devcontainerID+"/recreate", nil)
	if err != nil {
		return nicloudsdk.Response{}, xerrors.Errorf("do request: %w", err)
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusAccepted {
		return nicloudsdk.Response{}, nicloudsdk.ReadBodyAsError(res)
	}
	var m nicloudsdk.Response
	if err := json.NewDecoder(res.Body).Decode(&m); err != nil {
		return nicloudsdk.Response{}, xerrors.Errorf("decode response body: %w", err)
	}
	return m, nil
}
...
func (c *agentConn) LS(ctx context.Context, path string, req LSRequest) (LSResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return LSResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ReadFileLines(ctx context.Context, path string, offset, limit int64, limits ReadFileLinesLimits) (ReadFileLinesResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return ReadFileLinesResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ReadFile(ctx context.Context, path string, offset, limit int64) (io.ReadCloser, string, error) {
	...
	if res.StatusCode != http.StatusOK {
		// nicloudsdk.ReadBodyAsError will close the body.
		return nil, "", nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) StartProcess(ctx context.Context, req StartProcessRequest) (StartProcessResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return StartProcessResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ListProcesses(ctx context.Context) (ListProcessesResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return ListProcessesResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ListMCPTools(ctx context.Context) (ListMCPToolsResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return ListMCPToolsResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ContextConfig(ctx context.Context) (ContextConfigResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return ContextConfigResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) CallMCPTool(ctx context.Context, req CallMCPToolRequest) (CallMCPToolResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return CallMCPToolResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) ProcessOutput(ctx context.Context, id string, opts *ProcessOutputOptions) (ProcessOutputResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return ProcessOutputResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *agentConn) EditFiles(ctx context.Context, edits FileEditRequest) (FileEditResponse, error) {
	...
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return FileEditResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
