...
type tunnelerEvent struct {
	...
	provisionerJobLog *nicloudsdk.ProvisionerJobLog
	agentUpdate       *workspacesdk.AgentUpdate
	agentLog          *nicloudsdk.WorkspaceAgentLog
...
func (t *Tunneler) handleBuildUpdate(update *workspacesdk.BuildUpdate) {
	...
	if update.Transition == nicloudsdk.WorkspaceTransitionDelete {
	...
	if update.Transition == nicloudsdk.WorkspaceTransitionStart && canMakeProgress {
	...
	if update.Transition == nicloudsdk.WorkspaceTransitionStart && update.JobStatus == nicloudsdk.ProvisionerJobSucceeded {
	...
	if update.Transition == nicloudsdk.WorkspaceTransitionStop {
		...
		if update.JobStatus == nicloudsdk.ProvisionerJobSucceeded {
			switch t.state {
...
func (*Tunneler) handleProvisionerJobLog(*nicloudsdk.ProvisionerJobLog) {
}
...
func (t *Tunneler) handleAgentUpdate(update *workspacesdk.AgentUpdate) {
	...
	// in mind when implementing the watch in Neural Inverse Cloudd.
	switch update.Lifecycle {
	case nicloudsdk.WorkspaceAgentLifecycleReady:
		doConnect()
		return
	case nicloudsdk.WorkspaceAgentLifecycleStarting,
	...
	case nicloudsdk.WorkspaceAgentLifecycleShuttingDown:
	case nicloudsdk.WorkspaceAgentLifecycleShutdownError:
	case nicloudsdk.WorkspaceAgentLifecycleShutdownTimeout:
	case nicloudsdk.WorkspaceAgentLifecycleOff:
	case nicloudsdk.WorkspaceAgentLifecycleCreated: // initial state, so it hasn't connected yet
	default:
...
func (*Tunneler) handleAgentLog(*nicloudsdk.WorkspaceAgentLog) {
}
