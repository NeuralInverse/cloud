...
func TestHandleBuildUpdate_Coverage(t *testing.T) {
	...
	for s := range maxState {
		for _, trans := range nicloudsdk.WorkspaceTransitionEnums() {
			for _, jobStatus := range nicloudsdk.ProvisionerJobStatusEnums() {
...
func TestBuildUpdatesStoppedWorkspace(t *testing.T) {
	...
	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStop, JobStatus: nicloudsdk.ProvisionerJobPending})
	require.Equal(t, waitToStart, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.False(t, fWorkspaceStarter.started)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStop, JobStatus: nicloudsdk.ProvisionerJobRunning})
	require.Equal(t, waitToStart, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.False(t, fWorkspaceStarter.started)

	// when stop job succeeds, we start the workspace
	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStop, JobStatus: nicloudsdk.ProvisionerJobSucceeded})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)
	require.True(t, fWorkspaceStarter.started)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStart, JobStatus: nicloudsdk.ProvisionerJobPending})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStart, JobStatus: nicloudsdk.ProvisionerJobRunning})
	require.Equal(t, waitForWorkspaceStarted, uut.state)
	waitForGoroutines(testCtx, t, uut)

	uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStart, JobStatus: nicloudsdk.ProvisionerJobSucceeded})
...
func TestBuildUpdatesBadJobs(t *testing.T) {
	t.Parallel()
	for _, jobStatus := range []nicloudsdk.ProvisionerJobStatus{
		nicloudsdk.ProvisionerJobFailed,
		nicloudsdk.ProvisionerJobCanceling,
		nicloudsdk.ProvisionerJobCanceled,
		nicloudsdk.ProvisionerJobUnknown,
	} {
		t.Run(string(jobStatus), func(t *testing.T) {
			...
			uut.handleBuildUpdate(&workspacesdk.BuildUpdate{Transition: nicloudsdk.WorkspaceTransitionStop, JobStatus: jobStatus})
...
func TestAgentUpdate_Coverage(t *testing.T) {
	...
	for s := range maxState {
		for _, lifecycle := range nicloudsdk.WorkspaceAgentLifecycleOrder {
			for _, noAutostart := range []bool{true, false} {
				for _, noWaitForScripts := range []bool{true, false} {
...
func TestAgentUpdateReady(t *testing.T) {
	...
	uut.handleAgentUpdate(&workspacesdk.AgentUpdate{Lifecycle: nicloudsdk.WorkspaceAgentLifecycleReady, ID: agentID})
...
func TestAgentUpdateNoWait(t *testing.T) {
	...
	uut.handleAgentUpdate(&workspacesdk.AgentUpdate{Lifecycle: nicloudsdk.WorkspaceAgentLifecycleStarting, ID: agentID})
	require.Equal(t, establishTailnet, uut.state)
	event := testutil.RequireReceive(testCtx, t, uut.events)
	require.NotNil(t, event.tailnetUpdate)
	require.True(t, fClient.dialed)
...
func TestTunneler_EventLoop_Signal(t *testing.T) {
	...
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			...
			JobStatus:  nicloudsdk.ProvisionerJobPending,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			Transition: nicloudsdk.WorkspaceTransitionStart,
			JobStatus:  nicloudsdk.ProvisionerJobRunning,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		buildUpdate: &workspacesdk.BuildUpdate{
			Transition: nicloudsdk.WorkspaceTransitionStart,
			JobStatus:  nicloudsdk.ProvisionerJobSucceeded,
		},
	})
	testutil.RequireSend(testCtx, t, uut.events, tunnelerEvent{
		agentUpdate: &workspacesdk.AgentUpdate{
			Lifecycle: nicloudsdk.WorkspaceAgentLifecycleReady,
			ID:        agentID,
		},
	})
