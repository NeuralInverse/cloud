...
func (w *WebsocketDialer) checkResumeTokenFailure(ctx context.Context, sdkErr *nicloudsdk.Error) bool {
	if sdkErr == nil {
		return false
	}
...
func (w *WebsocketDialer) Dial(ctx context.Context, r tailnet.ResumeTokenController,
	...
	*u = *w.url
	...
	if w.isFirst {
		if res != nil && slices.Contains(permanentErrorStatuses, res.StatusCode) {
			...
			if xerrors.As(err, &sdkErr) {
				...
				if sdkErr.Message == nicloudsdk.DatabaseNotReachable &&
					sdkErr.StatusCode() == http.StatusInternalServerError {
					err = xerrors.Errorf("%w: %v", nicloudsdk.ErrDatabaseNotReachable, err)
				}
			}
			w.connected <- err
			return tailnet.ControlProtocolClients{}, err
	...
	if err != nil {
		bodyErr := nicloudsdk.ReadBodyAsError(res)
		var sdkErr *nicloudsdk.Error
