...
func TestUserSecretNameValid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := nicloudsdk.UserSecretNameValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errMsg != "" {
...
func TestUserSecretEnvNameValid(t *testing.T) {
	...
	tests := []struct {
		...
		// Length cap.
		{name: "ExactlyAtLengthLimit", input: strings.Repeat("A", nicloudsdk.MaxUserSecretEnvNameLength)},
		{name: "OverLengthLimit", input: strings.Repeat("A", nicloudsdk.MaxUserSecretEnvNameLength+1), wantErr: true, errMsg: "256 bytes"},
		...
		// NEURALINVERSE_ prefix.
		{name: "CoderExact", input: "CODER", wantErr: true, errMsg: "NEURALINVERSE_"},
		{name: "CoderPrefix", input: "NEURALINVERSE_WORKSPACE_NAME", wantErr: true, errMsg: "NEURALINVERSE_"},
		{name: "CoderAgentToken", input: "NEURALINVERSE_AGENT_TOKEN", wantErr: true, errMsg: "NEURALINVERSE_"},
		{name: "CoderLowerCase", input: "coder_foo", wantErr: true, errMsg: "NEURALINVERSE_"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := nicloudsdk.UserSecretEnvNameValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errMsg != "" {
					assert.Contains(t, err.Error(), tt.errMsg)
...
func TestUserSecretFilePathValid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := nicloudsdk.UserSecretFilePathValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestUserSecretValueValid(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ExactlyAtLimit", input: strings.Repeat("a", nicloudsdk.MaxUserSecretValueBytes)},
		{name: "OverLimit", input: strings.Repeat("a", nicloudsdk.MaxUserSecretValueBytes+1), wantErr: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := nicloudsdk.UserSecretValueValid(tt.input)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
