...
type IDPSyncMapping[ResourceIdType uuid.UUID | string] struct {
	...
	Given string
	// The ID of the Neural Inverse Cloud resource the user should be added to
	Gets ResourceIdType
}
...
type GroupSyncSettings struct {
	...
	// Mapping is a map from OIDC groups to Neural Inverse Cloud group IDs
	...
	// are automatically created in Neural Inverse Cloud if they are missing.
	AutoCreateMissing bool `json:"auto_create_missing_groups"`
	// LegacyNameMapping is deprecated. It remaps an IDP group name to
	// a Neural Inverse Cloud group name. Since configuration is now done at runtime,
...
type RoleSyncSettings struct {
	...
	Field string `json:"field"`
	// Mapping is a map from OIDC groups to Neural Inverse Cloud organization roles.
...
type OrganizationSyncSettings struct {
	...
	Field string `json:"field"`
	// Mapping maps from an OIDC claim --> Neural Inverse Cloud organization uuid
	Mapping map[string][]uuid.UUID `json:"mapping"`
