...
// These cookies are Neural Inverse Cloud-specific. If a new one is added or changed, the name
// shouldn't be likely to conflict with any user-application set cookies.
// Be sure to strip additional cookies in httpapi.StripNeural Inverse CloudCookies!
...
const (
	...
	SessionTokenHeader = "NI-Session-Token"
	...
	// Neural Inverse CloudDesktopTelemetryHeader contains a JSON-encoded representation of Desktop telemetry
...
// New creates a Neural Inverse Cloud client for the provided URL.
...
// Client is an HTTP caller for methods to the Neural Inverse Cloud API.
...
type Client struct {
	...
	// Trace can be enabled to propagate tracing spans to the Neural Inverse Cloud API.
...
// ReadBodyAsError reads the response as a nicloudsdk.Response, and
// wraps it in a nicloudsdk.Error type for easy marshaling.
//
...
// CoderDesktopTelemetry represents the telemetry data sent from Neural Inverse Cloud Desktop clients.
// @typescript-ignore Neural Inverse CloudDesktopTelemetry
...
func (t *CoderDesktopTelemetry) IsEmpty() bool {
	return t.DeviceID == "" && t.DeviceOS == "" && t.NIDesktopVersion == ""
}
