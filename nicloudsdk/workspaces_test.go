...
func TestResolveWorkspace(t *testing.T) {
	...
	// errResponse builds a nicloudsdk.Response suitable for error
	// replies.
	errResponse := func(msg string) nicloudsdk.Response {
		return nicloudsdk.Response{Message: msg}
	}

	// newWorkspace returns a Workspace with the given ID and name.
	newWorkspace := func(id uuid.UUID, name string) nicloudsdk.Workspace {
		return nicloudsdk.Workspace{ID: id, Name: name}
	}
	...
	t.Run("TransportError", func(t *testing.T) {
		...
		client := nicloudsdk.New(srvURL)

		_, err = client.ResolveWorkspace(t.Context(), uuid.NewString())
		require.Error(t, err)

		// Transport errors must not be swallowed by the 404
		// fallback path. The error should NOT be a *nicloudsdk.Error.
		var sdkErr *nicloudsdk.Error
		require.False(t, errors.As(err, &sdkErr), "transport error should not be a nicloudsdk.Error")
	})
