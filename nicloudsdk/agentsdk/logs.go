...
func LogsWriter(ctx context.Context, sender func(ctx context.Context, log ...Log) error, source uuid.UUID, level nicloudsdk.LogLevel) io.WriteCloser {
		...
		for {
			...
			if (done || flushed) && len(backlog) > 0 {
				...
				for r := retry.New(time.Second, 5*time.Second); r.Wait(ctx); {
					...
					// This error is expected to be nicloudsdk.Error, but it has
...
func (l *LogSender) Enqueue(src uuid.UUID, logs ...Log) {
	...
	for k, log := range logs {
		...
		// examined the Neural Inverse Cloud agent logs.
