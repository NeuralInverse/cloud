		...
		Apps: []nicloudsdk.WorkspaceApp{
			{
				...
				SharingLevel:  nicloudsdk.WorkspaceAppSharingLevelAuthenticated,
				Healthcheck: nicloudsdk.Healthcheck{
					URL:       "http://localhost:3030/healthz",
					Interval:  55555666,
					Threshold: 55555666,
				},
				Health: nicloudsdk.WorkspaceAppHealthHealthy,
				Hidden: false,
			},
			{
				ID:            uuid.New(),
				...
				SharingLevel:  nicloudsdk.WorkspaceAppSharingLevelPublic,
				Healthcheck: nicloudsdk.Healthcheck{
...
func TestSubsystems(t *testing.T) {
	t.Parallel()
	ss := []nicloudsdk.AgentSubsystem{
		nicloudsdk.AgentSubsystemEnvbox,
		nicloudsdk.AgentSubsystemEnvbuilder,
...
func TestProtoFromLifecycle(t *testing.T) {
	t.Parallel()
	now := dbtime.Now()
	for _, s := range nicloudsdk.WorkspaceAgentLifecycleOrder {
