...
// GoogleSessionTokenExchanger exchanges a Google instance JWT document for a Neural Inverse Cloud session token.
...
func WithGoogleInstanceIdentity(serviceAccount string, gcpClient *metadata.Client, opts ...InstanceIdentityOption) SessionTokenSetup {
	...
	return func(client *nicloudsdk.Client) RefreshableSessionTokenProvider {
...
func (g *GoogleSessionTokenExchanger) exchange(ctx context.Context) (AuthenticateResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return AuthenticateResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
