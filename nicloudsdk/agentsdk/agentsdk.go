...
// way because cloud instance identity (AWS, Azure, Google, etc.) requires interacting with nicloud to exchange tokens.
// This means that the token providers need a nicloudsdk.Client. However, the SessionTokenProvider is itself used by
// the client to authenticate requests. Thus, the dependency is bidirectional. Functions of this type are used in
// New() to ensure that things are set up correctly so there is only one instance of the nicloudsdk.Client created.
// @typescript-ignore SessionTokenSetup
type SessionTokenSetup func(client *nicloudsdk.Client) RefreshableSessionTokenProvider

// New creates a new *Client which can be used by an agent to connect to Neural Inverse Cloudd. Use a SessionTokenSetup function
// to define the session token provider for the Client. This overrides the SessionTokenProvider on the underlying
// `*nicloudsdk.Client`, so any `nicloudsdk.ClientOptions` passed as `opts` should not set this property.
func New(serverURL *url.URL, setup SessionTokenSetup, opts ...nicloudsdk.ClientOption) *Client {
	var provider RefreshableSessionTokenProvider
	opts = append(opts, func(c *nicloudsdk.Client) {
		provider = setup(c)
		c.SessionTokenProvider = provider
	})
	c := nicloudsdk.New(serverURL, opts...)
	return &Client{
		SDK:                             c,
...
// Client wraps `nicloudsdk.Client` with specific functions
// scoped to a workspace agent.
type Client struct {
	RefreshableSessionTokenProvider
	SDK *nicloudsdk.Client
}
...
func (c *Client) GitSSHKey(ctx context.Context) (GitSSHKey, error) {
	...
	if res.StatusCode != http.StatusOK {
		return GitSSHKey{}, nicloudsdk.ReadBodyAsError(res)
	}
...
type Metadata struct {
	Key string `json:"key"`
	nicloudsdk.WorkspaceAgentMetadataResult
}
...
// performance.
type PostMetadataRequestDeprecated = nicloudsdk.WorkspaceAgentMetadataResult
...
type Manifest struct {
	...
	Metadata                 []nicloudsdk.WorkspaceAgentMetadataDescription `json:"metadata"`
	Scripts                  []nicloudsdk.WorkspaceAgentScript              `json:"scripts"`
	Devcontainers            []nicloudsdk.WorkspaceAgentDevcontainer        `json:"devcontainers"`
}
...
// GetAnnouncementBanners, but is useful when you want to be maximally compatible with Neural Inverse Cloudd
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.12+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.13+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.18+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.20+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.23+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.24+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.30+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.31+
...
// maximally compatible with Neural Inverse Cloudd Release Versions from 2.32+
...
func (c *Client) connectRPCVersion(ctx context.Context, version *apiversion.APIVersion, role string) (drpc.Conn, error) {
	...
	conn, res, err := websocket.Dial(ctx, rpcURL.String(), &websocket.DialOptions{
		...
		HTTPHeader: http.Header{
			nicloudsdk.SessionTokenHeader: []string{c.SDK.SessionToken()},
		},
	})
	if err != nil {
		if res == nil {
		...
		return nil, nicloudsdk.ReadBodyAsError(res)
	}
...
type PostAppHealthsRequest struct {
	...
	Healths map[uuid.UUID]nicloudsdk.WorkspaceAppHealth
}
...
type RefreshableSessionTokenProvider interface {
	nicloudsdk.SessionTokenProvider
...
// TokenExchanger obtains a session token by exchanging a cloud instance identity credential for a Neural Inverse Cloud session token.
// @typescript-ignore TokenExchanger
type TokenExchanger interface {
	exchange(ctx context.Context) (AuthenticateResponse, error)
}

func (i *InstanceIdentitySessionTokenProvider) AsRequestOption() nicloudsdk.RequestOption {
	t := i.GetSessionToken()
	return func(req *http.Request) {
		req.Header.Set(nicloudsdk.SessionTokenHeader, t)
	}
}
...
func (i *InstanceIdentitySessionTokenProvider) SetDialOption(opts *websocket.DialOptions) {
	...
	if opts.HTTPHeader.Get(nicloudsdk.SessionTokenHeader) == "" {
		opts.HTTPHeader.Set(nicloudsdk.SessionTokenHeader, t)
	}
}
...
// FixedSessionTokenProvider wraps the nicloudsdk variant to add a no-op RefreshToken method to satisfy the
...
type FixedSessionTokenProvider struct {
	nicloudsdk.FixedSessionTokenProvider
}
...
func WithFixedToken(token string) SessionTokenSetup {
	return func(_ *nicloudsdk.Client) RefreshableSessionTokenProvider {
		return FixedSessionTokenProvider{FixedSessionTokenProvider: nicloudsdk.FixedSessionTokenProvider{SessionToken: token}}
	}
}
...
type PostLifecycleRequest struct {
	State     nicloudsdk.WorkspaceAgentLifecycle `json:"state"`
...
type PostStartupRequest struct {
	...
	Subsystems        []nicloudsdk.AgentSubsystem `json:"subsystems"`
}

type Log struct {
	CreatedAt time.Time         `json:"created_at"`
	Output    string            `json:"output"`
	Level     nicloudsdk.LogLevel `json:"level"`
}
...
type PatchAppStatus struct {
	...
	State   nicloudsdk.WorkspaceAppStatusState `json:"state"`
...
func (c *Client) PostLogSource(ctx context.Context, req PostLogSourceRequest) (nicloudsdk.WorkspaceAgentLogSource, error) {
	res, err := c.SDK.Request(ctx, http.MethodPost, "/api/v2/workspaceagents/me/log-source", req)
	if err != nil {
		return nicloudsdk.WorkspaceAgentLogSource{}, err
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusCreated {
		return nicloudsdk.WorkspaceAgentLogSource{}, nicloudsdk.ReadBodyAsError(res)
	}
	var logSource nicloudsdk.WorkspaceAgentLogSource
...
func (c *Client) ExternalAuth(ctx context.Context, req ExternalAuthRequest) (ExternalAuthResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return ExternalAuthResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *Client) WaitForReinit(ctx context.Context) (*ReinitializationEvent, error) {
	...
	req.Header[nicloudsdk.SessionTokenHeader] = []string{c.SDK.SessionToken()}
...
func (s *SSEAgentReinitTransmitter) Transmit(ctx context.Context, reinitEvents <-chan ReinitializationEvent) error {
	...
	for {
		...
		case reinitEvent, ok := <-reinitEvents:
			...
			err := sseSendEvent(nicloudsdk.ServerSentEvent{
...
func (s *SSEAgentReinitReceiver) Receive(ctx context.Context) (*ReinitializationEvent, error) {
	nextEvent := nicloudsdk.ServerSentEventReader(ctx, s.r)
	for {
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		...
		case sse.Type == nicloudsdk.ServerSentEventTypeError:
			return nil, xerrors.Errorf("unexpected server sent event type error")
		case sse.Type == nicloudsdk.ServerSentEventTypePing:
			continue
		case sse.Type != nicloudsdk.ServerSentEventTypeData:
			return nil, xerrors.Errorf("unexpected server sent event type: %s", sse.Type)
		}

		// At this point we know that the sent event is of type nicloudsdk.ServerSentEventTypeData
...
func (c *Client) AddChatContext(ctx context.Context, req AddChatContextRequest) (AddChatContextResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return AddChatContextResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
...
func (c *Client) ClearChatContext(ctx context.Context, req ClearChatContextRequest) (ClearChatContextResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return ClearChatContextResponse{}, nicloudsdk.ReadBodyAsError(res)
	}
