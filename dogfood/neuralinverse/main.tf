...
locals {
  ...
  repo_base_dir  = data.ni_parameter.repo_base_dir.value == "~" ? "/home/coder" : replace(data.ni_parameter.repo_base_dir.value, "/^~\\//", "/home/coder/")
  ...
  cache_cleanup_hour   = parseint(substr(data.ni_workspace.me.id, 0, 2), 16) % 24
  cache_cleanup_minute = parseint(substr(data.ni_workspace.me.id, 2, 2), 16) % 60
}
...
data "ni_workspace_preset" "pittsburgh" {
  ...
  parameters = {
    (data.ni_parameter.region.name)                   = "us-pittsburgh"
    (data.ni_parameter.image_type.name)               = data.ni_parameter.image_type.default
    (data.ni_parameter.repo_base_dir.name)            = "~"
    (data.ni_parameter.res_mon_memory_threshold.name) = 80
    (data.ni_parameter.res_mon_volume_threshold.name) = 90
    (data.ni_parameter.res_mon_volume_path.name)      = "/home/coder"
  }
  prebuilds {
    instances = 2
  }
...
data "ni_parameter" "repo_base_dir" {
  type        = "string"
...
locals {
  ...
  user_groups = data.ni_workspace_owner.me.groups
...
data "ni_parameter" "res_mon_memory_threshold" {
  type        = "number"
...
data "ni_parameter" "res_mon_volume_threshold" {
  type        = "number"
...
data "ni_parameter" "res_mon_volume_path" {
  type        = "string"
  name        = "Volume path"
  default     = "/home/coder"
...
data "ni_parameter" "devcontainer_autostart" {
  type        = "bool"
...
data "ni_parameter" "enable_ai_gateway" {
  type        = "bool"
  name        = "Use AI Gateway"
  default     = true
...
provider "docker" {
  host = lookup(local.docker_host, data.ni_parameter.region.value)
}
...
data "ni_workspace_tags" "prebuild" {
...
data "ni_parameter" "vscode_channel" {
  count       = contains(jsondecode(data.ni_parameter.ide_choices.value), "vscode") ? 1 : 0
  type        = "string"
...
module "code-server" {
  count                   = contains(jsondecode(data.ni_parameter.ide_choices.value), "code-server") ? data.ni_workspace.me.start_count : 0
...
module "vscode-web" {
  count                   = contains(jsondecode(data.ni_parameter.ide_choices.value), "vscode-web") ? data.ni_workspace.me.start_count : 0
...
module "jetbrains" {
  count         = contains(jsondecode(data.ni_parameter.ide_choices.value), "jetbrains") ? data.ni_workspace.me.start_count : 0
...
module "cursor" {
  count    = contains(jsondecode(data.ni_parameter.ide_choices.value), "cursor") ? data.ni_workspace.me.start_count : 0
...
module "windsurf" {
  count    = contains(jsondecode(data.ni_parameter.ide_choices.value), "windsurf") ? data.ni_workspace.me.start_count : 0
...
module "zed" {
  count      = contains(jsondecode(data.ni_parameter.ide_choices.value), "zed") ? data.ni_workspace.me.start_count : 0
...
resource "ni_agent" "dev" {
  ...
  env = merge(
    {
      OIDC_TOKEN : data.ni_workspace_owner.me.oidc_access_token,
    ...
    data.ni_parameter.enable_ai_gateway.value ? {
      ANTHROPIC_BASE_URL : "https://dev.coder.com/api/v2/aibridge/anthropic",
      ANTHROPIC_AUTH_TOKEN : data.ni_workspace_owner.me.session_token,
      OPENAI_BASE_URL : "https://dev.coder.com/api/v2/aibridge/openai/v1",
      OPENAI_API_KEY : data.ni_workspace_owner.me.session_token,
    } : {}
  )
  startup_script_behavior = "blocking"

  display_apps {
    vscode          = contains(jsondecode(data.ni_parameter.ide_choices.value), "vscode") && try(data.ni_parameter.vscode_channel[0].value, "stable") == "stable"
    vscode_insiders = contains(jsondecode(data.ni_parameter.ide_choices.value), "vscode") && try(data.ni_parameter.vscode_channel[0].value, "stable") == "insiders"
  }
  ...
  resources_monitoring {
    memory {
      enabled   = true
      threshold = data.ni_parameter.res_mon_memory_threshold.value
    }
    volume {
      enabled   = true
      threshold = data.ni_parameter.res_mon_volume_threshold.value
      path      = data.ni_parameter.res_mon_volume_path.value
    }
    volume {
      enabled   = true
      threshold = data.ni_parameter.res_mon_volume_threshold.value
...
resource "coder_script" "go-cache-cleanup-cron" {
  ...
  icon         = "${data.ni_workspace.me.access_url}/emojis/1f9f9.png" // 🧹
...
resource "coder_devcontainer" "coder" {
  count            = data.ni_parameter.devcontainer_autostart.value ? data.ni_workspace.me.start_count : 0
  agent_id         = ni_agent.dev.id
...
resource "docker_volume" "homebrew_volume" {
  name = "coder-${data.ni_workspace.me.id}-homebrew"
  # Protect the volume from being deleted due to changes in attributes.
  lifecycle {
    ignore_changes = all
  }
...
data "docker_registry_image" "dogfood" {
  name = local.image_tags[data.ni_parameter.image_type.value]
}

resource "docker_image" "dogfood" {
  name = "${local.image_tags[data.ni_parameter.image_type.value]}@${data.docker_registry_image.dogfood.sha256_digest}"
...
resource "docker_container" "workspace" {
  ...
  entrypoint = ["sh", "-c", ni_agent.dev.init_script]
  # CPU limits are unnecessary since Docker will load balance automatically
  memory  = data.ni_workspace_owner.me.name == "code-asher" ? 65536 : 32768
  runtime = "sysbox-runc"
  ...
  env = [
    "CODER_AGENT_TOKEN=${ni_agent.dev.token}",
...
resource "coder_metadata" "container_info" {
  ...
  item {
    key   = "region"
    value = data.ni_parameter.region.option[index(data.ni_parameter.region.option.*.value, data.ni_parameter.region.value)].name
  }
}
...
module "claude-code" {
  ...
  version           = "5.2.0"
  enable_ai_gateway = data.ni_parameter.enable_ai_gateway.value
  anthropic_api_key = data.ni_parameter.enable_ai_gateway.value ? "" : var.anthropic_api_key
  agent_id          = ni_agent.dev.id
...
module "codex" {
  ...
  workdir           = local.repo_dir
  enable_ai_gateway = data.ni_parameter.enable_ai_gateway.value
  openai_api_key    = data.ni_parameter.enable_ai_gateway.value ? "" : var.openai_api_key
  mcp               = <<-EOT
