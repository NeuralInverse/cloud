...
data "ni_parameter" "use_ai_bridge" {
  type        = "bool"
  name        = "Use AI Bridge"
  default     = true
...
resource "ni_agent" "dev" {
  ...
  env = merge(
    ...
    data.ni_parameter.use_ai_bridge.value ? {
...
module "claude-code" {
  ...
  version           = "5.2.0"
  enable_ai_gateway = data.ni_parameter.use_ai_bridge.value
  anthropic_api_key = data.ni_parameter.use_ai_bridge.value ? "" : var.anthropic_api_key
  agent_id          = ni_agent.dev.id
