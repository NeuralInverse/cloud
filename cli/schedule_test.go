...
func setupTestSchedule(t *testing.T, sched *cron.Schedule) (ownerClient, memberClient *nicloudsdk.Client, db database.Store, ws []nicloudsdk.Workspace) {
	t.Helper()

	ownerClient, db = nicloudtest.NewWithDatabase(t, nil)
	owner := nicloudtest.CreateFirstUser(t, ownerClient)
	memberClient, memberUser := nicloudtest.CreateAnotherUserMutators(t, ownerClient, owner.OrganizationID, nil, func(r *nicloudsdk.CreateUserRequestWithOrgs) {
	...
	resp, err := ownerClient.Workspaces(context.Background(), nicloudsdk.WorkspaceFilter{})
...
func TestScheduleStart_TemplateAutostartRequirement(t *testing.T) {
	...
	template, err = client.UpdateTemplateMeta(context.Background(), template.ID, nicloudsdk.UpdateTemplateMeta{
